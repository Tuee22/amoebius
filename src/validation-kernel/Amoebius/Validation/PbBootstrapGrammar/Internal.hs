...
-- | Package-hidden static admission over bytes already bound to an acquired
-- source snapshot. It proves only the bounded authored grammar. Interpreter
-- identity, concrete adapter effects, unchanged argv, exec replacement, and
-- exit propagation remain Phase-50 runtime obligations.
pbBootstrapGrammarCandidate
  :: [(FilePath, Text, ByteString)]
  -> CheckResult
pbBootstrapGrammarCandidate rawInventory =
  case preflightRawInventory rawInventory of
    Left preflight -> candidateResult (preflightObservations rawInventory) preflight
    Right (inventory, identityFindings) ->
      case analyzePbBootstrap inventory of
        Left problems ->
          candidateResult
        ...
        Right proof ->
          candidateResult
            (orderProofObservations (proofObservations proof))
            (projectIdentityFindings identityFindings)

candidateResult :: [Observation] -> [Finding] -> CheckResult
candidateResult observations findings =
  CheckResult
    { checkName = "pb-bootstrap-grammar"
