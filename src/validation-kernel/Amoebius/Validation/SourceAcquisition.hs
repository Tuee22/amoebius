...
-- This module is intentionally diagnostic-only.  It specifies and checks the
-- bytes an external source-acquisition authority would have to provide, but it
-- neither owns an external trust anchor nor constructs AcquiredSourceSnapshot.
-- A caller-supplied public key and replay set can exercise the cryptographic
-- and canonical-format boundary; they cannot establish candidate custody.
