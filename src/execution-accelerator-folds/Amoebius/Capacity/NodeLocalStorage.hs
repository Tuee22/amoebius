...
provisionNodeImageStorage catalog demand = mutateImageResult original
 where
  original
...
fitLayoutComponents layout components = mutateLayoutResult $ do
...
validateFilesystemLayoutObservation declared observed = mutateObservationResult $ do
...
provisionPhysicalDiskPartition partition = mutatePartitionResult $ do
