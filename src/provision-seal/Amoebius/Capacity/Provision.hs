...
validateInfrastructurePlan ledger observedSnapshot plan
  | planReplayDetected = Left (InfrastructurePlanReplay (infrastructurePlanToken plan))
  | actionReplayDetected = Left (InfrastructureActionReplay (providerActionToken batch))
...
provision context topology deployment = do
  ...
  executionCandidate <- mapExecution (provisionExecutionEpochs (priorExecutionCatalog (contextPriorCatalog context)) available (BoundExecutionInventory (boundDeploymentTransition deployment) units))
  let execution = mutateExecutionCandidate executionCandidate
