...
-- | Stable application-facing projection.  Binding choices never enter these
...
renderCapabilityArmDhallType :: Text
renderCapabilityArmDhallType =
  "< ObjectStore | SecretStore | MessageBus | Sql | Identity | Observability | Registry | Edge | InferenceEngine >"

renderEngineRuntimeDhallType :: Text
renderEngineRuntimeDhallType =
  "< AppleMetal : Text | Cuda : Text | LinuxCpu : Text >"

renderAppCapabilityNeedDhallType :: Text
renderAppCapabilityNeedDhallType =
  "{ capability : " <> renderCapabilityArmDhallType <> ", name : Text }"
