...
main = do
  ...
  root <- projectRoot
  pureChecks root
  reuse <- lookupEnv "PHASE35_REUSE_FRESH_LIVE"
  if reuse == Just "1"
...
pureChecks root = do
  ...
  goldenFrame <- Text.strip <$> TextIO.readFile (root </> "amoebius-pulsar/test/golden/connect_frame.hex")
  assertEqual "spec-derived CONNECT frame" goldenFrame (toHex frame)
  let routes = gateRoutes
      expectedTopics =
        [ "persistent://phase35-tenant/phase35-namespace/round-trip.command.linux-cpu"
        , "persistent://phase35-tenant/phase35-namespace/round-trip.event.linux-cpu"
        ]
      actualTopics = [renderTopic (topicFor "phase35-tenant" "phase35-namespace" route LinuxCpu) | route <- routes]
  ...
  expectedApi <- filter (not . Text.null) . Text.lines <$> TextIO.readFile (root </> "amoebius-pulsar/test/golden/api_surface.txt")
...
runLive root = do
  setup <- runSetup root
  let resultPath = "/tmp/amoebius-phase35-result-" <> Text.unpack (challenge setup) <> ".json"
      cleanup = callHelper root ["cleanup", "--state", stateFile setup]
  (do
      results <- forM (namespaces setup) $ \namespace -> do
        brokerPod <- maybe (fail ("phase35-owner-missing:" <> Text.unpack namespace)) pure (Map.lookup namespace (brokerPods setup))
...
runRound setup namespace = withNativeClient (Broker "127.0.0.1" "16650") $ \client -> do
  putStrLn ("phase35-live-stage:" <> Text.unpack namespace <> ":connected")
  (commandRoute, eventRoute) <- case gateRoutes of
    [commandValue, eventValue] -> pure (commandValue, eventValue)
    _ -> fail "phase35-gate-route-cardinality"
  ...
  putStrLn ("phase35-live-stage:" <> Text.unpack namespace <> ":resources-open")
  ...
  putStrLn ("phase35-live-stage:" <> Text.unpack namespace <> ":duplicate-sent")
  receivedCommand <- receiveWithin commandConsumer
  putStrLn ("phase35-live-stage:" <> Text.unpack namespace <> ":command-received")
  ...
  putStrLn ("phase35-live-stage:" <> Text.unpack namespace <> ":event-received")
  ...
  putStrLn ("phase35-live-stage:" <> Text.unpack namespace <> ":seek-received")
  ...
  putStrLn ("phase35-live-stage:" <> Text.unpack namespace <> ":redelivery-first")
  ...
  putStrLn ("phase35-live-stage:" <> Text.unpack namespace <> ":redelivery-second")
  ...
  putStrLn ("phase35-live-stage:" <> Text.unpack namespace <> ":round-complete")
  pure
    RoundResult
...
timed label action = do
  ...
  maybe (fail ("phase35-live-timeout:" <> label)) pure outcome
...
runSetup root = do
  (exitCode, output, errors) <- readProcessWithExitCode "/usr/bin/python3" [root </> "tools/phase35_pulsar_live.py", "setup"] ""
  unless (exitCode == ExitSuccess) (fail ("phase35-live-setup:" <> output <> errors))
  case eitherDecodeStrict' (Text.encodeUtf8 (Text.strip (Text.pack output))) of
    Left problem -> fail ("phase35-live-setup-json:" <> problem <> ":" <> output)
    Right setup -> pure setup

callHelper :: FilePath -> [String] -> IO ()
callHelper root arguments = do
  (exitCode, output, errors) <- readProcessWithExitCode "/usr/bin/python3" ((root </> "tools/phase35_pulsar_live.py") : arguments) ""
  unless (exitCode == ExitSuccess) (fail ("phase35-live-helper:" <> show arguments <> ":" <> output <> errors))
...
validateFreshEvidence root = do
  ...
  unless present (fail "phase35-live-evidence-missing")
  value <- eitherDecode <$> Lazy.readFile evidence
  case value of
    Left problem -> fail ("phase35-live-evidence-json:" <> problem)
    ...
    Right _ -> fail "phase35-live-evidence-not-object"
...
assertJsonBool evidenceObject key expected = case lookupKey key evidenceObject of
  ...
  _ -> fail ("phase35-evidence-field-missing:" <> show key)
  where
