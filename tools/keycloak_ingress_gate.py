...
MUTANTS = (
    ("M-drop-oidc", "keycloak-ingress-drop-oidc-mutant", "oidc-guard-required"),
    ("M-netpol-swap", "keycloak-ingress-netpol-swap-mutant", "network policy independent oracle"),
    ("M-delete-noop", "keycloak-ingress-delete-noop-mutant", "cluster-recreate-identity-unchanged"),
    ("M-drop-origin", "keycloak-ingress-drop-origin-mutant", "websocket-exact-origin-required"),
    ("M-nonce-replay", "keycloak-ingress-nonce-replay-mutant", "websocket-single-use-nonce-required"),
    ("M-direct-backend", "keycloak-ingress-direct-backend-mutant", "direct-websocket-backend-forbidden"),
)
...
def reject_mutant(name: str, flag: str, marker: str) -> dict[str, str]:
    arguments = (
        CABAL, "test", "keycloak-ingress-edge-spec", f"-f{flag}",
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 32,
        "gate_command": "python3 tools/keycloak_ingress_gate.py",
        "register": "3",
        "substrate": "linux-cpu",
        "date": "2026-08-10",
        "layers": [
...
def evidence_domain(*, require_fresh: bool) -> dict[str, Any]:
    if not LIVE_EVIDENCE.is_file() or not REBIND_EVIDENCE.is_file():
        raise GateFailure("keycloak-ingress-live-evidence-absent")
    if require_fresh and time.time() - LIVE_EVIDENCE.stat().st_mtime > 7200:
        raise GateFailure("keycloak-ingress-live-evidence-stale")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows = [
            invoke(
                "pure-edge-contract",
                (CABAL, "test", "keycloak-ingress-edge-spec", "--test-show-details=direct", "-j1"),
            ),
        ]
        if arguments.seal_existing_live:
            rows.append({
                "name": "keycloak-ingress-live",
                "command": "sealed just-produced tools/keycloak_ingress_live.py receipt",
            ...
            rows.append(invoke(
                ...
                (python, "tools/keycloak_ingress_live.py"), timeout=3600,
            ))
        evidence_domain(require_fresh=arguments.seal_existing_live)
        rows.append(invoke(
            "external-live-reader",
            (CABAL, "test", "keycloak-ingress-keycloak-ingress-live", "--test-show-details=direct", "-j1"),
        ))
        rows.extend(reject_mutant(*mutant) for mutant in MUTANTS)
        disabled = tuple(f"-f-{flag}" for _, flag, _ in MUTANTS)
        rows.append(invoke(
            "baseline-restored",
            (CABAL, "test", "keycloak-ingress-edge-spec", *disabled, "--test-show-details=direct", "-j1"),
        ))
        ...
        print(
            f"keycloak-ingress-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; "
        ...
        print(f"keycloak-ingress-gate: FAIL: {problem}", file=sys.stderr)
        return 1
