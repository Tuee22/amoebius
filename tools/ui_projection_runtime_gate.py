...
MUTANTS = (
    ("M-drop-owner-key", "ui-projection-runtime-drop-owner-key-mutant", "ui-projection-runtime-drop-owner-key:"),
    ("M-drop-owner-subscription", "ui-projection-runtime-drop-owner-subscription-mutant", "ui-projection-runtime-drop-owner-subscription:"),
    ("M-drop-receipt-command-id", "ui-projection-runtime-drop-receipt-command-id-mutant", "ui-projection-runtime-drop-receipt-command-id:"),
)
...
def reject_mutant(name: str, flag: str, marker: str) -> dict[str, str]:
    ...
    environment = os.environ.copy()
    environment["UI_PROJECTION_RUNTIME_PURE_ONLY"] = "1"
    result = subprocess.run(
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 38,
        "gate_command": "python3 tools/ui_projection_runtime_gate.py --reuse-fresh-live",
        "register": "3",
        "substrate": "linux-cpu",
        "date": "2026-08-11",
        "layers": [
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows.append(invoke("sealed-live-reader", (
            CABAL, "test", "ui-projection-runtime-live", *disabled,
            "--test-show-details=direct", "-j1", "-v0",
        ), extra_env={"UI_PROJECTION_RUNTIME_REUSE_FRESH_LIVE": "1"}))
        ...
        print(f"ui-projection-runtime-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']})")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"ui-projection-runtime-gate: FAIL: {error}", file=sys.stderr)
        return 1
