...
def apply(value: dict[str, Any]) -> None:
    kubectl(
        "apply", "--server-side", "--field-manager=phase38-harness", "--force-conflicts",
...
def pod(namespace: str, name: str, role: str) -> dict[str, Any]:
    return {
        ...
        "metadata": {"name": name, "namespace": namespace, "labels": {"app": name, "amoebius.io/phase38-role": role}},
...
def setup_network(suffix: str) -> dict[str, Any]:
    ...
    apply({
        ...
        "spec": {
            "podSelector": {"matchLabels": {"amoebius.io/phase38-role": "worker"}},
            "policyTypes": ["Egress"],
            "egress": [{
...
def cleanup_stale() -> None:
    for path in Path("/tmp").glob("amoebius-phase38-state-*.json"):
        try:
...
def setup() -> dict[str, Any]:
    ...
    namespace = "ui"
    realm = "phase38-" + suffix
    state_path = Path(f"/tmp/amoebius-phase38-state-{suffix}.json")
    cleanup_stale()
    ...
    state = {
        ...
        "network": network,
        "fixtureDigests": {
            name: file_digest(ROOT / "test/fixtures/phase_38" / name)
            for name in (
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "setup":
            ...
            print("phase38-live-finish: PASS")
        else:
            cleanup(args.state)
            print("phase38-live-cleanup: PASS")
        return 0
    except (LiveFailure, OSError, ValueError, KeyError, json.JSONDecodeError) as error:
        print(f"phase38-live: FAIL: {error}", file=os.sys.stderr)
        return 1
