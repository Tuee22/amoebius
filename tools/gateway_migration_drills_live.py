...
MULTICLUSTER_SPAWN_GEOREPL_RECEIPT = ROOT / "DEVELOPMENT_PLAN/evidence/phase_42/phase-receipt.json"
LIVE_ROOT = Path("/var/tmp/amoebius-gateway-migration-drills-live")
JOURNAL_ROOT = Path("/var/tmp/amoebius-gateway-migration-drills-journal")
...
BUCKET = "gateway-migration-drills-gateway-migration"
...
def create_cluster(name: str) -> None:
    ...
    kubectl(name, "create", "namespace", "gateway-migration-drills-system")
...
def set_authority(cluster: str, branch: str, owner: str) -> None:
    value = {
        ...
        "metadata": {"name": f"gateway-authority-{branch}", "namespace": "gateway-migration-drills-system"},
...
def observe_authority(cluster: str, branch: str) -> str:
    result = kubectl(cluster, "-n", "gateway-migration-drills-system", "get", "configmap", f"gateway-authority-{branch}", "-o", "json")
...
def phase43_live() -> dict[str, Any]:
    ...
    phase42 = json.loads(MULTICLUSTER_SPAWN_GEOREPL_RECEIPT.read_text(encoding="utf-8"))
    if phase42.get("result") != "PASS" or phase42.get("secondPassMutations") != 0:
        raise Phase43Failure("multicluster-spawn-georepl-prerequisite-receipt")
    try:
...
def main() -> int:
    ...
    print("gateway-migration-drills-gateway-migration-live: PASS")
    print(f"gateway-migration-drills-gateway-migration-cleanup: PASS ({evidence['evidenceDigest']})")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"gateway-migration-drills-gateway-migration-live: FAIL: {error}", file=sys.stderr, flush=True)
        raise SystemExit(1)
