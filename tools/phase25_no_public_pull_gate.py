...
EXPECTED_FAILURE = ROOT / "test/fixtures/phase25/expected_pull_failure.txt"
...
def load_standup() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("phase25_registry_standup_gate", path)
...
def kubectl(*arguments: str, stdin: str | None = None, check: bool = True) -> subprocess.CompletedProcess[str]:
    return run(
        ("/usr/bin/kubectl", "--kubeconfig", str(KUBECONFIG), *arguments),
        stdin=stdin,
        check=check,
        timeout=180,
    )
...
def apply(objects: list[dict[str, Any]]) -> None:
    ...
    kubectl("apply", "--server-side", "--field-manager=amoebius-phase25-gate", "-f", "-", stdin=json.dumps(payload))
...
def configure_in_cluster_registry() -> dict[str, Any]:
    ...
    if "config_path = \"/etc/containerd/certs.d\"" not in config:
        docker_exec(
            ...
            "/etc/containerd/config.toml.amoebius-phase25-before",
        )
