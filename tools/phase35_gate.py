...
UNVERIFIED = {
    "phase37-workflow-content-store",
    "cross-cluster-pulsar-correspondence",
    "broker-consensus-internals",
}
MUTANTS = (
    ("M-topic-literal", "phase35-topic-literal-mutant", "independent derived-topic oracle"),
    ("M-drop-one-sided", "phase35-drop-one-sided-mutant", "one-sided-link-not-rejected"),
    ("M-produce-raw", "phase35-produce-raw-mutant", "CBOR-only API surface"),
)
...
def reject_mutant(name: str, flag: str, marker: str) -> dict[str, str]:
    ...
    environment = os.environ.copy()
    environment["PHASE35_REUSE_FRESH_LIVE"] = "1"
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 35,
        "gate_command": "python3 tools/phase35_gate.py --reuse-fresh-live",
...
def compile_refusal(name: str, fixture: str, marker: str) -> dict[str, str]:
    script = f":load test/compile_fail/{fixture}\n:quit\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows.append(invoke("sealed-live-reader", (CABAL, "test", "pulsar-client-live", *disabled, "--test-show-details=direct", "-j1"), extra_env={"PHASE35_REUSE_FRESH_LIVE": "1"}))
        ...
        rows.append(invoke("baseline-restored", (CABAL, "test", "pulsar-client-live", "pulsar-dedup-sim", *disabled, "--test-show-details=direct", "-j1"), extra_env={"PHASE35_REUSE_FRESH_LIVE": "1"}))
        ...
        print(f"phase35-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']})")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, StopIteration, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"phase35-gate: FAIL: {error}", file=sys.stderr)
        return 1
