...
KIND_CONFIG = Path()
IMAGE_ARCHIVE = Path()
# The digest Phase 31 published and the export it published from, both caller-supplied:
...
AUDIT_POLICY = ROOT / "test/fixture/retained_storage/audit_policy.yaml"
ROWS = ROOT / "test/oracle/retained_storage/claimref_table.csv"
POSTGRES_INVENTORY = ROOT / "test/fixture/base_image_registry/bake_inventory_expected.dhall"
POSTGRES_BIN_DIR = ""
POSTGRES_MAJOR = ""
RUN_ROOT = Path()
# Later phases may reuse this proven harness against an isolated cluster while
# pinning their own committed marker bytes.  Phase 33 leaves both unset and
...
def postgres_inventory() -> dict[str, str]:
    """Read the PostgreSQL executable path from Phase 31's independent inventory."""
...
def materialize_kind_config() -> Path:
    ...
    replacements = {
        "__AMOEBIUS_RETAINED_ROOT__": str(RETAINED_ROOT),
        "__AMOEBIUS_AUDIT_ROOT__": str(AUDIT_ROOT),
        "__AMOEBIUS_AUDIT_POLICY__": str(AUDIT_POLICY),
    }
    expected_counts = {
        "__AMOEBIUS_RETAINED_ROOT__": 2,
        "__AMOEBIUS_AUDIT_ROOT__": 1,
        "__AMOEBIUS_AUDIT_POLICY__": 1,
    }
    for marker, expected in expected_counts.items():
        if template.count(marker) != expected:
            raise RebindFailure(f"kind-config-placeholder:{marker}:{template.count(marker)}:{expected}")
        template = template.replace(marker, replacements[marker])
    if "__AMOEBIUS_" in template:
        raise RebindFailure("kind-config-unresolved-placeholder")
...
def create_cluster() -> dict[str, Any]:
    ...
    run(("/usr/bin/docker", "update", "--cpus", "2", "--memory", "4g", "--memory-swap", "4g", NODE), timeout=300)
    envelope = run(("/usr/bin/docker", "inspect", "--format", "{{.HostConfig.NanoCpus}} {{.HostConfig.Memory}} {{.HostConfig.MemorySwap}}", NODE)).stdout.strip()
    if envelope != "2000000000 4294967296 4294967296":
        raise RebindFailure(f"node-envelope:{envelope}")
...
def prepare_cluster() -> dict[str, Any]:
    ...
    return {
        "schema": "amoebius.retained-storage.cluster-preflight.v1",
        "register": 3,
        "substrate": "linux-cpu",
        "cluster": CLUSTER,
...
def statefulset(row: dict[str, str]) -> dict[str, Any]:
    ...
    if name == "pg-witness":
        container = {
            ...
            "command": ["/bin/sh", "-c"],
            "args": [f"if test ! -f /volume/data/PG_VERSION; then {POSTGRES_BIN_DIR}/initdb -D /volume/data --username=testuser --auth=trust; fi; exec {POSTGRES_BIN_DIR}/postgres -D /volume/data -c listen_addresses='*' -c unix_socket_directories=/tmp -c fsync=on"],
...
def apply_workloads(rows: dict[str, dict[str, str]]) -> dict[str, Any]:
    ...
    for name in rows:
        kubectl("-n", NAMESPACE, "wait", "--for=create", f"pod/{name}-0", "--timeout=300s")
...
def pg_sql(sql: str) -> str:
    return kubectl("-n", NAMESPACE, "exec", "pg-witness-0", "--", f"{POSTGRES_BIN_DIR}/psql", "-h", "127.0.0.1", "-U", "testuser", "-d", "postgres", "-Atqc", sql).stdout.strip()
...
def s3_request(method: str, path: str, body: bytes = b"") -> tuple[int, bytes]:
    ...
    key_date = hmac.new(("AWS4" + MINIO_PASSWORD).encode(), date_stamp.encode(), hashlib.sha256).digest()
    ...
    authorization = f"AWS4-HMAC-SHA256 Credential={MINIO_USER}/{scope}, SignedHeaders={signed_headers}, Signature={signature}"
...
def execute(*, prepared_cluster: bool = False) -> dict[str, Any]:
    ...
    nonce = MARKER_TEXT if MARKER_TEXT is not None else "retained-storage-" + secrets.token_hex(24)
    ...
    return {
        "schema": "amoebius.retained-storage.rebind-live.v1", "register": 3, "substrate": "linux-cpu",
        ...
        "controlPlaneNoPvc": "UNVERIFIED (Phase 38 subject absent)",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--artifact", type=Path, required=True, help="the Phase-31 OCI export to import")
    ...
    try:
        ...
        if arguments.prepare_cluster_only:
            print("retained-storage-rebind-live: PASS (predecessor cluster ready with imported Phase-31 image)")
        else:
        ...
        print(f"retained-storage-rebind-live: FAIL: {problem}", file=sys.stderr)
        return 1
