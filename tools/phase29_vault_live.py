#!/usr/bin/env python3
"""Run the Phase-29 root Vault, PKI, direct-client, and retained-rebuild proof."""
...
AUDIT_CAPACITY = "64Mi"
EVIDENCE = ROOT / "DEVELOPMENT_PLAN/evidence/phase_29/vault-live.json"
CANARY_FIXTURE = ROOT / "test/golden/vault/canary.json"
UNLOCK_ENVELOPE = RETAINED_ROOT / "phase29-unlock.age"
LOCAL_PORT = 18290
...
def install_current_binary() -> dict[str, Any]:
    run((CABAL, "build", "exe:amoebius"), timeout=1200)
    binary = Path(text(run((CABAL, "list-bin", "exe:amoebius"))).strip())
    destination = RETAINED_ROOT / "mounts/phase29-bin/amoebius"
...
def create_cluster() -> dict[str, Any]:
    ...
    if PRIVATE_IMAGE not in images and imported:
    ...
    return {"archive": str(IMAGE_ARCHIVE), "digest": IMAGE_DIGEST, "selectedPlatform": "linux/amd64", "imagePullPolicy": "Never", "publicPulls": 0}
        ...
        {"apiVersion": "v1", "kind": "ServiceAccount", "metadata": {"name": "vault-reader", "namespace": CONSUMER_NAMESPACE}},
        {
            ...
            "spec": {
                ...
                "volumes": [
                    {
                        "name": "amoebius-live",
                        "hostPath": {"path": "/amoebius-retained/phase29-bin/amoebius", "type": "File"},
                    }
                ],
            },
        },
...
def verify_certificates(root_certificate: str, issuing_ca: str, leaf_certificate: str) -> dict[str, str]:
    temporary = RETAINED_ROOT / "phase29-cert-verify"
...
def execute() -> dict[str, Any]:
    ...
    if development_password is not None and operator_password is not None:
        raise VaultLiveFailure("multiple-operator-password-sources")
    if operator_password is not None:
        password = operator_password.encode()
    elif development_password is not None:
        password = development_password.encode()
    else:
        password = secrets.token_urlsafe(32).encode()

    if os.environ.get("PHASE29_RESUME_CLEAN_RUN1") == "1":
        images = text(run(("/usr/bin/docker", "exec", NODE, "ctr", "-n", "k8s.io", "images", "list", "-q"))).splitlines()
        imported = next((name for name in images if name.startswith("import-") and name.endswith("@" + IMAGE_DIGEST)), None)
        if not imported:
            raise VaultLiveFailure("resume-run1-import-absent")
        for target in (PRIVATE_IMAGE, "docker.io/library/" + imported):
            if target not in images:
                run(("/usr/bin/docker", "exec", NODE, "ctr", "-n", "k8s.io", "images", "tag", imported, target))
        kubectl("-n", NAMESPACE, "delete", "statefulset", "root-vault", "--ignore-not-found=true", "--cascade=foreground", "--wait=true", check=False)
        source1 = {"archive": str(IMAGE_ARCHIVE), "digest": IMAGE_DIGEST, "imagePullPolicy": "Never", "publicPulls": 0, "resumedCleanRun1": True}
    else:
        delete_cluster()
        source1 = create_cluster()
    identity1 = cluster_identity()
    prebind1 = apply_vault()
    if os.environ.get("PHASE29_RESUME_INITIALIZED_RUN1") == "1":
        before_init = vault_status()
    ...
    return {
        "schema": "amoebius.phase29.vault-live.v1",
        "register": 3,
        "substrate": "linux-cpu",
        "initOnce": {
        ...
        "unlockEnvelope": {
            ...
            "developmentPasswordUsed": development_password is not None,
            "operatorPasswordSupplied": operator_password is not None,
        },
        "storage": {
        ...
        "artifactSource": {"run1": source1, "run2": source2},
        "deferred": {
...
def main() -> int:
    try:
        ...
        print("phase29-vault-live: PASS (init once, retained unseal, PKI chain, Kubernetes-auth Haskell client)")
        return 0
        ...
        print(f"phase29-vault-live: FAIL: {problem}", file=sys.stderr)
        return 1
