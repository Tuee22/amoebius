...
ORACLE = ROOT / "test/oracle/capacity_topology/compile_fail.tsv"
...
def main() -> int:
    ...
    if len(rows) != 7:
        print("capacity-topology-compile-fail: expected seven oracle rows", file=sys.stderr)
        return 1
    for row in rows:
        legal = compile_fixture(ghc, row["legal"])
        if legal.returncode != 0:
            print(f"capacity-topology-compile-fail: legal fixture failed: {row['legal']}\n{legal.stdout}", file=sys.stderr)
            return 1
        illegal = compile_fixture(ghc, row["illegal"])
        if illegal.returncode == 0:
            print(f"capacity-topology-compile-fail: illegal fixture compiled: {row['illegal']}", file=sys.stderr)
            return 1
        expected = (ROOT / row["expected"]).read_text(encoding="utf-8").strip()
        if expected not in illegal.stdout:
            print(f"capacity-topology-compile-fail: wrong diagnostic for {row['illegal']}\n{illegal.stdout}", file=sys.stderr)
            return 1
        if any(token in illegal.stdout for token in ("Variable not in scope", "parse error", "Could not find module")):
            print(f"capacity-topology-compile-fail: non-type diagnostic for {row['illegal']}\n{illegal.stdout}", file=sys.stderr)
            return 1
    print("capacity-topology-compile-fail: PASS (7 legal/illegal minimal pairs)")
    return 0
