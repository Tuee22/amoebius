...
MUTANTS = (
    ("M-accept-stale-authority", "phase40-accept-stale-authority-digest-mutant", "phase40-accept-stale-authority-digest:"),
    ("M-publish-mixed-plan-pair", "phase40-publish-mixed-plan-pair-mutant", "phase40-publish-mixed-plan-pair:"),
    ("M-rebuild-runtime-per-program", "phase40-rebuild-runtime-per-program-mutant", "phase40-rebuild-runtime-per-program:"),
)
...
def reject_mutant(name: str, flag: str, marker: str) -> dict[str, str]:
    ...
    environment = os.environ.copy()
    environment["PHASE40_PURE_ONLY"] = "1"
    result = subprocess.run(
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 40,
        "gate_command": "python3 tools/phase40_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows.append(invoke("sealed-live-reader", (
            CABAL, "test", "ui-program-release-live-gate", *disabled, "--test-show-details=direct", "-j1", "-v0",
        ), extra_env={"PHASE40_REUSE_FRESH_LIVE": "1"}))
        ...
        print(f"phase40-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']})")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"phase40-gate: FAIL: {error}", file=sys.stderr)
        return 1
