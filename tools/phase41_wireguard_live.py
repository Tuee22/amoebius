...
def build_binary() -> Path:
    flags = [
        "-f-phase41-missing-peer-key-mutant",
        "-f-phase41-hub-no-endpoint-mutant",
        "-f-phase41-drop-resource-envelope-mutant",
        "-f-phase41-early-listener-replacement-mutant",
    ]
...
def vault_key_custody(binary: Path, suffix: str, temporary: Path) -> Iterator[dict[str, bytes]]:
    ...
    service_account = "phase41-wireguard-reader"
...
def start_listener(namespace: str, address: str, cgroup: Path, log_path: Path) -> subprocess.Popen[bytes]:
    ...
    command = (
        ...
        "phase41-listener", SERVER_CODE, address, "19410",
    )
...
def execute() -> dict[str, Any]:
    ...
    hub = f"a41h-{suffix}"
    spoke = f"a41s-{suffix}"
    cgroups = [Path("/sys/fs/cgroup") / f"amoebius-phase41-{suffix}-{role}" for role in ("hub", "spoke")]
    processes: list[subprocess.Popen[bytes]] = []
    canary = ("phase41-" + secrets.token_urlsafe(30)).encode()
    demand_bytes = (ROOT / "test/fixtures/phase41/expected-fabric-demand.json").read_bytes()
    ...
    binary = build_binary()
    temporary_path = Path(tempfile.mkdtemp(prefix=f"amoebius-phase41-{suffix}-", dir="/var/tmp"))
    ...
    try:
        with vault_key_custody(binary, suffix, temporary_path) as keys:
            ...
            # The singleton's discover/diff pass re-reads the kernel and emits no
...
def main() -> int:
    try:
        ...
        print("phase41-wireguard-live: PASS")
        print("phase41-wireguard-cleanup: PASS")
        return 0
    except Exception as failure:
        print(f"phase41-wireguard-live: FAIL ({failure})", file=sys.stderr)
        return 1
