#!/usr/bin/env python3
"""The Phase-3 gate — the gateway-migration model, both branches.
...
EMITTED = ROOT / "gen" / "tla" / "gateway-migration-model-spec"
RESULTS = EMITTED / "phase-results.tsv"
CONTRACT = "DEVELOPMENT_PLAN/phase_03_gateway_migration_model.md"
GATE_COMMAND = "python3 tools/phase3_gate.py"

CHECKS = {
    "emitted-spec-untracked": "the emitted .tla/.cfg live under gen/ and are not in the snapshot",
...
# The authored oracle, read off the Phase-3 contract's Gate paragraph.
EXPECTED_RESULTS = {
...
def toolchain_side() -> tuple[bool, dict[str, Any]]:
    ...
    try:
        ...
        return False, {}
    for name in ("cabal", "java", "tla2tools"):
...
def main() -> int:
    gate = gate_common.PhaseGate(
        phase=3, contract=CONTRACT, command=GATE_COMMAND, register="1", substrate="none", sides=SIDES
    )
    gate.begin()
