#!/usr/bin/env python3
"""Run and seal the Phase-24 bootstrap-coordinator and single-node kind gate.
...
`gen/runs/`, the surface enumeration is joined two-way to an authored expectation, and the
result is bound to a source-snapshot digest and externally attested.
"""
...
MUTANT_FIXTURES = ROOT / "test/host/mutants"
RESULTS = ROOT / "gen/dsl/phase24/phase-results.tsv"
CONTRACT = "DEVELOPMENT_PLAN/phase_24_bootstrap_coordinator_kind.md"
...
def verify_oracles() -> None:
    actual = {path.name for path in MUTANT_FIXTURES.glob("phase_24_M-*.mutant")}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    gate = gate_common.PhaseGate(
        phase=24, contract=CONTRACT, command=GATE_COMMAND, register="3", substrate="linux-cpu", sides=SIDES
    )
    gate.begin()
    ...
    try:
        ...
        run((cabal, f"--with-compiler={COMPILER}", "test", "phase24-host-spec", "--test-show-details=direct"))
        print("  ok    host-spec")
        results["static"] = True
        ...
        if arguments.execute:
            ...
            run((
                sys.executable, str(ROOT / "tools/phase24_pristine_gate.py"),
                "--execute", "--provider", "incus", "--evidence", str(evidence),
            ))
            print(f"  ok    pristine gate passed; evidence in {gate_common.rel(evidence)}")
        elif evidence is None:
            raise GateFailure("phase-24 needs --execute or an --evidence directory from a completed live run")
        else:
