...
LIVE_ROOT = Path("/var/tmp/amoebius-multicluster-spawn-georepl-live")
...
PARENT_CONFIG = ROOT / "test/fixture/multicluster_spawn_georepl/kind-parent.yaml"
PHASE24_KUBECONFIG = Path.home() / ".amoebius/phase24/kubeconfig"
MINIO_BUCKET = "multicluster-spawn-georepl-pulumi"
...
def remove_live_root() -> None:
    if LIVE_ROOT != Path("/var/tmp/amoebius-multicluster-spawn-georepl-live"):
...
def executor_job(name: str, child: str, mode: str, passphrase: str) -> dict[str, Any]:
    stack = "multicluster-spawn-georepl-" + child.removeprefix("amoebius-p42-")
    if mode == "up":
        operation = f"""
...
echo multicluster-spawn-georepl-pulumi-up-complete:{child}
"""
    elif mode == "destroy":
        operation = f"""
...
echo multicluster-spawn-georepl-pulumi-destroy-complete:{child}
"""
    else:
...
"""
    return {
        "apiVersion": "batch/v1",
        "kind": "Job",
        "metadata": {"name": name, "namespace": "multicluster-spawn-georepl-system", "labels": {"amoebius.io/phase": "42", "amoebius.io/child": child, "amoebius.io/mode": mode}},
        "spec": {
            "backoffLimit": 0,
                ...
                "spec": {
                    ...
                    "containers": [{
                        ...
                        "volumeMounts": [
                            ...
                            {"name": "multicluster-spawn-georepl-bin", "mountPath": "/phase42/bin", "readOnly": True},
                    ...
                    "volumes": [
                        ...
                        {"name": "multicluster-spawn-georepl-bin", "hostPath": {"path": "/phase42/bin", "type": "Directory"}},
...
def create_executor_pair(prefix: str, mode: str, passphrase: str) -> list[dict[str, Any]]:
    kubectl(parent_kubeconfig(), "create", "namespace", "multicluster-spawn-georepl-system", check=False)
    jobs = []
    ...
    for value in jobs:
        ...
        while time.monotonic() < deadline:
            observed = json.loads(text(kubectl(parent_kubeconfig(), "-n", "multicluster-spawn-georepl-system", "get", "job", name, "-o", "json")))
            status = observed.get("status", {})
            if status.get("succeeded") == 1:
                break
            if status.get("failed", 0) > 0:
                logs = text(kubectl(parent_kubeconfig(), "-n", "multicluster-spawn-georepl-system", "logs", f"job/{name}", check=False))
                description = text(kubectl(parent_kubeconfig(), "-n", "multicluster-spawn-georepl-system", "describe", f"job/{name}", check=False))
...
def executor_readback(prefix: str) -> list[dict[str, Any]]:
    raw = json.loads(text(kubectl(parent_kubeconfig(), "-n", "multicluster-spawn-georepl-system", "get", "jobs", "-l", "amoebius.io/phase=42", "-o", "json")))
    rows = []
...
def pulumi_stack_list() -> list[dict[str, Any]]:
    environment = {
        ...
        "PULUMI_CONFIG_PASSPHRASE": "multicluster-spawn-georepl-observer-does-not-decrypt-secrets",
    }
...
def current_amoebius_binary() -> Path:
    flags = (
        "-f-multicluster-spawn-georepl-classifier-default-confluent-mutant",
        "-f-multicluster-spawn-georepl-project-identity-mutant",
        "-f-multicluster-spawn-georepl-drop-parallel-executor-mutant",
    )
...
def minio_idempotent_blob(minio: dict[str, Any], suffix: str) -> dict[str, Any]:
    payload = b"multicluster-spawn-georepl-result:alpha+beta"
...
def postgres_replication(suffix: str) -> dict[str, Any]:
    ...
    for namespace, app, secret_dir in (
        ("grafana-db", "grafana-postgres", "/platform-services-2-secrets"),
        ("keycloak-db", "keycloak-postgres", "/keycloak-ingress-secrets"),
    ):
...
INSERT INTO {table} VALUES
 ...
 ('work-42-canary',0,'multicluster-spawn-georepl-command'),
 ('work-42-canary',1,'transform:alpha'),
 ('work-42-canary',3,'multicluster-spawn-georepl-result:alpha+beta')
...
def pulsar_native_roundtrip() -> dict[str, Any]:
    flags = (
        "-f-pulsar-client-topic-literal-mutant",
        "-f-pulsar-client-drop-one-sided-mutant",
        "-f-pulsar-client-produce-raw-mutant",
    )
    environment = os.environ.copy()
    environment.pop("PULSAR_CLIENT_REUSE_FRESH_LIVE", None)
    run(
    ...
    if len(rounds) != 2 or not all(row.get("resultNativeProtocol") and row.get("resultDuplicateCollapsed") for row in rounds):
        raise Phase42Failure("pulsar-client-native-roundtrip-readback")
    return {
...
def phase42_live() -> dict[str, Any]:
    ...
    try:
        ...
        for name in ("p42-up-a", "p42-up-b"):
            kubectl(parent_kubeconfig(), "-n", "multicluster-spawn-georepl-system", "delete", "job", name, "--wait=true", "--timeout=120s")
    ...
    evidence: dict[str, Any] = {
        ...
        "replication": {
            ...
            "foldOracle": "test/fixture/multicluster_spawn_georepl/idempotent-write.golden.json",
            "boundary": "two real child clusters; retained HA native-protocol data plane",
        },
        "classifier": {
            "oracle": "test/fixture/inject/confluence/expected_classes.dhall",
...
def main() -> int:
    ...
    print("multicluster-spawn-georepl-multicluster-live: PASS")
    print(f"multicluster-spawn-georepl-multicluster-cleanup: PASS ({evidence['evidenceDigest']})")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"multicluster-spawn-georepl-multicluster-live: FAIL: {error}", file=sys.stderr, flush=True)
        raise SystemExit(1)
