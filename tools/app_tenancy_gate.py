...
MUTANTS = (
    ("M-drop-provider-arm", "app-tenancy-drop-provider-arm-mutant", "provider-arm-incomplete:Pulsar"),
    ("M-collapse-tenant-key", "app-tenancy-collapse-tenant-key-mutant", "tenant-key-collapse"),
)
...
def reject_mutant(name: str, flag: str, marker: str) -> dict[str, str]:
    ...
    environment = os.environ.copy()
    environment["APP_TENANCY_REUSE_FRESH_LIVE"] = "1"
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 34,
        "gate_command": "python3 tools/app_tenancy_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows.append(invoke("sealed-live-reader", (CABAL, "test", "tenant-provider-provisioning-live", "--test-show-details=direct", "-j1"), extra_env={"APP_TENANCY_REUSE_FRESH_LIVE": "1"}))
        rows.extend(reject_mutant(*mutant) for mutant in MUTANTS)
        disabled = tuple(f"-f-{flag}" for _, flag, _ in MUTANTS)
        rows.append(invoke("baseline-restored", (CABAL, "test", "tenant-provider-provisioning-live", *disabled, "--test-show-details=direct", "-j1"), extra_env={"APP_TENANCY_REUSE_FRESH_LIVE": "1"}))
        ...
        print(f"app-tenancy-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']})")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"app-tenancy-gate: FAIL: {error}", file=sys.stderr)
        return 1
