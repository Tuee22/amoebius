...
FLAGS = (
    "phase61-ack-redis-publish-mutant",
    "phase61-omit-durable-lookup-mutant",
    "phase61-drop-membership-validation-mutant",
    "phase61-two-tab-replay-mutant",
    "phase61-discard-pending-disconnect-mutant",
    "phase61-remove-scope-idempotency-mutant",
)
UNVERIFIED = {
...
def derive_ledger():
    ...
    value = {
        "phase": 61,
        "gate_command": "python3 tools/phase61_gate.py",
        "register": "3",
        "substrate": "linux-cpu",
        "date": "2026-08-11",
...
def main():
    ...
    result = run((sys.executable, "tools/phase61_replay_live.py"))
    if result.returncode:
        print(result.stdout)
        return 1
    ...
    print(
        f"phase61-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; "
