...
FLAGS = (
    "phase60-store-plaintext-mutant",
    "phase60-retain-credentials-mutant",
    "phase60-two-replay-leaders-mutant",
    "phase60-omit-fencing-mutant",
    "phase60-silent-dependency-eviction-mutant",
    "phase60-reuse-partition-key-mutant",
)
...
def derive_ledger():
    ...
    value = {
        "phase": 60,
        "gate_command": "python3 tools/phase60_gate.py",
        "register": "2",
        "substrate": "none",
        "date": "2026-08-11",
        "layers": [
...
def main():
    ...
    result = run((sys.executable, "tools/phase60_browser_live.py"))
    if result.returncode:
        print(result.stdout)
        return 1
    ...
    print(
        f"phase60-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; "
