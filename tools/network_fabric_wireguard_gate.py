...
MUTANTS = (
    ("M-missing-peer-key", "network-fabric-wireguard-missing-peer-key-mutant", "network-fabric-wireguard-missing-peer-key:"),
    ("M-hub-no-endpoint", "network-fabric-wireguard-hub-no-endpoint-mutant", "network-fabric-wireguard-hub-no-endpoint:"),
    ("M-drop-resource-envelope", "network-fabric-wireguard-drop-resource-envelope-mutant", "network-fabric-wireguard-drop-resource-envelope:"),
    ("M-early-listener-replacement", "network-fabric-wireguard-early-listener-replacement-mutant", "network-fabric-wireguard-early-listener-replacement:"),
)
...
def cabal_test_args(enabled: str | None = None) -> tuple[str, ...]:
    return (
        CABAL, "test", "network-fabric-wireguard-wireguard-live-gate", "-w", GHC, *all_flags(enabled),
...
def reject_mutant(name: str, flag: str, marker: str) -> dict[str, str]:
    environment = os.environ.copy()
    environment["NETWORK_FABRIC_WIREGUARD_PURE_ONLY"] = "1"
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 41,
        "gate_command": "python3 tools/network_fabric_wireguard_gate.py --reuse-fresh-live",
...
def no_live_residue() -> dict[str, str]:
    ...
    cgroups = list(Path("/sys/fs/cgroup").glob("amoebius-network-fabric-wireguard-*"))
    require(not cgroups, "live-cgroup-residue")
    temporary = list(Path("/var/tmp").glob("amoebius-network-fabric-wireguard-*"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows = [invoke("source-build", (
            CABAL, "build", "amoebius:dsl-core", "network-fabric-wireguard-wireguard-live-gate", "-w", GHC, *disabled, "-j1", "-v0",
        ))]
        ...
        rows.append(invoke("sealed-live-reader", cabal_test_args(), extra_env={"NETWORK_FABRIC_WIREGUARD_REUSE_FRESH_LIVE": "1"}))
        rows.extend(reject_mutant(*mutant) for mutant in MUTANTS)
        rows.append(invoke("baseline-restored", cabal_test_args(), extra_env={"NETWORK_FABRIC_WIREGUARD_REUSE_FRESH_LIVE": "1"}))
        ...
        print(f"network-fabric-wireguard-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']})")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"network-fabric-wireguard-gate: FAIL: {error}", file=sys.stderr)
        return 1
