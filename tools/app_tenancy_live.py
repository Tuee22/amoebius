...
SYSTEM_NAMESPACE = "app-tenancy-system"
...
VAULT_PORT = 18204
PULSAR_PORT = 18085
TOKEN_CA = Path(f"/tmp/amoebius-app-tenancy-ca-{os.getpid()}.crt")
...
def apply(value: dict[str, Any]) -> None:
    kubectl("apply", "--server-side", "--field-manager=app-tenancy-harness", "--force-conflicts", "-f", "-", stdin=json.dumps(value))


def apply_as(token: str, value: dict[str, Any]) -> None:
    kubectl_as(token, "apply", "--server-side", "--field-manager=amoebius-app-tenancy-provider", "--force-conflicts", "-f", "-", stdin=json.dumps(value))
...
def setup_authorities(run_suffix: str) -> tuple[dict[str, str], str]:
    apply({"apiVersion": "v1", "kind": "Namespace", "metadata": {"name": SYSTEM_NAMESPACE, "labels": {"amoebius.dev/app-tenancy-run": run_suffix}}})
    providers = {
    ...
    for provider, namespace in providers.items():
        ...
        role_name = f"app-tenancy-{provider.lower()}-pod-reader"
        apply({
    ...
    apply({
        ...
        "metadata": {"name": "app-tenancy-kubernetes-observer"},
        "rules": [
    ...
    apply({
        ...
        "roleRef": {"apiGroup": "rbac.authorization.k8s.io", "kind": "ClusterRole", "name": "app-tenancy-kubernetes-observer"},
    })
    apply({
        "apiVersion": "rbac.authorization.k8s.io/v1", "kind": "ClusterRole",
        "metadata": {"name": "app-tenancy-provider-enactor"},
        "rules": [
    ...
    apply({
        ...
        "roleRef": {"apiGroup": "rbac.authorization.k8s.io", "kind": "ClusterRole", "name": "app-tenancy-provider-enactor"},
    })
    tokens: dict[str, str] = {}
...
def keycloak_apply(app: str, tenants: list[str], challenge: str) -> dict[str, Any]:
    ...
    admin = keycloak_token("master", {"grant_type": "password", "client_id": "admin-cli", "username": "keycloak-ingress-admin", "password": password})
    ...
    client_id = f"app-tenancy-observer-{challenge[:8]}"
...
def vault_apply(root_token: str, app: str, tenants: list[str], challenge: str) -> dict[str, Any]:
    ...
    observer_policy = f"app-tenancy-observer-{challenge[:8]}"
    policy_text = "\n".join(
...
def minio_apply(app: str, tenants: list[str], challenge: str) -> dict[str, Any]:
    ...
    for tenant in tenants:
        ...
        status, payload = phase30.s3_request("PUT", bucket, "app-tenancy-challenge.json", json.dumps({"challenge": challenge, "tenant": tenant}, sort_keys=True).encode())
        if status != 200:
...
def minio_observe(state: dict[str, Any], observer_token: str, challenge: str) -> list[dict[str, Any]]:
    ...
    for row in state["rows"]:
        ...
        marker_url, marker_headers = s3_headers("GET", row["bucket"], "app-tenancy-challenge.json")
...
def minio_cleanup(state: dict[str, Any]) -> None:
    for row in state["rows"]:
        for method, key, query, expected in (("DELETE", "app-tenancy-challenge.json", "", 204), ("DELETE", "", "policy=", 204), ("DELETE", "", "", 204)):
...
def postgres_apply(app: str, tenants: list[str], challenge: str, enactor_token: str) -> dict[str, Any]:
    ...
    script = "export PGPASSWORD=\"$(cat /platform-services-2-secrets/superuser)\"; /usr/lib/postgresql/17/bin/psql -h 127.0.0.1 -U postgres -d postgres -v ON_ERROR_STOP=1 <<'SQL'\n" + "\n".join(statements) + "\nSQL\n"
...
def cleanup_authorities() -> None:
    for kind, name in (("clusterrolebinding", "app-tenancy-kubernetes-observer"), ("clusterrolebinding", "app-tenancy-provider-enactor"), ("clusterrole", "app-tenancy-kubernetes-observer"), ("clusterrole", "app-tenancy-provider-enactor")):
    ...
    for namespace, role in (("edge-system", "app-tenancy-keycloak-pod-reader"), ("vault-system", "app-tenancy-vault-pod-reader"), ("pulsar-system", "app-tenancy-pulsar-pod-reader"), ("platform-system", "app-tenancy-minio-pod-reader"), ("grafana-db", "app-tenancy-postgres-pod-reader")):
...
def target_inventory(app: str, tenants: list[str], challenge: str, root_token: str, enactor_token: str) -> dict[str, Any]:
    ...
    clients, _ = json_request("GET", KEYCLOAK_PORT, "/keycloak/admin/realms/amoebius/clients?clientId=" + urllib.parse.quote(f"app-tenancy-observer-{challenge[:8]}"), headers=keycloak_headers, expected={200})
    vault_found = []
    pulsar_found = []
    minio_found = []
    kubernetes_found = []
    ...
    postgres_script = f"export PGPASSWORD=\"$(cat /platform-services-2-secrets/superuser)\"; /usr/lib/postgresql/17/bin/psql -h 127.0.0.1 -U postgres -d postgres -Atqc \"select rolname from pg_roles where rolname in ({names_sql}) union select nspname from pg_namespace where nspname in ({names_sql}) order by 1\""
...
def cleanup_stale(root_token: str) -> None:
    ...
    with port_forward("edge-system", "service/keycloak", KEYCLOAK_PORT, 8080):
        ...
        for client in clients:
            if str(client.get("clientId", "")).startswith("app-tenancy-observer-"):
    ...
    with port_forward("vault-system", "service/root-vault", VAULT_PORT, 8200):
        ...
        for policy in policy_list.get("data", {}).get("keys", []):
            if str(policy).startswith(("p34app", "app-tenancy-observer-")):
    ...
    with port_forward("platform-system", "service/minio", MINIO_PORT, 9000):
        ...
        for bucket in re.findall(r"<Name>(p34app[^<]+)</Name>", payload.decode(errors="replace")):
            phase30.s3_request("DELETE", bucket, "app-tenancy-challenge.json")
        ...
        if not cleaned and enactor:
            ...
            for label, cleanup_step in cleanup_steps:
                try:
                    ...
                    print(f"app-tenancy-cleanup-warning:{label}:{cleanup_error}", flush=True)
        cleanup_authorities()
...
def main() -> int:
    ...
    if args.reuse_fresh_live and EVIDENCE.is_file() and time.time() - EVIDENCE.stat().st_mtime < 1800:
        ...
        if current.get("schemaVersion") == "amoebius.phase34.live-evidence.v1" and current.get("cleanup", {}).get("inventoriesEqual"):
            print("app-tenancy-tenant-provider-live: PASS (reused fresh sealed evidence)")
            return 0
    ...
    try:
        ...
        print("app-tenancy-tenant-provider-live: PASS (six live provider arms, separated observers, paired rejects, teardown)")
        return 0
    finally:
...
if __name__ == "__main__":
    try:
        ...
        print(f"app-tenancy-tenant-provider-live: FAIL: {error}", flush=True)
        raise SystemExit(1)
