...
AMOEBIUS = "/live-dsl-deploy-artifacts/amoebius"
...
FIELD_MANAGER = "amoebius-live-dsl-deploy"
ENACT_ORDER = [
...
def transit_encrypt(token: str, plaintext: bytes) -> str:
    result = vault_request(
        "POST", "transit/encrypt/live-dsl-deploy-control-plane", token,
...
def transit_decrypt(token: str, ciphertext: str) -> bytes:
    result = vault_request(
        "POST", "transit/decrypt/live-dsl-deploy-control-plane", token,
...
def vault_kubernetes_token() -> str:
    ...
    result = vault_request(
        "POST", "auth/kubernetes/login", payload={"role": "live-dsl-deploy-daemon", "jwt": jwt}, expected={200},
    )
...
def desired_hash(value: dict[str, Any]) -> str:
    ...
    candidate.setdefault("metadata", {}).setdefault("annotations", {}).pop("amoebius.io/live-dsl-deploy-desired-hash", None)
...
def apply_if_changed(value: dict[str, Any], run_id: str) -> bool:
    ...
    metadata.setdefault("annotations", {})["amoebius.io/live-dsl-deploy-desired-hash"] = digest
    metadata.setdefault("labels", {})["app.kubernetes.io/managed-by"] = "amoebius"
    if identity(value).split("/", 1)[0] in {"Deployment", "Service", "HTTPRoute"} and metadata["name"] == "live-dsl-deploy-trivial":
        metadata["labels"]["amoebius.dev/live-dsl-deploy-run"] = run_id
    path = object_path(value)
    status, observed = kubernetes_request("GET", path)
    if (
        status == 200
        and observed.get("metadata", {}).get("annotations", {}).get("amoebius.io/live-dsl-deploy-desired-hash") == digest
        ...
        "spec": {
            ...
            "template": {"metadata": {"labels": {"app": "prometheus-query-proxy"}}, "spec": {
                ...
                "containers": [{
                    ...
                    "command": ["/usr/bin/python3"], "args": ["/platform-services-2-query-proxy/query_proxy.py", "--max-concurrent=4", "--max-series=64", "--max-samples=4096", "--max-range-seconds=3600", "--timeout-seconds=30"],
                    ...
                    "volumeMounts": [{"name": "query-proxy", "mountPath": "/platform-services-2-query-proxy", "readOnly": True}],
                }],
...
def trivial_objects(run_id: str) -> list[dict[str, Any]]:
    labels = {"app": "keycloak-ingress-route-probe", "amoebius.io/live-dsl-deploy-app": "trivial"}
    script = "from http.server import BaseHTTPRequestHandler,HTTPServer\nclass H(BaseHTTPRequestHandler):\n def do_GET(s):\n  b=('live-dsl-deploy-trivial:'+s.path).encode();s.send_response(200);s.send_header('Content-Length',str(len(b)));s.end_headers();s.wfile.write(b)\n def log_message(s,*a): pass\nHTTPServer(('0.0.0.0',8080),H).serve_forever()"
    deployment = {
        "apiVersion": "apps/v1", "kind": "Deployment", "metadata": {"name": "live-dsl-deploy-trivial", "namespace": "edge-system"},
        "spec": {"replicas": 1, "strategy": {"type": "Recreate"}, "selector": {"matchLabels": {"amoebius.io/live-dsl-deploy-app": "trivial"}},
    ...
    service = {
        "apiVersion": "v1", "kind": "Service", "metadata": {"name": "live-dsl-deploy-trivial", "namespace": "edge-system"},
        "spec": {"type": "ClusterIP", "selector": {"amoebius.io/live-dsl-deploy-app": "trivial"}, "ports": [{"name": "http", "port": 8080, "targetPort": 8080}]},
    }
    route = {
        "apiVersion": "gateway.networking.k8s.io/v1", "kind": "HTTPRoute", "metadata": {"name": "live-dsl-deploy-trivial", "namespace": "edge-system", "annotations": {"amoebius.io/auth-owner": "Keycloak", "amoebius.io/data-plane-projection": "static-envoy-v1"}},
        "spec": {"parentRefs": [{"name": "keycloak-edge"}], "hostnames": ["phase32.amoebius.internal"], "rules": [{"matches": [{"path": {"type": "PathPrefix", "value": "/phase33/"}}], "backendRefs": [{"name": "live-dsl-deploy-trivial", "port": 8080}]}]},
    }
    return [deployment, route, service]


def ensure_envoy_route(run_id: str) -> list[str]:
    config_path = "/api/v1/namespaces/edge-system/configmaps/keycloak-ingress-envoy"
    ...
    load_assignment:
      ...
      endpoints: [{lb_endpoints: [{endpoint: {address: {socket_address: {address: live-dsl-deploy-trivial.edge-system.svc.cluster.local, port_value: 8080}}}}]}]
'''
        if "admin:\n" not in config:
            raise HelperError("envoy-admin-anchor-absent")
        config = config.replace("admin:\n", cluster + "admin:\n", 1)
        desired = {"apiVersion": "v1", "kind": "ConfigMap", "metadata": {"name": "keycloak-ingress-envoy", "namespace": "edge-system"}, "data": {"envoy.yaml": config}}
        if apply_if_changed(desired, run_id):
            mutations.append("ConfigMap/edge-system/keycloak-ingress-envoy")
        ...
        patch = {"metadata": {"resourceVersion": live["metadata"]["resourceVersion"]}, "spec": {"template": {"metadata": {"annotations": {"amoebius.io/live-dsl-deploy-config-sha": rollout_digest}}}}}
...
def reconcile(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if not re.fullmatch(r"live-dsl-deploy-[a-f0-9]{12}", run_id):
        raise HelperError("live-dsl-deploy-run-id-invalid")
    mutations: list[str] = []
    ...
    wait_deployment("edge-system", "live-dsl-deploy-trivial")
