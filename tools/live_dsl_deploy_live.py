...
NAMESPACE = "live-dsl-deploy-system"
CONTROL_PLANE = "amoebius-control-plane"
...
NEGATIVE = ROOT / "test/fixture/live_dsl_deploy/negative-expected-tags.tsv"
...
def apply(value: dict[str, Any], manager: str = "live-dsl-deploy-bootstrap-host") -> None:
...
def phase32_module() -> Any:
    path = ROOT / "tools/keycloak_ingress_live.py"
    spec = importlib.util.spec_from_file_location("phase32_for_phase33", path)
    if spec is None or spec.loader is None:
        raise LiveFailure("keycloak-ingress-module-load")
...
def configure_vault(root_token: str) -> dict[str, Any]:
    with port_forward("vault-system", "service/root-vault", 18200, 8200):
        vault_local("POST", "transit/keys/live-dsl-deploy-control-plane", root_token, {}, {200, 204, 400})
...
path "transit/decrypt/live-dsl-deploy-control-plane" { capabilities = ["update"] }
'''
        vault_local("PUT", "sys/policies/acl/live-dsl-deploy-daemon", root_token, {"policy": policy})
        vault_local("POST", "auth/kubernetes/role/live-dsl-deploy-daemon", root_token, {
            "bound_service_account_names": CONTROL_PLANE,
            "bound_service_account_namespaces": NAMESPACE,
            "policies": "live-dsl-deploy-daemon", "ttl": "15m",
        })
    return {"transitKey": "live-dsl-deploy-control-plane", "kubernetesRole": "live-dsl-deploy-daemon", "minioCredentialPath": "secret/data/amoebius/phase33/minio"}
...
def install_artifacts() -> dict[str, Any]:
    control_plane_binary = cabal_binary("exe:amoebius-singleton")
    amoebius = cabal_binary("exe:amoebius")
    if not control_plane_binary.is_file() or not amoebius.is_file() or not UNLOCK.is_file():
        raise LiveFailure("live-dsl-deploy-artifact-missing")
    docker("exec", NODE, "/bin/rm", "-rf", NODE_ARTIFACTS)
    docker("exec", NODE, "/bin/mkdir", "-p", NODE_ARTIFACTS)
    for source, name in (
        (control_plane_binary, "amoebius-singleton"),
        (amoebius, "amoebius"),
        (ROOT / "tools/live_dsl_deploy_runtime_helper.py", "phase33_runtime_helper.py"),
    ...
    return {
        "controlPlaneSha256": "sha256:" + hashlib.sha256(control_plane_binary.read_bytes()).hexdigest(),
        "amoebiusSha256": "sha256:" + hashlib.sha256(amoebius.read_bytes()).hexdigest(),
        "helperSha256": "sha256:" + hashlib.sha256((ROOT / "tools/live_dsl_deploy_runtime_helper.py").read_bytes()).hexdigest(),
        "staticallyLinked": "statically linked" in run(("/usr/bin/file", str(control_plane_binary))).stdout,
...
def clean_previous() -> None:
    ...
    for resource in ("clusterrole/live-dsl-deploy-daemon", "clusterrole/live-dsl-deploy-harness-observer", "clusterrolebinding/live-dsl-deploy-daemon", "clusterrolebinding/live-dsl-deploy-harness-observer"):
...
def resources() -> list[dict[str, Any]]:
    return [
        ...
        {"apiVersion": "v1", "kind": "ServiceAccount", "metadata": {"name": CONTROL_PLANE, "namespace": NAMESPACE}},
        {"apiVersion": "rbac.authorization.k8s.io/v1", "kind": "ClusterRole", "metadata": {"name": "live-dsl-deploy-daemon"}, "rules": [
        ...
        {"apiVersion": "rbac.authorization.k8s.io/v1", "kind": "ClusterRoleBinding", "metadata": {"name": "live-dsl-deploy-daemon"}, "subjects": [{"kind": "ServiceAccount", "name": CONTROL_PLANE, "namespace": NAMESPACE}], "roleRef": {"apiGroup": "rbac.authorization.k8s.io", "kind": "ClusterRole", "name": "live-dsl-deploy-daemon"}},
        ...
        {"apiVersion": "v1", "kind": "Service", "metadata": {"name": "amoebius-admin", "namespace": NAMESPACE, "labels": {"amoebius.io/endpoint-class": "HostLocalPeer"}}, "spec": {"type": "NodePort", "selector": {"app": CONTROL_PLANE}, "ports": [{"name": "admin", "port": 18080, "targetPort": 18080, "nodePort": NODE_PORT}]}},
        {"apiVersion": "apps/v1", "kind": "Deployment", "metadata": {"name": CONTROL_PLANE, "namespace": NAMESPACE, "labels": {"app.kubernetes.io/managed-by": "amoebius"}}, "spec": {
            "replicas": 1, "strategy": {"type": "Recreate"}, "selector": {"matchLabels": {"app": CONTROL_PLANE}},
            "template": {"metadata": {"labels": {"app": CONTROL_PLANE}}, "spec": {
                "serviceAccountName": CONTROL_PLANE, "automountServiceAccountToken": True, "securityContext": {"fsGroup": 1000, "fsGroupChangePolicy": "OnRootMismatch"},
                ...
                "containers": [{"name": "control-plane", "image": IMAGE, "imagePullPolicy": "Never", "command": ["/live-dsl-deploy-artifacts/amoebius-singleton"], "ports": [{"name": "admin", "containerPort": 18080}],
                    ...
                    "volumeMounts": [{"name": "artifacts", "mountPath": "/live-dsl-deploy-artifacts", "readOnly": True}, {"name": "dhall", "mountPath": "/live-dsl-deploy-dhall"}, {"name": "podinfo", "mountPath": "/etc/podinfo", "readOnly": True}, {"name": "artifacts", "mountPath": "/var/lib/amoebius/phase29-unlock.age", "subPath": "phase29-unlock.age", "readOnly": True}],
                }],
...
def setup_control_plane() -> None:
    for value in resources():
        apply(value)
    kubectl("apply", "--server-side", "--field-manager=live-dsl-deploy-bootstrap-host", "--force-conflicts", "-f", "test/fixture/live_dsl_deploy/harness-rbac.yaml")


def pod_name() -> str:
    items = json.loads(kubectl("-n", NAMESPACE, "get", "pods", "-l", f"app={CONTROL_PLANE}", "-o", "json").stdout)["items"]
    if len(items) != 1:
        raise LiveFailure(f"control-plane-pod-cardinality:{len(items)}")
...
def wait_pod_running(timeout: int = 180) -> str:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            pass
        time.sleep(1)
    raise LiveFailure("control-plane-pod-not-running")
...
def release_bootstrap_holder() -> dict[str, Any]:
    ...
    kubectl("-n", NAMESPACE, "patch", "lease", LEASE, "--type=merge", "--field-manager=live-dsl-deploy-bootstrap-host", "-p", json.dumps(patch))
    ...
    kubectl("-n", NAMESPACE, "patch", "lease", LEASE, "--type=merge", "--field-manager=live-dsl-deploy-bootstrap-host", "-p", json.dumps(acknowledgement))
    return {
...
def wait_control_plane_holder(pod_uid: str, timeout: int = 180) -> dict[str, Any]:
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        ...
        time.sleep(1)
    raise LiveFailure("control-plane-holder-timeout")
...
def audit_events_since(start: dt.datetime) -> list[dict[str, Any]]:
    events = []
    with tempfile.TemporaryDirectory(prefix="amoebius-live-dsl-deploy-audit-") as temporary:
...
def control_plane_mutations(events: list[dict[str, Any]]) -> tuple[set[str], list[dict[str, Any]]]:
    user = f"system:serviceaccount:{NAMESPACE}:{CONTROL_PLANE}"
    mutations = []
    for event in events:
...
def check_history_capacity() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="amoebius-live-dsl-deploy-history-") as temporary:
        docker("cp", f"{NODE}:/var/log/kubernetes/audit", temporary, timeout=180)
...
def test_admin_sequence(password: str, normalized_dhall: str, run_id: str) -> dict[str, Any]:
    ...
    first_audit_set, first_audit = control_plane_mutations(audit_events_since(first_start))
    ...
    second_set, second_audit = control_plane_mutations(audit_events_since(second_start))
    if second_set:
        raise LiveFailure(f"second-audit-not-empty:{sorted(second_set)}")
    for verb, name, value in (("put", "live-dsl-deploy-canary", "live-dsl-deploy-kv-value"), ("get", "live-dsl-deploy-canary", None), ("list", "", None), ("delete", "live-dsl-deploy-canary", None)):
        ...
        if verb == "get" and result.get("value") != "live-dsl-deploy-kv-value":
...
def negative_corpus(password: str) -> dict[str, Any]:
    ...
    for line in NEGATIVE.read_text(encoding="utf-8").splitlines()[1:]:
        ...
        status, result = admin_request("/v1/dhall/update", {"password": password, "dhall": source, "probes": [], "runId": "live-dsl-deploy-000000000000"}, timeout=300)
...
def admin_negatives(password: str, normalized_dhall: str, run_id: str) -> dict[str, Any]:
    ...
    admissions = []
    fixture_root = ROOT / "test/fixture/admin/secrets-capability"
    for line in ADMISSION.read_text(encoding="utf-8").splitlines()[1:]:
        ...
        writes, _ = control_plane_mutations(audit_events_since(start))
        if writes:
...
hits=[]
for root in ['/tmp','/live-dsl-deploy-artifacts','/live-dsl-deploy-dhall']:
...
'''
    process = subprocess.run(
        ["/usr/bin/kubectl", "--kubeconfig", str(KUBECONFIG), "-n", NAMESPACE, "exec", "-i", "-c", "control-plane", pod, "--", "/usr/bin/python3", "-c", scanner],
    ...
    return {"containerFilesystem": "clear", "processArgvAndEnvironment": "clear", "kubernetesObjects": "clear", "controlPlaneLogs": "clear", "passwordPersisted": False}
...
def edge_probe() -> dict[str, Any]:
    ...
    del token
    if status != 200 or body.decode() != "live-dsl-deploy-trivial:/probe":
        raise LiveFailure(f"live-dsl-deploy-edge-probe:{status}:{body!r}")
...
def off_host_denial() -> dict[str, Any]:
    ...
    pod = json.loads(kubectl("-n", "keycloak-ingress-wan", "get", "pods", "-o", "json").stdout)["items"][0]["metadata"]["name"]
    program = "import socket\ns=socket.socket();s.settimeout(3)\ntry:s.connect((%r,%d));print('REACHABLE')\nexcept Exception:print('DENIED')" % (node_ip, NODE_PORT)
    result = kubectl("-n", "keycloak-ingress-wan", "exec", pod, "--", "/usr/bin/python3", "-c", program)
...
def replacement_probe(old_pod: str, expected_sha: str) -> dict[str, Any]:
    ...
    kubectl("-n", NAMESPACE, "rollout", "status", f"deployment/{CONTROL_PLANE}", "--timeout=240s", timeout=260)
    new_pod = pod_name()
    new_uid = get("pod", new_pod, NAMESPACE)["metadata"]["uid"]
    if new_uid == old_uid:
        raise LiveFailure("control-plane-pod-uid-unchanged")
    ...
    holder = wait_control_plane_holder(new_uid)
...
def restore_shared_stack(snapshots: dict[str, dict[str, Any]], run_id: str) -> dict[str, Any]:
    kubectl("-n", "edge-system", "delete", "deployment,service,httproute", "-l", f"amoebius.dev/live-dsl-deploy-run={run_id}", "--ignore-not-found=true")
    for value in snapshots.values():
        apply(sanitize(value), manager="live-dsl-deploy-postflight")
    ...
    sweep = kubectl("get", "deployment,service,httproute", "-A", "-l", f"amoebius.dev/live-dsl-deploy-run={run_id}", "-o", "name").stdout.strip().splitlines()
    if sweep:
...
def main() -> int:
    ...
    if not root_token or not operator_password:
        raise LiveFailure("LIVE_DSL_DEPLOY_VAULT_ROOT_TOKEN-and-PHASE29_OPERATOR_PASSWORD-required")
    ...
    if phase32_receipt.get("result") != "PASS":
        raise LiveFailure("keycloak-ingress-prerequisite-not-green")
    snapshots = {
        ...
        "envoyConfig": get("configmap", "keycloak-ingress-envoy", "edge-system"),
        "envoyDeployment": get("deployment", "envoy", "edge-system"),
    }
    if "phase33_trivial" in snapshots["envoyConfig"].get("data", {}).get("envoy.yaml", ""):
        raise LiveFailure("preflight-live-dsl-deploy-envoy-route-leaked")
    clean_previous()
    ...
    kubectl("-n", NAMESPACE, "rollout", "status", f"deployment/{CONTROL_PLANE}", "--timeout=180s", timeout=200)
    handoff["controlPlane"] = wait_control_plane_holder(pod_uid)
    handoff["sameLeaseUid"] = handoff["bootstrap"]["uid"] == handoff["released"]["uid"] == handoff["controlPlane"]["uid"]
    handoff["podNonServingBeforeRelease"] = True
    handoff["noControlPlaneDaemonMutationBeforeAcquire"] = len(control_plane_mutations(audit_events_since(bootstrap_mutation_end))[0]) == 0
    normalized = run(("/home/matthewnowak/.local/bin/dhall", "--file", "dhall/examples/platform_plus_trivial_app.dhall"), timeout=300).stdout
    run_id = "live-dsl-deploy-" + secrets.token_hex(6)
    ...
    harness_user = f"system:serviceaccount:{NAMESPACE}:live-dsl-deploy-harness"
    ...
    result = {
        "schema": "amoebius.live-dsl-deploy.v1", "register": 3, "substrate": "linux-cpu",
        ...
        "manifest": {"kind": "Deployment", "replicas": 1, "strategy": "Recreate", "persistentVolumeClaims": [], "standbyReplicas": 0, "amoebiusElection": False, "image": IMAGE, "fieldManager": "amoebius-live-dsl-deploy"},
        ...
        "attribution": {"controlPlaneServiceAccount": f"system:serviceaccount:{NAMESPACE}:{CONTROL_PLANE}", "controlPlaneWriteCount": len(admin["firstPass"]["audit"]), "harnessPrincipal": harness_user, "harnessPlatformWrites": 0, "observer": "kube-apiserver audit log"},
        "artifactSource": {"image": IMAGE, "imagePullPolicy": "Never", "publicPulls": 0, "haskellControlPlaneDaemon": True, "pythonEffectHelper": True},
    ...
    print("live-dsl-deploy-live: PASS (typed Lease handoff, Haskell admin control-plane daemon, exact live reconcile/no-op, durable replacement, negatives, teardown)")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"live-dsl-deploy-live: FAIL: {problem}", file=sys.stderr)
        raise SystemExit(1)
