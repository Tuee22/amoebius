...
journal = "/tmp/ui-program-release-action-journal"
...
def setup(plans_path: Path) -> dict[str, Any]:
    for stale in Path("/tmp").glob("amoebius-ui-program-release-state-*.json"):
        try:
            cleanup(stale)
        except Exception:
            pass
    ...
    state_path = Path(f"/tmp/amoebius-ui-program-release-state-{suffix}.json")
...
def execute_cases(state: dict[str, Any], cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for revision in ports:
        ...
        journal = phase34.kubectl(
            ...
            "test ! -f /tmp/ui-program-release-action-journal || cat /tmp/ui-program-release-action-journal",
        ).stdout.splitlines()
...
def cleanup(path: Path) -> dict[str, Any]:
    ...
    if not residue:
        ...
        Path(f"/tmp/amoebius-ui-program-release-result-{challenge}.json").unlink(missing_ok=True)
...
def finish(state_path: Path, result_path: Path) -> dict[str, Any]:
    ...
    stable = {
        ...
        "fixtureDigests": {
            name: "sha256:" + hashlib.sha256((ROOT / "test/fixture/phase_40" / name).read_bytes()).hexdigest()
            for name in (
    ...
    print("ui-program-release-ui-release-live: PASS", flush=True)
    return evidence
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "setup":
            ...
            print("ui-program-release-ui-release-cleanup: PASS")
        return 0
    except (LiveFailure, OSError, ValueError, KeyError, json.JSONDecodeError) as error:
        print(f"ui-program-release-ui-release-live: FAIL: {error}")
        return 1
