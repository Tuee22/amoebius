...
def observe_fingerprint() -> dict[str, Any]:
    ...
    fake = list(witnesses)
    fake_result = run((str(ROOT / "test/fake/phase_48_fake_ghc"),), env={}, timeout=30)
    fake[0] = {**fake[0], "absoluteProbe": str(ROOT / "test/fake/phase_48_fake_ghc"), "argv": [str(ROOT / "test/fake/phase_48_fake_ghc")], "valueSha256": hashlib.sha256(fake_result.stdout).hexdigest()}
...
def determinism_drill(bucket: str, substrate: dict[str, Any]) -> dict[str, Any]:
    ...
    runs = [
        ...
        run_compute("det-alt-input", "phase48-pinned-input-b", seed, bucket, f"{experiment}/run-alt-input/output"),
    ]
...
def owner_deployment(generation: str) -> dict[str, Any]:
    return {
        ...
        "spec": {"replicas": 1, "strategy": {"type": "Recreate"}, "selector": {"matchLabels": {"app": "phase48-cache-owner"}}, "template": {
            "metadata": {"labels": {"app": "phase48-cache-owner", "amoebius.io/phase": "48"}, "annotations": {"amoebius.io/generation": generation}},
...
def owner_pod() -> dict[str, Any]:
    pods = [
        pod for pod in get_json("-n", NAMESPACE, "get", "pods", "-l", "app=phase48-cache-owner")["items"]
...
def registry_digest_lines() -> int:
    return sum(1 for line in registry_log().splitlines() if ENGINE_DIGEST in line and f"/v2/amoebius/phase48-engine/blobs/{ENGINE_DIGEST}" in line)


def cache_drill(registry_before: set[str]) -> tuple[dict[str, Any], set[str]]:
    apply({"apiVersion": "v1", "kind": "ConfigMap", "metadata": {"name": "engine-recipe", "namespace": NAMESPACE}, "data": {"engine_source.sh": (ROOT / "test/oracle/phase_48_engine_source.sh").read_text(encoding="utf-8")}})
    ...
    repository = "amoebius/phase48-engine"
    ...
    while get_json("-n", NAMESPACE, "get", "pods", "-l", "app=phase48-cache-owner")["items"]:
...
print(json.dumps({"beforeBytes":before,"afterBytes":after,"measuredPeakBytes":max(before,after),"pinnedPresent":os.path.exists(cache+"/engine"),"unpinnedPresent":os.path.exists(unpinned),"incomingPresent":os.path.exists(incoming)},sort_keys=True))'''
    ...
    added = {key for key in registry_after - registry_before if "phase48-engine" in key or digest_hex in key}
    return ({
    ...
    try:
        ...
        try:
            with phase34.port_forward("platform-system", "service/minio", phase30.MINIO_PORT, 9000):
                ...
                digest_hex = ENGINE_DIGEST.removeprefix("sha256:")
                remaining_added = sorted(key for key in set(phase37.list_keys("registry")) - registry_before if "phase48-engine" in key or digest_hex in key) if registry_before else []
...
def main() -> int:
    try:
        evidence = execute()
        print(f"phase48-determinism-jitcache-live: PASS ({evidence['evidenceDigest']})")
        return 0
    except Exception as error:
        print(f"phase48-determinism-jitcache-live: FAIL: {error}", file=sys.stderr)
        reset_namespace()
        return 1
