...
FLAGS = (
    "ui-ha-multizone-replicas-one-mutant",
    "ui-ha-multizone-sticky-session-required-mutant",
    "ui-ha-multizone-drop-pdb-mutant",
    "ui-ha-multizone-drop-topology-spread-mutant",
    "ui-ha-multizone-fault-one-node-only-mutant",
    "ui-ha-multizone-drop-tenant-cursor-key-mutant",
    "ui-ha-multizone-drop-keycloak-zone-spread-mutant",
    "ui-ha-multizone-redis-one-node-mutant",
    "ui-ha-multizone-redis-persistent-receipt-mutant",
    "ui-ha-multizone-skip-cursor-repair-mutant",
)
UNVERIFIED = {
...
def contract(enabled=None):
    return (
        CABAL, "test", "ui-live:ui-ha-multizone-ui-ha-multizone", "-w", GHC,
...
def derive_ledger():
    ...
    value = {
        "phase": 58,
        "gate_command": "python3 tools/ui_ha_multizone_gate.py",
        "register": "3",
...
def main():
    ...
    result = run((sys.executable, "tools/ui_ha_multizone_live.py"))
    if result.returncode:
        print(result.stdout)
        return 1
    ...
    print(
        f"ui-ha-multizone-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; "
