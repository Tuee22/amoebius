...
MUTANT_FLAG = "provider-child-bringup-public-pull-mutant"
...
def phase0_domain() -> dict[str, str]:
    ...
    return {"name": "phase0-custody", "command": "read test/oracle/preimplementation_artifacts.tsv", "output": "6 oracles; 1 mutant", "result": "PASS"}


def oracle_domain() -> dict[str, str]:
    result = subprocess.run(
        (DHALL_TO_JSON, "--file", "test/fixture/dhall/provider_child_bringup/provider_provision.dhall"), cwd=ROOT,
    ...
    require(child.get("hostSubstrate") is None and child.get("controlPlanePods") == child.get("capacitySchedulerPods") == 1, "hostless-topology")
    require(child.get("hostDaemonPods") == child.get("hostNodePortPeers") == 0, "hostless-counts")
    expected_services = sorted(line.strip() for line in (ROOT / "test/golden/standard_service_set.txt").read_text(encoding="utf-8").splitlines() if line.strip())
    require(sorted(topology.get("standardServices", [])) == expected_services and len(expected_services) == 16, "service-oracle")
    convergence = (ROOT / "test/golden/convergence_argv.txt").read_text(encoding="utf-8")
    require("forbidden-tool=helm" in convergence and "forbidden-image-source=docker.io/" in convergence, "convergence-oracle")
    sequence = (ROOT / "test/golden/lease_handoff_sequence.txt").read_text(encoding="utf-8").splitlines()
    require(len(sequence) == 6 and "freshly observed holder absent" in sequence[3], "lease-sequence-oracle")
    pairs = json.loads((ROOT / "test/fixture/provider_child_bringup/hostless-topology-pairs.json").read_text(encoding="utf-8"))
    ...
    rows = list(csv.DictReader((ROOT / "test/fixture/provider_child_bringup/bootstrap-negative-tags.tsv").open(encoding="utf-8"), delimiter="\t"))
...
def evidence_domain(*, fresh: bool) -> None:
    ...
    require(sequence[1].get("readyReplicas") == 0 and sequence[4].get("readyReplicas") == 1, "control-plane-serving-order")
    ...
    require({key: topology.get(key) for key in ("controlPlaneRoles", "capacitySchedulerRoles", "hostDaemonRoles", "hostNodePortPeers", "hostSubstrate")} == {"controlPlaneRoles": 1, "capacitySchedulerRoles": 1, "hostDaemonRoles": 0, "hostNodePortPeers": 0, "hostSubstrate": None}, "hostless-object-topology")
...
def no_live_residue() -> dict[str, str]:
    namespace = subprocess.run(("/usr/bin/kubectl", "--kubeconfig", str(Path.home() / ".amoebius/phase24/kubeconfig"), "get", "namespace", "provider-child-bringup-system"), text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False, timeout=60)
    require(namespace.returncode != 0, "provider-child-bringup-namespace-residue")
    ...
    return {"name": "external-cleanup-readback", "command": "namespace and kind inventories", "output": "only retained amoebius-bootstrap-coordinator remains", "result": "PASS"}
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 45, "gate_command": "python3 tools/provider_child_bringup_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"provider-child-bringup-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']}; EKS convergence UNVERIFIED)")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"provider-child-bringup-gate: FAIL: {error}", file=sys.stderr)
        return 1
