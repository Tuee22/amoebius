...
MANIFEST_HEX = ROOT / "amoebius-store/test/golden/manifest_canonical.cbor"
MANIFEST_SHA = ROOT / "amoebius-store/test/golden/manifest_canonical.sha256"
NONCANONICAL_HEX = ROOT / "amoebius-store/test/golden/manifest_noncanonical.cbor"
HEAD_GOLDEN = ROOT / "amoebius-runtime/test/golden/head_nofault.bin"
FORBIDDEN_PACKAGES = ROOT / "amoebius-runtime/test/golden/forbidden_coordination_packages.txt"
...
MUTANTS = (
    ("M-ack-before-store", "phase37-ack-before-store-write-mutant", "worker-critical-window-order", None),
    ("M-cleanup-on-status", "phase37-cleanup-on-job-status-mutant", "terminal-persist", None),
    ("M-double-apply", "phase37-double-apply-on-redelivery-mutant", "double-application-on-redelivery", "workflow-sim-invariant:NoDoubleApplication"),
    ("M-insertion-order", "phase37-insertion-order-encoder-mutant", "canonical-manifest-writer-b", None),
    ("M-lease-election", "phase37-lease-election-mutant", "forbidden-coordination-surface", None),
    ("M-orphan-budget-omitted", "phase37-orphan-budget-omitted-mutant", "write-budget-peak-mismatch:exact-fit", None),
    ("M-orphan-consumer", "phase37-orphan-consumer-on-promotion-mutant", "standby-promotion-invariant", "workflow-sim-invariant:NoOrphanConsumerAfterPromotion"),
    ("M-orphan-free-conflict", "phase37-orphan-free-on-pointer-conflict-mutant", "write-budget-peak-mismatch:pre-horizon-orphan-exact", None),
    ("M-sweep-skips-pulsar", "phase37-sweep-skips-pulsar-mutant", "postflight-sweep-domain", None),
    ("M-trust-gateway-ack", "phase37-trust-gateway-ack-mutant", "terminal-retain-without-readback", None),
)
...
def reject_mutant(name: str, flag: str, marker: str, simulation_marker: str | None) -> dict[str, str]:
    ...
    environment = os.environ.copy()
    environment["PHASE37_PURE_ONLY"] = "1"
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 37,
        "gate_command": "python3 tools/phase37_gate.py --reuse-fresh-live",
...
def oracle_domain() -> dict[str, str]:
    result = subprocess.run((sys.executable, "tools/phase37_oracle.py"), cwd=ROOT, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False, timeout=60)
    ...
    return {"name": "independent-oracle", "command": f"{sys.executable} tools/phase37_oracle.py", "output": f"{expected_sha}; mismatch=24; custody=17", "result": "PASS"}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows.append(invoke("sealed-live-reader", (CABAL, "test", "content-store-workflow-live", *disabled, "--test-show-details=direct", "-j1"), extra_env={"PHASE37_REUSE_FRESH_LIVE": "1"}))
        ...
        rows.append(invoke("baseline-restored", (CABAL, "test", "content-store-workflow-live", "workflow-failover-sim", *disabled, "--test-show-details=direct", "-j1"), extra_env={"PHASE37_REUSE_FRESH_LIVE": "1"}))
        ...
        print(f"phase37-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']})")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, StopIteration, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"phase37-gate: FAIL: {error}", file=sys.stderr)
        return 1
