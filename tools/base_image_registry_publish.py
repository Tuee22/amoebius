...
RUN_TMP = Path(os.environ.get(
    "AMOEBIUS_RUN_TMP",
    str(ROOT / ".build/tmp/base-image-registry"),
))
PCAP = RUN_TMP / "publish.pcap"
...
def load_standup() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("base_image_registry_standup", path)
...
def verify_ephemeral_docker_config(capability: str, proof: str, tag: str, index_digest: str) -> dict[str, Any]:
    ...
    config_directory = Path(tempfile.mkdtemp(prefix="publisher-", dir=RUN_TMP))
    try:
...
def verify_current(evidence: Path) -> dict[str, Any]:
    recorded = json.loads((evidence / "publication.json").read_text(encoding="utf-8"))
    if (
