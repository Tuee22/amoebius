#!/usr/bin/env python3
"""Run the scoped Phase-50 infernix UI lift against retained services."""
...
def run_worker(namespace: str, suffix: str, command: str, input_value: str) -> dict[str, Any]:
    name = "phase50-reference-worker-" + suffix
    script = (
        "import json,sys; "
...
def browser_run(state: UiState, tokens: dict[str, str]) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="amoebius-phase50-") as temporary:
        ...
        with servers(state) as (port_a, port_b):
            result = subprocess.run(
                [NODE, str(ROOT / "test/ui/live/phase50_browser.mjs"), str(token_path), str(port_a), str(port_b),
...
def main() -> int:
    require(PHASE49_RECEIPT.is_file(), "phase49-receipt-absent")
    phase49_receipt = json.loads(PHASE49_RECEIPT.read_text(encoding="utf-8"))
    require(phase49_receipt.get("result") == "PASS-SCOPED", "phase49-prerequisite")
    challenge = secrets.token_hex(24)
    suffix = challenge[:8]
    realm = "phase50-" + suffix
    namespace = "phase50-ui-" + suffix
    tenant = "p50" + suffix
    pulsar_namespace = "ui"
    bucket = "p50-" + suffix
    ...
    print(f"phase50-infernix-ui-live: PASS-SCOPED ({evidence['evidenceDigest']}; full edge/replica/native-CBOR/production inference chain UNVERIFIED)")
    return 0
