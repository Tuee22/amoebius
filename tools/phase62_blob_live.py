...
def main():
    canary = "phase62-" + secrets.token_hex(12)
    ...
    with tempfile.TemporaryDirectory(prefix="amoebius-phase62-") as directory:
        root = Path(directory)
        ...
        thread.start()
        try:
            with tempfile.TemporaryDirectory(prefix="amoebius-phase62-profile-") as profile:
    ...
    if not all(assertions.values()):
        raise SystemExit("phase62-live: FAIL " + json.dumps(assertions, sort_keys=True))
    value = {
    ...
    print(f"phase62-live: PASS-SCOPED ({value['evidenceDigest']}; MinIO/Gateway/Kubernetes/CNI UNVERIFIED)")
