#!/usr/bin/env python3
"""Run and seal the Phase-27 `amoebius-capacity` scheduler and bootstrap-cutover gate.
...
RESULTS = ROOT / "gen/dsl/phase27/phase-results.tsv"
CONTRACT = "DEVELOPMENT_PLAN/phase_27_capacity_scheduler.md"
GATE_COMMAND = "python3 tools/phase27_gate.py --execute"
...
# reservation claims, and Sprint 27.4 re-runs the same seven against the live cutover — so
...
def execute_sprints(evidence: Path, image: str) -> None:
    ...
    for number in (1, 2, 3, 4, 5):
        arguments = [
            sys.executable, f"tools/phase27_sprint27_{number}_gate.py", "--evidence", str(evidence),
        ]
        if number == 4:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--image", default=None,
        help="the digest-pinned in-cluster reference Phase 25 published, which the scheduled Pods pull",
    )
    arguments = parser.parse_args(argv)

    gate = gate_common.PhaseGate(
        phase=27, contract=CONTRACT, command=GATE_COMMAND, register="3", substrate="linux-cpu", sides=SIDES
    )
    gate.begin()
    ...
    try:
        ...
        run((cabal, f"--with-compiler={compiler}", "test", "scheduler-ledger-spec",
        ...
        if arguments.execute:
            ...
            if not arguments.image:
                raise GateFailure("--execute needs --image: the scheduled Pods pull the Phase-25 published digest")
            print("\nlive side — the five sprints on the linux-cpu kind cluster\n")
            execute_sprints(evidence, arguments.image)
        elif evidence is None:
            raise GateFailure("phase-27 needs --execute or an --evidence bundle from a completed live run")
        else:
    ...
    return gate.finish(
        ...
        dependencies={
            "cluster": "kind (phase 24)",
            "registry": "in-cluster distribution (phase 25)",
