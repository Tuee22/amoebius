#!/usr/bin/env python3
"""Validate the single inert retained StorageClass on the Phase-24 cluster."""
...
ORACLE = ROOT / "test/live/fixtures/storageclass_expected.yaml"
NAMESPACE = "amoebius-phase28-storageclass"
STORAGE_CLASS = "amoebius-retained"
PV = "amoebius-phase28-storageclass-bind"
...
def kubectl(*arguments: str, stdin: str | None = None, check: bool = True, timeout: int = 240) -> subprocess.CompletedProcess[str]:
    result = subprocess.run(
        ("/usr/bin/kubectl", "--kubeconfig", str(KUBECONFIG), *arguments), cwd=ROOT,
...
def execute() -> dict[str, Any]:
    ...
    positive_pv = {
        ...
        "spec": {
            ...
            "hostPath": {"path": "/var/tmp/amoebius-phase28-storageclass-bind", "type": "DirectoryOrCreate"},
        },
    }
    apply(positive_pv)
    apply(positive_pvc)
    ...
    return {
        "schema": "amoebius.phase28.sprint28.1-live.v1", "register": 3, "substrate": "linux-cpu",
        "inventory": {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print("phase28-sprint28.1-live: PASS (one inert class, wait reason, explicit bind, negative, cleanup)")
        return 0
    except (LiveFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired, yaml.YAMLError) as problem:
        cleanup_test_resources()
        print(f"phase28-sprint28.1-live: FAIL: {problem}", file=sys.stderr)
        return 1
