...
def main():
    canary = "phase60-" + secrets.token_hex(16)
    unlock = "unlock-" + secrets.token_hex(16)
    server = ThreadingHTTPServer(("127.0.0.1", 0), Handler)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    try:
        with tempfile.TemporaryDirectory(prefix="amoebius-phase60-profile-") as profile:
    ...
    if not all(assertions.values()):
        raise SystemExit("phase60-browser: FAIL " + json.dumps(assertions, sort_keys=True))
    ...
    print(
        f"phase60-browser: PASS-SCOPED ({value['evidenceDigest']}; "
