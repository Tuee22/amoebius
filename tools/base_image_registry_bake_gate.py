...
SOURCE = _load("base_image_registry_source_probe", Path(__file__).resolve().parent / "base_image_registry_source_probe.py")
...
def seal(
    ...
    rows = [
        invoke(
            ...
            (
                ...
                f"--with-compiler={compiler}", "test", "base-image-registry-spec",
        ...
        invoke(
            ...
            (python, "tools/base_image_registry_source_probe.py", "--reconcile-only"),
        ),
        invoke(
            "builder-boundary",
            (
                python, "tools/base_image_registry_builder_probe.py",
                "--container", "amoebius-base-image-registry-buildkitd",
        ...
        invoke(
            "cpu-memory-overruns",
            (
                python, "tools/base_image_registry_cgroup_overrun_probe.py",
        ...
        invoke(
            ...
            (
                python, "tools/base_image_registry_enospc_probe.py",
                "--builder-image", builder_image,
        ...
        invoke(
            "oci-object-audit",
            (
                python, "tools/base_image_registry_oci_probe.py", str(artifact),
                "--bounds", "test/fixture/base_image_registry/image_artifact_bounds.json",
        ...
        invoke(
            ...
            (
                python, "tools/base_image_registry_oci_file_probe.py", str(artifact),
        ...
        invoke(
            "spdx-file-inventory",
            (
                python, "tools/base_image_registry_sbom.py",
        ...
        invoke(
            "seeded-mutants",
            (
                python, "tools/base_image_registry_bake_mutation_gate.py",
                "--official-file-join", str(evidence / "official-file-execution-join.json"),
                "--evidence", str(evidence / "bake-mutants.json"),
            ),
        ),
    ]
    ...
    mutants = json_file(evidence / "bake-mutants.json")
    ...
    expected_payloads = 2 * len(SOURCE.published_payload_oracle())
    ...
    if len(file_join.get("payloads", [])) != expected_payloads:
        raise GateFailure(
            f"published-payload-domain:{len(file_join.get('payloads', []))}!={expected_payloads}"
        )
...
def write_results(evidence: Path, rows: list[dict[str, str]], receipt: dict[str, Any]) -> None:
    ...
    (evidence / "bake-receipt.json").write_text(
        json.dumps(receipt, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    with (evidence / "bake-results.tsv").open("w", encoding="utf-8") as handle:
    ...
    (evidence / "bake-gate.log").write_text("\n".join(log) + "\n", encoding="utf-8")
