...
convenience:
  ...
  * The observer was `strace -e trace=execve`, which exists on one of the four declared
    ...
    python3 tools/gadt_decode_ir_gate.py
...
GENERATED = ROOT / ".build" / "dsl" / "gadt-decode"
...
ORACLE = ROOT / "test" / "oracle" / "gadt_decode_ir"
...
EXPECTED_RESULTS = {
    ...
    "dhall-typecheck-preconditions": "3/3-green",
    ...
    # oracle test/oracle/gadt_decode_ir/positive_trees.tsv (998 + 929 + 924 + 1962 + 714). It is
...
SURFACE_EVIDENCE: dict[str, tuple[str, str] | None] = {
    ...
    for surface, metric in {
        ...
        "gadt-decode-positive-corpus": "positive-fixtures",
        "gadt-decode-tagged-negative-corpus": "tagged-negatives",
        "dhall-typecheck-green-negative-precondition": "dhall-typecheck-preconditions",
...
def observer_mutants(resolved: dict[str, Any]) -> tuple[bool, str]:
    ...
    `test/mutant/gadt_decode_ir/` and neither needs the suite rebuilt: each exercises
...
def mutant_side(
    ...
    env["AMOEBIUS_GADT_DECODE_SCHEMA_FIXTURE"] = "test/mutant/gadt_decode_ir/legalized_schema.dhall"
    result = subprocess.run(
        [observer.tool("cabal"), f"--with-compiler={resolved['ghc']['path']}",
         f"--builddir={BUILD_ROOT}", f"--store-dir={ROOT / '.build' / 'cabal-store'}",
         "test", "gadt-decode-spec", "--test-show-details=direct"],
...
def measure(observed: dict[str, Any], mutant: str, scan: str) -> tuple[dict[str, str], bool]:
    ...
    measured = {
        ...
        "dhall-typecheck-preconditions": f"{observed['pairs']}/3-green",
        "import-policy-negatives": "4/4-red-ForbiddenImport-including-nested",
...
def main() -> int:
    ...
    layers = {
        "Decision": "tested" if rows.get("acceptance-token") == "spec-composition-proven-gadt_decode" else "UNVERIFIED",
        "Protocol": "UNVERIFIED",
        "Runtime": "UNVERIFIED",
    }
