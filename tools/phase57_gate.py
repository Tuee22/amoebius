...
FLAGS = (
    "phase57-shift-before-watermark-mutant",
    "phase57-discard-cursor-mutant",
    "phase57-drop-tenant-cursor-key-mutant",
    "phase57-stale-registration-mutant",
)
UNVERIFIED = {
...
def contract(enabled=None):
    return (
        CABAL,
        "test",
        "ui-live:phase57-ui-rollout-reconnect",
        "-w",
        GHC,
...
def derive_ledger():
    ...
    value = {
        "phase": 57,
        "gate_command": "python3 tools/phase57_gate.py",
        "register": "3",
        "substrate": "linux-cpu",
        "date": "2026-08-11",
        "layers": [
...
def main():
    ...
    result = run((sys.executable, "tools/phase57_ui_rollout_live.py"))
    if result.returncode:
        print(result.stdout)
        return 1
    ...
    print(
        f"phase57-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; "
