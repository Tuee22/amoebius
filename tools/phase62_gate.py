...
FLAGS = (
    "phase62-store-plaintext-mutant", "phase62-expose-opfs-handle-mutant",
    "phase62-omit-partition-scope-mutant", "phase62-replay-before-verification-mutant",
    "phase62-silent-blob-eviction-mutant", "phase62-trust-caller-digest-mutant",
)
UNVERIFIED = {
...
def derive_ledger():
    ...
    value = {
        "phase": 62, "gate_command": "python3 tools/phase62_gate.py",
...
def main():
    ...
    result = run((sys.executable, "tools/phase62_blob_live.py"))
    if result.returncode:
        print(result.stdout)
        return 1
    ...
    print(f"phase62-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']}; platform providers UNVERIFIED)")
    return 0
