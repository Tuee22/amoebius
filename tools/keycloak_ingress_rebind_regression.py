...
CLUSTER = "amoebius-keycloak-ingress-rebind"
NODE = CLUSTER + "-control-plane"
KUBECONFIG = Path.home() / ".amoebius/keycloak-ingress-rebind/kubeconfig"
RETAINED_ROOT = Path("/var/tmp/amoebius-keycloak-ingress-rebind")
AUDIT_ROOT = Path("/var/tmp/amoebius-keycloak-ingress-rebind-audit")
ROW_FIXTURE = ROOT / "test/fixture/keycloak_ingress/marker-row.sql"
OBJECT_FIXTURE = ROOT / "test/fixture/keycloak_ingress/marker-object.bin"
...
def configure_harness() -> None:
    ...
    phase28.KIND_CONFIG = ROOT / "test/fixture/live/keycloak_ingress/rebind-kind.yaml"
...
def execute() -> dict[str, Any]:
    if EXPECTED_ROW_MARKER not in ROW_FIXTURE.read_text(encoding="utf-8"):
        raise RegressionFailure("keycloak-ingress-row-fixture-drift")
    configure_harness()
...
def main() -> int:
    try:
        ...
        print(
            "keycloak-ingress-rebind-regression: PASS "
        ...
        print(f"keycloak-ingress-rebind-regression: FAIL: {problem}", file=sys.stderr)
        return 1
