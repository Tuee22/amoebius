...
def migration_job(namespace: str) -> dict[str, Any]:
    return {
        ...
        "metadata": {
            "name": "release-lifecycle-migrate", "namespace": namespace,
...
def setup() -> dict[str, Any]:
    for stale in Path("/tmp").glob("amoebius-release-lifecycle-state-*.json"):
        try:
            cleanup(stale)
        except Exception:
            pass
    ...
    state_path = Path(f"/tmp/amoebius-release-lifecycle-state-{suffix}.json")
    state = {
...
def run_store_round(state: dict[str, Any], row: dict[str, Any], release_hash: str, release_body: bytes) -> dict[str, Any]:
    ...
    previous_hash = hashlib.sha256(f"release-lifecycle-previous-{prefix}".encode()).hexdigest()
...
def run_database_and_rollout(state: dict[str, Any], row: dict[str, Any], release_hash: str) -> dict[str, Any]:
    ...
    apply(deployment(namespace, "release-lifecycle-base", "base-apply"))
    phase34.kubectl("-n", namespace, "rollout", "status", "deployment/release-lifecycle-base", "--timeout=180s", timeout=200)
    base = object_observation(namespace, "deployment", "release-lifecycle-base", "deployment/release-lifecycle-base:Available")
...
""")
    ...
    phase34.kubectl("-n", namespace, "wait", "--for=condition=complete", "job/release-lifecycle-migrate", "--timeout=180s", timeout=200)
    migration = object_observation(
        namespace, "job", "release-lifecycle-migrate", "job/release-lifecycle-migrate:Complete+sql-copy:verified",
    )
    ...
    apply(deployment(namespace, "release-lifecycle-final", "finalize"))
    phase34.kubectl("-n", namespace, "rollout", "status", "deployment/release-lifecycle-final", "--timeout=180s", timeout=200)
    final = object_observation(
        namespace, "deployment", "release-lifecycle-final", "deployment/release-lifecycle-final:Available+old-schema:retired",
    )
...
def cleanup(path: Path) -> dict[str, Any]:
    ...
    if not residue:
        ...
        if challenge:
            Path(f"/tmp/amoebius-release-lifecycle-result-{challenge}.json").unlink(missing_ok=True)
        path.unlink(missing_ok=True)
    return {
...
def finish(state_path: Path, result_path: Path) -> dict[str, Any]:
    ...
    print("release-lifecycle-live-finish: PASS", flush=True)
    return evidence
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "setup":
            ...
            print("release-lifecycle-live-cleanup: PASS")
        return 0
    except (LiveFailure, OSError, ValueError, KeyError, json.JSONDecodeError) as error:
        print(f"release-lifecycle-release-live: FAIL: {error}", flush=True)
        return 1
