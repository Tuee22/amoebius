...
PHASE51_RECEIPT = ROOT / "DEVELOPMENT_PLAN/evidence/phase_51/phase-receipt.json"
PHASE50_RECEIPT = ROOT / "DEVELOPMENT_PLAN/evidence/phase_50/phase-receipt.json"
NODE = "/usr/bin/node"
...
def browser_run(state: UiState, tokens: dict[str, str]) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="amoebius-phase52-browser-") as temporary:
        ...
        with servers(state) as (port_a, port_b):
            result = subprocess.run(
                [NODE, str(ROOT / "test/ui/live/phase52_browser.mjs"), str(token_path), str(port_a), str(port_b),
...
def main() -> int:
    require(PHASE51_RECEIPT.is_file(), "phase51-receipt-absent")
    require(PHASE50_RECEIPT.is_file(), "phase50-receipt-absent")
    phase51_receipt = json.loads(PHASE51_RECEIPT.read_text(encoding="utf-8"))
    phase50_receipt = json.loads(PHASE50_RECEIPT.read_text(encoding="utf-8"))
    require(phase51_receipt.get("result") == "PASS-SCOPED", "phase51-prerequisite")
    require(phase50_receipt.get("result") == "PASS-SCOPED", "phase50-prerequisite")
    ...
    store_cleaned = False
    with tempfile.TemporaryDirectory(prefix="amoebius-phase52-durable-") as temporary:
    ...
    print(f"phase52-jitml-ui-live: PASS-SCOPED ({evidence['evidenceDigest']}; retained providers/Kubernetes UI replicas/Envoy/native-CBOR/full sibling serving UNVERIFIED)")
    return 0
