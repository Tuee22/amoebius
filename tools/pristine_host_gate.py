#!/usr/bin/env python3
"""Provider-aware pristine Linux guest harness for the bootstrap-coordinator live gate.
...
DEFAULT_EVIDENCE = ROOT / ".build/runs/bootstrap-coordinator/pristine-standalone"
HOST_TEMP_ROOT = ROOT / ".build/tmp/bootstrap-coordinator"
GUEST_ROOT = Path("/root/amoebius")
GUEST_TOOL_HOME = GUEST_ROOT / ".build/toolchain/runtime/home"
GUEST_LOCAL_BIN = GUEST_TOOL_HOME / ("." + "local") / "bin"
GUEST_TEST_ROOT = GUEST_ROOT / ".test_data/runs/bootstrap-coordinator"
KUBECONFIG = Path("/root/amoebius/.data/bootstrap-coordinator/kubeconfig")
CLUSTER = "amoebius-bootstrap-coordinator"
NODE = f"{CLUSTER}-control-plane"
MANAGED_TOOLS = ("ghc", "cabal", "ghcup", "kind", "kubectl")
INSTANCE_PATTERN = re.compile(r"^amoebius-bootstrap-coordinator-[a-z0-9][a-z0-9-]*$")
...
class GateConfig:
    provider: Provider
    instance: str = "amoebius-bootstrap-coordinator-pristine"
    cpus: int = 4
    memory_gib: int = 8
    disk_gib: int = 80
    source: Path = ROOT
...
def run(
    ...
    *,
    ...
    result = subprocess.run(
        list(arguments),
        # An Incus client that inherits a controlling terminal may try to read it from a
        # background process group and loop forever on SIGTTIN after the guest command has
        # already completed. Only the source-tar transfer owns an input stream; every other
        # provider command receives an explicit EOF.
    ...
    if check and result.returncode:
        ...
        raise GateError(f"command-failed:{rendered}:{result.returncode}\n{output}")
    return result
...
def prepare_unified_backing(guest: Guest) -> str:
    ...
    image = "/root/amoebius/.data/bootstrap-coordinator/unified.img"
    mountpoint = "/root/amoebius/.data/bootstrap-coordinator/unified"
    ...
    guest.execute((
        ...
        "/root/amoebius/.data/bootstrap-coordinator/patches",
    ))
    guest.execute((
        "/usr/bin/chmod", "0777",
        f"{mountpoint}/kubelet", f"{mountpoint}/containerd",
...
SPLIT_ROLE_MOUNTS = (
    ("nodefs", "/root/amoebius/.data/bootstrap-coordinator/nodefs", ("kubelet", "system/etcd", "system/audit", "system/pods")),
    ("imagefs", "/root/amoebius/.data/bootstrap-coordinator/imagefs", ("containerd",)),
)
...
def observe_split_runtime_layout(guest: Guest) -> dict[str, str | bytes]:
    ...
    result = pb_trace(guest, "/root/amoebius/.test_data/runs/bootstrap-coordinator/split-runtime-execve.log", layout="split-runtime")
    ...
    trace = guest.execute(("/usr/bin/cat", "/root/amoebius/.test_data/runs/bootstrap-coordinator/split-runtime-execve.log")).stdout
    guest.execute((
        str(GUEST_LOCAL_BIN / "kind"), "delete", "cluster", "--name", CLUSTER,
...
def source_filter(info: tarfile.TarInfo) -> tarfile.TarInfo | None:
    ...
    excluded = {
        ".git", ".build", ".data", ".test_data", "gen", "dist-newstyle", "node_modules", "__pycache__",
    }
...
def transfer_source(guest: Guest, source: Path) -> None:
    ...
    HOST_TEMP_ROOT.mkdir(parents=True, exist_ok=True)
    with tempfile.NamedTemporaryFile(
        prefix="source-", suffix=".tar", dir=HOST_TEMP_ROOT,
    ) as archive:
...
def observation_triple(guest: Guest) -> tuple[bytes, bytes, bytes]:
    ...
    clusters = guest.execute((str(GUEST_LOCAL_BIN / "kind"), "get", "clusters")).stdout
...
def node_uid(guest: Guest) -> bytes:
    return guest.execute((
        str(GUEST_LOCAL_BIN / "kubectl"), "--kubeconfig", str(KUBECONFIG),
...
set -eu
cd {GUEST_ROOT}
export HOME={GUEST_TOOL_HOME}
export TMPDIR={GUEST_ROOT}/.build/tmp/bootstrap-coordinator
export TMP=$TMPDIR TEMP=$TMPDIR
export XDG_CACHE_HOME={GUEST_ROOT}/.build/toolchain/cache/xdg
export XDG_CONFIG_HOME={GUEST_TOOL_HOME}/.config
export XDG_DATA_HOME={GUEST_TOOL_HOME}/.local/share
export XDG_STATE_HOME={GUEST_TOOL_HOME}/.local/state
export PATH={GUEST_ROOT}/tools:/usr/bin:/bin
{GUEST_TOOL_HOME}/.ghcup/bin/cabal \
  --store-dir={GUEST_ROOT}/.build/cabal-store build \
  --builddir={GUEST_ROOT}/.build/dist-newstyle/bootstrap-coordinator-mutant \
  --with-compiler={GUEST_TOOL_HOME}/.ghcup/bin/ghc \
  ./amoebius.cabal:exe:amoebius -j2 \
  --flags=+bootstrap-coordinator-one-shot-kind-guard-mutant >{GUEST_TEST_ROOT}/m3-build.log 2>&1
{GUEST_TOOL_HOME}/.ghcup/bin/cabal \
  --store-dir={GUEST_ROOT}/.build/cabal-store list-bin \
  --builddir={GUEST_ROOT}/.build/dist-newstyle/bootstrap-coordinator-mutant \
  --with-compiler={GUEST_TOOL_HOME}/.ghcup/bin/ghc \
  ./amoebius.cabal:exe:amoebius \
  --flags=+bootstrap-coordinator-one-shot-kind-guard-mutant
""")
    ...
    attempt = guest.execute((
        "/usr/bin/strace", "-f", "-e", "trace=execve,execveat", "-o", "/root/amoebius/.test_data/runs/bootstrap-coordinator/m3-execve.log",
    ...
    trace = guest.execute(("/usr/bin/cat", "/root/amoebius/.test_data/runs/bootstrap-coordinator/m3-execve.log")).stdout
    ...
    repair = pb_trace(guest, "/root/amoebius/.test_data/runs/bootstrap-coordinator/m3-repair-execve.log")
...
def run_gate(config: GateConfig) -> None:
    ...
    try:
        guest.create()
        stage = "wait-guest-ready"
        guest.wait_ready()
        stage = "clean-preflight"
        preflight = require_clean_preflight(guest)
        stage = "prepare-prerequisites"
        prerequisite_log = prepare_prerequisites(guest)
        stage = "prepare-unified-backing"
        layout_prerequisite = prepare_unified_backing(guest)
        stage = "transfer-source"
        transfer_source(guest, config.source)
        guest.execute(("/usr/bin/mkdir", "-p", str(GUEST_TEST_ROOT)))
        guest.execute(("/usr/bin/touch", str(GUEST_TEST_ROOT / ".amoebius-test-root")))

        stage = "initial-bootstrap"
        first = pb_trace(guest, "/root/amoebius/.test_data/runs/bootstrap-coordinator/initial-execve.log")
        write_evidence(config.evidence / "pristine-initial.log", first.stdout)
        if b"bootstrap-handoff: ready" not in first.stdout:
            raise GateError("phase24-initial-bootstrap-not-ready")
        stage = "initial-observation-triple"
        before = observation_triple(guest)

        stage = "idempotent-rerun"
        rerun = pb_trace(guest, "/root/amoebius/.test_data/runs/bootstrap-coordinator/rerun-execve.log")
        write_evidence(config.evidence / "pristine-rerun.log", rerun.stdout)
        ...
        rerun_trace = guest.execute(("/usr/bin/cat", "/root/amoebius/.test_data/runs/bootstrap-coordinator/rerun-execve.log")).stdout
        ...
        stage = "observe-process-envelopes"
        process_envelopes = observe_process_envelopes(guest)
        write_evidence(config.evidence / "pristine-process-envelopes.tsv", process_envelopes)
        stage = "drive-host-engine-cpu"
        host_engine_throttle = drive_host_engine_cpu(guest)
        write_evidence(config.evidence / "pristine-host-engine-throttle.tsv", host_engine_throttle)

        stage = "stopped-node-repair"
        original_id = guest.execute(("/usr/bin/docker", "inspect", "--format", "{{.Id}}", NODE)).stdout
        original_uid = node_uid(guest)
        guest.execute(("/usr/bin/docker", "stop", NODE))
        stopped = pb_trace(guest, "/root/amoebius/.test_data/runs/bootstrap-coordinator/stopped-node-execve.log")
        stopped_trace = guest.execute(("/usr/bin/cat", "/root/amoebius/.test_data/runs/bootstrap-coordinator/stopped-node-execve.log")).stdout
        ...
        stage = "missing-kubeconfig-repair"
        guest.execute(("/usr/bin/rm", str(KUBECONFIG)))
        missing = pb_trace(guest, "/root/amoebius/.test_data/runs/bootstrap-coordinator/missing-kubeconfig-execve.log")
        missing_trace = guest.execute(("/usr/bin/cat", "/root/amoebius/.test_data/runs/bootstrap-coordinator/missing-kubeconfig-execve.log")).stdout
        ...
        inventory = guest.execute(("/usr/bin/cat", "/root/amoebius/.data/bootstrap-coordinator/observed-inventory.json")).stdout
        ready = guest.execute((
            str(GUEST_LOCAL_BIN / "kubectl"), "--kubeconfig", str(KUBECONFIG), "get", "nodes", "-o", "wide",
        )).stdout
        initial_trace = guest.execute(("/usr/bin/cat", "/root/amoebius/.test_data/runs/bootstrap-coordinator/initial-execve.log")).stdout

        stage = "one-shot-guard-mutant"
        m3_report, m3_trace = observe_one_shot_guard_mutant(guest)

        stage = "unified-teardown"
        ...
        stage = "split-runtime-lifecycle"
        split = observe_split_runtime_layout(guest)
        split_clusters = guest.execute((str(GUEST_LOCAL_BIN / "kind"), "get", "clusters")).stdout
        ...
        guest.execute(("/usr/bin/test", "-f", str(GUEST_TEST_ROOT / ".amoebius-test-root")))
        guest.execute(("/usr/bin/rm", "-rf", str(GUEST_TEST_ROOT)))
        print(f"pristine-host-gate: PASS ({config.provider.value})")
    except Exception as problem:
        write_evidence(
            config.evidence / "pristine-failure.txt",
            f"stage\t{stage}\nerror\t{type(problem).__name__}: {problem}\n",
        )
        if guest.created:
            for guest_path in (
                "/root/amoebius/.test_data/runs/bootstrap-coordinator/initial-execve.log.stdout",
                "/root/amoebius/.test_data/runs/bootstrap-coordinator/rerun-execve.log.stdout",
                "/root/amoebius/.test_data/runs/bootstrap-coordinator/stopped-node-execve.log.stdout",
                "/root/amoebius/.test_data/runs/bootstrap-coordinator/missing-kubeconfig-execve.log.stdout",
                "/root/amoebius/.test_data/runs/bootstrap-coordinator/m3-build.log",
                "/root/amoebius/.test_data/runs/bootstrap-coordinator/m3-repair-execve.log.stdout",
                "/root/amoebius/.test_data/runs/bootstrap-coordinator/split-runtime-execve.log.stdout",
            ):
                try:
                    captured = guest.execute(("/usr/bin/cat", guest_path), check=False)
                    if captured.stdout:
                        write_evidence(config.evidence / Path(guest_path).name, captured.stdout)
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--instance", default="amoebius-bootstrap-coordinator-pristine")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"pristine-host-gate: {problem}", file=sys.stderr)
        return 1
