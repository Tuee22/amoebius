...
RUNTIME_FLAGS = (
    "phase47-ignore-signal-mutant", "phase47-apply-over-quota-mutant",
    "phase47-unreachable-as-gone-mutant", "phase47-ignore-live-csinode-mutant",
    "phase47-dedup-distinct-pvcs-mutant", "phase47-template-id-as-physical-mutant",
)
PULUMI_FLAGS = ("phase47-skip-sweep-mutant", "phase47-untagged-orphan-mutant")
RUNTIME_MUTANTS = {
    "phase47-ignore-signal-mutant": "mut-47.1-ignore-signal",
    "phase47-apply-over-quota-mutant": "mut-47.1-apply-over-quota",
    "phase47-unreachable-as-gone-mutant": "mut-47.1-unreachable-as-gone",
    "phase47-ignore-live-csinode-mutant": "mut-47.1-ignore-live-csinode",
    "phase47-dedup-distinct-pvcs-mutant": "mut-47.1-dedup-distinct-pvcs",
    "phase47-template-id-as-physical-mutant": "mut-47.1-template-id-as-physical",
}
PULUMI_MUTANTS = {
    "phase47-skip-sweep-mutant": "mut-47.2-skip-sweep",
    "phase47-untagged-orphan-mutant": "mut-47-untagged-orphan",
}
UNVERIFIED = {
...
def oracle_domain() -> dict[str, str]:
    legal = load_dhall("test/dhall/phase_47_provider_provision.dhall")
    over = load_dhall("test/dhall/phase_47_provider_over_quota.dhall")
    missing = load_dhall("test/dhall/phase_47_provider_missing_capability.dhall")
    ...
    sweep_rows = list(csv.DictReader((ROOT / "test/goldens/provider_ephemeral_sweep_expected.txt").open(encoding="utf-8"), delimiter="\t"))
    ...
    identities = list(csv.DictReader((ROOT / "test/goldens/provider_two_instance_identity_map.txt").open(encoding="utf-8"), delimiter="\t"))
    require(len(identities) == 4 and len({row["expected-physical-identity"] for row in identities}) == 4, "identity-map")
    refusals = list(csv.DictReader((ROOT / "test/fixtures/phase47/node-refusal-matrix.tsv").open(encoding="utf-8"), delimiter="\t"))
    require(len(refusals) == 8 and len({row["expected_tag"] for row in refusals}) == 8, "refusal-matrix")
    corpus = json.loads((ROOT / "test/fixtures/phase47/run-owned-sweep-corpus.json").read_text(encoding="utf-8"))
...
def no_live_residue() -> dict[str, str]:
    for namespace in ("phase47-system", "phase47-audit"):
    ...
    return {"name": "external-cleanup-readback", "command": "namespace and kind inventories", "output": "no Phase-47 namespaces; only retained amoebius-phase24 remains", "result": "PASS"}
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 47,
        "gate_command": "python3 tools/phase47_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.reuse_fresh_live:
            ...
            rows.append(invoke("scoped-signal-sweep-analogue", (sys.executable, "tools/phase47_provider_dynamic_live.py"), timeout=1200))
        ...
        print(f"phase47-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']}; AWS node/leak sweep UNVERIFIED)")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"phase47-gate: FAIL: {error}", file=sys.stderr)
        return 1
