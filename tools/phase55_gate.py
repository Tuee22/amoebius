...
FLAGS=("phase55-disable-csrf-mutant","phase55-dispatch-before-auth-mutant","phase55-local-socket-map-mutant","phase55-redis-receipt-authority-mutant","phase55-canned-response-mutant","phase55-open-provider-edge-mutant","phase55-drop-networkpolicy-mutant")
...
def cmd(on=None):return (CABAL,"test","ui-live:phase55-ui-single-tenant-live","-w",GHC,*cfg(on),"--test-show-details=direct","-j1","-v0")
def run(n,a):
...
def derive():
 s=[x for x in ENUM.read_text().splitlines() if x and not x.startswith("#")];v={"phase":55,"gate_command":"python3 tools/phase55_gate.py","register":"3","substrate":"linux-cpu","date":"2026-08-11","layers":[{"name":"Decision","status":"tested"},{"name":"Protocol","status":"tested"},{"name":"Runtime","status":"tested"}],"coverage":[{"surface":x,"status":"UNVERIFIED" if x in UNV else "tested"} for x in s]};q=dict(v);v["ledger_hash"]=fp(q);return v
def main():
 try:
  if "--derive-ledger" in sys.argv:print(json.dumps(derive(),separators=(",",":")));return 0
  rows=[run("contract",cmd()),run("scoped-live",(sys.executable,"tools/phase55_ui_single_tenant_live.py"))]
