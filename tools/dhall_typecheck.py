...
ORACLE = ROOT / "test" / "oracle" / "dhall_typecheck_schema"
RESULTS = ROOT / ".build" / "dhall" / "dhall-typecheck" / "phase-results.tsv"
...
def check_resource_mutants() -> tuple[int, int, int]:
    manifest = (ROOT / "test/mutant/dhall_typecheck_schema/dhall_typecheck_resource_mutations.tsv").read_text(encoding="utf-8").splitlines()
    expected_manifest = [
        "family\toracle\toperator\texpected",
        "resource-field-deletion\ttest/oracle/dhall_typecheck_schema/resource_fields.csv\tdelete-each-field\tresource-inventory-red",
        "resource-type-substitution\ttest/oracle/dhall_typecheck_schema/resource_type_requirements.csv\tdelete-each-required-token\ttype-requirement-red",
    ]
    ...
    prior = ROOT / "test/mutant/dhall_typecheck_schema/dhall_typecheck_prior_ref_provisioned.dhall"
    ...
    duplicate = ROOT / "test/mutant/dhall_typecheck_schema/dhall_typecheck_duplicate_event_authority.dhall"
    ...
    optional = ROOT / "test/mutant/dhall_typecheck_schema/dhall_typecheck_optional_transition.dhall"
    ...
    latest = ROOT / "test/mutant/dhall_typecheck_schema/dhall_typecheck_implicit_latest_transition.dhall"
...
def check_constructor_rejections() -> int:
    fixtures = sorted((ROOT / "test/fixture/dhall_typecheck_schema/ctor_reject").glob("*.dhall"))
    if len(fixtures) < 9:
...
def check_mutant() -> None:
    ...
    mutant = "|".join(union_arms(ROOT / "test/mutant/dhall_typecheck_schema/dhall_typecheck_capability_custom_arm.dhall", "Capability"))
...
def write_results(
    ...
    RESULTS.write_text(
        ...
        f"dhall-typecheck-negatives\t{negative_count}/8-red-specific\n"
...
def main() -> int:
    ...
    try:
        if DHALL is None:
            raise GateFailure(
                "AMOEBIUS_DHALL is unset: run this battery through tools/dhall_typecheck_schema_gate.py, "
        ...
        if args.positive_only:
            print(f"dhall-typecheck: PASS ({len(SCHEMAS)} schemas, {len(POSITIVES)} positives)")
            return 0
        ...
        if args.negative_only:
            print(f"dhall-typecheck-negatives: PASS ({negative_count} catalog negatives)")
            return 0
        ...
        # `gadt-decode-residue` metric below, which records UNVERIFIED because binding- and
        ...
        print(
            f"dhall-typecheck: PASS ({len(SCHEMAS)} schemas, {len(POSITIVES)} positives, "
        ...
        print(f"dhall-typecheck: FAIL: {problem}", file=sys.stderr)
        return 1
