...
convenience:
  ...
  * `test/dsl/DecodeSpec.hs` hard-coded one developer's `ghc` and `dhall` paths. The suite
...
EXPECTED_RESULTS = {
    ...
    # oracle tests/oracle/gate2/positive_trees.tsv (998 + 929 + 924 + 1962 + 714). It is
...
def mutant_side(resolved: dict[str, Any], run_dir: Path) -> tuple[bool, str]:
    ...
    env["AMOEBIUS_GATE2_SCHEMA_FIXTURE"] = "tests/mutants/gate2/legalized_schema.dhall"
    result = subprocess.run(
        [resolved["cabal"]["path"], f"--with-compiler={resolved['ghc']['path']}",
         "test", "dsl-spec", "--test-show-details=direct"],
...
def main() -> int:
    gate = gate_common.PhaseGate(
        phase=5, contract=CONTRACT, command=GATE_COMMAND, register="1", substrate="none", sides=SIDES
    )
    gate.begin()
