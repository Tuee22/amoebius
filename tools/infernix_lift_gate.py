...
FLAGS = (
    "infernix-lift-drop-artifact-scope-mutant", "infernix-lift-mint-ready-before-pointer-commit-mutant",
    "infernix-lift-use-wallclock-seed-mutant", "infernix-lift-regenerate-command-id-mutant",
)
MUTANTS = {
    "infernix-lift-drop-artifact-scope-mutant": "mut-49-drop-artifact-scope",
    "infernix-lift-mint-ready-before-pointer-commit-mutant": "mut-49-mint-ready-before-pointer-commit",
    "infernix-lift-use-wallclock-seed-mutant": "mut-49-use-wallclock-seed",
    "infernix-lift-regenerate-command-id-mutant": "mut-49-regenerate-command-id",
}
UNVERIFIED = {
...
def oracle_domain() -> dict[str, str]:
    base = load_dhall("test/fixture/dhall/infernix_lift/infernix_core_artifact_lift.dhall")
    short = load_dhall("test/fixture/dhall/infernix_lift/cpu_budget_one_short.dhall")
    config = load_dhall("dhall/infernix/infernix.dhall")
    ...
    base_source = (ROOT / "test/fixture/dhall/infernix_lift/infernix_core_artifact_lift.dhall").read_text(encoding="utf-8")
    config_source = (ROOT / "dhall/infernix/infernix.dhall").read_text(encoding="utf-8")
    ...
    request = decode_cbor_text(ROOT / "test/fixture/infernix_lift/request.cbor")
    golden = decode_cbor_text(ROOT / "test/fixture/infernix_lift/sibling_golden.cbor")
    require("phase0-command-49" in request and golden == GOLDEN, "cbor-fixtures")
    expected = dict(csv.reader((ROOT / "test/fixture/infernix_lift/expected_hashes.tsv").open(encoding="utf-8"), delimiter="\t"))
    expected.pop("name", None)
    require(expected["model"] == MODEL_SHA, "expected-model-hash")
    request_preimage = b"infernix-lift-request-v1|tenant=tenant-a|command=phase0-command-49|nonce=phase0-nonce-49|seed=0000000000000001|input=explain content addressing"
    ...
    commands = list(csv.DictReader((ROOT / "test/fixture/infernix_lift/command_identity_matrix.tsv").open(encoding="utf-8"), delimiter="\t"))
    artifacts = list(csv.DictReader((ROOT / "test/fixture/infernix_lift/artifact_scope_readiness_matrix.tsv").open(encoding="utf-8"), delimiter="\t"))
...
def foreclosure_domain() -> list[dict[str, str]]:
    ...
    rows = [invoke("catalog-positive", (*prefix, "test/negative/compile_fail/infernix_lift/catalog_positive.hs"), timeout=300)]
    negatives = {
        "forge-ready-handle-red": ("test/negative/compile_fail/infernix_lift/forge_ready_handle.hs", "ReadyArtifactHandle"),
        "free-url-engine-red": ("test/negative/compile_fail/infernix_lift/url_engine_arm.hs", "Url"),
    }
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 49, "gate_command": "python3 tools/infernix_lift_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.derive_ledger:
            ...
            return 0
        rows = [invoke("infernix-lift-source-build", (CABAL, "build", "infernix-lift:lib:infernix-lift", "infernix-lift-native-driver", "-w", GHC, *flags(), "-j1", "-v0"))]
        ...
        print(f"infernix-lift-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']}; production TinyLlama/cross-substrate/full causal chain UNVERIFIED)")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"infernix-lift-gate: FAIL: {error}", file=sys.stderr)
        return 1
