...
def main():
    canary = "phase63-" + secrets.token_hex(12)
    server = ThreadingHTTPServer(("127.0.0.1", 0), Handler)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    try:
        with tempfile.TemporaryDirectory(prefix="amoebius-phase63-") as directory:
            root = Path(directory)
    ...
    if not all(assertions.values()):
        raise SystemExit("phase63-live: FAIL " + json.dumps(assertions, sort_keys=True))
    value = {
    ...
    print(f"phase63-live: PASS-SCOPED ({value['evidenceDigest']}; Gateway/Pulsar/providers UNVERIFIED)")
