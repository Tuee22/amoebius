...
def stand_up_runtime() -> dict[str, Any]:
    ...
    apply({
        ...
        "spec": {"replicas": 1, "strategy": {"type": "Recreate"}, "selector": {"matchLabels": {"app": "phase49-cache-owner"}}, "template": {
            "metadata": {"labels": {"app": "phase49-cache-owner", "amoebius.io/role": "cache-owner"}},
    ...
    apply({
        ...
        "spec": {"selector": {"app": "phase49-cache-owner"}, "ports": [{"name": "http", "port": 8080, "targetPort": "http"}]},
    })
    apply({
        "apiVersion": "networking.k8s.io/v1", "kind": "NetworkPolicy", "metadata": {"name": "phase49-workload-egress", "namespace": NAMESPACE},
        "spec": {"podSelector": {}, "policyTypes": ["Ingress", "Egress"], "ingress": [{"from": [{"podSelector": {"matchLabels": {"amoebius.io/role": "worker"}}}], "ports": [{"protocol": "TCP", "port": 8080}]}],
                 "egress": [
                     ...
                     {"to": [{"podSelector": {"matchLabels": {"app": "phase49-cache-owner"}}}], "ports": [{"protocol": "TCP", "port": 8080}]},
                 ]},
    })
    kubectl("-n", NAMESPACE, "rollout", "status", "deployment/cache-owner", "--timeout=180s", timeout=200)
    pod = get_json("-n", NAMESPACE, "get", "pods", "-l", "app=phase49-cache-owner")["items"]
    if len(pod) != 1:
...
def setup_vault(root_token: str, suffix: str, challenge: str) -> dict[str, Any]:
    ...
    for tenant in ("tenant-a", "tenant-b"):
        policy = f"phase49-{suffix}-{tenant}"
...
def run_live(root_token: str) -> dict[str, Any]:
    ...
    try:
        ...
        driver_path = run((CABAL, "list-bin", "phase49-native-driver", "-w", GHC), timeout=300).stdout.strip()
        ...
        if "phase49-native-driver: PASS" not in driver.stdout:
        ...
        evidence: dict[str, Any] = {
            ...
            "resourceNames": {"kubernetesNamespace": NAMESPACE, "minioBucket": bucket, "pulsarTenant": tenant, "vaultPolicyPrefix": f"phase49-{suffix}-"},
        ...
        if bucket_created:
            try:
                with phase34.port_forward("platform-system", "service/minio", phase37.MINIO_PORT, 9000):
                    ...
                    phase37.require_s3({204}, phase37.s3_request("DELETE", bucket), "phase49-delete-bucket")
            except Exception:
                pass
        if vault_state:
            try:
...
def main() -> int:
    ...
    print(f"phase49-infernix-artifact-live: PASS-SCOPED ({evidence['evidenceDigest']}; production TinyLlama/cross-substrate/general noninterference UNVERIFIED)")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"phase49-infernix-artifact-live: FAIL: {problem}", file=sys.stderr, flush=True)
        raise SystemExit(1)
