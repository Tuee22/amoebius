...
async function migrate(){const db=await openDb();const value=await navigator.locks.request("amoebius-phase64-migration",async()=>{const state=await getState(db);const next={...state,release:"B",schema:2};await putState(db,next);return{release:next.release,count:next.outbox.length,cursor:next.cursor};});db.close();return value;}
...
def main():
    canary = "phase64-" + secrets.token_hex(12)
    ...
    with tempfile.TemporaryDirectory(prefix="amoebius-phase64-") as directory:
        root = Path(directory)
        ...
        for thread in threads:
            thread.start()
        try:
            with tempfile.TemporaryDirectory(prefix="amoebius-phase64-profile-") as profile:
    ...
    if not all(assertions.values()):
        raise SystemExit("phase64-live: FAIL " + json.dumps(assertions, sort_keys=True))
    value = {
    ...
    print(f"phase64-live: PASS-SCOPED ({value['evidenceDigest']}; provider multi-zone continuity UNVERIFIED)")
