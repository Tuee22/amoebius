...
OCI = load_module("base_image_registry_oci_probe", ROOT / "tools/base_image_registry_oci_probe.py")
...
def verify(layout_path: Path, evidence_paths: list[Path]) -> dict[str, Any]:
    ...
    try:
        ...
        payload_oracle = SOURCE.published_payload_oracle()
        for (_operating_system, architecture), descriptor in sorted(descriptors.items()):
            ...
            resolved_payloads: dict[str, tuple[str, Entry]] = {}
            for payload in payload_oracle:
                name = str(payload["payload"])
                actual_path, entry = resolve(files, str(payload["required_file"]))
                resolved_payloads[name] = (actual_path, entry)
                selected.setdefault(entry.layer_digest, set()).add(entry.member_name)
            ...
            for payload in payload_oracle:
                name = str(payload["payload"])
                actual_path, entry = resolved_payloads[name]
                digest, _header = measured[(entry.layer_digest, entry.member_name)]
                payload_results.append(
                    {
                        "architecture": architecture,
                        "owner": payload["owner"],
                        "payload": name,
                        "path": payload["required_file"],
                        "resolvedPath": "/" + actual_path,
                        "layerDigest": entry.layer_digest,
                        "sha256": digest,
                    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.evidence:
            ...
            print(encoded, end="")
        print(
            f"phase25-oci-file-probe: PASS ({len(result['rows'])} official-file/execution joins; "
            f"{len(result['payloads'])} published payload files)"
        )
        return 0
    except (
