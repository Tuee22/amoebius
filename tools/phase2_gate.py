...
EMITTED = ROOT / "gen" / "tla" / "formal-model-spec"
...
def toolchain_side() -> tuple[bool, dict[str, Any]]:
    ...
    try:
        resolved = toolchain.resolve(["cabal", "java", "tla2tools"])
...
def suite_side(resolved: dict[str, Any], run_dir: Path) -> tuple[bool, dict[str, str]]:
    ...
    try:
        result = run(
            [resolved["cabal"]["path"], f"--with-compiler={resolved.get('ghc', {}).get('path', 'ghc')}",
...
def main() -> int:
    gate = gate_common.PhaseGate(
        phase=2, contract=CONTRACT, command=GATE_COMMAND, register="1", substrate="none", sides=SIDES
    )
    gate.begin()
