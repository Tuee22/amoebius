...
MUTANTS = (
    ("M-drop-user-predicate", "phase36-drop-user-predicate-mutant", "matrix-mismatch:deny-same-tenant-foreign-subject"),
    ("M-accept-body-tenant", "phase36-accept-body-tenant-mutant", "matrix-mismatch:allow-body-tenant-ignored"),
)
...
def reject_mutant(name: str, flag: str, marker: str) -> dict[str, str]:
    ...
    environment = os.environ.copy()
    environment["PHASE36_REUSE_FRESH_LIVE"] = "1"
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 36,
        "gate_command": "python3 tools/phase36_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows.append(invoke("sealed-live-reader", (CABAL, "test", "user-tenant-isolation-live", *disabled, "--test-show-details=direct", "-j1"), extra_env={"PHASE36_REUSE_FRESH_LIVE": "1"}))
        rows.extend(reject_mutant(*mutant) for mutant in MUTANTS)
        rows.append(invoke("baseline-restored", (CABAL, "test", "user-tenant-isolation-live", *disabled, "--test-show-details=direct", "-j1"), extra_env={"PHASE36_REUSE_FRESH_LIVE": "1"}))
        ...
        print(f"phase36-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']})")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"phase36-gate: FAIL: {error}", file=sys.stderr)
        return 1
