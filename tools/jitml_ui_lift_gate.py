...
FLAGS = (
    "jitml-ui-lift-mint-ready-from-checkpoint-path-mutant", "jitml-ui-lift-ignore-artifact-scope-mutant",
    "jitml-ui-lift-ignore-artifact-owner-mutant", "jitml-ui-lift-local-only-websocket-route-mutant",
    "jitml-ui-lift-redis-as-receipt-mutant",
)
MUTANTS = {
    "jitml-ui-lift-mint-ready-from-checkpoint-path-mutant": "mut-52-mint-ready-from-checkpoint-path",
    "jitml-ui-lift-ignore-artifact-scope-mutant": "mut-52-ignore-artifact-scope",
    "jitml-ui-lift-ignore-artifact-owner-mutant": "mut-52-ignore-artifact-owner",
    "jitml-ui-lift-local-only-websocket-route-mutant": "mut-52-local-only-websocket-route",
    "jitml-ui-lift-redis-as-receipt-mutant": "mut-52-redis-as-receipt",
}
UNVERIFIED = {
    ...
    "jitml-lift-cuda-training-commit-in-same-ui-flow", "direct-jitml-worker-networkpolicy-denial",
...
def oracle_domain() -> dict[str, str]:
    decoded = subprocess.run(
        (DHALL_TO_JSON, "--file", "test/fixture/dhall/jitml_ui_lift/jitml_ui.dhall"), cwd=ROOT,
        text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False, timeout=120,
    )
    require(decoded.returncode == 0, f"jitml-ui-lift-dhall-oracle:{decoded.stdout}")
    ...
    matrix = list(csv.DictReader((ROOT / "test/fixture/jitml_ui_lift/readiness_owner_scope_matrix.tsv").open(encoding="utf-8"), delimiter="\t"))
    ...
    timeline = list(csv.DictReader((ROOT / "test/fixture/jitml_ui_lift/cross_pod_receipt_timeline.tsv").open(encoding="utf-8"), delimiter="\t"))
    require([row["source"] for row in timeline] == ["ui-A", "jitml-worker", "ui-B", "harness", "ui-current"]
            ...
            "receipt-timeline")
    contract = (ROOT / "test/fixture/jitml_ui_lift/public_contract.golden").read_text(encoding="utf-8")
...
def external_cleanup() -> dict[str, str]:
    ...
    require(namespaces.returncode == 0 and not any("phase52" in row for row in namespaces.stdout.splitlines()),
            "jitml-ui-lift-namespace-residue")
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 52,
        "gate_command": "python3 tools/jitml_ui_lift_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.derive_ledger:
            ...
            return 0
        disabled = flags()
        rows = [invoke("jitml-ui-lift-source-build", (CABAL, "build", "jitml-ui-lift:lib:jitml-ui-lift",
        ...
        print(f"jitml-ui-lift-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']}; retained providers/Kubernetes UI replicas/Envoy/native-CBOR/full sibling serving UNVERIFIED)")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"jitml-ui-lift-gate: FAIL: {error}", file=sys.stderr)
        return 1
