...
NAMESPACE = "provider-child-bringup-system"
...
def deployment(name: str, *, control_plane: bool) -> dict[str, Any]:
    probe = {"exec": {"command": ["/bin/sh", "-c", "test -f /tmp/lease-held"]}, "periodSeconds": 1, "failureThreshold": 1} if control_plane else {"exec": {"command": ["/bin/true"]}, "periodSeconds": 1}
    return {
        "apiVersion": "apps/v1", "kind": "Deployment",
        "metadata": {"name": name, "namespace": NAMESPACE, "labels": {"amoebius.io/phase": "45", "amoebius.io/role": "control-plane" if control_plane else "capacity-scheduler"}},
        "spec": {
...
def reconcile_first_pass() -> dict[str, Any]:
    ...
    apply(deployment("amoebius-capacity-scheduler", control_plane=False), "parent-bootstrap")
    ...
    apply(deployment("amoebius-child-control-plane", control_plane=True), "parent-bootstrap")
    control_plane_before = wait_deployment("amoebius-child-control-plane", available=False)
    before_ready = control_plane_before.get("status", {}).get("readyReplicas", 0)
    if before_ready != 0:
    ...
    kubectl("-n", NAMESPACE, "patch", "lease", LEASE, "--type=merge", "-p", '{"spec":{"holderIdentity":"child-control-plane-pod-uid"}}', actor="child-control-plane")
    acquired = lease_snapshot()
    if acquired["uid"] != parent["uid"] or acquired["resourceVersion"] == released["resourceVersion"] or acquired["holder"] != "child-control-plane-pod-uid":
        raise Phase45Failure("child-holder-acquire")
    pod = ""
    deadline = time.monotonic() + 180
    while time.monotonic() < deadline:
        candidates = get_json("-n", NAMESPACE, "get", "pods", "-l", "app=amoebius-child-control-plane")["items"]
        ...
        time.sleep(1)
    if not pod:
        raise Phase45Failure("child-control-plane-container-not-running")
    kubectl("-n", NAMESPACE, "exec", pod, "--", "/bin/touch", "/tmp/lease-held", actor="child-control-plane")
    control_plane_after = wait_deployment("amoebius-child-control-plane", available=True)
    for name in SERVICES:
        apply(service(name), "child-control-plane")
    ...
    if roles != ["capacity-scheduler", "control-plane"]:
    ...
    child_before_acquire = [row for row in MUTATIONS[:child_acquire_index] if row["actor"] == "child-control-plane"]
    ...
    return {
        ...
        "handoff": {
            "sequence": [
                ...
                {"event": "child-ready", "readyReplicas": control_plane_after.get("status", {}).get("readyReplicas", 0)},
            ],
        ...
        "topology": {"controlPlaneRoles": 1, "capacitySchedulerRoles": 1, "hostDaemonRoles": 0, "hostNodePortPeers": len(node_ports), "hostSubstrate": None, "boundary": "ManagedEks shape emulated on retained kind API"},
    }
...
def read_only_second_pass() -> dict[str, Any]:
    ...
    after = len(MUTATIONS)
    if holder["holder"] != "child-control-plane-pod-uid" or services != sorted(SERVICES) or len(deployments) != 2 or len(reservations) != len(ADD_ONS) or after != before:
...
def main() -> int:
    evidence = execute()
    print("provider-child-bringup-provider-child-scoped-live: PASS")
    print(f"provider-child-bringup-eks-convergence: UNVERIFIED ({evidence['evidenceDigest']})")
    return 0
