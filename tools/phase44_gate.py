...
MUTANTS = (
    ("M-static-key", "phase44-static-key-mutant", "mut-44.1-static-key"),
    ("M-leak-path", "phase44-leak-path-mutant", "mut-44.1-leak-path"),
    ("M-drop-parallel-executor", "phase44-drop-parallel-executor-mutant", "mut-44.1-drop-parallel-executor"),
)
UNVERIFIED = {
    "aws-provider-account-observation", "eks-control-plane-materialization",
    "managed-node-group-materialization", "pulumi-up-from-singleton",
...
def phase0_domain() -> dict[str, str]:
    ...
    return {"name": "phase0-custody", "command": "read test/phase0_oracle_manifest.tsv", "output": "9 oracles; 3 mutants", "result": "PASS"}


def oracle_domain() -> dict[str, str]:
    result = subprocess.run(
        (DHALL_TO_JSON, "--file", "test/dhall/phase_44_provider_provision.dhall"), cwd=ROOT,
    ...
    for path in (
        ROOT / "test/goldens/checkpoint_envelope.json",
        ROOT / "test/fixtures/phase44/provider-sku-snapshot.json",
        ROOT / "test/fixtures/phase44/observed-provider-account.json",
        ROOT / "test/fixtures/phase44/expected-provider-demand.json",
        ROOT / "test/fixtures/phase44/live-observer-contract.json",
    ):
        require(isinstance(json.loads(path.read_text(encoding="utf-8")), dict), f"json-oracle:{path.name}")
    execve = (ROOT / "test/goldens/engine_execve.txt").read_text(encoding="utf-8")
    require("environment=EMPTY" in execve and "argv[0]=/usr/local/bin/pulumi" in execve, "execve-oracle")
    matrix = list(csv.DictReader((ROOT / "test/fixtures/phase44/one-short-matrix.tsv").open(encoding="utf-8"), delimiter="\t"))
    require(len(matrix) == 8 and len({row["failure_tag"] for row in matrix}) == 8, "one-short-matrix")
    negative = subprocess.run(
        ("/bin/bash", "test/negatives/host_shell_pulumi_up.sh"), cwd=ROOT,
        text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False, timeout=60,
    )
    require(negative.returncode == 44 and negative.stdout.strip() == "NoSingletonContext", "host-shell-negative")
...
def evidence_domain(*, fresh: bool) -> None:
    ...
    singleton = live.get("singleton", {})
    require(singleton.get("replicas") == singleton.get("readyReplicas") == singleton.get("availableReplicas") == 1, "singleton-readback")
    require(singleton.get("bespokeElection") is False, "singleton-election")
...
def no_live_residue() -> dict[str, str]:
    namespace = subprocess.run(
        ("/usr/bin/kubectl", "--kubeconfig", str(Path.home() / ".amoebius/phase24/kubeconfig"), "get", "namespace", "phase44-system"),
        text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False, timeout=60,
    )
    require(namespace.returncode != 0, "phase44-namespace-residue")
    require(not Path("/var/tmp/amoebius-phase44-live").exists(), "phase44-root-residue")
    ...
    return {"name": "external-cleanup-readback", "command": "namespace/root/kind inventories", "output": "only retained amoebius-phase24 remains", "result": "PASS"}
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 44,
        "gate_command": "python3 tools/phase44_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.reuse_fresh_live:
            ...
            rows.append(invoke("scoped-live-boundaries", (sys.executable, "tools/phase44_provider_checkpoint_live.py"), timeout=3600))
        evidence_domain(fresh=args.reuse_fresh_live)
        rows.append(no_live_residue())
        rows.append(invoke("sealed-live-reader", live_reader_args(), extra_env={"PHASE44_REUSE_FRESH_LIVE": "1"}))
        rows.extend(reject_mutant(*mutant) for mutant in MUTANTS)
        rows.append(invoke("baseline-restored-contract", contract_args()))
        rows.append(invoke("baseline-restored-live-reader", live_reader_args(), extra_env={"PHASE44_REUSE_FRESH_LIVE": "1"}))
        ...
        print(f"phase44-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']}; AWS/EKS UNVERIFIED)")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"phase44-gate: FAIL: {error}", file=sys.stderr)
        return 1
