...
**Substrate:** none
**Register:** 1
**Gate:** `cabal test example-spec` is green — the committed golden corpus decodes against an independent oracle and one committed seeded mutant turns it red (Gate; 1.2 V1). The apparatus is restated inline here rather than delegated by anchor: the paired positives, the negative catalog entries and their expected tags, the coverage floors, the independent reference table, and each committed mutant with the assertion it must redden.
