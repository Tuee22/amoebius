...
source-snapshot digest and externally attested, and `cabal`, `ghc`, `dhall`, and the browser
resolve from `toolchain/requirements.json` instead of the absolute developer paths this gate
...
HARNESS = ROOT / "test/ui/local/phase23_local_composition.mjs"
RESULTS = ROOT / "gen/dsl/phase23/phase-results.tsv"
GENERATED_LEDGER = ROOT / "gen/dsl/phase23/validation-locus-ledger.tsv"
CONTRACT = "DEVELOPMENT_PLAN/phase_23_ui_local_composition.md"
...
def main() -> int:
    gate = gate_common.PhaseGate(
        phase=23, contract=CONTRACT, command=GATE_COMMAND, register="2", substrate="none", sides=SIDES
    )
    gate.begin()
    ...
    try:
        resolved = toolchain.resolve(["cabal", "ghc", "dhall", "chromium", "purs", "spago"])
        print("toolchain side — the composition lane resolved from authored requirements\n")
        for name in ("cabal", "ghc", "dhall", "chromium", "purs", "spago"):
