...
COMPONENTS = [
    ("alpha", hashlib.sha256(b"content-store-workflow-alpha").digest()),
    ("zeta", hashlib.sha256(b"content-store-workflow-zeta").digest()),
]
...
def head(major: int, length: int) -> bytes:
    ...
    raise ValueError("content-store-workflow-oracle-length-out-of-domain")
...
def encode(value: Any) -> bytes:
    ...
    raise TypeError(f"content-store-workflow-oracle-type:{type(value).__name__}")
