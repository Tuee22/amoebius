...
MUTANTS = (
    ("M-hash-omits-substrate", "phase39-hash-omits-substrate-mutant", "phase39-hash-omits-substrate:"),
    ("M-blind-put", "phase39-blind-put-mutant", "phase39-blind-put:"),
    ("M-gate-admits-unverified", "phase39-gate-admits-unverified-mutant", "phase39-gate-admits-unverified:"),
    ("M-rollout-reorders-retire", "phase39-rollout-reorders-retire-mutant", "phase39-rollout-reorders-retire:"),
    ("M-phase-gate-selfreport", "phase39-phase-gate-selfreport-mutant", "phase39-phase-gate-selfreport:"),
    ("M-scalar-migration-peak", "phase39-scalar-migration-peak-mutant", "phase39-scalar-migration-peak:"),
    ("M-drop-old-schema-on-failure", "phase39-drop-old-schema-on-failure-mutant", "phase39-drop-old-schema-on-failure:"),
    ("M-drop-verification-wal", "phase39-drop-verification-wal-mutant", "phase39-drop-verification-wal:"),
)
...
def reject_mutant(name: str, flag: str, marker: str) -> dict[str, str]:
    ...
    environment = os.environ.copy()
    environment["PHASE39_PURE_ONLY"] = "1"
    result = subprocess.run(
...
def compile_boundary() -> dict[str, str]:
    positive = subprocess.run(
        (CABAL, "exec", "-v0", "--", GHC, "-fno-code", "test/accept/phase_39_environment.hs"),
    ...
    negative = subprocess.run(
        (CABAL, "exec", "-v0", "--", GHC, "-fno-code", "test/reject/fourth_environment.hs"),
        cwd=ROOT, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False, timeout=300,
    )
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 39,
        "gate_command": "python3 tools/phase39_gate.py --reuse-fresh-live",
...
def evidence_domain(*, fresh: bool) -> None:
    ...
    for row in rounds:
        ...
        require([entry.get("condition") for entry in apply_order] == [
            "deployment/phase39-base:Available",
            "job/phase39-migrate:Complete+sql-copy:verified",
            "deployment/phase39-final:Available+old-schema:retired",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows.append(invoke("sealed-live-reader", (
            CABAL, "test", "release-lifecycle-live", *disabled,
            "--test-show-details=direct", "-j1", "-v0",
        ), extra_env={"PHASE39_REUSE_FRESH_LIVE": "1"}))
        ...
        print(f"phase39-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']})")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"phase39-gate: FAIL: {error}", file=sys.stderr)
        return 1
