...
def observe(cache_root: Path, scratch_root: Path, docker_config: Path) -> dict[str, Any]:
    ...
    if any(line.startswith("amoebius-phase25-bounded") for line in builders.splitlines()):
        raise PreflightFailure("unknown-build-commitment:amoebius-phase25-bounded")
