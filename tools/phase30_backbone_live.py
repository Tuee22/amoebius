#!/usr/bin/env python3
"""Exercise the Phase-30 platform backbone against the Phase-24 kind cluster."""
...
PRIVATE_IMAGE = "registry.amoebius.invalid:5000/amoebius/base@" + IMAGE_DIGEST
RETAINED_ROOT = Path("/var/tmp/amoebius-phase28-retained")
EVIDENCE = ROOT / "DEVELOPMENT_PLAN/evidence/phase_30/backbone-live.json"
...
MIB = 1024 * 1024
HOT_TIER_CAP = 65536
DEV_OFFLOADERS = os.environ.get("PHASE30_DEV_OFFLOADERS") == "1"
...
def apply_metallb() -> dict[str, Any]:
    ...
    apply({"apiVersion": "v1", "kind": "Secret", "metadata": {"name": "memberlist", "namespace": METALLB_NAMESPACE}, "type": "Opaque", "stringData": {"secretkey": "phase30-memberlist-test-key-32b"}})
    apply({
    ...
    apply({"apiVersion": "metallb.io/v1beta1", "kind": "IPAddressPool", "metadata": {"name": "phase30", "namespace": METALLB_NAMESPACE}, "spec": {"addresses": ["172.18.255.200-172.18.255.210"]}})
...
def apply_registry(filesystem: bool, source_dir: Path) -> None:
    ...
    apply({
        ...
        "spec": {"replicas": 1, "selector": {"matchLabels": {"app": name}}, "template": {"metadata": {"labels": {"app": name}}, "spec": {"securityContext": {"runAsUser": 0, "runAsGroup": 0}, "containers": [{
            "name": "registry", "image": PRIVATE_IMAGE, "imagePullPolicy": "Never", "command": ["/usr/bin/registry"], "args": ["serve", "/etc/distribution/config.yml"],
...
def assert_vault_unsealed() -> dict[str, Any]:
    ...
    status = status_value()
    unsealed_during_phase30 = False
    if status.get("sealed") and status.get("initialized"):
        ...
        if operator_password is not None and development_password is not None:
            raise BackboneLiveFailure("multiple-vault-operator-password-sources")
        password = operator_password if operator_password is not None else development_password
        binary = RETAINED_ROOT / "mounts/phase29-bin/amoebius"
        envelope = RETAINED_ROOT / "phase29-unlock.age"
        if password is None or not binary.is_file() or not envelope.is_file():
            raise BackboneLiveFailure("vault-sealed-operator-password-required")
        opened = run((str(binary), "vault-open-unlock"), input_bytes=password.encode() + b"\n" + envelope.read_bytes(), timeout=120)
        material = json.loads(opened.stdout)
        unseal_key = material["unseal_key"]
        payload = json.dumps({"key": unseal_key}).encode()
        with port_forward("vault-system", "pod/root-vault-0", 18291, 8200):
            connection = http.client.HTTPConnection("127.0.0.1", 18291, timeout=30)
            try:
                connection.request(
                    "PUT", "/v1/sys/unseal", body=payload,
                    headers={"Content-Type": "application/json", "Content-Length": str(len(payload))},
                )
                response = connection.getresponse()
                unsealed_status = response.status
                unsealed_payload = response.read()
            finally:
                connection.close()
        del material
        del unseal_key
        del payload
        if unsealed_status != 200:
            raise BackboneLiveFailure(f"vault-operator-unseal-failed:{unsealed_status}")
        status = json.loads(unsealed_payload)
        unsealed_during_phase30 = True
    ...
    return {
        ...
        "unsealedDuringPhase30": unsealed_during_phase30,
    }
...
def native_dedup_probe(topic: str) -> str:
    ...
    return "phase30-dedup-probe: PASS native=true sequenceIds=7,7,8 delivered=2 cborByteIdentical=true"
...
def fresh_haskell_render_plan() -> list[dict[str, Any]]:
    binary = text(run((CABAL, "list-bin", "phase30-backbone-spec"))).strip()
    if not binary:
...
def main() -> int:
    try:
        evidence = execute()
        print(f"phase30-backbone-live: PASS (MetalLB, four-drive MinIO, registry {evidence['registryRehome']['backend']}, Pulsar HA/offload)")
        return 0
    except (BackboneLiveFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired, ElementTree.ParseError) as problem:
        print(f"phase30-backbone-live: FAIL: {problem}", file=sys.stderr)
        return 1
