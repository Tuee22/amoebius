...
`tools/phase25_sprint25_1_gate.py` audits four things it does not create: the host
...
The render happens first and is compared to `test/fixtures/phase25/Dockerfile.golden`
...
GOLDEN = ROOT / "test/fixtures/phase25/Dockerfile.golden"
BUILDKIT_CONFIG = ROOT / "test/fixtures/phase25/buildkitd.toml"
BUILDER_NAME = "amoebius-phase25-bounded"
BUILDKIT_CONTAINER = "amoebius-phase25-buildkitd"
STATE_VOLUME = "amoebius-phase25-buildkit-state"
...
def preflight(evidence: Path, cache_root: Path, scratch_root: Path, docker_config: Path) -> dict[str, Any]:
    ...
    run((sys.executable, "tools/phase25_build_preflight.py",
...
def load_platform(artifact: Path, arch: str, docker_config: Path, staging: Path) -> str:
    ...
    envelope.
    """
    reference = f"amoebius-phase25-final:{arch}"
...
def probe(reference: str, arch: str, evidence: Path) -> None:
    run((sys.executable, "tools/phase25_source_probe.py",
