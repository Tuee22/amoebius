...
R=Path(__file__).resolve().parents[1];E=R/"DEVELOPMENT_PLAN/evidence/phase_56";N=R/"test/enumeration/phase_56_surfaces.txt";L=R/"test/golden/phase_56_ledger.json";C="/home/matthewnowak/.ghcup/bin/cabal";G="/home/matthewnowak/.ghcup/ghc/9.12.4/bin/ghc";F=("phase56-accept-unlisted-choice-mutant","phase56-drop-tenant-key-mutant","phase56-drop-user-key-mutant","phase56-drop-scope-epoch-mutant");U={"keycloak-real-sessions","real-browser-tenant-switch","provider-zero-delta-observers","kubernetes-audit","foreign-pod-cni-probe","redis-realtime-scope-change"}
...
def derive():
 s=[x for x in N.read_text().splitlines() if x];v={"phase":56,"gate_command":"python3 tools/phase56_gate.py","register":"3","substrate":"linux-cpu","date":"2026-08-11","layers":[{"name":"Decision","status":"tested"},{"name":"Protocol","status":"tested"},{"name":"Runtime","status":"tested"}],"coverage":[{"surface":x,"status":"UNVERIFIED" if x in U else "tested"} for x in s]};v["ledger_hash"]=fp(v);return v
def main():
 if "--derive-ledger" in sys.argv:print(json.dumps(derive(),separators=(",",":")));return 0
 rows=[];p=run(cmd());
 if p.returncode:print(p.stdout);return 1
 rows.append(("contract","PASS"));p=run((sys.executable,"tools/phase56_ui_multi_tenant_live.py"));
 if p.returncode:print(p.stdout);return 1
