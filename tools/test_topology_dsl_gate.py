...
FLAGS=("test-topology-dsl-skip-teardown-mutant","test-topology-dsl-tag-query-mutant","test-topology-dsl-all-tested-mutant","test-topology-dsl-wrong-subscription-mutant")
...
def derive():
    ...
    v={"phase":54,"gate_command":"python3 tools/test_topology_dsl_gate.py","register":"3","substrate":"per generated test","date":"2026-08-11","layers":[{"name":"Decision","status":"tested"},{"name":"Protocol","status":"tested"},{"name":"Runtime","status":"tested"}],"coverage":[{"surface":s,"status":"UNVERIFIED" if s in UNVERIFIED else "tested"} for s in surfaces]}; v["ledger_hash"]=canon(v); return v
def main():
  try:
    ...
    rows.append(invoke("topology-dhall",(DHALL,"type","--file","test/fixture/dhall/test_topology_dsl/failover.dhall","--quiet")))
    bad=subprocess.run((DHALL,"type","--file","test/fixture/dhall/test_topology_dsl/pvc_binding_illegal.dhall","--quiet"),cwd=ROOT,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT); require(bad.returncode!=0 and "Assertion failed" in bad.stdout,"illegal-pvc")
    rows.append({"name":"illegal-pvc","command":"dhall type illegal","output":"Assertion failed","result":"RED"})
    rows.append(invoke("contract",contract())); rows.append(invoke("live",(sys.executable,"tools/test_topology_dsl_live.py")))
    ...
    print(f"test-topology-dsl-gate: PASS-SCOPED ({len(rows)} checks; {d['ledger_hash']}; {receipt['receiptFingerprint']}; Kubernetes/Pulsar/retained-PV/Vault/AWS UNVERIFIED)"); return 0
  except (Failure,OSError,ValueError,KeyError,subprocess.TimeoutExpired) as e: print(f"test-topology-dsl-gate: FAIL: {e}",file=sys.stderr); return 1
