#!/usr/bin/env python3
"""Acceptance gate for Phase 64's scoped offline multi-zone continuity."""
...
FLAGS = (
    "phase64-fault-one-pod-mutant", "phase64-sticky-routing-mutant",
    "phase64-redis-receipt-mutant", "phase64-skip-cursor-repair-mutant",
    "phase64-prefault-authority-mutant", "phase64-drop-outbox-scope-mutant",
    "phase64-clear-state-release-mutant", "phase64-duplicate-blob-dependency-mutant",
)
UNVERIFIED = {
...
def derive_ledger():
    ...
    value = {
        "phase": 64,
        "gate_command": "python3 tools/phase64_gate.py",
...
def main():
    ...
    result = run((sys.executable, "tools/phase64_continuity_live.py"))
    if result.returncode:
        print(result.stdout)
        return 1
    ...
    print(f"phase64-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; "
