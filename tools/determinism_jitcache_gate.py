...
FLAGS = (
    "determinism-jitcache-content-order-leak-mutant", "determinism-jitcache-const-fingerprint-mutant",
    "determinism-jitcache-rng-workerid-mutant", "determinism-jitcache-const-output-mutant",
    "determinism-jitcache-fixed-marker-mutant", "determinism-jitcache-prune-noop-mutant", "determinism-jitcache-one-byte-short-mutant",
)
MUTANTS = {
    "determinism-jitcache-content-order-leak-mutant": "mut-48-content-address-field-order-leak",
    "determinism-jitcache-const-fingerprint-mutant": "mut-48-experiment-hash-const-fingerprint",
    "determinism-jitcache-rng-workerid-mutant": "mut-48-rng-workerid-mixed",
    "determinism-jitcache-const-output-mutant": "mut-48-determinism-const-output",
    "determinism-jitcache-fixed-marker-mutant": "mut-48-cache-fixed-marker",
    "determinism-jitcache-prune-noop-mutant": "mut-48-cache-prune-noop",
    "determinism-jitcache-one-byte-short-mutant": "mut-48-cache-one-byte-short",
}
UNVERIFIED = {
...
def oracle_domain() -> dict[str, str]:
    ...
    alt_seed = load_dhall("test/fixture/dhall/determinism_jitcache/determinism_repro_alt_seed.dhall")
    alt_input = load_dhall("test/fixture/dhall/determinism_jitcache/determinism_repro_alt_input.dhall")
    cache = load_dhall("test/fixture/dhall/determinism_jitcache/engine_cache.dhall")
    ...
    engine = load_dhall("test/oracle/determinism_jitcache/oracle.dhall")
    recipe = subprocess.run((str(ROOT / "test/oracle/determinism_jitcache/engine_source.sh"),), cwd=ROOT, stdout=subprocess.PIPE, check=False, timeout=30).stdout
    ...
    seeds = json.loads((ROOT / "test/golden/determinism_jitcache/splitmix_seeds.json").read_text(encoding="utf-8"))
    ...
    schema = json.loads((ROOT / "test/golden/determinism_jitcache/substrate_fingerprint.schema.json").read_text(encoding="utf-8"))
    ...
    equivalence = list(csv.DictReader((ROOT / "test/oracle/determinism_jitcache/logical_equivalence.tsv").open(encoding="utf-8"), delimiter="\t"))
    ...
    expected_negatives = {
        "test/negative/determinism_jitcache/cache_digest_size_conflict.dhall": "ResidentSizeConflict",
        "test/negative/determinism_jitcache/cache_deletion_credit.dhall": "DeletionNotObserved",
        "test/negative/determinism_jitcache/cache_concurrency_overflow.dhall": "CachePeakExceedsBudget",
        "test/negative/determinism_jitcache/ephemeral_under_reserved.dhall": "OwnerEphemeralUnderReserved",
    }
...
def foreclosure_domain() -> list[dict[str, str]]:
    ...
    rows = [invoke("content-address-positive", (*prefix, "test/negative/compile_fail/determinism_jitcache/content_address_positive.hs"), timeout=300)]
    rows.append(invoke("catalog-identity-positive", (*prefix, "test/negative/determinism_jitcache/catalog_identity_positive.hs"), timeout=300))
    negatives = {
        "forge-content-address-red": ("test/negative/compile_fail/determinism_jitcache/forge_blobsha.hs", "BlobSha"),
        "free-cache-key-red": ("test/negative/determinism_jitcache/freestring_key.hs", "CacheKey"),
        "free-url-arm-red": ("test/negative/determinism_jitcache/url_arm.hs", "Url"),
    }
...
def no_live_residue() -> dict[str, str]:
    namespace = subprocess.run((KUBECTL, "--kubeconfig", str(Path.home() / ".amoebius/phase24/kubeconfig"), "get", "namespace", "determinism-jitcache-system"), text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False, timeout=60)
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 48, "gate_command": "python3 tools/determinism_jitcache_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.derive_ledger:
            ...
            return 0
        rows = [invoke("determinism-jitcache-source-build", (CABAL, "build", "amoebius:dsl-core", "-w", GHC, *flags(), "-j1", "-v0"))]
        ...
        print(f"determinism-jitcache-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']}; cross-substrate/cross-node/Tier-2/Tier-3 UNVERIFIED)")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"determinism-jitcache-gate: FAIL: {error}", file=sys.stderr)
        return 1
