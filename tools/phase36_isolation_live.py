...
MATRIX = ROOT / "test/fixtures/live_isolation/user_tenant_access_matrix.tsv"
KEYCLOAK_PORT = 18087
...
def apply(value: dict[str, Any]) -> None:
    kubectl("apply", "--server-side", "--field-manager=phase36-harness", "--force-conflicts", "-f", "-", stdin=json.dumps(value))
...
def keycloak_admin() -> str:
    ...
    return str(form_request("/realms/master/protocol/openid-connect/token", {
        "grant_type": "password", "client_id": "admin-cli", "username": "phase32-admin", "password": password,
    })["access_token"])
...
"""
    postgres_exec(pod, "export PGPASSWORD=\"$(cat /phase31-secrets/superuser)\"; /usr/lib/postgresql/17/bin/psql -h 127.0.0.1 -U postgres -d postgres -v ON_ERROR_STOP=1 <<'SQL'\n" + admin_sql + "\nSQL")
    ...
    observer = postgres_exec(pod, f"export PGPASSWORD=\"$(cat /phase31-secrets/superuser)\"; /usr/lib/postgresql/17/bin/psql -h 127.0.0.1 -U postgres -d postgres -AtF '|' -v ON_ERROR_STOP=1 -c \"SELECT id,tenant,owner,value,version FROM {schema}.items ORDER BY id\"").strip().splitlines()
...
def cleanup_postgres_stale() -> None:
    ...
    output = postgres_exec(pod, "export PGPASSWORD=\"$(cat /phase31-secrets/superuser)\"; /usr/lib/postgresql/17/bin/psql -h 127.0.0.1 -U postgres -d postgres -qAt -v ON_ERROR_STOP=1 -c \"" + query + "\"")
...
def pod_manifest(namespace: str, name: str, role: str) -> dict[str, Any]:
    return {
        "apiVersion": "v1", "kind": "Pod", "metadata": {"name": name, "namespace": namespace, "labels": {"app": name, "amoebius.io/phase36-role": role}},
...
def setup_network(suffix: str) -> dict[str, Any]:
    ...
    for namespace, role in ((sut_ns, "sut"), (foreign_ns, "foreign")):
        apply({"apiVersion": "v1", "kind": "Namespace", "metadata": {"name": namespace, "labels": {"amoebius.io/phase36-role": role, "amoebius.io/run": suffix}}})
    ...
    apply({"apiVersion": "networking.k8s.io/v1", "kind": "NetworkPolicy", "metadata": {"name": "sut-provider-egress", "namespace": sut_ns}, "spec": {"podSelector": {"matchLabels": {"amoebius.io/phase36-role": "sut"}}, "policyTypes": ["Egress"], "egress": [
...
def setup() -> dict[str, Any]:
    ...
    suffix = challenge[:8]
    realm = "phase36-" + suffix
    state_path = Path(f"/tmp/amoebius-phase36-state-{suffix}.json")
    cleanup_stale()
...
def cleanup_state(state: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        postgres_exec(state["postgres"]["pod"], f"export PGPASSWORD=\"$(cat /phase31-secrets/superuser)\"; /usr/lib/postgresql/17/bin/psql -h 127.0.0.1 -U postgres -d postgres -v ON_ERROR_STOP=1 -c 'DROP SCHEMA IF EXISTS {state['postgres']['schema']} CASCADE' -c 'DROP ROLE IF EXISTS {state['postgres']['role']}'")
...
def cleanup_stale() -> None:
    for path in Path("/tmp").glob("amoebius-phase36-state-*.json"):
        try:
            state = json.loads(path.read_text(encoding="utf-8"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "setup":
            ...
            print("phase36-live-finish: PASS")
        else:
            cleanup(args.state)
            print("phase36-live-cleanup: PASS")
        return 0
    except (LiveFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"phase36-live: FAIL: {error}", file=sys.stderr)
        return 1
