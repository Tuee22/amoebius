...
SIBLING = RETAINED_ROOT / "images/sibling.ext4"
...
def kubectl(*arguments: str, input_text: str | None = None, check: bool = True, timeout: int = 240) -> subprocess.CompletedProcess[str]:
    return run(("/usr/bin/kubectl", "--kubeconfig", str(KUBECONFIG), *arguments), input_text=input_text, check=check, timeout=timeout)
...
def read_claim_oracle() -> dict[str, str]:
    with (FIXTURES / "claimref_table.csv").open(encoding="utf-8", newline="") as source:
...
def cleanup_kubernetes() -> dict[str, bool]:
    ...
    run(("/usr/bin/docker", "exec", NODE, "sh", "-c", f"rm -rf {NODE_PATH}"), check=False)
    return {
...
def observe_hard_ceiling(raw_bytes: int) -> dict[str, Any]:
    ...
    ephemeral_before = shutil.disk_usage("/var/lib/docker").used
    ...
    ephemeral_after = shutil.disk_usage("/var/lib/docker").used
    ...
    statefulset = {
        ...
        "spec": {
            ...
            "template": {"metadata": {"labels": {"app": STATEFULSET}}, "spec": {
                "containers": [{"name": "witness", "image": PRIVATE_IMAGE, "imagePullPolicy": "IfNotPresent", "command": ["/usr/bin/minio"],
...
def execute() -> dict[str, Any]:
    ...
    durable = 536870912
    full_debit = sum(int(item["provisioned_bytes"]) for item in csv.DictReader((FIXTURES / "claimref_table.csv").open(encoding="utf-8", newline="")))
    ...
    retained = make_ext4_image(IMAGE, MOUNT, raw_bytes)
    ...
    run(("/usr/bin/truncate", "-s", str(raw_bytes), str(SIBLING)))
    hard_ceiling = observe_hard_ceiling(raw_bytes)
    one_short = raw_bytes - 1
    wrong_fs = "xfs"

    run(("/usr/bin/docker", "exec", NODE, "sh", "-c", f"mkdir -p {NODE_PATH} && chmod 0777 {NODE_PATH}"))
    ...
    nonce = "phase28-" + secrets.token_hex(16)
    ...
    return {
        "schema": "amoebius.phase28.sprint28.2-live.v1", "register": 3, "substrate": "linux-cpu",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        cleanup_kubernetes()
        print(f"phase28-sprint28.2-live: FAIL: {problem}\n{debug}", file=sys.stderr)
        return 1
