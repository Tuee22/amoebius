...
PHASE49_RECEIPT = ROOT / "DEVELOPMENT_PLAN/evidence/phase_49/phase-receipt.json"
...
FLAGS = ("phase50-trust-client-artifact-scope-mutant", "phase50-drop-command-id-from-terminal-mutant")
MUTANTS = {
    "phase50-trust-client-artifact-scope-mutant": "mut-50-trust-client-artifact-scope",
    "phase50-drop-command-id-from-terminal-mutant": "mut-50-drop-command-id-from-terminal",
}
UNVERIFIED = {
    "browser-through-envoy-to-ui-server", "kubernetes-ui-server-replicas", "phase50-native-cbor-chain",
    "full-phase49-inference-output-correspondence", "production-tinyllama-inference",
...
def oracle_domain() -> dict[str, str]:
    decoded = subprocess.run(
        (DHALL_TO_JSON, "--file", "test/dhall/phase_50/infernix_ui.dhall"), cwd=ROOT,
        text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False, timeout=120,
    )
    require(decoded.returncode == 0, f"phase50-dhall-oracle:{decoded.stdout}")
    value = json.loads(decoded.stdout)
    require(value == {"program": "infernix-ui", "ports": ["WorkflowProgress", "ArtifactProvenance", "ModelInteractor"], "mode": "SingleTenant"}, "phase50-dhall-domain")
    model_rows = list(csv.DictReader((ROOT / "test/fixtures/phase_50/public_model_input.tsv").open(encoding="utf-8"), delimiter="\t"))
    require(model_rows == [
    ...
    scope = list(csv.DictReader((ROOT / "test/fixtures/phase_50/scope_matrix.tsv").open(encoding="utf-8"), delimiter="\t"))
    ...
    interaction = list(csv.DictReader((ROOT / "test/fixtures/phase_50/expected_interaction.tsv").open(encoding="utf-8"), delimiter="\t"))
    require([row["event"] for row in interaction] == ["start", "progress", "ready", "invoke"], "interaction-oracle")
    identity = list(csv.DictReader((ROOT / "test/fixtures/phase_50/terminal_receipt_identity.tsv").open(encoding="utf-8"), delimiter="\t"))
    ...
    contract = (ROOT / "test/fixtures/phase_50/public_contract.golden").read_text(encoding="utf-8")
...
def evidence_domain(*, fresh: bool) -> None:
    ...
    phase49 = json.loads(PHASE49_RECEIPT.read_text(encoding="utf-8"))
    require(live["prerequisite"] == {"phase49ReceiptFingerprint": phase49["receiptFingerprint"], "phase49Result": "PASS-SCOPED"}, "phase49-prerequisite")
...
def external_cleanup() -> dict[str, str]:
    ...
    with phase34.port_forward("edge-system", "service/keycloak", phase36.KEYCLOAK_PORT, 8080):
        ...
        require(not any(str(row.get("realm", "")).startswith("phase50-") for row in realms), "phase50-keycloak-residue")
    with phase34.port_forward("platform-system", "service/minio", phase37.MINIO_PORT, 9000):
        status, payload, _ = phase37.s3_request("GET", "")
        require(status == 200 and b"<Name>p50-" not in payload, "phase50-minio-residue")
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 50,
        "gate_command": "python3 tools/phase50_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.derive_ledger:
            ...
            return 0
        disabled = flags()
        rows = [invoke("phase50-source-build", (CABAL, "build", "infernix-ui-lift:lib:infernix-ui-lift", "infernix-ui-lift-contract", "infernix-ui-lift-live-gate", "-w", GHC, *disabled, "-j1", "-v0"))]
        ...
        if args.reuse_fresh_live:
            ...
            rows.append(invoke("infernix-ui-live", (sys.executable, "tools/phase50_infernix_ui_live.py"), timeout=2400))
        ...
        print(f"phase50-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']}; full edge/replica/native-CBOR/production inference chain UNVERIFIED)")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"phase50-gate: FAIL: {error}", file=sys.stderr)
        return 1
