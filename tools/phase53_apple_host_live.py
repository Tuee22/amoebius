...
REFERENCE = ROOT / "test/golden/phase_53/metal_job_ref.py"
...
def main() -> int:
    expected_a = bytes.fromhex((ROOT / "test/golden/phase_53/job_A.expected").read_text().strip())
    expected_b = bytes.fromhex((ROOT / "test/golden/phase_53/job_B.expected").read_text().strip())
    observed_a = reference(ROOT / "test/golden/phase_53/job_A.input")
    observed_b = reference(ROOT / "test/golden/phase_53/job_B.input")
    with tempfile.TemporaryDirectory(prefix="amoebius-phase53-") as temporary:
    ...
    print(f"phase53-apple-host-live: PASS-SCOPED ({evidence['evidenceDigest']}; physical Apple/Lima/Metal UNVERIFIED)")
    return 0
