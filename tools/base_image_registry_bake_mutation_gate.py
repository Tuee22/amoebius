...
TEMP_ROOT = ROOT / ".build/tmp/base-image-registry-mutants"
MUTANTS = ROOT / "test/mutant/base_image_registry"
...
SOURCE = load_module("base_image_registry_source_probe", ROOT / "tools/base_image_registry_source_probe.py")
...
def scratch_accounting(_mutation: dict[str, str], _rows: dict[tuple[str, str], dict[str, Any]]) -> str:
    envelope = SOURCE.decode_dhall(ROOT / "test/fixture/base_image_registry/build_execution_envelope.dhall")
...
def omit_pulsar_offloaders(
    _mutation: dict[str, str], _rows: dict[tuple[str, str], dict[str, Any]]
) -> str:
    catalog = SOURCE.catalog_steps(SOURCE.decode_dhall(SOURCE.CATALOG))
    expected = {
        (row["owner"], row["payload"], row["target_root"])
        for row in SOURCE.published_payload_oracle()
    }
    observed = {
        (owner, str(payload.get("name", "")), str(payload.get("targetPath", "")))
        for owner, artifact in catalog.items()
        for payload in artifact.get("payloads", [])
    }
    mutated = {row for row in observed if row[1] != "pulsar-offloaders"}
    return "PublishedPayloadMissing" if expected - mutated else "SURVIVED"


def dockerfile_handedit(_mutation: dict[str, str], _rows: dict[tuple[str, str], dict[str, Any]]) -> str:
    golden = (ROOT / "test/fixture/base_image_registry/Dockerfile.golden").read_bytes()
...
def unbounded_worker(_mutation: dict[str, str], _rows: dict[tuple[str, str], dict[str, Any]]) -> str:
    decoded = json.loads(
        subprocess.run(
            (DOCKER, "inspect", "amoebius-base-image-registry-buildkitd"),
...
def authored_rungs() -> dict[str, str]:
    rows: dict[str, str] = {}
    for line in (ROOT / "test/fixture/base_image_registry/acquisition_rungs.tsv").read_text(encoding="utf-8").splitlines():
...
def scavenge_available_apt_rung(
    ...
    with tempfile.TemporaryDirectory(prefix="catalog-", dir=TEMP_ROOT) as temporary:
...
def run_gate(join: Path) -> dict[str, Any]:
    ...
    handlers: dict[str, Callable[[dict[str, str], dict[tuple[str, str], dict[str, Any]]], str]] = {
        ...
        "omit-pulsar-offloaders": omit_pulsar_offloaders,
