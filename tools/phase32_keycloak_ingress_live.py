...
PHASE31_EVIDENCE = ROOT / "DEVELOPMENT_PLAN/evidence/phase_31/services-live.json"
ROUTE_ORACLE = ROOT / "test/fixtures/phase32/route-inventory.golden"
REALM_FIXTURE = ROOT / "test/fixtures/phase32/realm.json"
BACKDOOR_SEED = ROOT / "test/fixtures/phase32/backdoor-seed.yaml"
MARKER_ROW = ROOT / "test/fixtures/phase32/marker-row.sql"
MARKER_OBJECT = ROOT / "test/fixtures/phase32/marker-object.bin"
POLICY_ORACLE = ROOT / "test/fixtures/phase32/netpol-expected.golden"
...
def ensure_prerequisites() -> dict[str, Any]:
    ...
    if not PHASE31_EVIDENCE.is_file():
        raise EdgeLiveFailure("phase31-live-evidence-absent")
    phase31_live = json.loads(PHASE31_EVIDENCE.read_text(encoding="utf-8"))
    if phase31_live.get("schema") != "amoebius.phase31.services-live.v1":
        raise EdgeLiveFailure("phase31-live-evidence-schema")
    expected = {
    ...
    for name, (ns, object_name) in expected.items():
        ...
        if result.returncode:
            raise EdgeLiveFailure(f"phase31-prerequisite-absent:{name}")
        ready[name] = "observed"
    if phase31_live.get("postgres", {}).get("readyReplicas") != 3:
        raise EdgeLiveFailure("phase31-patroni-not-three-ready")
    return {
...
def vault_material() -> tuple[dict[str, str], dict[str, Any]]:
    ...
    with phase29.port_forward():
        ...
        if status == 200:
            ...
            raise EdgeLiveFailure(f"vault-phase32-kv-read:{status}")
        for key in (
        ...
        if any(not generated.get(key) for key in ("tls.crt", "tls.key", "ca.crt", "certificateSerial")):
            ...
            if any(not certificate.get(key) for key in required):
                raise EdgeLiveFailure("vault-phase32-pki-material-incomplete")
            generated.update({
    ...
    if observed != generated:
        raise EdgeLiveFailure("vault-phase32-readback-drift")
...
def apply_secret_material(material: dict[str, str]) -> None:
    apply({
        ...
        "metadata": {
            "name": "phase32-edge-secrets", "namespace": EDGE_NAMESPACE,
            "annotations": {
...
def prepare_keycloak_database(
    ...
    backings = [
        phase31.prepare_node_backing(
            f"phase32-keycloak-postgres-256-{ordinal}", 256 * phase31.MIB,
        )
                ...
                "spec": {
                    ...
                    "containers": [{
                        ...
                        "volumeMounts": [
                            ...
                            {"name": "credentials", "mountPath": "/phase32-secrets", "readOnly": True},
...
export PGPASSWORD="$(cat /phase32-secrets/superuser)"
    ...
    apply({
        ...
        "spec": {
            ...
            "listeners": [{
                ...
                "tls": {
                    "mode": "Terminate",
                    "certificateRefs": [{"kind": "Secret", "name": "phase32-edge-secrets"}],
                },
...
def apply_envoy_gateway_controller() -> dict[str, Any]:
    ...
    apply({
        ...
        "metadata": {"name": "phase32-envoy-gateway-observer"},
        "rules": [{
            "apiGroups": ["*"], "resources": ["*"],
    ...
    apply({
        ...
        "roleRef": {
            ...
            "name": "phase32-envoy-gateway-observer",
        },
        "subjects": [{
        ...
        "spec": {
            ...
            "template": {
                ...
                "spec": {
                    ...
                    "volumes": [
                        {"name": "xds-certs", "secret": {"secretName": "phase32-edge-secrets"}},
...
def apply_route_probe() -> None:
    apply({
        "apiVersion": "v1", "kind": "ConfigMap",
        "metadata": {"name": "phase32-route-probe", "namespace": EDGE_NAMESPACE},
    ...
    apply({
        ...
        "spec": {
            "replicas": 1,
            "selector": {"matchLabels": {"app": "phase32-route-probe"}},
            "template": {
                "metadata": {"labels": {"app": "phase32-route-probe", "surface": "platform-api-websocket"}},
                "spec": {"containers": [{
                ...
                }], "volumes": [{"name": "program", "configMap": {"name": "phase32-route-probe"}}]},
            },
        },
    })
    apply({
        ...
        "metadata": {"name": "phase32-route-probe", "namespace": EDGE_NAMESPACE},
        "spec": {
            "type": "ClusterIP", "selector": {"app": "phase32-route-probe"},
            "ports": [{"name": "http", "port": 8080, "targetPort": 8080}],
        },
    })
    kubectl(
        "-n", EDGE_NAMESPACE, "rollout", "status", "deployment/phase32-route-probe",
        "--timeout=180s", timeout=200,
    )
...
def apply_keycloak() -> dict[str, Any]:
    ...
    apply({
        "apiVersion": "v1", "kind": "ConfigMap",
        "metadata": {"name": "phase32-keycloak-realm", "namespace": EDGE_NAMESPACE},
                ...
                "spec": {
                    ...
                    "containers": [{
                        ...
                        "env": [
                            ...
                            {"name": "KC_DB_PASSWORD", "valueFrom": {"secretKeyRef": {"name": "phase32-edge-secrets", "key": "keycloak-database"}}},
                            {"name": "KC_BOOTSTRAP_ADMIN_USERNAME", "value": "phase32-admin"},
                            {"name": "KC_BOOTSTRAP_ADMIN_PASSWORD", "valueFrom": {"secretKeyRef": {"name": "phase32-edge-secrets", "key": "keycloak-admin"}}},
                    ...
                    "volumes": [{"name": "realm", "configMap": {"name": "phase32-keycloak-realm"}}],
                },
            },
        },
    })
...
USER_ID="$(/opt/keycloak/bin/kcadm.sh get users -r amoebius -q username=phase32-tester --fields id --format csv --noquotes | head -n 1)"
test -n "$USER_ID"
/opt/keycloak/bin/kcadm.sh update "users/${USER_ID}" -r amoebius \
  -s firstName=Phase32 -s lastName=Tester \
  -s email=phase32-tester@amoebius.invalid -s emailVerified=true \
...
def envoy_config() -> str:
  ...
  - name: route_probe
    ...
    load_assignment:
      ...
      endpoints: [{{lb_endpoints: [{{endpoint: {{address: {{socket_address: {{address: phase32-route-probe.{EDGE_NAMESPACE}.svc.cluster.local, port_value: 8080}}}}}}}}]}}]
admin:
  address:
...
def apply_envoy(*, load_balancer: bool) -> None:
    ...
    apply({
        "apiVersion": "v1", "kind": "ConfigMap",
        "metadata": {"name": "phase32-envoy", "namespace": EDGE_NAMESPACE},
        ...
        "spec": {
            ...
            "template": {
                ...
                "spec": {"containers": [{
                    ...
                    {"name": "config", "configMap": {"name": "phase32-envoy"}},
                    {"name": "tls", "secret": {"secretName": "phase32-edge-secrets"}},
                ]},
            },
        },
    })
...
def apply_acme_recording_job() -> dict[str, Any]:
    job_name = "phase32-acme-provenance"
    kubectl(
...
'''
    apply({
        "apiVersion": "v1", "kind": "ConfigMap",
        "metadata": {"name": "phase32-acme-recording-shim", "namespace": EDGE_NAMESPACE},
        ...
        "spec": {
            ...
            "template": {
                ...
                "spec": {
                    ...
                    "volumes": [
                        {"name": "shim", "configMap": {"name": "phase32-acme-recording-shim"}},
                        {"name": "vault-material", "secret": {
                            "secretName": "phase32-edge-secrets",
    ...
    secret = json.loads(text(kubectl(
        "-n", EDGE_NAMESPACE, "get", "secret", "phase32-edge-secrets", "-o", "json",
    )))
...
def apply_edge_network_policies() -> None:
    policies = [
        ...
        },
        {
            ...
            "spec": {
                ...
                "egress": [
                    ...
                    {"to": [{"podSelector": {"matchLabels": {"app": "phase32-route-probe"}}}], "ports": [{"protocol": "TCP", "port": 8080}]},
        ...
        },
        {
            ...
            "spec": {
                "podSelector": {"matchLabels": {"app": "phase32-route-probe"}}, "policyTypes": ["Ingress"],
                "ingress": [{"from": [{"podSelector": {"matchLabels": {"app": "envoy"}}}], "ports": [{"protocol": "TCP", "port": 8080}]}],
            },
        },
        {
...
def graph_variation_probe() -> dict[str, Any]:
    scratch_namespace = "phase32-scratch"
...
def scanner_seed_drill() -> dict[str, Any]:
    ...
    seeded_name = "Service/edge-system/phase32-seeded-backdoor:NodePort"
...
def configure_host_local_nodeport(origin_pod: tuple[str, str]) -> dict[str, Any]:
    ...
    apply({
        "apiVersion": "v1", "kind": "Service",
        "metadata": {
            "name": "phase32-host-local", "namespace": EDGE_NAMESPACE,
            "labels": {"amoebius.io/endpoint-class": "HostLocalPeer"},
        },
        "spec": {
            "type": "NodePort", "selector": {"app": "phase32-route-probe"},
            "ports": [{
    ...
    return {
        "service": "phase32-host-local", "nodePort": HOST_LOCAL_NODE_PORT,
...
def obtain_oidc_token(host: str = EDGE_VIP, port: int = EDGE_PORT) -> str:
    ...
    form = urllib.parse.urlencode({
        "grant_type": "password", "client_id": "phase32-probe",
...
def websocket_attempt(
    ...
    return {
        ...
        "challengeMatched": challenge == b"phase32-challenge",
    }


def websocket_drill(token: str) -> dict[str, Any]:
    nonce = "phase32-" + secrets.token_hex(20)
    ...
    wrong_origin = websocket_attempt(
        ...
        nonce="phase32-" + secrets.token_hex(20), subprotocol="amoebius.v1",
    )
    wrong_subprotocol = websocket_attempt(
        EDGE_VIP, EDGE_PORT, token, origin=f"https://{EDGE_HOST}",
        nonce="phase32-" + secrets.token_hex(20), subprotocol="amoebius.v0",
    )
    ...
    if not (
        valid == {
            "status": 101,
            "challengeSha256": hashlib.sha256(b"phase32-challenge").hexdigest(),
...
def direct_backend_denial(origin_pod: tuple[str, str]) -> dict[str, Any]:
    ...
    target = f"http://phase32-route-probe.{EDGE_NAMESPACE}.svc.cluster.local:8080/platform/"
    ...
    logs = text(kubectl(
        "-n", EDGE_NAMESPACE, "logs", "deployment/phase32-route-probe", "--tail=200",
        check=False,
    ))
    return {
        "source": f"{namespace_name}/{pod}", "target": "phase32-route-probe:8080",
...
def run_rebind_regression() -> dict[str, Any]:
    script = ROOT / "tools/phase32_rebind_regression.py"
    evidence_path = ROOT / "DEVELOPMENT_PLAN/evidence/phase_32/rebind-regression.json"
    if os.environ.get("PHASE32_REUSE_FRESH_REBIND") == "1":
        if not evidence_path.is_file() or time.time() - evidence_path.stat().st_mtime > 7200:
            raise EdgeLiveFailure("phase32-fresh-rebind-evidence-required")
        ...
        if (
            ...
            raise EdgeLiveFailure("phase32-reused-rebind-evidence-domain")
        return {**evidence, "reusedFreshEvidence": True}
    if not script.is_file():
        raise EdgeLiveFailure("phase32-rebind-regression-script-absent")
    result = run((sys.executable, str(script)), timeout=1800)
    if not evidence_path.is_file():
        raise EdgeLiveFailure("phase32-rebind-regression-evidence-absent")
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
    if (
        evidence.get("schema") != "amoebius.phase32.rebind-regression.v1"
        or not evidence.get("freshCluster", {}).get("allIdentitiesChanged")
        ...
        raise EdgeLiveFailure(f"phase32-rebind-regression-domain:{text(result)}")
    return evidence
...
def execute() -> dict[str, Any]:
    ...
    wan_pod = ensure_origin_pod("phase32-wan", "wan")
    lan_pod = ensure_origin_pod("phase32-lan", "lan")
    ...
    wan_routes = pod_route_probe("phase32-wan", wan_pod, token)
    lan_routes = pod_route_probe("phase32-lan", lan_pod, token)
    with edge_port_forward():
        localhost_routes = probe_http_routes("127.0.0.1", 19443, token)
    websocket = websocket_drill(token)
    del token
    host_local = configure_host_local_nodeport(("phase32-wan", wan_pod))
    ...
    direct_denial = direct_backend_denial(("phase32-wan", wan_pod))
    ...
    return {
        ...
        "routeInventory": {
            ...
            "origins": {
                "wan": {"namespace": "phase32-wan", "pod": wan_pod, "statuses": wan_routes},
                "lan": {"namespace": "phase32-lan", "pod": lan_pod, "statuses": lan_routes},
    ...
    try:
        if sys.argv[1:] == ["--refresh-provenance"]:
            if not EVIDENCE.is_file():
                raise EdgeLiveFailure("phase32-live-evidence-absent")
            ...
            print("phase32-keycloak-ingress-live: PASS (provenance refreshed)")
            return 0
        if sys.argv[1:]:
        ...
        print(
            "phase32-keycloak-ingress-live: PASS "
        ...
        print(f"phase32-keycloak-ingress-live: FAIL: {problem}", file=sys.stderr)
        return 1
