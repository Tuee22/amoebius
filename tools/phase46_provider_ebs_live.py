...
NAMESPACE = "phase46-system"
STATIC_PV = "phase46-static-ebs-object"
...
def reset() -> None:
    ...
    for name in (STATIC_PV, "phase46-retained-a", "phase46-retained-b"):
...
def static_ebs_object() -> dict[str, Any]:
    apply({
        ...
        "spec": {
            ...
            "csi": {"driver": "ebs.csi.aws.com", "volumeHandle": "vol-phase46-object-only", "fsType": "ext4"},
    ...
    expected = ("Retain", "amoebius-retained", "ebs.csi.aws.com", "vol-phase46-object-only", "topology.ebs.csi.aws.com/zone", ["us-east-1a"])
...
def retained_marker_roundtrip(path: str) -> dict[str, Any]:
    ...
    apply(host_path_pv("phase46-retained-a", path))
    apply(claim("phase46-retained-a"))
    wait_bound_claim("phase46-retained-a")
    ...
    first_pv = get_json("get", "persistentvolume", "phase46-retained-a")
    ...
    kubectl("delete", "persistentvolume", "phase46-retained-a", "--wait=true", "--timeout=120s")
    apply(host_path_pv("phase46-retained-b", path))
    apply(claim("phase46-retained-b"))
    wait_bound_claim("phase46-retained-b")
    ...
    second_pv = get_json("get", "persistentvolume", "phase46-retained-b")
    if recovered != marker:
...
def checkpoint_class_roundtrip(binary: Path) -> dict[str, Any]:
    ...
    bucket = "phase46-checkpoints-" + secrets.token_hex(4)
...
def main() -> int:
    evidence = execute()
    print("phase46-provider-ebs-scoped-live: PASS")
    print(f"phase46-aws-ebs-iam: UNVERIFIED ({evidence['evidenceDigest']})")
    return 0
