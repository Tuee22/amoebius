...
EXPECTED_DIGEST = ROOT / "test/fixtures/phase31/expected-base-digest.txt"
POSTGRES_SHARE_DIGEST = ROOT / "test/fixtures/phase31/postgres-share-package.sha256"
CABAL = "/home/matthewnowak/.ghcup/bin/cabal"
UNVERIFIED = {"keycloak-edge", "singleton-owned-reconcile"}
MUTANTS = (
    ("M-patroni-async-default", "phase31-patroni-async-default-mutant", "synchronous_mode_strict-not-on"),
    ("M-dag-drop-edge", "phase31-dag-drop-edge-mutant", "declared DAG equals independent oracle"),
    ("M-dag-inject-cycle", "phase31-dag-inject-cycle-mutant", "declared DAG equals independent oracle"),
    ("M-redis-pvc", "phase31-redis-pvc-mutant", "redis-persistence-forbidden"),
    ("M-redis-unbounded-buffer", "phase31-redis-unbounded-buffer-mutant", "redis-bounds-must-be-finite-positive"),
    ("M-redis-public-image", "phase31-redis-public-image-mutant", "redis-public-image-forbidden"),
    ("M-redis-receipt-authority", "phase31-redis-receipt-authority-mutant", "redis-receipt-authority-forbidden"),
    ("M-fixed-prometheus", "phase31-fixed-prometheus-mutant", "Prometheus derived storage boundary"),
)
...
def reject_mutant(name: str, flag: str, marker: str) -> dict[str, str]:
    arguments = (CABAL, "test", "phase31-services-spec", f"-f{flag}", "--test-show-details=direct", "-j1")
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 31,
        "gate_command": "python3 tools/phase31_gate.py",
        "register": "3",
        "substrate": "linux-cpu",
        "date": "2026-08-10",
        "layers": [
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows = [
            invoke("pure-service-contract", (CABAL, "test", "phase31-services-spec", "--test-show-details=direct", "-j1")),
            invoke("bringup-simulation", (CABAL, "test", "phase31-bringup-sim", "--test-show-details=direct", "-j1")),
        ]
        if not arguments.seal_existing_live:
            rows.append(invoke("services-live", (python, "tools/phase31_services_live.py"), timeout=3600))
        else:
            rows.append({"name": "services-live", "command": "sealed just-produced tools/phase31_services_live.py receipt", "output": "fresh final evidence", "result": "PASS"})
        evidence_domain(require_fresh=arguments.seal_existing_live)
        rows.append(invoke("external-live-reader", (CABAL, "test", "phase31-services-live", "--test-show-details=direct", "-j1")))
        rows.extend(reject_mutant(*mutant) for mutant in MUTANTS)
        disabled = tuple(f"-f-{flag}" for _, flag, _ in MUTANTS)
        rows.append(invoke("baseline-restored", (CABAL, "test", "phase31-services-spec", *disabled, "--test-show-details=direct", "-j1")))
        ...
        print(f"phase31-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']})")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as problem:
        print(f"phase31-gate: FAIL: {problem}", file=sys.stderr)
        return 1
