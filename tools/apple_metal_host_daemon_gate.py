...
FLAGS = (
    "apple-metal-host-daemon-const-output-mutant", "apple-metal-host-daemon-echo-golden-mutant", "apple-metal-host-daemon-lb-nodeport-mutant",
    "apple-metal-host-daemon-omit-metal-work-item-mutant", "apple-metal-host-daemon-favorable-metal-epoch-mutant",
    "apple-metal-host-daemon-drop-metal-overlap-debit-mutant",
)
MUTANTS = {
    "apple-metal-host-daemon-const-output-mutant": "job B",
    "apple-metal-host-daemon-echo-golden-mutant": "run-time challenge",
    "apple-metal-host-daemon-lb-nodeport-mutant": "NodePortRequired",
    "apple-metal-host-daemon-omit-metal-work-item-mutant": "MetalKeyDomainMismatch",
    "apple-metal-host-daemon-favorable-metal-epoch-mutant": "Metal coexistence peak",
    "apple-metal-host-daemon-drop-metal-overlap-debit-mutant": "host memory debit",
}
UNVERIFIED = {
...
def dhall_domain() -> list[dict[str, str]]:
    rows = [invoke("green-host-comms-dhall", (DHALL, "type", "--file", "test/fixture/dhall/apple_metal_host_daemon/illegal/green_host_comms.dhall", "--quiet"))]
    expected = (ROOT / "test/golden/apple_metal_host_daemon/illegal_expected_errors.tsv").read_text().splitlines()[1:]
    for source in expected:
        fixture, locus, tag, message = source.split("\t")
        argv = (DHALL, "type", "--file", f"test/fixture/dhall/apple_metal_host_daemon/illegal/{fixture}", "--quiet")
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 53, "gate_command": "python3 tools/apple_metal_host_daemon_gate.py",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows.append(invoke("portable-live", (sys.executable, "tools/apple_metal_host_daemon_live.py")))
        evidence_domain()
        ...
        print(f"apple-metal-host-daemon-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']}; physical Apple/Lima/Metal/Pulsar/gateway/MinIO UNVERIFIED)")
        return 0
    except (Failure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"apple-metal-host-daemon-gate: FAIL: {error}", file=sys.stderr)
        return 1
