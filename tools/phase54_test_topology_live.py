...
def failover_probe(root: Path) -> dict[str, object]:
    lock = root / "phase54-failover.lock"
    ...
    b.terminate()
    b.wait(timeout=5)
    return {"subscription": "phase54-failover", "type": "process-lock-analogue", "killed": "worker-a",
            "promoted": "worker-b" if promoted else "none", "processesReaped": True,
            "pulsarBrokerStats": "UNVERIFIED"}


def main() -> int:
    with tempfile.TemporaryDirectory(prefix="amoebius-phase54-") as temporary:
    ...
    print(f"phase54-test-topology-live: PASS-SCOPED ({evidence['evidenceDigest']}; Kubernetes/Pulsar/retained-PV/Vault/AWS UNVERIFIED)")
    return 0
