#!/usr/bin/env python3
"""Seal the object reconciler's staged transitions, terminal retention, and delete authority."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # The corpus pod pulls the Phase-25 published digest from the in-cluster registry.
    ...
    try:
        rows = [
            ...
            invoke("live-staged-transitions", (sys.executable, "tools/object_reconciler_execution_live.py", "--output", str(live), "--image", arguments.image)),
            invoke("label-only-delete-mutant", cabal("-fobject-reconciler-label-only-delete-mutant"), expect_failure="changed resourceVersion"),
            invoke("baseline-restored", cabal()),
            invoke("documentation-lint", (sys.executable, "tools/doc_lint.py")),
        ]
        stable = {"schema": "amoebius.phase26.sprint26.3-receipt.v1", "register": 3, "substrate": "linux-cpu", **validate_live(live), "seededMutantsRed": 1, "result": "PASS"}
