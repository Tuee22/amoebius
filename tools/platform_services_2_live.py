...
CABAL = phase30.CABAL
MIB = 1024 * 1024
RETAINED_ROOT = Path("/var/tmp/amoebius-platform-services-2-retained")
...
def prepare_node_backing(name: str, raw_bytes: int) -> dict[str, Any]:
    ...
    node_target = f"/amoebius-platform-services-2-retained/{name}"
...
def vault_material() -> tuple[dict[str, str], dict[str, Any]]:
    ...
    with phase29.port_forward():
        ...
        if status == 200:
            ...
            raise ServicesLiveFailure(f"vault-platform-services-2-kv-read:{status}")
        if any(not isinstance(generated.get(key), str) or not generated[key] for key in password_keys):
            raise ServicesLiveFailure("vault-platform-services-2-password-material-incomplete")
...
def apply_operator_contract() -> None:
    ...
    apply({
        "apiVersion": "rbac.authorization.k8s.io/v1", "kind": "ClusterRole", "metadata": {"name": "platform-services-2-percona-operator"},
        "rules": [
    ...
    apply({
        "apiVersion": "rbac.authorization.k8s.io/v1", "kind": "ClusterRoleBinding", "metadata": {"name": "platform-services-2-percona-operator"},
        "roleRef": {"apiGroup": "rbac.authorization.k8s.io", "kind": "ClusterRole", "name": "platform-services-2-percona-operator"},
                ...
                "spec": {
                    ...
                    "containers": [{
                        ...
                        "volumeMounts": [
                            ...
                            {"name": "credentials", "mountPath": "/platform-services-2-secrets", "readOnly": True},
...
export PGPASSWORD="$(cat /platform-services-2-secrets/superuser)"
if ! /usr/lib/postgresql/17/bin/psql -h 127.0.0.1 -U postgres -tAc "select 1 from pg_roles where rolname='grafana'" | grep -qx 1; then
  /usr/lib/postgresql/17/bin/psql -h 127.0.0.1 -U postgres -v ON_ERROR_STOP=1 -c "create role grafana login password '$(cat /platform-services-2-secrets/grafana)'"
fi
...
"""
    ...
    query_command = [
        "/usr/bin/python3", "/platform-services-2-query-proxy/query_proxy.py", "--max-concurrent=4", "--max-series=64",
    ...
    apply(deployment(
        ...
        volume_mounts=[{"name": "query-proxy", "mountPath": "/platform-services-2-query-proxy", "readOnly": True}],
...
def fresh_haskell_render_plan() -> list[dict[str, Any]]:
    run((CABAL, "build", "platform-services-2-services-spec"), timeout=900)
    binary = text(run((CABAL, "list-bin", "platform-services-2-services-spec"))).strip()
...
def execute() -> dict[str, Any]:
    ...
    evidence = {
        ...
        "deferred": {"keycloakIngress": "UNVERIFIED", "controlPlaneOwnedReconcile": "UNVERIFIED"},
        "universalLinuxCpu": {
...
def main() -> int:
    try:
        evidence = execute()
        print(
            "platform-services-2-services-live: PASS "
        ...
        print(f"platform-services-2-services-live: FAIL: {problem}", file=sys.stderr)
        return 1
