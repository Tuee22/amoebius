...
LIVE_ROOT = Path("/var/tmp/amoebius-phase44-live")
...
NAMESPACE = "phase44-system"
BUCKET = "phase44-pulumi"
...
def reset() -> None:
    ...
    if LIVE_ROOT != Path("/var/tmp/amoebius-phase44-live"):
...
def singleton_readback() -> dict[str, Any]:
    deployment = json.loads(text(kubectl("-n", "phase33-system", "get", "deployment", "amoebius-control-plane", "-o", "json")))
    ...
    if (replicas, ready, available) != (1, 1, 1):
        raise Phase44Failure(f"singleton-readback:{replicas}:{ready}:{available}")
    return {
        "kind": "Deployment", "namespace": "phase33-system", "name": "amoebius-control-plane",
...
def execute() -> dict[str, Any]:
    ...
    evidence: dict[str, Any] = {
        ...
        "singleton": singleton,
        "executorPlacement": {"boundedParallel": 2, "jobs": executors, "source": "external gate harness; real singleton Pulumi up UNVERIFIED"},
        ...
        "deferred": {
            "pulumiUpFromSingleton": "UNVERIFIED", "awsPluginExecve": "UNVERIFIED",
...
def main() -> int:
    evidence = execute()
    print("phase44-provider-checkpoint-scoped-live: PASS")
    print(f"phase44-provider-materialization: UNVERIFIED ({evidence['evidenceDigest']})")
    return 0
