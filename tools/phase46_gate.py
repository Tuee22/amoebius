...
FLAGS = (
    "phase46-allow-delete-mutant",
    "phase46-dynamic-provisioner-mutant",
    "phase46-credit-old-mutant",
    "phase46-drop-copy-executor-mutant",
    "phase46-bypass-validated-batch-mutant",
)
MUTANTS = {
    "phase46-allow-delete-mutant": "mut-46.1-allow-delete",
    "phase46-dynamic-provisioner-mutant": "mut-46.1-enable-dynamic-provisioner",
    "phase46-credit-old-mutant": "mut-46.1-credit-old-before-observed-delete",
    "phase46-drop-copy-executor-mutant": "mut-46.1-drop-copy-executor",
    "phase46-bypass-validated-batch-mutant": "mut-46.1-bypass-validated-batch",
}
UNVERIFIED = {
...
def oracle_domain() -> dict[str, str]:
    topology = load_dhall("test/dhall/phase_46_provider_provision.dhall")
    ...
    credential_rows = list(csv.DictReader((ROOT / "test/goldens/ebs_credential_matrix.txt").open(encoding="utf-8"), delimiter="\t"))
    ...
    binaries = load_dhall("test/fixtures/phase46/ebs_csi_bake_expected.dhall")
    ...
    rounding = list(csv.DictReader((ROOT / "test/fixtures/phase46/provider-volume-rounding.tsv").open(encoding="utf-8"), delimiter="\t"))
    ...
    one_short = list(csv.DictReader((ROOT / "test/fixtures/phase46/migration-one-short.tsv").open(encoding="utf-8"), delimiter="\t"))
    require(len(one_short) == 8 and all(int(row["available"]) + 1 == int(row["required"]) for row in one_short), "migration-one-short")
    static = json.loads((ROOT / "test/fixtures/phase46/static-pv-oracle.json").read_text(encoding="utf-8"))
...
def no_live_residue() -> dict[str, str]:
    namespace = subprocess.run(("/usr/bin/kubectl", "--kubeconfig", str(Path.home() / ".amoebius/phase24/kubeconfig"), "get", "namespace", "phase46-system"), text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False, timeout=60)
    require(namespace.returncode != 0, "phase46-namespace-residue")
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 46,
        "gate_command": "python3 tools/phase46_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.reuse_fresh_live:
            ...
            rows.append(invoke("scoped-retained-storage-checkpoint", (sys.executable, "tools/phase46_provider_ebs_live.py"), timeout=2400))
        ...
        print(f"phase46-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']}; AWS EBS/IAM UNVERIFIED)")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"phase46-gate: FAIL: {error}", file=sys.stderr)
        return 1
