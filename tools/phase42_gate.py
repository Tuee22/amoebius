...
MUTANTS = (
    ("M-classifier-default-confluent", "phase42-classifier-default-confluent-mutant", "phase42-classifier-default-confluent:"),
    ("M-project-identity", "phase42-project-identity-mutant", "phase42-project-identity:"),
    ("M-drop-parallel-executor", "phase42-drop-parallel-executor-mutant", "phase42-drop-parallel-executor:"),
)
UNVERIFIED = {
...
def reject_mutant(name: str, flag: str, marker: str) -> dict[str, str]:
    environment = os.environ.copy()
    environment.update({"PHASE42_PURE_ONLY": "1"})
...
def phase0_domain() -> dict[str, str]:
    ...
    return {"name": "phase0-custody", "command": "read test/phase0_oracle_manifest.tsv", "output": "12 oracles; 3 mutants", "result": "PASS"}


def compile_corpus() -> dict[str, str]:
    base = (CABAL, "exec", "--", GHC, "-fno-code", "-XGHC2024", "-isrc", "-package", "text")
    positive = subprocess.run(
        (*base, "test/compile-fail/ChildInForceSpec/Positive.hs"), cwd=ROOT, text=True,
    ...
    for stem in ("NegativeSibling", "NegativeAncestor"):
        result = subprocess.run(
            (*base, f"test/compile-fail/ChildInForceSpec/{stem}.hs"), cwd=ROOT, text=True,
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False, timeout=300,
        )
        expected = (ROOT / f"test/compile-fail/ChildInForceSpec/{stem}.stderr").read_text(encoding="utf-8").strip()
...
def oracle_domain() -> dict[str, str]:
    classification = subprocess.run(
        (DHALL_TO_JSON, "--file", "test/inject/confluence/expected_classes.dhall"), cwd=ROOT,
    ...
    for name in ("idempotent-write.golden.json", "expected-forest-demand.json", "live-observer-contract.json"):
        require(isinstance(json.loads((ROOT / "test/fixtures/phase42" / name).read_text(encoding="utf-8")), dict), f"json-oracle:{name}")
    for name in ("kind-parent.yaml", "amoebius-p42-a.yaml", "amoebius-p42-b.yaml"):
        value = (ROOT / "test/fixtures/phase42" / name).read_text(encoding="utf-8")
...
def no_live_residue() -> dict[str, str]:
    ...
    require(not Path("/var/tmp/amoebius-phase42-live").exists(), "temporary-root-residue")
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 42,
        "gate_command": "python3 tools/phase42_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.reuse_fresh_live:
            ...
            rows.append(invoke("multicluster-live", (sys.executable, "tools/phase42_multicluster_live.py"), timeout=5400))
        evidence_domain(fresh=args.reuse_fresh_live)
        rows.append(no_live_residue())
        rows.append(invoke("sealed-live-reader", test_args(), extra_env={"PHASE42_REUSE_FRESH_LIVE": "1"}))
        rows.extend(reject_mutant(*mutant) for mutant in MUTANTS)
        rows.append(invoke("baseline-restored", test_args(), extra_env={"PHASE42_REUSE_FRESH_LIVE": "1"}))
        ...
        print(f"phase42-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']})")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"phase42-gate: FAIL: {error}", file=sys.stderr)
        return 1
