...
FLAGS = (
    "jitml-lift-cuda-silent-cpu-fallback-mutant", "jitml-lift-cuda-spend-raw-vram-mutant",
    "jitml-lift-cuda-mint-artifact-before-cas-mutant", "jitml-lift-cuda-regenerate-command-id-mutant",
)
MUTANTS = {
    "jitml-lift-cuda-silent-cpu-fallback-mutant": "mut-51-silent-cpu-fallback",
    "jitml-lift-cuda-spend-raw-vram-mutant": "mut-51-spend-raw-vram",
    "jitml-lift-cuda-mint-artifact-before-cas-mutant": "mut-51-mint-artifact-before-cas",
    "jitml-lift-cuda-regenerate-command-id-mutant": "mut-51-regenerate-command-id",
}
UNVERIFIED = {
...
def oracle_domain() -> dict[str, str]:
    decoded = subprocess.run(
        (DHALL_TO_JSON, "--file", "test/fixture/dhall/jitml_lift_cuda/jitml_cuda_artifact.dhall"), cwd=ROOT,
        text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False, timeout=120,
    )
    require(decoded.returncode == 0, f"jitml-lift-cuda-dhall-oracle:{decoded.stdout}")
    ...
    require(value["universalLinuxCpu"] == {"availableOnEveryHardwareSubstrate": True,
            ...
            "universal-linux-cpu")
    capacity = list(csv.DictReader((ROOT / "test/fixture/jitml_lift_cuda/cuda_capacity_matrix.tsv").open(encoding="utf-8"), delimiter="\t"))
    ...
    artifact = list(csv.DictReader((ROOT / "test/fixture/jitml_lift_cuda/committed_artifact_contract.tsv").open(encoding="utf-8"), delimiter="\t"))
    require([row["constructor_available"] for row in artifact] == ["false", "false", "false", "true", "false"], "artifact-matrix")
    commands = list(csv.DictReader((ROOT / "test/fixture/jitml_lift_cuda/command_identity_matrix.tsv").open(encoding="utf-8"), delimiter="\t"))
    ...
    resource = json.loads((ROOT / "test/fixture/jitml_lift_cuda/resource_shape.json").read_text(encoding="utf-8"))
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 51,
        "gate_command": "python3 tools/jitml_lift_cuda_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.derive_ledger:
            ...
            return 0
        disabled = flags()
        rows = [invoke("jitml-lift-cuda-source-build", (CABAL, "build", "jitml-lift:lib:jitml-lift",
        ...
        print(f"jitml-lift-cuda-gate: PASS-SCOPED ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']}; Kubernetes accelerator-owner/native-CBOR/full sibling trainer UNVERIFIED)")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"jitml-lift-cuda-gate: FAIL: {error}", file=sys.stderr)
        return 1
