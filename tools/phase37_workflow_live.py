...
COMPONENTS = {"alpha": b"phase37-alpha", "zeta": b"phase37-zeta"}
...
def setup() -> dict[str, Any]:
    for stale in Path("/tmp").glob("amoebius-phase37-state-*.json"):
        try:
            cleanup(stale)
        except Exception:
            pass
    ...
    state_path = Path(f"/tmp/amoebius-phase37-state-{suffix}.json")
    state: dict[str, Any] = {
...
def orphan_gc_drill(bucket: str) -> dict[str, Any]:
    blob = b"phase37-orphan-blob"
    manifest = b"phase37-orphan-manifest"
    prefix = "orphan-drill"
    ...
    failed_status, _, _ = require_s3(
        ...
        s3_request(
            ...
            conditional={"if-match": '"phase37-impossible-etag"'},
        ),
...
def finish(path: Path, result_path: Path) -> None:
    ...
    print("phase37-live-finish: PASS", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print(f"phase37-workflow-live: FAIL: {error}", file=__import__("sys").stderr, flush=True)
        raise SystemExit(1)
