#!/usr/bin/env python3
"""Run and seal the Phase-26 typed-action object-reconciler gate.

The capability claim is unchanged: the Phase-13 deployment-global render is validated and
...
RESULTS = ROOT / "gen/dsl/phase26/phase-results.tsv"
CONTRACT = "DEVELOPMENT_PLAN/phase_26_object_reconciler.md"
GATE_COMMAND = "python3 tools/phase26_gate.py --execute"

# The committed mutant domain, each mapped to the sprint whose evidence decides it. The
# live delete mutant is decided twice — Sprint 26.3 owns the staged-deletion claim and
# Sprint 26.4 re-runs it inside the convergence battery — and one red observation of it is
...
def execute_sprints(evidence: Path, image: str) -> None:
    evidence.mkdir(parents=True, exist_ok=True)
    for number in (1, 2, 3, 4, 5):
        arguments = [
            sys.executable, f"tools/phase26_sprint26_{number}_gate.py", "--evidence", str(evidence),
        ]
        if number in (3, 4):
            arguments += ["--image", image]
        run(arguments, timeout=21600)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--image", default=None,
        help="the digest-pinned in-cluster reference Phase 25 published, which the corpus pulls",
    )
    arguments = parser.parse_args(argv)

    gate = gate_common.PhaseGate(
        phase=26, contract=CONTRACT, command=GATE_COMMAND, register="3", substrate="linux-cpu", sides=SIDES
    )
    gate.begin()
    ...
    try:
        ...
        run((sys.executable, "tools/phase13_gate.py"))
        ...
        if arguments.execute:
            ...
            if not arguments.image:
                raise GateFailure("--execute needs --image: the corpus pulls the Phase-25 published digest")
            print("\nlive side — the five sprints on the linux-cpu kind cluster\n")
            execute_sprints(evidence, arguments.image)
        elif evidence is None:
            raise GateFailure("phase-26 needs --execute or an --evidence bundle from a completed live run")
        else:
    ...
    return gate.finish(
        ...
        dependencies={"cluster": "kind (phase 24)", "registry": "in-cluster distribution (phase 25)"},
