#!/usr/bin/env python3
"""Read-only admission for the Phase-31.2 selected-image/registry transition."""
...
def kubectl_json(*arguments: str) -> dict[str, Any]:
    decoded = json.loads(run((KUBECTL, "--kubeconfig", str(KUBECONFIG), *arguments, "-o", "json")))
...
def filesystem_observation(host_storage_root: Path) -> dict[str, Any]:
    storage = containment.require_state_path(host_storage_root, "test", actor="test")
    if not storage.is_dir():
        raise PreflightFailure(f"host-storage-root-absent:{storage}")
    host = run((
        "/usr/bin/findmnt", "-bno", "SOURCE,FSTYPE,SIZE,AVAIL,TARGET",
        "-T", str(storage),
    )).strip().split()
    if len(host) != 5:
    ...
    # Free space is a capacity observation, not filesystem identity.  The node can
    # write between these two read-only findmnt calls, so including AVAIL here made
    # one unified backing spuriously look split.  Admission still uses the minimum
    # of every observed AVAIL value below.
    identities = {(row["backingSource"], row["type"], row["sizeBytes"]) for row in node_rows}
    if len(identities) != 1:
        raise PreflightFailure("layout-alias-mismatch")
    return {
        "hostStorageRoot": str(storage),
...
def observe(
    ...
    filesystem = filesystem_observation(host_storage_root)
    ...
    # Kubernetes permits CPU/memory limit overcommit.  Phase 31 deliberately
    ...
    domain = run((KUBECTL, "--kubeconfig", str(KUBECONFIG), "get", "namespace", "amoebius-bootstrap", "--ignore-not-found", "-o", "name")).strip()
    if domain:
...
def main() -> None:
    ...
    parser.add_argument(
        "--host-storage-root", type=Path, required=True,
        help="existing .test_data directory that backs the private container engine",
    )
