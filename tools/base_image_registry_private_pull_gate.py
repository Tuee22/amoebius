...
def validate_evidence(evidence: Path, index_digest: str) -> dict[str, Any]:
    original = json_object(evidence / "private-pull.json")
    current = json_object(evidence / "private-pull-verification.json")
    mutants = json_object(evidence / "private-pull-mutants.json")
    standup = json_object(evidence / "standup-receipt.json")
    publication = json_object(evidence / "publication-receipt.json")
...
def seal(evidence: Path, index_digest: str) -> tuple[list[dict[str, str]], dict[str, Any]]:
    ...
    rows = [
        ...
        invoke(
            ...
            (
                python,
                "tools/base_image_registry_private_pull.py",
                "--verify-only",
                "--evidence",
                str(evidence),
                "--index-digest",
                index_digest,
                "--output",
                str(evidence / "private-pull-verification.json"),
            ),
            timeout=600,
        ),
        invoke(
            ...
            (
                python,
                "tools/base_image_registry_private_pull_mutation_gate.py",
                "--live",
                str(evidence / "private-pull.json"),
                "--evidence",
                str(evidence / "private-pull-mutants.json"),
            ),
            timeout=1800,
        ),
...
def write_results(evidence: Path, rows: list[dict[str, str]], receipt: dict[str, Any]) -> None:
    (evidence / "private-pull-receipt.json").write_text(
        json.dumps(receipt, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (evidence / "private-pull-results.tsv").write_text(
    ...
    (evidence / "private-pull-gate.log").write_text("\n".join(log) + "\n", encoding="utf-8")
