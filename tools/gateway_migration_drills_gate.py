...
MUTANTS = (
    ("M-verify-caught-up-stub", "gateway-migration-drills-verify-caught-up-stub-mutant", "gateway-migration-drills-verify-caught-up:"),
    ("M-promote-before-fence", "gateway-migration-drills-promote-before-fence-mutant", "gateway-migration-drills-promote-before-fence:"),
)
...
def reject_mutant(name: str, flag: str, marker: str) -> dict[str, str]:
    environment = os.environ.copy()
    environment.update({"GATEWAY_MIGRATION_DRILLS_PURE_ONLY": "1"})
...
def oracle_domain() -> dict[str, str]:
    result = subprocess.run(
        (DHALL_TO_JSON, "--file", "test/fixture/dhall/gateway_migration_drills/gateway_migration.dhall"), cwd=ROOT,
    ...
    for path in (
        ROOT / "test/fixture/inject/journal/gateway_migration_drills/schema.json",
...
def no_live_residue() -> dict[str, str]:
    ...
    require(not Path("/var/tmp/amoebius-gateway-migration-drills-live").exists(), "live-root-residue")
    require(not Path("/var/tmp/amoebius-gateway-migration-drills-journal").exists(), "journal-root-residue")
...
def derive_ledger() -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "phase": 43,
        "gate_command": "python3 tools/gateway_migration_drills_gate.py --reuse-fresh-live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rows.append(invoke("sealed-live-reader", live_reader_args(), extra_env={"GATEWAY_MIGRATION_DRILLS_REUSE_FRESH_LIVE": "1"}))
        rows.extend(reject_mutant(*mutant) for mutant in MUTANTS)
        rows.append(invoke("baseline-restored-simulation", sim_args()))
        rows.append(invoke("baseline-restored-live-reader", live_reader_args(), extra_env={"GATEWAY_MIGRATION_DRILLS_REUSE_FRESH_LIVE": "1"}))
        ...
        print(f"gateway-migration-drills-gate: PASS ({len(rows)} checks; {derived['ledger_hash']}; {receipt['receiptFingerprint']})")
        return 0
    except (GateFailure, OSError, ValueError, KeyError, json.JSONDecodeError, subprocess.TimeoutExpired) as error:
        print(f"gateway-migration-drills-gate: FAIL: {error}", file=sys.stderr)
        return 1
