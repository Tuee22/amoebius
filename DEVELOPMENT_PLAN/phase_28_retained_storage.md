...
8 supplies the policy-only `ProvisionedStorageScalingEnvelope` and pure observe-then-plan fold, and Phase 26
...
Phase 30 and HA Patroni-via-Percona Postgres in Phase 31, the Vault-enveloping of secrets is Phase 29, and the
Keycloak-owned edge is Phase 32 — none of which this phase requires. The control plane
...
- **Bytes survive on the host, not in a surviving apiserver.** "Bytes intact between delete and recreate" is
  ...
  destroyed. The PV `Released` *status* is observed only in Sprint 28.2's live PVC-delete step against the
...
- **Honest ledger.** The gate emits its proven/tested/assumed ledger; the aggregate durable-backing fold and
  image-backed host hard cap are live-tested here. The Phase-33 control-plane singleton's no-PVC property
  (which has no realized subject at Phase 28) stays marked **UNVERIFIED**, not asserted as passing.
...
- **M-soft-delete** (dropped-effect operator) — a `cluster delete` that deletes only the witness
  ...
  absent" OS-boundary assertion (Gate; 28.3 V1).
...
- **M-reclaim-delete** (guard weakening) — a PV rendered with `reclaimPolicy: Delete` instead of `Retain`. Must
  go red on the `Released`/rebind assertion (28.2 V3/V4).
...
- **M-raw-host-directory** (mechanism substitution) — backs a PV with an ordinary retained-root directory
  ...
  `ENOSPC`, changes shared-parent occupancy, or lacks the raw-size/fs-type/usable witness (28.2 V2).
- **M-skip-durable-aggregate** (dropped validation) — allocates/applies an aggregate retained set larger than
  `DurableBacking`. Must go red on the over-backing negative and zero-write assertion (28.2 V1).
- **M-sum-unequal-ordinals** (wrong aggregation) — debits the unequal per-ordinal provisioned map instead of
  ...
  `uniform_claim_skew_over_backing` and the accepted group's byte-identical PVC-size assertion (28.2 V1/V2).
- **M-uniform-before-allocation** (stage-reordering operator) — groups authorable logical/usable demand before
  ...
  fixtures and the per-slot-witness-before-uniformity assertion (28.2 V1/V2).
- **M-collapse-uniform-backing-debits** (wrong aggregation) — collapses the per-backing debit map into a single
  ...
  member backing is one byte short (28.2 V1).
- **M-cutover-before-verify** (guard-weakening / stage-reordering operator) —
  ...
  `migration_verify_mismatch`, where a mismatched copy is cut over anyway (28.2 V1).
- **M-credit-before-cleanup** (dropped-validation operator) — `src/Amoebius/Storage/RetainedScaling.hs`:
  ...
  observed deletion" step (28.2 V1).
- **M-fake-verify** (mechanism-substitution operator) — `src/Amoebius/Storage/RetainedScaling.hs`: the
  ...
  byte-for-byte nonce readback (28.2 V1).
...
- [`cluster_lifecycle_doctrine.md §7`](../documents/engineering/cluster_lifecycle_doctrine.md#7-ephemeral-spin-updown-with-deterministic-rebind)
  ...
  (delivered in Phase 26) — the SSA reconciler that renders and applies the StorageClass and PV objects.
...
- A single rendered StorageClass — `provisioner: kubernetes.io/no-provisioner`, `reclaimPolicy: Retain`,
  `volumeBindingMode: WaitForFirstConsumer` — applied through the Phase-26 reconciler under the `amoebius`
  field manager.
...
### Deliverables
- Minimal single-ordinal Postgres and MinIO witness StatefulSets on retained PVs (from Sprint 28.2), served
  from baked-binary images; these are rebind witnesses, not distributed MinIO (Phase 30) or HA
  Patroni-via-Percona (Phase 31), and carry no Vault-enveloping (Phase 29).
...
- `documents/engineering/storage_lifecycle_doctrine.md` — the §6 lossless-rebind guarantee gains its first
  ...
  planning-ownership pointer resolves to delivered Phase-28 sprints.
...
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 28's gate substrate (linux-cpu) in the per-phase substrate map.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the
  Phase-26 SSA reconciler that applies the StorageClass and retained-PV objects
...
- [phase_26](phase_26_object_reconciler.md) — the typed renderer + live SSA reconciler this phase builds on
- [phase_29](phase_29_vault_pki.md) — the root Vault whose durable KV rebinds on the retained storage proven here
- [phase_30](phase_30_platform_backbone.md) / [phase_31](phase_31_platform_services_2.md) — the HA
