...
1. **Phase 0 was reopened again on 2026-08-14 by the final-layout amendment, and resealed the same day.**
...
| 25 | **Policy-conformant** | Refreshed 2026-08-14 (third). Sealed: `python3 tools/phase25_gate.py --execute` passes on all eleven sides against a 1,963-file source snapshot. One command renders the Dockerfile against its committed golden, admits the build against the observed host, exports one OCI manifest list covering both arches, executes all 22 baked binaries by absolute path on each, side-loads that export into the Phase-24 node, stands the `distribution` registry up from it with zero public-registry connections, publishes the manifest list atomically under an immutable digest-pinned ref, and proves the enforced node-level egress denial with the paired canaries — the `docker.io/library/busybox` canary failing `ImagePull` while the in-cluster pull succeeds. All 14 committed mutants go red, 15 recorded metrics equal their authored values, and 40 surfaces join completely to 40 enumerated items. Three of the four sprints had audited evidence that nothing in the tree produced, so `--execute` now performs each live transition immediately before the gate that seals it; the side-load did not exist at all and now imports this run's export by digest inside the packet-capture window. Three more fixed paths that would have decided a run from whatever a previous one left behind — the preflight's and the publisher's OCI archive constants, and a resident-image check naming a tag nothing creates — are caller-supplied and digest-named. The standup's Deployment named `/usr/bin/registry`, the scavenged image's path, and now reads both container commands from the inventory oracle, so the standup and the bake gate cannot mean different binaries | None. Phase 25 is sealed |
| 26 | **Policy-conformant** | Refreshed 2026-08-14. Sealed: `python3 tools/phase26_gate.py --execute` passes on all ten sides. The reconciler converged the pinned corpus on the live `kind` cluster — pulling its Deployment from the registry Phase 25 published to — and re-ran byte-stable with zero planned mutations; readiness was observed rather than slept through, both `OnDelete` slots replaced in order through observed absence with distinct UIDs, the terminal Job Pod stayed retained with no completion object, the CR's child conformed to its envelope, and one of two simultaneous reservations was admitted under a one-Pod quota with zero over-allocation. The Register-2.5 half ran 2,048 deterministic schedules against the same real action modules. All 11 mutants red; 31 surfaces join, with `modeled-apiserver-fidelity`, `completion-release-ledger`, and `rollback-ledger` honestly UNVERIFIED. The retired gate could not run at all: it read a plan-tree evidence root that no longer exists, compared a committed golden ledger against a derived one, read a surface list from a missing enumeration file, and called a developer-home `cabal` in eight places. The corpus also pinned a Phase-25 image digest from a build that is gone — including in the one assertion meant to tie the running pod to it, which compared that constant to itself | None. Phase 26 is sealed |
| 27 | **Policy-conformant** | Refreshed 2026-08-14. Sealed: `python3 tools/phase27_gate.py --execute` passes on all ten sides. The two-stage cutover ran in the authored six-event order on the live cluster, every Binding followed a successful whole-ledger reservation CAS, and the immediate re-run issued zero new Binding requests while staying byte-stable, with a clean postflight. The Register-2.5 half replayed 1,792 deterministic schedules across seven fault classes with byte-identical same-seed replay. All 14 mutants red; 33 surfaces join, with modeled-apiserver fidelity, the completion and rollback ledgers, and the in-cluster singleton honestly UNVERIFIED. The retired gate was unrunnable in the same four ways Phase 26's was — vanished evidence root, committed golden ledger, missing enumeration, developer-home `cabal` — and its live driver plus both independent evidence readers named a Phase-25 digest from a build that is gone | None. Phase 27 is sealed |
| 28–43 | **Observed footprint** | Each phase has implementation or gate material. Generated evidence/enumeration, unresolved TSV and oracle provenance, digest/checksum fixtures, and same-commit regression fixtures prevent a current policy-conformant result. Every one of them also inherits the defects Phases 26 and 27 found in their own tooling: a developer-home `cabal`, the retired plan-tree evidence root, and the stale Phase-25 image digest that eight later live drivers still name | Classify and migrate artifacts, independently review or replace regression fixtures, and rerun each current gate in numeric order |
...
|---|---|---|---|---|---|
| 0 | Documentation suite + the final repository layout | none | — | ✅ Done — resealed 2026-08-14, attestation `sha256:a45fc9bb9ff0fab2f0c84b8a4789df4a8fb5dacd4b230b08606701e90fab379f` | [phase_00](phase_00_documentation_suite.md) |
...
| 25 | Typed bake catalog + multi-arch base image + jit-build resolver + distribution registry | linux-cpu | 3 | ✅ Done — sealed 2026-08-14, attestation `sha256:e1d9be21b5adc923c05dd13e5f59c4b44dd3dfddd37f76889b92b522ccddcce3` | [phase_25](phase_25_base_image_registry.md) |
| 26 | Typed renderer + object reconciler | linux-cpu | 3 | ✅ Done — sealed 2026-08-14, attestation `sha256:0ba91376b2c536930ba52ade70c9581798a9dfb861d468a28f5bb0d13fc353e0` | [phase_26](phase_26_object_reconciler.md) |
| 27 | amoebius-capacity scheduler + bootstrap cutover | linux-cpu | 3 | ✅ Done — sealed 2026-08-14, attestation `sha256:d576e18d7180eef7a8ee6aaf0b2e53c7d38d90d6ba81050539ea3fa7c68f9c79` | [phase_27](phase_27_capacity_scheduler.md) |
| 28 | No-provisioner retained storage + lossless rebind | linux-cpu | 3 | 🔄 Active — the lowest phase not yet sealed | [phase_28](phase_28_retained_storage.md) |
