...
On 2026-09-07 the serialized `validation-pb-boundary-run-component` diagnostic started the concrete Phase-50
supervisor. The supervisor evaluated `ByteString.take 32` only after strict `ByteString.readFile` consumption
of `/dev/urandom`, which never reaches EOF. It published no challenge and retained input until the kernel killed
the process at 119,734,580 KiB anonymous RSS. This was a host-global OOM, not a qualified refusal; the tmux
server survived, while its remaining shell was terminated later.

The same attempt exposed a separate Phase-50 defect after the entropy locus: the isolated child environment
contains only the handoff protocol variable, so `ghcup` refuses because `PATH` is absent. `LTD-VAL-007` assigns
the [bounded entropy, memory/deadline envelope, and forced-cleanup replacement](../documents/engineering/validation_frame_doctrine.md#23-the-handoff-supervisor-is-resource-bounded)
to Sprint 50.4.
`LTD-VAL-008` assigns the explicit contained environment to Sprint 50.2. Both reader rows precede their typed
Haskell bindings and therefore make documentation correspondence red until those bindings and independent
reintroduction negatives exist. The attempt is an implementation diagnostic, not candidate evidence or a gate
pass. Phase 50 remains Active, Phase 51 remains Blocked, and no completed predecessor is reopened.
...
| 46 | Haskell-generated browser contracts and bundle | none | `none` | 1 | ✅ Done | [Contract](phase_46_ui_contract_generation.md) |
