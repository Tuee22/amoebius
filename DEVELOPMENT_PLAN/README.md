...
- **Standard platform services on every cluster** (below), using one **HA-capable topology** at every replica
  count. `replicas=1` preserves topology parity but is not highly available; an HA claim requires admitted
  redundant failure domains and an externally observed fault.
...
| `phase_00_…` … `phase_58_…` | One substantive document per phase (linked in the Phase overview below). |
| [later_phases.md](later_phases.md) | The in-scope, high-numbered phases (59+) not yet given their own document. |
...
**pre-cluster band** (substrate `none`, Registers 1–2); phases **24–58** are the **live band** (Register 3),
ordered by substrate; phases **59+** are the backlog.
...
| 38 | Owner-scoped UI projection runtime | linux-cpu | 3 | `UiProjectionWorker` consumes owner-keyed compacted projections to TableViews; an independent native Pulsar consumer and broker watermarks verify the fresh projection while owner-key/subscription mutants turn red | 📋 Planned | [phase_38](phase_38_ui_projection_runtime.md) |
...
| 49 | infernix core artifact lift | linux-cpu | 3 | a scoped ready artifact drives two independently observed CPU inferences through the linked infernix adapter; foreign-scope and pre-commit twins have zero effects, sibling/hash oracles agree, and scope/readiness/seed mutants turn red | 📋 Planned | [phase_49](phase_49_infernix_lift.md) |
| 50 | infernix UI lift | linux-cpu | 3 | a real browser follows UI server → workflow → ready infernix artifact → projection → typed interaction; Keycloak/Envoy/Pulsar/MinIO/worker/browser observers recover a fresh challenge while the client-artifact-scope mutant exposes a foreign-tenant dispatch/read | 📋 Planned | [phase_50](phase_50_infernix_ui_lift.md) |
| 51 | Core jitML CUDA artifact lift | linux-cuda | 3 | one scope-bound training request executes in the exact observed/provisioned CUDA owner and yields an opaque artifact only after pointer CAS; reserve/free one-short and pre-CAS twins reject, while CPU-fallback/raw-VRAM/premature-artifact mutants turn red | 📋 Planned | [phase_51](phase_51_jitml_lift_cuda.md) |
| 52 | jitML UI lift | linux-cuda | 3 | train→commit→serve→interact traverses the generic UI path; MinIO/Pulsar and an external CUDA trace recover the fresh challenge while same-tenant non-owner and foreign-tenant replays plus readiness/owner/scope mutants turn red | 📋 Planned | [phase_52](phase_52_jitml_ui_lift.md) |
...
| 55 | Single-tenant low-code UI live path | linux-cpu | 3 | one generic app traverses real OIDC/Envoy, SQL, MinIO, Pulsar/workflow/projection, and infernix; provider/CNI/request-audit/worker observers recover a fresh nonce and show the foreign subject, wrong-Origin/CSRF, and artifact-dispatch bypasses have zero read/dispatch/effect while five mutants turn red | 📋 Planned | [phase_55](phase_55_ui_single_tenant_live.md) |
...
| 58 | UI multi-zone high availability | linux-cpu → provider | 3 | ≥3 UI-server replicas, ≥3 projector replicas, and redundant dependencies survive provider-confirmed isolation of every member in one zone; a cookie-empty post-fault OIDC login/current-membership check plus read, idempotent mutation, workflow start, subscription, and two-tenant denials are externally observed across surviving zones | 📋 Planned | [phase_58](phase_58_ui_ha_multizone.md) |
| 59+ | Later phases | varies | — | each high-numbered in-scope phase gets its own single-session gate when reached (manifest correctness, native JIT/clients, additional cloud/GPU/NPU families, distillation, niche substrates, proof hardening, live backup) | 📋 Planned | [later_phases](later_phases.md) |
