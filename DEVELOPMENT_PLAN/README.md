...
5. **Validate in the registers; pre-cluster before live.** Validation happens in registers
   ...
   [`testing_doctrine.md`](../documents/engineering/testing_doctrine.md)). There are **three phase-gate
   registers** — **Register 1** (pure/golden, in-process, no cluster), **Register 2** (boundary integration
   ...
   The **pre-cluster band (phases 1–16, substrate `none`)** discharges
...
- **Illegal/unsafe cluster state is unrepresentable in a decoded `InForceSpec`** — foreclosed before any effect
  by the Dhall typecheck, the typed Haskell decoder, or the provision seal. Which stage catches a given entry is
  stated by that entry's own `Validation-locus:` line, over the five-locus axis (`Gate-1-editor` ·
  `Gate-2-decoder` · `provision-seal` · `rendered-output-golden` · `live-effect` residue) defined in
...
The formal-model phases (Phase 2, Phase 3) run TLC through a `tla2tools.jar` release **to be pinned in
Phase 2** — no version is recorded yet, so this is a named acquisition path, **not** a pin like the exact
...
| 20 | amoebius-capacity scheduler + bootstrap cutover | linux-cpu | 3 | layered on the converged Phase-19 reconciler, the `amoebius-capacity` scheduler mints `BootstrapCapacitySchedulerReady`→`ManagedCapacityReady` and binds the guarded Pod set exclusively through CAS `Reserved`→`BindingInFlight`→Binding→`Bound`, an independent observer recording that no Binding precedes a reservation CAS and each UID debited once; a pre-ready guarded workload is rejected with zero writes, re-run is a no-op, and the scheduler mutants turn it red | 📋 Planned | [phase_20](phase_20_capacity_scheduler.md) |
...
| 31 | WireGuard network fabric | linux-cpu | 3 | the topology-derived peer/rate/queue/log cost fits each node before raw-kernel mutation; the singleton renders peers from Vault key names, reconciles WireGuard, and externally demonstrates hub reachability and zero-effect overdraw | 📋 Planned | [phase_31](phase_31_network_fabric_wireguard.md) |
