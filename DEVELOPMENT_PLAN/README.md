...
| 18 | Typed bake catalog + multi-arch base image + jit-build resolver + distribution registry | linux-cpu | 3 | a snapshot-bound host envelope admits buildx CPU/memory/scratch/cache/concurrency before execution; the Dockerfile is **generated** from the typed `BakeCatalog` against a byte-for-byte golden (the hand-edit mutant turns it red); the resulting multi-arch base image publishes atomically into the in-cluster registry with no public-registry pulls | 📋 Planned | [phase_18](phase_18_base_image_registry.md) |
