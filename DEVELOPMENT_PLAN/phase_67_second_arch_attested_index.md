# Phase 67: Complementary-architecture child + the attested multi-architecture index
...
> **Read this if**: phase 67 is next in the queue, or a later phase depends on the multi-architecture index.

Phase 67 delivers the complementary architecture of the base image and the index that joins it to
[Phase 30](phase_30_base_image_registry.md)'s; its design is owned by
...
Phase 30's and are consumed unchanged. Register 3, live, on the `apple` substrate's `linux-cpu/arm64` lane.
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_30_base_image_registry.md
...
- [Sprints](#sprints)
- [Sprint 67.1: The complementary-architecture bake on its own hardware ⏸️](#sprint-671-the-complementary-architecture-bake-on-its-own-hardware-)
- [Sprint 67.2: The attested index join and its atomic advertisement ⏸️](#sprint-672-the-attested-index-join-and-its-atomic-advertisement-)
- [Sprint 67.3: The no-emulation and unattested-child negatives ⏸️](#sprint-673-the-no-emulation-and-unattested-child-negatives-)
...
⏸️ Blocked pending Phase-66 revalidation. Authored 2026-08-16 by the natural-architecture amendment, which
...
Phase 30 leaves the cluster pulling only from itself, at one architecture. This phase supplies the other one
and makes the pair a single addressable image.

It bakes the **same typed catalog** on a host whose natural architecture is the complement of Phase 30's,
...
What this phase deliberately does not do is re-derive Phase 30's work. The acquisition ladder, the bake
inventory oracle, the registry standup, the mutation-admission proxy, and the egress boundary are consumed as
Phase 30 sealed them. This phase adds one architecture, one join, and the negatives that keep the join honest.
...
**not** exercised here — that is [Phase 68](phase_68_apple_metal_host_daemon.md)'s — so naming `apple` claims
the host, not the accelerator. Any host natively running `arm64` Linux satisfies the same requirement.

**Lane:** linux-cpu/arm64 — the complement of Phase 30's `linux-cpu/amd64`.
...
```
*Design intent. Phase 67's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses and [`substrate_doctrine.md` §1.1](../documents/engineering/substrate_doctrine.md#11-the-natural-architecture-rule) the rule they enforce.*

**What the gate command accepts.** On a host whose natural architecture is `arm64`, the typed catalog bakes
that architecture's child, every baked binary executes natively by absolute path against the Phase-30 oracle's
pinned probes, and the child's attestation is written. The gate then joins that child with the Phase-30 child
...
- `test/mutant/attested_index/emulated-child` — the complementary child produced under a mounted
  foreign-architecture interpreter; red at Sprint 67.1's kernel-read `binfmt` negative.
- `test/mutant/attested_index/stub-arm64-binary` — a zero-byte binary at a baked path in the `arm64` child;
  red at Sprint 67.1's native execution and ELF check. This is the pre-amendment Phase-30 mutant, re-homed to
...
- `test/mutant/attested_index/foreign-attestation` — the `arm64` child paired with the `amd64` child's
  attestation; red at Sprint 67.2's per-child architecture binding.
...
- [`substrate_doctrine.md` §1.1 — the natural-architecture rule](../documents/engineering/substrate_doctrine.md#11-the-natural-architecture-rule):
  ...
  on a separate host rather than as a second platform argument to Phase 30's build.
...
## Sprint 67.1: The complementary-architecture bake on its own hardware ⏸️

**Status**: Blocked by the Phase-30 gate, which produces the child this sprint complements.
**Implementation**: `tools/attested_index_build.py`, `src/Amoebius/Image/Attestation.hs`
**Blocked by**: Phase 30's gate.
...
baked binary executes natively by absolute path against Phase 30's committed
...
## Sprint 67.2: The attested index join and its atomic advertisement ⏸️

**Status**: Blocked by Sprint 67.1.
...
## Sprint 67.3: The no-emulation and unattested-child negatives ⏸️

**Status**: Blocked by Sprint 67.2.
...
- `documents/engineering/substrate_doctrine.md` — the `apple` substrate's `linux-cpu/arm64` lane gains its
  first live result, while the Metal lane stays UNVERIFIED until Phase 68.

**Cross-references to add:**
- `DEVELOPMENT_PLAN/substrates.md` — the per-phase map row for this phase.
- `DEVELOPMENT_PLAN/phase_30_base_image_registry.md` — the predecessor whose child this phase joins.

## Related Documents
- [Phase 30](phase_30_base_image_registry.md) — the catalog, ladder, registry, and first child this phase consumes
- [Phase 31](phase_31_object_reconciler.md) — runs on the native-architecture child alone; the joined index is published after it, so no phase before this one consumes the join
