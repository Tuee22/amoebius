# Phase 10: Execution-epoch + scheduler + accelerator + provider-root folds
...
> Phase 8 base capacity fold and the Phase 9 storage geometry into the full-resource-vector `place` witness that
...
- [Sprints](#sprints)
- [Sprint 10.1: Execution-epoch expansion + scheduler-reservation algebra ✅](#sprint-101-execution-epoch-expansion--scheduler-reservation-algebra-)
- [Sprint 10.2: kubelet/CRI runtime-metadata + node-local OCI content/snapshot/image + physical-disk parent accounting ✅](#sprint-102-kubeletcri-runtime-metadata--node-local-oci-contentsnapshotimage--physical-disk-parent-accounting-)
- [Sprint 10.3: Accelerator residency/net-allocatable-VRAM + provider-root disk template + engine/build/etcd/monitoring compute ✅](#sprint-103-accelerator-residencynet-allocatable-vram--provider-root-disk-template--enginebuildetcdmonitoring-compute-)
- [Sprint 10.4: The composed full-resource-vector place-witness — properties + independent validator + per-axis mutants ✅](#sprint-104-the-composed-full-resource-vector-place-witness--properties--independent-validator--per-axis-mutants-)
- [Sprint 10.5: The execution/accelerator/provider-root fold-negative corpus + the composed gate ✅](#sprint-105-the-executionacceleratorprovider-root-fold-negative-corpus--the-composed-gate-)
...
bound to source snapshot `sha256:374fe1168768d0fe…`; Phase 10 owns no remaining migration deferral.
...
and the gap is recorded against Phase 10 in
...
axes with the Phase-8 base fold and the Phase-9 storage geometry into the single whole-deployment place-witness.
Phase 12's post-bind provision seal invokes the same folds only after full bind/expansion; Phase 10 does not move
...
durable/cache demand (Phase 9), accelerator devices plus every identity-complete owner residency epoch, and every
...
([phase_08_capacity_core_folds.md](phase_08_capacity_core_folds.md)); the logical→physical
...
([phase_09_storage_geometry_folds.md](phase_09_storage_geometry_folds.md)); the capability→provider→shape
binder ([phase_11_capability_bind.md](phase_11_capability_bind.md)) and the whole-deployment provision seal
([phase_12_provision_seal.md](phase_12_provision_seal.md)) that re-exercise these folds post-bind; the
`InferenceEngine` capability + accelerator coexistence provision
([phase_13_inference_accelerator_provision.md](phase_13_inference_accelerator_provision.md)); and the live
residue — the same-binary scheduler role's Reserved→BindingInFlight→Bound around real Kubernetes Binding
([phase_32_capacity_scheduler.md](phase_32_capacity_scheduler.md)), the snapshot-bound live preflight and
action/token/CAS plumbing ([phase_31_object_reconciler.md](phase_31_object_reconciler.md)), retained-carve
allocation ([phase_33_retained_storage.md](phase_33_retained_storage.md)), provider-capacity creation
([phase_49_provider_deploy_checkpoint.md](phase_49_provider_deploy_checkpoint.md), [phase_52_provider_dynamic_nodes.md](phase_52_provider_dynamic_nodes.md)), and live CUDA/Metal enaction
([phase_65_jitml_lift_cuda.md](phase_65_jitml_lift_cuda.md), [phase_68_apple_metal_host_daemon.md](phase_68_apple_metal_host_daemon.md)). Phase 10 owns the pure
...
analogous to the Phase 6 decode battery, the Phase 7 property suite, and the sibling Phase 8/9 fold gates.
...
clauses require for Phase 10; it strengthens, never weakens, the Gate and sprint Validations above. It is this
phase's **partition** of the forty-one fixtures / per-fold mutant battery of the original capacity/topology corpus
— the base-fold and topology slice is owned by [phase_08_capacity_core_folds.md](phase_08_capacity_core_folds.md)
and the storage-geometry slice by [phase_09_storage_geometry_folds.md](phase_09_storage_geometry_folds.md); all
...
flowchart LR
  ...
  s0["Sprint 10.1: Execution-epoch expansion + scheduler-reservation algebra"]
  s1["Sprint 10.2: kubelet/CRI runtime-metadata + node-local OCI…"]
  s2["Sprint 10.3: Accelerator residency/net-allocatable-VRAM + provider-root…"]
  s3["Sprint 10.4: The composed full-resource-vector place-witness —…"]
  s4["Sprint 10.5: The execution/accelerator/provider-root fold-negative…"]
  gate["the phase 10 gate"]
...
```
*Orientation. The seams Phase 10 built and sealed in order; [Gate integrity](#gate-integrity) owns the validated apparatus.*
...
the bind, infrastructure-plan, context, and provision path around those folds belongs to Phase 12 and is no
...
owned by [phase_08_capacity_core_folds.md](phase_08_capacity_core_folds.md) but is re-exercised through the
...
`illegal_elastic_*`) belong to Phase 8; the storage-geometry negatives (`illegal_store_over_backing`,
`illegal_topic_time_only_offload`, `illegal_hot_tier_over_bookie`, `illegal_cache_over_local_pool`,
`illegal_incluster_cache_bound_mismatch`) belong to Phase 9. This phase does not
re-assert them.
- `illegal_hard_ceiling_overcommit` is a stable fixture identifier; **its kind-indexed execution controller/rollout/live-epoch cases are variants of it** — separately making only a controller webhook,
  ...
  internal guard-weakening mutant that attempts to inject `{ 0, 0 }`; the Phase-6
...
aggregate. The per-axis/per-capability validator mutants of Sprint 10.4 are additional and separately required.
...
Defined in Sprint 10.4 Deliverables; the composed validator never
...
durable/cache (Phase 9), accelerator net-allocatable VRAM, execution/admission envelopes, and provider-root.
...
- [`resource_capacity_doctrine.md §4`](../documents/engineering/resource_capacity_doctrine.md#4-the-total-fold-fits-carve-place-and-the-nesting)
  ...
  and the execution/accelerator/provider-root demands this phase adds atop the Phase-8 base fold.
...
- [`illegal_state_catalog.md §4.6`](../documents/illegal_state/illegal_state_techniques.md#46-capacity-accounting--placement-witness-compute-and-summed-demand-within-capacity-storage-checked)
  ...
  (The [§4.7](../documents/illegal_state/illegal_state_techniques.md#47-compatibility--topology-relations-by-construction-over-a-collection) compatibility/topology technique and the [§3.13](../documents/illegal_state/illegal_state_topology.md#313-a-compute-engine-incompatible-with-its-substrates-managed-providers-first-class)–[§3.16](../documents/illegal_state/illegal_state_topology.md#316-a-multi-node-rke2-cluster-with-fewer-linux-hosts-than-nodes-or-a-host-reused) topology entries are discharged by [phase_08_capacity_core_folds.md](phase_08_capacity_core_folds.md); the durable/object/Pulsar [§3.19](../documents/illegal_state/illegal_state_storage.md#319-an-application-consuming-more-storage-than-its-backing-minio-and-pulsar)–[§3.21](../documents/illegal_state/illegal_state_storage.md#321-capacity-growth-without-an-amoebius-owned-scaling-policy) storage-geometry entries by [phase_09_storage_geometry_folds.md](phase_09_storage_geometry_folds.md).)
...
## Sprint 10.1: Execution-epoch expansion + scheduler-reservation algebra ✅
...
partitions. These declarations live beside their owner folds rather than enlarging the Phase-8 base
...
numbers only — the pure expansion fold Phase 12's `provision` seal later invokes.
...
- The pure scheduler model seals prior+desired, controller-child-indexed candidate templates and one aggregate
  ...
  [phase_32_capacity_scheduler.md](phase_32_capacity_scheduler.md)) performs Reserved→BindingInFlight→Bound
...
## Sprint 10.2: kubelet/CRI runtime-metadata + node-local OCI content/snapshot/image + physical-disk parent accounting ✅
...
`PhysicalDiskPartition` parent equations — all as pure, checked `provision-seal` operations invoked by Phase 12's
`provision` seal.
...
- The declarations these two folds exchange live beside them rather than in the Phase-8 base
...
## Sprint 10.3: Accelerator residency/net-allocatable-VRAM + provider-root disk template + engine/build/etcd/monitoring compute ✅
...
- `MonitoringWorkBudget` with finite workflow/rule/series/sample-rate/interval/CPU/memory/retention, structural
  ...
  concurrent executor/workspace peak as an executor envelope; its checkpoint storage remains the Phase-9
  object-store arm.
...
## Sprint 10.4: The composed full-resource-vector place-witness — properties + independent validator + per-axis mutants ✅

**Status**: Done — the capability is re-established by the migrated gate; the sprint's committed-ledger, pinned-toolchain, and repository-resident evidence mechanics are superseded
**Implementation**: `src/Amoebius/Capacity/Composed.hs` retains the Phase-8 placement, Phase-9 storage, and
Phase-10 execution/runtime/accelerator/provider-root witnesses; `test/spec/dsl/ExecutionAcceleratorProps.hs`
...
- The per-axis/per-capability seeded-mutant battery: **one seeded mutant per resource/capability axis** — drop
  ...
  individually required to turn the suite red (§M.2), plus the host-only compute mutants of Sprint 10.3.
...
## Sprint 10.5: The execution/accelerator/provider-root fold-negative corpus + the composed gate ✅
...
**Blocked by**: None.
**Independent Validation**: the gate applies the Phase-10 folds (the execution expansion, scheduler-reservation algebra,
runtime-metadata/node-local, accelerator-residency, provider-root, and engine/etcd/build/monitoring helper
folds, composed with the Phase-8 base `fits`/`podFits`/`carve`/`place` and the Phase-9 storage folds)
**directly to each hand-authored demand/capacity fixture**. The higher-level bind, infrastructure-plan,
context, and provision path is outside this phase and covered by phases 11–12. The composed positives place
...
Register-1), `documents/engineering/testing_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-10
status when the gate passes), `DEVELOPMENT_PLAN/substrates.md` (the Phase-10 `none` gate row).
...
- The fold-negative fixtures on this seam — `illegal_hard_ceiling_overcommit` ([§3.17](../documents/illegal_state/illegal_state_capacity.md#317-an-over-committed-deploy-or-workload-host--vm--cluster-capacity-exceeded)),
  ...
  - Compile-time-foreclosed neighbours
    ...
    [phase_08_capacity_core_folds.md](phase_08_capacity_core_folds.md) and
    [phase_09_storage_geometry_folds.md](phase_09_storage_geometry_folds.md).
- The composed positive fixtures `legal_multisubstrate_cluster` (the [§3.13](../documents/illegal_state/illegal_state_topology.md#313-a-compute-engine-incompatible-with-its-substrates-managed-providers-first-class) heterogeneous carve-out, exercising distinguishable `Unified` and `SplitRuntime` routing plus a presentation-adjusted VM/instance-store fit; its case table includes exact-fit physical-raw and nested-VM-usable parent equations, including `systemReserve raw parent debit + Σ unique VM provisionedBytes + Σ unique other raw parent debits == allocatableRawBytes`, exact-empty `FirstDeployment`, a recreate zero-live step, an update with different old/new full execution envelopes plus added/removed units, and an exact component→role→layout-backing metadata accounting with one debit per grouped carve) and `legal_managed_eks` (EKS first-class, requiring two materialized instances from one candidate class, deriving each root-EBS request from its class-local system/carve high-water, debiting the distinct root quota, and proving instantiated backing/carve/device identities are disjoint), each asserted to decode and `place` feasibly across the whole resource vector. Their
  ...
  is owned by [phase_08_capacity_core_folds.md](phase_08_capacity_core_folds.md) but also places feasibly through
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Capacity/{Scheduler,HostReservation,
  ...
  branches of `Types.hs`/`Fold.hs`, and the composed placement property + gate suites as Phase-10 design-first
  rows.
- `DEVELOPMENT_PLAN/phase_08_capacity_core_folds.md` / `DEVELOPMENT_PLAN/phase_09_storage_geometry_folds.md` —
  the base-fold and storage-geometry slices this phase composes into the full-resource-vector witness.
- `DEVELOPMENT_PLAN/phase_12_provision_seal.md` — the post-bind provision seal that re-exercises these folds.
...
- [phase_07](phase_07_illegal_state_corpus.md) — the illegal-state corpus, properties, and validation-locus
  ledger this phase extends
- [phase_08_capacity_core_folds.md](phase_08_capacity_core_folds.md) — the base `fits`/`carve`/`place` fold and
  the `ComputeEngine`/`LinuxHost`/`Topology` relation this phase composes over
- [phase_09_storage_geometry_folds.md](phase_09_storage_geometry_folds.md) — the logical→physical storage
  geometry this phase composes into the full-resource vector
- [phase_11_capability_bind.md](phase_11_capability_bind.md) — the capability → provider → shape binder built
  atop these folds
- [phase_12_provision_seal.md](phase_12_provision_seal.md) — the whole-deployment provision seal that re-exercises
  these folds post-bind
- [phase_13_inference_accelerator_provision.md](phase_13_inference_accelerator_provision.md) — the
  `InferenceEngine` capability + accelerator residency/coexistence provision built atop the accelerator fold
- [phase_32_capacity_scheduler.md](phase_32_capacity_scheduler.md) — the live same-binary scheduler role that
  enacts Reserved→BindingInFlight→Bound around Kubernetes Binding
- Phase 10 execution/accelerator ledger — the human-readable
