# Phase 12: Whole-deployment provision seal + expansion

> **Purpose**: Take the fully expanded `BoundDeployment` produced by [Phase 11](phase_11_capability_bind.md),
> run it through the conditional infrastructure planner/materialization boundary and then the Phase-8/9/10
...
- [Sprints](#sprints)
- [Sprint 12.1: The conditional infrastructure planner + materialization boundary (`planInfrastructure`) ✅](#sprint-121-the-conditional-infrastructure-planner--materialization-boundary-planinfrastructure-)
- [Sprint 12.2: The whole-deployment `provision` fold + execution/runtime-storage/object/observability/migration/scheduler expansion ✅](#sprint-122-the-whole-deployment-provision-fold--executionruntime-storageobjectobservabilitymigrationscheduler-expansion-)
- [Sprint 12.3: The `ProvisionedSpec` seal + identity-keyed render-source set + four-stage activation ✅](#sprint-123-the-provisionedspec-seal--identity-keyed-render-source-set--four-stage-activation-)
- [Sprint 12.4: The provision-seal property/corpus + the Register-1 gate ✅](#sprint-124-the-provision-seal-propertycorpus--the-register-1-gate-)
...
`sha256:450cd22ff1ff6667…`; Phase 12 owns no remaining migration deferral.
...
all ten seeded mutants redden. Evidence and the ledger move into `.build/runs/phase_12/<run-id>/`, and 26
...
gap is recorded against Phase 12 in [`legacy_tracking_for_deletion.md`](legacy_tracking_for_deletion.md).
...
is recorded in `ledgers/phase_12_provision_seal.md`. This phase opened after the [Phase 11](phase_11_capability_bind.md)
gate (the capability union + representational `bind` + object-node-multiset shape oracle, which produces the
wholly-unprovisioned `BoundDeployment` this phase consumes) and the [Phase 10](phase_10_execution_accelerator_folds.md)
...
`MaterializedExecutionInstance`s and empty-capable `ExecutionEpoch`s, drives the Phase-8 capacity fold, the
Phase-9 storage-geometry fold, and the Phase-10 execution-epoch / runtime-metadata / scheduler-reservation /
...
- the capability union, the `CapabilityBinding`, the total `bind`, the object-node-multiset shape oracle,
  and the Gate-1/Gate-2 negatives ([Phase 11](phase_11_capability_bind.md))
- the *representational* `InferenceEngine`/`EngineRuntime` union and the identity-complete accelerator
  ...
  ([Phase 13](phase_13_inference_accelerator_provision.md))
- the *soundness* of the `fits`/`carve`/`place` folds and the composed full-resource-vector place-witness
  gate this phase merely invokes ([Phase 8](phase_08_capacity_core_folds.md), [Phase 9](phase_09_storage_geometry_folds.md), [Phase 10](phase_10_execution_accelerator_folds.md))
- the pure `renderAll :: ProvisionedSpec -> [K8sObject]` that consumes the sealed set ([Phase 14](phase_14_render_manifest_goldens.md))
- the live `amoebius-capacity` scheduler runtime ([Phase 32](phase_32_capacity_scheduler.md))
- and the live realization of any provider or the actual jit-resolve of an engine
  ([Phase 53](phase_53_determinism_jitcache.md), the live band).

**Substrate:** none — no host, no cluster, no provider; the gate is an in-process `cabal test` over
`planInfrastructure` + `provision` + `provisionRenderSources`, analogous to the Phase-10 fold battery it invokes.
...
[Phase 11](phase_11_capability_bind.md); the identity-complete accelerator source/workload/residency/coexistence
...
mutants) stays in [Phase 13](phase_13_inference_accelerator_provision.md). This phase inherits only the
seal-locus slice below.

```mermaid
flowchart LR
  %% register: orientation
  s0["Sprint 12.1: The conditional infrastructure planner + materialization…"]
  s1["Sprint 12.2: The whole-deployment provision fold…"]
  s2["Sprint 12.3: The ProvisionedSpec seal + identity-keyed render-source set…"]
  s3["Sprint 12.4: The provision-seal property/corpus + the Register-1 gate"]
...
```
*Orientation. The seams Phase 12 built in order; [Gate integrity](#gate-integrity) owns the passing apparatus.*
...
`bind`-checked against its `golden_servicespec_<arm>_<shape>` in Phase 11 — are provisioned against their
...
- [`resource_capacity_doctrine.md §3`](../documents/engineering/resource_capacity_doctrine.md#3-the-types-quantity-capacity-demand-budget)
  ...
  ordering's tail: after Phase 11 expands every provider/shape into a `BoundDeployment`, the seal runs the
  Phase-8/9/10 folds and hands only the checked opaque result to Phase 14. A capacity check over a pre-bind
...
- [`service_capability_doctrine.md §4`](../documents/engineering/service_capability_doctrine.md#4-capability--provider--shape-the-binding)
  ...
  deployment rules and expanded by `bind` (Phase 11); this phase provisions that fully expanded graph against
...
- [`service_capability_doctrine.md §4.1`](../documents/engineering/service_capability_doctrine.md#41-the-inferenceengine-capability--the-engine-is-target-offering-selected-and-jit-resolved-never-authored)
  ...
  **`provision-seal` `Left`**. This phase implements that seal-locus rejection over the accelerator folds it
  ...
  are [Phase 13](phase_13_inference_accelerator_provision.md).
...
- [`manifest_generation_doctrine.md §2`](../documents/engineering/manifest_generation_doctrine.md#2-the-typed-manifest-model-renderall-is-the-sole-public-pure-function-to-objects)
  ...
  unique identity-keyed `ProvisionedRenderSourceSet` under the opaque `ProvisionedSpec` that Phase 14's
...
## Sprint 12.1: The conditional infrastructure planner + materialization boundary (`planInfrastructure`) ✅
...
- An in-file honesty note: `planInfrastructure` produces a *plan value*, not a live provider action; the live
  validation/CAS-enaction of any batch is the live band ([Phase 49](phase_49_provider_deploy_checkpoint.md)).
...
## Sprint 12.2: The whole-deployment `provision` fold + execution/runtime-storage/object/observability/migration/scheduler expansion ✅
...
provision the fully expanded `BoundDeployment` against its topology by driving the Phase-8/9/10 folds over the
...
- `provision :: ProvisionContext -> Topology -> BoundDeployment -> Either ProvisionError ProvisionedSpec`, run
  ...
  [Phase 13](phase_13_inference_accelerator_provision.md) — the `ProvisionedCudaOwnerDemand` /
...
- Placement evaluates the **full** resource vector in every epoch — CPU, memory, pod/CNI/CSI slots, logical Pod
  ...
  device/residency epochs — by driving the Phase-8/9/10 folds, and retains the componentwise transition witness.
...
- The runtime-storage fold (`src/Amoebius/Capacity/RuntimeStorage.hs`) derives every metadata component's bytes
  ...
  form keyed by authenticated `PodUid`; Phase 12 tests only the pure planned/normalized forms.
...
- The transition resolution: using the Gate-2-validated branded refs, `provision` resolves
  ...
  into their private storage-geometry witnesses (Phase-9 fold), and stores exactly one deployment-global
...
- An in-file honesty note: `provision` produces a value, not a live provider; the same pure identity/revision
  epoch algebra is later reused by live admission ([Phase 32](phase_32_capacity_scheduler.md)), which performs
...
## Sprint 12.3: The `ProvisionedSpec` seal + identity-keyed render-source set + four-stage activation ✅
...
identity-keyed render-source set with per-field ownership and a four-stage activation order, so Phase 14's
...
- The activation discipline: a later typed diff/enactor must honor activation, so a managed-node
  ...
  desired set. Phase 14 privately maps the unique source set and exposes only whole-deployment `renderAll`.
- An in-file honesty note: this seal produces the *input* to `renderAll`, not manifests; the byte-for-byte
  golden-locked render is [Phase 14](phase_14_render_manifest_goldens.md), and the live diff/enact honoring
...
## Sprint 12.4: The provision-seal property/corpus + the Register-1 gate ✅
...
The 18 per-arm/shape fixtures remain inherited from [Phase 11](phase_11_capability_bind.md).
**Blocked by**: None.
...
`documents/engineering/testing_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-12 status when the
gate passes), `DEVELOPMENT_PLAN/substrates.md` (the Phase-12 `none` gate row).
...
- The property battery (`test/spec/capability/ProvisionProps.hs`,
  ...
  Phase-13-provided accelerator arms) accelerator witnesses; the structural inventory proves `BoundDeployment`
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `planInfrastructure`/`provision`/`provisionRenderSources`
  ...
  property + gate suites as Phase-12 design-first rows.
...
- [phase_11](phase_11_capability_bind.md) — the capability union + total `bind` + shape oracle that produces
  the wholly-unprovisioned `BoundDeployment` this phase seals
- [phase_10](phase_10_execution_accelerator_folds.md) — the execution-epoch / scheduler-reservation /
  runtime-metadata / accelerator folds and the composed full-resource-vector place-witness gate this seal
  invokes
- [phase_09](phase_09_storage_geometry_folds.md) — the logical→physical storage-geometry fold this seal invokes
- [phase_08](phase_08_capacity_core_folds.md) — the base `fits`/`carve`/`place` capacity fold this seal invokes
- [phase_13](phase_13_inference_accelerator_provision.md) — the representational `InferenceEngine`/`EngineRuntime`
  union and the identity-complete accelerator source/workload/residency/coexistence provision that layers on
  this seal
- [phase_14](phase_14_render_manifest_goldens.md) — the pure deployment-global
  `renderAll :: ProvisionedSpec -> [K8sObject]` that consumes the identity-keyed render-source set this phase seals - [phase_32](phase_32_capacity_scheduler.md) — the live `amoebius-capacity` scheduler that reuses this seal's
