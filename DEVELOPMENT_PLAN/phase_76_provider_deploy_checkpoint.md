...
Blocked by redesigned Phase 75, its independent validation, and gate pass; every earlier
...
witness. Gate-passed Phase 25 schema generation, Phase 26 GADT decoding, and Phase 9 capacity/topology folds
...
| `Command` | UNRESOLVED — blocks validation: typed semantic payload and gate evidence missing; prior prose: `pb validate phase 76` is the target command only; `pb` may only make the minimal platform distinction, establish the contained toolchain, build the source-bound binary, and exec it with argv unchanged, while the Haskell verdict entry point remains UNRESOLVED and blocks validation. |
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and complete gate execution missing; prior prose: Exact `ImmediatePredecessorPass` for Phase 75; candidate execution refuses an absent, stale, replayed, or different-source result. |
...
**Blocked by**: [Phase 75](phase_75_gateway_migration_drills.md) gate pass
...
1. The control-plane daemon issues a provider deploy that reaches a ready EKS control plane + base node group. The
   ...
   code-inspection claim: a Haskell-declared negative generates a run-local attempted host-shell invocation
