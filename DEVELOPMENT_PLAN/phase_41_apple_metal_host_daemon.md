# Phase 41: Apple-Metal host compute daemon
...
> sole Phase-29 mutation gateway fronting MinIO, with the native worker built **headless
...
binds the in-cluster Phase-29 content-mutation gateway and Pulsar service to **host-only loopback NodePorts** reachable only
...
messages, mutations pass only through the Phase-29 gateway into the content-addressed MinIO store, and there is
...
content-addressed cache** (Phase 38), never a baked or URL-fetched payload; on the Apple substrate the cache
...
them: Phase 17's substrate detection, `pb` midwife handoff, and no-`PATH` tool-ensure kernel; Phase 23's MinIO
and Pulsar standard services; Phase 28's native Pulsar client; Phase 29's content-addressed store and workflow
runtime; Phase 38's determinism kernel; Phase 38's jit-build engine cache; and Phase 22's Vault secrets-by-name.

```mermaid
flowchart LR
  apple[Apple Silicon host: detected apple substrate] --> lima[Lima Ubuntu-24.04 VM: single-node cluster]
  lima --> svc[Phase-29 content-mutation gateway fronting MinIO plus Pulsar]
...
the sole Phase-29 content-mutation gateway and Pulsar (not raw MinIO) on loopback NodePorts, builds the native worker **headless on-host via the fixed Metal bridge
...
- **Input-dependent output oracle (§M.1/§M.3).** The artifact retrieved from MinIO by content address must be
  ...
  worker that compiles MSL yet **discards the GPU result and re-uploads the committed golden bytes keyed by input
  identity** would still satisfy input-dependence, content-addressing, and the cache-bypass recompute — and the
  ...
  off-implementation NumPy reference `test/golden/phase_41/metal_job_ref.py` over that run-time input; the
...
- **Representative set (§M.7).** The gate's representative set is exactly: one apple substrate (Apple-Silicon
  ...
  and the four wild-exposure negatives enumerated in Sprint 41.5. No other shape is claimed.
- **VM-disk geometry is materialized and observed (§M.3/§M.4).** Before create, the independent oracle
  ...
  twice.
- **Leak-free, defined (§M).** "Tears down leak-free" at Phase 41 means, asserted by the postflight probe:
  ...
  swept to its pre-run state. Phase 42's general postflight sweep is not yet available, so this phase pins its
...
The worker's source-equal Phase-28/29 work/result topic and subscription demand also retains finite message/
...
The host content client terminates at the Phase-29 sole content-mutation gateway NodePort; raw MinIO backend
...
The content gateway/collector, MinIO backend, Pulsar, Vault and Phase-26 singleton instances are inherited
...
- [`host_cluster_comms_doctrine.md §2`](../documents/engineering/host_cluster_comms_doctrine.md#2-the-decision-that-was-open-and-is-now-resolved)
  ...
  (*coordination is Pulsar + MinIO*, with MinIO mutation mediated by the Phase-29 gateway), the
...
## Sprint 41.1: Apple substrate management — Lima Linux VM + brew lazy tool-ensure 📋
...
**Blocked by**: Phase 17 gate (external prereq — substrate detection, the `pb` midwife handoff, and the closed-enum no-`PATH` lazy-tool-ensure kernel that invokes by absolute path, here extended to the brew root and the Lima provider on apple)
...
invoked by absolute path through brew — the substrate foundation every later Phase-41 sprint stands on. The
...
## Sprint 41.2: Host-only loopback NodePort exposure of the content-mutation gateway + Pulsar 📋

**Status**: Planned
**Implementation**: `src/Amoebius/HostComms/NodePort.hs`, `src/Amoebius/HostComms/Loopback.hs` (target paths; not yet built)
**Blocked by**: Sprint 41.1 (the Lima VM provides the node network whose NodePorts must be bound to the host's loopback); Phase 23 gate (external prereq — the in-cluster Pulsar and MinIO-backed storage services); Phase 29 gate (external prereq — the sole content-mutation gateway fronting MinIO)
**Independent Validation**: after bring-up, the Phase-29 content-mutation gateway and Pulsar are reachable
...
## Sprint 41.3: Headless host-native Metal bridge + native worker build (no Tart) 📋
...
- Runtime MSL compilation: the host binary renders Metal Shading Language, writes a content-addressed
  source-metadata cache record into the Phase-38 `CacheBudget`-bounded cache, and dispatches through the
  bridge's `MTLDevice.makeLibrary(source:options:)` with fast-math **off** (the Phase-38 determinism
...
2. Compile generated MSL at runtime through the bridge and dispatch a kernel for both `job_A` and `job_B`;
   ...
   reference `test/golden/phase_41/metal_job_ref.py` over that input — no committed golden for `job_C` exists, so a
...
## Sprint 41.4: Host compute daemon lifecycle as a managed subprocess 📋

**Status**: Planned
**Implementation**: `src/Amoebius/HostWorker/Lifecycle.hs`, `src/Amoebius/HostWorker/Supervise.hs` (target paths; not yet built)
**Blocked by**: Sprint 41.3 (the built native worker binary is what the lifecycle manages); Sprint 41.1 (the apple substrate context the subprocess runs in)
...
## Sprint 41.5: Channel-2 peer + wild-exposure unrepresentable + the Apple-Metal peer gate 📋
...
- The gate `.dhall` (`test/dhall/phase_41_apple_metal_peer.dhall`) is a **generated artifact emitted from
  ...
  `test/dhall/phase_41_illegal/` (authored, committed in Phase 0). The gate `.dhall`, once emitted, drives:
...
### Validation
1. Each of the four committed wild-exposure negatives in `test/dhall/phase_41_illegal/` (NodePort as
...
- `documents/engineering/host_cluster_comms_doctrine.md` — its §9 planning-ownership pointer resolves to
  delivered Phase-41 sprints, and the §2/§5/§6 honesty notes flip from "resolved design decision / sibling
...
- `documents/engineering/substrate_doctrine.md` — its §9 planning-ownership pointer resolves to delivered
  Phase-41 sprints; the §4.3 "no macOS build VM" note and the §5 host-worker description gain their first
...
- `documents/engineering/resource_capacity_doctrine.md` — record the Phase-41 live check for private
...
- [README.md](README.md) — flip the Phase-41 row status once the gate passes and link this document.
- [substrates.md](substrates.md) — record Phase 41's gate substrate (apple) in the per-phase substrate map, and
...
- [system_components.md](system_components.md) — register the host-worker and host-comms modules
  ...
  as Phase-41 design-first rows.

## Related Documents
- [README.md](README.md) — the live tracker; the Phase 41 row is the authoritative one-line gate and status
...
- [phase_38](phase_38_determinism_jitcache.md) — the jit-build engine resolver + `CacheBudget` cache the Metal
  kernel is materialized into
- [phase_40](phase_40_jitml_lift_cuda.md) — the CUDA jitML lift; its Windows-CUDA case is the structurally
