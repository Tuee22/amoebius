# Phase 48: Content store + workflow runtime (Pulsar-Failover single-writer)

> **Purpose**: Stand up amoebius's durable-artifact substrate — the three-tier content-addressed MinIO store —
> and the orchestrator/worker workflow runtime on top of the Phase-46 native Pulsar client, gated live on
...
- [Sprints](#sprints)
- [Sprint 48.1: Three-tier content-addressed MinIO store ⏸️](#sprint-481-three-tier-content-addressed-minio-store-)
- [Sprint 48.2: Orchestrator/worker workflow runtime + store/fetch by manifest SHA ⏸️](#sprint-482-orchestratorworker-workflow-runtime--storefetch-by-manifest-sha-)
- [Sprint 48.3: Pulsar Failover standby takeover + leak-free teardown (gate) ⏸️](#sprint-483-pulsar-failover-standby-takeover--leak-free-teardown-gate-)
- [Sprint 48.4: Register-2.5 workflow failover takeover under simulated fault ⏸️](#sprint-484-register-25-workflow-failover-takeover-under-simulated-fault-)
...
`kind` cluster brought up by the Phase 35 bootstrap coordinator with
Pulsar and MinIO already standing as HA platform services (Phase 41) on retained storage (Phase 39), and it
opens only after the Phase 46 gate (the native-protocol Pulsar client, CBOR codec, four subscription types,
...
ML-workflow runtime; those remain sibling context, while the Phase-48 evidence is an amoebius result. The
gate sealed the canonical store, terminal-Job protocol, two native Failover cycles, full three-class cleanup,
256 deterministic schedules, and all ten red mutants. Cross-cluster replication, the Phase-59 experiment-hash
...
`experiment-hash` namespace within an app's Phase-45 ObjectStore bucket. Every namespace also binds a finite
...
charged until an observed GC deletion; the logical peak consumes Phase 41's MinIO erasure/healing and uniform
claim-plan witness rather than assuming logical bytes equal physical disk. Second, an **orchestrator/worker workflow runtime** on top of the Phase-46 client: an orchestrator worker produces a workflow `command` on a
...
This phase also closes the Job-terminal live-proof boundary deliberately left open in Phase 37. Phase 37
...
name-ordered standby, with the Phase-46 at-least-once contract redelivering the un-acked command; the store's
...
typeclass, and SplitMix seed derivation are the Phase 59 determinism kernel, not this phase.
...
Sprints 38.1 and 38.3, and MUST turn red on the mutants named there — among them the insertion-order-leaking
...
- **The critical-window kill:** the active worker is killed **after its store write and before its `event`
  ack** — the same window Sprint 48.4 injects in simulation — and that placement (store-write-done,
...
- **The failed-commit capacity drill:** a separate drill uploads the maximum blob/manifest write set and
  ...
  the Phase-41 per-drive erasure/healing and uniform-claim witness.
- **The first live Phase-37 terminal protocol:** the collector/verification Job's exact completion variant is
...
- **Leak-free teardown and an independent re-run:** the whole topology spins up, runs, and tears down
  leak-free — the postflight sweep inventories every resource class enumerated in Sprint 48.3 and fails hard
...
- [`daemon_topology_doctrine.md §3.1`](../documents/engineering/daemon_topology_doctrine.md#31-exactly-one-pod-is-a-k8setcd-property-not-an-amoebius-election)
  ...
  control-plane daemon (Phase 44), whose single-instance is delegated to k8s/etcd, so nothing in this phase
...
- [`pulsar_client_doctrine.md §5`](../documents/engineering/pulsar_client_doctrine.md#5-the-capability-surface-lookup--produce--consume--subscribe--seek)
  ...
  — *the capability surface (the Failover subscription)* / *at-least-once with broker-side dedup*: the Phase-46
...
- [`deterministic_simulation_doctrine.md §4`](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  — *Register 2.5 — where deterministic simulation sits*: Sprint 48.4 runs the real Sprint-43.2/32.3 workflow
  runtime under `IOSimPOR` against the Phase-22 modeled environment as a Register-2.5 lower-register cross-check
...
- [`chaos_failover_doctrine.md §12`](../documents/engineering/chaos_failover_doctrine.md#12-the-moral-core--proven-tested-assumed)
  ...
  **cross-cluster** failover boundary and its formal model are owned by
  ...
  and realized by Phase 53's geo-replication plus Phase 54's gateway-migration drills, not here — this phase
...
## Sprint 48.1: Three-tier content-addressed MinIO store ⏸️
...
- A mandatory `ObjectStoreDemand` per namespace: exact physical-id-keyed committed residents, structural
  ...
  future/transient extent peak, merges it with all Phase-41 producer arms, and feeds that structure—not a byte
...
- The collector/verification Job consumes the Phase-37 terminal state machine live for the first time. Its
...
- Store keys taken under a caller-supplied `experiment-hash` namespace string within the app's ObjectStore
  ...
  seed derivation (Phase 59 kernel work).
...
1. Run this suite at **Register 3** against the **single-node kind cluster's live MinIO** — the standing
   Phase-41 HA service on the Phase-39 retained PV, never an in-process or local S3 fake, so the evidential
...
2. Encode the same logical manifest from **two writers that each first construct it with a distinct component
   ...
   independent canonicalizer under `.build/runs/phase_42/`. Assert the generated noncanonical case fails
...
the run, and rerun the Phase-48 gate.

## Sprint 48.2: Orchestrator/worker workflow runtime + store/fetch by manifest SHA ⏸️
...
through the Phase-46 dedup. The numbered `### Validation` list below adds the no-election audit and the
...
wire the Phase-46 client, its topology algebra, and the Sprint-43.1 store into an orchestrator/worker runtime
...
### Deliverables
- An orchestrator daemon that, using the Phase-46 topology algebra, produces a workflow `command` on the
  derived topic and consumes the corresponding `event`; it is an unelected worker daemon, not a leader.
- Worker daemons that consume the command, write a content-addressed artifact to the store (Sprint 48.1), and
  produce an `event` carrying the manifest SHA — CBOR payloads throughout (Phase 46), a large artifact carried
...
- The runtime is scheduled under the Deployment-`replicas=1` control-plane daemon (Phase 44); no orchestrator/worker role
...
2. Assert a retried produce and a redelivered consume are collapsed by the Phase-46 broker-side dedup so
...
### Remaining Work
None. Delivered and validated by the Phase-48 gate.

## Sprint 48.3: Pulsar Failover standby takeover + leak-free teardown (gate) ⏸️
...
### Deliverables
- Worker daemons attached over a Pulsar **Failover** subscription (Phase 46): one active, the rest
  name-ordered hot standbys; single-writer liveness is the subscription, safety is the store's ETag-CAS commit
  point plus the typed `AdvancePredicate` (Sprint 48.1), so even a bounded failover overlap cannot regress
  HEAD.
- The **critical-window kill-injection path**: the kill lands **after the active worker has completed its store write and before it acks the `event`** (the same window Sprint 48.4 injects in simulation, restated for the
  live gate), so the load-bearing standby re-fetch + bounded failover overlap are actually exercised — not a kill
  against an idle, already-drained worker. Pulsar promotes the name-ordered standby, the Phase-46 at-least-once
...
- The **postflight sweep's explicit inventory contract**: the sweep MUST inventory, and the ledger MUST record,
  ...
  (the durable test-flagged bytes reclaimed by Phase 62). The sweep emits its **full inventory list and the named retained set** into the per-run ledger; **any non-empty remainder outside the retained set is a hard gate failure**. (Durable-byte reclaim staying with Phase 62 is the *only* exemption, and only for the
...
2. **Idempotency and leak-free teardown, disambiguated.** "**Idempotent setup**" means a *second `apply` of the
   topology against the still-standing topology is a byte-stable no-op* (the Phase-37 sense — zero fields
...
> and realized by Phase 53's geo-replication plus Phase 54's gateway-migration drills, not here. Pulsar's own
...
> reclaim of test-flagged durable bytes is the elevated harness's prerogative (Phase 62), kept out of the
...
## Sprint 48.4: Register-2.5 workflow failover takeover under simulated fault ⏸️
...
**Register 2.5** on the **`none`** substrate: run the *real* Sprint-43.2/32.3 workflow runtime and its
Failover-takeover path — the daemon/workflow code written against `io-classes` — under `IOSimPOR` against the
Phase 22 Sprint 22.2 modeled fault-injectable environment, and assert the same load-bearing properties the Sprint 48.3
...
### Deliverables
- A `WorkflowFailoverSimSpec` that binds `Amoebius.Workflow.Runtime`/`Orchestrator`/`Worker` (Sprints 38.2–38.3)
  to the Phase 22 Sprint 22.2 `Amoebius.Sim.Env` substrate through `io-classes` and drives it under `IOSimPOR` — the
...
- The injected fault schedule (`WorkflowSimScenario`): a `kill-worker-mid-workflow` inside the gate's critical
  ...
  **partition** — modeled by the fake Pulsar/MinIO of Phase 22 Sprint 22.2, not a live cluster.
...
> only by this phase's **Register-3 live gate** (Sprint 48.3) on the linux-cpu kind cluster — the deterministic
...
- `documents/engineering/content_addressing_doctrine.md` — record that §2 (the three-tier store + the two write
  ...
  `experimentHash` derivation and seed kernel explicitly deferred to Phase 59; note §5 confluence is consumed
...
- `documents/engineering/resource_capacity_doctrine.md` — record the content-store logical peak boundary
  corpus and its consumption of Phase 41's MinIO physical/uniform-claim witness.
- `documents/engineering/storage_lifecycle_doctrine.md` — record that the store's blob/manifest/pointer bytes
  land on the Phase-39 retained PV under the standing Phase-41 MinIO service, and that CAS-loser orphans stay
...
- `documents/engineering/chaos_failover_doctrine.md` — record the §12 per-run proven/tested/assumed ledger for
  the intra-cluster failover injection, and that the §16 cross-cluster boundary stays deferred to Phase 53
  geo-replication plus Phase 54 gateway-migration drills.
...
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 48's gate substrate (linux-cpu) in the per-phase substrate map.
...
- [Native Pulsar Client Doctrine](../documents/engineering/pulsar_client_doctrine.md) — the Phase-46 capability
...
- [phase_46](phase_46_pulsar_client.md) — the native Pulsar client this workflow runtime is built on
- [phase_59](phase_59_determinism_jitcache.md) — the `experimentHash` derivation + SplitMix seed kernel deferred
