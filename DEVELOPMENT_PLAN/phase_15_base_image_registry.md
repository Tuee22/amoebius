...
SSA reconciler that will eventually own the registry is a Phase 16 concern; `no-provisioner` retained storage
is Phase 17 and MinIO is Phase 19. Phase 15 is an explicit bootstrap cycle-break, not a resource or render
exception. `provisionBootstrapRegistry` binds the complete registry/proxy execution, storage, and node-image
import demand against the Phase-14 topology and returns an opaque `ProvisionedBootstrapRegistry`. A fresh
...
same package-private Phase-9 source serializer, but neither constructs a minimal `ProvisionedServiceSpec` nor
...
`BootstrapRegistryWholeDeploymentHandoffIdentityDigest`. When Phase 16 first constructs the complete
...
(Phase 18), so host-only **read** access is credential-free on the node. Mutation is already exclusive: the
...
- **Bootstrap cycle-break is typed and narrow:** Phase 15 constructs no minimal `ProvisionedServiceSpec` and
...
## Sprint 15.1: Multi-arch base image bake — services + jit-build resolver/toolchain, not engine payloads 📋
...
- The Phase-0-committed oracle `test/fixtures/phase15/bake_inventory_expected.dhall` (the canonical
  ...
  mutants `mutant/phase15/stub-arm64-binary`, `mutant/phase15/wrong-arch-layer`, and
  `mutant/phase15/drop-build-scratch-accounting`.
...
6. The committed mutants `mutant/phase15/stub-arm64-binary`,
   `mutant/phase15/wrong-arch-layer`, and `mutant/phase15/drop-build-scratch-accounting` turn the
   validation red (§M.2).

### Remaining Work
The whole sprint (📋 Planned).

## Sprint 15.2: Node side-load + the single-binary `distribution` registry standup 📋
...
`src/Amoebius/Manifest/Render.hs` supplies the same source serializer Phase 9 uses — target paths, not yet built.
...
registry/proxy object domain through Phase 9's package-private serializer; it is not a minimal
...
- A registry-specific pure/live boundary:
  ...
  bootstrap authority only; it cannot require the Phase-16 scheduler-ready/full managed inventory. The action
...
- `ProvisionedBootstrapRegistryExecution` is a finite cycle-break, not ordinary
  ...
  mandatory members of Phase 16's default→`amoebius-capacity` cutover. They cannot remain as a second
...
- A `ProvisionedBootstrapRegistry` whose `BootstrapRegistryAction` side-loads the selected image and initializes
  ...
  Phase 9. There is no public `render :: ProvisionedServiceSpec -> …` or bootstrap `ProvisionedSpec`; public
  ...
  initialized-field digest. Phase 16 may adopt those exact objects into a later whole `ProvisionedSpec` only
  after equal live readback and a one-time typed ownership handoff; mismatch rejects without a second writer,
  apply, delete, or recreate. The backing is replaced by the MinIO S3 driver in Phase 19.
...
## Sprint 15.3: Atomic multi-arch publication + immutable digest-pinned refs 📋
...
**Blocked by**: Sprint 15.2 (the running registry to push into), Sprint 15.1 (the manifest list to publish).
...
against an ephemeral config directory. Vault does not yet exist (Phase 18), so host-only reads are
...
- The no-env build mechanics: an ephemeral `docker --config <dir>` created per build and scrubbed afterward, the
  ...
  Vault `SecretRef` identity is flagged as the Phase-18+ transport-hardening target.
...
## Sprint 15.4: The no-public-registry-pull gate 📋
...
- `DEVELOPMENT_PLAN/system_components.md` — register `docker/base/Dockerfile`, `src/Amoebius/Image/*`, and the
  `distribution` registry standup as Phase-15 design-first rows, reconciled against the component inventory.
...
- [phase_16](phase_16_renderer_reconciler.md) — the typed reconciler that adopts the equal-digest bootstrap
