# Phase 29: No-provisioner retained storage + lossless rebind
...
- [Sprints](#sprints)
- [Sprint 29.1: The one inert `no-provisioner` StorageClass ✅](#sprint-291-the-one-inert-no-provisioner-storageclass-)
- [Sprint 29.2: Deterministic retained-PV generation + the explicit bind ✅](#sprint-292-deterministic-retained-pv-generation--the-explicit-bind-)
- [Sprint 29.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip ✅](#sprint-293-the-lossless-rebind-gate--postgres-row--minio-marker-round-trip-)
...
⏸️ Blocked pending Phase-28 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
Blocked (superseded) — reopened 2026-08-16 behind the amended Phase-25 handoff and Phase-28 revalidation. Its prior capability record remains historical until the exact predecessor chain is resealed.
...
The gate verified the exact Phase-28 seal and Phase-25 OCI handoff, sealed all three sprints, matched all
...
it opens only after the Phase 27 gate (the typed renderer + live SSA reconciler) closes, because the
...
8 supplies the policy-only `ProvisionedStorageScalingEnvelope` and pure observe-then-plan fold, and Phase 27
...
resize. `CreateProviderCapacity` has no retained-host mutation capability and remains owned by Phase 44.
...
Phase 31 and HA Patroni-via-Percona Postgres in Phase 32, the Vault-enveloping of secrets is Phase 30, and the
Keycloak-owned edge is Phase 33 — none of which this phase requires. The control plane
...
- **Real teardown, not soft delete.** After `cluster delete`, an OS-boundary observer on the host (not the
  ...
  fresh Phase-24 `pb bootstrap` producing a **new** apiserver/etcd (verified by a changed apiserver
...
- **Bytes survive on the host, not in a surviving apiserver.** "Bytes intact between delete and recreate" is
  ...
  destroyed. The PV `Released` *status* is observed only in Sprint 29.2's live PVC-delete step against the
...
- **Honest ledger.** The gate emits its proven/tested/assumed ledger; the aggregate durable-backing fold and
  image-backed host hard cap are live-tested here. The Phase-34 control-plane singleton's no-PVC property
  (which has no realized subject at Phase 29) stays marked **UNVERIFIED**, not asserted as passing.
...
flowchart LR
  ...
  s0["Sprint 29.1: The one inert no-provisioner StorageClass"]
  s1["Sprint 29.2: Deterministic retained-PV generation + the explicit bind"]
  s2["Sprint 29.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip"]
...
```
*Orientation. The seams Phase 29 built and validated in order; [Gate integrity](#gate-integrity) owns the apparatus.*
...
- `test/oracle/retained_storage/storage_class.yaml` — the exact single-StorageClass golden (Sprint 29.1),
...
- `tools/no_retained_delete_check.sh` — the committed static check that no non-harness `src/` module issues a
  ...
  explicitly outside this check because the backing lives outside the cluster (Sprint 29.3 Validation 2a).
...
- **M-soft-delete** (dropped-effect operator) — a `cluster delete` that deletes only the witness
  ...
  absent" OS-boundary assertion (Gate; 29.3 V1).
...
- **M-reclaim-delete** (guard weakening) — a PV rendered with `reclaimPolicy: Delete` instead of `Retain`. Must
  go red on the `Released`/rebind assertion (29.2 V3/V4).
...
- **M-raw-host-directory** (mechanism substitution) — backs a PV with an ordinary retained-root directory
  ...
  `ENOSPC`, changes shared-parent occupancy, or lacks the raw-size/fs-type/usable witness (29.2 V2).
- **M-skip-durable-aggregate** (dropped validation) — allocates/applies an aggregate retained set larger than
  `DurableBacking`. Must go red on the over-backing negative and zero-write assertion (29.2 V1).
- **M-sum-unequal-ordinals** (wrong aggregation) — debits the unequal per-ordinal provisioned map instead of
  ...
  `uniform_claim_skew_over_backing` and the accepted group's byte-identical PVC-size assertion (29.2 V1/V2).
- **M-uniform-before-allocation** (stage-reordering operator) — groups authorable logical/usable demand before
  ...
  fixtures and the per-slot-witness-before-uniformity assertion (29.2 V1/V2).
- **M-collapse-uniform-backing-debits** (wrong aggregation) — collapses the per-backing debit map into a single
  ...
  member backing is one byte short (29.2 V1).
- **M-cutover-before-verify** (guard-weakening / stage-reordering operator) —
  ...
  `migration_verify_mismatch`, where a mismatched copy is cut over anyway (29.2 V1).
- **M-credit-before-cleanup** (dropped-validation operator) — `src/Amoebius/Storage/RetainedScaling.hs`:
  ...
  observed deletion" step (29.2 V1).
- **M-fake-verify** (mechanism-substitution operator) — `src/Amoebius/Storage/RetainedScaling.hs`: the
  ...
  byte-for-byte nonce readback (29.2 V1).
...
- [`cluster_lifecycle_doctrine.md §7`](../documents/engineering/cluster_lifecycle_doctrine.md#7-ephemeral-spin-updown-with-deterministic-rebind)
  ...
  (delivered in Phase 27) — the SSA reconciler that renders and applies the StorageClass and PV objects.
...
## Sprint 29.1: The one inert `no-provisioner` StorageClass ✅
...
- A single rendered StorageClass — `provisioner: kubernetes.io/no-provisioner`, `reclaimPolicy: Retain`,
  `volumeBindingMode: WaitForFirstConsumer` — applied through the Phase-27 reconciler under the `amoebius`
  field manager.
...
### Remaining Work
Generate the sprint receipt under `.build/runs/phase_29/` and retain it only through repository-local attestation.

## Sprint 29.2: Deterministic retained-PV generation + the explicit bind ✅
...
`.build/runs/phase_29/` for repository-local attestation, never an authored root.

## Sprint 29.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip ✅
...
### Deliverables
- Minimal single-ordinal Postgres and MinIO witness StatefulSets on retained PVs (from Sprint 29.2), served
  from baked-binary images; these are rebind witnesses, not distributed MinIO (Phase 31) or HA
  Patroni-via-Percona (Phase 32), and carry no Vault-enveloping (Phase 30).
...
2. Assert the full deletion reclaimed no backing volume (fresh PV objects re-appear post-recreate and
   ...
   host bytes are present. The control-plane singleton is a Phase-34 subject with **no realized instance at Phase 29**, so its "mounts
...
`.build/runs/phase_29/`; externally attest the bundle and rerun after Phase 28 closes.
...
- `documents/engineering/storage_lifecycle_doctrine.md` — the §6 lossless-rebind guarantee gains its first
  ...
  planning-ownership pointer resolves to delivered Phase-29 sprints.
...
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 29's gate substrate (linux-cpu) in the per-phase substrate map.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the
  Phase-27 SSA reconciler that applies the StorageClass and retained-PV objects
...
- [phase_27](phase_27_object_reconciler.md) — the typed renderer + live SSA reconciler this phase builds on
- [phase_30](phase_30_vault_pki.md) — the root Vault whose durable KV rebinds on the retained storage proven here
- [phase_31](phase_31_platform_backbone.md) / [phase_32](phase_32_platform_services_2.md) — the HA
