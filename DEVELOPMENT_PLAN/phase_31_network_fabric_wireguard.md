# Phase 31: WireGuard network fabric

**Status**: Authoritative source
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_32_multicluster_spawn_georepl.md, DEVELOPMENT_PLAN/system_components.md
...
over the VPN with an external-observer probe. It opens after Phase 22 (root Vault/PKI — the KV custody of the
Curve25519 peer keys) and Phase 26 (the control-plane singleton whose reconcile loop renders and enacts the
peer config), and it **precedes** Phase 32 geo-replication and Phase 33 gateway migration (which repoints the
WireGuard hub).
...
will not decode. Phase 31 adds the **runtime residue**: a real `wg0` interface on each node, brought up by the
...
geo-replication wire is *not* delivered here — its per-peer `render()` obligation is deferred to Phase 32; the
two spans this phase renders are the remote-worker↔home attach carrier and the fabric-bound listener boundary.
The gateway-migration hub *repoint* is Phase 33; this phase establishes the static fabric it will
later move.

**Substrate:** linux-cpu — the single-node `kind` host of Phases 17–26, whose kernel provides the WireGuard
...
fabric-render/diff work performed by the already-existing Phase-26 singleton is added to that singleton's
...
- [`network_fabric_doctrine.md §4` — Topology: the hub is the gateway role](../documents/engineering/network_fabric_doctrine.md#4-topology-the-hub-is-the-gateway-role-and-the-fabric-moves-with-it):
  ...
  phase stands up the static hub; Phase 33 later repoints it on migration.
...
## Sprint 31.1: Vault-KV Curve25519 peer keys — secrets by name, minted and custodied in Vault 📋

**Status**: Planned
**Implementation**: `src/Amoebius/Fabric/Keys.hs` (the Curve25519 peer-key KV secret class: mint into Vault-KV,
resolve a peer's key by `SecretRef` name at render time via the Phase-22 Vault client); the Dhall
`WireGuardPeer` schema field carrying the key as a `SecretRef` name only — target paths, not yet built.
**Blocked by**: Phase 22 gate (root Vault + the `SecretRef`-by-name Vault client — the KV store that mints and
...
rendered config (Sprint 31.2) stands on.
...
- The Curve25519 peer-key KV secret class: mint a keypair into Vault-KV, and resolve a peer's private/public key
  by `SecretRef` name at render time through the Phase-22 built-in Vault client (no agent sidecar).
...
2. A `SecretRef` naming an absent/rotated KV entry yields a specific-reason resolution error (never an empty or
   default key), so the missing-key mutant of the gate (Sprint 31.3) has a defined failure to trip on.

### Remaining Work
The whole sprint (📋 Planned).

## Sprint 31.2: Rendered peer config + the wg reconcile — render → wg show/diff/wg set 📋
...
**Blocked by**: Sprint 31.1 (the `SecretRef`-named peer keys the render resolves); Phase 26 gate (the
...
- The pure total `render(nodeInventory) -> [WireGuardPeerConfig]`: keyless peer type-foreclosed; overlapping VPN
  ...
  `SecretRef` name resolved from Vault-KV (Sprint 31.1). The gateway-role hub renders a stable hub VPN-IP +
...
## Sprint 31.3: Phase gate harness — live fabric + external-observer reachability over the VPN IP 📋
...
**Blocked by**: Sprint 31.1, Sprint 31.2; Phase 26 gate (the singleton whose reconcile enacts the fabric);
Phase 22 gate (the Vault-KV custody of the peer keys).
...
`DEVELOPMENT_PLAN/README.md` (flip the Phase-31 status when the gate passes).
...
- A **Register-3** proven/tested/assumed ledger recording the live-reachability result (the spoke reached the
  ...
  **broker↔broker** geo-replication render obligation (Phase 32), the **gateway-migration hub repoint**
  (Phase 33), and the stretched kubelet↔apiserver `ControlPlanePeer` span (owned by cluster_topology). The
...
- `documents/engineering/network_fabric_doctrine.md` — the §2–§5 "Phase 0 design intent" honesty note flips from
  ...
  gateway-migration hub repoint remain deferred (Phase 33).
...
- `DEVELOPMENT_PLAN/README.md` — add the Phase-31 row and flip its status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — add the Phase-31 linux-cpu gate row (the raw-kernel WireGuard fabric).
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Fabric/{Keys,WgRender,WgReconcile}.hs` as
  Phase-31 design-first rows against `network_fabric_doctrine.md`.
...
- [phase_22](phase_22_vault_pki.md) — the root Vault + `SecretRef`-by-name client custodying the peer keys
- [phase_26](phase_26_live_dsl_singleton.md) — the control-plane singleton whose reconcile enacts the fabric
- [phase_32](phase_32_multicluster_spawn_georepl.md) — multi-cluster spawn + geo-replication (the deferred broker↔broker per-peer render obligation)
- [phase_33](phase_33_gateway_migration_drills.md) — the gateway migration that later repoints the WireGuard hub
