# Phase 10: Gateway-migration model (both branches)
...
- [Sprints](#sprints)
- [Sprint 10.1: Author the `GatewayMigration` `Model` — both branches ✅](#sprint-101-author-the-gatewaymigration-model--both-branches-)
- [Sprint 10.2: `emitTLA` render + TLC exhaustive proof (both branches) ✅](#sprint-102-emittla-render--tlc-exhaustive-proof-both-branches-)
- [Sprint 10.3: io-sim agreement + seeded-mutation catch ✅](#sprint-103-io-sim-agreement--seeded-mutation-catch-)
- [Sprint 10.4: Scope-2 pairwise cutoff + decode-time structural-fit fold ✅](#sprint-104-scope-2-pairwise-cutoff--decode-time-structural-fit-fold-)
...
naming Phase 10 in its own run bundle rather than Phase 9.

**Opened 2026-08-17** when Phase 9 resealed; **reopened 2026-08-16 by the natural-architecture amendment.**
...
`sha256:ab0f84be5f1d3562…`; Phase 10 owns no remaining migration deferral.
...
five named invariants — from which the Phase-9 `interpret` (the runtime decision core) and `emitTLA` (the
...
explorer), analogous to the Phase-0 documentation lint and the Phase-9 kernel round-trip.
...
This section pins the apparatus the Phase-10 gate closes over, in the slot
...
authored in Sprint 10.1, is enabled on some reachable state. An invariant whose antecedent is unreachable, or
...
[Phase 9](phase_09_formal_model_kernel.md#phase-summary)** is caught on this concrete model. A **committed
...
[`phase_9`](phase_09_formal_model_kernel.md#phase-summary) §M.1 pins the `ToyModel`
...
## Sprint 10.1: Author the `GatewayMigration` `Model` — both branches ✅

**Status**: Done — the capability is re-established by the migrated gate; the sprint's committed-ledger, pinned-toolchain, and repository-resident evidence mechanics are superseded
**Implementation**: `src/Amoebius/Formal/GatewayMigration.hs` (the concrete `Model`
value + its five named invariants), atop the Phase-9
...
**Independent Validation**: the value typechecks against the Phase-9 `Model` EDSL, and the reachability
...
(Phase-10 status backlink), `documents/engineering/backup_recovery_doctrine.md` (§8 — the `FreshnessWitness`
...
### Deliverables
- The `GatewayMigration` `Model` value in the Phase-9 first-order fragment, both branches expressed as guarded
...
### Validation
1. The value typechecks against the Phase-9 `Model` EDSL; the explorer visits both branches and enumerates a
...
## Sprint 10.2: `emitTLA` render + TLC exhaustive proof (both branches) ✅
...
## Sprint 10.3: io-sim agreement + seeded-mutation catch ✅
...
## Sprint 10.4: Scope-2 pairwise cutoff + decode-time structural-fit fold ✅
...
- A QuickCheck property over random migration graphs asserting **accepts ⟺ pairwise ∧ independent ∧ acyclic ∧ in-parameter-envelope** (equivalence) — the *in-parameter-envelope* conjunct co-equal with the
  ...
  `CONSTANTS` — the four regime bounds being the Sprint-9.1 model `CONSTANTS`, oracle-pinned in the §M.1
...
- `DEVELOPMENT_PLAN/phase_53_multicluster_spawn_georepl.md` — backlink: this design-model is the artifact
