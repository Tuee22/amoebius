...
Patroni-via-Percona Postgres and distributed MinIO chart is Phase 19, the Vault-enveloping of secrets is
Phase 18, and the Keycloak-owned edge is Phase 21 — none of which this phase requires. The control plane
...
- **Real teardown, not soft delete.** After `cluster delete`, an OS-boundary observer on the host (not the
  ...
  fresh Phase-13 `pb bootstrap` producing a **new** apiserver/etcd (verified by a changed apiserver
...
- **Bytes survive on the host, not in a surviving apiserver.** "Bytes intact between delete and recreate" is
  ...
  destroyed. The PV `Released` *status* is observed only in Sprint 16.2's live PVC-delete step against the
...
- **Honest ledger.** The gate still emits its proven/tested/assumed ledger; the host-side hard-cap enforcement
  (§5) and the Phase-20 control-plane singleton's no-PVC property (which has no realized subject at Phase 17)
...
**Blocked by**: Sprint 16.1 (the inert class the retained PVs bind under); Phase 16 gate (the reconciler
...
**Blocked by**: Sprint 16.2 (the retained PVs + deterministic bind the round-trip depends on); Phase 15 gate
...
real `cluster delete` destroys the apiserver, so the PV `Released` *status* is observed only in Sprint 16.2's
live PVC-delete step, not here. `recreate` is a fresh Phase-13 bootstrap (new apiserver/etcd, changed cluster
UID) into which the PV objects are re-rendered and re-applied before rebind is asserted; the recreated cluster
re-binds the *same* PVs by identity (matched against `test/live/fixtures/claimref_table.csv`). The only actor
...
- Minimal single-ordinal Postgres and MinIO witness StatefulSets on retained PVs (from Sprint 16.2), served
  ...
  charts (Phase 19) and carry no Vault-enveloping (Phase 18).
...
1. Run the cycle on the concrete representative set of [§N](#n-gate-integrity-provisions) (exactly two
   ...
   `${RETAINED_ROOT}` still holds the bytes, `cluster recreate` as a fresh Phase-13 bootstrap (new apiserver
...
2. Assert the delete released claims but reclaimed no volume (the retained PV objects re-appear post-recreate
   and `${RETAINED_ROOT}` bytes persist throughout). The "no normal-operation code path deletes a retained PV
   or its bytes" universal negative is discharged two concrete ways: (a) a committed static/CI check asserts no
   non-harness module in `src/` issues a PV or retained-PVC delete (grep-level, committed as
   `test/ci/no_retained_delete.sh`), and (b) post-cycle the PV objects still exist and host bytes are present.
   The control-plane singleton is a Phase-20 subject with **no realized instance at Phase 17**, so its "mounts
...
witness image is a Phase-14 baked binary served only from the in-cluster `distribution` registry (an
...
- `test/live/fixtures/storageclass_expected.yaml` — the exact single-StorageClass golden (Sprint 16.1),
...
- `test/live/fixtures/claimref_table.csv` — the independent reference table mapping
  ...
  helper (Sprints 16.2, 16.3).
- `test/ci/no_retained_delete.sh` — the committed static check that no non-harness `src/` module deletes a
  retained PV or PVC (Sprint 16.3 Validation 2a).
...
- **M-soft-delete** (dropped-effect operator) — a `cluster delete` that deletes only the witness
  ...
  absent" OS-boundary assertion (Gate; 16.3 V1).
...
- **M-reclaim-delete** (guard weakening) — a PV rendered with `reclaimPolicy: Delete` instead of `Retain`. Must
  go red on the `Released`/rebind assertion (16.2 V2/V3).
...
- [phase_19](phase_19_platform_backbone.md) — the HA Postgres/MinIO platform stack that supersedes the witnesses
