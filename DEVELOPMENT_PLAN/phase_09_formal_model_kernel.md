# Phase 9: Formal-model EDSL (`Model`/`interpret`/`emitTLA`)
...
Phase 9 delivers the formal-model EDSL (`Model`/`interpret`/`emitTLA`); its design is owned by [formal_model_doctrine.md](../documents/engineering/formal_model_doctrine.md), [generated_artifacts_doctrine.md](../documents/engineering/generated_artifacts_doctrine.md), [conformance_harness_doctrine.md](../documents/engineering/conformance_harness_doctrine.md), and the plan for reaching it is owned here.
Register 1: an in-process battery, no cluster.
The gate passed on 2026-08-09; Phase-10 code correspondence and runtime fidelity remain UNVERIFIED.
...
- [Sprints](#sprints)
- [Sprint 9.1: The `Model` fragment EDSL (the reifiable value) ✅](#sprint-91-the-model-fragment-edsl-the-reifiable-value-)
- [Sprint 9.2: `interpret` + the in-process reachability explorer ✅](#sprint-92-interpret--the-in-process-reachability-explorer-)
- [Sprint 9.3: `emitTLA` renderer + never-committed emission ✅](#sprint-93-emittla-renderer--never-committed-emission-)
- [Sprint 9.4: Round-trip + single-source correspondence on `ToyModel` ✅](#sprint-94-round-trip--single-source-correspondence-on-toymodel-)
...
than about nothing in particular. It also names Phase 9 rather than Phase 2 in its run bundle, which the
...
`sha256:a0b29d7c344b8990…`; Phase 9 owns no remaining migration deferral.
...
`.build/runs/phase_03/<run-id>/`, and 14 surfaces join to 39 run-time enumerated items. A new `artifact` side
...
proven-for-the-model/tested kernel result on substrate `none`, not correspondence to Phase-10 code and not
...
**not** authored here; that is [Phase 10](phase_10_gateway_migration_model.md). This phase proves the *kernel*
...
The apparatus the phase-9 gate closes over, in the slot
...
mutual-exclusion model of Sprint 9.1 — plus the QuickCheck fragment generator below. No other model is in
scope for Phase 9.
...
golden itself, **not** by any in-process liveness checker: the Sprint 9.1 structural assertion forces
...
fixed in Sprint 9.3**, the **expected `INVARIANT`/`PROPERTY` name set** the emitted `.cfg` must equal exactly
...
exist** (§M.1) — the convention-independent oracles in Phase 0, and the byte-for-byte golden in Sprint 9.3
...
per-constructor coverage percentages; and model-correspondence-to-Phase-10-code and runtime fidelity marked
...
```
*Design intent. Phase 9's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
## Sprint 9.1: The `Model` fragment EDSL (the reifiable value) ✅
...
transition relation.
**Docs to update**: `documents/engineering/formal_model_doctrine.md` (Phase-9 status
...
2. `ToyModel` is not a boolean-only strawman: it **exercises the harder fragment constructors** the round-trip
   ...
   and at least one each of an `Always`, an `Eventually`, and a `LeadsTo` temporal property — so the Sprint 9.4
   round-trip and mutation checks cannot pass while quantifier/function/fairness translation stays stubbed, and
   so the Sprint 9.3 byte golden pins the rendered bytes of **all five** liveness/fairness constructors
...
## Sprint 9.2: `interpret` + the in-process reachability explorer ✅
...
2. The `(event, state) → state` transition table and the `ToyModel` reachable-**distinct**-state count and
   ...
   independent re-derivation of the same count in Sprint 9.4 corroborates but does not replace this Phase-0
...
## Sprint 9.3: `emitTLA` renderer + never-committed emission ✅
...
`documents/engineering/generated_artifacts_doctrine.md` (the `emitTLA` row → built by Phase 9),
...
- **The canonical TLA+ rendering convention**, pinned in Phase 0 before the golden is authored — the analogue
  of the canonical Aeson encoding [`phase_20`](phase_20_render_manifest_goldens.md) pins for `renderAll`, and
...
- Because the Sprint 9.1 structural assertion forces `ToyModel` to carry **all five** liveness/fairness
  constructors, this same byte golden is the **only** oracle that pins liveness/fairness *rendering*
  faithfulness (the Sprint 9.4 differential test is legitimately safety-scoped and cannot see a
...
2. The byte golden pins the rendered bytes of **all five** liveness/fairness constructors carried by `ToyModel`
   ...
   **red** — so a renderer that mistranslates a fairness or temporal constructor cannot pass, even though the
   Sprint 9.4 differential oracle stays legitimately safety-scoped.

### Remaining Work
None. The emitter and CLI are byte-golden locked; generated `.tla`/`.cfg` files remain ignored and untracked.

## Sprint 9.4: Round-trip + single-source correspondence on `ToyModel` ✅
...
+ TLC pairing as a no-live-infrastructure instance). The Phase-9 status flip in `DEVELOPMENT_PLAN/README.md`
...
- **The `tla2tools` pin.** [`README.md`](README.md) and [`phase_01`](phase_01_toolchain_spike.md) both delegate
  this acquisition path to Phase 9, so this sprint records the **exact `tla2tools.jar` release** the harness
...
- The **generated, schema-checked Register-1 ledger** under `.build/runs/` ([§K](development_plan_standards.md#k-honesty-proven--tested--assumed); its schema and external retention are owned by `testing_doctrine.md` and Phase 0), whose Phase-9 rows are:
  ...
  differential-test case count and per-constructor coverage percentages; (d) model-correspondence-to-Phase-10-code
...
machine-derived ledger all pass. Phase-10 code correspondence and runtime fidelity remain UNVERIFIED by design.

## Documentation Requirements

**Engineering docs to update (when the gate runs, flip the honest layer, never before):**
- `documents/engineering/formal_model_doctrine.md` — backlink §2/§3/§4/§5/§6 to the Phase-9 kernel that
...
- `documents/engineering/generated_artifacts_doctrine.md` — mark the `emitTLA` row's renderer as built by
  Phase 9 and note the golden-locked, never-committed emission.
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Formal/{Model,Interpret,Explore,EmitTLA}.hs`
  and `src/Amoebius/Cli/Formal.hs` as Phase-9 design-first rows.
- `DEVELOPMENT_PLAN/phase_10_gateway_migration_model.md` — backlink: the gateway-migration `Model` is authored
...
- [Gateway Migration Model Doctrine](../documents/engineering/gateway_migration_model_doctrine.md) — the one
  concrete `Model` that rides this kernel, authored in Phase 10
...
- [phase_10](phase_10_gateway_migration_model.md) — the gateway-migration model built on this kernel
