# Phase 25: Bounded UI-program schema
...
Phase 25 delivers the UI program schema; its design is owned by [low_code_ui_runtime_doctrine.md](../documents/engineering/low_code_ui_runtime_doctrine.md), [dsl_doctrine.md](../documents/engineering/dsl_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 25.1: Bounded `UiSource` and total structural checker ✅](#sprint-251-bounded-uisource-and-total-structural-checker-)
...
⏸️ Blocked pending Phase-24 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
bound to source snapshot `sha256:7c8debd11c8ba98a…`; Phase 25 owns no remaining migration deferral.
...
and the ledger move into `.build/runs/phase_19/<run-id>/`, and 30 run-time items partition one-to-one across the
...
network observer is normalized to the sanctioned pair as in Phase 21, so a host proving isolation by the other
route still passes.
...
UNVERIFIED. See the Phase-25 ledger.
...
**Depends on:** [Phase 22](phase_22_deterministic_sim_substrate.md) — the pre-cluster design band and deterministic test substrate are available; this
...
## Sprint 25.1: Bounded `UiSource` and total structural checker ✅
...
- Phase 26 — consume only the sealed program and reified public-value universe.
