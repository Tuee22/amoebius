# Phase 30: Typed bake catalog + native-architecture base image + jit-build resolver + distribution registry
...
- [Sprints](#sprints)
- [Sprint 30.1: Native-architecture base image bake — services + jit-build resolver/toolchain, not engine payloads 📋](#sprint-301-native-architecture-base-image-bake--services--jit-build-resolvertoolchain-not-engine-payloads-)
- [Sprint 30.2: Node side-load + the single-binary `distribution` registry standup ⏸️](#sprint-302-node-side-load--the-single-binary-distribution-registry-standup-)
- [Sprint 30.3: Atomic publication + immutable digest-pinned refs ⏸️](#sprint-303-atomic-publication--immutable-digest-pinned-refs-)
- [Sprint 30.4: The no-public-registry-pull gate ⏸️](#sprint-304-the-no-public-registry-pull-gate-)
...
⏸️ Blocked pending Phase-29 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.** The
...
child plus the joined index move to [Phase 67](phase_67_second_arch_attested_index.md). What the amendment
...
**Most complete diagnostic run:** `20260816T180210Z` built and probed both architectures, passed the OCI companion-file oracle and Sprint 30.1 gate, then side-loaded the exact artifact and stood the registry up. The Sprint 30.2 gate stopped before writing its receipt because documentation lint found the Phase Status section one prose line over budget. That standards defect is fixed, but Sprints 25.2–25.4 and the phase remain unsealed.
...
both architectures, imported that export into the Phase-29 node's containerd, stood the `distribution`
...
gives to exactly one phase, and that phase is [Phase 65](phase_65_jitml_lift_cuda.md). The authored rung table
...
path stays credential-free on the node, and the Vault credential that hardens it remains Phase 34's, exactly
...
This phase turns the empty `kind` cluster delivered by Phase 29 into a cluster that pulls only from itself. It
...
[Phase 65](phase_65_jitml_lift_cuda.md) under [§L](development_plan_standards.md#l-one-substrate-discipline);
and **not** the Apple-Metal bridge,
which is a headless on-host Mach-O dylib produced only on the **apple** substrate at
[Phase 68](phase_68_apple_metal_host_daemon.md), never a linux ELF here). The ML **engine payloads**
...
[Phase 67](phase_67_second_arch_attested_index.md)'s, because no host may build the half it cannot execute
...
SSA reconciler that will eventually own the registry is a Phase 31 concern; `no-provisioner` retained storage
is Phase 33 and MinIO is Phase 34. Phase 30 is an explicit bootstrap cycle-break, not a resource or render
exception. `provisionBootstrapRegistry` binds the complete registry/proxy execution, storage, and node-image
import demand against the Phase-29 topology and returns an opaque `ProvisionedBootstrapRegistry`. A fresh
...
same package-private Phase-14 source serializer, but neither constructs a minimal `ProvisionedServiceSpec` nor
...
`BootstrapRegistryWholeDeploymentHandoffIdentityDigest`. When Phase 31 first constructs the complete
...
(Phase 34), so host-only **read** access is credential-free on the node. Mutation is already exclusive: the
...
at `amd64`. An accelerator-bearing host does not expose its accelerator to this gate, so Phase 30 exercises
...
**Lane:** linux-cpu/amd64 — the architecture the live chain from Phase 29 onward is sealed at, and the one
the provider nodes of Phases 49–52 pull. Its complement is [Phase 67](phase_67_second_arch_attested_index.md)'s.
...
the oracles below are authored and **committed in this phase's oracle-pinning sprint** — before the Phase 30
...
- **Bootstrap cycle-break is typed and narrow:** Phase 30 constructs no minimal `ProvisionedServiceSpec` and
...
- **Native execution, not presence and not emulation (§M.5):** each baked service binary and each
  **linux-runnable** toolchain binary (`g++`/pinned compilers; `nvcc` left for the `linux-cuda` lane with the
  ...
  only on the **apple** substrate at [Phase 68](phase_68_apple_metal_host_daemon.md) (never a linux ELF in phases 0..18; a Mach-O dylib can never satisfy a linux `e_machine` check), where its `/usr/bin/clang`
  source-build + `dlopen` probe is validated; Phase 30 owns no such sub-clause and carries **no forward dependency** on Phase 68 ([§L](development_plan_standards.md#l-one-substrate-discipline), one-substrate).
...
- `test/fixture/base_image_registry/bake_inventory_expected.dhall` — the canonical standard-platform-services inventory
  ...
  lane's at [Phase 65](phase_65_jitml_lift_cuda.md), so its absence from the authored rung table is the
  expectation and not a gap in it — and the Apple-Metal bridge, an apple-substrate Mach-O dylib pinned and
  probed at Phase 68, never a linux ELF here.
...
- `test/fixture/base_image_registry/registry_storage_demand.dhall` — the independently authored
  ...
  registry admission/publisher code and is reused unchanged by Phase 35's MinIO rehome.
...
- `test/mutant/base_image_registry/stub-native-binary` — a zero-byte binary at a baked path; red at Sprint 30.1 (execution +
  ELF check). Its pre-amendment name was `stub-arm64-binary`, which named an architecture this gate no longer
  builds; the arm64 instance belongs to [Phase 67](phase_67_second_arch_attested_index.md).
- `test/mutant/base_image_registry/wrong-arch-layer` — foreign-architecture bytes placed in this architecture's layer; red at Sprint 30.1 (`e_machine`
  check).
- `test/mutant/base_image_registry/gxx-version-skew` — the baked `g++` shimmed (path: `/usr/bin/g++`; operator:
  ...
  `bake_inventory_expected.dhall` entry; red at Sprint 30.1's native `<bin> --version` match, paired with the
...
- [`image_build_doctrine.md` §7 — what amoebius bakes vs builds: the base container is the supply chain](../documents/engineering/image_build_doctrine.md#7-what-amoebius-bakes-vs-builds--the-base-container-is-the-supply-chain):
  ...
  image is rebuilt and republished at [Phase 54](phase_54_infernix_lift.md) /
  [Phase 65](phase_65_jitml_lift_cuda.md), never here), so Phase 30 carries no forward dependency on the
  extension lifts. The amoebius binary's own UI-server surface travels with it: the compiled generic
  PureScript client bundle from [Phase 25](phase_25_ui_browser_interpreter.md) is a **baked asset of this
  image**, not a second image — the UI server is a worker responsibility of the same executable, and a UI
  release is release *data* ([Phase 45](phase_45_ui_program_release.md)), never an image build. That is
...
- [`image_build_doctrine.md` §3 — multi-architecture images: one natively built child per architecture](../documents/engineering/image_build_doctrine.md#3-multi-architecture-images--one-natively-built-child-per-architecture):
  ...
  index that joins it to its complement is [Phase 67](phase_67_second_arch_attested_index.md)'s.
...
flowchart LR
  ...
  s0["Sprint 30.1: Multi-arch base image bake — services + jit-build resolver…"]
  s1["Sprint 30.2: Node side-load + the single-binary distribution registry standup"]
  s2["Sprint 30.3: Atomic publication + immutable digest-pinned refs"]
  s3["Sprint 30.4: The no-public-registry-pull gate"]
  gate["the phase 30 gate"]
...
```
*Orientation. The seams Phase 30 builds, in order; [Gate integrity](#gate-integrity) owns the apparatus. Sprint 30.1 is sealed; the later sprints remain open at the requested pause.*

## Sprint 30.1: Native-architecture base image bake — services + jit-build resolver/toolchain, not engine payloads 📋
...
and probed at [Phase 68](phase_68_apple_metal_host_daemon.md), never a linux ELF in phases 0..18) and
...
- A base `Dockerfile` baking this platform's binaries by the supply-chain preference order, plus the
  ...
  [Phase 54](phase_54_infernix_lift.md) / [Phase 65](phase_65_jitml_lift_cuda.md), never here, so Phase 30
...
## Sprint 30.2: Node side-load + the single-binary `distribution` registry standup ⏸️

**Status**: Blocked by Sprint 30.1's amended bake, which no run has produced. The pre-amendment artifact was side-loaded and its registry stood up in diagnostic
...
`src/Amoebius/Manifest/Render.hs` supplies the same source serializer Phase 14 uses; live admission/import/
standup and OS-boundary observation are implemented by `tools/base_image_registry_{bootstrap_preflight,standup}.py`.
**Blocked by**: none; Sprint 30.1 and the amended Phase-29 gate are sealed.
...
reconciler (Phase 31), retained storage (Phase 33), and MinIO (Phase 35) do not yet exist, the registry comes up
...
registry/proxy object domain through Phase 14's package-private serializer; it is not a minimal
...
- A registry-specific pure/live boundary:
  ...
  bootstrap authority only; it cannot require the Phase-31 scheduler-ready/full managed inventory. The action
...
- `ProvisionedBootstrapRegistryExecution` is a finite cycle-break, not ordinary
  ...
  mandatory members of Phase 32's default→`amoebius-capacity` cutover. They cannot remain as a second
...
- A `ProvisionedBootstrapRegistry` whose `BootstrapRegistryAction` side-loads the selected image and initializes
  ...
  Phase 14. There is no public `render :: ProvisionedServiceSpec -> …` or bootstrap `ProvisionedSpec`; public
  ...
  initialized-field digest. Phase 31 may adopt those exact objects into a later whole `ProvisionedSpec` only
  after equal live readback and a one-time typed ownership handoff; mismatch rejects without a second writer,
  apply, delete, or recreate. The backing is replaced by the MinIO S3 driver in Phase 34.
...
7. A fake later whole-deployment adoption with the equal handoff digest succeeds once without object
   ...
   object writes. Phase 35 preserves this private demand's `objectSet`, `derivedPeak`, and upload/orphan
...
9. Assert no public-registry pull occurred during registry standup **from the OS-boundary observer** (§M.5 —
   ...
   registries), recorded for the Sprint 30.4 gate — never from a self-emitted compliance trace.

### Remaining Work
Repeat the side-load and registry standup in the uninterrupted phase run, then pass the Sprint 30.2 gate and
retain its receipt. Atomic publication remains exclusively Sprint 30.3 work. The MinIO-backed backend is
Phase 35's, and Phase 31 owns adoption of these objects through the one-time typed handoff.

## Sprint 30.3: Atomic publication + immutable digest-pinned refs ⏸️

**Status**: Blocked by Sprint 30.2's amended standup; the prior capability record is historical. Previously the fault-injected
...
**Blocked by**: none; Sprint 30.2 is sealed.
...
against an ephemeral config directory. Vault does not yet exist (Phase 34), so host-only reads are
...
- The no-env build mechanics: an ephemeral `docker --config <dir>` created per build and scrubbed afterward, the
  ...
  Vault `SecretRef` identity is flagged as the Phase-34+ transport-hardening target.
...
### Remaining Work
Revalidate atomic publication of the amended handoff. Sprint 30.4 consumes this run's immutable digest reference and the standup/publication OS-boundary
captures, and installs the enforcing deny boundary the paired canaries prove.

## Sprint 30.4: The no-public-registry-pull gate ⏸️

**Status**: Blocked by Sprint 30.3's amended publication; the prior capability record is historical. Previously under the enforced
...
**Blocked by**: none; Sprint 30.3 is sealed.
...
later reconciler-owned rendering (Phase 31) and the MinIO-backed blob store (Phase 35) marked UNVERIFIED here.
...
externally attested against the run's source snapshot, and the gate ran after Phase 29 closed. The later
reconciler-owned rendering correspondence and MinIO-backed registry storage correspondence remain
`UNVERIFIED` and are owned by Phases 31 and 35 respectively.
...
- `documents/engineering/content_addressing_doctrine.md` — annotate §4.5 that the base image contributes the
  ...
  resolved on first miss (the resolver's own live proof lands in Phase 53).
...
- `documents/engineering/platform_services_doctrine.md` — the §3 registry-as-single-image-source note flips from
  design intent to a delivered `distribution` standup, with the MinIO-backed S3 driver still a Phase-35 target.
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-30 row from Planned to its delivered status and link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 30's gate substrate (linux-cpu) in the per-phase substrate map.
- `DEVELOPMENT_PLAN/system_components.md` — register `docker/base/Dockerfile`, `src/Amoebius/Image/*`, and the
  `distribution` registry standup as Phase-30 design-first rows, reconciled against the component inventory.
...
- [phase_29](phase_29_bootstrap_coordinator_kind.md) — the `pb` bootstrap coordinator + empty `kind` cluster this phase publishes into
- [phase_31](phase_31_object_reconciler.md) — the typed reconciler that adopts the equal-digest bootstrap
  objects into the first whole-deployment `ProvisionedSpec` through the one-time ownership handoff
- [phase_35](phase_35_platform_backbone.md) — the standard service stack whose MinIO backs the registry's S3-driver blob store
- [phase_53](phase_53_determinism_jitcache.md) — the live jit-build engine resolver + `CacheBudget` cache the toolchain here feeds
