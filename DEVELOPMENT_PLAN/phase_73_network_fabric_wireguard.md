...
Blocked by redesigned Phase 72, its independent validation, and gate pass; every earlier
...
Its three obligations map onto machinery supplied only by gate-passed predecessors:
...
gate-passed; that host's kernel must provide the WireGuard
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and complete gate execution missing; prior prose: Exact `ImmediatePredecessorPass` for Phase 72; candidate execution refuses an absent, stale, replayed, or different-source result. |
...
**Blocked by**: [Phase 72](phase_72_ui_program_release.md) gate pass
...
- **Haskell-authored changed-subject mutation operators (§M.2):** `missing-peer-key` (the spoke's
  ...
  `hub-no-endpoint` (the hub-role config omits its `Endpoint` and the probe fails red). GateReady Haskell
...
- The separately authored Haskell oracle bundle declares the topology, peer config, demand, reachability,
...
- A **Register-3** proven/tested/assumed ledger recording the live-reachability result (the spoke reached the
  ...
  **broker↔broker** geo-replication render obligation (Phase 74), the **gateway-migration hub repoint**
  ...
  corresponding predecessor model gates are gate-passed in the
  pre-cluster band.
