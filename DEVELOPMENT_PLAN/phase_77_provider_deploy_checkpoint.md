# Phase 77: Haskell-derived provider Pulumi program and enveloped checkpoint
...
- [Sprints](#sprints)
- [Sprint 77.1: Provider-cluster Pulumi deploy from inside a parent ⏸️](#sprint-771-provider-cluster-pulumi-deploy-from-inside-a-parent-)
...
This is the **first arm** of the provider-managed-cluster split (Phases 77–80): the target mechanism by which
...
linux-cpu parent, plus its gate; it is the substrate on which [phase_78_provider_child_bringup.md](phase_78_provider_child_bringup.md)
(the stateless hostless in-cluster control plane), [phase_79_provider_ebs_credential.md](phase_79_provider_ebs_credential.md)
(per-PV EBS + the create-vs-delete credential model), and [phase_80_provider_dynamic_nodes.md](phase_80_provider_dynamic_nodes.md)
...
an already-running amoebius cluster, issued by the Deployment-`replicas=1` control-plane daemon (Phase 66) —
whose single-instance is a k8s/etcd property, never a bespoke amoebius election — generalizing Phase 75's
...
or standard-service convergence for a provider child ([phase_78_provider_child_bringup.md](phase_78_provider_child_bringup.md));
per-PV durable EBS, the `protect`/`Retain` state class, the create-vs-delete credential split, or the static
`ebs.csi.aws.com` CSI path ([phase_79_provider_ebs_credential.md](phase_79_provider_ebs_credential.md)); or
dynamic node provisioning by signal, the second reconcile no-op, and the independent leak-free teardown sweep
([phase_80_provider_dynamic_nodes.md](phase_80_provider_dynamic_nodes.md)). The base node group here is a fixed
size-1 group provisioned and observed by this sub-phase alone; the `Managed Eks` arm carries **no** `LinuxHost`
witness. Human-approved Phase 26 schema generation, Phase 27 GADT decoding, and Phase 9 capacity/topology folds
...
**Depends on:** [Phase 76](phase_76_gateway_migration_drills.md)
...
| Key | Contract |
|---|---|
| `Claim` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: one cohesive claim — *the provider control plane is raised only from inside the parent, and its checkpoint is never readable outside Vault's envelope*. No host-side `pulumi up` exists. Explicit exclusions: every layer named in `Residue` remains UNVERIFIED. |
...
- [`cluster_lifecycle_doctrine.md` §3 — Amoebic spawning — the recursive forest](../documents/engineering/cluster_lifecycle_doctrine.md#3-amoebic-spawning--the-recursive-forest)
  — *amoebic spawning — the recursive forest*: provider-cluster spawn is the *cloud-keyed* sibling of Phase 75's
...
## Sprint 77.1: Provider-cluster Pulumi deploy from inside a parent ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 76](phase_76_gateway_migration_drills.md) human approval
...
object set, generalizing Phase 75's SSH-keyed self-managed spawn to a cloud-keyed provider spawn. The `pulumi`
...
- An `Amoebius.Pulumi.Provider.Eks` program that provisions the EKS control plane + a base managed node group
  ...
  the cluster ready for its in-cluster control-plane bootstrap ([phase_78_provider_child_bringup.md](phase_78_provider_child_bringup.md)).
...
- `planInfrastructure` derives the provider-cluster demand from the exact `BoundDeployment` and declared
  ...
  [phase_79_provider_ebs_credential.md](phase_79_provider_ebs_credential.md).) Only the receipt-bound provider
...
> `mut-47.2-skip-sweep`) is [phase_80_provider_dynamic_nodes.md](phase_80_provider_dynamic_nodes.md)'s gate,
...
- `documents/engineering/cluster_lifecycle_doctrine.md` — record that §3 (the cloud-keyed amoebic spawn) gains
  an amoebius EKS reference as the cloud-keyed sibling of the Phase 75 SSH-keyed spawn.
...
- `documents/engineering/testing_doctrine.md` — record the Phase 77 per-run ledger artifact for the
...
- `DEVELOPMENT_PLAN/system_components.md` — map the reused `amoebius-pulumi` Engine and EncryptedMinio backend
  to their first delivery in Phase 75; register `Amoebius.Pulumi.Provider.Eks`, the `observeProviderAccount`
  boundary, and the `PulumiExecutionDemand` / `PulumiCheckpointObjectDemand` types as Phase-77 design-first rows,
...
- [README.md](README.md) — the live tracker; the Phase 77 objective, gate, and substrate
...
- [phase_75](phase_75_multicluster_spawn_georepl.md) — the SSH-keyed amoebic spawn, encrypted MinIO backend, and
  per-child envelope this phase generalizes to a cloud-keyed provider spawn
- [phase_78](phase_78_provider_child_bringup.md) — the stateless hostless in-cluster control-plane daemon + capacity-scheduler
  bring-up that layers on this deploy
- [phase_79](phase_79_provider_ebs_credential.md) — the per-PV EBS + create-vs-delete credential model + static
  CSI path that layers on this deploy
- [phase_80](phase_80_provider_dynamic_nodes.md) — dynamic node provisioning by signal and the independent
