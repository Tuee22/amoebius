# Phase 44: Gateway-migration drills + model-correspondence
...
> (RPO=0) and a `Failover` emergency takeover (bounded rebind) against the live [Phase 43](phase_43_multicluster_spawn_georepl.md) forest, trace-validate it against the Phase-3 emitted spec, and
...
- [Sprints](#sprints)
- [Sprint 44.1: The gateway-migration runtime — both branches over the Phase-3 `interpret` core ⏸️](#sprint-441-the-gateway-migration-runtime--both-branches-over-the-phase-3-interpret-core-)
- [Sprint 44.2: Teardown-with-cleanup vs chaos-failover + unsatisfiable-`.dhall` push-back ⏸️](#sprint-442-teardown-with-cleanup-vs-chaos-failover--unsatisfiable-dhall-push-back-)
- [Sprint 44.3: Register-2.5 gateway-migration runtime fidelity — simulation + trace validation ⏸️](#sprint-443-register-25-gateway-migration-runtime-fidelity--simulation--trace-validation-)
- [Sprint 44.4: Register-3 correspondence — Inject drills against the running forest + live gate `.dhall` + ledger ⏸️](#sprint-444-register-3-correspondence--inject-drills-against-the-running-forest--live-gate-dhall--ledger-)
...
Done (invalidated). The runtime delegates every modeled edge to the Phase-3 `interpret` core, validates both pinned traces
...
Because [Phase 43](phase_43_multicluster_spawn_georepl.md) turned the single cluster into a geo-replicated
...
This phase consumes earlier phases and does not re-implement them: Phase 43's geo-replicated forest and
invariant-confluence classifier, Phase 3's `GatewayMigration` `Model` + `interpret` + decode-time
structural-fit fold, Phase 33's Keycloak-owned wild ingress, Phase 42's WireGuard fabric (whose hub role the
...
**Substrate:** linux-cpu — the gate drives the migration over the parent and both child clusters that Phase 43
spins up as `kind` clusters on a single linux-cpu host; no accelerator and no provider cluster is in
scope (provider-managed clusters are [Phase 45](phase_45_provider_deploy_checkpoint.md)). Partition tolerance is
...
- **The five named invariants, and correspondence.** The Register-3 Inject drills assert all five Phase-3 safety
  ...
  `src/Amoebius/Multicluster/*` runtime is the Phase-3 `GatewayMigration` model's `interpret` is discharged as
  the step-by-step trace-validation of Sprint 44.3 (Register 2.5); Register 3 adds only the
...
- **External-observer teardown check.** Phase 44 claims only its pre-Phase-55 teardown scope; the later
  ...
  `no-provisioner` PVs that Sprint 44.2 deliberately preserves are explicitly exempt (named in the fixture as the
  retained set).
- **Machine-derived ledger + validator.** The ledger is generated from the run record (measured RPO/RTO,
  ...
  *proven-for-the-model at scope 2* — a Phase-3 design result, never a runtime guarantee. Layers outside
...
flowchart LR
  ...
  g{{"the phase 44 gate command"}}:::gate
...
```
*Design intent. Phase 44's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
The runtime executes inside the Phase-34 singleton, whose complete pod envelope is expanded for migration-plan
...
clusters' Phase-33 edge controllers, operator-derived data-plane children, admission webhooks/gateways, and
rollout operands; the source transparent proxy and target serving path coexist through the full DNS drain
window. The Phase-42 `NetworkFabricSystemDemand` is derived for the source, overlap, and target peer/hub
graphs, including kernel/listener CPU and memory, queues, and rotated nodefs logs. The route53 repoint retains
the Phase-43 in-cluster `PulumiExecutionDemand`, exact executor-Job envelopes, plugin/workspace volumes,
...
The non-idle drill workload is provisioned too. Its source-equal Phase-36/38 client/workflow projection carries
...
## Sprint 44.1: The gateway-migration runtime — both branches over the Phase-3 `interpret` core ⏸️
...
## Sprint 44.2: Teardown-with-cleanup vs chaos-failover + unsatisfiable-`.dhall` push-back ⏸️
...
## Sprint 44.3: Register-2.5 gateway-migration runtime fidelity — simulation + trace validation ⏸️
...
Phase-3 emitted spec, pulling the runtime-fidelity (Tier-2) obligation **forward** from Register-3-only chaos
into deterministic, replayable simulation. Substrate `none`, Register 2.5.
**Docs to update**:
`documents/engineering/deterministic_simulation_doctrine.md` (Phase-44 status backlink),
...
simulation against the modeled world here (Register 2.5), then as live Inject drills (Sprint 44.4, Register 3)
...
- A Register-2.5 proven/tested/assumed ledger — the built forest upholds the safety invariants and refines the
  ...
  replication-lag / clock-skew physics remain the Register-3 residue (Sprint 44.4).
...
## Sprint 44.4: Register-3 correspondence — Inject drills against the running forest + live gate `.dhall` + ledger ⏸️
...
- The Register-3 Inject drill set run in the inter-cluster dimension against the running forest — cut
  ...
  past the bound, and fail back with late plus duplicate arrivals — each asserting the Phase-3 named invariants.
...
- `test/dhall/phase_44_gateway_migration.dhall`: spin two children up, geo-replicate, run a `Planned` handover
  ...
  **committed at the start of Phase 44, before that implementation is trusted** (the §M.1
...
1. The built decision core resolves to the Phase-3 `interpret` — the correspondence-check mechanic is fixed as:
   the step-by-step trace-validation of Sprint 44.3 (Register 2.5) **plus** a Register-3 **modeled-action-coverage assertion** that every modeled action fired ≥ 1 time across the drill set (no orphaned modeled action); step-by-step
   ...
   safety half; the recovery-time half is the separate *tested* RTO datapoint). `phase_44_gateway_migration.dhall`
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `src/Amoebius/Multicluster/GatewayMigration.hs`,
  ...
  modules and the Register-2.5 simulation + Register-3 Inject + gate suites as Phase-44 rows.
- `DEVELOPMENT_PLAN/substrates.md` — the Phase-44 → linux-cpu row in the per-phase substrate map.

## Related Documents
- [README.md](README.md) — the live tracker; Phase 44 objective, gate, and substrate
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the Register-2.5 io-sim + trace-validation (Sprint 44.3) that pulls the runtime-fidelity obligation forward from Register-3-only chaos
...
- [phase_43](phase_43_multicluster_spawn_georepl.md) — the prior phase; the geo-replicated forest and confluence classifier this phase runs over
- [phase_45](phase_45_provider_deploy_checkpoint.md) — the next phase; the forest extended to provider-managed clusters
