...
✅ Done — sealed 2026-08-14, attestation `sha256:0ba91376b2c536930ba52ade70c9581798a9dfb861d468a28f5bb0d13fc353e0`. Reopened 2026-08-11 because the prior seal
did not include the universal artifact-hygiene postcondition; `python3 tools/phase26_gate.py --execute` now
...
batteries, and two live observations out of `DEVELOPMENT_PLAN/evidence/phase_26` — a plan-tree directory that
...
a run bundle: `test/phase_26_surface_expectations.tsv` authors 31 surfaces for the first time, the ledger is
...
*The corpus was reconciling a digest that no longer existed.* `tools/phase26_reconcile_live.py` and
`tools/phase26_sprint26_3_live.py` each pinned the Phase-25 base image by a literal digest from a build that is
...
over a Pod/controller or mutable ledger/Lease field; Phase 27 owns the witnesses that unlock the guarded
stages.
...
completion variants, digest/outcome equality, cleanup deadlines, and `CompletedJobNoOp`; the live Phase-26
cluster has neither MinIO nor the Phase-37 sole content-mutation gateway, so it does **not** claim a durable
...
Pod without typed authorization — must fail Sprint 26.3; and (e) a **healthy-CR/over-bound-child** mutant whose
amoebius controller reports its **own in-phase capacity/reservation CR** healthy while its actual child exceeds the
provisioned resource/PVC/rollout envelope — must prevent convergence after child enumeration in Sprint 26.4. (The
...
- [`deterministic_simulation_doctrine.md §4`](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  — **Register 2.5, where deterministic simulation sits.** Sprint 26.5 validates the *built* reconciler and staged
...
> pass claims and `Remaining Work: None` statements below describe the 2026-08-14 seal. Any repository-resident
> evidence path retained in the prose describes the retired record only: this phase's evidence is written into
> its run bundle under `gen/runs/`. Functional and validation outcomes remain target requirements. Any instruction to commit generated output, freeze dependency resolution,
...
**Status**: Done — sealed 2026-08-14 inside `python3 tools/phase26_gate.py --execute`; the corpus, its
independently authored action table, and the read-only preflight boundary all held, and
`sprint-26.1-receipt.json` in that run's bundle records it.
**Implementation**:
...
- `renderAll → validateAndIndexRenderedObjects → observeInventoryAndObjects → authenticateExecutions →
  ...
  grants generic SSA over the full list; the scheduler-cutover stage-unlock witnesses are Phase 27). The observed
...
- A host-aware observed identity union: `KubernetesPod PodUid | HostProcess HostProcessInstanceId |
  ...
  reservation ledger, distinct from the Phase-27 k8s scheduler reservation ledger.
...
- Execution actions cover `NoOp`, kind-indexed Pod-controller apply, the three serial OnDelete stages, host
  ...
  gateway capability, which is absent from the Phase-26 live environment. Every mutating constructor carries only
...
- Preserve the operator child-admission and migration envelopes: admission/quota policy must be Ready before a CR
  ...
  retain old+new+workspace/temp/WAL until verified cutover. Phase 26 owns only the generic storage-scaling
  observation/validation/token boundary: Phase 28 supplies the retained-carve and verified-migration enactors, and
  Phase 44 supplies the cloud provider-capacity enactor. Durable backing is never generic prune.
- A mechanical no-release-store/no-write preflight boundary: AST/import lint over the modules above, plus a runtime
  ...
  Phase 37 supplies the live gateway.
...
None. The receipt and its three-check transcript are written into this run's bundle under `gen/runs/`, never
into the plan tree. Sprint 26.2 consumes the typed action/authority boundary.

## Sprint 26.2: Bootstrap Lease authority + generic typed-action dispatcher + scoped SSA + storage-scaling dispatch ✅

**Status**: Done — sealed 2026-08-14 inside `python3 tools/phase26_gate.py --execute`; the Lease CAS, scoped
managed fields, stable no-op, and clean postflight were observed live, and `sprint-26.2-receipt.json` in
that run's bundle records it.
...
precursor must hold the same provisioned mandatory `Lease` that the Phase-33 singleton later receives by an
...
- A dispatcher for every non-scheduler `ValidatedExecutionTransitionAction`. `ApplyDesiredPodController` preserves
  ...
  and prior-controller delete are sent to their own enactors (built in Sprint 26.3); Job completion/cleanup dispatch
  additionally requires the gateway capability and therefore cannot run in the live Phase-26 gate. `NoOp` and
...
- A storage-scaling dispatcher that accepts only a `ValidatedStorageScalingAction`, immediately rechecks its exact
  ...
  migration are abstract effect arms supplied in Phase 28, while provider-capacity creation also consumes the
...
3. Storage-scaling action tests run at the pure/fake boundary in this phase: stale allocation, backing,
   ...
   a lost response forces a fresh observation. Live retained and cloud mutations remain the Phase-28 and Phase-44
   gates respectively.
...
**Status**: Done — sealed 2026-08-14 inside `python3 tools/phase26_gate.py --execute`; both ordered
replacements, the retained terminal Pod, and the authenticated delete were observed live, and
`sprint-26.3-receipt.json` in that run's bundle records it.
**Implementation**:
...
validated in pure, fake, and IOSim executions here; the live Phase-26 environment intentionally supplies no such
capability.
...
- Job terminal protocol for both `Succeeded` and `FailedBackoffExhausted`: the closed model writes the exact
  ...
  `CompletedJobNoOp`; a failed write retains the terminal Pod/ledger axes and retries safely. In the Phase-26 live
  cluster the gateway capability is absent, so the only inhabitant is `RetainTerminalAwaitingCompletionGateway`; the
  Pod, API bytes, logs/metadata, and retained partition remain charged. Phase 37 performs the first live
...
2. Register-2/2.5 transition tests cover ordinary host exit, CUDA device holds/free VRAM, and Metal drain/allocation/
   ...
   substrate proof remains owned by Phase 51 and Phase 53.
3. Register-1/2 Job tests, available at this sprint's completion, cover all-success and backoff-exhausted failure
   ...
   these same variants are additionally exercised in Sprint 26.5. In the live Phase-26 run, an independent apiserver
   observer proves the terminal UID remains, no Job-completion object is written, and no delete occurs. The first live
   object-store/apiserver persist-before-delete proof is the Phase-37 gate.
...
**Status**: Done — sealed 2026-08-14 inside `python3 tools/phase26_gate.py --execute`; the corpus converged
and re-ran byte-stable with zero mutations, with all four red controls red, and `sprint-26.4-receipt.json`
in that run's bundle records it.
...
- The convergence battery over the pinned corpus: enact the exact `renderAll`/action plan → observe workload
  ...
  bind-before-CAS mutants are Phase 27's gate.)
...
None. The receipt and the live/mutation results are written into this run's bundle under `gen/runs/`. The
...
**Status**: Done — sealed 2026-08-14 inside `python3 tools/phase26_gate.py --execute`; 2,048 deterministic
...
- Committed mutants for lost `Lease`/resourceVersion retry, mutation without holder, sleep-gated readiness, serial
  ...
  collapsed scheduler readiness stages, premature managed taint/full RBAC — are Phase 27's `SchedulerSim`.)
...
`gen/runs/`; modeled-apiserver fidelity remains assumed, as the register boundary requires, and the same run's
...
- `documents/engineering/manifest_generation_doctrine.md` — §5's typed SSA/staged-action/delete/wait engine flips
  ...
  design intent until Phase 27); §6's planned-vs-observed and `renderAll` model gains its first validation. Keep
...
- `DEVELOPMENT_PLAN/system_components.md` — register the Phase-26
...
- [Daemon Topology Doctrine](../documents/engineering/daemon_topology_doctrine.md) — [§3](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-singleton) the Deployment-`replicas=1`
  singleton (delegated single-instance, no election) that will own this reconciler in Phase 33
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — [§4](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits) the
  Register-2.5 io-sim environment the reconciler is validated against in Sprint 26.5, before the Register-3 live gate
...
- [phase_27_capacity_scheduler.md](phase_27_capacity_scheduler.md) — the `amoebius-capacity` scheduler, two-stage
  bootstrap cutover, and execution-identity admission layered on this reconciler
- [phase_28_retained_storage.md](phase_28_retained_storage.md) — the retained-carve and verified-migration
  storage-scaling enactors this phase's dispatch boundary calls into
- [phase_33_live_dsl_singleton.md](phase_33_live_dsl_singleton.md) — the Deployment-`replicas=1` singleton that stands
  the reconciler up in-cluster and receives the `Lease` by observed handoff
- [phase_37_content_store_workflow.md](phase_37_content_store_workflow.md) — the first live gateway write → readback →
