# Phase 33: No-provisioner retained storage + lossless rebind
...
- [Sprints](#sprints)
- [Sprint 33.1: The one inert `no-provisioner` StorageClass ✅](#sprint-331-the-one-inert-no-provisioner-storageclass-)
- [Sprint 33.2: Deterministic retained-PV generation + the explicit bind ✅](#sprint-332-deterministic-retained-pv-generation--the-explicit-bind-)
- [Sprint 33.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip ✅](#sprint-333-the-lossless-rebind-gate--postgres-row--minio-marker-round-trip-)
...
⏸️ Blocked pending Phase-32 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
Blocked (superseded) — reopened 2026-08-16 behind the amended Phase-30 handoff and Phase-32 revalidation. Its prior capability record remains historical until the exact predecessor chain is resealed.
...
The gate verified the exact Phase-32 seal and Phase-30 OCI handoff, sealed all three sprints, matched all
...
**Register 3** (live infrastructure) — a single-node `kind` cluster brought up by the Phase 29 bootstrap coordinator, and
it opens only after the Phase 31 gate (the typed renderer + live SSA reconciler) closes, because the
...
8 supplies the policy-only `ProvisionedStorageScalingEnvelope` and pure observe-then-plan fold, and Phase 31
...
resize. `CreateProviderCapacity` has no retained-host mutation capability and remains owned by Phase 48.
...
Phase 35 and HA Patroni-via-Percona Postgres in Phase 36, the Vault-enveloping of secrets is Phase 34, and the
Keycloak-owned edge is Phase 37 — none of which this phase requires. The control plane
...
The live gate uses the Phase-29 single-node `kind` cluster; pure StorageClass/PV rendering remains Register 1–2.
...
- **Real teardown, not soft delete.** After `cluster delete`, an OS-boundary observer on the host (not the
  ...
  fresh Phase-29 `pb bootstrap` producing a **new** apiserver/etcd (verified by a changed apiserver
...
- **Bytes survive on the host, not in a surviving apiserver.** "Bytes intact between delete and recreate" is
  ...
  destroyed. The PV `Released` *status* is observed only in Sprint 33.2's live PVC-delete step against the
...
flowchart LR
  ...
  s0["Sprint 33.1: The one inert no-provisioner StorageClass"]
  s1["Sprint 33.2: Deterministic retained-PV generation + the explicit bind"]
  s2["Sprint 33.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip"]
...
```
*Orientation. The seams Phase 33 built and validated in order; [Gate integrity](#gate-integrity) owns the apparatus.*
...
witness executes the exact Phase-30 baked binary from the run-audited Phase-30 OCI archive, restored
locally into each fresh node under the Phase-30 digest with `imagePullPolicy: Never`; an OS-boundary observer
confirms zero public-registry pull during the cycle. The harness reads the PostgreSQL executable and major
version from Phase 30's independently authored bake-inventory expectation, then uses that exact baked
...
- `test/oracle/retained_storage/storage_class.yaml` — the exact single-StorageClass golden (Sprint 33.1),
...
- `tools/no_retained_delete_check.sh` — the committed static check that no non-harness `src/` module issues a
  ...
  explicitly outside this check because the backing lives outside the cluster (Sprint 33.3 Validation 2a).
...
- **M-soft-delete** (dropped-effect operator) — a `cluster delete` that deletes only the witness
  ...
  absent" OS-boundary assertion (Gate; 33.3 V1).
...
- **M-reclaim-delete** (guard weakening) — a PV rendered with `reclaimPolicy: Delete` instead of `Retain`. Must
  go red on the `Released`/rebind assertion (33.2 V3/V4).
...
- **M-raw-host-directory** (mechanism substitution) — backs a PV with an ordinary retained-root directory
  ...
  `ENOSPC`, changes shared-parent occupancy, or lacks the raw-size/fs-type/usable witness (33.2 V2).
- **M-skip-durable-aggregate** (dropped validation) — allocates/applies an aggregate retained set larger than
  `DurableBacking`. Must go red on the over-backing negative and zero-write assertion (33.2 V1).
- **M-sum-unequal-ordinals** (wrong aggregation) — debits the unequal per-ordinal provisioned map instead of
  ...
  `uniform_claim_skew_over_backing` and the accepted group's byte-identical PVC-size assertion (33.2 V1/V2).
- **M-uniform-before-allocation** (stage-reordering operator) — groups authorable logical/usable demand before
  ...
  fixtures and the per-slot-witness-before-uniformity assertion (33.2 V1/V2).
- **M-collapse-uniform-backing-debits** (wrong aggregation) — collapses the per-backing debit map into a single
  ...
  member backing is one byte short (33.2 V1).
- **M-cutover-before-verify** (guard-weakening / stage-reordering operator) —
  ...
  `migration_verify_mismatch`, where a mismatched copy is cut over anyway (33.2 V1).
- **M-credit-before-cleanup** (dropped-validation operator) — `src/Amoebius/Storage/RetainedScaling.hs`:
  ...
  observed deletion" step (33.2 V1).
- **M-fake-verify** (mechanism-substitution operator) — `src/Amoebius/Storage/RetainedScaling.hs`: the
  ...
  byte-for-byte nonce readback (33.2 V1).
...
- [`cluster_lifecycle_doctrine.md §7`](../documents/engineering/cluster_lifecycle_doctrine.md#7-ephemeral-spin-updown-with-deterministic-rebind)
  ...
  (delivered in Phase 31) — the SSA reconciler that renders and applies the StorageClass and PV objects.
...
## Sprint 33.1: The one inert `no-provisioner` StorageClass ✅
...
- A single rendered StorageClass — `provisioner: kubernetes.io/no-provisioner`, `reclaimPolicy: Retain`,
  `volumeBindingMode: WaitForFirstConsumer` — applied through the Phase-31 reconciler under the `amoebius`
  field manager.
...
## Sprint 33.2: Deterministic retained-PV generation + the explicit bind ✅
...
## Sprint 33.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip ✅
...
### Deliverables
- Minimal single-ordinal Postgres and MinIO witness StatefulSets on retained PVs (from Sprint 33.2), served
  from baked-binary images; these are rebind witnesses, not distributed MinIO (Phase 35) or HA
  Patroni-via-Percona (Phase 36), and carry no Vault-enveloping (Phase 34).
...
1. Run the cycle on the concrete representative set of [Gate integrity](#gate-integrity) (exactly two witnesses): generate a per-run nonce, assert its absence, write it as the Postgres row and the MinIO object,
   ...
   `${RETAINED_ROOT}` still holds the bytes, `cluster recreate` as a fresh Phase-29 bootstrap (new apiserver
...
`.build/runs/phase_33/`; externally attest the bundle and rerun after Phase 32 closes.
...
- `documents/engineering/storage_lifecycle_doctrine.md` — the §6 lossless-rebind guarantee gains its first
  ...
  planning-ownership pointer resolves to delivered Phase-33 sprints.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the
  Phase-31 SSA reconciler that applies the StorageClass and retained-PV objects
...
- [phase_31](phase_31_object_reconciler.md) — the typed renderer + live SSA reconciler this phase builds on
- [phase_34](phase_34_vault_pki.md) — the root Vault whose durable KV rebinds on the retained storage proven here
- [phase_35](phase_35_platform_backbone.md) / [phase_36](phase_36_platform_services_2.md) — the HA
