# Phase 36: Platform services-2 (Redis/Sentinel + Percona/Patroni + pgAdmin + observability + readiness-DAG)
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_33_retained_storage.md, DEVELOPMENT_PLAN/phase_35_platform_backbone.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 36.1: Percona/Patroni Postgres per consumer + pgAdmin + Prometheus/Grafana ⏸️](#sprint-361-perconapatroni-postgres-per-consumer--pgadmin--prometheusgrafana-)
- [Sprint 36.2: Ephemeral Redis/Sentinel realtime coordination ⏸️](#sprint-362-ephemeral-redissentinel-realtime-coordination-)
- [Sprint 36.3: The full derived readiness-DAG bring-up + the standard-stack gate ⏸️](#sprint-363-the-full-derived-readiness-dag-bring-up--the-standard-stack-gate-)
- [Sprint 36.4: Register-2.5 readiness-DAG bring-up under simulated partial failure ⏸️](#sprint-364-register-25-readiness-dag-bring-up-under-simulated-partial-failure-)
...
⏸️ Blocked pending Phase-35 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
**Observed artifact migration — 2026-08-11:** `expected-base-digest.txt` duplicates Phase-30 run output, and
`postgres-share-package.sha256` is a package/archive integrity observation. Both are generated. Phase 36 must
consume Phase 30's verified image identity and Phase 1's dynamic package resolution without committed checksum
files.

**Invalidated historical record:**

**Done.** All four sprints are implemented and the Phase-36 gate is sealed. The gate combines the pure
service model, 256 deterministic `IOSim` fault schedules plus `IOSimPOR`, eight specifically red mutants,
an independent live-receipt reader, and a warm reconciliation of the real Phase-35 cluster. It runs on the
**linux-cpu** substrate across **Register 3** (live infrastructure) — the same
single-node `kind` cluster on a linux-cpu host, on top of the Phase-30 registry + baked base image, the
Phase-31 typed renderer + SSA reconciler, the Phase-33 no-provisioner retained storage, the Phase-34 unsealed
root Vault, and the Phase-35 MetalLB/MinIO/Pulsar backbone. Redis/Sentinel, Patroni Postgres, pgAdmin,
...
This phase completes the standard platform-service set on top of the Phase-35 backbone. It renders and
...
`postgres` Pod), each rendered as typed Kubernetes objects by the Phase-31 `renderAll` path (no Helm, no
third-party charts, manifests generated from Haskell and never committed), each served from binaries **baked into the native-architecture base image** with no public-registry pull, and each on the Phase-33 `no-provisioner`
...
The phase then assembles the *whole* standard stack — the Phase-35 backbone plus these services — as one
...
order by the Phase-31 reconciler's **event-driven wait-for-ready**: a dependent starts on its dependency's
...
```
*Validated Phase-36 gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
The apparatus phase 36's gate closes over, in the slot
...
- **Image-identity provenance (§M.5 OS-boundary observer).** Every running container's `imageID` digest
  (`kubectl get pods -A -o jsonpath={..imageID}`) MUST equal the digest in the verified Phase-30 attestation
...
- **Applied-manifest oracle (§M.3 independent provenance).** Recompute `renderAll` during the gate and compare its
  ...
  third-party YAML therefore diverges from the Phase-31 renderer and fails.
- **Phase-36 resource witness readback (§M.3 independent projection).** Normalize every live resource-bearing
...
operator, the per-consumer Patroni Postgres clusters with pgAdmin for the named consumer set of Sprint 36.1,
Prometheus + Grafana, and Redis primary/replicas/Sentinel — no more, no fewer. The full derived DAG spans these plus the Phase-35 backbone
(MetalLB, MinIO, Pulsar) and the Phase-34 Vault.
...
- [`ui_realtime_coordination_doctrine.md §5 — Redis is ephemeral platform-internal coordination`](../documents/engineering/ui_realtime_coordination_doctrine.md#5-redis-is-ephemeral-platform-internal-coordination):
  Phase 36 stands up the bounded TLS/ACL Redis/Sentinel topology with no persistence or application authority.
...
- [`platform_services_doctrine.md §8 — Postgres, Patroni-via-Percona, one cluster per consumer, with pgAdmin`](../documents/engineering/platform_services_doctrine.md#8-postgres--patroni-via-percona-one-cluster-per-consumer-with-pgadmin)
  ...
  Phase 36 stands up the cluster-wide Percona operator reconciling one Patroni Postgres cluster per consuming
...
- [`platform_services_doctrine.md §7 — Prometheus / Grafana, observability is not an add-on`](../documents/engineering/platform_services_doctrine.md#7-prometheus--grafana--observability-is-not-an-add-on)
  ...
  through the generated admission proxy; the browser surfaces gated behind the (Phase 37) Keycloak edge
...
- [`manifest_generation_doctrine.md §5 — the apply/reconcile engine: server-side apply, owned field manager, prune, wait`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  Phase 36 reuses the Phase-31 pure `renderAll :: ProvisionedSpec -> [K8sObject]` and typed-action reconciler whose **wait-for-ready is observed from the live object, never a `threadDelay`** to apply and sequence the set. - [`image_build_doctrine.md §2 — the single distribution rule`](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster):
  every service binary (Redis, Percona operator, Patroni, pgAdmin, Prometheus, Grafana) is baked into the Phase-30
...
- [`storage_lifecycle_doctrine.md §2 — one storage class, and it provisions nothing`](../documents/engineering/storage_lifecycle_doctrine.md#2-one-storage-class-and-it-provisions-nothing)
  ...
  each Patroni cluster and Prometheus lands its durable bytes on the Phase-33 `no-provisioner` retained PVs.
- [`deterministic_simulation_doctrine.md §4`](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  ...
  *real* readiness-DAG orchestration runs unchanged under `IOSimPOR` against the Phase-15.4 modeled substrates
...
## Sprint 36.1: Percona/Patroni Postgres per consumer + pgAdmin + Prometheus/Grafana ⏸️
...
- The Percona operator rendered as a cluster-wide platform component (from the shared inventory so it installs
  ...
  the `distribution` registry takes **no** database ([§3](../documents/engineering/platform_services_doctrine.md#3-the-registry--the-single-image-source)). The consumer set is pinned here (resolving the `platform_services_doctrine.md §8` "Phase 35 delivery detail"); a `PerconaPGCluster` consumed by nothing does
...
- Prometheus + Grafana scraping platform workloads, with the per-workflow recording/alert rules and
  ...
  - The browser surfaces reach a user only through the (Phase 37) Keycloak edge under a mandatory
...
1. Assert the Percona operator is Ready before any `PerconaPGCluster`, then that the named consumer set
   ...
   `{Grafana}` because Keycloak's own store arrives with the [Phase 37](phase_37_keycloak_ingress.md) edge and
...
## Sprint 36.3: The full derived readiness-DAG bring-up + the standard-stack gate ⏸️
...
### Deliverables
- A `BringUp` assembly that folds the whole standard-service set (Phase-35 backbone + Sprint-31.1 and Sprint-31.2 services)
...
2. Round-trip MinIO put/get and Pulsar produce/consume against the assembled stack; assert Postgres is a
   Patroni cluster, never a bare Pod, carrying the mandated synchronous config (the Sprint 36.1 oracle).
3. Assert the complete standard stack is ready and preserves Phase 35's already-gated backbone topology and
   provenance. For the Phase-36 additions, a one-versus-many replica render diff may change only count fields;
   ...
   pull log and require every live `imageID` to equal the digest in the verified Phase-30 attestation and the
...
against the Phase-15.4 fakes with injected partial failure, restart, and partition. The numbered validation
...
3. Replay a captured seed and assert a bit-identical schedule and outcome; emit the Register-2.5 ledger — substrate `none`, Register 2.5 — recording the honest limit that modeled-substrate fidelity is *assumed* and is discharged only by this phase's Register-3 live gate (Sprint 36.3).
...
- `platform_services_doctrine.md`, `chaos_failover_doctrine.md`, `readiness_ordering_doctrine.md`,
  ...
  `deterministic_simulation_doctrine.md` now record the tested Phase-36 boundary and its honest limits.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the Phase-31 renderer + SSA wait-for-ready that applies and sequences the set
...
- [UI Realtime Coordination](../documents/engineering/ui_realtime_coordination_doctrine.md) — the ephemeral
  Redis/Sentinel topology and failure boundary delivered by Sprint 36.2
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the Register-2.5 `IOSim`/`IOSimPOR` simulation of the real bring-up over the Phase-15.4 modeled substrates
- [phase_34](phase_34_vault_pki.md) — the root Vault/PKI whose unseal edge gates secret-dependent startup here
- [phase_35](phase_35_platform_backbone.md) — the MetalLB/MinIO/Pulsar backbone this phase's services and DAG build on
- [phase_37](phase_37_keycloak_ingress.md) — the Keycloak-owned ingress edge that fronts Grafana and pgAdmin next
