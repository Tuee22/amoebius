# Phase 56: Hostless provider child + convergence + Lease handoff

> **Purpose**: Bring a provider-managed EKS child — deployed and checkpoint-observed by
> [Phase 55](phase_55_provider_deploy_checkpoint.md) — to the same fungible shape as any self-managed amoebius
...
Phase 56 delivers the hostless provider child + convergence + Lease handoff; its design is owned by [cluster_lifecycle_doctrine.md](../documents/engineering/cluster_lifecycle_doctrine.md), [daemon_topology_doctrine.md](../documents/engineering/daemon_topology_doctrine.md), [image_build_doctrine.md](../documents/engineering/image_build_doctrine.md), and the plan for reaching it is owned here.
...
UNVERIFIED because Phase 55 could not authenticate to AWS.
...
- [Sprints](#sprints)
- [Sprint 56.1: Two-stage capacity bootstrap for a hostless provider child ⏸️](#sprint-561-two-stage-capacity-bootstrap-for-a-hostless-provider-child-)
- [Sprint 56.2: Parent→child Lease handoff + hostless daemon topology ⏸️](#sprint-562-parentchild-lease-handoff--hostless-daemon-topology-)
- [Sprint 56.3: Standard-HA platform-service convergence from typed manifests ⏸️](#sprint-563-standard-ha-platform-service-convergence-from-typed-manifests-)
- [Sprint 56.4: The provider-child bring-up gate ⏸️](#sprint-564-the-provider-child-bring-up-gate-)
...
[Phase 55](phase_55_provider_deploy_checkpoint.md) gate (the provider-cluster Pulumi deploy-from-inside + Vault-Transit-enveloped MinIO checkpoint + `observeProviderAccount`, which lands a ready `Managed Eks` control plane and its base managed node group with the pinned amoebius base/scheduler OCI content already imported and capacity-debited into the first node's CRI store). It runs on the **linux-cpu → provider** substrate in
**Register 3** (live infrastructure): the parent amoebius cluster is a single-node `kind` cluster on linux-cpu
(the Phase 35 bootstrap coordinator), from inside which the Deployment-`replicas=1` control-plane daemon (Phase 44) drove the Phase-55
...
convergence** — never the Pulumi deploy/checkpoint that produced its input ([Phase 55](phase_55_provider_deploy_checkpoint.md)),
never the per-PV durable EBS + create-vs-delete credential model ([Phase 57](phase_57_provider_ebs_credential.md)),
and never dynamic node provisioning or the leak-free teardown sweep ([Phase 58](phase_58_provider_dynamic_nodes.md)).
...
First, **the two-stage capacity bootstrap for a hostless child**. Once the EKS API and Phase-55 base node are
...
and capacity-debited into the base node's CRI store by Phase 55, never the not-yet-ready child registry or a
...
Envoy/Gateway API, Keycloak, cloud LoadBalancer — through the Phase-37 reconciler, **not** a thinner or different
...
flowchart LR
%% register: algebra
  p34["Phase 55: ready Managed Eks control plane + base node, scheduler OCI preloaded"]:::intent --> boot[/"Parent bootstrap Lease holder: create amoebius-capacity-scheduler pods=1"/]:::effect
...
Phase-55 deploy and now drives this convergence; `→ provider` (EKS) is the deploy target class, not a hardware
...
Phase-55-deployed `Managed Eks` child. Exit 0 and a self-reported empty diff cannot satisfy it.
...
[Phase 55](phase_55_provider_deploy_checkpoint.md); the per-PV EBS / credential apparatus
...
`mut-46.1-drop-copy-executor` mutants) stays in [Phase 57](phase_57_provider_ebs_credential.md); the dynamic-node
...
[Phase 58](phase_58_provider_dynamic_nodes.md). This phase inherits only the convergence/handoff slice below.
...
- **Representative set (§M.7)** — the committed topology `test/fixture/dhall/provider_ebs_credential/provider_provision.dhall`, restricted
  ...
  base managed node group (size 1, deployed by Phase 55), the child `amoebius-capacity-scheduler` (`pods=1`), the
...
handoff, and the no-op re-run against a Phase-55-deployed base node group; it does **not** provision a dynamic
extra node ([Phase 58](phase_58_provider_dynamic_nodes.md)), bind a per-PV durable EBS
([Phase 57](phase_57_provider_ebs_credential.md)), or assert the independent leak-free tag-sweep at teardown
([Phase 58](phase_58_provider_dynamic_nodes.md)) — the run tears the base provider stack down through Phase 49's
own deploy lifecycle, and the leak-free sweep witness is recorded **deferred to Phase 58**, never as proven here.
...
- [`cluster_lifecycle_doctrine.md §1`](../documents/engineering/cluster_lifecycle_doctrine.md#1-two-cluster-kinds-one-lifecycle-shape)
  ...
  mandatory capacity-scheduler role — as the child-side of Phase 55's cloud-keyed amoebic spawn, converging the
...
- [`image_build_doctrine.md §2`](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster)
  ...
  the exact OCI digest Phase 55 preloaded into the base node's CRI store, and every standard platform service is
...
## Sprint 56.1: Two-stage capacity bootstrap for a hostless provider child ⏸️
...
Phase-38 `Amoebius.Scheduler.*` role and its two-stage bootstrap-cutover / execution-identity-admission
...
- Creation of `amoebius-capacity-scheduler` with exact `pods=1`, whose Deployment references the **exact OCI digest preloaded and capacity-debited into the base node's CRI store by Phase 55** — never the not-yet-ready child
...
- The managed-authority mint: install the managed-node taint, execution-identity admission, and full exclusive
  ...
  the cutover.
- An in-file honesty note: this stages a *provider child* through the same two-stage cutover Phase 38 proved on a
  self-managed cluster; the provider realization is validated here for the first time.

### Validation
1. Against a Phase-55-deployed child, the parent bootstrap holder creates `amoebius-capacity-scheduler`
...
### Remaining Work
Run the built protocol against a Phase-55-created EKS control plane and managed node, then read back the real
provider add-on and CRI-preload boundaries. The local and retained-Kubernetes protocol seams are complete.

## Sprint 56.2: Parent→child Lease handoff + hostless daemon topology ⏸️
...
## Sprint 56.3: Standard-HA platform-service convergence from typed manifests ⏸️
...
standard-service convergence), converging through the Phase-37 reconciler and consuming the Phase 36/39–43
...
Phase-37 reconciler — reachable, HA, wild ingress only via Keycloak — **not** a thinner or different service
...
typed manifests via the Phase-37 reconciler, with no Helm and no public-registry pull, so substrate equivalence
...
- Convergence of the **complete** standard HA platform-service stack — registry (`distribution`), MinIO, Vault,
  ...
  LoadBalancer — through the Phase-37 reconciler from typed manifests, using the same HA-capable topology at
...
## Sprint 56.4: The provider-child bring-up gate ⏸️
...
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: one end-to-end run takes a Phase-55-deployed `Managed Eks` child from bootstrap
...
per-run ledger; the leak-free sweep deferred to Phase 58), `DEVELOPMENT_PLAN/README.md`.
...
- A per-run proven/tested/assumed ledger recording: provider-child bootstrap + handoff + standard-HA convergence
  ...
  OS-boundary audit; and the elevated-harness leak-free durable-resource *sweep* as **explicitly deferred to Phase 58, not asserted here** — skipping an applicable observation move marks that layer UNVERIFIED, never
  green.
...
4. Assert the run emits a proven/tested/assumed ledger recording the independent Lease-handoff sequence, the
   ...
   Phase 55's deploy lifecycle and the leak-free tag-sweep is recorded **deferred to Phase 58**, never asserted
   green here.

### Remaining Work
Re-run without `PARTIAL_EXTERNAL_AUTHORITY` once valid AWS authority permits Phase 55 to materialize the EKS
child. Eleven enumerated provider/cloud surfaces remain explicitly UNVERIFIED; Phase 58 still owns the final
...
- `documents/engineering/testing_doctrine.md` — record the Phase 56 per-run ledger artifact and the explicit
  deferral of the elevated leak-free durable-resource sweep to Phase 57.

**Cross-references to add:**
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-runtime` provider-child control-plane daemon wiring
  (`Amoebius.Daemon.InClusterControlPlane`) and `Amoebius.Cluster.ProviderBringUp` as Phase-56 design-first rows,
  each mapped to its owning doctrine; map the reused `Amoebius.Scheduler.*` role to its Phase 38 delivery and the
  reconciler to Phase 37.
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 56 → `linux-cpu` (parent) row with the `provider` (EKS)
  deploy target annotated as a target class, not a fifth hardware substrate.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase 56 row's status once the gate passes; link this document.
...
- [phase_55](phase_55_provider_deploy_checkpoint.md) — the provider-cluster Pulumi deploy-from-inside +
...
- [phase_44](phase_44_live_dsl_deploy.md) — supplies the control-plane daemon role and Lease authority protocol used by
  the provider child
- [phase_57](phase_57_provider_ebs_credential.md) — the per-PV durable EBS + create-vs-delete credential + static
  EBS CSI arm, layered on Phase 55, not exercised here
- [phase_58](phase_58_provider_dynamic_nodes.md) — dynamic node provisioning by signal and the leak-free provider
