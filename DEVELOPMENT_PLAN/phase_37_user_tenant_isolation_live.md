# Phase 37: Live subject/tenant isolation
...
Phase 37 delivers the live subject/tenant isolation; its design is owned by [tenancy_doctrine.md](../documents/engineering/tenancy_doctrine.md), [low_code_ui_runtime_doctrine.md](../documents/engineering/low_code_ui_runtime_doctrine.md), [testing_doctrine.md](../documents/engineering/testing_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 37.1: Live scoped request and provider-isolation gate ⏸️](#sprint-371-live-scoped-request-and-provider-isolation-gate-)
...
provider observers, cleanup equality, and two red source mutants. Browser tenant switching remains Phase 57;
...
coordinates or credentials to the caller. The phase tests enforcement, not a full browser UX; Phase 57 owns
...
**Dependency:** Phases 33, 35, and 36 — respectively Keycloak-owned ingress, delivered and independently read
six-provider administrative policy, and the native Pulsar client. Phase 35 supplies readiness only; this phase
...
`src/Amoebius/Test/{Harness,Sweep}.hs` that [Phase 55](phase_55_test_topology_dsl.md) later consolidates these
...
- One minimal same-binary server worker behind the Phase-33 Keycloak/Envoy edge, with a sealed action registry
...
## Sprint 37.1: Live scoped request and provider-isolation gate ⏸️
...
- `DEVELOPMENT_PLAN/phase_57_ui_multi_tenant_live.md` — consume this provider-isolation result unchanged.

## Related Documents

- [Phase 35](phase_35_app_tenancy.md) — tenant policy derivation and provider materialization.
- [Phase 36](phase_36_pulsar_client.md) — native authenticated message path used by this gate.
