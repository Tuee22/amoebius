# Phase 42: Platform services-2 (Redis/Sentinel + Percona/Patroni + pgAdmin + observability + readiness-DAG)
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_39_retained_storage.md, DEVELOPMENT_PLAN/phase_41_platform_backbone.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 42.1: Percona/Patroni Postgres per consumer + pgAdmin + Prometheus/Grafana ⏸️](#sprint-421-perconapatroni-postgres-per-consumer--pgadmin--prometheusgrafana-)
- [Sprint 42.2: Ephemeral Redis/Sentinel realtime coordination ⏸️](#sprint-422-ephemeral-redissentinel-realtime-coordination-)
- [Sprint 42.3: The full derived readiness-DAG bring-up + the standard-stack gate ⏸️](#sprint-423-the-full-derived-readiness-dag-bring-up--the-standard-stack-gate-)
- [Sprint 42.4: Register-2.5 readiness-DAG bring-up under simulated partial failure ⏸️](#sprint-424-register-25-readiness-dag-bring-up-under-simulated-partial-failure-)
...
⏸️ Blocked pending Phase-41 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
**Observed artifact migration — 2026-08-11:** `expected-base-digest.txt` duplicates Phase-36 run output, and
`postgres-share-package.sha256` is a package/archive integrity observation. Both are generated. Phase 42 must
consume Phase 36's verified image identity and Phase 1's dynamic package resolution without committed checksum
files.

**Invalidated historical record:**

**Done.** All four sprints are implemented and the Phase-42 gate is sealed. The gate combines the pure
service model, 256 deterministic `IOSim` fault schedules plus `IOSimPOR`, eight specifically red mutants,
an independent live-receipt reader, and a warm reconciliation of the real Phase-41 cluster. It runs on the
**linux-cpu** substrate across **Register 3** (live infrastructure) — the same
single-node `kind` cluster on a linux-cpu host, on top of the Phase-36 registry + baked base image, the
Phase-37 typed renderer + SSA reconciler, the Phase-39 no-provisioner retained storage, the Phase-40 unsealed
root Vault, and the Phase-41 MetalLB/MinIO/Pulsar backbone. Redis/Sentinel, Patroni Postgres, pgAdmin,
...
This phase completes the standard platform-service set on top of the Phase-41 backbone. It renders and
...
`postgres` Pod), each rendered as typed Kubernetes objects by the Phase-37 `renderAll` path (no Helm, no
third-party charts, manifests generated from Haskell and never committed), each served from binaries **baked into the native-architecture base image** with no public-registry pull, and each on the Phase-39 `no-provisioner`
...
The phase then assembles the *whole* standard stack — the Phase-41 backbone plus these services — as one
...
order by the Phase-37 reconciler's **event-driven wait-for-ready**: a dependent starts on its dependency's
...
browser surface Grafana and pgAdmin reach a user through — is [Phase 43](phase_43_keycloak_ingress.md); this
...
[Phase 44](phase_44_live_dsl_deploy.md). Until then, a host-side operator drives this phase's fixed service
set.
...
**Gate:** `python3 tools/platform_services_2_gate.py` brings the whole standard stack up on the Phase-41 backbone cluster
...
```
*Validated Phase-42 gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
The apparatus phase 42's gate closes over, in the slot
...
- **Derived-DAG order (§M.2 committed mutant, §M.3 independent oracle, §M.4 coverage floor, §M.5 external-observer trace).** The
  ...
  (Sprint 42.3), checked against a committed hand-authored edge→order reference table
...
- **Image-identity provenance (§M.5 OS-boundary observer).** Every running container's `imageID` digest
  (`kubectl get pods -A -o jsonpath={..imageID}`) MUST equal the digest in the verified Phase-36 attestation
...
- **Applied-manifest oracle (§M.3 independent provenance).** Recompute `renderAll` during the gate and compare its
  ...
  third-party YAML therefore diverges from the Phase-37 renderer and fails.
- **Phase-42 resource witness readback (§M.3 independent projection).** Normalize every live resource-bearing
...
operator, the per-consumer Patroni Postgres clusters with pgAdmin for the named consumer set of Sprint 42.1,
Prometheus + Grafana, and Redis primary/replicas/Sentinel — no more, no fewer. The full derived DAG spans these plus the Phase-41 backbone
(MetalLB, MinIO, Pulsar) and the Phase-40 Vault.
...
- [`ui_realtime_coordination_doctrine.md §5 — Redis is ephemeral platform-internal coordination`](../documents/engineering/ui_realtime_coordination_doctrine.md#5-redis-is-ephemeral-platform-internal-coordination):
  Phase 42 stands up the bounded TLS/ACL Redis/Sentinel topology with no persistence or application authority.
...
- [`platform_services_doctrine.md §8 — Postgres, Patroni-via-Percona, one cluster per consumer, with pgAdmin`](../documents/engineering/platform_services_doctrine.md#8-postgres--patroni-via-percona-one-cluster-per-consumer-with-pgadmin)
  ...
  Phase 42 stands up the cluster-wide Percona operator reconciling one Patroni Postgres cluster per consuming
...
- [`platform_services_doctrine.md §7 — Prometheus / Grafana, observability is not an add-on`](../documents/engineering/platform_services_doctrine.md#7-prometheus--grafana--observability-is-not-an-add-on)
  ...
  through the generated admission proxy; the browser surfaces gated behind the (Phase 43) Keycloak edge
...
- [`manifest_generation_doctrine.md §5 — the apply/reconcile engine: server-side apply, owned field manager, prune, wait`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  Phase 42 reuses the Phase-37 pure `renderAll :: ProvisionedSpec -> [K8sObject]` and typed-action reconciler whose **wait-for-ready is observed from the live object, never a `threadDelay`** to apply and sequence the set. - [`image_build_doctrine.md §2 — the single distribution rule`](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster):
  every service binary (Redis, Percona operator, Patroni, pgAdmin, Prometheus, Grafana) is baked into the Phase-36
...
- [`storage_lifecycle_doctrine.md §2 — one storage class, and it provisions nothing`](../documents/engineering/storage_lifecycle_doctrine.md#2-one-storage-class-and-it-provisions-nothing)
  ...
  each Patroni cluster and Prometheus lands its durable bytes on the Phase-39 `no-provisioner` retained PVs.
- [`deterministic_simulation_doctrine.md §4`](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  ...
  *real* readiness-DAG orchestration runs unchanged under `IOSimPOR` against the Phase-21.4 modeled substrates
...
## Sprint 42.1: Percona/Patroni Postgres per consumer + pgAdmin + Prometheus/Grafana ⏸️
...
- The Percona operator rendered as a cluster-wide platform component (from the shared inventory so it installs
  ...
  the `distribution` registry takes **no** database ([§3](../documents/engineering/platform_services_doctrine.md#3-the-registry--the-single-image-source)). The consumer set is pinned here (resolving the `platform_services_doctrine.md §8` "Phase 41 delivery detail"); a `PerconaPGCluster` consumed by nothing does
...
- Prometheus + Grafana scraping platform workloads, with the per-workflow recording/alert rules and
  ...
  - The browser surfaces reach a user only through the (Phase 43) Keycloak edge under a mandatory
...
1. Assert the Percona operator is Ready before any `PerconaPGCluster`, then that the named consumer set
   ...
   `{Grafana}` because Keycloak's own store arrives with the [Phase 43](phase_43_keycloak_ingress.md) edge and
...
and monitoring expectations. The Keycloak browser edge remains Phase 43 and control-plane-owned continuous
reconciliation remains Phase 44, both explicitly UNVERIFIED.

## Sprint 42.2: Ephemeral Redis/Sentinel realtime coordination ⏸️
...
Sentinel voters from the Phase-36 image, and an independent TLS/ACL client observes replication and a forced
...
## Sprint 42.3: The full derived readiness-DAG bring-up + the standard-stack gate ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `src/Amoebius/Platform/Services.hs`,
`src/Amoebius/Platform/BringUp.hs`, `tools/platform_services_2_gate.py`
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: the full standard stack (Phase-41 backbone + the
Sprint-37.1 and Sprint-37.2 services) is assembled as one acyclic derived readiness DAG whose edges are the
...
### Deliverables
- A `BringUp` assembly that folds the whole standard-service set (Phase-41 backbone + Sprint-37.1 and Sprint-37.2 services)
...
- Live enactment by the Phase-37 reconciler's wait-for-ready — each dependent constructed to start on its
  dependency's observed-ready edge (a `runtime-checked` observation from the live object), never a duration;
  the control-plane daemon that will later own this loop is [Phase 44](phase_44_live_dsl_deploy.md) (Deployment `replicas=1`, no election).
- The phase-gate harness brings the standard stack onto Phase 41's live backbone and observes every required
...
2. Round-trip MinIO put/get and Pulsar produce/consume against the assembled stack; assert Postgres is a
   Patroni cluster, never a bare Pod, carrying the mandated synchronous config (the Sprint 42.1 oracle).
3. Assert the complete standard stack is ready and preserves Phase 41's already-gated backbone topology and
   provenance. For the Phase-42 additions, a one-versus-many replica render diff may change only count fields;
   ...
   pull log and require every live `imageID` to equal the digest in the verified Phase-36 attestation and the
...
### Remaining Work
Remove `expected-base-digest.txt`, consume the verified Phase-36 identity as run input, and rerun the warm
...
## Sprint 42.4: Register-2.5 readiness-DAG bring-up under simulated partial failure ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `test/spec/platform/BringUpSim.hs` (the `IOSimPOR` harness
driving the unmodified Sprint-37.3 `src/Amoebius/Platform/BringUp.hs` orchestration through a typed fault
observer)
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: the exact Sprint-37.3 bring-up orchestration, unmodified, runs under `IOSimPOR`
against the Phase-21.4 fakes with injected partial failure, restart, and partition. The numbered validation
...
phase's own bring-up. The subject is the *real* Sprint-37.3 readiness-DAG orchestration: the derived graph
...
Phase-21.4 modeled substrates, so the ordering and fail-closed invariants are validated deterministically
in-process before the Register-3 live gate ever runs.

### Deliverables
- An `IOSimPOR` harness that drives the *unmodified* Sprint-37.3 `BringUp` orchestration (written against `io-classes`, no real IO) against the Phase-21.4 fake Pulsar/MinIO/apiserver/route53/Vault/clock (`src/Amoebius/Sim/Env.hs` + `src/Amoebius/Sim/Fakes/*`), with injected **partial failure, restart, and network partition** on the modeled dependencies.
...
3. Replay a captured seed and assert a bit-identical schedule and outcome; emit the Register-2.5 ledger — substrate `none`, Register 2.5 — recording the honest limit that modeled-substrate fidelity is *assumed* and is discharged only by this phase's Register-3 live gate (Sprint 42.3).
...
- `platform_services_doctrine.md`, `chaos_failover_doctrine.md`, `readiness_ordering_doctrine.md`,
  ...
  `deterministic_simulation_doctrine.md` now record the tested Phase-42 boundary and its honest limits.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the Phase-37 renderer + SSA wait-for-ready that applies and sequences the set
...
- [UI Realtime Coordination](../documents/engineering/ui_realtime_coordination_doctrine.md) — the ephemeral
  Redis/Sentinel topology and failure boundary delivered by Sprint 42.2
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the Register-2.5 `IOSim`/`IOSimPOR` simulation of the real bring-up over the Phase-21.4 modeled substrates
- [phase_40](phase_40_vault_pki.md) — the root Vault/PKI whose unseal edge gates secret-dependent startup here
- [phase_41](phase_41_platform_backbone.md) — the MetalLB/MinIO/Pulsar backbone this phase's services and DAG build on
- [phase_43](phase_43_keycloak_ingress.md) — the Keycloak-owned ingress edge that fronts Grafana and pgAdmin next
