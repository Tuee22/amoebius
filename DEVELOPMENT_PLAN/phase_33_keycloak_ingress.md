# Phase 33: Keycloak-owned ingress
...
- [Sprints](#sprints)
- [Sprint 33.1: The Keycloak-owned edge — LoadBalancer → Envoy/Gateway API → Keycloak ⏸️](#sprint-331-the-keycloak-owned-edge--loadbalancer--envoygateway-api--keycloak-)
- [Sprint 33.2: No self-published wild ingress + public-edge TLS ⏸️](#sprint-332-no-self-published-wild-ingress--public-edge-tls-)
- [Sprint 33.3: East-west NetworkPolicy posture — derived default-deny ⏸️](#sprint-333-east-west-networkpolicy-posture--derived-default-deny-)
- [Sprint 33.4: The single-door + storage-rebind regression gate ⏸️](#sprint-334-the-single-door--storage-rebind-regression-gate-)
...
**Done.** All four sprints are implemented and the Phase-33 gate is sealed. The retained linux-cpu stack
...
real Phase-32 operator; as in Phase 32, the exact Patroni child is honestly recorded as an amoebius-owned
...
scratch cluster is removed. This isolated projection reuses the Phase-29 persistence harness and deliberately
does not destroy the retained Phase 24–33 platform cluster.
...
for every wild request. It composes the LoadBalancer address published by Phase 31 (MetalLB on `linux-cpu`)
with an **Envoy + Gateway API L7 data plane** and **Keycloak OIDC/JWT enforcement**, rendered as typed
manifests by the Phase-27 reconciler and applied to the live cluster, so that WAN, LAN, and even a
...
is applied and exercised live. Finally the phase re-runs the Phase-29 lossless-rebind proof behind the new
...
app tenancy, and the Pulsar/workflow runtime are Phase 34+ concerns; this phase exercises the edge from the
host binary against the fixed standard service set that Phases 31–32 stood up. The one genuinely new-vs-prodbox
...
Phase-31/32 standard service stack that the edge fronts: **Grafana, the Keycloak admin console, the Vault UI, the MinIO console, the platform API surface, and a platform-owned authenticated-WebSocket upgrade probe** (the exact set is the golden; if the stack's surface set changes,
...
replayed-nonce, wrong-subprotocol, and direct-Service attempts produce no backend frame. And the Phase-29
...
```
*Validated Phase-33 gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
- [`storage_lifecycle_doctrine.md` §6](../documents/engineering/storage_lifecycle_doctrine.md#6-the-lossless-teardown-guarantee-deterministic-rebind)
  — **the lossless-teardown guarantee: deterministic rebind**: the gate re-runs the Phase-29 marker-bytes
...
## Sprint 33.1: The Keycloak-owned edge — LoadBalancer → Envoy/Gateway API → Keycloak ⏸️
...
- Envoy + Gateway API rendered as the L7 edge (a `Gateway` listener plus `HTTPRoute`s as typed `K8sObject`s),
  terminating TLS and routing, applied by the Phase-27 reconciler.
- Keycloak deployed against its Phase-32 Patroni DB, owning OIDC/JWT enforcement in front of every platform
...
## Sprint 33.2: No self-published wild ingress + public-edge TLS ⏸️
...
## Sprint 33.3: East-west NetworkPolicy posture — derived default-deny ⏸️
...
- A default-deny east-west baseline plus allow-edges derived from the declared dependency graph, rendered by
  the Phase-27 reconciler and applied to the live cluster — no hand-authored policy.
...
3. Prove "derived" by **graph variation**: deploy a scratch consumer, add a declared edge to a provider,
   ...
   applied set must therefore be a total function of the declared graph, not of the fixed Phase-31/32 service
...
## Sprint 33.4: The single-door + storage-rebind regression gate ⏸️
...
- The storage-rebind regression: write the committed marker row (`marker-row.sql`) into the Keycloak Patroni DB
  and the committed marker object (`marker-object.bin`) into a MinIO bucket. Perform the Phase-29 intermediate
  ...
  same bytes back — the Phase-29 guarantee re-run behind the new edge. Plus committed mutant (c), a
...
**Engineering docs updated with the tested Phase-33 result:**
...
- [README.md](README.md) — flip the Phase 33 row status once work begins, and link this document from the
  Phase 33 paragraph.
- [substrates.md](substrates.md) — record `linux-cpu` as the Phase 33 gate substrate in the per-phase map.
...
- [README.md](README.md) — the live tracker; its Phase 33 row is the authoritative one-line gate and status.
...
- [Manifest Generation Doctrine](../documents/engineering/manifest_generation_doctrine.md) — the typed renderer
  + server-side-apply reconciler that enacts the edge (delivered in Phase 27).
