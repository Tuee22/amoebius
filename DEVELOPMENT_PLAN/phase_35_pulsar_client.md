# Phase 35: Native Pulsar client (CBOR)
...
orchestrator/worker workflow runtime with Pulsar-Failover single-writer takeover (both Phase 37), the topic
...
- **Committed oracle corpus (representative set, §M.7, §M.1).** The gate's representative set is named
  ...
  hand-computed canonical-CBOR byte strings; (c) the non-CBOR negative fixtures of Sprint 35.2 V4 with their
  ...
  strings), authored independently of `topicFor` (§M.3); (e) a pre-run snapshot of the standing Phase-30
...
- **Leak-free is an external enumerate-and-compare sweep (§M.5).** "Leak-free" is defined as: after teardown,
  ...
  bookkeeping) and asserts the set is empty, and asserts the standing Phase-30 namespace's policy set
...
orchestrator/worker. The Phase-35 live gate therefore declares a client-runner Pod with immutable image and OCI
...
bytes, concurrent/rate-window offloads, deletion lag, failed/orphan horizon, mutation admission). The Phase-30
...
- A structural `PulsarClientExecutionDemand` for connection/request/frame/reconnect peaks, bound into the
  complete envelope of the Phase-35 client-runner Pod and, later, each actual consumer Pod; there is no
...
seek-based replay — over the persistent session from Sprint 35.1, with **every application payload encoded
...
- A typed **CBOR payload codec** (`Amoebius.Pulsar.Cbor`, on `serialise`/`cborg`): `produce`/`consume` accept
  ...
  CBOR (shared with the store's canonical encoder, Phase 37) is used where the payload is content-addressed; a
...
`test/integration/Phase35RuntimeStorage.hs` (runner planned-slot→observed-Pod-UID join, component-role/layout
...
**Blocked by**: Sprint 35.2, Sprint 35.3; Phase 33 — the live DSL deploy via the `replicas=1` singleton (the
...
`DEVELOPMENT_PLAN/README.md` (flip the Phase-35 status when the gate passes).
...
**Blocked by**: Sprint 35.4 (the built at-least-once + dedup client); the Phase 15 gate (the `io-classes`
...
is only *modeled* here. Deterministically replayable, substrate `none`, Register 2.5.
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-pulsar` package and its target module paths
  ...
  `Topology`, `Dedup`, `Namespace`) as Phase-35 design-first rows against the component inventory.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 35's gate substrate (`linux-cpu`) in the per-phase substrate
  map, and the topology algebra's per-substrate topic lanes.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-35 row's status from this plan once the gate passes; link this
  document.
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the Register-2.5 io-sim environment the dedup fold's exactly-once is validated against in Sprint 35.5
...
- [Daemon Topology Doctrine](../documents/engineering/daemon_topology_doctrine.md) — who runs producers/
  consumers and picks subscription roles (Phase 37), delegated here
- [phase_30](phase_30_platform_backbone.md) — the standard-service stack that brings Pulsar up HA
- [phase_34](phase_34_app_tenancy.md) — the app tenancy this phase opens after
- [phase_37](phase_37_content_store_workflow.md) — the content store + workflow runtime that consumes this
  client
