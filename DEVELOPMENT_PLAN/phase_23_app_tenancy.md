...
**Register 3** — live infrastructure: the single-node `kind` cluster assembled through Phases 14–22, including
root Vault in Phase 18, the platform stack in Phases 19–20, and the Keycloak-owned edge in Phase 21. The
...
**Substrate:** linux-cpu — the whole gate runs on the single-node `kind` cluster assembled through Phases 14–22; no apple,
...
Phase 23 applies and reads back the **Pulsar provider's administrative tenant/namespace/ACL state only**. It
...
- [`tenancy_doctrine.md §3`](../documents/engineering/tenancy_doctrine.md#3-what-a-tenant-is) — *what a tenant
  ...
  `<t>/<bucket>` policy scopes exactly the `<app>/<bucket>` bytes Sprint 23.1 stores), and an
...
- [`vault_pki_doctrine.md §3`](../documents/engineering/vault_pki_doctrine.md#3-the-secretref-contract-a-name-never-a-value)
  ...
  provider credential, are carried as typed `SecretRef` names resolved in-cluster by the Phase-18 built-in Vault
  client — never a literal in Dhall.

## Sprints

## Sprint 23.1: The app data-tenant projection — namespace + `<app>/<bucket>` ObjectStore + in-namespace `Sql` 📋
...
projection is reconciled onto); Phases 19–20 (the standard MinIO + Patroni platform services this binds to, an
...
- A complete app-service/rollout `PodResourceEnvelope` and a pure `PatroniSqlDemand`; the latter expands to a
  ...
  free middleware.
- Provider credentials carried as typed `SecretRef` names (never literals), resolved in-cluster via the Phase-18
...
## Sprint 23.2: The `TenantSpec`/`UserSpec`/`RoleBinding` typed surface + no-foreign-tenant foreclosure 📋
...
**Blocked by**: Sprint 23.1 (the tenant-tagged data projection these bindings reference); the Phase-6
...
2. **One committed negative fixture per resource-reference arm the unification fold checks — `Ref` to a foreign
   ...
   **Each negative ships a minimal positive twin, byte-identical except the foreign tenant id corrected to the
   ...
   `RoleBinding` cases (§M.4), and the committed guard-negation mutant of the fold (Sprint 23.4) is run here and
   must turn this suite red.

### Remaining Work
The whole sprint (📋 Planned).

## Sprint 23.3: One provider-indexed tenant-policy transaction from the tenant→role graph 📋
...
**Blocked by**: Sprint 23.1, Sprint 23.2; Phase 18 (the root Vault + built-in client that hosts the derived
per-tenant policies), Phases 19–20 (MinIO, Pulsar, and Patroni persistence), and Phase 21 (the Keycloak-owned
edge) — external earlier-phase prerequisites. Phase 24 is deliberately **not** a prerequisite: Phase 23 uses
Pulsar's administrative policy adapter only; Phase 24 owns the native-client data-path gate.
...
7. The live Phase-23 gate checks MinIO and Vault same-tenant success paired with cross-tenant refusal, and exact
   ...
   Phase 24 must later pair the Pulsar cross-tenant refusal with a same-credential native-client round trip; until
   that gate passes, Pulsar data-path isolation remains unverified rather than being inferred from admin readback.

### Remaining Work
The whole sprint (📋 Planned).

## Sprint 23.4: Phase gate harness — deploy-a-tenant-app + reject-the-foreign-tenant-spec 📋
...
**Blocked by**: Sprint 23.1, Sprint 23.2, Sprint 23.3.
...
**Docs to update**: `DEVELOPMENT_PLAN/README.md` (flip the Phase-23 status when the gate passes),
`DEVELOPMENT_PLAN/substrates.md` (the Phase-23 linux-cpu gate row), `documents/engineering/testing_doctrine.md`,
...
- A **Register-3** proven/tested/assumed ledger recording the live-realization result (namespace + bucket + Sql
  ...
  explicitly marking the deferred surfaces — the Phase-24 Pulsar native-client round trip, tenant-admin
...
3. Assert the resource-boundary corpus rejects before any provider mutation and that the positive run's live
   ...
   produce/consume remains a Phase-24 gate.
...
- `documents/engineering/testing_doctrine.md` — record the Register-3 ledger variant this gate emits (the
  Phase-24 Pulsar client data path, tenant-admin surface, child-cluster promotion, and derivation fidelity
  UNVERIFIED).
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/App/Tenancy.hs`,
  ...
  `src/Amoebius/Tenancy/Provider/*.hs` adapters (with `dhall/amoebius/Tenant.dhall`) as Phase-23 rows; keep the
  authenticated native Pulsar client gate assigned to Phase 24.
...
- [phase_24](phase_24_pulsar_client.md) — owns the authenticated Pulsar native-client produce/consume gate that
  Phase 23's administrative ACL readback deliberately does not claim
