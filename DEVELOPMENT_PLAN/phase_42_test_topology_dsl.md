# Phase 42: Test-topology DSL + suggest-test + elevated harness

**Status**: Authoritative source
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_36_provider_ebs_credential.md, DEVELOPMENT_PLAN/phase_37_provider_dynamic_nodes.md, DEVELOPMENT_PLAN/phase_39_infernix_lift.md, DEVELOPMENT_PLAN/system_components.md
...
single-node `kind` cluster brought up by the Phase 17 midwife with Pulsar and MinIO already standing HA
(Phase 23) on retained storage (Phase 21). The mechanisms generalize patterns *proven in the sibling prodbox
...
Pulsar `Exclusive`/`Failover` subscription of Phase 29, while the control-plane singleton it deploys under is
...
the `replicas=1` singleton (Phase 26), the native Pulsar client and Pulsar-`Failover` worker takeover
...
(real Phase-17 host classification; the SSH/AWS credential probe run for real, or its layer explicitly
...
1. **Leak-freedom by implementation-independent inventory diff, not tag query.** Leak-freedom is asserted by a
   ...
   parent-drives-provider form over Phase 37's leak-free provider teardown, still one substrate per generated
   test (§L) — via the committed mutant `test/mutants/phase_42_cloud_leak_untyped.dhall`, an untagged AWS
...
2. **Committed seeded mutant that must go red (Cheat-1 operator: dropped effect).** The committed mutant
   `test/mutants/phase_42_leak_untyped.dhall` creates one resource (a `ConfigMap` and a backing PVC)
   *outside* the typed allocation call — hence never test-owned-tagged — and the gate run over that mutant
   ...
   `test/mutants/phase_42_leak_host_backing.dhall` removes its PVC/PV API bindings but deliberately leaves the
   ...
   `test/mutants/phase_42_ledger_all_tested.dhall` (invariant-clause delete: an emitter hardcoding every
...
3. **suggest-test provenance is falsifiable, not narrative.** The per-run record captures the raw
   ...
   (`test/dhall/phase_42_review_allowlist.json`); a review edit outside that set fails the gate.
4. **Ledger applicability is derived and oracle-pinned, not self-declared.** The set of applicable moves is
   ...
   the externally hand-authored expected-ledger fixture `test/golden/phase_42_ledger.json` (committed in
...
7. **Failover takeover pinned to an external broker-stats observer, not the `ExpectationWitness`.** The
   ...
   statistics (`pulsar-admin topics stats`/`topics partitioned-stats` on the inherited Phase-29 command topic)
   MUST show the name-ordered standby as the *active consumer* on that same subscription, equal to the
   externally hand-authored `test/golden/phase_42_failover_takeover.json` predicate — mirroring how leak-freedom
   is pinned to `kubectl`/`${RETAINED_ROOT}`/cloud enumerations rather than to a self-report. The committed
   seeded mutant `test/mutants/phase_42_failover_standby_wrong_subscription.dhall` (structural-takeover-defeat
   ...
   operator-authored witness would pass — paired with the canonical `test/dhall/phase_42_failover.dhall`
...
the `linux-cpu` single-node `kind` cluster, at minimum: the pre-standing Phase-23 HA Pulsar and MinIO on
retained Phase-21 storage; **at least one newly-allocated retained `no-provisioner` PV** (so the sole-deleter
rule of Sprint 42.4 is exercised against a resource this phase created); **one control-plane workload as a
...
`test/golden/phase_42_resource_shape.json`; the summed/effective pod demands must fit with a concrete placement
...
- `test/golden/phase_42_ledger.json` — the externally hand-authored expected ledger of Gate criterion 4;
...
- `test/golden/phase_42_cloud_inventory.json` — the hand-authored cloud resource-type/API inventory for the
...
- `test/golden/phase_42_resource_shape.json` — the independently authored exact capacity/capability supply and
...
- `test/golden/phase_42_optional_resource_shapes.json` — independently authored exact positive witnesses for
...
- `test/golden/phase_42_failover_takeover.json` — the externally hand-authored takeover predicate of Gate
...
- `test/mutants/phase_42_resource_overcommit_*.dhall` and
  `test/mutants/phase_42_missing_capability.dhall` — one-field variants covering
...
- `test/mutants/phase_42_leak_untyped.dhall` — the Cheat-1 seeded mutant (dropped-effect operator: a resource
  allocated outside the typed path) that MUST turn the inventory-diff red.
- `test/mutants/phase_42_leak_host_backing.dhall` — a backing-leak mutant that removes the PVC/PV API objects
  but leaves a marker-bearing retained allocation under `${RETAINED_ROOT}`; the host-allocation inventory diff
  MUST turn red.
- `test/mutants/phase_42_ledger_all_tested.dhall` — the Cheat-2 seeded mutant (invariant-clause-delete
  operator: an emitter marking every applicable move tested) that MUST fail the expected-ledger match.
- `test/mutants/phase_42_failover_standby_wrong_subscription.dhall` — the Gate-criterion-7 seeded takeover
  ...
  `test/dhall/phase_42_failover.dhall` positive the broker reports promoted.
- `test/mutants/phase_42_cloud_leak_untyped.dhall` — the provider-substrate cloud-leak mutant (Cheat-1
...
- `test/dhall/phase_42_review_allowlist.json` — the committed allowlist bounding which fields operator review
  may edit, making emitted→reviewed provenance falsifiable.

**Register binding for all sprint validations.** Every Sprint 42.1–36.5 Independent Validation and Validation
...
in the ledger; the leak-free teardown, the live identity-boundary denial (Sprint 42.4), and the failover
...
The topology inherits and source-checks Phase-29's exact command/event topic, Failover subscription,
...
- [`testing_doctrine.md §3`](../documents/engineering/testing_doctrine.md#3-the-test-topology-contract-spin-up--run--always-tear-down)
  — *the test-topology contract: spin up → run → always tear down*: Sprint 42.1 realizes the four-clause
...
- [`testing_doctrine.md §5`](../documents/engineering/testing_doctrine.md#5-suggest-test-detect-the-world-emit-a-representative-test-dhall)
  — *`suggest-test`: detect the world, emit a representative test `.dhall`*: Sprint 42.2 builds the generator
...
- [`testing_doctrine.md §6`](../documents/engineering/testing_doctrine.md#6-flagged-test-credentials)
  — *flagged test credentials*: Sprint 42.3 establishes the distinct flagged test-simulation identity (the
...
- [`testing_doctrine.md §4`](../documents/engineering/testing_doctrine.md#4-no-skips-fail-fast-and-the-per-run-ledger-artifact)
  — *no skips, fail fast, and the per-run ledger artifact*: Sprint 42.5 emits the per-run
...
## Sprint 42.1: The test-topology type — a deployment-rules layer that always tears down 📋
...
**Blocked by**: Phase 26 gate (the live DSL deploy via the Deployment-`replicas=1` singleton — the production
spec a test composes over); Phase 29 gate (the workflow runtime + Pulsar-`Failover` worker takeover the
schedule injects); Phase 21 / Phase 17 gates (the retained storage + cluster-lifecycle teardown the topology
drives)
**Independent Validation**: the resource-reclamation criteria are discharged in **Register 3** on the
`linux-cpu` `kind` cluster (a teardown against Phase-14 fakes does not prove real reclamation, so it does not
...
## Sprint 42.2: `suggest-test` — detect substrate + credential authority, emit a representative test `.dhall` 📋
...
**Blocked by**: Sprint 42.1 (the `TestTopology` type it emits values of); Phase 17 gate (substrate
detection — the pure host classification and the full-path substrate probe)
**Independent Validation**: (Register 2 — pure, over Phase-14 fake tools; its live probe layer recorded
...
to `test/golden/phase_42_optional_resource_shapes.json`; no selected positive arm may be ignored. The
...
2. Property tests perturb each supply axis independently. Lowering CPU, memory, local ephemeral storage
   ...
   enumerated committed one-short mutants (the pinned `phase_42_resource_overcommit_*`/`phase_42_missing_capability`
...
3. Exercise the closed optional-branch fixture matrix. For registry publication, independently shorten OCI
   ...
   `phase_42_optional_resource_shapes.json`. The committed
   `drop_registry_publication_envelope.dhall`, `drop_pulumi_executor_envelope.dhall`,
   `drop_pulumi_checkpoint_demand.dhall`, `drop_migration_copy_envelope.dhall`, and
...
4. The emitted value is immediately passed through `provision`; the canonical generated
   placement/storage/capability/quota witness matches `test/golden/phase_42_resource_shape.json`, while every
...
## Sprint 42.3: Flagged test credentials + test-owned resource tagging 📋
...
**Blocked by**: Sprint 42.1 (the topology whose allocations get tagged); Phase 22 gate (root Vault +
secret-by-name injection)
**Independent Validation**: (Register 2 — type-level, over Phase-14 fake tools; this sprint pins the
*representability* boundary, while the *live* identity-boundary denial is Sprint 42.4's Register-3 obligation):
...
- The flagged credential resolved by name only through Vault (Phase 22) — flagging changes *which* credential
...
## Sprint 42.4: The elevated harness as sole automated deleter of test-owned storage + leak-free sweep 📋
...
**Blocked by**: Sprint 42.1 (the teardown the sweep follows); Sprint 42.3 (the test-owned flag the sweep is
scoped by); Phase 21 gate (the retained `no-provisioner` PV model the rule protects); Phase 37 gate (the
...
### Deliverables
- A delete path reachable only by the elevated harness under the flagged credential of Sprint 42.3, scoped to
...
- A postflight sweep that, after teardown, asserts leak-freedom by the implementation-independent inventory
  ...
  `test/mutants/phase_42_leak_untyped.dhall` (an API resource allocated outside the typed path) and
  `test/mutants/phase_42_leak_host_backing.dhall` (API bindings removed, host backing left behind) are the
...
- An explicit scope boundary: this harness reclaims **test-owned** backing only. Production
  ...
  Phase 42 neither holds nor tests authority over production backing.
...
3. Leak detection uses the Gate-criterion-1 inventory diff, not a test-owned tag query: the committed seeded
   mutant `test/mutants/phase_42_leak_untyped.dhall` — a resource allocated *outside* the typed path, hence
   ...
   `test/mutants/phase_42_leak_host_backing.dhall`, which removes API bindings but leaves the new host backing,
   ...
   untagged-AWS-resource mutant `test/mutants/phase_42_cloud_leak_untyped.dhall` that MUST surface via
   ...
   (Phase 34), keeping this single-substrate `linux-cpu` run's own scope intact (§L).
...
## Sprint 42.5: The per-run ledger artifact + the delegated-failover gate topology 📋

**Status**: Planned
**Implementation**: `src/Amoebius/Test/Ledger.hs` (the proven/tested/assumed artifact emitter),
`test/dhall/phase_42_failover.dhall` (the gate topology), `test/live/FailoverGateSpec.hs`, and
...
paths; not yet built)
**Blocked by**: Sprint 42.1 (the topology runner); Sprint 42.2 (`suggest-test` produces the gate topology);
Sprint 42.4 (the leak-free sweep the gate asserts empty); Phase 29 gate (the Pulsar-`Failover` worker takeover
the fault injects)
...
election — the takeover pinned by the external Pulsar broker subscription/consumer-stats observer of Gate
criterion 7 against `test/golden/phase_42_failover_takeover.json`, not the operator-authored
`ExpectationWitness`), after its complete resource shape has passed pure provision and snapshot-bound live preflight and
...
injects is delegated to Pulsar (Phase 29), never a bespoke amoebius election.
...
- The gate topology `test/dhall/phase_42_failover.dhall` — the **committed reviewed** proposal (authored
  ...
  from the committed reviewed file only within the allowlist `test/dhall/phase_42_review_allowlist.json`. The
...
1. The gate topology — captured as the raw `suggest-test` emitted `.dhall` (pre-review), the reviewed `.dhall`,
   ...
   `test/dhall/phase_42_review_allowlist.json`.
...
2. The run emits a ledger whose applicable-move set is derived (from `ChaosSchedule`/`FaultTarget` + §11.1
   ...
   `test/mutants/phase_42_ledger_all_tested.dhall` (an emitter marking every applicable move tested) MUST fail
...
5. Independently make the generator/harness, controller, active, standby, replacement, selected images,
   ...
   structural-takeover-defeat `phase_42_failover_standby_wrong_subscription.dhall` (caught by the external
...
> and delivered by Phase 33's gateway-migration drills, not here. The delegated-failover shape is proven in the sibling `infernix`
...
- `documents/engineering/app_vs_deployment_doctrine.md` — §3's deployment-rules surface gains the realized
  test-topology layer (Sprint 42.1): the `TestTopology` is an ordinary deployment-rules layer over a production
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-42 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/system_components.md` — add the `Amoebius/Test/*` modules (`Topology`, `Runner`,
  `SuggestTest`, `Credentials`, `Harness`, `Sweep`, `Ledger`) to the component inventory as Phase-42
  design-first rows, mapped to the owning testing and storage-lifecycle doctrines.
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 42 "per generated test" substrate posture (each generated
...
- [phase_29](phase_29_content_store_workflow.md) — the Pulsar-`Failover` worker takeover this phase's gate injects
- [phase_33](phase_33_gateway_migration_drills.md) — the cross-cluster gateway-migration obligation, distinct
  from this phase's intra-cluster failover
- [phase_37](phase_37_provider_dynamic_nodes.md) — the leak-free provider teardown this harness extends to test cycles
