...
- [Sprints](#sprints)
- [Sprint 41.1: Vault-KV Curve25519 peer keys — secrets by name, minted and custodied in Vault ⏸️](#sprint-411-vault-kv-curve25519-peer-keys--secrets-by-name-minted-and-custodied-in-vault-)
- [Sprint 41.2: Rendered peer config + the wg reconcile — render → wg show/diff/wg set ⏸️](#sprint-412-rendered-peer-config--the-wg-reconcile--render--wg-showdiffwg-set-)
- [Sprint 41.3: Phase gate harness — live fabric + external-observer reachability over the VPN IP ⏸️](#sprint-413-phase-gate-harness--live-fabric--external-observer-reachability-over-the-vpn-ip-)
...
✅ Done. Fresh Vault-custodied Curve25519 keypairs resolved through the current-tree Haskell Kubernetes-auth
...
will not decode. Phase 41 adds the **runtime residue**: a real `wg0` interface on each node, brought up by the
...
geo-replication wire is *not* delivered here — its per-peer `render()` obligation is deferred to Phase 42; the
two spans this phase renders are the remote-worker↔home attach carrier and the fabric-bound listener boundary.
The gateway-migration hub *repoint* is Phase 43; this phase establishes the static fabric it will
later move.

**Substrate:** linux-cpu — the single-node `kind` host of Phases 24–33, whose kernel provides the WireGuard
...
fabric-render/diff work performed by the already-existing Phase-33 singleton is added to that singleton's
...
- [`network_fabric_doctrine.md §4` — Topology: the hub is the gateway role](../documents/engineering/network_fabric_doctrine.md#4-topology-the-hub-is-the-gateway-role-and-the-fabric-moves-with-it):
  ...
  phase stands up the static hub; Phase 43 later repoints it on migration.
...
## Sprint 41.1: Vault-KV Curve25519 peer keys — secrets by name, minted and custodied in Vault ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `src/Amoebius/Fabric/Keys.hs` (the Curve25519 peer-key KV secret
class: validate and resolve a peer's key by `SecretRef` name through the Phase-29 Vault client); the Dhall
...
rendered config (Sprint 41.2) stands on.
...
- The Curve25519 peer-key KV secret class: mint a keypair into Vault-KV, and resolve a peer's private/public key
  by `SecretRef` name at render time through the Phase-29 built-in Vault client (no agent sidecar).
...
### Validation
1. A minted peer keypair is readable from Vault-KV by `SecretRef` name through the Phase-29 client, and no key
...
2. A `SecretRef` naming an absent/rotated KV entry yields a specific-reason resolution error (never an empty or
   default key), so the missing-key mutant of the gate (Sprint 41.3) has a defined failure to trip on.

### Remaining Work
None for this sprint.

## Sprint 41.2: Rendered peer config + the wg reconcile — render → wg show/diff/wg set ⏸️
...
- The pure total `render(nodeInventory) -> [WireGuardPeerConfig]`: keyless peer type-foreclosed; overlapping VPN
  ...
  `SecretRef` name resolved from Vault-KV (Sprint 41.1). The gateway-role hub renders a stable hub VPN-IP +
...
## Sprint 41.3: Phase gate harness — live fabric + external-observer reachability over the VPN IP ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `test/integration/Phase41Gate.hs` plus `tools/phase41_{wireguard_live,gate}.py` (linux-cpu two-peer fabric spin-up
...
- A **Register-3** proven/tested/assumed ledger recording the live-reachability result (the spoke reached the
  ...
  **broker↔broker** geo-replication render obligation (Phase 42), the **gateway-migration hub repoint**
  (Phase 43), and the stretched kubelet↔apiserver `ControlPlanePeer` span (owned by cluster_topology). The
...
### Remaining Work
None for this sprint. The Phase-42/43 and stretched-control-plane surfaces named above remain UNVERIFIED.
...
- `documents/engineering/network_fabric_doctrine.md` — the §2–§5 "Phase 0 design intent" honesty note flips from
  ...
  gateway-migration hub repoint remain deferred (Phase 43).
...
- `DEVELOPMENT_PLAN/README.md` — add the Phase-41 row and flip its status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — add the Phase-41 linux-cpu gate row (the raw-kernel WireGuard fabric).
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Fabric/{Keys,WgRender,WgReconcile}.hs` as
  Phase-41 design-first rows against `network_fabric_doctrine.md`.
...
- [phase_29](phase_29_vault_pki.md) — the root Vault + `SecretRef`-by-name client custodying the peer keys
- [phase_33](phase_33_live_dsl_singleton.md) — the control-plane singleton whose reconcile enacts the fabric
- [phase_42](phase_42_multicluster_spawn_georepl.md) — multi-cluster spawn + geo-replication (the deferred broker↔broker per-peer render obligation)
- [phase_43](phase_43_gateway_migration_drills.md) — the gateway migration that later repoints the WireGuard hub
