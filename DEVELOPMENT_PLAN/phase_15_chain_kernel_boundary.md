# Phase 15: chain/Step kernel + `--dry-run` + boundary fake-tool harness + Gate-3 AST checker
...
Phase 15 delivers the chain/Step kernel + `--dry-run` + boundary fake-tool harness; its design is owned by [dsl_doctrine.md](../documents/engineering/dsl_doctrine.md), [conformance_harness_doctrine.md](../documents/engineering/conformance_harness_doctrine.md), [generated_artifacts_doctrine.md](../documents/engineering/generated_artifacts_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 15.1: The `Step` algebra + `chain :: cfg -> \[Step\]` builder ✅](#sprint-151-the-step-algebra--chain--cfg---step-builder-)
- [Sprint 15.2: The pure descent — `nextFrameAfter` / `foldLift` (golden-locked) ✅](#sprint-152-the-pure-descent--nextframeafter--foldlift-golden-locked-)
- [Sprint 15.3: `renderChainPlan` / `--dry-run` byte-for-byte render (no live infra) ✅](#sprint-153-renderchainplan----dry-run-byte-for-byte-render-no-live-infra-)
- [Sprint 15.4: The plan-render golden battery (`chain-spec`) — the Part-A gate ✅](#sprint-154-the-plan-render-golden-battery-chain-spec--the-part-a-gate-)
- [Sprint 15.5: The single typed subprocess seam + `boundary-spec` skeleton ✅](#sprint-155-the-single-typed-subprocess-seam--boundary-spec-skeleton-)
- [Sprint 15.6: The fake `kubectl`/`helm`/`docker`/`pulumi` recorders ✅](#sprint-156-the-fake-kubectlhelmdockerpulumi-recorders-)
- [Sprint 15.7: The boundary battery — exact commands + applied bytes + no-`PATH` — the Part-B gate ✅](#sprint-157-the-boundary-battery--exact-commands--applied-bytes--no-path--the-part-b-gate-)
- [Sprint 15.8: The sanctioned-API surface — what extension source may reach ✅](#sprint-158-the-sanctioned-api-surface--what-extension-source-may-reach-)
- [Sprint 15.9: Gate 3 — the extension AST checker and the link seal ✅](#sprint-159-gate-3--the-extension-ast-checker-and-the-link-seal-)
...
`sha256:d3d92e8c10c59625…`; Phase 15 owns no remaining migration deferral.
...
The Phase-15 ledger records the exact tested and UNVERIFIED
boundary.
...
`cabal test chain-spec` plan-render battery (Part A, analogous to the Phase-14 rendered-output goldens) plus an
...
Phase-15 ledger records the exact commands, oracle coverage,
...
flowchart LR
  ...
  s0["Sprint 15.1: The Step algebra + chain :: cfg -> [Step] builder"]
  s1["Sprint 15.2: The pure descent — nextFrameAfter / foldLift (golden-locked)"]
  s2["Sprint 15.3: renderChainPlan / --dry-run byte-for-byte render (no live infra)"]
  s3["Sprint 15.4: The plan-render golden battery (chain-spec) — the Part-A gate"]
  s4["Sprint 15.5: The single typed subprocess seam + boundary-spec skeleton"]
  s5["Sprint 15.6: The fake kubectl/helm/docker/pulumi recorders"]
  s6["Sprint 15.7: The boundary battery — exact commands + applied bytes…"]
  s7["Sprint 15.8: The sanctioned-API surface — what extension source may reach"]
  s8["Sprint 15.9: Gate 3 — the extension AST checker and the link seal"]
...
- **Cross-golden render identity (§M.3, independent oracle).** `chain` produces a pure `[Step]` value whose
  manifest-bearing steps are identity-selected subsets of the one **Phase-14** `renderAll` golden for the
  fixture's whole `ProvisionedSpec`; their disjoint identity union equals that complete object set byte-for-byte,
  and no Step invokes a per-service renderer. Both cfg fixtures are the same oracle-pinned fixtures Phase 14
...
- **Oracle-pinning — the expected-argv transcript (§M.1 named exception, §M.3 independent oracle).** The
  ...
  at the **start of Phase 15 before the executor implementation** (the §M.1 named exception), authored at
...
- **Applied-manifest bytes (§M.3, cross-golden).** The applied-manifest bytes the fakes capture equal the
  **Phase-14** `renderAll` goldens and the **Part-A** plan goldens byte-for-byte — the same rendered value the
...
- **Committed mutants (§M.2), re-run every gate run.** `cabal test boundary-spec` turns **red** on each of three
  ...
  a Phase-14 `renderAll` golden), and **mB3** (`mB3_path_resolve`, a `PATH`-resolution mutant — the seam
...
- **No-`PATH` invariant via an external-observer trace (§M.5, OS-boundary observer).** The no-`PATH` invariant is
  ...
  **absent** from `PATH` and a decoy directory holding same-named sabotage executables (each writing a distinct
  ...
  site over all of `src/`: Phase 15 initially placed it in `src/Amoebius/Exec/Tool.hs`; Phase 29 moved the raw
...
- [`conformance_harness_doctrine.md §3 — rendering never touches live infrastructure`](../documents/engineering/conformance_harness_doctrine.md#3-the-load-bearing-invariant-rendering-never-touches-live-infrastructure)
  ...
  ([phase_31_object_reconciler.md](phase_31_object_reconciler.md)), never the render or boundary path.
...
## Sprint 15.1: The `Step` algebra + `chain :: cfg -> [Step]` builder ✅
...
whole-deployment `renderAll` result; the disjoint union equals the Phase-14 golden byte-for-byte.
**Docs to update**:
...
- Each `Step`'s renderable shape embeds its identity-selected projection of the Phase-14 whole-deployment
...
2. The identity-disjoint union of all manifest-bearing Step projections equals the Phase-14 committed `renderAll`
...
## Sprint 15.2: The pure descent — `nextFrameAfter` / `foldLift` (golden-locked) ✅
...
## Sprint 15.3: `renderChainPlan` / `--dry-run` byte-for-byte render (no live infra) ✅
...
- **The argv dispatch itself**, in `app/amoebius/Main.hs` and nothing else: argv selects a *verb*, and a verb
  ...
  `FrameConfig` Phase 29 mints, never inferred from the verb, the executable's filename, or the environment
...
observed. Both plans and descent assignments match their goldens; Step projections union to the Phase-14
...
1. `cabal test chain-spec`, run with credentials scrubbed and socket calls blocked and observed, is green. For each
   ...
   identity-disjoint and union to the Phase-14 committed whole-deployment `renderAll` golden, the step set matches
...
## Sprint 15.5: The single typed subprocess seam + `boundary-spec` skeleton ✅

**Status**: Done — the capability is re-established by the migrated gate; the sprint's committed-ledger, pinned-toolchain, and repository-resident evidence mechanics are superseded
implementation behind `AbsExe` without widening the seam.
**Implementation**: `src/Amoebius/Exec/Tool.hs` (the Phase-15 boundary facade that invokes a tool by absolute
path over the `[Step]`/effect data) and, after Phase 29, `src/Amoebius/Host/Ensure.hs` (the sole raw
...
Phase-15 boundary cases continue to pass through `Exec/Tool.hs`.
...
- `src/Amoebius/Exec/Tool.hs`: the boundary facade that runs a resolved tool **by absolute path** (never a
  ...
  streams. Phase 29 preserves this contract while delegating its sole raw process call to the opaque-`AbsExe`
...
## Sprint 15.6: The fake `kubectl`/`helm`/`docker`/`pulumi` recorders ✅
...
## Sprint 15.7: The boundary battery — exact commands + applied bytes + no-`PATH` — the Part-B gate ✅

**Status**: Done — the capability is re-established by the migrated gate; the sprint's committed-ledger, pinned-toolchain, and repository-resident evidence mechanics are superseded
**Implementation**: `test/spec/boundary/BoundarySpec.hs`; the applied-manifest bytes reuse
the Phase-14 `renderAll` goldens; the **expected-argv transcripts are a separate committed hand-authored oracle** (`test/golden/chain_boundary/argv/`), NOT derived from the Part-A plan golden by any executor-reachable
...
- `test/spec/boundary/BoundarySpec.hs` asserting: the recorded argv stream equals the committed hand-authored
  expected-argv transcript; the applied-manifest bytes equal the Phase-14/Part-A goldens byte-for-byte (the same
...
1. `cabal test boundary-spec` is green — commands match the committed hand-authored argv transcript, applied
   bytes match the Phase-14/Part-A goldens exactly, the three invoked tool transcripts (`kubectl`/`docker`/`pulumi`)
   are non-empty and the `helm` transcript is empty, and invocation is by absolute path under the hostile
   decoy-`PATH` arrangement. This is the **Part-B (Register 2)** half of the phase gate; together with Sprint 15.4
   (Part A) it constitutes the two-part Phase-15 gate.
...
## Sprint 15.8: The sanctioned-API surface — what extension source may reach ✅
...
## Sprint 15.9: Gate 3 — the extension AST checker and the link seal ✅
...
- `documents/engineering/dsl_doctrine.md` — backlink §2's chain/Step kernel and its pure `renderChainPlan` to
  this in-process Phase-15 seed; keep the effectful `runChainFromFrame` live invocation as the deferred Register-3
  residue.
- `documents/engineering/conformance_harness_doctrine.md` — record that §3's rendering-never-touches-infra
  invariant and §4's Plan spine step are golden-locked in Phase 15 for the `[Step]` plan (Part A), and that §2/§4's
  fake-apply step is exercised by the in-process Phase-15 boundary harness (Part B); keep Register 3 (live apply)
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Kernel/{Step,Chain,Descent,Plan}.hs`, the
  ...
  `test/spec/boundary/` (the fakes + `Fakes.hs` + `BoundarySpec.hs`), and the `boundary-spec` test-suite as Phase-15
  design-first rows. Phase 29 separately registers `src/Amoebius/Host/Ensure.hs` as the strengthened raw
...
- [phase_12](phase_12_provision_seal.md) — the whole-deployment provision seal that constructs the opaque
  `ProvisionedSpec` the plan config carries
- [phase_14](phase_14_render_manifest_goldens.md) — the pure `renderAll` output from which a step selects its
...
- [phase_06](phase_06_gadt_decoder_gate2.md) — supplies the typed decoder consumed before planning
