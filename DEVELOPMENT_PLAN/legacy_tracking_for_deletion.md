...
typed Haskell loci. The compiled inventory also determines predecessor and Phase-50 closure; nothing in the
...
same review confirms a row deletion only after the typed closure binding reaches zero, its independently
authored Haskell negative detects reintroduction, predecessor approval is present, and the evidence is
retrievable.
...
**Observed footprint / Known partial.** The `LTD-HOST-003` row in section 4 has no compiled constructor. It is a
reader-facing declaration of work Phase 10 owns, and the typed universe must gain its twenty-sixth constructor, owner,
observation/closure binding, and reintroduction-case identity before that row can be evaluated at all. Until then this
register is deliberately ahead of the compiled inventory, and the dispatcher must refuse rather than read the row as
satisfied or as evidence of anything. `Amoebius.Validation.Legacy` now owns a closed 25-constructor ID
...
|---|---|---|---|
| `LTD-SRC-000` | The public Phase-0 dispatcher composes a descriptor-pinned Git observer, `SourceClosure`, exact v2 later-owned debt comparison, closed source-consumer roles, linked-GHC compiler analysis, and legacy routing, but the integrated candidate is RED. After the 2026-08-23 adversarial rejection, current acquisition hardening makes caller-selected Git diagnostic-only, compares exact final index-object/mode and HEAD, permanently refuses raw closure/debt checks, and kills five focused authority/race/mask mutants. No authenticated atomic external acquisition authority exists. Compiler hardening now refuses missing/non-exact Cabal plans, applies exact source roots, derives only four witnessed facts, retains eight unestablished semantic fact classes, and kills three focused plan/subject mutants. Dispatcher omission coverage remains active work. Independent per-blob SHA-256 commitments, no-follow authored-root checks, exact contained-state residue, and deny-only licence/notice handling remain supporting component work rather than validation. | **Phase 0.** Complete, compose, and independently qualify the Haskell source-snapshot classifier, semantic consumer/effect graph, descriptor-relative no-follow authored-root walker, authenticated acquisition authority, and external workspace observer, then make their complete accounting a mandatory precondition of every phase gate. Later-owned migrations remain explicit typed Haskell bindings; they cannot make Phase 0 depend on a later phase. | The typed Haskell join must account for every tracked path exactly once against the compiled legacy inventory. It rejects behavioral non-Haskell bytes outside `pb/**`, disguised source by executable mode/shebang/semantic consumer role, index concealment flags, and independently observed kind/mode/byte mismatch. Its unbound-finding, executable, shebang, content-role, compiler-graph completeness/staleness, acquisition-bypass, same-path index-object race, HEAD race, assume-unchanged, skip-worktree, ancestor-symlink, special-file, replacement-race, cleanroom-root, authored-root concealment, independent-blob-hash, and mode/byte-divergence mutants must fail. |
| `LTD-SRC-001` | `tools/**` contains 237 tracked files: 201 Python programs, four shell programs, tables, JSON, copied documentation corpora, and other gate inputs. | **Phase 48.** Haskell values generate every required tool lazily beneath `.build/tools/**`; the authored `tools/**` root is removed. | `trackedUnder "tools" snapshot == []`; a generated Python gate at `tools/reintroduced_gate.py` fails `sourceBoundary` before execution, and clean-room gate generation succeeds with `tools/**` absent. |
| `LTD-SRC-002` | The frozen SourceDhall family contains 279 paths: 278 `.dhall` files across `dhall/**`, `test/**`, `probe/**`, and the repository root, plus the behavioural `dhall/examples/locus_registry.tsv` input. | **Phase 26.** Haskell checked-IR types and Haskell-authored semantic cases generate every Dhall projection and associated serialized registry beneath `.build/dhall/**`; no Dhall/registry source is tracked. | `sourceDhallFamily snapshot == []`; generated schema, registry, legal cases, and illegal cases materialize from Haskell in a clean room, while a tracked one-file Dhall or TSV reintroduction fails. |
| `LTD-SRC-003` | `proto/Amoebius/Pulsar/Proto/PulsarApi.proto` is tracked behavioral source; the other current Proto path belongs to the frozen top-level-vendor family `LTD-SRC-009`, so no locus is double-counted. | **Phase 27.** Haskell message declarations generate the wire schema and bindings beneath `.build/proto/**`; maintained Haskell consumes only generated bindings. | `trackedBySuffix ".proto" snapshot == []`; the generated schema round-trips every message constructor and a changed field-number mutant fails the independent Haskell wire oracle. |
| `LTD-SRC-004` | `ui/**` contains 15 tracked PureScript, JavaScript, and YAML files, and root `package.json` is a second authored UI build input. | **Phase 47.** Haskell public-boundary values generate the generic browser interpreter, foreign-function shims, package description, and build description beneath `.build/ui/**`. | `trackedUnder "ui" snapshot == [] && not (tracked "package.json" snapshot)`; a clean-room browser build consumes only generated UI files, and a reintroduced `.purs`, `.js`, or package manifest fails. |
| `LTD-SRC-005` | `pulumi/child-cluster/Pulumi.yaml` is tracked serialized infrastructure-program input. | **Phase 48.** Typed Haskell provider declarations generate Pulumi program metadata beneath `.build/pulumi/**` before the complete hardware-free DSL barrier; no later source-migration exception exists. | `trackedUnder "pulumi" snapshot == []`; clean-room provider metadata generation reaches the declared engine from Haskell alone, and a tracked YAML reintroduction fails. |
| `LTD-SRC-006` | `test/**` contains 998 non-Haskell files. The 108 Dhall files belong to `LTD-SRC-002`; the other 890 include Python, shell, JSON/YAML/TSV tables, goldens, expected outputs, patches, and mutant bodies. | **Phase 48.** Tests and independent expectations are Haskell; every serialized fixture, executable fake, rendered oracle, and applied mutant is generated lazily beneath `.build/test-corpora/**`. | `all isHaskellSource (behavioralTrackedUnder "test" snapshot)` after excluding no format by filename; clean-room test generation covers every declared case and mutant, while one reintroduced table, script, golden, or patch fails. |
...
| `LTD-HOST-001` | `installAndVerify` has no production caller, `Host.Context` retains a second executable discovery helper, and host-tool requirements are duplicated across Haskell, Python, and tables. | **Phase 52.** One closed Haskell requirement algebra and one probe-first ensure interpreter serve every production host path. | A production entry point reaches `installAndVerify`; `executableResolvers production == 1`; the closed tool set is declared once; absent→installed→resolved and idempotent replay pass, while no-caller, stale-snapshot, duplicate-resolver, and constant-success mutants fail. |
| `LTD-HOST-002` | Current Haskell production modules and Haskell specs retain developer-home executable paths, ambient `/tmp` or `/var/tmp` state, and external-state fallbacks; condemned non-Haskell gates are already covered by `LTD-SRC-001`. As written, this binding's home-prefix negative would also reject the one operator-owned host claim root, which is not amoebius state escaping into the machine but host configuration amoebius appends a single record to. | **Phase 52.** Logical tools resolve through the host kernel; all run-local state is beneath `.build/**` or the exact live-test `.test_data/**` ownership root. | `ambientPathLoci haskellSource == []` after excluding exactly one declared operator-owned host claim root, by a typed predicate naming that root rather than by a path-shaped allowance; two-host resolution uses disjoint `<os>-<arch>` roots, and home-prefix, ambient-temp, unresolved-`PATH`, changed-owner, and production-overlap mutants fail before effects. A mutant that widens the exclusion past that single root, and one that reaches the root from any locus outside the declared resource-index seam, must fail at their own loci. |
| `LTD-HOST-003` | amoebius implements no part of the host claim ledger: no Haskell decodes or encodes a claim record, no admission arithmetic exists, and no charge is derived from the resource index. amoebius therefore cannot coordinate with another program sharing the machine, and two projects can each start a cluster after both observed the same free memory correctly. | **Phase 10.** Haskell owns the total record decoder and encoder, the admission arithmetic over frozen dimensions and prefix-conflicting opaque domains, and one total conversion from the resource index to a declared charge. Phase 52 then exchanges records with a real root under a supplied observed host; deriving domains or charges from probed hardware is later hardware-bearing work and is not owned here. | A conforming reader agrees with an independently written reader on occupancy and on per-dimension totals across a valid free record, a live `Persistent` claim, a stale `Transient` claim whose `.live` is unlocked, a record with a corrupted checksum, and a claim holding a domain from an unrecognised family with a `ram.bytes` charge. A decoder that classifies any unreadable record as free, a second checker substituted for the independent implementation, a charge authored twice rather than converted once, and a collapsed `Busy`/`Insufficient`/`Unsupported` refusal must each fail at their own loci. |
| `LTD-IMG-001` | Image code and fixtures still construct a joined `linux/amd64,linux/arm64` build, require emulator arguments, and carry QEMU/binfmt paths although the architecture rule requires native per-architecture artifacts. | **Phase 57.** Build and attest one architecture-qualified image on a host of that natural architecture; Phase 58 later performs the complementary host run without an index join. | `imageBuildPlatforms host request == [naturalArchitecture host]`; the emitted command has no emulator/index/multi-platform operation, the kernel's binfmt table is unchanged, and cross-architecture or manifest-list mutants fail. |
| `LTD-RUN-001` | The Cabal file now declares one executable, but tools, tests, fixtures, and runtime strings still name `amoebius-singleton` and phase-ordinal artifact paths. | **Phase 56.** One binary receives its role as a decoded Haskell value; no path, field manager, image command, or fixture invents a second executable identity. | `amoebiusExecutableStanzas == 1 && obsoleteRuntimeIdentities snapshot == []`; identity-by-executable, phase-path, and second-stanza mutants fail. |
| `LTD-SEED-001` | `cabal.project` resolves infernix from externally fetched commit `e48f4a0ad2c9fc75550da456b70289ffe18d6282`, and six `src/Infernix/**` modules retain an upstream namespace inside amoebius. Pinning removes moving-branch ambiguity but does not authenticate or eliminate the foreign dependency. | **Phase 92.** Re-derive the required behavior under `Amoebius.*`; no build input, vendored tree, module, hash table, or captured output depends on the seed. | `seedDependencies Infernix snapshot == [] && trackedUnder "src/Infernix" snapshot == []`; a clean build succeeds with no sibling/network access, and seed-fetch, upstream-namespace, copied-hash, and captured-output mutants fail. |
| `LTD-SEED-002` | `cabal.project` resolves jitML from externally fetched commit `f6f917fe3e8aaeb80885504a72cd90f164325af4`; tests name `JitML.Codegen.RuntimeOperationsCuda`, which the tracked source does not provide. Pinning removes moving-branch ambiguity but does not authenticate or eliminate the foreign dependency. | **Phase 94.** Re-derive the required CUDA-training behavior under `Amoebius.*`; no build or validation input depends on the seed or a missing upstream module. | `seedDependencies JitML snapshot == []`; a clean build and semantic suite succeed with no sibling/network access, and seed-fetch, missing-module, copied-output, and silent-CPU-fallback mutants fail. |
...
3. Phase 10 closes `LTD-HOST-003` once its typed binding exists; the narrowed `LTD-HOST-002` exclusion must be
   compiled before Phase 52 may exchange records with a real root.
4. Phase 26 and Phase 27 establish Haskell-derived schema and wire projections.
5. Phase 28 removes Markdown/TSV behavioral authority under `LTD-DOC-001`.
6. Phase 47 and Phase 48 remove the remaining authored generated programs, fixtures, tools, and mutants.
7. Phase 50 requires every source-migration Haskell binding to be zero and passes the hardware-free DSL
   promotion barrier.
8. Only then may Phase 51 validate the already-bounded `pb` handoff. Phase 52 must next validate the Haskell
   host-ensure kernel against fake boundaries with no hardware. Only after both receive their own human
   approvals may Phase 53 begin the first hardware-bearing candidate; image, cluster, and live-provider bands
