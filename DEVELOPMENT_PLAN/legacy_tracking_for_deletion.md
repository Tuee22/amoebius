...
| `src/Amoebius/Host/Ensure.hs` carries `installMechanism :: String`, `installAndVerify` has no caller, and `HostTool` has five constructors and no `Docker` arm | the ensure algebra is typed data with a probe-first driver | Phase 4. Closure: an install step is a resolved tool plus arguments and the driver is exercised |
| `src/Amoebius/Cluster/Bootstrap.hs` refuses `apple` and `windows` outright | every catalog member reaches a frame | Phase 4. Closure: no substrate is refused at the bootstrap entry |
| `pb/` is an `argparse` CLI under a setuptools `pyproject.toml` with no configured type checker, formatter, linter, or test runner | a Poetry distribution installed with `pipx`, checked under `mypy --strict` with no explicit `Any` | Phase 3. Closure: the quality gate runs and passes |
