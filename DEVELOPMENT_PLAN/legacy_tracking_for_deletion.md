...
| ID explained for readers | Current observation | Owner/replacement explanation for readers | Closure behavior explained for readers |
|---|---|---|---|
| `LTD-VAL-001` | All 96 phase contracts name the future `pb validate phase NN` spelling. The unstaged one-file bootstrap candidate forwards all arguments to a source-built Haskell binary rather than parsing public verbs, but Phase 0 still has only a refusal-bearing Haskell dispatcher and the Git index retains the condemned old Python runner family. No bootstrap execution is admissible before Phase 50; the universal Haskell qualification protocol remains active work and NOT VALIDATED. | **Phase 0.** Complete and independently qualify the universal Haskell-owned subject/oracle/negative/evidence protocol and statically admit the bounded opaque handoff source. No phase command outside that future handoff may name Python or an authored generated tool. | `allPhaseGatesHaskell contracts` and `verdictAuthorityGraph contracts == Right haskellOnly`; replacing a subject with no-op, replacing a checker with constant success, passing a forged exit code, or omitting discovery makes gate qualification fail. |
