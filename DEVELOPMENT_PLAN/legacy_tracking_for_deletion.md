...
🔄 **Active through Phase 52.** The repository contains implementation and generated migration material. The
...
| 84 phase contracts still carrying their pre-rebaseline sprint bodies | each phase | sprint bodies rewritten with an amended gate, as that phase opens. Phases 8–19 are now rewritten around the standalone indices, their composition boundary, the formal kernel, the bounded explicit-state/symbolic/refinement checkers, the structured compile-fail harness, the deterministic simulation substrate, the concrete gateway-migration proof boundary, the bounded DSL/protocol model set, and the pure reconcile-core simulation. The [§D](development_plan_standards.md#d-the-per-phase-document-skeleton) surface is already re-authored and checked: every contract carries a Phase scope that says something its Purpose does not (`d5`), a [§M](development_plan_gate_integrity.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) clause-13 discharge or an explicit not-applicable (`d6`), no surviving completion marker (`d7`), and a dependency edge naming what it actually consumes |
| 19 gates whose acceptance still rests on a byte golden of generated output — phases 32, 48, 55, 58, 59, 60, 61, 62, 63, 64, 67, 69, 72, 73, 78, 80, 91, 92, 94 | each phase | the golden is replaced by a semantic oracle asserting the property it stood in for; the address already detects a byte change, so the comparison is redundant as well as weak. Phase 11 retired its two `ToyModel` output snapshots in favour of renderer facts and an invariant truth table. Phase 16 compares only two fresh same-seed traces, with changed-seed sensitivity and semantic schedule/calculus oracles. Phase 17 replaces generated TLA+/CFG snapshots with twelve semantic facts and two renderer mutants. Phase 19 likewise compares fresh same-seed encodings while authored verdict, final-inventory, count, and event rows decide meaning; neither simulation phase commits generated trace bytes. Phase 30 replaces 18 rendered `BoundServiceSpec` snapshots with nine authored rows whose two-shape product/object/controller/execution/intent projections decide meaning. Phase 34 replaces its plan/descent output snapshots with two consumed authored cases and nineteen ordered semantic Plan rows, retaining generated plan bytes only for per-run canonical round-trip checks. Phase 35 replaces the planned renderer-output golden with twenty-two authored recipe semantics and retains the Dockerfile only beneath `.build/**`. Phase 37 replaces the normalized-wire byte snapshot with three authored tenant/module/node/link semantic rows |
...
Each row's owner is the phase whose gate must clear it, never a later one. Phase 49 corrected one category
error in the earlier inventory: an independently authored gate predicate or mutation seed is an oracle, not a
generated result. Phase 47's byte-identical materializations are fresh equivalence challenges beneath
`.build/**`; deleting their authored side would remove the independence that makes self-reference admissible.
...
| `ui/**` emitted contracts, codecs and bundle | Phase 46 | **closed 2026-08-22:** contracts and codecs render from the checked public boundary and the generic ABI bundle compiles only beneath `.build/ui/**`; no generated PureScript or bundle is committed |
...
| ~~`dhall/amoebius/BakeCatalog.dhall` carries `baseDigest` and the rendered golden pins `ubuntu:24.04@sha256:…`~~ | no authored digest in any recipe or catalog; the parent is a channel resolved per run | **Closed by Phase 35 on 2026-08-21.** The catalog field and renderer-output golden are absent; `BaseChannel` rejects digest syntax, and the generated recipe has one dynamic base reference |
...
- **The DSL lacked an amoebius-owned bounded proof/simulation seam before live.** TLA+ proved a throwaway
  model and `IOSim` a toy reconcile loop; neither instrument was pointed at the DSL. Phase 18 now supplies the
  bounded DSL/protocol model and actual-code projection tranche. Phase 19 now supplies the pure reconcile core,
  four bounded modeled-store schedules, and token/reservation code readings; live runtime correspondence remains
  later work.
...
| **infernix single-arch (amd64-only)** image publication | sibling infernix image-build pipeline | Replaced by one natively built image per architecture under an architecture-qualified tag, with no manifest-list join; see [`image_build_doctrine.md` §3 — one image per architecture](../documents/engineering/image_build_doctrine.md#3-one-image-per-architecture--the-tag-carries-the-architecture-not-an-index) | [Phase 56](phase_56_base_image_registry.md) and [Phase 57](phase_57_complementary_arch_child.md) | Historical pre-amendment publication exists; the current native per-tag gates remain open |
