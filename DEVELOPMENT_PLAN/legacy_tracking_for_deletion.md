...
| 11 catalogue cells are *unknown* rather than empty, and the entry tags that would settle them | Phase 0 | each entry pairs its foreclosure layer to the locus that catches it, instead of listing both in prose the grid must guess at; `tools/covering_grid.py` then decides the eleven without inventing a reason for any of them, and its over-crediting note retires with the same change |
