...
- **Why two phases on the transport rows.** The native `amoebius-pulsar` client lands in
  [Phase 5](phase_05_pulsar_content_store_workflow.md), but the sibling transports are only *deleted* when
  each library is migrated onto it — infernix's WebSocket gateway at [Phase 6](phase_06_determinism_infernix.md)
  and jitML's Node-subprocess client at [Phase 7](phase_07_jitml_ha_coordinator.md), one subsystem at a time
...
- **The substrate-equivalence row is a standing prohibition, not a one-time deletion.** "Per-substrate
  ...
  enforcing build-time check is itself a [Phase 3](phase_03_platform_services_storage_vault.md) deliverable,
...
- [phase_03_platform_services_storage_vault.md](phase_03_platform_services_storage_vault.md) — owns the no-Helm platform render, the baked operators, `distribution`, multi-arch, and the substrate-equivalence check
- [phase_05_pulsar_content_store_workflow.md](phase_05_pulsar_content_store_workflow.md) — owns the native `amoebius-pulsar` client that retires the WebSocket transports
