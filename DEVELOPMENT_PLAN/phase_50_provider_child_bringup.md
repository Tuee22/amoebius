# Phase 50: Hostless provider child + convergence + Lease handoff

> **Purpose**: Bring a provider-managed EKS child — deployed and checkpoint-observed by
> [Phase 49](phase_49_provider_deploy_checkpoint.md) — to the same fungible shape as any self-managed amoebius
...
Phase 50 delivers the hostless provider child + convergence + Lease handoff; its design is owned by [cluster_lifecycle_doctrine.md](../documents/engineering/cluster_lifecycle_doctrine.md), [daemon_topology_doctrine.md](../documents/engineering/daemon_topology_doctrine.md), [image_build_doctrine.md](../documents/engineering/image_build_doctrine.md), and the plan for reaching it is owned here.
...
UNVERIFIED because Phase 49 could not authenticate to AWS.
...
- [Sprints](#sprints)
- [Sprint 50.1: Two-stage capacity bootstrap for a hostless provider child ⏸️](#sprint-501-two-stage-capacity-bootstrap-for-a-hostless-provider-child-)
- [Sprint 50.2: Parent→child Lease handoff + hostless daemon topology ⏸️](#sprint-502-parentchild-lease-handoff--hostless-daemon-topology-)
- [Sprint 50.3: Standard-HA platform-service convergence from typed manifests ⏸️](#sprint-503-standard-ha-platform-service-convergence-from-typed-manifests-)
- [Sprint 50.4: The provider-child bring-up gate ⏸️](#sprint-504-the-provider-child-bring-up-gate-)
...
⏸️ Blocked pending Phase-49 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
[Phase 49](phase_49_provider_deploy_checkpoint.md) gate (the provider-cluster Pulumi deploy-from-inside + Vault-Transit-enveloped MinIO checkpoint + `observeProviderAccount`, which lands a ready `Managed Eks` control plane and its base managed node group with the pinned amoebius base/scheduler OCI content already imported and capacity-debited into the first node's CRI store). It runs on the **linux-cpu → provider** substrate in
**Register 3** (live infrastructure): the parent amoebius cluster is a single-node `kind` cluster on linux-cpu
(the Phase 29 bootstrap coordinator), from inside which the Deployment-`replicas=1` singleton (Phase 38) drove the Phase-49
...
convergence** — never the Pulumi deploy/checkpoint that produced its input ([Phase 49](phase_49_provider_deploy_checkpoint.md)),
never the per-PV durable EBS + create-vs-delete credential model ([Phase 51](phase_51_provider_ebs_credential.md)),
and never dynamic node provisioning or the leak-free teardown sweep ([Phase 52](phase_52_provider_dynamic_nodes.md)).
...
First, **the two-stage capacity bootstrap for a hostless child**. Once the EKS API and Phase-49 base node are
...
and capacity-debited into the base node's CRI store by Phase 49, never the not-yet-ready child registry or a
...
Envoy/Gateway API, Keycloak, cloud LoadBalancer — through the Phase-31 reconciler, **not** a thinner or different
...
flowchart LR
%% register: algebra
  p34["Phase 49: ready Managed Eks control plane + base node, scheduler OCI preloaded"]:::intent --> boot[/"Parent bootstrap Lease holder: create amoebius-capacity-scheduler pods=1"/]:::effect
...
Phase-49 deploy and now drives this convergence; `→ provider` (EKS) is the deploy target class, not a hardware
...
Phase-49-deployed `Managed Eks` child. Exit 0 and a self-reported empty diff cannot satisfy it.
...
[Phase 49](phase_49_provider_deploy_checkpoint.md); the per-PV EBS / credential apparatus
...
`mut-46.1-drop-copy-executor` mutants) stays in [Phase 51](phase_51_provider_ebs_credential.md); the dynamic-node
...
[Phase 52](phase_52_provider_dynamic_nodes.md). This phase inherits only the convergence/handoff slice below.
...
- **Representative set (§M.7)** — the committed topology `test/dhall/phase_46_provider_provision.dhall`, restricted
  ...
  base managed node group (size 1, deployed by Phase 49), the child `amoebius-capacity-scheduler` (`pods=1`), the
...
handoff, and the no-op re-run against a Phase-49-deployed base node group; it does **not** provision a dynamic
extra node ([Phase 52](phase_52_provider_dynamic_nodes.md)), bind a per-PV durable EBS
([Phase 51](phase_51_provider_ebs_credential.md)), or assert the independent leak-free tag-sweep at teardown
([Phase 52](phase_52_provider_dynamic_nodes.md)) — the run tears the base provider stack down through Phase 49's
own deploy lifecycle, and the leak-free sweep witness is recorded **deferred to Phase 52**, never as proven here.
...
- [`cluster_lifecycle_doctrine.md §1`](../documents/engineering/cluster_lifecycle_doctrine.md#1-two-cluster-kinds-one-lifecycle-shape)
  ...
  mandatory capacity-scheduler role — as the child-side of Phase 49's cloud-keyed amoebic spawn, converging the
...
- [`image_build_doctrine.md §2`](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster)
  ...
  the exact OCI digest Phase 49 preloaded into the base node's CRI store, and every standard platform service is
...
## Sprint 50.1: Two-stage capacity bootstrap for a hostless provider child ⏸️
...
Phase-32 `Amoebius.Scheduler.*` role and its two-stage bootstrap-cutover / execution-identity-admission
...
- Creation of `amoebius-capacity-scheduler` with exact `pods=1`, whose Deployment references the **exact OCI digest preloaded and capacity-debited into the base node's CRI store by Phase 49** — never the not-yet-ready child
...
- The managed-authority mint: install the managed-node taint, execution-identity admission, and full exclusive
  ...
  the cutover.
- An in-file honesty note: this stages a *provider child* through the same two-stage cutover Phase 32 proved on a
  self-managed cluster; the provider realization is validated here for the first time.

### Validation
1. Against a Phase-49-deployed child, the parent bootstrap holder creates `amoebius-capacity-scheduler`
...
### Remaining Work
Run the built protocol against a Phase-49-created EKS control plane and managed node, then read back the real
provider add-on and CRI-preload boundaries. The local and retained-Kubernetes protocol seams are complete.

## Sprint 50.2: Parent→child Lease handoff + hostless daemon topology ⏸️
...
## Sprint 50.3: Standard-HA platform-service convergence from typed manifests ⏸️
...
standard-service convergence), converging through the Phase-31 reconciler and consuming the Phase 30/33–37
...
Phase-31 reconciler — reachable, HA, wild ingress only via Keycloak — **not** a thinner or different service
...
typed manifests via the Phase-31 reconciler, with no Helm and no public-registry pull, so substrate equivalence
...
- Convergence of the **complete** standard HA platform-service stack — registry (`distribution`), MinIO, Vault,
  ...
  LoadBalancer — through the Phase-31 reconciler from typed manifests, using the same HA-capable topology at
...
- An in-file honesty note: the standard set and its HA-capable shape are planned for self-managed clusters in
  Phases 30/33–37; convergence of that exact set on a hostless provider child is tested here for the first time.
...
## Sprint 50.4: The provider-child bring-up gate ⏸️
...
`tools/phase45_{provider_child_live,gate}.py`, with its generated ledger under `.build/runs/phase_50/`
(observed footprint; provider OS/cloud observers UNVERIFIED)
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: one end-to-end run takes a Phase-49-deployed `Managed Eks` child from bootstrap
...
per-run ledger; the leak-free sweep deferred to Phase 52), `DEVELOPMENT_PLAN/README.md`.
...
- A per-run proven/tested/assumed ledger recording: provider-child bootstrap + handoff + standard-HA convergence
  ...
  OS-boundary audit; and the elevated-harness leak-free durable-resource *sweep* as **explicitly deferred to Phase 52, not asserted here** — skipping an applicable observation move marks that layer UNVERIFIED, never
  green.
...
4. Assert the run emits a proven/tested/assumed ledger recording the independent Lease-handoff sequence, the
   ...
   Phase 49's deploy lifecycle and the leak-free tag-sweep is recorded **deferred to Phase 52**, never asserted
   green here.

### Remaining Work
Re-run without `PARTIAL_EXTERNAL_AUTHORITY` once valid AWS authority permits Phase 49 to materialize the EKS
child. Eleven enumerated provider/cloud surfaces remain explicitly UNVERIFIED; Phase 52 still owns the final
...
- `documents/engineering/testing_doctrine.md` — record the Phase 50 per-run ledger artifact and the explicit
  deferral of the elevated leak-free durable-resource sweep to Phase 51.

**Cross-references to add:**
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-runtime` provider-child singleton wiring
  (`Amoebius.Daemon.InClusterSingleton`) and `Amoebius.Cluster.ProviderBringUp` as Phase-50 design-first rows,
  each mapped to its owning doctrine; map the reused `Amoebius.Scheduler.*` role to its Phase 32 delivery and the
  reconciler to Phase 31.
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 50 → `linux-cpu` (parent) row with the `provider` (EKS)
  deploy target annotated as a target class, not a fifth hardware substrate.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase 50 row's status once the gate passes; link this document.

## Related Documents
- [README.md](README.md) — the live tracker; Phase 50 objective, gate, and substrate
...
- [phase_49](phase_49_provider_deploy_checkpoint.md) — the provider-cluster Pulumi deploy-from-inside +
...
- [phase_38](phase_38_live_dsl_singleton.md) — supplies the singleton role and Lease authority protocol used by
  the provider child
- [phase_51](phase_51_provider_ebs_credential.md) — the per-PV durable EBS + create-vs-delete credential + static
  EBS CSI arm, layered on Phase 49, not exercised here
- [phase_52](phase_52_provider_dynamic_nodes.md) — dynamic node provisioning by signal and the leak-free provider
  gate (the independent teardown tag-sweep), layered on Phases 49/50/51
