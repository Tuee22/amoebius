# Phase 24: Platform services-2 (Percona/Patroni + pgAdmin + observability + readiness-DAG)

**Status**: Authoritative source
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_21_retained_storage.md, DEVELOPMENT_PLAN/phase_23_platform_backbone.md, DEVELOPMENT_PLAN/system_components.md
...
statement is design intent, never a tested amoebius result. This phase opens after the Phase 23 backbone gate
...
Phase-19 typed renderer + SSA reconciler, the Phase-21 no-provisioner retained storage, the Phase-22 unsealed
root Vault, and the Phase-23 MetalLB/MinIO/Pulsar backbone. The Percona/Patroni, pgAdmin, and
...
This phase completes the standard platform-service set on top of the Phase-23 backbone. It renders and
...
`postgres` Pod), each rendered as typed Kubernetes objects by the Phase-19 `renderAll` path (no Helm, no
third-party charts, manifests generated from Haskell and never committed), each served from binaries **baked
into the multi-arch base image** with no public-registry pull, and each on the Phase-21 `no-provisioner`
...
The phase then assembles the *whole* standard stack — the Phase-23 backbone plus these services — as one
...
order by the Phase-19 reconciler's **event-driven wait-for-ready**: a dependent starts on its dependency's
...
browser surface Grafana and pgAdmin reach a user through — is [Phase 25](phase_25_keycloak_ingress.md); this
...
apple, linux-cuda, or windows substrate is touched in Phase 24.
...
**Gate:** on the Phase-23 backbone cluster the remaining standard services — the Percona operator, the
...
named as gate oracles in the Sprint 24.1–20.3 Deliverables:
...
- **Render byte-identity (§M.3 independent provenance).** At gate time the pure `renderAll` is re-run in-process
  ...
  `renderAll` output — pinning applied manifests to the Phase-19 renderer, so hand-written or `helm
...
operator, the per-consumer Patroni Postgres clusters with pgAdmin for the named consumer set of Sprint 24.1,
and Prometheus + Grafana — no more, no fewer. The full derived DAG spans these plus the Phase-23 backbone
(MetalLB, MinIO, Pulsar) and the Phase-22 Vault.
...
- [`platform_services_doctrine.md §7 — Prometheus / Grafana, observability is not an add-on`](../documents/engineering/platform_services_doctrine.md#7-prometheus--grafana--observability-is-not-an-add-on)
  ...
  through the generated admission proxy; the browser surfaces gated behind the (Phase 25) Keycloak edge
...
- [`manifest_generation_doctrine.md §5 — the apply/reconcile engine: server-side apply, owned field manager, prune, wait`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  Phase 24 reuses the Phase-19 pure `renderAll :: ProvisionedSpec -> [K8sObject]` and typed-action reconciler
...
- [`image_build_doctrine.md §2 — the single distribution rule`](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster):
  every service binary (Percona operator, Patroni, pgAdmin, Prometheus, Grafana) is baked into the Phase-18
...
## Sprint 24.1: Percona/Patroni Postgres per consumer + pgAdmin + Prometheus/Grafana 📋
...
- Prometheus + Grafana scraping platform workloads, with the per-workflow recording/alert rules and dashboards
  **derived, never hand-authored**. A mandatory finite `MonitoringWorkBudget` bounds workflow/rule/series
  ...
  only through the (Phase 25) Keycloak edge under a mandatory `AccessScope` with no `Public` arm — deferred
...
## Sprint 24.2: The full derived readiness-DAG bring-up + the standard-stack gate 📋
...
- Live enactment by the Phase-19 reconciler's wait-for-ready — each dependent constructed to start on its
  dependency's observed-ready edge (a `runtime-checked` observation from the live object), never a duration;
  the singleton that will later own this loop is [Phase 26](phase_26_live_dsl_singleton.md) (Deployment
  `replicas=1`, no election).
- The phase-gate harness: bring the whole standard stack up on the Phase-23 backbone cluster and assert the
...
- `documents/engineering/monitoring_doctrine.md` — the §3 derived rules/dashboards are recorded as stood up
  (browser access still behind the Phase-25 edge, marked UNVERIFIED here).
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-24 status when the gate passes and link this document.
...
- [phase_22](phase_22_vault_pki.md) — the root Vault/PKI whose unseal edge gates secret-dependent startup here
- [phase_23](phase_23_platform_backbone.md) — the MetalLB/MinIO/Pulsar backbone this phase's services and DAG build on
- [phase_25](phase_25_keycloak_ingress.md) — the Keycloak-owned ingress edge that fronts Grafana and pgAdmin next
