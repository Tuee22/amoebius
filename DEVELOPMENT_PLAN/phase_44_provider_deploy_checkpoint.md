# Phase 44: Provider Pulumi deploy-from-inside + enveloped checkpoint
...
This is the **first arm** of the provider-managed-cluster split (Phases 44–47): the mechanism by which
...
linux-cpu parent, plus its gate; it is the substrate on which [phase_45_provider_child_bringup.md](phase_45_provider_child_bringup.md)
(the stateless hostless in-cluster control plane), [phase_46_provider_ebs_credential.md](phase_46_provider_ebs_credential.md)
(per-PV EBS + the create-vs-delete credential model), and [phase_47_provider_dynamic_nodes.md](phase_47_provider_dynamic_nodes.md)
...
an already-running amoebius cluster, issued by the Deployment-`replicas=1` control-plane singleton (Phase 33) —
whose single-instance is a k8s/etcd property, never a bespoke amoebius election — generalizing Phase 42's
...
or standard-service convergence for a provider child ([phase_45_provider_child_bringup.md](phase_45_provider_child_bringup.md));
per-PV durable EBS, the `protect`/`Retain` state class, the create-vs-delete credential split, or the static
`ebs.csi.aws.com` CSI path ([phase_46_provider_ebs_credential.md](phase_46_provider_ebs_credential.md)); or
dynamic node provisioning by signal, the second reconcile no-op, and the independent leak-free teardown sweep
([phase_47_provider_dynamic_nodes.md](phase_47_provider_dynamic_nodes.md)). The base node group here is a fixed
...
> **Provider corpus split (by design).** Phases 44–47 (the provider split) share one provider **topology
...
> sub-phase (34 → `mut-44.1`; 35 → `mut-45.1`; 36 → `mut-46.1`; 37 → `mut-47.1`/`mut-47.2`). A sub-phase
...
[phase_47_provider_dynamic_nodes.md](phase_47_provider_dynamic_nodes.md)'s; the per-PV durable EBS +
...
- [`pulumi_iac_doctrine.md §1`](../documents/engineering/pulumi_iac_doctrine.md#1-pulumi-runs-only-from-inside-an-existing-amoebius-cluster)
  ...
  [phase_46_provider_ebs_credential.md](phase_46_provider_ebs_credential.md), not here.)
- [`cluster_lifecycle_doctrine.md §3`](../documents/engineering/cluster_lifecycle_doctrine.md#3-amoebic-spawning--the-recursive-forest)
  — *amoebic spawning — the recursive forest*: provider-cluster spawn is the *cloud-keyed* sibling of Phase 42's
...
Vault-Transit-enveloped MinIO backend (`.../Backend/EncryptedMinio.hs`) **first delivered by Phase 42 and reused
...
**Blocked by**: Phase 42 gate (amoebic spawning via Pulumi with the encrypted-MinIO backend + per-child
Vault-envelope, the SSH-keyed spawn this generalizes); Phase 33 gate (the Deployment-`replicas=1` singleton
live-deploy path that runs the engine); Phase 30 gate (MinIO reachable as a standard HA platform service);
Phase 29 gate (root Vault + the Transit envelope) — all external earlier-phase prerequisites.
...
object set, generalizing Phase 42's SSH-keyed self-managed spawn to a cloud-keyed provider spawn. The `pulumi`
...
- An `Amoebius.Pulumi.Provider.Eks` program that provisions the EKS control plane + a base managed node group
  ...
  the cluster ready for its in-cluster control-plane bootstrap ([phase_45_provider_child_bringup.md](phase_45_provider_child_bringup.md)).
...
- `planInfrastructure` derives the provider-cluster demand from the exact `BoundDeployment` and declared
  ...
  [phase_46_provider_ebs_credential.md](phase_46_provider_ebs_credential.md).) Only the receipt-bound provider
...
> `mut-47.2-skip-sweep`) is [phase_47_provider_dynamic_nodes.md](phase_47_provider_dynamic_nodes.md)'s gate,
...
- `documents/engineering/cluster_lifecycle_doctrine.md` — record that §3 (the cloud-keyed amoebic spawn) gains
  an amoebius EKS reference as the cloud-keyed sibling of the Phase 42 SSH-keyed spawn.
...
- `documents/engineering/testing_doctrine.md` — record the Phase 44 per-run ledger artifact for the
...
- `DEVELOPMENT_PLAN/system_components.md` — map the reused `amoebius-pulumi` Engine and EncryptedMinio backend
  to their first delivery in Phase 42; register `Amoebius.Pulumi.Provider.Eks`, the `observeProviderAccount`
  boundary, and the `PulumiExecutionDemand` / `PulumiCheckpointObjectDemand` types as Phase-44 design-first rows,
  each mapped to its owning doctrine.
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 44 → `linux-cpu` (parent) row with the `provider` (EKS)
  deploy target annotated as a target class, not a fifth hardware substrate.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase 44 row's status once the gate passes; link this document.
...
- [phase_42](phase_42_multicluster_spawn_georepl.md) — the SSH-keyed amoebic spawn, encrypted MinIO backend, and
  per-child envelope this phase generalizes to a cloud-keyed provider spawn
- [phase_45](phase_45_provider_child_bringup.md) — the stateless hostless in-cluster singleton + capacity-scheduler
  bring-up that layers on this deploy
- [phase_46](phase_46_provider_ebs_credential.md) — the per-PV EBS + create-vs-delete credential model + static
  CSI path that layers on this deploy
- [phase_47](phase_47_provider_dynamic_nodes.md) — dynamic node provisioning by signal and the independent
