...
- [Sprints](#sprints)
- [Sprint 9.1: The `Topology` relation — `ComputeEngine` / `LinuxHost` witness / elementwise compatibility fold 📋](#sprint-91-the-topology-relation--computeengine--linuxhost-witness--elementwise-compatibility-fold-)
- [Sprint 9.2: The base capacity fold — `fits` / `podFits` / `carve` / `place` 📋](#sprint-92-the-base-capacity-fold--fits--podfits--carve--place-)
- [Sprint 9.3: QuickCheck properties — soundness, totality, elementwise compatibility + the independent witness validator 📋](#sprint-93-quickcheck-properties--soundness-totality-elementwise-compatibility--the-independent-witness-validator-)
- [Sprint 9.4: The base capacity/topology fold-negative corpus + the gate 📋](#sprint-94-the-base-capacitytopology-fold-negative-corpus--the-gate-)
...
⏸️ Blocked pending Phase-8 revalidation. Reopened 2026-08-19 by the generative re-baseline: the artifact, budget, lift, workflow and evidence calculi change what this phase's gate must cover, so any earlier seal is history and no longer presents completion evidence.
...
bound to source snapshot `sha256:6eecf9051956a260…`; Phase 9 owns no remaining migration deferral.
...
fold, compile-fail, dhall-typecheck, compatibility, and mutant oracles carry their authored shapes, the three dhall-typecheck
...
move into `.build/runs/phase_54/<run-id>/`, 25 surfaces join to 25 run-time enumerated items, and the run
...
resolves it per run, failing closed when unset — a PATH fallback would have defeated the Phase-26 absolute-argv
...
[phase_28_storage_geometry_folds.md](phase_28_storage_geometry_folds.md) owns the logical→physical storage
...
[phase_29_execution_accelerator_folds.md](phase_29_execution_accelerator_folds.md) owns the execution-epoch
...
representation, and the two-ceiling Pulsar fold ([phase_28_storage_geometry_folds.md](phase_28_storage_geometry_folds.md));
...
full-resource-vector place-witness gate ([phase_29_execution_accelerator_folds.md](phase_29_execution_accelerator_folds.md)).
...
([phase_30_capability_bind.md](phase_30_capability_bind.md)); and the whole-deployment provision seal
([phase_31_provision_seal.md](phase_31_provision_seal.md)) that re-exercises these same base folds after full
bind/expansion. Phase 9 does not move the folds into `Dhall.inputFile`.

**Phase scope:** one cohesive claim — *a demand that does not fit its capacity has no placement witness*. Everything here is arithmetic over hand-authored fixtures, deliberately before any host exists to measure.

**Substrate:** none — no host, no cluster; the gate is an in-process `cabal test` fold + QuickCheck battery,
analogous to the Phase 26 decode battery and the Phase 27 property suite.

**Lane:** none ([§L](development_plan_standards.md#l-one-substrate-discipline))

**Register:** 1 — pure/golden, in-process, no cluster ([§K](development_plan_standards.md#k-honesty-proven--tested--assumed)).

**Depends on:** [Phase 3](phase_03_artifact_calculus.md) — the artifact calculus, whose recipes this fold sizes. The scope index at the numeric predecessor is the other of the two indices and is not consumed here: a capacity fold has no tenant in it.

**Gate:** `python3 tools/capacity_topology_gate.py` is green on the base slice: the capacity fold and topology relation are
...
The suite proves the folds total; the gate additionally re-derives the placement witness against the authored capacity table and requires every seeded fold mutant to redden its own locus.
...
clauses require for Phase 9; it strengthens, never weakens, the Gate and sprint Validations above. The source
...
storage-geometry fixtures are exercised by [phase_28_storage_geometry_folds.md](phase_28_storage_geometry_folds.md)
and the execution/accelerator/provider-root fixtures by
[phase_29_execution_accelerator_folds.md](phase_29_execution_accelerator_folds.md); the full corpus is not
re-checked here.
...
[phase_31_provision_seal.md](phase_31_provision_seal.md) re-exercises these same folds through its post-bind
...
flowchart LR
  ...
  g{{"the phase 9 gate command"}}:::gate
...
```
*Implemented Phase 9 gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
variant is owned by [phase_29_execution_accelerator_folds.md](phase_29_execution_accelerator_folds.md). The
...
`legal_tmpfs_two_concurrent_writers_single_debit` are routed to phases 28 and 29 and are **not** in this gate's
...
declared `owner_phase` is `Phase-9`; every row has evidence at its registered dhall-typecheck, GADT-index, or
...
undercounted). The per-axis and per-eligibility validator mutants of Sprint 9.3 (drop CPU, memory, ephemeral
...
provider-root mutants of the source battery are the property of phases 28 and 29 and are not named here.
...
Defined in Sprint 9.3 Deliverables; it never calls `podFits` or
...
- [`jit_budget_doctrine.md`](../documents/engineering/jit_budget_doctrine.md) — the bytes capacity core fold + topology relation causes to exist are charged to a grant that carries its ceiling and concurrency together.
- [`resource_capacity_doctrine.md §4`](../documents/engineering/resource_capacity_doctrine.md#4-the-total-fold-fits-carve-place-and-the-nesting)
  ...
  adopted by [phase_28_storage_geometry_folds.md](phase_28_storage_geometry_folds.md); the accelerator and
  provider-root arithmetic by [phase_29_execution_accelerator_folds.md](phase_29_execution_accelerator_folds.md).
...
flowchart LR
  ...
  s1["Sprint 9.1: the Topology relation and its witnesses"]
  s2["Sprint 9.2: the base capacity fold"]
  s3["Sprint 9.3: QuickCheck properties and the independent validator"]
  s4["Sprint 9.4: the negative corpus and the gate"]
  g{{"the phase 9 gate"}}
...
## Sprint 9.1: The `Topology` relation — `ComputeEngine` / `LinuxHost` witness / elementwise compatibility fold 📋
**Status**: Planned
...
The edges this sprint carried to the decoder and to the illegal-state covering are **reversed** by the
generative re-baseline: the resource index is pure Haskell, and the Dhall schema and its decoder are
generated from it rather than consumed by it.
...
each check.
**Docs to update**: `documents/engineering/cluster_topology_doctrine.md` (Phase-9 status backlink),
...
- The derived closed `NodeSupply = Fixed (NonEmpty Node) | Elastic { floor : [Node],
  ...
  **templates**, the fresh instance-scoped symbolic-id qualification, and the root-EBS/instance-store arithmetic
  are the deliverable of [phase_29_execution_accelerator_folds.md](phase_29_execution_accelerator_folds.md);
  Phase 9 consumes only the CPU/memory/slot capacity of each class.)
...
- The `mkRke2` distinctness fold over `servers ∪ agentFloor` rejecting a duplicate `HostId` — the checked
  ...
  "more nodes than hosts" are already type-foreclosed upstream (Phase 25/12).
...
2. The `bareAppleHost` / `bareWindowsHost` / even-server-quorum constructors have no inhabitant, proven the
   Phase-27 way: seven committed `ghc -fno-code` expect-fail compile goldens ([Gate integrity](#gate-integrity),
...
## Sprint 9.2: The base capacity fold — `fits` / `podFits` / `carve` / `place` 📋
**Status**: Planned
...
`Amoebius/Capacity/*.hs` modules, are added by phases 28 and 29.
**Blocked by**: Sprint 9.1 (the `Topology` `place` folds over). The decoder edge is **reversed**: the decoder is generated from these types.
...
**Docs to update**:
`documents/engineering/resource_capacity_doctrine.md` (Phase-9 status backlink for §3/§4),
...
- `Quantity` (refined non-zero, unit-tagged) for declarations plus `Residual = Zero | Remaining Quantity` /
  ...
  that [Phase 26](phase_26_gadt_decode_ir.md) refines. The pad enters the effective demand once, after the
  ...
  by phases 28 and 29; the base `PodLocalStorageDemand` is the disk-backed `emptyDir` + writable-rootfs/log
...
- An in-file honesty note: every capacity **sum** here is a total checked provision-seal operation,
  ...
  `Growable` growth re-run ([phase_28_storage_geometry_folds.md](phase_28_storage_geometry_folds.md)) never
  silently invalidates the base placement. Accelerator device/VRAM fit, execution-epoch peaks,
  durable/object-store geometry, and provider-root arithmetic are appended to this same fold by phases 15 and
...
## Sprint 9.3: QuickCheck properties — soundness, totality, elementwise compatibility + the independent witness validator 📋
**Status**: Planned
...
witness validator), `test/spec/dsl/CapacityTopologyMutants.hs`, and the `capacity_topology` rows of `test/mutant/registry.tsv`, reusing
the Phase-27 property harness. (The
runtime-metadata and provider-root property modules are the deliverable of phases 28 and 29.)
...
## Sprint 9.4: The base capacity/topology fold-negative corpus + the gate 📋
**Status**: Planned
...
`test/oracle/capacity_topology/fold_cases.tsv`, `test/oracle/capacity_topology/dhall_typecheck_cases.tsv`, and
`test/oracle/capacity_topology/compile_fail.tsv`.
**Blocked by**: Sprint 9.1, Sprint 9.2, Sprint 9.3. The corpus edge is **reversed**: the positive corpus is rendered from these types.
...
- The base fold-negative fixtures: `illegal_engine_substrate_mismatch` ([§3.13](../documents/illegal_state/illegal_state_topology.md#313-a-compute-engine-incompatible-with-its-substrates-managed-providers-first-class)), `illegal_rke2_reused_host`
  ([§3.16](../documents/illegal_state/illegal_state_topology.md#316-a-multi-node-rke2-cluster-with-fewer-linux-hosts-than-nodes-or-a-host-reused) distinctness), `illegal_overcommit_{host,vm,cluster}` ([§3.17](../documents/illegal_state/illegal_state_capacity.md#317-an-over-committed-deploy-or-workload-host--vm--cluster-capacity-exceeded), on the base CPU/memory/pod-slot/ CSI-attach/logical-ephemeral axis — the `PhysicalDiskPartition` disk-parent variant of `illegal_overcommit_host` is owned by [phase_29_execution_accelerator_folds.md](phase_29_execution_accelerator_folds.md)),
...
- The positive fixtures `legal_multisubstrate_cluster` (the [§3.13](../documents/illegal_state/illegal_state_topology.md#313-a-compute-engine-incompatible-with-its-substrates-managed-providers-first-class) heterogeneous carve-out, exercising the elementwise compatibility fold and the fixed-topology first-fit-decreasing witness) and `legal_managed_eks`
  ...
  concern of phases 28 and 29; this gate exercises only their compatibility/base-capacity aspects.
  `legal_tmpfs_two_concurrent_writers_single_debit` is routed to phases 28 and 29.)
- The committed corpus behind those fixtures: `test/spec/dsl/CapacityTopologyFixtures.hs` holds fifteen direct,
  ...
  `test/oracle/capacity_topology/fold_cases.tsv`; the three dhall-typecheck Dhall foreclosure pairs are pinned by
  `test/oracle/capacity_topology/dhall_typecheck_cases.tsv`; seven `ghc -fno-code` pairs live under `test/spec/dsl/compilefail7/` and
  are pinned by `test/oracle/capacity_topology/compile_fail.tsv`; and the actual Phase-26 `legal_multisubstrate_cluster`
  ...
  storage/execution/accelerator fixtures of the committed forty-one-fixture corpus are exercised by phases 15
  and 10, not here.
...
2. The gate applies `fits`/`podFits`/`carve`/`place` and the topology relation **directly to each
   ...
   [phase_30_capability_bind.md](phase_30_capability_bind.md) and
   [phase_31_provision_seal.md](phase_31_provision_seal.md), whose gate re-exercises these same folds through
...
- `documents/engineering/resource_capacity_doctrine.md` — backlink §3's types and §4/§4.1's base fold to the
  ...
  storage arithmetic backlink is owned by [phase_28_storage_geometry_folds.md](phase_28_storage_geometry_folds.md).)
...
- `documents/illegal_state/illegal_state_catalog.md` — annotate the applicable §3.13–§3.17 parts with their
  ...
  runtime-checked entries (layer 3) deferred, and leave §3.19–§3.22/§3.27–§3.30 to phases 28 and 29.
...
- [phase_27](phase_27_illegal_state_covering.md) — the illegal-state corpus, properties, and validation-locus ledger this phase extends
- [phase_28_storage_geometry_folds.md](phase_28_storage_geometry_folds.md) — the logical→physical storage geometry, `StorageBudget`/`Growable`, and two-ceiling Pulsar fold built atop this base fold
- [phase_29_execution_accelerator_folds.md](phase_29_execution_accelerator_folds.md) — the execution-epoch expansion, scheduler-reservation algebra, accelerator/VRAM, and provider-root arithmetic that compose the full-resource-vector place witness on this base fold
- [phase_30_capability_bind.md](phase_30_capability_bind.md) — the capability → provider → shape binder built atop these folds
- [phase_31_provision_seal.md](phase_31_provision_seal.md) — the whole-deployment provision seal that re-exercises these base folds after bind
- Phase 9 capacity/topology ledger — the human-readable Register-1 proof/test/assumption boundary
