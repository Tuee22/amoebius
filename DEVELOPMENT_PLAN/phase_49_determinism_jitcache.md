# Phase 49: Determinism kernel + jit-build CacheBudget cache
...
- [Sprints](#sprints)
- [Sprint 49.1: `ContentAddress` typeclass kernel primitive ⏸️](#sprint-491-contentaddress-typeclass-kernel-primitive-)
- [Sprint 49.2: `experimentHash` identity over the live substrate fingerprint ⏸️](#sprint-492-experimenthash-identity-over-the-live-substrate-fingerprint-)
- [Sprint 49.3: SplitMix seed derivation, worker-count-independent ⏸️](#sprint-493-splitmix-seed-derivation-worker-count-independent-)
- [Sprint 49.4: The live same-substrate reproducibility gate ⏸️](#sprint-494-the-live-same-substrate-reproducibility-gate-)
- [Sprint 49.5: The `CacheBudget`-bounded content-addressed cache + peak-occupancy provision fold ⏸️](#sprint-495-the-cachebudget-bounded-content-addressed-cache--peak-occupancy-provision-fold-)
- [Sprint 49.6: The jit-build resolver — `resolve = {download | build}` on first miss, no URL arm ⏸️](#sprint-496-the-jit-build-resolver--resolve--download--build-on-first-miss-no-url-arm-)
- [Sprint 49.7: Per-node cache-owner reuse across client pods ⏸️](#sprint-497-per-node-cache-owner-reuse-across-client-pods-)
- [Sprint 49.8: The live first-miss / reuse / resource-admission gate + Register-3 ledger ⏸️](#sprint-498-the-live-first-miss--reuse--resource-admission-gate--register-3-ledger-)
...
⏸️ Blocked pending Phase-48 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
This phase turns the content-addressed store delivered in Phase 38 into one cohesive **reproducible resolved- asset lifecycle**. Its ordered sprint seams first establish the shared determinism identity and then use that
...
**The determinism kernel.** First, it lifts Phase 38's concrete blob/manifest key renderers into a kernel-level
...
a prebuilt engine or builds it from source (using the Phase-25 baked toolchain) into the cache; there is no arm
...
a request-carried derived seed — deliberately **not** an infernix inference run (that lift is Phase 50) and
**not** a jit-resolved ML engine (this phase resolves the engine, but the infernix inference that rides it is
Phase 50; the CUDA/jitML kernel tier is Phase 52). The `ModelArtifact` staging tier (Tier 2) and the JIT kernel
...
%% register: algebra
  ...
  ca[["ContentAddress typeclass"]]:::intent --> store[/"Phase 38 content-addressed store"/]:::effect
  eh --> store
...
Phase 38 content store plus workflow runtime. Every *how-it-behaved* claim is read from an OS-boundary observer
...
flowchart LR
  ...
  s0["Sprint 49.1: ContentAddress typeclass kernel primitive"]
  s1["Sprint 49.2: experimentHash identity over the live substrate fingerprint"]
  s2["Sprint 49.3: SplitMix seed derivation, worker-count-independent"]
  s3["Sprint 49.4: The live same-substrate reproducibility gate"]
  s4["Sprint 49.5: The CacheBudget-bounded content-addressed cache…"]
  s5["Sprint 49.6: The jit-build resolver — resolve = {download | build} on…"]
  s6["Sprint 49.7: Per-node cache-owner reuse across client pods"]
  s7["Sprint 49.8: The live first-miss / reuse / resource-admission gate…"]
  gate["the phase 49 gate"]
...
```
*Validated orientation. The seams Phase 49 built in order; [Gate integrity](#gate-integrity) owns the apparatus.*
...
backoff, replacement-on-Failed, terminal retention) and is also an exact Phase-38 `WorkflowRuntimeDemand`
...
Phase-38 runtime/mutation unit) must each turn the resource gate red even if the output bytes match. For the
...
- [`content_addressing_doctrine.md §2`](../documents/engineering/content_addressing_doctrine.md#2-the-three-tier-store-blobs--manifests--pointers)
  — *the three-tier store (blobs ← manifests ← pointers)*: the `ContentAddress` typeclass lifts Phase 38's
...
- [`image_build_doctrine.md §7`](../documents/engineering/image_build_doctrine.md#7-what-amoebius-bakes-vs-builds--the-base-container-is-the-supply-chain)
  ...
  identities — the Phase-25 split this phase exercises live for the first time.
...
- [`testing_doctrine.md §2`](../documents/engineering/testing_doctrine.md#2-the-registers-of-amoebius-testing)
  ...
  emits a proven/tested/assumed ledger naming that register, with the model/kernel tiers (Phases 50/52) marked
  deferred.
...
## Sprint 49.1: `ContentAddress` typeclass kernel primitive ⏸️
...
lift Phase 38's concrete blob/manifest key renderers into a kernel-level `ContentAddress` typeclass so that
"a content-derived name cannot be forged" is a single reusable primitive shared later by the engine cache
(Sprint 49.5) and by both infernix (Phase 50) and jitML (Phase 52), not a per-store copy.
...
- Adapters binding the typeclass to Phase 38's `blobs/<sha256>` and `manifests/<sha256>` writers — the
...
## Sprint 49.2: `experimentHash` identity over the live substrate fingerprint ⏸️
...
consuming the Phase-4 normal form and the Phase-24 full-path substrate probe, per the substrate doctrine's
...
## Sprint 49.3: SplitMix seed derivation, worker-count-independent ⏸️
...
## Sprint 49.4: The live same-substrate reproducibility gate ⏸️
...
without an infernix inference run (Phase 50) or a jit-resolved engine (that is the cache seam below and the
Phase-50 lift), and prove same-substrate reproducibility as one half of the phase gate without overclaiming
...
- The gate `.dhall` (`test/dhall/phase_49_determinism_repro.dhall`) that spins up the Phase-38 workflow, runs the
...
## Sprint 49.5: The `CacheBudget`-bounded content-addressed cache + peak-occupancy provision fold ⏸️
...
4. **Pin-aware pruning is exercised, not declared:** a cache filled to `CacheBudget` with a mix of pinned and
   ...
   ([Gate integrity](#gate-integrity) part (b) mutant (b)) must turn this clause red (the over-budget residency survives). This is the pure-pool property; its live on-disk counterpart is the Sprint 49.8 postflight residency
...
## Sprint 49.6: The jit-build resolver — `resolve = {download | build}` on first miss, no URL arm ⏸️
...
miss into the bounded cache — downloaded prebuilt or built from source with the Phase-25 baked toolchain — with
...
- The build-from-source path invoking the Phase-25 baked toolchain by absolute path (no `PATH`, no env), and the
...
- An in-file honesty note: URL-foreclosure and identity-from-closed-catalog are **proven-in-types** (Gate 1); the
  ...
  model (Tier 2) and kernel (Tier 3) tiers reuse this resolver but land in Phases 50/51.
...
### Remaining Work
Production model inference through the resolved engine is Phase 50, not a hidden acceptance claim here.

## Sprint 49.7: Per-node cache-owner reuse across client pods ⏸️
...
- The pod wiring (rendered by the Phase-27 reconciler) that gives the owner a bounded disk-backed `emptyDir` with
...
## Sprint 49.8: The live first-miss / reuse / resource-admission gate + Register-3 ledger ⏸️
...
**Docs to update**:
`documents/engineering/content_addressing_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-49
...
- A Register-3 ledger recording: URL-foreclosure and the required bounded-`CacheBudget` field as
  ...
  **deferred** (Phases 50/52), with cross-node and cross-substrate reuse explicitly not asserted.
...
3. The Register-3 ledger is emitted and marks first-miss resolution, cross-pod reuse, and pin-aware eviction as
   *tested on linux-cpu*, URL-foreclosure and the required bounded-`CacheBudget` field as *proven-in-types*, the
   numeric cache/ephemeral inequalities as *provision-seal checked*, and the model/kernel tiers (Phases 50/52) and
   cross-node/cross-substrate reuse as **UNVERIFIED**.

### Remaining Work
The Tier-2 model and Tier-3 CUDA kernel reuse remain assigned to Phases 50 and 52 respectively.
...
- `documents/engineering/content_addressing_doctrine.md` — the §6 proven/tested/assumed table gains an
  ...
  Tier-2/Tier-3 realizations remain Phases 50/52 targets, and add the kernel module paths
...
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 49's gate substrate (linux-cpu) in the per-phase substrate map.
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Kernel/ContentAddress.hs`,
  ...
  `test/negative/phase_49_url_arm.hs`, `test/mutants/phase_48_determinism/`, `test/mutants/phase_48_cache/`) as Phase-49
  design-first rows.
...
- [phase_27](phase_27_object_reconciler.md) — the typed SSA object reconciler that renders the cache owner and client pods
- [phase_31](phase_31_platform_backbone.md) — the platform backbone the cache owner/client pods schedule onto
- [phase_38](phase_38_content_store_workflow.md) — the content store + workflow runtime this phase lifts and runs on
- [phase_50](phase_50_infernix_lift.md) — the infernix CPU-inference lift whose reproducibility reuses this kernel and rides this resolver next
- [phase_52](phase_52_jitml_lift_cuda.md) — the jitML/CUDA lift whose kernel tier reuses this resolver and kernel
