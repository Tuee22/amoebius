# Phase 45: Provider Pulumi deploy-from-inside + enveloped checkpoint
...
- [Sprints](#sprints)
- [Sprint 45.1: Provider-cluster Pulumi deploy from inside a parent ⏸️](#sprint-451-provider-cluster-pulumi-deploy-from-inside-a-parent-)
...
⏸️ Blocked pending Phase-44 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
This is the **first arm** of the provider-managed-cluster split (Phases 45–48): the mechanism by which
...
linux-cpu parent, plus its gate; it is the substrate on which [phase_46_provider_child_bringup.md](phase_46_provider_child_bringup.md)
(the stateless hostless in-cluster control plane), [phase_47_provider_ebs_credential.md](phase_47_provider_ebs_credential.md)
(per-PV EBS + the create-vs-delete credential model), and [phase_48_provider_dynamic_nodes.md](phase_48_provider_dynamic_nodes.md)
...
an already-running amoebius cluster, issued by the Deployment-`replicas=1` control-plane singleton (Phase 34) —
whose single-instance is a k8s/etcd property, never a bespoke amoebius election — generalizing Phase 43's
...
or standard-service convergence for a provider child ([phase_46_provider_child_bringup.md](phase_46_provider_child_bringup.md));
per-PV durable EBS, the `protect`/`Retain` state class, the create-vs-delete credential split, or the static
`ebs.csi.aws.com` CSI path ([phase_47_provider_ebs_credential.md](phase_47_provider_ebs_credential.md)); or
dynamic node provisioning by signal, the second reconcile no-op, and the independent leak-free teardown sweep
([phase_48_provider_dynamic_nodes.md](phase_48_provider_dynamic_nodes.md)). The base node group here is a fixed
...
> **Provider corpus split (by design).** Phases 45–48 (the provider split) share one provider **topology > shape**, but each sub-phase commits its own Phase-0 corpus and gates only its own slice: the topology
...
[phase_48_provider_dynamic_nodes.md](phase_48_provider_dynamic_nodes.md)'s; the per-PV durable EBS +
create-vs-delete slice (the `ec2:DeleteVolume` expected-denied-call tag, the `protect`/`Retain` state, the
static-CSI `volumeHandle` bind, `mut-46.1-*`) is [phase_47_provider_ebs_credential.md](phase_47_provider_ebs_credential.md)'s;
and the scheduler-cutover slice (`mut-45.1-public-pull`, the `LinuxHost`-absence foreclosure tag) is
[phase_46_provider_child_bringup.md](phase_46_provider_child_bringup.md)'s. Partitioned to this seam:
...
[phase_48_provider_dynamic_nodes.md](phase_48_provider_dynamic_nodes.md)'s gate, deferred and never depended
...
- [`pulumi_iac_doctrine.md §1`](../documents/engineering/pulumi_iac_doctrine.md#1-pulumi-runs-only-from-inside-an-existing-amoebius-cluster)
  ...
  than a bespoke state machine. (The EBS create-vs-delete credential arm of [§6](../documents/engineering/pulumi_iac_doctrine.md#6-the-ebs-create-vs-delete-credential-model) is realized in [phase_47_provider_ebs_credential.md](phase_47_provider_ebs_credential.md), not here.)
- [`cluster_lifecycle_doctrine.md §3`](../documents/engineering/cluster_lifecycle_doctrine.md#3-amoebic-spawning--the-recursive-forest)
  — *amoebic spawning — the recursive forest*: provider-cluster spawn is the *cloud-keyed* sibling of Phase 43's
...
## Sprint 45.1: Provider-cluster Pulumi deploy from inside a parent ⏸️
...
**first delivered by Phase 43 and reused here, not rebuilt**; the `observeProviderAccount` boundary and the
...
object set, generalizing Phase 43's SSH-keyed self-managed spawn to a cloud-keyed provider spawn. The `pulumi`
...
- An `Amoebius.Pulumi.Provider.Eks` program that provisions the EKS control plane + a base managed node group
  ...
  the cluster ready for its in-cluster control-plane bootstrap ([phase_46_provider_child_bringup.md](phase_46_provider_child_bringup.md)).
...
- `planInfrastructure` derives the provider-cluster demand from the exact `BoundDeployment` and declared
  ...
  [phase_47_provider_ebs_credential.md](phase_47_provider_ebs_credential.md).) Only the receipt-bound provider
...
> `mut-47.2-skip-sweep`) is [phase_48_provider_dynamic_nodes.md](phase_48_provider_dynamic_nodes.md)'s gate,
...
- `documents/engineering/cluster_lifecycle_doctrine.md` — record that §3 (the cloud-keyed amoebic spawn) gains
  an amoebius EKS reference as the cloud-keyed sibling of the Phase 43 SSH-keyed spawn.
...
- `documents/engineering/testing_doctrine.md` — record the Phase 45 per-run ledger artifact for the
...
- `DEVELOPMENT_PLAN/system_components.md` — map the reused `amoebius-pulumi` Engine and EncryptedMinio backend
  to their first delivery in Phase 43; register `Amoebius.Pulumi.Provider.Eks`, the `observeProviderAccount`
  boundary, and the `PulumiExecutionDemand` / `PulumiCheckpointObjectDemand` types as Phase-45 design-first rows,
...
- [README.md](README.md) — the live tracker; the Phase 45 objective, gate, and substrate
...
- [phase_43](phase_43_multicluster_spawn_georepl.md) — the SSH-keyed amoebic spawn, encrypted MinIO backend, and
  per-child envelope this phase generalizes to a cloud-keyed provider spawn
- [phase_46](phase_46_provider_child_bringup.md) — the stateless hostless in-cluster singleton + capacity-scheduler
  bring-up that layers on this deploy
- [phase_47](phase_47_provider_ebs_credential.md) — the per-PV EBS + create-vs-delete credential model + static
  CSI path that layers on this deploy
- [phase_48](phase_48_provider_dynamic_nodes.md) — dynamic node provisioning by signal and the independent
