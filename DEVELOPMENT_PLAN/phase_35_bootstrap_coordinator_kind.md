# Phase 35: Python bootstrap coordinator + substrate detect + single kind cluster
...
and Windows uses WSL2 for the same clean boundary. The complete Phase-35 gate passed on the Incus route;
...
- [Sprints](#sprints)
- [Sprint 35.1: Live substrate detection ✅](#sprint-351-live-substrate-detection-)
- [Sprint 35.2: No-`PATH` lazy tool-ensure — closed enum, `AbsExe`, install-and-verify ✅](#sprint-352-no-path-lazy-tool-ensure--closed-enum-absexe-install-and-verify-)
- [Sprint 35.3: The Python `pb` bootstrap coordinator (package-manager root → toolchain → build → `exec`) ✅](#sprint-353-the-python-pb-bootstrap-coordinator-package-manager-root--toolchain--build--exec-)
- [Sprint 35.4: The in-binary `bootstrap` command — idempotent single-node kind bring-up ✅](#sprint-354-the-in-binary-bootstrap-command--idempotent-single-node-kind-bring-up-)
...
Evidence and the ledger land in `.build/runs/phase_29/<run-id>/`, and 28 surfaces join two-way to 30 items.
...
control-plane daemon — those land in Phase 36 onward. Before `kind create`, bootstrap observes physical-host
...
[phase_44](phase_44_live_dsl_deploy.md) Sprint 44.4) — so the
...
flowchart LR
  ...
  s0["Sprint 35.1: Live substrate detection"]
  s1["Sprint 35.2: No-PATH lazy tool-ensure — closed enum, AbsExe…"]
  s2["Sprint 35.3: The Python pb bootstrap coordinator (package-manager root → toolchain →…"]
  s3["Sprint 35.4: The in-binary bootstrap command — idempotent single-node…"]
  gate["the phase 35 gate"]
...
```
*Orientation. The seams Phase 35 built in order; [Gate integrity](#gate-integrity) owns the now-passed apparatus.*
...
start state the identical run **converges without recreating the cluster** (divergence-repair, Sprint 35.4).
...
not a sixth Phase-35 `HostTool`: it establishes the machine boundary from which the five-member bootstrap coordinator gate
...
implementation exists: the `classify` decision table (Sprint 35.1), the per-substrate `[InstallStep]` golden
plans and the `mkAbsExe`-reject expected-error set (Sprint 35.2), and the named divergent-start fixtures with
their expected converged observation (Sprint 35.4). A golden regenerated from the implementation is not
admitted.
...
## Sprint 35.1: Live substrate detection ✅
...
## Sprint 35.2: No-`PATH` lazy tool-ensure — closed enum, `AbsExe`, install-and-verify ✅
...
- A **CI structural check that no module outside `src/Amoebius/Host/Ensure.hs` imports any process-spawn API**
  ...
  level).
- A `HostConfig` carrying the detected substrate (Sprint 35.1) plus a `Map HostTool AbsExe`, and a probe-first,
...
## Sprint 35.3: The Python `pb` bootstrap coordinator (package-manager root → toolchain → build → `exec`) ✅
...
`pb/pb/admin.py` in [phase_44](phase_44_live_dsl_deploy.md) Sprint 44.4). No shell script: amoebius owns
none.
**Blocked by**: None.
...
Linux frameworks on Apple and Windows and their fresh-guest command plans are unit-tested; Phase 35 exercises
...
## Sprint 35.4: The in-binary `bootstrap` command — idempotent single-node kind bring-up ✅
...
### Deliverables
- A `bootstrap` command chain that detects the substrate (Sprint 35.1), assembles the **`FrameConfig`** —
  ...
  ensures `kind`/`kubectl` by absolute path (Sprint 35.2), and
...
- An empty cluster as the end state: one node, `Ready`, with **no** platform services (those are Phase 36+).
- A typed `ObservedInventory` assembled from an independent apiserver/OS-boundary read: net node allocatable
  ...
  in Phase 74.) The bootstrap handoff exists only after `declaredCapacity <= observedCapacity` on every
...
- An engine-system accounting rule: every kind node has an enforced role-indexed `EngineSystemReserve`.
  ...
  These bootstrap add-ons may still use `default-scheduler` in this Phase-35 empty-cluster state; the later
  ...
  leaving the scheduler bootstrap Pod as the sole `default-scheduler` exception — is Phase 38's scope and is
  specified there.
...
throttling, mutant results, rerun/repair observations, and postflight sweep under `.build/runs/phase_29/`. The
...
- `documents/engineering/bootstrap_sequence_doctrine.md` — record that `pb`'s **bootstrap coordinator** mode is delivered here
  and that its **admin-REST client** mode (§5) is delivered by [phase_44](phase_44_live_dsl_deploy.md)
  Phase 44 Sprint 44.4, not left to an unassigned "later phase".
...
- `documents/engineering/resource_capacity_doctrine.md` — record Phase 35 as the first live producer of the
...
- [README.md](README.md) — flip the Phase 35 tracker-row status once work begins and keep its link current.
- [substrates.md](substrates.md) — record `linux-cpu` as the Phase 35 gate substrate (the first Register-3 row).
...
- [README.md](README.md) — the live tracker; its Phase 35 row is the authoritative one-line gate and status.
...
- [Bootstrap Sequence Doctrine](../documents/engineering/bootstrap_sequence_doctrine.md) — the unified `pb` CLI's
  two modes (bootstrap coordinator here; admin-REST client in [phase_44](phase_44_live_dsl_deploy.md) Sprint 44.4).
