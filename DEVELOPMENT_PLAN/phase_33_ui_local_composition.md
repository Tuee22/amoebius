...
- [Sprints](#sprints)
- [Sprint 33.1: Single-/multi-tenant workflow-to-artifact composition gate ✅](#sprint-331-single-multi-tenant-workflow-to-artifact-composition-gate-)
...
remain beneath `.build/**`; the outside-host inventory is unchanged; Phase 33 owns no migration deferral. The
...
network, and all five mutants redden. Evidence and the ledger move into `.build/runs/phase_27/<run-id>/`, and 58
surfaces join two-way to 71 run-time enumerated items.

**The composition only became runnable when Phase 32's ABI landed.** `use-artifact` had no row in the closed
action table, so the third step of the workflow returned the non-enumerating refusal and the artifact result
never appeared. The table now carries one row per Phase-28 port effect.
...
**Dependencies:** Phase 31 — generic browser interpreter; Phase 32 — authenticated scoped UI-server boundary.
...
of [Gate integrity](#gate-integrity). Phase 34 does not open unless it emits a green Register-2 ledger with all
...
## Sprint 33.1: Single-/multi-tenant workflow-to-artifact composition gate ✅

**Status**: Done — the composition runs end to end on the Phase-32 boundary; the sprint's committed-ledger, pinned-toolchain, and repository-resident evidence mechanics are superseded
...
- [Phase 31](phase_31_ui_browser_interpreter.md) — the required generic browser runtime.
- [Phase 32](phase_32_ui_server_boundary.md) — the required authenticated scoped server boundary.
