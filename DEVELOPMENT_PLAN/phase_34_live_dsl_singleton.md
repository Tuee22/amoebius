# Phase 34: Live DSL deploy via the replicas=1 singleton
...
- [Sprints](#sprints)
- [Sprint 34.1: The control-plane singleton — a Deployment replicas=1, single-instance from k8s/etcd ⏸️](#sprint-341-the-control-plane-singleton--a-deployment-replicas1-single-instance-from-k8setcd-)
- [Sprint 34.2: Live reconcile of the platform + a trivial app from one `.dhall` ⏸️](#sprint-342-live-reconcile-of-the-platform--a-trivial-app-from-one-dhall-)
- [Sprint 34.3: Phase gate harness — live deploy + the pre-cluster negative corpus as a live regression guard ⏸️](#sprint-343-phase-gate-harness--live-deploy--the-pre-cluster-negative-corpus-as-a-live-regression-guard-)
- [Sprint 34.4: The admin REST surface — `vault init/unseal`, `dhall update`, secret KV-CRUD ⏸️](#sprint-344-the-admin-rest-surface--vault-initunseal-dhall-update-secret-kv-crud-)
...
secret is admissible with no Vault interaction at all, which is what leaves Phases 25–29 independent of
Phase 29. The paired positive — the same spec admitted after the Phase-30 prompt CLI writes the secret — is
this claim's other half; neither alone is evidence.

**Invalidated historical record:**

**Done.** All four sprints are implemented and the Phase-34 Register-3 gate is sealed. It ran after the
Phase 33 gate (Keycloak-owned ingress) on the **linux-cpu** substrate: the retained single-node
`kind` cluster after Phases 24–33, with the full standing shape assembled by the registry/base-image work
(Phase 25), Vault/PKI (Phase 30), platform services (Phases 31–32), and Keycloak-owned edge (Phase 33), all
applied through the Phase-27 typed renderer + SSA reconciler onto Phase-29 retained storage. The delivered
...
authored and proven **in-process, with no cluster**. Phase 34 adds the runtime residue: the in-cluster
...
it, applying the standard platform stack plus a trivial app through the Phase-27 reconciler to convergence with
...
in-namespace Sql) is deliberately deferred to Phase 35; the app here is trivial.

The initial ownership transition is explicit. Phase 27 acquired the deployment-global mandatory reconciler
Lease under the bootstrap-host holder before any host-driven apply and kept renewing it through Phases 29–32.
...
**Substrate:** linux-cpu — the single-node `kind` cluster from Phases 24–33; no apple, linux-cuda, or windows
...
flowchart LR
  ...
  s0["Sprint 34.1: The control-plane singleton — a Deployment replicas=1…"]
  s1["Sprint 34.2: Live reconcile of the platform + a trivial app from one .dhall"]
  s2["Sprint 34.3: Phase gate harness — live deploy + the pre-cluster negative…"]
  s3["Sprint 34.4: The admin REST surface — vault init/unseal, dhall update…"]
...
```
*Orientation. The seams Phase 34 delivered in order; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns the sealed apparatus.*
...
- **History capacity is a gate precondition, not assumed retention.** Before the first platform/app mutation,
  the harness reads the Phase-24 `ControlPlaneStorageDemand` enforcement and proves Event/audit retention
...
- **Concrete representative set (§M.7).** The Phase-31/32 service set reconciled by this fixture is exactly:
...
- **oracle-pinned oracle (§M.1).** The positive fixture `dhall/examples/platform_plus_trivial_app.dhall`, the
  ...
  admin-surface oracles of [Sprint 34.4](#sprint-344-the-admin-rest-surface--vault-initunseal-dhall-update-secret-kv-crud-)
...
- **Committed seeded mutant (§M.2).** The gate names **≥1 committed seeded mutant** that MUST turn it red:
  ...
  MUST also go red via the attribution clause above. [Sprint 34.4](#sprint-344-the-admin-rest-surface--vault-initunseal-dhall-update-secret-kv-crud-)
...
- **The paired positive.** The identical spec, after the Phase-30 prompt CLI writes those secrets, is
...
- [`preflight_validation_doctrine.md`](../documents/engineering/preflight_validation_doctrine.md)
  ...
  by [phase_45](phase_45_provider_deploy_checkpoint.md) / [phase_48](phase_48_provider_dynamic_nodes.md).
- [`daemon_topology_doctrine.md §3`](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-singleton)
  ...
  bespoke election. This phase also performs the one-way authority handoff from the observed Phase-27
...
- [`dsl_doctrine.md §5`](../documents/engineering/dsl_doctrine.md#5-the-illegal-state-unrepresentable-contract)
  ...
  and Phase-13 `renderAll` were discharged in-process in the pre-cluster band. This phase runs the **runtime residue** — the live path must follow decoded IR → bind/expand → `planInfrastructure` → explicit
...
## Sprint 34.1: The control-plane singleton — a Deployment replicas=1, single-instance from k8s/etcd ⏸️
...
re-observe` loop wrapping the Phase-27 typed reconciler and its observed-Pod/runtime-storage normalization);
...
### Deliverables
- A control-plane singleton deployed as a **generated typed `Deployment replicas=1`** by the Phase-27
...
- The `discover → diff → enact → re-observe` reconcile loop that decodes the `InForceSpec` in-process
  ...
  Phase-27 typed reconciler — idempotently, driven only by observed cluster state.
...
- A closed initial `AuthorityHandoff`: while holding the exact Lease the Phase-27 host applies the singleton
...
- Secret authority fused to the role (operates root Vault as the single in-cluster writer) and the admin-REST
  ...
  four-endpoint surface by [Sprint 34.4](#sprint-344-the-admin-rest-surface--vault-initunseal-dhall-update-secret-kv-crud-).
...
## Sprint 34.2: Live reconcile of the platform + a trivial app from one `.dhall` ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
second-pass audit/enact set after fresh discovery, routes the trivial app through the Phase-33 edge, and
...
down leaving the shared stack as found. Because Phases 24–33 leave that stack pre-converged, the numbered
...
### Deliverables
- A positive deploy `.dhall` composing the standard platform-service stack (Phases 31–32) and a **trivial**
  single-service app — deliberately narrower than the Phase-35 tenancy projection (no per-app namespace,
...
- A leak-free teardown obligation carried by the deploy fixture — a test-topology `.dhall` whose postflight
  ...
  reclaimed, while the pre-existing Phase-31/32 service set and Phase-33 edge are restored to Ready rather than
  swept.
...
1. Before the first pass the harness deletes the components named in
   ...
   example Prometheus's — so a pre-converged Phase-31/32 stack cannot ride the gate (§M.6). The first pass
   ...
   to `expected-enact-pass1.json`; the app is reachable through the Phase-33 Keycloak-owned edge; and a re-run
...
2. Teardown leaves no leaked resources. The postflight sweep is scoped to this run's provisioned objects,
   ...
   it creates — that label set is authored here, and Phase-55 flag-at-creation machinery is not assumed — and
   the sweep is empty over it. Separately, every platform component the harness perturbed is asserted back at
   Ready so the shared Phase-31/32 stack is left as found, and the apiserver audit log records that **every**
...
## Sprint 34.3: Phase gate harness — live deploy + the pre-cluster negative corpus as a live regression guard ⏸️
...
equality, reservation/observed no-double-debit, and alias controls); the reused Phase-6 negative corpus
...
under the perturbation and attribution regime of Sprint 34.2, tears down leak-free, then re-runs the Phase-6
...
(flip the Phase-34 status when the gate passes).
...
- A **Register-3** proven/tested/assumed ledger recording the live-enforcement result (the apiserver admitted
  the rendered manifests) and marking the deferred surfaces — full app tenancy (Phase 35), and the
...
## Sprint 34.4: The admin REST surface — `vault init/unseal`, `dhall update`, secret KV-CRUD ⏸️
...
[phase_24](phase_24_bootstrap_coordinator_kind.md) Sprint 24.3 and owned here), `pb/pb/cli.py`, and
`app/singleton/Main.hs` — delivered. This sprint promotes Sprint 34.1's admin-REST
**control-surface stub** to the real surface; it does not re-implement the Sprint-29.1 Argon2id→AEAD unlock
envelope, the Phase-30 Vault client, or the Sprint-33.2 reconcile loop — it is the operator-facing channel
into all three.
**Blocked by**: reopened numeric predecessor gates.
...
- **`dhall update`** — deliver a new `InForceSpec` to a running cluster (requires an unsealed Vault and a root
  token). The singleton decrypts/stores the envelope in-process and reconciles toward it via Sprint 34.2's loop.
...
- A **Register-3** proven/tested/assumed ledger recording the admission gate as **runtime-checked, live**, and
  ...
  ([phase_35](phase_35_app_tenancy.md)) and the parent→child `ParentReachChannel` use of this surface
  ([phase_46](phase_46_provider_child_bringup.md)), neither of which this phase exercises.
...
### Validation
1. **The post-handoff operator sequence, end to end.** After Sprint 34.1's observed handoff, drive `vault
...
- `documents/engineering/substrate_doctrine.md` — the §6 bootstrap coordinator-contract note that `pb`'s second mode is "a
  later phase" resolves to Sprint 34.4.

**Cross-references to add:**
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-34 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — confirm the Phase-34 linux-cpu gate row (the replicas=1 singleton, no
  election).
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/ControlPlane/{Singleton,Reconcile,Deploy,AdminApi}.hs`
  as Phase-34 design-first rows, and re-anchor the in-cluster-singleton row to the current
  `#3-the-control-plane-singleton` (no election).
- `DEVELOPMENT_PLAN/phase_24_bootstrap_coordinator_kind.md` — Sprint 24.3's deferred `pb` admin-REST client mode
  now names Sprint 34.4 as its owner.
- `DEVELOPMENT_PLAN/phase_46_provider_child_bringup.md` — its post-handoff child admin-REST bring-up can now
  name Sprint 34.4 in `Blocked by`.
...
- [Bootstrap Sequence Doctrine](../documents/engineering/bootstrap_sequence_doctrine.md) — the [§5](../documents/engineering/bootstrap_sequence_doctrine.md#5-the-admin-control-plane-the-cli--the-singleton-rest-api) admin control
  plane (CLI ↔ singleton REST) delivered by Sprint 34.4, and the [§4](../documents/engineering/bootstrap_sequence_doctrine.md#4-the-host-daemon--singleton-handoff) handoff point at which it is exposed
...
- [phase_24](phase_24_bootstrap_coordinator_kind.md) — the two-mode `pb` CLI whose admin-REST client mode Sprint
  24.3 defers to the singleton and Sprint 34.4 owns
- [phase_30](phase_30_vault_pki.md) — the root Vault, unlock-material envelope, and built-in Vault client the
  admin endpoints call
- [phase_46](phase_46_provider_child_bringup.md) — the parent→child bring-up that drives a child through this
  same surface over its `ParentReachChannel`
- [phase_27](phase_27_object_reconciler.md) — the typed renderer + SSA reconciler that renders and applies
  the singleton and its manifests
- [phase_31](phase_31_platform_backbone.md) — the standard platform-service stack the live `.dhall` deploys
- [phase_33](phase_33_keycloak_ingress.md) — the Keycloak-owned edge the trivial app routes through
- [phase_35](phase_35_app_tenancy.md) — the app-tenancy projection (namespace + ObjectStore + Sql) deferred
  from this phase
