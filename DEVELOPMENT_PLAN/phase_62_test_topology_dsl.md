# Phase 62: Test-topology DSL + suggest-test + elevated harness
...
- [Sprints](#sprints)
- [Sprint 62.1: The test-topology type — a deployment-rules layer that always tears down ⏸️](#sprint-621-the-test-topology-type--a-deployment-rules-layer-that-always-tears-down-)
- [Sprint 62.2: `suggest-test` — detect substrate + credential authority, emit a representative test `.dhall` ⏸️](#sprint-622-suggest-test--detect-substrate--credential-authority-emit-a-representative-test-dhall-)
- [Sprint 62.3: Flagged test credentials + test-owned resource tagging ⏸️](#sprint-623-flagged-test-credentials--test-owned-resource-tagging-)
- [Sprint 62.4: The elevated harness as sole automated deleter of test-owned storage + leak-free sweep ⏸️](#sprint-624-the-elevated-harness-as-sole-automated-deleter-of-test-owned-storage--leak-free-sweep-)
- [Sprint 62.5: The per-run ledger artifact + the delegated-failover gate topology ⏸️](#sprint-625-the-per-run-ledger-artifact--the-delegated-failover-gate-topology-)
...
⏸️ Blocked pending Phase-61 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
single-node `kind` cluster brought up by the Phase 35 bootstrap coordinator with Pulsar and MinIO already standing HA
(Phase 41) on retained storage (Phase 39). The mechanisms generalize patterns *proven in the sibling prodbox
...
Pulsar `Exclusive`/`Failover` subscription of Phase 48, while the control-plane daemon it deploys under is
...
the `replicas=1` control-plane daemon (Phase 44), the native Pulsar client and Pulsar-`Failover` worker takeover
(Phases 46–48), the retained `no-provisioner` PV model (Phase 39), substrate detection (Phase 35),
Vault secret-by-name injection (Phase 40), and the leak-free provider teardown (Phase 58).
...
**Lane:** per generated test ([§L](development_plan_standards.md#l-one-substrate-discipline))
...
1. **Leak-freedom by implementation-independent inventory diff, not tag query.** Leak-freedom is asserted by a
   ...
   parent-drives-provider form over Phase 58's leak-free provider teardown, still one substrate per generated
...
4. **Ledger applicability is derived and oracle-pinned, not self-declared.** The set of applicable moves is
   ...
   MUST match the independently authored `test/oracles/phase_55_expected_moves.json`; the run-local result then
...
7. **Failover takeover pinned to an external broker-stats observer, not the `ExpectationWitness`.** The
   ...
   statistics (`pulsar-admin topics stats`/`topics partitioned-stats` on the inherited Phase-48 command topic)
...
comes from a real `amoebius suggest-test` execution on the gate host: a real Phase-35 host classification,
...
the `linux-cpu` single-node `kind` cluster, at minimum: the pre-standing Phase-41 HA Pulsar and MinIO on
retained Phase-39 storage; **at least one newly-allocated retained `no-provisioner` PV** (so the sole-deleter
rule of Sprint 62.4 is exercised against a resource this phase created); **one control-plane workload as a Deployment `replicas=1`**; and **at least two Pulsar `Failover`-subscription consumer pods** (an active worker
...
flowchart LR
  ...
  s0["Sprint 62.1: The test-topology type — a deployment-rules layer that…"]
  s1["Sprint 62.2: suggest-test — detect substrate + credential authority…"]
  s2["Sprint 62.3: Flagged test credentials + test-owned resource tagging"]
  s3["Sprint 62.4: The elevated harness as sole automated deleter of…"]
  s4["Sprint 62.5: The per-run ledger artifact + the delegated-failover gate…"]
  gate["the phase 62 gate"]
...
```
*Orientation. The seams phase 62 builds, in order; [Gate integrity](#gate-integrity) owns the apparatus. Not run.*

**Committed, oracle-pinned oracles and mutants (authored before any implementation exists).**
- `test/oracles/phase_55_expected_moves.json` — the independently authored expected applicability/strength
...
**Register binding for all sprint validations.** Every Sprint 62.1–47.5 Independent Validation and Validation
below is discharged in the register named on the criterion. Where a criterion names Register 2 (Phase-21 fake
tools / fixed fake inputs), it is a type-level or pure-function check and its live layer is recorded UNVERIFIED
in the ledger; the leak-free teardown, the live identity-boundary denial (Sprint 62.4), and the failover
...
The topology inherits and source-checks Phase-48's exact command/event topic, Failover subscription,
...
- [`testing_doctrine.md §3`](../documents/engineering/testing_doctrine.md#3-the-test-topology-contract-spin-up--run--always-tear-down)
  — *the test-topology contract: spin up → run → always tear down*: Sprint 62.1 realizes the four-clause
...
- [`testing_doctrine.md §5`](../documents/engineering/testing_doctrine.md#5-suggest-test-detect-the-world-emit-a-representative-test-dhall)
  — *`suggest-test`: detect the world, emit a representative test `.dhall`*: Sprint 62.2 builds the generator
...
- [`testing_doctrine.md §6`](../documents/engineering/testing_doctrine.md#6-flagged-test-credentials)
  — *flagged test credentials*: Sprint 62.3 establishes the distinct flagged test-simulation identity (the
...
- [`testing_doctrine.md §7`](../documents/engineering/testing_doctrine.md#7-the-elevated-harness-is-the-sole-automated-deleter-of-test-owned-durable-storage-leak-free-cycles)
  ...
  Sprint 62.4 implements one-deleter/one-credential, flag-then-sweep, and the independent postflight-inventory
...
- [`testing_doctrine.md §4`](../documents/engineering/testing_doctrine.md#4-no-skips-fail-fast-and-the-per-run-ledger-artifact)
  — *no skips, fail fast, and the per-run ledger artifact*: Sprint 62.5 emits the per-run
...
## Sprint 62.1: The test-topology type — a deployment-rules layer that always tears down ⏸️
...
**Register 3** on the `linux-cpu` `kind` cluster (a teardown against Phase-21 fakes does not prove real
...
## Sprint 62.2: `suggest-test` — detect substrate + credential authority, emit a representative test `.dhall` ⏸️
...
**Independent Validation**: Register 2 — pure, over Phase-21 fake tools, so the live probe layer is recorded
...
2. Property tests perturb each supply axis independently.
   ...
   - Reject-branch coverage — that the structured "no representative topology fits" result actually fires —
     ...
     mutants (the pinned `phase_56_resource_overcommit_*`/`phase_56_missing_capability` and `drop_*`
...
3. Exercise the closed optional-branch fixture matrix over its four fixed-input shapes — all-three-negative,
   ...
   `phase_56_optional_resource_shapes.json`. The committed
   `drop_registry_publication_envelope.dhall`, `drop_pulumi_executor_envelope.dhall`,
   `drop_pulumi_checkpoint_demand.dhall`, `drop_migration_copy_envelope.dhall`, and
...
## Sprint 62.3: Flagged test credentials + test-owned resource tagging ⏸️
...
paths; not yet built)
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: Register 2 — type-level, over Phase-21 fake tools. This sprint pins the
*representability* boundary only; the *live* identity-boundary denial is Sprint 62.4's Register-3 obligation.
...
- The flagged credential resolved by name only through Vault (Phase 40) — flagging changes *which* credential
...
## Sprint 62.4: The elevated harness as sole automated deleter of test-owned storage + leak-free sweep ⏸️
...
### Deliverables
- A delete path reachable only by the elevated harness under the flagged credential of Sprint 62.3, scoped to
...
- An explicit scope boundary: this harness reclaims **test-owned** backing only. Production
  ...
  Phase 62 neither holds nor tests authority over production backing.
...
3. Leak detection uses the Gate-criterion-1 inventory diff, not a test-owned tag query: the committed seeded
   ...
   (Phase 55), keeping this single-substrate `linux-cpu` run's own scope intact ([§L](development_plan_standards.md#l-one-substrate-discipline)).
...
## Sprint 62.5: The per-run ledger artifact + the delegated-failover gate topology ⏸️
...
injects is delegated to Pulsar (Phase 48), never a bespoke amoebius election.
...
- A `Ledger` emitter producing, per run, a record of which correctness layers were reached and at what strength
  ...
  expected-move table `test/oracles/phase_55_expected_moves.json`, authored independently of `Ledger.hs`, is
...
5. Independently make the generator/harness, controller, active, standby, replacement, selected images,
   ...
   structural-takeover-defeat `phase_56_failover_standby_wrong_subscription.dhall` (caught by the external
...
> and delivered by Phase 54's gateway-migration drills, not here. The delegated-failover shape is proven in the sibling `infernix`
...
- `documents/engineering/app_vs_deployment_doctrine.md` — §3's deployment-rules surface gains the realized
  test-topology layer (Sprint 62.1): the `TestTopology` is an ordinary deployment-rules layer over a production
...
- `DEVELOPMENT_PLAN/system_components.md` — add the `Amoebius/Test/*` modules (`Topology`, `Runner`,
  `SuggestTest`, `Credentials`, `Harness`, `Sweep`, `Ledger`) to the component inventory as Phase-62
  design-first rows, mapped to the owning testing and storage-lifecycle doctrines.
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 62 "per generated test" substrate posture (each generated
  test substrate-locked; the canonical gate run on `linux-cpu`) in the per-phase substrate map.

## Related Documents
- [README.md](README.md) — the live tracker; Phase 62 objective, gate, and substrate
...
- [phase_48](phase_48_content_store_workflow.md) — the Pulsar-`Failover` worker takeover this phase's gate injects
- [phase_54](phase_54_gateway_migration_drills.md) — the cross-cluster gateway-migration obligation, distinct
  from this phase's intra-cluster failover
- [phase_58](phase_58_provider_dynamic_nodes.md) — the leak-free provider teardown this harness extends to test cycles
