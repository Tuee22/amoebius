# Phase 45: Hostless provider child + convergence + Lease handoff
...
> [Phase 44](phase_44_provider_deploy_checkpoint.md) — to the same fungible shape as any self-managed amoebius
...
convergence** — never the Pulumi deploy/checkpoint that produced its input ([Phase 44](phase_44_provider_deploy_checkpoint.md)),
...
First, **the two-stage capacity bootstrap for a hostless child**. Once the EKS API and Phase-44 base node are
...
and capacity-debited into the base node's CRI store by Phase 44, never the not-yet-ready child registry or a
...
```mermaid
flowchart LR
  p34["Phase 44: ready Managed Eks control plane + base node, scheduler OCI preloaded"]:::intent --> boot[/"Parent bootstrap Lease holder: create amoebius-capacity-scheduler pods=1"/]:::effect
...
> **Provider corpus split (by design).** `test/dhall/phase_45_provider_provision.dhall` and the
...
[Phase 44](phase_44_provider_deploy_checkpoint.md); the per-PV EBS / credential apparatus
...
`mut-46.1-drop-copy-executor` mutants) stays in [Phase 46](phase_46_provider_ebs_credential.md); the dynamic-node
/ quota / teardown-sweep apparatus (`test/dhall/phase_47_provider_over_quota.dhall`, the expected two-instance
...
- **Representative set (§M.7)** — the committed topology `test/dhall/phase_45_provider_provision.dhall`, restricted
  ...
  base managed node group (size 1, deployed by Phase 44), the child `amoebius-capacity-scheduler` (`pods=1`), the
...
`test/dhall/phase_45_provider_provision.dhall` plus the explicit committed service set). The bootstrap-ordering
...
handoff, and the no-op re-run against a Phase-44-deployed base node group; it does **not** provision a dynamic
extra node ([Phase 47](phase_47_provider_dynamic_nodes.md)), bind a per-PV durable EBS
([Phase 46](phase_46_provider_ebs_credential.md)), or assert the independent leak-free tag-sweep at teardown
([Phase 47](phase_47_provider_dynamic_nodes.md)) — the run tears the base provider stack down through Phase 44's
own deploy lifecycle, and the leak-free sweep witness is recorded **deferred to Phase 47**, never as proven here.
...
- [`cluster_lifecycle_doctrine.md §1`](../documents/engineering/cluster_lifecycle_doctrine.md#1-two-cluster-kinds-one-lifecycle-shape)
  ...
  mandatory capacity-scheduler role — as the child-side of Phase 44's cloud-keyed amoebic spawn, converging the
...
- [`image_build_doctrine.md §2`](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster)
  ...
  the exact OCI digest Phase 44 preloaded into the base node's CRI store, and every standard platform service is
...
add-on cutover, and managed-authority mint for a provider child), reusing the Phase-27 `Amoebius.Scheduler.*` role
and its two-stage bootstrap-cutover / execution-identity-admission machinery (target paths; not yet built)
**Blocked by**: Phase 44 gate (the ready `Managed Eks` control plane + base managed node group, with the pinned
amoebius base/scheduler OCI content preloaded and capacity-debited into the first node's CRI store); Phase 27 gate
(the `amoebius-capacity` scheduler role, the two-stage bootstrap cutover, and execution-identity admission this
sprint applies to a provider child); Phase 26 gate (the observe→diff→scoped-SSA→staged-enact reconciler the
cutover patches run under); Phase 25 gate (the multi-arch baked-binary base image the scheduler image is a digest
of) — all external earlier-phase prerequisites.
**Independent Validation**: against a Phase-44-deployed child, the parent bootstrap holder creates
...
- Creation of `amoebius-capacity-scheduler` with exact `pods=1`, whose Deployment references the **exact OCI digest
  preloaded and capacity-debited into the base node's CRI store by Phase 44** — never the not-yet-ready child
...
- The managed-authority mint: install the managed-node taint, execution-identity admission, and full exclusive
  ...
  the cutover.
- An in-file honesty note: this stages a *provider child* through the same two-stage cutover Phase 27 proved on a
...
**Blocked by**: Sprint 45.1; Phase 33 gate (the Deployment-`replicas=1` singleton and the mandatory reconciler
...
through the Phase-26 reconciler and consuming the Phase 25/28–32 platform-service manifests (target paths; not yet
built)
**Blocked by**: Sprint 45.2; [phase_33](phase_33_live_dsl_singleton.md) Sprint 33.4 (the admin REST surface —
...
typed manifests via the Phase-26 reconciler, with no Helm and no public-registry pull, so substrate equivalence
...
- An in-file honesty note: the standard set and its HA-capable shape are planned for self-managed clusters in
  Phases 25/28–32; convergence of that exact set on a hostless provider child is tested here for the first time.
...
**Status**: Planned
**Implementation**: `test/dhall/phase_45_provider_provision.dhall` (the committed gate topology, exercised here on
...
bring-up as a no-op (zero mutating cloud/K8s calls in the OS-boundary audit trail). `mut-45.1-public-pull` goes
red on the image-pull observer. The run emits a proven/tested/assumed ledger; the base provider stack is torn down
through Phase 44's deploy lifecycle, and the leak-free tag-sweep witness is recorded **deferred to Phase 47**.
...
ledger; the leak-free sweep deferred to Phase 47), `DEVELOPMENT_PLAN/README.md`.
...
- The gate over the committed representative set (§M.7): the provider-child bring-up + standard-service-convergence
  slice of `test/dhall/phase_45_provider_provision.dhall`, driven end-to-end — bootstrap scheduler readiness,
...
- A per-run proven/tested/assumed ledger recording: provider-child bootstrap + handoff + standard-HA convergence
  ...
  Phase 47, not asserted here** — skipping an applicable observation move marks that layer UNVERIFIED, never
  green.
...
4. Assert the run emits a proven/tested/assumed ledger recording the independent Lease-handoff sequence, the
   ...
   Phase 44's deploy lifecycle and the leak-free tag-sweep is recorded **deferred to Phase 47**, never asserted
   green here.
...
- `documents/engineering/testing_doctrine.md` — record the Phase 45 per-run ledger artifact and the explicit
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-runtime` provider-child singleton wiring
  (`Amoebius.Daemon.InClusterSingleton`) and `Amoebius.Cluster.ProviderBringUp` as Phase-45 design-first rows,
  each mapped to its owning doctrine; map the reused `Amoebius.Scheduler.*` role to its Phase 27 delivery and the
  reconciler to Phase 26.
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 45 → `linux-cpu` (parent) row with the `provider` (EKS)
  deploy target annotated as a target class, not a fifth hardware substrate.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase 45 row's status once the gate passes; link this document.
...
- [phase_47](phase_47_provider_dynamic_nodes.md) — dynamic node provisioning by signal and the leak-free provider
  gate (the independent teardown tag-sweep), layered on Phases 44/45/46
