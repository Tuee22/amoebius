# Phase 20: Live DSL deploy via the replicas=1 singleton
...
is design intent, never a tested amoebius result. This phase opens after the Phase 19 gate (Keycloak-owned
ingress) and runs on the **linux-cpu** substrate in **Register 3** — live infrastructure: the single-node
`kind` cluster of Phases 13–19, its standard platform-service stack (Phase 18) already reconciled by the
...
authored and proven **in-process, with no cluster**. Phase 20 adds the runtime residue: the in-cluster
...
in-namespace Sql) is deliberately deferred to Phase 21; the app here is trivial.
...
**Blocked by**: Phase 15 gate (the typed renderer + SSA reconciler — the singleton is itself a rendered,
applied object); Phase 17 gate (root Vault — the singleton is the in-cluster principal that operates Vault);
Phase 13 gate (the `kind` cluster + the host-daemon→singleton handoff the midwife begins).
...
1. The singleton manifest is a `Deployment replicas=1` with no PVC; the pod runs the spine and reports
   `/readyz` ready; a pod delete reschedules exactly one replacement with no state loss.
2. The reconcile loop runs one idempotent pass to convergence from a decoded spec and a re-run is a no-op; the
...
not yet built.
**Blocked by**: Sprint 20.1 (the running singleton + reconcile loop); Phase 18 gate (the standard
...
reconciles the standard platform stack plus a trivial single-service app to ready on the linux-cpu cluster; a
second invocation is a **no-op**; a teardown leaves **no leaked resources** (a postflight sweep is empty).
...
### Deliverables
- A positive deploy `.dhall` composing the standard platform-service stack (Phase 18) and a **trivial**
...
- The singleton's live reconcile of that spec: decode → capability-bind → render → SSA-apply → wait-to-ready,
  each edge a witnessed condition, with a re-run proven idempotent (no drift, no re-apply).
- A leak-free teardown obligation carried by the deploy fixture — a test-topology `.dhall` whose postflight
  sweep asserts every provisioned object was reclaimed.

### Validation
1. The `.dhall` brings the platform + trivial app up on the linux-cpu cluster; the app is reachable through the
   Phase-19 Keycloak-owned edge; a re-run is a no-op.
2. Teardown leaves no leaked resources (the postflight sweep is empty); the run records that the live
   apiserver admitted the rendered manifests.
...
validation-locus ledger, already proven in Registers 1–2); Phase 19 gate (the Keycloak-owned edge the deployed
...
tears down leak-free, then re-runs each Phase-6 negative fixture against the live deploy path and asserts each
still **fails at Gate 1 or Gate 2** (no fixture reaches the apiserver), and each positive fixture still
decodes; the run emits a **Register-3** proven/tested/assumed ledger naming the live substrate.
...
- The negative regression guard: the Phase-6 corpus (a bad PVC↔PV pairing, a Keycloak-bypassing open ingress, a
  ...
  live deploy path, each asserted to fail at Gate 1 or Gate 2 with its foreclosure locus annotated — **never
  re-establishing** the type discipline, only guarding that the deploy path inherits it.
- A **Register-3** proven/tested/assumed ledger recording the live-enforcement result (the apiserver admitted
  the rendered manifests) and marking the deferred surfaces — full app tenancy (Phase 21), and the
  cross-cluster gateway-migration correspondence (the multi-cluster phase) — as UNVERIFIED, never green.

### Validation
1. The positive `.dhall` brings the platform + trivial app up, the app is reachable through the Keycloak edge,
   and teardown leaves no leaked resources.
2. Every Phase-6 negative fixture is rejected before any binary acts on the cluster, the positive fixtures
   decode, and the ledger honestly classifies each foreclosure (no runtime-checked or deferred claim is
   reported as proven).
...
- [phase_15](phase_15_renderer_reconciler.md) — the typed renderer + SSA reconciler that renders and applies
  the singleton and its manifests
- [phase_18](phase_18_platform_services.md) — the standard platform-service stack the live `.dhall` deploys
- [phase_19](phase_19_keycloak_ingress.md) — the Keycloak-owned edge the trivial app routes through
- [phase_21](phase_21_app_tenancy.md) — the app-tenancy projection (namespace + ObjectStore + Sql) deferred
  from this phase
