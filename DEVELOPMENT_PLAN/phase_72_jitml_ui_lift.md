...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_71_jitml_lift_cuda.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 72.1: Bind the jitML training-to-ready-model UI adapter ⏸️](#sprint-721-bind-the-jitml-training-to-ready-model-ui-adapter-)
...
⏸️ Blocked pending Phase-71 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
owner-matched Phase-71 committed artifact, preserves command/work identity into the Phase-49 receipt shape,
...
replicas, the full sibling serving engine, and the same-flow Phase-71 train/commit chain remain UNVERIFIED.

## Phase Summary

This phase owns one adapter seam: the Phase-71 training/checkpoint/model contracts become typed UI workflow and
artifact ports consumed by the generic runtime and released through Phase 50. The linked Haskell adapter
...
as the `CommandId` supplied to Phase 61. That value remains the work-id in all native-CBOR training events.
Phase 49 projects a terminal event into an owner/command-keyed receipt containing the normalized training
...
accept it with `cabal test jitml-ui-lift-live-gate`. Split if the work changes Phase-71 training/commit,
reopens Phase-48 failover, adds a generic UI
...
- **Fresh authority and challenge.** After Keycloak/Envoy, at least two UI-server replicas, Redis, and the
  Phase-71 jitML workers are Ready, the harness obtains least-privilege sessions for tenant A's artifact owner,
...
- **Cross-pod receipt routing and repair.** The harness proves from edge/backend identity that the authenticated
  ...
  must recover the exact scoped receipt and terminal model handle from Phase 49 once, without sticky routing,
...
- **Honesty.** This gate tests one bounded linux-cuda workflow, one ready model, two readiness failures, one
  ...
  inherits Phase-71 training/commit and Phase-48 failover evidence without retesting either boundary or
...
Phase-71 trainer/accelerator demand, and the pinned fault window. A one-replica, sticky-only, pod-local receipt,
...
## Sprint 72.1: Bind the jitML training-to-ready-model UI adapter ⏸️
...
- One Dhall UI module released as Phase-51 content under the unchanged amoebius runtime image.
...
retained MinIO/Pulsar/Redis receipt routing, the full sibling serving engine, same-flow Phase-71 training and
...
- `documents/engineering/lift_and_compose_doctrine.md` — keep the sibling SPA a UX fixture and Phase-71's
...
- `DEVELOPMENT_PLAN/system_components.md` — register the jitML UI adapter and Dhall module under Phase 71.

## Related Documents

- [Development-plan standards](development_plan_standards.md)
- [Phase 49 — UI projection runtime](phase_49_ui_projection_runtime.md)
- [Phase 51 — atomic UI program release](phase_51_ui_program_release.md)
- [Phase 71 — jitML lift and CUDA](phase_71_jitml_lift_cuda.md)
