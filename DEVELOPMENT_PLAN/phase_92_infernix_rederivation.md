# Phase 92: The infernix inference core, re-derived
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_57_base_image_registry.md, DEVELOPMENT_PLAN/phase_81_determinism_jitcache.md, DEVELOPMENT_PLAN/phase_93_infernix_ui_rederivation.md, DEVELOPMENT_PLAN/phase_94_jitml_rederivation.md
...
- [Sprints](#sprints)
- [Sprint 92.1: Re-derive infernix behind one scoped artifact adapter ⏸️](#sprint-921-re-derive-infernix-behind-one-scoped-artifact-adapter-)
...
Blocked by redesigned Phase 91, its independent validation, and human promotion; every earlier
...
sibling source boundary is reviewed and resolved dynamically. Store operations use the Phase-70 three-tier content-addressed store;
commands and events use typed native CBOR over Pulsar; credentials remain Vault `SecretRef` names; named
engines resolve through the Phase-81 jit-build cache; CPU decode uses a deterministic pinned micro-model.
...
preserves it unchanged as the Phase-70 work-id in the canonical command payload and in every progress or
terminal event; it never derives a replacement from a pod, retry, run id, or artifact digest. Producer resends
retain the producer identity/sequence required by Phase 68, while consumer redelivery folds on that stable
...
raw provider handle. Phase 93 is the sole infernix-to-UI boundary; this phase owns no frontend, browser probe,
...
and buffer bounds, and complete CPU/memory/ephemeral demand to the inherited Phase-70 workflow envelope. Its
...
The bounded campaign must implement the thin locally re-derived adapter facade and submit it to the Phase-92 aggregate
...
**Depends on:** [Phase 91](phase_91_test_topology_live.md)
...
| Key | Contract |
|---|---|
| `Claim` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: one cohesive claim — *only a ready-pointer-committed, owned artifact can drive inference through amoebius services*. The re-derivation adds an ownership index the seed's adapter does not carry. Explicit exclusions: every layer named in `Residue` remains UNVERIFIED. |
...
## Sprint 92.1: Re-derive infernix behind one scoped artifact adapter ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 91](phase_91_test_topology_live.md) human approval
...
1. The pre-reset Python command is rejected and must not run. The future Haskell Phase-92 supporting suite must run on linux-cpu with
...
- `DEVELOPMENT_PLAN/phase_93_infernix_ui_rederivation.md` — remain the sole owner of infernix UI projection and
  interaction.
...
- [Phase 81 — determinism and JIT cache](phase_81_determinism_jitcache.md)
- [Phase 93 — infernix UI lift](phase_93_infernix_ui_rederivation.md)
- [Phase 94 — jitML lift](phase_94_jitml_rederivation.md)
