# Phase 28: Illegal-state corpus + validation-locus ledger
...
- [Sprints](#sprints)
- [Sprint 28.1: Exhaustive negative/positive corpus split by foreclosure locus ⏸️](#sprint-281-exhaustive-negativepositive-corpus-split-by-foreclosure-locus-)
- [Sprint 28.2: GADT-index compile-refusal cases (type-foreclosed layer) ⏸️](#sprint-282-gadt-index-compile-refusal-cases-type-foreclosed-layer-)
- [Sprint 28.3: QuickCheck property suite ⏸️](#sprint-283-quickcheck-property-suite-)
- [Sprint 28.4: The per-entry validation-locus ledger — the gate ⏸️](#sprint-284-the-per-entry-validation-locus-ledger--the-gate-)
...
non-Haskell source. This phase may cover only the Phase-26/27 pure loci and exact-owner deferrals. Capacity,
...
**Depends on:** [Phase 27](phase_27_gadt_decode_ir.md)
...
| `Command` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: `pb validate phase 28` is future public spelling only. Before current human approval of Phase 51, `pb` is inadmissible validation transport; the candidate must invoke the exact absolute source-bound Haskell executable directly from an authenticated, network-independent toolchain input. The Haskell verdict entry point remains `UNRESOLVED` and blocks validation. |
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 27; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
## Sprint 28.1: Exhaustive negative/positive corpus split by foreclosure locus ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: exact authored Haskell implementation paths have not been bound to this sprint.
**Blocked by**: [Phase 27](phase_27_gadt_decode_ir.md) human approval
...
- dhall-typecheck negatives (`illegal_dhall_typecheck_*`, must fail `dhall type`) are **exactly** the Haskell
  ...
  `owner_phase ≤ Phase-28`. Required members
  ...
  `ResourceEnvelope` ([§3.11](../documents/illegal_state/illegal_state_security.md#311-an-unsafe-workload-no-resource-limits-no-hardened-securitycontext); the Phase-26 `illegal_missing_resource_envelope` case), unbounded storage /
...
- gadt-decode negatives (`illegal_decode_*`, must pass `dhall type`, then decode-reject) are **exactly** registry
  ...
  `owner_phase ≤ Phase-28`; these exercise the Phase-27
  ...
  no such inhabitant and is pinned by the fifth compile-fail pair in Sprint 28.2.
  The declared-compute-headroom cases belong here rather than to the deferred set below, because both are
  cross-field checks the Phase-27 smart constructor owns: `illegal_decode_headroom_over_limit`, whose pad
...
- **Provisioning-deferred negatives are selected by tags, never section-number ranges.** A registry row with
  `validation_locus = provision-seal`, an `owner_phase` of `Phase-9` or `Phase-32`, and a `case_family` of
  `topology`, `capacity`, `storage`, `cache`, `accelerator`, or `capability-provision` carries no rejecting
  fixture in Phase 28 and is ledgered with that owner. The generated selection must include every current
  ...
  allocatable VRAM; and provider-expanded resource demand. Phase 9 owns the pure folds and Phase 32 owns the end-to-end post-bind
...
- **Per-Register-1-locus fixtures (disambiguation).** A catalog entry carrying more than one Register-1 locus
  ...
  `dhall-typecheck` < `gadt-decode`), but delivery follows each row's owner: Phase 28 supplies only rows owned
...
The pre-reset `None` claim is permanently invalid; Sprint 28.1 remains blocked and NOT VALIDATED.

## Sprint 28.2: GADT-index compile-refusal cases (type-foreclosed layer) ⏸️
...
Phase-26 honesty caveat routed here, since Dhall has no opaque types.
...
The pre-reset `None` claim is permanently invalid; Sprint 28.2 remains blocked and NOT VALIDATED.

## Sprint 28.3: QuickCheck property suite ⏸️
...
The pre-reset `None` claim is permanently invalid; Sprint 28.3 remains blocked and NOT VALIDATED.

## Sprint 28.4: The per-entry validation-locus ledger — the gate ⏸️
...
The truth-maker locus and delivery ownership stay separate: reached Dhall/GADT rows through Phase 28 are
...
[Phase 34](phase_34_render_manifest_oracles.md), and `live-effect` by the live band.
...
- Consumption of the Sprint-27.1 Haskell catalog enrichment and its run-local generated locus projection; Sprint 28.4 does not
...
- A ledger emitter that classifies each catalog entry into its earliest-sufficient truth-maker locus:
  ...
  (settled on emitted bytes in Phase 34), `live-effect` (settled only by a
...
- The independent Haskell locus oracle, which may render a diagnostic `locus_registry.tsv` only beneath
  ...
  provisioning-deferred set is a query over those Haskell-declared rows (`owner_phase ∈ {Phase-9, Phase-32}` plus the
...
3. The coverage assertion is green: every reached `dhall-typecheck` or `gadt-decode` row carries a
   ...
   topology/capacity/storage/cache/accelerator/capability-provision rows → Phase 9 or Phase 32,
   `rendered-artifact-oracle` → Phase 34, `live-effect` → the live band — without being reclassified.
...
- `documents/illegal_state/illegal_state_*.md` — annotate every themed entry with reader-facing
  ...
  owners; `rendered-artifact-oracle` → Phase 34; `live-effect` → the live band), and confirm the §5
  coverage matrix is exhausted for the Register-1 rows this phase owns.
- `documents/engineering/dsl_doctrine.md` — backlink §5's two gates to the exhaustive Phase-28 corpus and the
...
- `DEVELOPMENT_PLAN/README.md` — the human authority alone may change Phase-28 status after the redesigned gate and external approval; link this document.
...
- Phase 28 illegal-state ledger — the authored proven/tested/unverified account of this gate
...
- [phase_26](phase_26_dhall_schema_generation.md) — dhall-typecheck, the Dhall schema + positive corpus this phase extends
- [phase_27](phase_27_gadt_decode_ir.md) — gadt-decode, the GADT-indexed IR + decoder this corpus rides atop
...
- [phase_31](phase_31_capability_bind.md) — the post-bind provisioning/capability negatives selected from the
  registry as deferred from here
- [phase_34](phase_34_render_manifest_oracles.md) — the `rendered-artifact-oracle` locus this ledger points at
- [phase_66](phase_66_live_dsl_deploy.md) — the live band where the `live-effect` locus is discharged
