...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_51_ui_program_release.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 50.1: The immutable `Release` ledger (`releaseHash`) ⏸️](#sprint-501-the-immutable-release-ledger-releasehash-)
- [Sprint 50.2: The `Environment` ETag-CAS promotion pointer ⏸️](#sprint-502-the-environment-etag-cas-promotion-pointer-)
- [Sprint 50.3: The `PromotionGate` — promote-unverified→prod type-foreclosed ⏸️](#sprint-503-the-promotiongate--promote-unverifiedprod-type-foreclosed-)
- [Sprint 50.4: `RolloutPlan`/`RolloutPhase` readiness-gated apply + DB schema-migration phase (gate) ⏸️](#sprint-504-rolloutplanrolloutphase-readiness-gated-apply--db-schema-migration-phase-gate-)
...
its standard platform-service stack already reconciled by the Phase-44 control-plane daemon onto the Phase-39 retained
storage. It opens only after the Phase 44 gate (live DSL deploy via the Deployment-`replicas=1` control-plane daemon and
the SSA reconciler the `RolloutPlan` enacts on) and the Phase 48 gate (the three-tier content-addressed store
...
`releaseHash = sha256(resolved-deployment-dhall ‖ image-digests ‖ substrate-fp)`, written into the Phase-48
...
**`RolloutPlan`/`RolloutPhase`** apply on the Phase-44 in-cluster SSA reconciler: an ordered plan whose each
...
given: the `releaseHash` formula and the hash/pointer master registry are the Phase-48/Phase-59 store's
(consumed here as an opaque content-address protocol); the proven/tested/assumed evidence ledger the gate
reads is the contract owned by `testing_doctrine` (consumed here as an opaque witness; Phase 62 later automates
test-topology generation but is not a prerequisite); the Gateway-API canary weight-shift remains later-phase
work, and the cross-cluster/geo promotion boundary is exercised in Phase 53 — neither is part of this gate.
...
- **A live `Release`-ledger write produces a content-addressed `releaseHash`.** The key is recomputed by an
  ...
  to the oracle-pinned golden key, with the ledger entry immutable in the Phase-48 store.
...
- **A readiness-gated `RolloutPlan` applies its phases in order.** The phase-apply order is read from an
  ...
  **includes a DB schema-migration `RolloutPhase`** obeying `create-new → verified-migrate → retire-old`
  against the standing Phase-45 Postgres, whose retire phase never denotes byte destruction.
...
oracle-pinned fixtures named in Sprints 40.1–40.4 and **MUST turn red** on the committed seeded mutant
...
inside the Phase-44 control-plane daemon, so a pure `ReleaseExecutionDemand` expands the control-plane daemon's complete
...
- [`inforcespec_migration_doctrine.md §3`](../documents/engineering/inforcespec_migration_doctrine.md#3-the-dsl-exposes-no-destructive-verb--the-closed-storagemutation-union)
  ...
  denotes durable-byte destruction (Sprint 50.4).
- [`release_lifecycle_doctrine.md §1`](../documents/engineering/release_lifecycle_doctrine.md#1-no-external-cicd-control-plane--delivery-is-typed-composition-on-primitives-amoebius-owns)
  ...
  plane; the whole lifecycle is a handful of typed values composed over the Phase-44 reconciler and the
  Phase-48 store, with desired state recomputed from the immutable release's authenticated Dhall reference,
...
- [`app_vs_deployment_doctrine.md §3`](../documents/engineering/app_vs_deployment_doctrine.md#3-the-deployment-rules-surface--how-the-same-app-runs)
  ...
  `if prod then …` in an app spec and no rebuild between environments (realized by Sprint 50.2's app-bytes
  invariance).
...
- [`release_lifecycle_doctrine.md §5`](../documents/engineering/release_lifecycle_doctrine.md#5-rolloutplan--rolloutphase-the-readiness-gated-apply)
  — *`RolloutPlan`/`RolloutPhase`: the readiness-gated apply*: an ordered plan enacted by the Phase-44 SSA
  reconciler
...
## Sprint 50.1: The immutable `Release` ledger (`releaseHash`) ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `src/Amoebius/Release/Ledger.hs`,
`src/Amoebius/Release/ReleaseHash.hs`
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: at **Register 3** against the Phase-48 kind cluster's live MinIO store, a `Release`
...
- A `Release` value — `{ releaseHash, deploymentDhallRef, imageDigests, substrateFp }` — written as an
  append-only entry into the Phase-48 store (pointers → manifests → blobs), keyed by
  `releaseHash = sha256(resolved-deployment-dhall ‖ image-digests ‖ substrate-fp)`; the hash is consumed as
  the registered `releaseHash` class of the Phase-48/Phase-59 hash/pointer master table, not re-owned here.
...
### Validation
1. Execute at **Register 3** against the Phase-48 single-node kind cluster's live MinIO store, never an
...
## Sprint 50.2: The `Environment` ETag-CAS promotion pointer ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `src/Amoebius/Release/Environment.hs`,
`src/Amoebius/Release/Promote.hs`
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: this suite runs in **Register 3** against the live Phase-48 store, where
...
## Sprint 50.3: The `PromotionGate` — promote-unverified→prod type-foreclosed ⏸️
...
> validates the *live wiring* of that foreclosure through the Phase-48 store and the consumed per-run evidence
...
## Sprint 50.4: `RolloutPlan`/`RolloutPhase` readiness-gated apply + DB schema-migration phase (gate) ⏸️
...
enact a satisfied promotion as an ordered, readiness-gated `RolloutPlan` on the Phase-44 SSA reconciler — with
...
### Deliverables
- `RolloutPlan = [RolloutPhase]` where each `RolloutPhase` carries `{ phaseObjects, phaseGate }`: the desired slice this phase applies and the readiness condition observed from live state — rollout complete, `Ready`, or CR `status` healthy, never a `threadDelay` — that gates the next phase. Enacted by the Phase-44 SSA/ApplySet engine ([`manifest_generation_doctrine.md §5`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions));
  it introduces **no new reconciler**.
- A **DB schema-migration `RolloutPhase`** against the standing Phase-45 Postgres obeying
...
- **Pre-runtime evidence pinned in this phase's oracle-pinning sprint:** the committed
  ...
  **red** — `mutant/gate-admits-unverified` (Sprint 50.3; the phase-level mandated mutant — the promotion that
...
1. Run the gate topology end-to-end on the linux-cpu kind cluster and assert, **live**: (a) the `Release`
   ledger write emits a `releaseHash` byte-equal to the committed golden (Sprint 50.1, independently
   recomputed); (b) `release_unverified → Prod` is **refused with `PromotionRefused:RuntimeEvidenceMissing`**
   and the `Prod` pointer HEAD is unchanged in the store (Sprint 50.3); (c) `release_verified → Prod`
   **advances the ETag-CAS pointer** (Sprint 50.2), observed from the store's pointer history; (d) the
...
5. Assert the run emits and externally attests a generated **proven/tested/assumed ledger** naming its register (3) and substrate
   ...
   **cross-cluster/geo promotion and the Gateway-API canary weight-shift layers UNVERIFIED** (deferred to
   Phase 53 and a later phase, respectively); skipping an applicable move marks that layer UNVERIFIED, never
   green.
...
> their proof rides a later phase and Phase 53, respectively. Pulumi cloud-IaC (tier a) and the host spot-fleet
> reconciler
...
- `documents/engineering/release_lifecycle_doctrine.md` — record that §2 (the `releaseHash` release ledger),
  ...
  promotion boundary explicitly still deferred (later phase / Phase 53); flip the Phase-0 reference-only honesty
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-50 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 50's gate substrate (linux-cpu) in the per-phase substrate
  map.
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-release` package and its target module paths
  ...
  `SchemaMigration`), mapped to the owning release-lifecycle doctrine, as Phase-50 design-first rows.
...
- [phase_44](phase_44_live_dsl_deploy.md) — the live DSL deploy via the `replicas=1` control-plane daemon and the SSA
  reconciler the `RolloutPlan` enacts on
- [phase_48](phase_48_content_store_workflow.md) — the three-tier content-addressed store the `releaseHash`-keyed
  ledger writes into
