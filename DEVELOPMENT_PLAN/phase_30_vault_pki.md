# Phase 30: Root Vault + PKI + built-in Haskell Vault client
...
Phase 30 delivers the root Vault + PKI + built-in Haskell Vault client; its design is owned by [vault_pki_doctrine.md](../documents/engineering/vault_pki_doctrine.md), [platform_services_doctrine.md](../documents/engineering/platform_services_doctrine.md), [resource_capacity_doctrine.md](../documents/engineering/resource_capacity_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 30.1: Root single-node password-encrypted Vault — init-once / unseal-on-rebuild ✅](#sprint-301-root-single-node-password-encrypted-vault--init-once--unseal-on-rebuild-)
- [Sprint 30.2: The self-signed PKI trust anchor issues ✅](#sprint-302-the-self-signed-pki-trust-anchor-issues-)
- [Sprint 30.3: Built-in Haskell Vault client (no agent sidecar) reads a `SecretRef` by name — the gate ✅](#sprint-303-built-in-haskell-vault-client-no-agent-sidecar-reads-a-secretref-by-name--the-gate-)
- [Sprint 30.4: Register-2.5 fail-closed Vault unseal under simulated faults ✅](#sprint-304-register-25-fail-closed-vault-unseal-under-simulated-faults-)
...
⏸️ Blocked pending Phase-29 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
Blocked (superseded) — reopened 2026-08-16 behind the amended Phase-25 handoff and Phase-29 revalidation. Its prior capability record remains historical until the exact predecessor chain is resealed.
...
`python3 tools/vault_pki_gate.py --execute` verified the exact Phase-29 predecessor and Phase-25 image
...
a single-node `kind` cluster on the **linux-cpu** lane, exact Phase-25 OCI archive, and Phase-29 retained ext4
...
(Phases 31–32), the Keycloak-owned edge (Phase 33), and the parent/child unseal modes, parent secret injection, and the
...
## Sprint 30.1: Root single-node password-encrypted Vault — init-once / unseal-on-rebuild ✅
...
### Deliverables
- Root Vault in **Shamir seal mode**, rendered and reconciled onto the Phase-29 retained PV; first-ever `vault
...
- A **pluggable unlock-material backend** behind one interface — the load-bearing property is only that the
  material is password-AEAD-sealed and never plaintext at rest. **At the root Phase-30 bring-up the backend is the host-side `.age` file**: MinIO does not exist until Phase 31, so a MinIO-sealed object (and equally a cloud
...
## Sprint 30.2: The self-signed PKI trust anchor issues ✅
...
- The **three-planes distinction** recorded and enforced: internal PKI (this phase) is not public-edge TLS
  (ZeroSSL/route53, Phase 33) and is not the distro's own self-signed cluster CA (the chicken-and-egg floor,
...
## Sprint 30.3: Built-in Haskell Vault client (no agent sidecar) reads a `SecretRef` by name — the gate ✅
...
**Docs to update**: `documents/engineering/vault_pki_doctrine.md`,
`documents/engineering/testing_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-30 status when the
...
## Sprint 30.4: Register-2.5 fail-closed Vault unseal under simulated faults ✅
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Vault/{Init,Unseal,Seal,Pki,Client,SecretRef,Error}.hs`
  as Phase-30 design-first rows against the component inventory.
...
- [phase_27](phase_27_object_reconciler.md) — the typed renderer + SSA reconciler that renders and applies Vault
- [phase_29](phase_29_retained_storage.md) — the no-provisioner retained PV Vault's durable KV lives on
- [phase_31](phase_31_platform_backbone.md) — the standard-service stack that consumes these Vault secrets
