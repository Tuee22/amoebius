# Phase 22: Deterministic-simulation substrate
...
- [Sprints](#sprints)
- [Sprint 22.1: The `io-classes` `Env` effect interface + its two interpreters + `sim-spec` skeleton ✅](#sprint-221-the-io-classes-env-effect-interface--its-two-interpreters--sim-spec-skeleton-)
- [Sprint 22.2: The modeled fault-injectable substrates + the per-fake fault-contract corpus ✅](#sprint-222-the-modeled-fault-injectable-substrates--the-per-fake-fault-contract-corpus-)
- [Sprint 22.3: The deterministic-replay battery — same-seed determinism + schedule-sensitivity + fault-mutant — the gate ✅](#sprint-223-the-deterministic-replay-battery--same-seed-determinism--schedule-sensitivity--fault-mutant--the-gate-)
...
bound to source snapshot `sha256:4a7809fb4d96d811…`; Phase 22 owns no remaining migration deferral.
...
reddens at `NoActOnStaleRead`. Evidence and the ledger move into `.build/runs/phase_16/<run-id>/`.
...
run. What is *not* here: the boundary fake-tool harness (Phase 21, which this phase's Sprint 22.1 generalizes),
...
Phase-22 ledger records the exact tested/model-proven boundary
...
## Sprint 22.1: The `io-classes` `Env` effect interface + its two interpreters + `sim-spec` skeleton ✅
...
**Docs to update**: `documents/engineering/deterministic_simulation_doctrine.md` (Phase-22 status
...
test (`m = IOSim s`) from one source, generalizing the Phase-21 single IO seam.
...
- `src/Amoebius/Sim/Env.hs`: the typed `Env m` effect interface (publish/consume, put/get-blob, apply-object,
  ...
  clock and the seed seams the determinism kernel ([phase_59](phase_59_determinism_jitcache.md)) also uses — one
...
## Sprint 22.2: The modeled fault-injectable substrates + the per-fake fault-contract corpus ✅
...
## Sprint 22.3: The deterministic-replay battery — same-seed determinism + schedule-sensitivity + fault-mutant — the gate ✅
...
**Docs to update**: `DEVELOPMENT_PLAN/README.md` (flip the Phase-22 status
...
- `test/spec/sim/SimSpec.hs` asserting: same-seed → byte-identical trace; a distinct-seed / distinct-schedule run
  yields a **different** trace ([§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub)-6); the named per-fake fault contracts fire ([§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub)-7, from Sprint 22.2); and the
...
- `documents/engineering/deterministic_simulation_doctrine.md` — backlink §2/§3/§4 to the in-process Phase-22
...
- `documents/engineering/conformance_harness_doctrine.md` — backlink §2's Register-2.5 deterministic-simulation
  entry to the in-process Phase-22 substrate that serves it.

**Cross-references to add:**
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-22 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — the Phase-22 `none` gate row.
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Sim/Env.hs`,
  `src/Amoebius/Sim/Interp/{Real,Sim}.hs`, `src/Amoebius/Sim/Fakes/`, and the `sim-spec` test-suite as Phase-22
  design-first rows.
...
- [phase_21](phase_21_chain_kernel_boundary.md) — the `chain`/`Step` kernel + `--dry-run` plan the toy reconcile
  loop consumes
- [phase_21](phase_21_chain_kernel_boundary.md) — the boundary fake-tool harness whose single IO seam Sprint
  12.1 generalizes into the typed `Env m` effect interface
- [phase_59](phase_59_determinism_jitcache.md) — the determinism kernel that shares the seed / `MonadTime` seams
