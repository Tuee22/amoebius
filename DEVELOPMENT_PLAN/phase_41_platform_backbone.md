# Phase 41: Platform backbone (MetalLB + MinIO + Pulsar HA)
...
- [Sprints](#sprints)
- [Sprint 41.1: MetalLB LoadBalancer + MinIO object substrate + registry S3-driver rehoming 🔄](#sprint-411-metallb-loadbalancer--minio-object-substrate--registry-s3-driver-rehoming-)
- [Sprint 41.2: Pulsar native-protocol backbone + size-triggered S3 offload drill ⏸️](#sprint-412-pulsar-native-protocol-backbone--size-triggered-s3-offload-drill-)
- [Sprint 41.3: The backbone HA bring-up gate ⏸️](#sprint-413-the-backbone-ha-bring-up-gate-)
...
Blocked (superseded) — Phase 41's 2026-08-16 live validation proved that the Phase-36 image omitted Pulsar's separately published S3 offloader bundle. Phase 36 is reopened; Phases 37–40 must be revalidated in numerical order before this phase resumes. The registry executable correction and private-fixture containment hardening remain implemented.

**Superseded active record:** opened 2026-08-16 after the amended Phase-40 Vault/PKI gate sealed as
...
Phase-36 image handoff and exact Phase-40 predecessor, confines every generated/live byte to `.build/**` or
the marker-owned `.test_data/**` run, and reruns the full Register-3 contract before Phase 42 opens.
...
Phase-36 image observation. It must be removed. Image identity remains a live provenance check against the
verified Phase-36 attestation and current in-cluster registry catalog, not a committed digest file.
...
`Amoebius.Platform.Backbone.renderBackbone` result. Every runtime image ID matched the Phase-36 baked digest,
zero public pull was observed, all six seeded mutants went red, and the honesty ledger is
`dynamically-resolved`.

## Phase Summary

This phase turns the storage-and-secrets-provisioned cluster of Phase 40 into an amoebius cluster carrying the
...
Phase-37 `renderAll` path (no Helm, no third-party charts, and the emitted manifests are generated from Haskell
and never committed), each served from binaries **baked into the native-architecture base image** with no
public-registry pull, and each on the Phase-39 `no-provisioner` retained PVs where it holds durable state.

Two deferred gaps from earlier phases close here. First, the **registry→MinIO S3-driver rehoming**: the
Phase-36 `distribution` registry ran against **interim node-local (filesystem-driver) blob storage** with the
MinIO S3 driver named as this phase's target; Phase 41 moves the registry's blob store onto MinIO via the S3
...
registry's capacity proof. Phase 41 preserves the artifact/upload
operands from Phase 36's canonical `RegistryStorageDemand` — every digest-keyed compressed
...
erasure/healing geometry. Every pre-existing Phase-36 artifact is copied and independently digest-verified
...
clusters, pgAdmin, Prometheus/Grafana, and the **full derived readiness-DAG bring-up of the whole standard stack** are [Phase 42](phase_42_platform_services_2.md). The **Keycloak-owned ingress edge** — Envoy/Gateway
API terminating TLS and Keycloak owning all wild ingress so no workload publishes its own path — is
[Phase 43](phase_43_keycloak_ingress.md); this phase brings the backbone up behind no public edge. The
Deployment-`replicas=1` control-plane daemon that will eventually *own* this reconcile loop is
[Phase 44](phase_44_live_dsl_deploy.md); here the reconciler is driven from the operator/host path against a
...
```
*Orientation. Phase 41 converges one retained MinIO backing for registry/content/offload while Pulsar keeps its bounded hot path through BookKeeper, as owned by the [platform-services doctrine](../documents/engineering/platform_services_doctrine.md).*

**Gate integrity ([§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub)).**
The gate is closed to a stub by seven cross-checks named in the Sprint 41.1–31.3 deliverables. Existing
...
The apparatus phase 41's gate closes over, in the slot
...
get pods -A -o jsonpath={..imageID}` over the amoebius-rendered namespaces) MUST equal the Phase-36 baked
...
logging), covering the whole gate window. The accepted digest is read from the verified Phase-36 attestation
...
`renderAll` output — pinning applied manifests to the Phase-37 renderer, so hand-written or `helm
...
observed externally: every digest in a Phase-36 preexisting artifact is copied to its exact target object,
...
upload/orphan witness to match Phase 36. It also independently derives the private
...
Phase 36) is present as a rehoming consumer of MinIO, not re-delivered here.
...
- [`platform_services_doctrine.md §1 — the invariant: every cluster is the same cluster`](../documents/engineering/platform_services_doctrine.md#1-the-invariant-every-cluster-is-the-same-cluster)
  ...
  Phase 41 materializes the backbone slice of the fixed standard service set on linux-cpu, each service the
...
- [`platform_services_doctrine.md §4 — MinIO, the object substrate`](../documents/engineering/platform_services_doctrine.md#4-minio--the-object-substrate),
  ...
  (the MetalLB LoadBalancer half — the Keycloak edge is Phase 43): the concrete providers Phase 41 stands up
...
- [`image_build_doctrine.md §2 — the single distribution rule: bake the binaries, build the amoebius image, pull only in-cluster`](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster)
  ...
  every backbone binary is baked into the Phase-36 native-architecture base image and resolved only in-cluster; the
  registry stores its blobs in MinIO via the S3 driver, so MinIO must be serving before the registry — the
  thin ordering edge [§9](../documents/engineering/image_build_doctrine.md#9-bring-up-ordering--the-registry-chicken-and-egg-dissolves) names, and the rehoming Phase 41 delivers.
- [`manifest_generation_doctrine.md §5 — the apply/reconcile engine: snapshot-bound typed actions`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  Phase 41 reuses the Phase-37 pure `renderAll :: ProvisionedSpec -> [K8sObject]` and typed-action reconciler whose **wait-for-ready is observed from the live object, never a `threadDelay`** to apply and sequence the backbone. - [`platform_services_doctrine.md §10 — every execution unit declares its complete resource envelope`](../documents/engineering/platform_services_doctrine.md#10-every-execution-unit-declares-its-complete-resource-envelope)
  ...
  witness are unchanged from Phase 36 — including compressed objects,
...
- [`storage_lifecycle_doctrine.md §2 — one storage class, and it provisions nothing`](../documents/engineering/storage_lifecycle_doctrine.md#2-one-storage-class-and-it-provisions-nothing)
  ...
  each stateful backbone service (MinIO, Pulsar's ZooKeeper members and bookies) lands its durable bytes on the Phase-39
...
> **Current revalidation rule.** Sprint 41.1 is active; later sprints remain blocked by their preceding sprint. Historical dates,
...
## Sprint 41.1: MetalLB LoadBalancer + MinIO object substrate + registry S3-driver rehoming 🔄
...
**Blocked by**: [Phase 40](phase_40_vault_pki.md) gate.
...
with every Phase-36 digest copied and still pullable, the independent MinIO capacity corpus, and the exact
...
rehome the Phase-36 `distribution` registry's blob store off the interim node-local filesystem driver onto the
MinIO S3 driver — closing the Phase-36 deferred gap.

### Deliverables
- MetalLB rendered as a standard service that publishes a LoadBalancer address on the kind node, available
  before the (Phase 43) Envoy/Gateway edge needs one — the backbone's LoadBalancer root.
- MinIO (HA / distributed) as the S3 object substrate on Phase-39 retained PVs, holding the content store,
...
- The `distribution` registry's blob store rehomed onto the MinIO S3 driver — the registry holds no PV of its
  ...
  witness exactly match Phase 36's interim provision and whose
  ...
  observer sees it.
- A `RegistryBackendMigrationDemand` from the still-live Phase-36 private provision to the MinIO replacement.
...
### Validation
1. Apply through the Phase-37 reconciler; assert MetalLB advertises an address and MinIO reaches its Ready
...
3. Assert the registry is serving from the MinIO S3 driver: its storage config is byte-equal to the committed
   ...
   a pre-existing Phase-36 artifact was copied/verified and that artifact still pulls by the same digest, and a
   ...
   Compare the Phase-36 and Phase-41 private registry witnesses: `objectSet`, structured `objectStorePeak`,
...
5. Assert every app/init/sidecar container and volume in the `renderAll`-emitted MetalLB/MinIO/registry objects
   ...
   logging) records no public-registry pull and every running `imageID` digest resolves to the Phase-36 baked
...
## Sprint 41.2: Pulsar native-protocol backbone + size-triggered S3 offload drill ⏸️

**Status**: Blocked by Sprint 41.1; prior capability footprint retained for migration
**Implementation**: `src/Amoebius/Platform/Pulsar.hs`, `tools/phase30_backbone_live.py`
**Blocked by**: Sprint 41.1.
**Independent Validation**: the numbered Validation list below, which needs only the Sprint-36.1 MinIO
...
- Bounded per-topic retention with a **size-triggered MinIO offload** (no unbounded storage, no time-only
  trigger), wired to the Sprint-36.1 MinIO substrate; the drill topic's hot-tier size cap committed in
...
2. Produce then consume a message; assert an at-least-once round-trip. Dedup is **exercised**: inject a
   ...
   byte-for-byte (full native-client proof deferred to Phase 46, marked UNVERIFIED here).
...
## Sprint 41.3: The backbone HA bring-up gate ⏸️

**Status**: Blocked by Sprint 41.2; prior capability footprint retained for migration
...
and [`image_build_doctrine.md §9 — bring-up ordering`](../documents/engineering/image_build_doctrine.md#9-bring-up-ordering--the-registry-chicken-and-egg-dissolves):
assemble the Sprint 41.1–27.2 backbone services, bring them up event-driven behind the reconciler's
...
- A backbone bring-up that applies MetalLB, MinIO, the rehomed `distribution` registry, and Pulsar through the
  Phase-37 reconciler, each dependent starting on its dependency's observed-ready edge (MinIO serving before
...
- The phase-gate harness: bring the whole backbone up on a fresh single-node linux-cpu `kind` cluster and
  ...
  Postgres/observability (Phase 42), and control-plane-owned reconcile layers UNVERIFIED; the independent
  ...
  and the `mutant/dag-drop-edge` mutant, is [Phase 42](phase_42_platform_services_2.md)'s.
...
3. Recompute `storage-geometry-boundaries.csv` with the independent checker and compare it with the opaque
   ...
   replacement `objectSet`, `derivedPeak`, and upload/orphan witness exactly match Phase 36 while its backend
...
4. Assert the whole backbone is up and HA-shaped from generated manifests + baked-binary images. "HA-shaped"
   ...
   **and** every running container's `imageID` digest (`kubectl get pods -A -o jsonpath={..imageID}`) MUST
   equal the digest in the verified Phase-36 attestation and current in-cluster `distribution` catalog — any
   ...
   layer *tested* not *proven*, with the Keycloak edge, Phase-42 Postgres/observability, and control-plane-owned
   reconcile marked UNVERIFIED.

### Remaining Work
Remove `test/fixture/phase30/expected-base-digest.txt`, pass the verified Phase-36 digest into the run without
...
- `documents/engineering/image_build_doctrine.md` — the §9 "registry stores its blobs in MinIO via the S3
  driver" edge is delivered; the Phase-36 interim filesystem-driver residue is discharged, recorded as the
  Phase-41 rehoming.
- `documents/engineering/pulsar_client_doctrine.md` — the §6.1 size-triggered offload bound gains its first
  live drill (the platform-side bring-up); the native-client CBOR round-trip proof stays owned by Phase 45.
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-41 status when the gate passes and link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 41's gate substrate (linux-cpu) in the per-phase substrate map.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the Phase-37 renderer + SSA wait-for-ready that applies and sequences the backbone
...
- [phase_36](phase_36_base_image_registry.md) — the base image + `distribution` registry whose interim filesystem-driver blob store this phase rehomes onto MinIO
- [phase_40](phase_40_vault_pki.md) — the root Vault/PKI whose unseal edge gates Pulsar's secret-dependent startup here
- [phase_42](phase_42_platform_services_2.md) — the Percona/Patroni + pgAdmin + observability services and the full derived readiness-DAG gate that build on this backbone
- [phase_43](phase_43_keycloak_ingress.md) — the Keycloak-owned ingress edge that fronts this backbone next
- [phase_46](phase_46_pulsar_client.md) — the native `amoebius-pulsar` client whose full CBOR round-trip proof this phase defers
