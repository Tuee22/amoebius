# Phase 73: The complementary-architecture base image
...
> **Read this if**: phase 73 is next in the queue, or a later phase pulls the complementary architecture's tag.

Phase 73 delivers the complementary architecture of the base image, published beside
[Phase 36](phase_36_base_image_registry.md)'s under its own tag; its design is owned by
...
Phase 36's and are consumed unchanged. Register 3, live, on the `apple` substrate's `linux-cpu/arm64` lane.
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_36_base_image_registry.md
...
- [Sprints](#sprints)
- [Sprint 73.1: The complementary-architecture bake on its own hardware ⏸️](#sprint-731-the-complementary-architecture-bake-on-its-own-hardware-)
- [Sprint 73.2: The architecture-qualified publication and its atomic advertisement ⏸️](#sprint-732-the-architecture-qualified-publication-and-its-atomic-advertisement-)
- [Sprint 73.3: The no-emulation and unattested-child negatives ⏸️](#sprint-733-the-no-emulation-and-unattested-child-negatives-)
...
⏸️ Blocked pending Phase-72 revalidation. Authored 2026-08-16 by the natural-architecture amendment, which
split the former single multi-architecture claim: no host may build the half it cannot execute, so the second
architecture becomes its own phase on its own substrate
...
Phase 36 leaves the cluster pulling only from itself, at one architecture. This phase supplies the other one.

It bakes the **same typed catalog** on a host whose natural architecture is the complement of Phase 36's,
...
What this phase deliberately does not do is re-derive Phase 36's work. The acquisition ladder, the bake
inventory oracle, the registry standup, the mutation-admission proxy, and the egress boundary are consumed as
Phase 36 sealed them. This phase adds one architecture, one publication, and the negatives that keep the tag honest.
...
**not** exercised here — that is [Phase 74](phase_74_apple_metal_host_daemon.md)'s — so naming `apple` claims
the host, not the accelerator. Any host natively running `arm64` Linux satisfies the same requirement.

**Lane:** linux-cpu/arm64 — the complement of Phase 36's `linux-cpu/amd64`.
...
```
*Design intent. Phase 73's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses and [`substrate_doctrine.md` §1.1](../documents/engineering/substrate_doctrine.md#11-the-natural-architecture-rule) the rule they enforce.*

**What the gate command accepts.** On a host whose natural architecture is `arm64`, the typed catalog bakes
that architecture's child, every baked binary executes natively by absolute path against the Phase-36 oracle's
pinned probes, and the child's attestation is written. The gate then joins that child with the Phase-36 child
...
- `test/mutant/complementary_arch/emulated-build` — the complementary image produced under a mounted
  foreign-architecture interpreter; red at Sprint 73.1's kernel-read `binfmt` negative.
- `test/mutant/complementary_arch/stub-arm64-binary` — a zero-byte binary at a baked path in the `arm64` image;
  red at Sprint 73.1's native execution and ELF check. This is the pre-amendment Phase-36 mutant, re-homed to
  the phase whose gate can now execute it.
- `test/mutant/complementary_arch/foreign-attestation` — the `arm64` image paired with the `amd64` build's
  attestation; red at Sprint 73.2's architecture binding.
- `test/mutant/complementary_arch/unattested-image` — an image pushed with no attestation at all; red at
  Sprint 73.2's admission check.
- `test/mutant/complementary_arch/advertise-before-upload` — the tag advertised while the upload is still
  partial; red at Sprint 73.2's registry-API un-advertised assertion.
- `test/mutant/complementary_arch/divergent-catalog` — the two architectures baked from different catalog
  digests; red at Sprint 73.3's same-catalog reconciliation.
...
- [`substrate_doctrine.md` §1.1 — the natural-architecture rule](../documents/engineering/substrate_doctrine.md#11-the-natural-architecture-rule):
  ...
  on a separate host rather than as a second platform argument to Phase 36's build.
- [`image_build_doctrine.md` §3 — one image per architecture](../documents/engineering/image_build_doctrine.md#3-multi-architecture-images--one-natively-built-child-per-architecture):
  the architecture is in the tag and nothing joins the two, so this phase publishes beside Phase 36 rather
  than on top of it.
...
## Sprint 73.1: The complementary-architecture bake on its own hardware ⏸️

**Status**: Blocked by the Phase-36 gate, which produces the child this sprint complements.
**Implementation**: `tools/complementary_arch_build.py`, `src/Amoebius/Image/Attestation.hs`
**Blocked by**: [Phase 36](phase_36_base_image_registry.md)'s gate.
...
baked binary executes natively by absolute path against Phase 36's committed
...
## Sprint 73.2: The architecture-qualified publication and its atomic advertisement ⏸️

**Status**: Blocked by Sprint 73.1.
...
## Sprint 73.3: The no-emulation and unattested-child negatives ⏸️

**Status**: Blocked by Sprint 73.2.
...
- `documents/engineering/substrate_doctrine.md` — the `apple` substrate's `linux-cpu/arm64` lane gains its
  first live result, while the Metal lane stays UNVERIFIED until Phase 74.

**Cross-references to add:**
- `DEVELOPMENT_PLAN/substrates.md` — the per-phase map row for this phase.
- `DEVELOPMENT_PLAN/phase_36_base_image_registry.md` — the predecessor whose child this phase joins.

## Related Documents
- [Phase 36](phase_36_base_image_registry.md) — the catalog, ladder, registry, and first child this phase consumes
- [Phase 37](phase_37_object_reconciler.md) — runs on the native-architecture child alone; the joined index is published after it, so no phase before this one consumes the join
