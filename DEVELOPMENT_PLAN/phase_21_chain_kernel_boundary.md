# Phase 21: chain/Step kernel + `--dry-run` + boundary fake-tool harness + extension-astcheck AST checker
...
Phase 21 delivers the chain/Step kernel + `--dry-run` + boundary fake-tool harness; its design is owned by [dsl_doctrine.md](../documents/engineering/dsl_doctrine.md), [conformance_harness_doctrine.md](../documents/engineering/conformance_harness_doctrine.md), [generated_artifacts_doctrine.md](../documents/engineering/generated_artifacts_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 21.1: The `Step` algebra + `chain :: cfg -> \[Step\]` builder ✅](#sprint-211-the-step-algebra--chain--cfg---step-builder-)
- [Sprint 21.2: The pure descent — `nextFrameAfter` / `foldLift` (golden-locked) ✅](#sprint-212-the-pure-descent--nextframeafter--foldlift-golden-locked-)
- [Sprint 21.3: `renderChainPlan` / `--dry-run` byte-for-byte render (no live infra) ✅](#sprint-213-renderchainplan----dry-run-byte-for-byte-render-no-live-infra-)
- [Sprint 21.4: The plan-render golden battery (`chain-spec`) — the Part-A gate ✅](#sprint-214-the-plan-render-golden-battery-chain-spec--the-part-a-gate-)
- [Sprint 21.5: The single typed subprocess seam + `boundary-spec` skeleton ✅](#sprint-215-the-single-typed-subprocess-seam--boundary-spec-skeleton-)
- [Sprint 21.6: The fake `kubectl`/`helm`/`docker`/`pulumi` recorders ✅](#sprint-216-the-fake-kubectlhelmdockerpulumi-recorders-)
- [Sprint 21.7: The boundary battery — exact commands + applied bytes + no-`PATH` — the Part-B gate ✅](#sprint-217-the-boundary-battery--exact-commands--applied-bytes--no-path--the-part-b-gate-)
- [Sprint 21.8: The sanctioned-API surface — what extension source may reach ✅](#sprint-218-the-sanctioned-api-surface--what-extension-source-may-reach-)
- [Sprint 21.9: extension-astcheck — the extension AST checker and the link seal ✅](#sprint-219-extension-astcheck--the-extension-ast-checker-and-the-link-seal-)
...
kernel and Part-B boundary suites are green, the extension-astcheck AST checker holds its compile-fail seal, every seeded
...
`sha256:d3d92e8c10c59625…`; Phase 21 owns no remaining migration deferral.
...
The Phase-21 ledger records the exact tested and UNVERIFIED
boundary.
...
live SSA reconciler that replaces the fakes ([phase_37_object_reconciler.md](phase_37_object_reconciler.md)), and
the runtime-enforcement claim that a cluster admits what the fakes accepted, exercised against the live
Deployment-`replicas=1` control-plane daemon ([phase_44_live_dsl_deploy.md](phase_44_live_dsl_deploy.md)) — the
Tier-2 residue this two-register gate leaves UNVERIFIED by construction. The deterministic-simulation activity
that this boundary harness unblocks lives in [phase_22_deterministic_sim_substrate.md](phase_22_deterministic_sim_substrate.md).

**Substrate:** none — no host, no cluster, no live effectful interpreter; the gate is an in-process
`cabal test chain-spec` plan-render battery (Part A, analogous to the Phase-20 rendered-output goldens) plus an
...
**Gate:** `python3 tools/chain_boundary_gate.py` passes the Part-A, Part-B, extension-astcheck,
compile-fail, network-observer, mutant, and ledger checks. The committed
Phase-21 ledger records the exact commands, oracle coverage,
...
pinned at the start of Phase 21 before the executor — the §M.1 named exception).

```mermaid
flowchart LR
  %% register: orientation
  s0["Sprint 21.1: The Step algebra + chain :: cfg -> [Step] builder"]
  s1["Sprint 21.2: The pure descent — nextFrameAfter / foldLift (golden-locked)"]
  s2["Sprint 21.3: renderChainPlan / --dry-run byte-for-byte render (no live infra)"]
  s3["Sprint 21.4: The plan-render golden battery (chain-spec) — the Part-A gate"]
  s4["Sprint 21.5: The single typed subprocess seam + boundary-spec skeleton"]
  s5["Sprint 21.6: The fake kubectl/helm/docker/pulumi recorders"]
  s6["Sprint 21.7: The boundary battery — exact commands + applied bytes…"]
  s7["Sprint 21.8: The sanctioned-API surface — what extension source may reach"]
  s8["Sprint 21.9: extension-astcheck — the extension AST checker and the link seal"]
  gate["the phase 21 gate"]
...
```
*Orientation. The seams phase 21 builds, in order; [Gate integrity](#gate-integrity) owns the apparatus. Not run.*
...
- **Cross-golden render identity (§M.3, independent oracle).** `chain` produces a pure `[Step]` value whose
  manifest-bearing steps are identity-selected subsets of the one **Phase-20** `renderAll` golden for the
  fixture's whole `ProvisionedSpec`; their disjoint identity union equals that complete object set byte-for-byte,
  and no Step invokes a per-service renderer. Both cfg fixtures are the same oracle-pinned fixtures Phase 20
  golden-locked, so a corresponding whole-deployment `renderAll` golden exists for each fixture's
  `ProvisionedSpec`. The reference side is Phase 20's independently committed output, not the kernel's own.
...
- **Oracle-pinning — the expected-argv transcript (§M.1 named exception, §M.3 independent oracle).** The
  ...
  at the **start of Phase 21 before the executor implementation** (the §M.1 named exception), authored at
...
- **Applied-manifest bytes (§M.3, cross-golden).** The applied-manifest bytes the fakes capture equal the
  **Phase-20** `renderAll` goldens and the **Part-A** plan goldens byte-for-byte — the same rendered value the
...
- **Committed mutants (§M.2), re-run every gate run.** `cabal test boundary-spec` turns **red** on each of three
  ...
  a Phase-20 `renderAll` golden), and **mB3** (`mB3_path_resolve`, a `PATH`-resolution mutant — the seam
...
- **No-`PATH` invariant via an external-observer trace (§M.5, OS-boundary observer).** The no-`PATH` invariant is
  ...
  **absent** from `PATH` and a decoy directory holding same-named sabotage executables (each writing a distinct
  ...
  site over all of `src/`: Phase 21 initially placed it in `src/Amoebius/Exec/Tool.hs`; Phase 35 moved the raw
...
- [`conformance_harness_doctrine.md §3 — rendering never touches live infrastructure`](../documents/engineering/conformance_harness_doctrine.md#3-the-load-bearing-invariant-rendering-never-touches-live-infrastructure)
  ...
  ([phase_37_object_reconciler.md](phase_37_object_reconciler.md)), never the render or boundary path.
...
- [`testing_doctrine.md §2 — the registers of amoebius testing`](../documents/engineering/testing_doctrine.md#2-the-registers-of-amoebius-testing)
  ...
  [phase_37_object_reconciler.md](phase_37_object_reconciler.md) and
  [phase_44_live_dsl_deploy.md](phase_44_live_dsl_deploy.md)); fail-fast, no skips — a missing fake or a
...
## Sprint 21.1: The `Step` algebra + `chain :: cfg -> [Step]` builder ✅
...
whole-deployment `renderAll` result; the disjoint union equals the Phase-20 golden byte-for-byte.
**Docs to update**:
...
- Each `Step`'s renderable shape embeds its identity-selected projection of the Phase-20 whole-deployment
...
2. The identity-disjoint union of all manifest-bearing Step projections equals the Phase-20 committed `renderAll`
...
## Sprint 21.2: The pure descent — `nextFrameAfter` / `foldLift` (golden-locked) ✅
...
### Deliverables
- Pure `nextFrameAfter :: Frame -> [Step] -> Maybe Frame` and `foldLift :: cfg -> [Step] -> Plan`, neither carrying `IO`, computing the frame/step assignment and the fold-derived plan with no action run. - The effectful `runChainFromFrame` **declared** as the single IO seam, with an in-file honesty note that its *invocation* is out of scope in Part A — Part B exercises it against fake tools (Sprints 14.5–14.7) and Register 3 against the live Deployment-`replicas=1` control-plane daemon ([phase_44_live_dsl_deploy.md](phase_44_live_dsl_deploy.md)); there is no election or standby anywhere in
  the kernel.
...
## Sprint 21.3: `renderChainPlan` / `--dry-run` byte-for-byte render (no live infra) ✅
...
- **The argv dispatch itself**, in `app/amoebius/Main.hs` and nothing else: argv selects a *verb*, and a verb
  ...
  `FrameConfig` Phase 35 mints, never inferred from the verb, the executable's filename, or the environment
...
## Sprint 21.4: The plan-render golden battery (`chain-spec`) — the Part-A gate ✅
...
observed. Both plans and descent assignments match their goldens; Step projections union to the Phase-20
...
- `test/spec/kernel/PlanSpec.hs` asserting, for each fixture cfg: the `--dry-run` render of `chain cfg` equals its
  ...
  `renderAll` call and their union is byte-identical to the **Phase-20 committed whole-deployment golden** for the
...
- A Register-1 ledger led by a Tier-2-UNVERIFIED banner: the plan is proven pure and exact in-process, but no
  ...
  and [phase_44_live_dsl_deploy.md](phase_44_live_dsl_deploy.md) (live control-plane daemon).
...
1. `cabal test chain-spec`, run with credentials scrubbed and socket calls blocked and observed, is green. For each
   ...
   identity-disjoint and union to the Phase-20 committed whole-deployment `renderAll` golden, the step set matches
...
## Sprint 21.5: The single typed subprocess seam + `boundary-spec` skeleton ✅

**Status**: Done — the capability is re-established by the migrated gate; the sprint's committed-ledger, pinned-toolchain, and repository-resident evidence mechanics are superseded
implementation behind `AbsExe` without widening the seam.
**Implementation**: `src/Amoebius/Exec/Tool.hs` (the Phase-21 boundary facade that invokes a tool by absolute
path over the `[Step]`/effect data) and, after Phase 35, `src/Amoebius/Host/Ensure.hs` (the sole raw
...
Phase-21 boundary cases continue to pass through `Exec/Tool.hs`.
...
- `src/Amoebius/Exec/Tool.hs`: the boundary facade that runs a resolved tool **by absolute path** (never a
  ...
  streams. Phase 35 preserves this contract while delegating its sole raw process call to the opaque-`AbsExe`
...
## Sprint 21.6: The fake `kubectl`/`helm`/`docker`/`pulumi` recorders ✅
...
## Sprint 21.7: The boundary battery — exact commands + applied bytes + no-`PATH` — the Part-B gate ✅

**Status**: Done — the capability is re-established by the migrated gate; the sprint's committed-ledger, pinned-toolchain, and repository-resident evidence mechanics are superseded
**Implementation**: `test/spec/boundary/BoundarySpec.hs`; the applied-manifest bytes reuse
the Phase-20 `renderAll` goldens; the **expected-argv transcripts are a separate committed hand-authored oracle** (`test/golden/chain_boundary/argv/`), NOT derived from the Part-A plan golden by any executor-reachable
...
- `test/spec/boundary/BoundarySpec.hs` asserting: the recorded argv stream equals the committed hand-authored
  expected-argv transcript; the applied-manifest bytes equal the Phase-20/Part-A goldens byte-for-byte (the same
...
1. `cabal test boundary-spec` is green — commands match the committed hand-authored argv transcript, applied
   bytes match the Phase-20/Part-A goldens exactly, the three invoked tool transcripts (`kubectl`/`docker`/`pulumi`)
   are non-empty and the `helm` transcript is empty, and invocation is by absolute path under the hostile
   decoy-`PATH` arrangement. This is the **Part-B (Register 2)** half of the phase gate; together with Sprint 21.4
   (Part A) it constitutes the two-part Phase-21 gate.
...
## Sprint 21.8: The sanctioned-API surface — what extension source may reach ✅
...
## Sprint 21.9: extension-astcheck — the extension AST checker and the link seal ✅
...
### Objective
Deliver extension-astcheck: admit extension source against the Sprint-20.8 surface, and make unchecked source
...
- `documents/engineering/dsl_doctrine.md` — backlink §2's chain/Step kernel and its pure `renderChainPlan` to
  this in-process Phase-21 seed; keep the effectful `runChainFromFrame` live invocation as the deferred Register-3
  residue.
- `documents/engineering/conformance_harness_doctrine.md` — record that §3's rendering-never-touches-infra
  invariant and §4's Plan spine step are golden-locked in Phase 21 for the `[Step]` plan (Part A), and that §2/§4's
  fake-apply step is exercised by the in-process Phase-21 boundary harness (Part B); keep Register 3 (live apply)
...
- `DEVELOPMENT_PLAN/substrates.md` — the Phase-21 `none` gate row.
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Kernel/{Step,Chain,Descent,Plan}.hs`, the
  ...
  `test/spec/boundary/` (the fakes + `Fakes.hs` + `BoundarySpec.hs`), and the `boundary-spec` test-suite as Phase-21
  design-first rows. Phase 35 separately registers `src/Amoebius/Host/Ensure.hs` as the strengthened raw
...
- [phase_18](phase_18_provision_seal.md) — the whole-deployment provision seal that constructs the opaque
  `ProvisionedSpec` the plan config carries
- [phase_20](phase_20_render_manifest_goldens.md) — the pure `renderAll` output from which a step selects its
...
- [phase_12](phase_12_gadt_decode_ir.md) — supplies the typed decoder consumed before planning
- [phase_22](phase_22_deterministic_sim_substrate.md) — the deterministic-simulation substrate this boundary
  harness unblocks
- [phase_25](phase_25_ui_program_schema.md) — the bounded UI source/checker phase that consumes the same
  pre-cluster dhall-typecheck/gadt-decode discipline without adding a second boundary-runtime claim
- [phase_37](phase_37_object_reconciler.md) — the live SSA reconciler that replaces the fakes with real tools
- [phase_44](phase_44_live_dsl_deploy.md) — Register 3 runs the chain via the Deployment-`replicas=1` control-plane daemon
