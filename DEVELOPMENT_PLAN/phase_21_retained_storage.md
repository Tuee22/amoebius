# Phase 21: No-provisioner retained storage + lossless rebind
...
**Register 3** (live infrastructure) — a single-node `kind` cluster brought up by the Phase 17 midwife, and
it opens only after the Phase 19 gate (the typed renderer + live SSA reconciler) closes, because the
...
8 supplies the policy-only `ProvisionedStorageScalingEnvelope` and pure observe-then-plan fold, and Phase 19
...
resize. `CreateProviderCapacity` has no retained-host mutation capability and remains owned by Phase 34.
...
Phase 23 and HA Patroni-via-Percona Postgres in Phase 24, the Vault-enveloping of secrets is Phase 22, and the
Keycloak-owned edge is Phase 25 — none of which this phase requires. The control plane
...
- **Real teardown, not soft delete.** After `cluster delete`, an OS-boundary observer on the host (not the
  ...
  fresh Phase-17 `pb bootstrap` producing a **new** apiserver/etcd (verified by a changed apiserver
...
- **Bytes survive on the host, not in a surviving apiserver.** "Bytes intact between delete and recreate" is
  ...
  destroyed. The PV `Released` *status* is observed only in Sprint 21.2's live PVC-delete step against the
...
- **Honest ledger.** The gate emits its proven/tested/assumed ledger; the aggregate durable-backing fold and
  image-backed host hard cap are live-tested here. The Phase-26 control-plane singleton's no-PVC property
  (which has no realized subject at Phase 21) stays marked **UNVERIFIED**, not asserted as passing.
...
witness image is a Phase-18 baked binary served only from the in-cluster `distribution` registry (an
...
- `test/live/fixtures/storageclass_expected.yaml` — the exact single-StorageClass golden (Sprint 21.1),
...
- `test/ci/no_retained_delete.sh` — the committed static check that no non-harness `src/` module issues a
  ...
  explicitly outside this check because the backing lives outside the cluster (Sprint 21.3 Validation 2a).
...
- **M-cutover-before-verify** (guard-weakening / stage-reordering operator) —
  ...
  `migration_verify_mismatch`, where a mismatched copy is cut over anyway (21.2 V1).
- **M-credit-before-cleanup** (dropped-validation operator) — `src/Amoebius/Storage/RetainedScaling.hs`:
  ...
  observed deletion" step (21.2 V1).
- **M-fake-verify** (mechanism-substitution operator) — `src/Amoebius/Storage/RetainedScaling.hs`: the
  ...
  byte-for-byte nonce readback (21.2 V1).
...
- [`cluster_lifecycle_doctrine.md §7`](../documents/engineering/cluster_lifecycle_doctrine.md#7-ephemeral-spin-updown-with-deterministic-rebind)
  ...
  (delivered in Phase 19) — the SSA reconciler that renders and applies the StorageClass and PV objects.

## Sprints

## Sprint 21.1: The one inert `no-provisioner` StorageClass 📋

**Status**: Planned
**Implementation**: `src/Amoebius/Storage/StorageClass.hs` (target path; not yet built)
**Blocked by**: Phase 19 gate (the typed renderer + live SSA reconciler that renders and applies the
StorageClass object); Phase 17 gate (an idempotent single-node `kind` cluster to install it into).
...
- A single rendered StorageClass — `provisioner: kubernetes.io/no-provisioner`, `reclaimPolicy: Retain`,
  `volumeBindingMode: WaitForFirstConsumer` — applied through the Phase-19 reconciler under the `amoebius`
  field manager.
...
## Sprint 21.2: Deterministic retained-PV generation + the explicit bind 📋
...
**Blocked by**: Sprint 21.1 (the inert class the retained PVs bind under); Phase 19 gate (the reconciler
...
images plus proposed additions, deduplicated by stable identity) is summed against the Phase-17/19-observed
...
- Host-retained resize enactment consumes only a private `ProvisionedStorageMigration`: the binder starts from
  ...
  `PodResourceEnvelope`, and per-backing old+new+workspace high-water. The Phase-19 snapshot-bound reconciler
...
- The Phase-21 enactors for `AllocateWithinRetainedCarve` and `ShrinkByVerifiedMigration`. They accept only
  Phase 19's fresh, snapshot-bound `ValidatedStorageScalingAction`, immediately recheck the exact retained
...
## Sprint 21.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip 📋

**Status**: Planned
**Implementation**: `src/Amoebius/Storage/Rebind.hs`, `test/live/RebindSpec.hs` (target paths; not yet built)
**Blocked by**: Sprint 21.2 (the retained PVs + deterministic bind the round-trip depends on); Phase 18 gate
(the baked-binary Postgres and MinIO images served only from the in-cluster `distribution` registry — the
witnesses pull nothing from a public registry); Phase 17 gate (the midwife `cluster delete` + `recreate` the
gate drives).
...
real `cluster delete` destroys the apiserver, so the PV `Released` *status* is observed only in Sprint 21.2's
live PVC-delete step, not here. `recreate` is a fresh Phase-17 bootstrap (new apiserver/etcd, changed cluster
...
### Deliverables
- Minimal single-ordinal Postgres and MinIO witness StatefulSets on retained PVs (from Sprint 21.2), served
  from baked-binary images; these are rebind witnesses, not distributed MinIO (Phase 23) or HA
  Patroni-via-Percona (Phase 24), and carry no Vault-enveloping (Phase 22).
...
1. Run the cycle on the concrete representative set of [Gate integrity](#gate-integrity) (exactly two
   ...
   `${RETAINED_ROOT}` still holds the bytes, `cluster recreate` as a fresh Phase-17 bootstrap (new apiserver
...
2. Assert the full deletion reclaimed no backing volume (fresh PV objects re-appear post-recreate and
   ...
   host bytes are present. The control-plane singleton is a Phase-26 subject with **no realized instance at
   Phase 21**, so its "mounts
...
- `documents/engineering/storage_lifecycle_doctrine.md` — the §6 lossless-rebind guarantee gains its first
  ...
  planning-ownership pointer resolves to delivered Phase-21 sprints.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the
  Phase-19 SSA reconciler that applies the StorageClass and retained-PV objects
...
- [phase_19](phase_19_object_reconciler.md) — the typed renderer + live SSA reconciler this phase builds on
- [phase_22](phase_22_vault_pki.md) — the root Vault whose durable KV rebinds on the retained storage proven here
- [phase_23](phase_23_platform_backbone.md) / [phase_24](phase_24_platform_services_2.md) — the HA
