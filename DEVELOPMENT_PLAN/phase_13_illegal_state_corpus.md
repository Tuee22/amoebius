# Phase 13: Illegal-state corpus + validation-locus ledger
...
- [Sprints](#sprints)
- [Sprint 13.1: Exhaustive negative/positive corpus split by foreclosure locus ✅](#sprint-131-exhaustive-negativepositive-corpus-split-by-foreclosure-locus-)
- [Sprint 13.2: GADT-index compile-fail goldens (type-foreclosed layer) ✅](#sprint-132-gadt-index-compile-fail-goldens-type-foreclosed-layer-)
- [Sprint 13.3: QuickCheck property suite ✅](#sprint-133-quickcheck-property-suite-)
- [Sprint 13.4: The per-entry validation-locus ledger — the gate ✅](#sprint-134-the-per-entry-validation-locus-ledger--the-gate-)
...
14 dhall-typecheck and 13 gadt-decode negatives against 12 positives, every one of the four seeded defects reddens at its
...
`sha256:a086f6a584a75dcd…`; Phase 13 owns no remaining migration deferral.
...
catalog entries reconcile to 104 registry subcases, 14 dhall-typecheck and 13 gadt-decode negatives fail at their own loci
...
over the `dhall` typechecker, the Phase-12 decoder, a pinned `ghc -fno-code` expect-fail harness, and
QuickCheck. It discharged 33 Phase-11/12/13-owned registry subcases and recorded the remaining 71 subcases as
deferred to their exact owners. Runtime enforcement, capacity/provisioning, and rendered-output fidelity
remain UNVERIFIED.

## Phase Summary

This phase turns the typed spec gates stood up in Phases 11 and 12 into an *exhaustive*, honestly-classified
proof. Phase 11 proved dhall-typecheck rejects a representative dhall-typecheck-class negative set; Phase 12 proved the total
decoder rejects a representative gadt-decode-class negative set. This phase assembles the **whole** illegal-state
corpus — one negative fixture per catalog entry that Register 1 can settle — and requires each to be rejected
at exactly the locus its catalog classification names: a dhall-typecheck negative must fail `dhall type`, a gadt-decode
...
to the one truth-maker locus that settles it — `dhall-typecheck`, `gadt-decode`, `provision-seal`,
`rendered-output-golden`, or `live-effect` — asserting that the dhall-typecheck/gadt-decode loci owned here carry rejecting fixtures
and recording later loci as deferred to their owning phase. Deferred ownership is an orthogonal registry
field; `provision-seal` is the distinct post-bind Phase-18 locus where whole-deployment capacity, topology,
storage, and target compatibility return `ProvisionError`. Phase 14 builds/proves those pure folds and Phase 18
...
`rendered-output-golden` locus points at (Phase 20),
the representational SPA-composition corpus (Phase 25), and every `live-effect` residue (the live band).
...
mutant, and registry reconciliation named in [Gate integrity](#gate-integrity). Phase 14 does not open unless
...
```
*Implemented Phase-13 gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*

**What the gate command accepts.** Every negative fixture is rejected at its tagged locus and nowhere else.
A dhall-typecheck-class negative fails `dhall type` at authoring time with an error equal to its oracle-pinned
error-locus golden naming the foreclosing union/field (§M.8 specific-reason). A gadt-decode-class negative passes
...
composition-preservation, with the Sprint 13.3 coverage minima met. The per-entry validation-locus ledger
(`dhall-typecheck` / `gadt-decode` / `extension-astcheck` / `provision-seal` / `rendered-output-golden` /
...
- A registry row whose `owner_phase` is **this phase or an earlier one** (Phase 11/12/13) and which has **no committed fixture** turns the Phase-13 gate **red** — the fixture the reached owner phase was obliged to
  commit is missing. This is the same red-on-unmapped rule as the acceptance conditions above and the
  Sprint 13.4 validation below.
...
- **Oracle-pinning (§M.1) + specific-reason negatives (§M.8).** Each negative fixture ships a committed expected-failure
  ...
  (the `dhall type` error, naming the offending union/field) for each `illegal_dhall_typecheck_*`;
...
- **Committed mutation quota (§M.2).** Four committed seeded mutants (from the [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) operator set) MUST turn the gate
  ...
  index → a compile-fail golden compiles → `compile_fail.sh` red; (d) a **broken-smart-constructor / partialized-fold** mutant → each of the four QuickCheck properties red. Phase-14/17 fold mutants remain owned
  by those phases; Phase 13 cannot execute a fold that has not landed. The gate is itself red if any mutant
  survives.
- **Generator coverage (§M.4).** Sprint 13.3's `checkCoverage` minima (below) force the nontrivial reject/boundary
  arms to fire.
- **Concrete corpus (§M.7).** The representative set is enumerated explicitly in the Sprint 13.1/6.2 Deliverables
...
- [`illegal_state_techniques.md §6 — Three layers of foreclosure (and the honesty they force)`](../documents/illegal_state/illegal_state_techniques.md#6-three-layers-of-foreclosure-and-the-honesty-they-force):
  ...
  *dhall-typecheck-must-fail-`dhall type`* and *gadt-decode-must-fail-decode*, and never bill a gadt-decode-only foreclosure as a
  dhall-typecheck type-check failure. This phase reifies that split as fixtures.
...
- [`resource_capacity_doctrine.md §3 — The types: Quantity, Capacity, Demand, Budget`](../documents/engineering/resource_capacity_doctrine.md#3-the-types-quantity-capacity-demand-budget)
  ...
  **shape/normalization** cases its predecessors own (including the missing-envelope [§3.11](../documents/illegal_state/illegal_state_security.md#311-an-unsafe-workload-no-resource-limits-no-hardened-securitycontext) negative) and
  ...
  ownership tags; it never claims those Phase-14/17 folds have run early.
...
## Sprint 13.1: Exhaustive negative/positive corpus split by foreclosure locus ✅

**Status**: Done — the capability is re-established by the migrated gate; the sprint's committed-ledger, pinned-toolchain, and repository-resident evidence mechanics are superseded
**Implementation**: `dhall/examples/{legal_*,illegal_dhall_typecheck_*,illegal_decode_*}.dhall`
(extending the Phase-11 positive + dhall-typecheck corpus and the Phase-12 gadt-decode set to one fixture per
...
**Blocked by**: [Phase 12](phase_12_gadt_decode_ir.md) supplied the total decoder + GADT-indexed IR; [Phase 11](phase_11_dhall_typecheck_schema.md) supplied the
...
**Independent Validation**: the corpus battery rejects every Phase-11/12/13-owned negative at its own tagged
...
- dhall-typecheck negatives (`illegal_dhall_typecheck_*`, must fail `dhall type`) are **exactly** the rows enumerated by
  ...
  `owner_phase ∈ { Phase-11, Phase-12, Phase-13 }`. Required members
  ...
  `ResourceEnvelope` ([§3.11](../documents/illegal_state/illegal_state_security.md#311-an-unsafe-workload-no-resource-limits-no-hardened-securitycontext); the Phase-11 `illegal_missing_resource_envelope` case), unbounded storage /
...
- gadt-decode negatives (`illegal_decode_*`, must pass `dhall type`, then decode-reject) are **exactly** registry
  ...
  `owner_phase ∈ { Phase-11, Phase-12, Phase-13 }`; these exercise the Phase-12
  ...
  no such inhabitant and is pinned by the fifth compile-fail pair in Sprint 13.2.
  The declared-compute-headroom cases belong here rather than to the deferred set below, because both are
  cross-field checks the Phase-12 smart constructor owns: `illegal_decode_headroom_over_limit`, whose pad
...
- **Provisioning-deferred negatives are selected by tags, never section-number ranges.** A registry row with
  `validation_locus = provision-seal`, an `owner_phase` of `Phase-14` or `Phase-18`, and a `case_family` of
  `topology`, `capacity`, `storage`, `cache`, `accelerator`, or `capability-provision` carries no rejecting
  fixture in Phase 13 and is ledgered with that owner. The generated selection must include every current
  ...
  allocatable VRAM; and provider-expanded resource demand. Phase 14 owns the pure folds and Phase 18 owns the end-to-end post-bind
...
- **Near-miss twinning (forecloses wrong-reason negatives, §M.8).** Each dhall-typecheck negative is a **single-construct mutation** of a named committed legal fixture (its `legal_*` twin, differing only in the one foreclosed
...
- **Per-Register-1-locus fixtures (disambiguation).** A catalog entry carrying more than one Register-1 locus
  ...
  `dhall-typecheck` < `gadt-decode`), but delivery follows each row's owner: Phase 13 supplies only rows owned
  by Phases 11–13, while Phase-14/17-owned subcases remain visibly deferred to those phases.

### Validation
1. Every Phase-11/12/13-owned dhall-typecheck negative fails `dhall type` with its observed error matching the committed
   `<name>.typeerr` golden that names the foreclosing union/field, and its legal near-miss twin passes
   `dhall type`.
2. Every Phase-11/12/13-owned gadt-decode negative passes `dhall type` and then decodes to a `Left DecodeError` whose
...
4. The coverage note maps each fixture to its catalog entry and foreclosure layer and is reconciled against
   the committed `locus_registry.tsv`, and every Phase-14/17-owned provisioning row is present in the derived
...
### Remaining Work
None for Sprint 13.1.

## Sprint 13.2: GADT-index compile-fail goldens (type-foreclosed layer) ✅
...
Phase-11 honesty caveat routed here, since Dhall has no opaque types.
...
### Remaining Work
None for Sprint 13.2.

## Sprint 13.3: QuickCheck property suite ✅
...
### Remaining Work
None for Sprint 13.3.

## Sprint 13.4: The per-entry validation-locus ledger — the gate ✅
...
emitter, so the assertion goes red on any locus, owner, or family divergence and on any Phase-11/12/13-owned row
...
`DEVELOPMENT_PLAN/README.md` (flip the Phase-13 status when the gate passes).
...
Phase 14 or 8; `rendered-output-golden` is owned by
[Phase 20](phase_20_render_manifest_goldens.md), and `live-effect` by the live band.
...
### Deliverables
- Consumption of the Sprint-12.1-committed catalog enrichment and `locus_registry.tsv`; Sprint 13.4 does not
...
- A ledger emitter that classifies each catalog entry into its earliest-sufficient truth-maker locus:
  ...
  (settled on emitted bytes in Phase 20), `live-effect` (settled only by a
  ...
  (`dhall-typecheck` < `gadt-decode` < `provision-seal` < `rendered-output-golden`), while each sub-case retains
...
- The committed independent oracle: `dhall/examples/locus_registry.tsv`, reconciled by the Phase-0 lint from
  the catalog's per-entry `**Validation-locus:**` plus the Phase-13-added `**Delivery-owner:**` and
  `**Case-family:**` tags, with columns at minimum `entry`, `subcase`, `validation_locus`, `owner_phase`, and
  `case_family`. The
  provisioning-deferred set is a query over those committed rows (`owner_phase ∈ {Phase-14, Phase-18}` plus the
...
- A coverage assertion that **reconciles the emitter's locus for every entry against the registry** and goes
  red on any locus/owner/family divergence, then requires every Phase-11/12/13-owned `dhall-typecheck` /
...
1. The ledger emits with every catalog entry mapped to exactly one truth-maker locus (`dhall-typecheck` /
   `gadt-decode` / `provision-seal` / `rendered-output-golden` / `live-effect`) and one disposition
...
3. The coverage assertion is green: every Phase-11/12/13-owned `dhall-typecheck` or `gadt-decode` row carries a
   passing rejecting fixture here, and every deferred row names its registry owner — `provision-seal`
   topology/capacity/storage/cache/accelerator/capability-provision rows → Phase 14 or Phase 18,
   `rendered-output-golden` → Phase 20, `live-effect` → the live band — without being reclassified.
...
- `documents/illegal_state/illegal_state_*.md` — annotate every themed entry with committed
  ...
  Register 1, discharged here only when `owner_phase ∈ {Phase-11, Phase-12, Phase-13}`; Phase-14/17
  `provision-seal` rows
  remain deferred; `rendered-output-golden` → Phase 20; `live-effect` → the live band), and confirm the §5
  coverage matrix is exhausted for the Register-1 rows this phase owns.
- `documents/engineering/dsl_doctrine.md` — backlink §5's two gates to the exhaustive Phase-13 corpus and the
...
- `DEVELOPMENT_PLAN/system_components.md` — register `dhall/examples/illegal_*`, `test/spec/dsl/CorpusSpec.hs`,
  `test/spec/dsl/compilefail/`, `test/spec/dsl/DecisionPropSpec.hs`, and `test/spec/dsl/ValidationLocusLedger.hs` as Phase-13
  design-first rows.

## Related Documents
- [README.md](README.md) — the live tracker and phase order this document serves
- Phase 13 illegal-state ledger — the authored proven/tested/unverified account of this gate
...
- [phase_11](phase_11_dhall_typecheck_schema.md) — dhall-typecheck, the Dhall schema + positive corpus this phase extends
- [phase_12](phase_12_gadt_decode_ir.md) — gadt-decode, the GADT-indexed IR + decoder this corpus rides atop
- [phase_14](phase_14_capacity_core_folds.md) — the pure capacity/topology/storage fold negatives selected
  from the registry as deferred from here
- [phase_17](phase_17_capability_bind.md) — the post-bind provisioning/capability negatives selected from the
  registry as deferred from here
- [phase_20](phase_20_render_manifest_goldens.md) — the `rendered-output-golden` locus this ledger points at
- [phase_44](phase_44_live_dsl_deploy.md) — the live band where the `live-effect` locus is discharged
