# Phase 35: Tenant/provider provisioning
...
Phase 35 delivers the tenant/provider provisioning; its design is owned by [tenancy_doctrine.md](../documents/engineering/tenancy_doctrine.md), [service_capability_doctrine.md](../documents/engineering/service_capability_doctrine.md), [platform_services_doctrine.md](../documents/engineering/platform_services_doctrine.md), and the plan for reaching it is owned here.
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_34_live_dsl_singleton.md, DEVELOPMENT_PLAN/phase_36_pulsar_client.md, DEVELOPMENT_PLAN/phase_37_user_tenant_isolation_live.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 35.1: Derive, apply, and externally read back tenant provider policy ⏸️](#sprint-351-derive-apply-and-externally-read-back-tenant-provider-policy-)
...
Phase 35 binds a checked `TenantSpec` containing issuer-qualified `SubjectSpec`, `Membership`, `Owner`, and
...
The phase then applies that sealed derivation to the live Phase-34 cluster and reads each provider back through
...
policies: Phase 37 owns the real-credential, own-subject/foreign-subject and own-tenant/foreign-tenant
...
boundary; Phase 34 supplies the live checked-spec singleton. Phase 17 is transitive through Phase 18.
...
- The provider index is closed and exhaustive. NetworkPolicy is a Kubernetes-API payload; Postgres roles and
  ...
  objects. Pulsar application produce/consume is owned by Phase 36 and scoped end-to-end enforcement by Phase
  36.
...
- [`illegal_state_security.md` §3.80](../documents/illegal_state/illegal_state_security.md#380-a-subject-resolving-or-mutating-another-subjects-resource-without-a-grant): Phase 35 provisions the policy
...
## Sprint 35.1: Derive, apply, and externally read back tenant provider policy ⏸️
...
4. Tear down and require authenticated provider inventories to equal preflight; persist only hashed evidence
   and explicitly mark application data-path isolation `UNVERIFIED (Phase 37)`.

### Remaining Work

None. Phase 37 retains the application request-isolation residue.
...
- `documents/illegal_state/illegal_state_security.md` — attach the killed hand-authored/tenant-collapse evidence
  while retaining Phase 37's live enforcement residue.
...
- `DEVELOPMENT_PLAN/phase_36_pulsar_client.md` and `phase_37_user_tenant_isolation_live.md` — consume provider
...
- [Phase 34 — live DSL singleton](phase_34_live_dsl_singleton.md)
- [Phase 36 — Pulsar client](phase_36_pulsar_client.md)
- [Phase 37 — live subject/tenant isolation](phase_37_user_tenant_isolation_live.md)
