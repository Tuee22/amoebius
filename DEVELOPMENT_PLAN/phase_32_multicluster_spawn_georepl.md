# Phase 32: Multi-cluster spawn + geo-replication
...
> over which [Phase 33](phase_33_gateway_migration_drills.md) drives the gateway-migration runtime.
...
amoebius result. This phase opens after the Phase 31 gate (the WireGuard fabric) and runs on the **linux-cpu**
...
in [Phase 33](phase_33_gateway_migration_drills.md); this phase stands up the geo-replicated forest that
...
non-confluent bucket — is precisely the boundary hand-off the [Phase 33](phase_33_gateway_migration_drills.md)
...
This phase consumes earlier phases and does not re-implement them: Phase 17's `pb` bootstrap of a `kind`
cluster, Phase 22's root Vault/PKI trust anchor, Phases 23–24's platform services (MinIO, Pulsar, Patroni Postgres),
Phase 26's live DSL deploy via the `replicas=1` singleton, Phase 28's native Pulsar client, and Phase 29's
...
[Phase 34](phase_34_provider_deploy_checkpoint.md)). Before either child is created, the parent-owned
...
[Phase 33](phase_33_gateway_migration_drills.md) drills; here the boundary is established and classified.
...
## Sprint 32.1: Amoebic spawn — `project(subtree)` handoff + per-child unseal / Transit key / secret injection 📋
...
built.
**Blocked by**: Phase 17 (bootstrap a `kind` cluster idempotently); Phase 22 (root Vault/PKI trust
anchor); Phase 26 (the Dhall DSL deploy via the `replicas=1` singleton). **Pulumi-from-inside and its
Vault-enveloped MinIO backend are first built in this phase** (Sprint 32.1) — no earlier phase provisions them,
and [Phase 34](phase_34_provider_deploy_checkpoint.md) reuses this engine for provider-managed clusters rather than
building its own.
...
## Sprint 32.2: Geo-replication of two siblings + invariant-confluence classification 📋
...
**Blocked by**: Sprint 32.1; Phase 31 (the WireGuard network fabric gate the cross-cluster replication rides
on); Phase 28 (native Pulsar client, CBOR); Phase 29 (content-addressed store + workflow
runtime); Phase 23 (MinIO); Phase 24 (Patroni Postgres).
...
(ii) for the [Phase 33](phase_33_gateway_migration_drills.md) migration runtime.
...
- `documents/engineering/cluster_lifecycle_doctrine.md` — §3/§9 gain the realized module paths for the spawn,
  ...
  land with [Phase 33](phase_33_gateway_migration_drills.md)).
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `src/Amoebius/Multicluster/Spawn.hs`,
  ...
  gate suites as Phase-32 rows.
- `DEVELOPMENT_PLAN/substrates.md` — the Phase-32 → linux-cpu row in the per-phase substrate map.
...
- [phase_31](phase_31_network_fabric_wireguard.md) — the prior phase (the WireGuard fabric); its gate opens this one
- [phase_33](phase_33_gateway_migration_drills.md) — the next phase; the gateway-migration runtime and correspondence over this forest
- [phase_34](phase_34_provider_deploy_checkpoint.md) — the forest extended to provider-managed clusters
