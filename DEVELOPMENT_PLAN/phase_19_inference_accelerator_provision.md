# Phase 19: InferenceEngine capability + accelerator provision
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_16_execution_accelerator_folds.md, DEVELOPMENT_PLAN/phase_17_capability_bind.md, DEVELOPMENT_PLAN/phase_18_provision_seal.md, DEVELOPMENT_PLAN/phase_59_determinism_jitcache.md
...
- [Sprints](#sprints)
- [Sprint 19.1: The `InferenceEngine` capability — target-offering-selected runtime + accelerator provision ✅](#sprint-191-the-inferenceengine-capability--target-offering-selected-runtime--accelerator-provision-)
- [Sprint 19.2: The accelerator-provision corpus + the Register-1 gate ✅](#sprint-192-the-accelerator-provision-corpus--the-register-1-gate-)
...
`sha256:2a81c1595be8d2b2…`; Phase 19 owns no remaining migration deferral.
...
Phase 19 in [`legacy_tracking_for_deletion.md`](legacy_tracking_for_deletion.md).
...
`ledgers/phase_19_inference_accelerator_provision.md`.
This phase opened after the Phase 18 gate (the whole-deployment
provision seal, from which the accelerator epoch witnesses are constructed) and the Phase 16 gate (the
...
executable as the strictest instance of the capability→provider→shape binding. It **fills the ninth capability arm** whose reserved head [Phase 17](phase_17_capability_bind.md) delivers: the `InferenceEngine` capability and
...
handing the owner demands to the [Phase 16](phase_16_execution_accelerator_folds.md) accelerator-residency fold,
...
- the reserved `InferenceEngine` head and the eight-arm closed union around it, the representational `bind`,
  and the object-node-multiset shape oracle ([Phase 17](phase_17_capability_bind.md))
- the `provision` constructor, execution-epoch/runtime-storage/
  ...
  `ProvisionedSpec` seal it plugs into ([Phase 18](phase_18_provision_seal.md))
- the primitive `fits`/`carve`/`place` accelerator-device / net-allocatable-VRAM / identity-complete
  residency-coexistence epoch fold ([Phase 16](phase_16_execution_accelerator_folds.md))
...
- and the **live** jit-build resolve of the named `EngineRuntime` identity into its `CacheBudget`-bounded
  ...
  ([Phase 59](phase_59_determinism_jitcache.md)).
...
accelerator-provision fold + property/corpus battery, analogous to the Phase-16 accelerator fold and the
Phase-18 provision seal.
...
clauses require for Phase 19; it is the InferenceEngine/accelerator-provision **slice** of the source
capability-binder gate corpus, partitioned to this seam (the shape-oracle, execution-epoch, runtime-storage,
object-store, observability, and migration slices live with [Phase 17](phase_17_capability_bind.md) /
[Phase 18](phase_18_provision_seal.md); the primitive accelerator/VRAM fold and its internal seeded mutant live
with [Phase 16](phase_16_execution_accelerator_folds.md)). It strengthens, never weakens, the Gate and sprint
Validations above.
...
flowchart LR
  ...
  g{{"the phase 19 gate command"}}:::gate
...
```
*Design intent. Phase 19's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
[Phase-17](phase_17_capability_bind.md) object-node-multiset shape oracle against the reviewer-authored
...
- `illegal_engine_by_url` — an engine named by URL — **fails dhall-typecheck** (`dhall type`) at an
...
([§3.25](../documents/illegal_state/illegal_state_ml_asset.md#325-an-ml-asset-named-by-arbitrary-url-or-an-unready--unlanded-model) for the engine-by-url state) and its honest foreclosure layer (dhall-typecheck for `illegal_engine_by_url`; the post-bind `provision-seal` locus for the rest).
...
[Phase 17](phase_17_capability_bind.md) / [Phase 18](phase_18_provision_seal.md).

### Independent reference predicate (§M.3)

The equivalence side is defined **independently of the code under test** (`Amoebius.Capability.Engine` and the Phase-16 fold): (a) a committed **hand-authored per-device co-resident memory aggregation table** — for each policy-permitted coexistence epoch of each owner-demand
...
- [`resource_capacity_doctrine.md §3`](../documents/engineering/resource_capacity_doctrine.md#3-the-types-quantity-capacity-demand-budget)
  ...
  matching offering, then run the Phase-16 accelerator-residency/coexistence fold, then hand only the checked
...
## Sprint 19.1: The `InferenceEngine` capability — target-offering-selected runtime + accelerator provision ✅
...
- The **partial** family×lane availability relation making a served model whose family is unavailable on the
  ...
  [Phase-16](phase_16_execution_accelerator_folds.md) fold derives all policy-permitted epochs and sums every
...
- The accelerator-provision pairing at the provision seal: after bind expands the `InferenceEngine` provider's
  ...
  `ProvisionedCudaOwnerDemand`/`ProvisionedMetalOwnerDemand` epoch witnesses via the Phase-16 fold, and fits each
...
- An in-file honesty note: this is the representational union + relation and the pure accelerator-provision fold
  ...
  runtime-checked cross-lane weight-load residue, are the live band ([Phase 59](phase_59_determinism_jitcache.md))
...
## Sprint 19.2: The accelerator-provision corpus + the Register-1 gate ✅
...
one dhall-typecheck and eight provision negatives, and eight QuickCheck branches. The phase gate runs five mutants,
...
`documents/engineering/content_addressing_doctrine.md` (§4.5 Tier-1 engine read-side),
`documents/engineering/testing_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-19 status when the
gate passes), `DEVELOPMENT_PLAN/substrates.md` (the Phase-19 `none` gate row).
...
- A Register-1 validation-locus ledger mapping every entry to its catalog id ([§3.25](../documents/illegal_state/illegal_state_ml_asset.md#325-an-ml-asset-named-by-arbitrary-url-or-an-unready--unlanded-model) for the engine-by-URL state)
  ...
  backed by Phase-13-style coverage-assertion machinery (the ledger goes **red** if any corpus entry, negative
...
1. `cabal test capability-spec` is green over the InferenceEngine/accelerator slice — `legal_inference_cuda`
   ...
   mutants. The validation-locus ledger is present and its coverage-assertion machinery (Phase-13 precedent)
...
- `documents/engineering/resource_capacity_doctrine.md` — record that the accelerator arm of the post-fold
  boundary (§3/§4) is exercised through the `InferenceEngine` binding and the Phase-16 residency/coexistence fold
...
- `DEVELOPMENT_PLAN/substrates.md` — the Phase-19 `none` gate row.
- `DEVELOPMENT_PLAN/system_components.md` — register the `InferenceEngine` fill of `dhall/amoebius/Capability.dhall`,
  `src/Amoebius/Capability/Engine.hs`, and the engine/accelerator property + gate suites as Phase-19 design-first
  rows.
...
- [phase_16](phase_16_execution_accelerator_folds.md) — the identity-complete accelerator-device / net-allocatable-VRAM
  / residency-coexistence epoch fold these owner demands feed
- [phase_17](phase_17_capability_bind.md) — the closed nine-arm capability union (whose reserved `InferenceEngine` head this phase fills), the representational `bind`, and the object-node-multiset shape oracle
- [phase_18](phase_18_provision_seal.md) — the whole-deployment provision seal that constructs the accelerator
