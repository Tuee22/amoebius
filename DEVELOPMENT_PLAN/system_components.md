...
- **Planned module path** — the observed or intended Haskell module(s) / artifact(s). `BUILT` means the path
  ...
  `amoebius` package whose observed entrypoint is `app/Main.hs`.
...
Generated paths in a row are migration input only. Their canonical destination is the `gen/` tree defined by
...
|---|---|---|---|
| Root executable | `app/Main.hs` exists; older prose named an extra `amoebius/` directory | Keep `app/Main.hs` as the canonical root entrypoint and update package declarations/tests to agree | Phase 14 |
...
| Pulsar protocol | The `.proto` is authored while generated Haskell bindings and checksum lists appeared under legacy evidence | Retain the `.proto`; generate all bindings/checksums only under `gen/proto/**` or the build tree | Phases 1, 35 |
...
| Test topology | `src/Amoebius/Test/SuggestTest.hs` and supporting harness code exist alongside generated enumeration/ledger files in authored roots | Keep the authored generator/harness; move every enumeration and run record to `gen/**` and externally attest it | Phases 0, 54 |
...
|---|---|---|---|
| Executable entrypoint (argv dispatch, exit orchestration) | [daemon_topology §1](../documents/engineering/daemon_topology_doctrine.md#1-one-binary-three-contexts) | `app/Main.hs` (BUILT/VALIDATED Phase 14 boundary surface; later command families remain planned) | [phase_14_chain_kernel_boundary.md](phase_14_chain_kernel_boundary.md) |
...
| Haskell typed decoder (Gate 2) | [dsl_doctrine Gate 2](../documents/engineering/dsl_doctrine.md#gate-2--the-haskell-typed-decoder) | `src/Amoebius/Dsl/{Decode,Error,Ref,SmartConstructors,Types}.hs`, `test/dsl/DecodeSpec.hs`, `tools/phase5_gate.py` (BUILT/VALIDATED Phase 5; ledger `external-run-reference`) | [phase_05_gadt_decoder_gate2.md](phase_05_gadt_decoder_gate2.md) |
| Dhall smart-constructor vocabulary and constructor-rejection corpus | [dsl_doctrine §4](../documents/engineering/dsl_doctrine.md#4-total-composability) | `dhall/amoebius/*.dhall`, `tests/gate1/ctor_reject/*.dhall` (BUILT/VALIDATED Phase 4); Haskell refinement indices and compile pairs BUILT/VALIDATED Phase 5 | [phase_04_dhall_gate1_schema.md](phase_04_dhall_gate1_schema.md), [phase_05_gadt_decoder_gate2.md](phase_05_gadt_decoder_gate2.md) |
...
| SSA reconciler (`amoebius` field manager, ApplySet prune, wait; requires `ValidatedLiveTarget` and consumes `renderAll` over the whole checked deployment) | [manifest_generation §5](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions) | `src/Amoebius/Manifest/{Apply,Enact,Delete,Wait}.hs`, `tools/phase26_reconcile_live.py` (BUILT/VALIDATED Phase 26) | [phase_26_object_reconciler.md](phase_26_object_reconciler.md) |
| Sole inert StorageClass + deterministic retained PV/claimRef renderer, fixed-size host backing, uniform-claim/backing admission | [storage lifecycle §§2–5](../documents/engineering/storage_lifecycle_doctrine.md#2-one-storage-class-and-it-provisions-nothing) | `src/Amoebius/Storage/{StorageClass,RetainedPV,HostVolume}.hs`, `test/{storage,live}/Phase28*Spec.hs` (BUILT/VALIDATED Phase 28; ledger `external-run-reference`) | [phase_28_retained_storage.md](phase_28_retained_storage.md) |
| Lossless retained-storage delete/recreate rebind | [storage lifecycle §6](../documents/engineering/storage_lifecycle_doctrine.md#6-the-lossless-teardown-guarantee-deterministic-rebind) | `src/Amoebius/Storage/Rebind.hs`, `test/live/RebindSpec.hs`, `tools/phase28_rebind_live.py` (BUILT/VALIDATED Phase 28) | [phase_28_retained_storage.md](phase_28_retained_storage.md) |
...
| Closed `UiSource` Dhall algebra + normalized checked graph | [low_code_ui_runtime §§4–5](../documents/engineering/low_code_ui_runtime_doctrine.md#4-the-authored-dhall-surface) | `dhall/amoebius/ui/*.dhall`, `src/Amoebius/Ui/{Source,Check}.hs`, `test/ui/Phase16UiProgramSchemaSpec.hs` (BUILT/VALIDATED Phase 16) | [phase_16_ui_program_schema.md](phase_16_ui_program_schema.md) |
...
| Local client/server/infernix/jitML adapter composition harness | [low_code_ui_runtime §17](../documents/engineering/low_code_ui_runtime_doctrine.md#17-verification-obligations) | `test/ui/Phase23LocalCompositionSpec.hs`, `test/ui/local/phase23_local_composition.mjs`, `tools/phase23_gate.py` (BUILT/VALIDATED Phase 23; ledger `external-run-reference`) | [phase_23_ui_local_composition.md](phase_23_ui_local_composition.md) |
...
| Pristine Linux gate harness | [substrate §4](../documents/engineering/substrate_doctrine.md#4-virtualized-substrates-synthesizing-a-linux-host-where-the-host-is-not-linux) | `tools/phase24_pristine_gate.py`, `test/host/test_phase24_pristine_gate.py` (BUILT; Incus end-to-end live, Lima/WSL2 plans unit-tested) | [phase_24_bootstrap_coordinator_kind.md](phase_24_bootstrap_coordinator_kind.md) |
...
| Gate-3 extension AST checker + sanctioned-API surface (opaque `CheckedExtensionSource` is the only linkable value) | [dsl_doctrine Gate 3](../documents/engineering/dsl_doctrine.md#gate-3--the-extension-ast-checker), [dsl §8](../documents/engineering/dsl_doctrine.md#8-the-haskell-extension-dsl--the-constrained-surface-gate-3-admits) | `src/Amoebius/Dsl/{AstCheck,SanctionedApi}.hs`, `dhall/amoebius/SanctionedApi.dhall`, `test/dsl/AstCheckSpec.hs` (BUILT/VALIDATED Phase 14) | [phase_14_chain_kernel_boundary.md](phase_14_chain_kernel_boundary.md) |
...
|---|---|---|---|
| Dynamic compiler/package/browser/codegen resolution | [dsl_doctrine §9](../documents/engineering/dsl_doctrine.md#9-toolchain-note), [repository-layout doctrine §4](../documents/engineering/repository_layout_doctrine.md#4-dependency-and-toolchain-resolution) | authored `cabal.project`, `toolchain/requirements.json`, and `patches/**`; resolver `tools/toolchain.py`; generated `gen/{toolchain,locks}/**`; no lock/freeze file, package checksum, resolved path, or library SHA is tracked (BUILT) | [phase_01](phase_01_toolchain_spike.md) |
| Re-runnable buildability harness: Dhall decode pair, io-sim terminal oracle, resolver deps, PureScript Bridge, Supernova, proto-lens codegen, and mutants | [conformance_harness_doctrine](../documents/engineering/conformance_harness_doctrine.md), [testing_doctrine](../documents/engineering/testing_doctrine.md) | authored `probe/` and `tools/phase1_gate.py`; generated run output under `gen/runs/phase_01/**`; external attestation required | [phase_01](phase_01_toolchain_spike.md) |
| Formal-model EDSL (`Model` → `interpret` runtime fn + `emitTLA` generated `.tla`; safety `INVARIANT`s + fairness/temporal `PROPERTY`s; differential explorer↔TLC property) | [formal_model_doctrine](../documents/engineering/formal_model_doctrine.md) | `src/Amoebius/Formal/{Model,Interpret,EmitTLA,Explore,ToyModel}.hs`, `src/Amoebius/Cli/Formal.hs`, `test/formal/RoundTripSpec.hs`, `tools/phase2_gate.py` (BUILT/VALIDATED Phase 2; generated `.tla`/`.cfg` are not committed) | [phase_02](phase_02_formal_model_kernel.md) |
| Gateway-migration **design model**, both branches (TLC safety `UniqueGatewayOwner` / `SessionAlwaysRebindable` / `PlannedIsLossless` / `NoWriteAfterStaleFailover` / `NoTakeWithoutProvenFreshness` + liveness `MergeConverges` / `SessionEventuallyRebinds` / `PlannedMigrationTerminates` under fairness; proven-for-the-model at scope, argued/tested cutoff) + IOSimPOR + structural-fit fold | [gateway_migration_model_doctrine](../documents/engineering/gateway_migration_model_doctrine.md) | `src/Amoebius/Formal/GatewayMigration.hs`, `src/Amoebius/Multicluster/StructuralFit.hs`, `test/formal/gateway/GatewayMigrationSpec.hs`, `tools/phase3_gate.py` (BUILT/VALIDATED Phase 3; generated `gen/tla/*.tla` not committed) | [phase_03](phase_03_gateway_migration_model.md) |
| Deterministic-simulation substrate — the `io-classes` effect interface + real/sim interpreters + modeled fault-injectable environment (serves Register 2.5) + `IOSim`/`IOSimPOR` trace-validator | [deterministic_simulation_doctrine](../documents/engineering/deterministic_simulation_doctrine.md) | `src/Amoebius/Sim/{Env,Reconcile}.hs`, `src/Amoebius/Sim/Interp/{Real,Sim}.hs`, `src/Amoebius/Sim/Fakes/{Pulsar,MinIO,ApiServer,Route53,Vault,Clock}.hs`, `test/sim/*`, `tools/phase15_gate.py` (BUILT/VALIDATED Phase 15; fidelity ASSUMED) | [phase_15](phase_15_deterministic_sim_substrate.md) |
...
| Coverage obligation — enumerated surfaces vs authored expectation/fault status | [testing_doctrine §9](../documents/engineering/testing_doctrine.md#9-derivation-generated-enumeration-authored-expectation), [generated_artifacts_doctrine §3](../documents/engineering/generated_artifacts_doctrine.md#3-the-rule) | `src/Amoebius/Test/Ledger.hs`; generated `gen/test-surfaces/phase_54.json`; no enumeration under `test/` | [phase_54_test_topology_dsl.md](phase_54_test_topology_dsl.md) |
| Per-run ledger `coverage` array | [testing_doctrine §4](../documents/engineering/testing_doctrine.md#4-no-skips-fail-fast-and-the-per-run-ledger-artifact) | `tools/ledger_lint.py` + `src/Amoebius/Test/Ledger.hs`; generated `gen/runs/<phase>/<run-id>/ledger.json`, externally attested | [phase_00](phase_00_documentation_suite.md), [phase_54_test_topology_dsl.md](phase_54_test_topology_dsl.md) |
| Dhall schema **type discipline** (Gate 1 `dhall type`; illegal-state-unrepresentable surface) | [dsl_doctrine §2](../documents/engineering/dsl_doctrine.md#2-two-languages-one-system-dhall-carries-params-haskell-carries-logic), [illegal_state_catalog §6](../documents/illegal_state/illegal_state_catalog.md) | `dhall/amoebius/*.dhall`, `dhall/examples/legal_*.dhall`, `tests/oracle/gate1/*`, `tools/dhall_gate1.py` (authored and BUILT/VALIDATED Phase 4; decoded structural parity validated Phase 5) | [phase_04](phase_04_dhall_gate1_schema.md) |
| GADT IR + total decoder + smart constructors (Gate 2; exhaustive controller/resource seam completed in Phase 6) | [dsl_doctrine Gate 2](../documents/engineering/dsl_doctrine.md#gate-2--the-haskell-typed-decoder) | `amoebius:dsl-core`, `src/Amoebius/Dsl/{Types,Decode,Error,Ref,SmartConstructors}.hs`, `test/dsl/{DecodeSpec.hs,compile/*}`, `tests/oracle/gate2/*`, `tools/phase5_gate.py` (BUILT/VALIDATED Phase 5; 5,527 exact structural rows) | [phase_05](phase_05_gadt_decoder_gate2.md) |
| Illegal-state corpus + property tests + validation-locus ledger | [illegal_state_catalog §6](../documents/illegal_state/illegal_state_catalog.md) | `src/Amoebius/Dsl/{Cbor,Decision,Foreclosure}.hs`, `dhall/examples/{legal_*,illegal_*,locus_registry.tsv}`, `test/dsl/{CorpusSpec,DecisionPropSpec,DecisionPropMain,ValidationLocusLedger}.hs`, `test/dsl/compilefail/*`, `tests/oracle/phase6/*`, `tools/{locus_registry_lint,compile_fail,phase6_gate}.py` (BUILT/VALIDATED Phase 6; 88 entries, 104 subcases, 33 discharged, 71 deferred; ledger `external-run-reference`) | [phase_06](phase_06_illegal_state_corpus.md) |
