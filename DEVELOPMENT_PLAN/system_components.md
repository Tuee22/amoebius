...
(the one elected brain with total cluster + secret authority) and
...
| Smart constructors (illegal-state-unrepresentable values) | [dsl_doctrine §4](../documents/engineering/dsl_doctrine.md#4-total-composability) | `src/Amoebius/Dsl/SmartConstructors.hs` (PLANNED) | [phase_03_dsl_control_plane_singleton.md](phase_03_dsl_control_plane_singleton.md) |
...
elected singleton (§1), never by a CLI poke racing another writer.
...
[`substrate_doctrine.md` §6 — The `bootstrap.sh` contract: ensure a toolchain, build the binary, hand off](../documents/engineering/substrate_doctrine.md#6-the-bootstrapsh-contract-ensure-a-toolchain-build-the-binary-hand-off).
...
| Virtualized-substrate management (Lima / Tart / brew on Apple) | [substrate §3](../documents/engineering/substrate_doctrine.md#3-the-no-environment--no-path-lazy-tool-ensure-contract) | `src/Amoebius/Substrate/{Apple,Lima,Tart,Brew}.hs` (PLANNED) | [phase_07_host_compute_daemons.md](phase_07_host_compute_daemons.md) |
...
The engine and EBS programs land with provider clusters in Phase 10; multi-cluster child-spawn keying
(Phase 9, no phase doc authored yet — see the [README.md](README.md) Phase index) reuses the same backend.
...
- [Substrates](../documents/engineering/substrate_doctrine.md) — the tool-ensure and `bootstrap.sh` owner
