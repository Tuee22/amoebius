# Phase 25: Keycloak-owned ingress

**Status**: Authoritative source
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_23_platform_backbone.md, DEVELOPMENT_PLAN/phase_24_platform_services_2.md, DEVELOPMENT_PLAN/phase_26_live_dsl_singleton.md, DEVELOPMENT_PLAN/system_components.md
...
Phase-23/24 standard service stack. Keycloak owning all wild ingress is **sibling evidence, not an amoebius result** —
...
for every wild request. It composes the LoadBalancer address published by Phase 23 (MetalLB on `linux-cpu`)
with an **Envoy + Gateway API L7 data plane** and **Keycloak OIDC/JWT enforcement**, rendered as typed
manifests by the Phase-19 reconciler and applied to the live cluster, so that WAN, LAN, and even a
...
is applied and exercised live. Finally the phase re-runs the Phase-21 lossless-rebind proof behind the new
...
app tenancy, and the Pulsar/workflow runtime are Phase 26+ concerns; this phase exercises the edge from the
host binary against the fixed standard service set that Phases 23–24 stood up. The one genuinely new-vs-prodbox
...
localhost-only NodePort, a distinct endpoint type); and the **Phase-21 storage-rebind regression still holds**
...
route inventory** — `test/fixtures/phase25/route-inventory.golden` — listing every browser surface on the
Phase-23/24 standard service stack that the edge fronts: **Grafana, the Keycloak admin console, the Vault UI, the
...
three. The "test-realm user" is the Phase-0-committed `phase25-tester` realm/user fixture
(`test/fixtures/phase25/realm.json`), authored before the edge exists.
...
- Envoy + Gateway API rendered as the L7 edge (a `Gateway` listener plus `HTTPRoute`s as typed `K8sObject`s),
  terminating TLS and routing, applied by the Phase-19 reconciler.
- Keycloak deployed against its Phase-24 Patroni DB, owning OIDC/JWT enforcement in front of every platform
...
- The Phase-0-committed oracle fixtures this sprint checks against: `test/fixtures/phase25/route-inventory.golden`
  (the explicit surface set) and `test/fixtures/phase25/realm.json` (the `phase25-tester` realm/user), both
...
1. For each surface enumerated in the committed `route-inventory.golden`, complete a real OIDC login as
   `phase25-tester` and assert the surface's content is served (2xx) only after the request traversed Keycloak;
...
## Sprint 25.2: No self-published wild ingress + public-edge TLS 📋
...
- A live audit proving there is no non-Keycloak wild path: no chart opens a backdoor NodePort to the wild, and
  ...
  edge, per the render invariant golden-locked in Phase 13.
...
- The committed scanner-validation seed (`test/fixtures/phase25/backdoor-seed.yaml`, a raw-`kubectl`
...
- A default-deny east-west baseline plus allow-edges derived from the declared dependency graph, rendered by
  the Phase-19 reconciler and applied to the live cluster — no hand-authored policy.
...
- The Phase-0-committed expected-policy oracle `test/fixtures/phase25/netpol-expected.json`, an independent
...
## Sprint 25.4: The single-door + storage-rebind regression gate 📋

**Status**: Planned
**Implementation**: `src/Amoebius/Platform/Edge.hs`, `test/live/IngressRebindGate.hs` (target paths; not yet built)
**Blocked by**: Sprint 25.1, Sprint 25.2, Sprint 25.3, Phase 21 gate (external prereq — the no-provisioner retained-storage lossless rebind the regression reuses)
...
delete + recreate. Before full cluster deletion, the harness performs Phase 21's intermediate PVC-delete
...
- The storage-rebind regression: write the committed marker row (`marker-row.sql`) into the Keycloak Patroni DB
  and the committed marker object (`marker-object.bin`) into a MinIO bucket; perform the Phase-21 intermediate
  ...
  same bytes back — the Phase-21 guarantee re-run behind the new edge; plus committed mutant (c), a
...
1. Assert the single-door invariant holds end-to-end: an unauthenticated request is rejected at the edge, a real
   OIDC login as `phase25-tester` serves each surface in `route-inventory.golden`, and there is no backdoor wild
...
- [README.md](README.md) — flip the Phase 25 row status once work begins, and link this document from the
  Phase 25 paragraph.
- [substrates.md](substrates.md) — record `linux-cpu` as the Phase 25 gate substrate in the per-phase map.
...
- [README.md](README.md) — the live tracker; its Phase 25 row is the authoritative one-line gate and status.
...
- [Manifest Generation Doctrine](../documents/engineering/manifest_generation_doctrine.md) — the typed renderer
  + server-side-apply reconciler that enacts the edge (delivered in Phase 19).
