# Phase 70: Content store + workflow runtime (Pulsar-Failover single-writer)

> **Purpose**: Stand up amoebius's durable-artifact substrate — the three-tier content-addressed MinIO store —
> and the orchestrator/worker workflow runtime on top of the Phase-68 native Pulsar client, gated live on
...
- [Sprints](#sprints)
- [Sprint 70.1: Three-tier content-addressed MinIO store ⏸️](#sprint-701-three-tier-content-addressed-minio-store-)
- [Sprint 70.2: Orchestrator/worker workflow runtime + store/fetch by manifest SHA ⏸️](#sprint-702-orchestratorworker-workflow-runtime--storefetch-by-manifest-sha-)
- [Sprint 70.3: Pulsar Failover standby takeover + leak-free teardown (gate) ⏸️](#sprint-703-pulsar-failover-standby-takeover--leak-free-teardown-gate-)
- [Sprint 70.4: Register-2.5 workflow failover takeover under simulated fault ⏸️](#sprint-704-register-25-workflow-failover-takeover-under-simulated-fault-)
...
`experiment-hash` namespace within an app's Phase-67 ObjectStore bucket. Every namespace also binds a finite
...
charged until an observed GC deletion; the logical peak consumes Phase 63's MinIO erasure/healing and uniform
claim-plan witness rather than assuming logical bytes equal physical disk. Second, an **orchestrator/worker workflow runtime** on top of the Phase-68 client: an orchestrator worker produces a workflow `command` on a
...
This phase's gate must also close the Job-terminal live-proof boundary deliberately left open in Phase 59. Human-approved Phase 59
...
name-ordered standby, with the Phase-68 at-least-once contract redelivering the un-acked command; the store's
...
typeclass, and SplitMix seed derivation are the Phase 81 determinism kernel, not this phase.
...
**Depends on:** [Phase 69](phase_69_user_tenant_isolation_live.md)
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 69; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
- [`daemon_topology_doctrine.md` §3.1 — "Exactly one pod" is a k8s/etcd property, not an amoebius election](../documents/engineering/daemon_topology_doctrine.md#31-exactly-one-pod-is-a-k8setcd-property-not-an-amoebius-election)
  ...
  control-plane daemon (Phase 66), whose single-instance is delegated to k8s/etcd, so nothing in this phase
...
- [`pulsar_client_doctrine.md` §5 — The capability surface: lookup · produce · consume · subscribe · seek](../documents/engineering/pulsar_client_doctrine.md#5-the-capability-surface-lookup--produce--consume--subscribe--seek)
  ...
  — *the capability surface (the Failover subscription)* / *at-least-once with broker-side dedup*: the Phase-68
...
- [`deterministic_simulation_doctrine.md` §4 — Register 2.5 — where deterministic simulation sits](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  — *Register 2.5 — where deterministic simulation sits*: Sprint 70.4 runs the real Sprint-64.2/32.3 workflow
  runtime under `IOSimPOR` against the Phase-17 modeled environment as a Register-2.5 lower-register cross-check
...
- [`chaos_failover_doctrine.md` §12 — The moral core — proven, tested, assumed](../documents/engineering/chaos_failover_doctrine.md#12-the-moral-core--proven-tested-assumed)
  ...
  **cross-cluster** failover boundary and its formal model are owned by
  ...
  and realized by Phase 75's geo-replication plus Phase 76's gateway-migration drills, not here — this phase
...
## Sprint 70.1: Three-tier content-addressed MinIO store ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 69](phase_69_user_tenant_isolation_live.md) human approval
...
- A mandatory `ObjectStoreDemand` per namespace: exact physical-id-keyed committed residents, structural
  ...
  future/transient extent peak, merges it with all Phase-63 producer arms, and feeds that structure—not a byte
...
- The collector/verification Job consumes the Phase-59 terminal state machine live for the first time. Its
...
- Store keys taken under a caller-supplied `experiment-hash` namespace string within the app's ObjectStore
  ...
  seed derivation (Phase 81 kernel work).
...
1. Run this suite at **Register 3** against the **single-node kind cluster's live MinIO** — the standing
   Phase-63 HA service on the Phase-61 retained PV, never an in-process or local S3 fake, so the evidential
...
## Sprint 70.2: Orchestrator/worker workflow runtime + store/fetch by manifest SHA ⏸️
...
wire the Phase-68 client, its topology algebra, and the Sprint-64.1 store into an orchestrator/worker runtime
...
- An orchestrator daemon that, using the Phase-68 topology algebra, produces a workflow `command` on the
  derived topic and consumes the corresponding `event`; it is an unelected worker daemon, not a leader.
- Worker daemons that consume the command, write a content-addressed artifact to the store (Sprint 70.1), and
  produce an `event` carrying the manifest SHA — CBOR payloads throughout (Phase 68), a large artifact carried
...
- The runtime is scheduled under the Deployment-`replicas=1` control-plane daemon (Phase 66); no orchestrator/worker role
...
2. Assert a retried produce and a redelivered consume are collapsed by the Phase-68 broker-side dedup so
...
The pre-reset record said `None` and claimed delivery/validation by the old Phase-70 gate; both statements are
...
## Sprint 70.3: Pulsar Failover standby takeover + leak-free teardown (gate) ⏸️
...
- Worker daemons attached over a Pulsar **Failover** subscription (Phase 68): one active, the rest
  name-ordered hot standbys; single-writer liveness is the subscription, safety is the store's ETag-CAS commit
  point plus the typed `AdvancePredicate` (Sprint 70.1), so even a bounded failover overlap cannot regress
  HEAD.
- The **critical-window kill-injection path**: the kill lands **after the active worker has completed its store write and before it acks the `event`** (the same window Sprint 70.4 injects in simulation, restated for the
  live gate), so the load-bearing standby re-fetch + bounded failover overlap are actually exercised — not a kill
  against an idle, already-drained worker. Pulsar promotes the name-ordered standby, the Phase-68 at-least-once
...
- The **postflight sweep's explicit inventory contract**: the sweep MUST inventory, and the ledger MUST record,
  ...
  (the durable test-flagged bytes reclaimed by Phase 49). The sweep emits its **full inventory list and the named retained set** into the per-run ledger; **any non-empty remainder outside the retained set is a hard gate failure**. (Durable-byte reclaim staying with Phase 49 is the *only* exemption, and only for the
...
2. **Idempotency and leak-free teardown, disambiguated.** "**Idempotent setup**" means a *second `apply` of the
   topology against the still-standing topology is a byte-stable no-op* (the Phase-59 sense — zero fields
...
> and realized by Phase 75's geo-replication plus Phase 76's gateway-migration drills, not here. Pulsar's own
...
> reclaim of test-flagged durable bytes is the elevated live harness's prerogative (Phase 91), kept out of the
...
## Sprint 70.4: Register-2.5 workflow failover takeover under simulated fault ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 70.3
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
Phase 17 Sprint 17.2 modeled fault-injectable environment, and assert the same load-bearing properties the Sprint 70.3
...
- A `WorkflowFailoverSimSpec` that binds `Amoebius.Workflow.Runtime`/`Orchestrator`/`Worker` (Sprints 59.2–38.3)
  to the Phase 17 Sprint 17.2 `Amoebius.Sim.Env` substrate through `io-classes` and drives it under `IOSimPOR` — the
...
- The injected fault schedule (`WorkflowSimScenario`): a `kill-worker-mid-workflow` inside the gate's critical
  ...
  **partition** — modeled by the fake Pulsar/MinIO of Phase 17 Sprint 17.2, not a live cluster.
...
> only by this phase's **Register-3 live gate** (Sprint 70.3) on the linux-cpu kind cluster — the deterministic
...
- `documents/engineering/content_addressing_doctrine.md` — record that §2 (the three-tier store + the two write
  ...
  `experimentHash` derivation and seed kernel explicitly deferred to Phase 81; note §5 confluence is consumed
...
- `documents/engineering/resource_capacity_doctrine.md` — record the content-store logical peak boundary
  corpus and its consumption of Phase 63's MinIO physical/uniform-claim witness.
- `documents/engineering/storage_lifecycle_doctrine.md` — record that the store's blob/manifest/pointer bytes
  land on the Phase-61 retained PV under the standing Phase-63 MinIO service, and that CAS-loser orphans stay
...
- `documents/engineering/chaos_failover_doctrine.md` — record the §12 per-run proven/tested/assumed ledger for
  the intra-cluster failover injection, and that the §16 cross-cluster boundary stays deferred to Phase 75
  geo-replication plus Phase 76 gateway-migration drills.
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-70 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 70's gate substrate (linux-cpu) in the per-phase substrate map.
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-store` and `amoebius-runtime` packages and
  ...
  Phase-70 design-first rows.

## Related Documents

- [README.md](README.md) — the live tracker; Phase 70 objective, gate, and substrate
...
- [Native Pulsar Client Doctrine](../documents/engineering/pulsar_client_doctrine.md) — the Phase-68 capability
...
- [phase_68](phase_68_pulsar_client.md) — the native Pulsar client this workflow runtime is built on
- [phase_81](phase_81_determinism_jitcache.md) — the `experimentHash` derivation + SplitMix seed kernel deferred
