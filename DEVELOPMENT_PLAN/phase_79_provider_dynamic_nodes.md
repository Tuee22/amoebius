# Phase 79: Dynamic node provisioning by signal + leak-free provider gate
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_65_live_dsl_deploy.md, DEVELOPMENT_PLAN/phase_76_provider_deploy_checkpoint.md, DEVELOPMENT_PLAN/phase_77_provider_child_bringup.md, DEVELOPMENT_PLAN/phase_78_provider_ebs_credential.md, DEVELOPMENT_PLAN/phase_80_determinism_jitcache.md, documents/engineering/cluster_lifecycle_doctrine.md, documents/engineering/pulumi_iac_doctrine.md, documents/engineering/resource_capacity_doctrine.md
...
| Key | Contract |
|---|---|
| `Claim` | UNRESOLVED — blocks validation: typed semantic payload and gate evidence missing; prior prose: one cohesive claim — *a node set is grown and shrunk by a typed signal as an ordinary reconcile*. The quota bound is checked before any cloud mutation, and the arm closes leak-free. Explicit exclusions: every layer named in `Residue` remains UNVERIFIED. |
| `Subject` | UNRESOLVED — blocks validation: no production `.hs` module and entry point have been independently established for this reset contract. |
| `Command` | UNRESOLVED — blocks validation: typed semantic payload and gate evidence missing; prior prose: `pb validate phase 79` is the target command only; `pb` may only make the minimal platform distinction, establish the contained toolchain, build the source-bound binary, and exec it with argv unchanged, while the Haskell verdict entry point remains UNRESOLVED and blocks validation. |
...
| `Predecessor` | UNRESOLVED — blocks validation: the typed generation/compatibility binding still requires implementation. Require authenticated `ImmediatePredecessorPass` for Phase 78 in the admitted certification generation, plus the accepted verifier's current compatibility decision under [§M.6](development_plan_gate_integrity.md#m6-candidate-evidence-and-gate-pass). Missing, forged, revoked, incompatible, or wrong-phase evidence refuses before any phase effect. Historical source identity remains recorded; reuse requires unchanged relevant dependency and acceptance closures. |
...
- [`chaos_failover_doctrine.md` §12 — The moral core — proven, tested, assumed](../documents/engineering/chaos_failover_doctrine.md#12-the-moral-core--proven-tested-assumed)
  ...
  teardown-observation move is skipped. A ledger never makes the phase gate pass.
...
**Blocked by**: [Phase 78](phase_78_provider_ebs_credential.md) gate pass
...
- An `Amoebius.Cluster.NodeProvisioner` that reads the declared elastic-node rule — a typed `ScalingPolicy`
  ...
  [Phase 76](phase_76_provider_deploy_checkpoint.md) `observeProviderAccount` boundary re-read). SKU raw
...
- A staged managed-node join/leave protocol. Provider launch data makes the kubelet register with the exact
  ...
  the active [Phase 59](phase_59_capacity_scheduler.md) scheduler generation. Only an independent readback of
...
- The checked Haskell gate topology, lazily projected to
  ...
  rule and observe it join only after node-scoped scheduler authority (Sprint 79.1), bind the Pulumi-created EBS
  through [Phase 78](phase_78_provider_ebs_credential.md)'s static CSI PV and write a run-unique marker through
...
- An `Amoebius.Pulumi.Teardown` step: one `reconcileAbsent` over the **ephemeral** registry subset (VPC, EKS
  ...
  Unreachable → refuse* — leaving the durable EBS class ([Phase 78](phase_78_provider_ebs_credential.md))
...
1. Run the gate end-to-end over the checked Haskell representative set, lazily projected to
   ...
   parent bootstrap Lease holder releases before the authenticated child control-plane daemon acquires ([Phase 77](phase_77_provider_child_bringup.md)).
...
2. From the provider-stack-absent state left by Validation 1, with the durable EBS retained, run a second full
   spin-up → run → teardown cycle and repeat [Phase 78's retained-volume identity/readback oracle](phase_78_provider_ebs_credential.md)
...
- `DEVELOPMENT_PLAN/system_components.md` — register `Amoebius.Cluster.NodeProvisioner`,
  ...
  first delivery in [Phase 76](phase_76_provider_deploy_checkpoint.md) / [Phase 77](phase_77_provider_child_bringup.md)
  / [Phase 78](phase_78_provider_ebs_credential.md).
...
- [README.md](README.md) — the live tracker; the Phase 79 one-line gate and status
...
- [phase_76_provider_deploy_checkpoint.md](phase_76_provider_deploy_checkpoint.md) — the provider-cluster Pulumi
  deploy, Vault-Transit-enveloped MinIO checkpoint, and `observeProviderAccount` this phase reuses to add/drain a
  node group
- [phase_77_provider_child_bringup.md](phase_77_provider_child_bringup.md) — the stateless hostless in-cluster
  control-plane daemon + capacity-scheduler roles + parent→child Lease handoff the gate composes before node provisioning
- [phase_65_live_dsl_deploy.md](phase_65_live_dsl_deploy.md) — owns the control-plane daemon runtime role reused by
  the provider child
