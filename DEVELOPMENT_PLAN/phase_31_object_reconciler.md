# Phase 31: Typed renderer + object reconciler

> **Purpose**: Take an opaque whole-deployment `ProvisionedSpec`, re-observe and cross-check the target's
> complete resource/capability inventory before mutation, construct the Phase-14 deployment-global
...
- [Sprints](#sprints)
- [Sprint 31.1: Deployment-global desired state + authenticated live inventory + typed action plan ✅](#sprint-311-deployment-global-desired-state--authenticated-live-inventory--typed-action-plan-)
- [Sprint 31.2: Bootstrap Lease authority + generic typed-action dispatcher + scoped SSA + storage-scaling dispatch ✅](#sprint-312-bootstrap-lease-authority--generic-typed-action-dispatcher--scoped-ssa--storage-scaling-dispatch-)
- [Sprint 31.3: Staged execution transitions, Job terminal protocol, and authenticated deletion ✅](#sprint-313-staged-execution-transitions-job-terminal-protocol-and-authenticated-deletion-)
- [Sprint 31.4: Wait-for-ready + the idempotent-convergence gate (re-run no-op) ✅](#sprint-314-wait-for-ready--the-idempotent-convergence-gate-re-run-no-op-)
- [Sprint 31.5: Register-2.5 reconciler + staged-execution convergence under simulated faults ✅](#sprint-315-register-25-reconciler--staged-execution-convergence-under-simulated-faults-)
...
⏸️ Blocked pending Phase-30 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
Blocked (superseded) — reopened 2026-08-16 because Phase 30's successor OCI handoff is being amended to include Pulsar's S3 offloader bundle. Revalidate this phase against the amended exact handoff before Phase 32 proceeds.
...
The run verified Phase 30 attestation `sha256:e6ff9163d299b66b8ce77c216d5114e13af6fe4beb7e299834ee390e9963816a`
...
`kind` cluster from Phase 29, pulling its corpus Deployment from the `distribution` registry Phase 30 published
...
`tools/object_reconciler_execution_live.py` each pinned the Phase-30 base image by a literal digest from a build that is
gone, and the one assertion tying the running pod to Phase 30's published artifact compared against that same
...
Complete. Sprints 27.1–27.5 and the complete Register-3/2.5 phase gate passed. This phase opened after the **Phase 30 gate** (the
native-architecture base image + jit-build resolver + in-cluster `distribution` registry) and the **Phase 29 gate**
...
them to convergence. It builds the **object-reconciler half** of the manifest doctrine: Phase 14 already stood
...
- **2026-08-09 — Sprint 31.1 complete.** The separately validated desired index, authenticated observed
  ...
  unchanged Phase-14 renderer gate. An import lint found no writer boundary in the read-only preflight modules.
  Receipt: `sprint-26.1-receipt.json`, fingerprint
  `dynamically-resolved`.
- **2026-08-09 — Sprint 31.2 complete.** The typed Lease token/renewal rounding, effect dispatcher, and scoped
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 31.3 complete.** The pure serial, ordinary/CUDA/Metal release, Job terminal, and
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 31.4 complete.** A clean Register-3 run converged 19 externally observed objects,
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 31.5 complete.** The real Lease-token, scoped-SSA, serial, host/device-release, Job,
  ...
  independently bounded by Sprint 31.4's live evidence. Receipt:
  `sprint-26.5-receipt.json`, fingerprint
  `dynamically-resolved`.

## Phase Summary

This phase delivers amoebius's live typed-action **object reconciler**. Pure Phase 14 supplies
...
Phase 31. This phase owns the index-preservation and the fail-closed refusal to grant generic apply authority
over a Pod/controller or mutable ledger/Lease field; Phase 32 owns the witnesses that unlock the guarded
stages.
...
TerminalRetained` records are a distinct structure added in Phase 31.) The observed set validates the
...
the exact map of `ValidatedStorageScalingAction`s derived from Phase-9 policy-only envelopes and fresh storage
snapshots. (The scheduler-ledger CAS-version arm of `ValidatedLiveTarget` is added in Phase 31.) A final
...
completion variants, digest/outcome equality, cleanup deadlines, and `CompletedJobNoOp`; the live Phase-31
cluster has neither MinIO nor the Phase-42 sole content-mutation gateway, so it does **not** claim a durable
...
deadline/release → terminal cleanup gate is Phase 41. The Job controller still uses `restartPolicy=Never`,
...
migration/executor is `UnknownCommitment` and fails closed. Phase-29 engine/static components enter through
...
Phase 29; no apple, linux-cuda, or windows substrate is touched (the CUDA/Metal transition arms are exercised
against deterministic observed-device models here, and their live substrate proof is owned by Phase 65 and
Phase 68 respectively).
...
`bind-before-reservation-CAS` mutant are partitioned to Phase 32 and are **not** duplicated here. The
`amoebius-capacity` scheduler bootstrap, its CAS reservation/Binding ledger, and execution-identity admission are
likewise **not** part of this gate; they are Phase 32's gate, layered on this reconciler.

**What the acceptance run must do.** The Phase-14 pure deployment-global `renderAll` list for the pinned
...
Phase-30 registry/proxy objects are authenticated read-only as the one declared bootstrap predecessor and cannot
...
flowchart LR
  ...
  s0["Sprint 31.1: Deployment-global desired state + authenticated live…"]
  s1["Sprint 31.2: Bootstrap Lease authority + generic typed-action dispatcher…"]
  s2["Sprint 31.3: Staged execution transitions, Job terminal protocol, and…"]
  s3["Sprint 31.4: Wait-for-ready + the idempotent-convergence gate (re-run no-op)"]
  s4["Sprint 31.5: Register-2.5 reconciler + staged-execution convergence…"]
...
instantaneous*:
- (i) at least one Deployment whose container image is pulled from the Phase-30 in-cluster `distribution`
...
**The fix that pins item (v) to amoebius's own CRD rather than an external workload operator (e.g. the Percona operator built in Phase 36) is deliberate:** it keeps the corpus and its over-bound-child mutant
self-contained within this phase and earlier phases, removing a latent forward dependency on Phase 35. The
...
Pod without typed authorization — must fail Sprint 31.3; and (e) a **healthy-CR/over-bound-child** mutant whose
...
- [`manifest_generation_doctrine.md §5`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  named by [§5](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions) is Phase 32; durable Job completion/cleanup first runs live in Phase 42; rollback and the release
...
- [`manifest_generation_doctrine.md §2`](../documents/engineering/manifest_generation_doctrine.md#2-the-typed-manifest-model-renderall-is-the-sole-public-pure-function-to-objects)
  — **the typed manifest model** (the pure renderer half): this phase *consumes* the Phase-14 pure, total private
...
- [`deterministic_simulation_doctrine.md §4`](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  — **Register 2.5, where deterministic simulation sits.** Sprint 31.5 validates the *built* reconciler and staged
...
## Sprint 31.1: Deployment-global desired state + authenticated live inventory + typed action plan ✅
...
- `renderAll → validateAndIndexRenderedObjects → observeInventoryAndObjects → authenticateExecutions →
  ...
  grants generic SSA over the full list; the scheduler-cutover stage-unlock witnesses are Phase 32). The observed
...
- A host-aware observed identity union: `KubernetesPod PodUid | HostProcess HostProcessInstanceId |
  ...
  reservation ledger, distinct from the Phase-32 k8s scheduler reservation ledger.
...
- A complete `ValidatedLiveTarget` constructed from one `ObservedLiveResourceSnapshot`: mandatory whole
  ...
  and exact action-domain witness. Its storage-scaling map exact-joins every Phase-9
  ...
  `ValidatedLiveTarget` is added in Phase 31.)
- Execution actions cover `NoOp`, kind-indexed Pod-controller apply, the three serial OnDelete stages, host
  ...
  gateway capability, which is absent from the Phase-31 live environment. Every mutating constructor carries only
...
- Preserve the operator child-admission and migration envelopes: admission/quota policy must be Ready before a CR
  ...
  retain old+new+workspace/temp/WAL until verified cutover. Phase 31 owns only the generic storage-scaling
  observation/validation/token boundary: Phase 33 supplies the retained-carve and verified-migration enactors, and
  Phase 49 supplies the cloud provider-capacity enactor. Durable backing is never generic prune.
...
into the plan tree. Sprint 31.2 consumes the typed action/authority boundary.

## Sprint 31.2: Bootstrap Lease authority + generic typed-action dispatcher + scoped SSA + storage-scaling dispatch ✅
...
(`src/Amoebius/Admission/ExecutionIdentity.hs`) are layered on this dispatcher in Phase 31.
**Blocked by**: none; Sprint 31.1 is sealed.
...
- A dispatcher for every non-scheduler `ValidatedExecutionTransitionAction`. `ApplyDesiredPodController` preserves
  ...
  and prior-controller delete are sent to their own enactors (built in Sprint 31.3); Job completion/cleanup dispatch
  additionally requires the gateway capability and therefore cannot run in the live Phase-31 gate. `NoOp` and
...
- A storage-scaling dispatcher that accepts only a `ValidatedStorageScalingAction`, immediately rechecks its exact
  ...
  migration are abstract effect arms supplied in Phase 33, while provider-capacity creation also consumes the
  transition's exact validated cloud-action batch supplied in Phase 49. Every attempted effect requires post-attempt
...
- Scoped SSA under `fieldManager=amoebius` only for an ordinary desired-object action. It declares the exact owned
  ...
  `ManagedCapacityReady` is added in Phase 31.
...
3. Storage-scaling action tests run at the pure/fake boundary in this phase: stale allocation, backing,
   ...
   a lost response forces a fresh observation. Live retained and cloud mutations remain the Phase-33 and Phase-49
   gates respectively.
...
## Sprint 31.3: Staged execution transitions, Job terminal protocol, and authenticated deletion ✅
...
**Blocked by**: none; Sprint 31.2 is sealed.
...
validated in pure, fake, and IOSim executions here; the live Phase-31 environment intentionally supplies no such
capability.
...
- Job terminal protocol for both `Succeeded` and `FailedBackoffExhausted`: the closed model writes the exact
  ...
  `CompletedJobNoOp`; a failed write retains the terminal Pod/ledger axes and retries safely. In the Phase-31 live
  cluster the gateway capability is absent, so the only inhabitant is `RetainTerminalAwaitingCompletionGateway`; the
  Pod, API bytes, logs/metadata, and retained partition remain charged. Phase 42 performs the first live
...
2. Register-2/2.5 transition tests cover ordinary host exit, CUDA device holds/free VRAM, and Metal drain/allocation/
   ...
   substrate proof remains owned by Phase 65 and Phase 66.
3. Register-1/2 Job tests, available at this sprint's completion, cover all-success and backoff-exhausted failure
   ...
   these same variants are additionally exercised in Sprint 31.5. In the live Phase-31 run, an independent apiserver
   observer proves the terminal UID remains, no Job-completion object is written, and no delete occurs. The first live
   object-store/apiserver persist-before-delete proof is the Phase-42 gate.
...
None. Sprint 31.4 composes these actions into the full convergence/no-op corpus.

## Sprint 31.4: Wait-for-ready + the idempotent-convergence gate (re-run no-op) ✅
...
amoebius validation), `DEVELOPMENT_PLAN/README.md` (flip the Phase-31 status when the gate passes).
...
- The convergence battery over the pinned corpus: enact the exact `renderAll`/action plan → observe workload
  ...
  bind-before-CAS mutants are Phase 32's gate.)
...
## Sprint 31.5: Register-2.5 reconciler + staged-execution convergence under simulated faults ✅
...
real Manifest/Execution modules above on the Phase-15 `io-classes` `Env` — built and validated. (The
`test/spec/sim/SchedulerSim.hs` schedule battery is Phase 31.)
**Blocked by**: none; Sprint 31.4 is sealed.
...
replay by seed.
**Docs to update**: `documents/engineering/deterministic_simulation_doctrine.md` (Phase-31
...
open. The scheduler's CAS-race schedules are Phase 32's `SchedulerSim`.
...
- Committed mutants for lost `Lease`/resourceVersion retry, mutation without holder, sleep-gated readiness, serial
  ...
  collapsed scheduler readiness stages, premature managed taint/full RBAC — are Phase 32's `SchedulerSim`.)
- A Register-2.5 proven/tested/assumed ledger — the reconciler upholds convergence + fail-closed under the modeled
  schedules and faults; honest limit: modeled-apiserver fidelity is **assumed**, discharged by the Sprint-26.4
...
- `documents/engineering/manifest_generation_doctrine.md` — §5's typed SSA/staged-action/delete/wait engine flips
  ...
  design intent until Phase 32); §6's planned-vs-observed and `renderAll` model gains its first validation. Keep
...
- `DEVELOPMENT_PLAN/system_components.md` — register the Phase-31
  ...
  `Admission/ExecutionIdentity` modules are registered by Phase 31.)
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — [§4](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits) the
  Register-2.5 io-sim environment the reconciler is validated against in Sprint 31.5, before the Register-3 live gate
...
- [phase_14_render_manifest_goldens.md](phase_14_render_manifest_goldens.md) — the pure per-projection renderers and
  deployment-global `renderAll` owner-union goldens this phase enacts
- [phase_15_chain_kernel_boundary.md](phase_15_chain_kernel_boundary.md) — the Register-2 fake-apply this phase
  replaces with real tools, and the `io-classes` seams Sprint 31.5 drives
- [phase_16_deterministic_sim_substrate.md](phase_16_deterministic_sim_substrate.md) — the `IOSimPOR`
  deterministic-simulation substrate Sprint 31.5 runs on
- [phase_29_bootstrap_coordinator_kind.md](phase_29_bootstrap_coordinator_kind.md) — the live single-node `kind` cluster this
  phase applies to
- [phase_30_base_image_registry.md](phase_30_base_image_registry.md) — the in-cluster registry the applied workloads
  resolve images against
- [phase_32_capacity_scheduler.md](phase_32_capacity_scheduler.md) — the `amoebius-capacity` scheduler, two-stage
