# Phase 47: Multi-cluster spawn + geo-replication
...
> over which [Phase 48](phase_48_gateway_migration_drills.md) drives the gateway-migration runtime.
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_46_network_fabric_wireguard.md, DEVELOPMENT_PLAN/phase_48_gateway_migration_drills.md, DEVELOPMENT_PLAN/phase_49_provider_deploy_checkpoint.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 47.1: Amoebic spawn — `project(subtree)` handoff + per-child unseal / Transit key / secret injection ⏸️](#sprint-471-amoebic-spawn--projectsubtree-handoff--per-child-unseal--transit-key--secret-injection-)
- [Sprint 47.2: Geo-replication of two siblings + invariant-confluence classification ⏸️](#sprint-472-geo-replication-of-two-siblings--invariant-confluence-classification-)
...
the retained HA Phase-29 platform. A physically independent Pulsar broker set and a Vault server inside each
...
The design prerequisite is already sealed: [Phase 4](phase_04_gateway_migration_model.md) validated the shared
`GatewayMigration` value with TLC/explorer/IOSimPOR at Register 1. This phase does not reopen that design proof;
Phase 48 still owns correspondence to the live forest and remains UNVERIFIED until its Register-3 gate.
...
non-confluent bucket — is precisely the boundary hand-off the [Phase 48](phase_48_gateway_migration_drills.md)
...
The pure `Rke2ServerNode`/`Rke2AgentNode` and reserve/template folds exist in Phases 5–10, but no live
...
[Phase 49](phase_49_provider_deploy_checkpoint.md)). Before either child is created, the parent-owned
...
[Phase 48](phase_48_gateway_migration_drills.md) drills; here the boundary is established and classified.
...
flowchart LR
  ...
  g{{"the phase 47 gate command"}}:::gate
...
```
*Design intent. Phase 47's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
- [`cluster_lifecycle_doctrine.md §3`](../documents/engineering/cluster_lifecycle_doctrine.md#3-amoebic-spawning--the-recursive-forest)
  ...
  gateway-migration drills of [Phase 48](phase_48_gateway_migration_drills.md).
...
- [`content_addressing_doctrine.md §5`](../documents/engineering/content_addressing_doctrine.md#5-confluence-content-addressed-data-crosses-cluster-boundaries-safely)
  ...
  gateway authority and any CAS "latest" pointer in bucket (ii) for the [Phase 48](phase_48_gateway_migration_drills.md) migration runtime.
- [`chaos_failover_second_axis.md §16`](../documents/engineering/chaos_failover_second_axis.md#16-the-second-axis--when-one-cluster-becomes-a-forest)
  ...
  honest. The R7/R8/R9 boundary rules and the [§19](../documents/engineering/chaos_failover_second_axis.md#19-the-cross-boundary-ledger-and-conformance-rows) cross-boundary ledger are consumed by [Phase 48](phase_48_gateway_migration_drills.md).
...
## Sprint 47.1: Amoebic spawn — `project(subtree)` handoff + per-child unseal / Transit key / secret injection ⏸️
...
## Sprint 47.2: Geo-replication of two siblings + invariant-confluence classification ⏸️
...
(ii) for the [Phase 48](phase_48_gateway_migration_drills.md) migration runtime.
...
- `documents/engineering/cluster_lifecycle_doctrine.md` — §3/§9 gain the realized module paths for the spawn,
  ...
  land with [Phase 48](phase_48_gateway_migration_drills.md)).
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-47 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/system_components.md` — register the `src/Amoebius/Multicluster/Spawn.hs`,
  ...
  gate suites as Phase-47 rows.
- `DEVELOPMENT_PLAN/substrates.md` — the Phase-47 → linux-cpu row in the per-phase substrate map.
...
- [phase_46](phase_46_network_fabric_wireguard.md) — the prior phase (the WireGuard fabric); its gate opens this one
- [phase_48](phase_48_gateway_migration_drills.md) — the next phase; the gateway-migration runtime and correspondence over this forest
- [phase_49](phase_49_provider_deploy_checkpoint.md) — the forest extended to provider-managed clusters
