...
- **Phase-0 representative set.** Phase 0 commits
  ...
  `test/fixtures/phase_50/expected_interaction.tsv`, and a fixed public model/input corpus. The program uses
  `WorkflowProgress`, `ArtifactProvenance`, and `ModelInteractor` from the trusted catalog.
- **Fresh authority and challenge.** After the edge, UI server, and infernix worker are Ready, the harness
  ...
  nonce-bearing public input. Tenant A's workflow and inference must carry the nonce to the independently
  recomputed typed result.
...
- **Committed mutant.** Phase 0 commits
  `test/mutants/phase_50/mut-50-trust-client-artifact-scope.patch` (guard weakening). It accepts the
  foreign-scope handle and must turn the zero-effect/scope matrix red.
...
Pulsar, MinIO, worker, and browser observations and requires the committed scope mutant to fail.
...
- The committed `mut-50-trust-client-artifact-scope` mutant.
...
2. Start the nonce-bound workflow, wait for committed success, verify ready-handle issuance, invoke it, and
   compare browser output with the independent result oracle.
3. Replay the exact handle/input under tenant B and assert the pinned denial plus zero external effect.
4. Assert the hostile output remains escaped presentation and cannot select any authority-bearing value.
5. Apply the named mutant and require the unchanged command to fail before the ledger can be green.
