# Phase 59: Determinism kernel + jit-build CacheBudget cache
...
- [Sprints](#sprints)
- [Sprint 59.1: `ContentAddress` typeclass kernel primitive ⏸️](#sprint-591-contentaddress-typeclass-kernel-primitive-)
- [Sprint 59.2: `experimentHash` identity over the live substrate fingerprint ⏸️](#sprint-592-experimenthash-identity-over-the-live-substrate-fingerprint-)
- [Sprint 59.3: SplitMix seed derivation, worker-count-independent ⏸️](#sprint-593-splitmix-seed-derivation-worker-count-independent-)
- [Sprint 59.4: The live same-substrate reproducibility gate ⏸️](#sprint-594-the-live-same-substrate-reproducibility-gate-)
- [Sprint 59.5: The `CacheBudget`-bounded content-addressed cache + peak-occupancy provision fold ⏸️](#sprint-595-the-cachebudget-bounded-content-addressed-cache--peak-occupancy-provision-fold-)
- [Sprint 59.6: The jit-build resolver — `resolve = {download | build}` on first miss, no URL arm ⏸️](#sprint-596-the-jit-build-resolver--resolve--download--build-on-first-miss-no-url-arm-)
- [Sprint 59.7: Per-node cache-owner reuse across client pods ⏸️](#sprint-597-per-node-cache-owner-reuse-across-client-pods-)
- [Sprint 59.8: The live first-miss / reuse / resource-admission gate + Register-3 ledger ⏸️](#sprint-598-the-live-first-miss--reuse--resource-admission-gate--register-3-ledger-)
...
brought up by the Phase 35 bootstrap coordinator, whose base image (Phase 36) already bakes the shared **jit-build resolver and its build toolchain** but **no** ML engine payload, using the content-addressed store and workflow runtime
landed in Phase 47. The `experimentHash`/SplitMix shapes are already exercised in the sibling `jitML` project
...
This phase turns the content-addressed store delivered in Phase 48 into one cohesive **reproducible resolved- asset lifecycle**. Its ordered sprint seams first establish the shared determinism identity and then use that
...
**The determinism kernel.** First, it lifts Phase 48's concrete blob/manifest key renderers into a kernel-level
...
a prebuilt engine or builds it from source (using the Phase-36 baked toolchain) into the cache; there is no arm
...
"More cached than fits" is rejected at the post-bind `provision-seal` by the Phase-14 capacity fold, not
...
a request-carried derived seed — deliberately **not** an infernix inference run (that lift is Phase 60) and
**not** a jit-resolved ML engine (this phase resolves the engine, but the infernix inference that rides it is
Phase 60; the CUDA/jitML kernel tier is Phase 71). The `ModelArtifact` staging tier (Tier 2) and the JIT kernel
...
%% register: algebra
  ...
  ca[["ContentAddress typeclass"]]:::intent --> store[/"Phase 48 content-addressed store"/]:::effect
  eh --> store
...
**The two-part acceptance condition.** The run happens on the single-node linux-cpu `kind` cluster — Phase 35
bootstrap coordinator, Phase 36 base image plus baked resolver/toolchain plus in-cluster `distribution` registry,
Phase 48 content store plus workflow runtime. Every *how-it-behaved* claim is read from an OS-boundary observer
...
observation, or bounded-parallel-derived overflow is rejected by the Phase-14 fold at the Phase-18
**`provision-seal`** before any resolve. The run emits a Register-3 proven/tested/assumed ledger.

```mermaid
flowchart LR
  %% register: orientation
  s0["Sprint 59.1: ContentAddress typeclass kernel primitive"]
  s1["Sprint 59.2: experimentHash identity over the live substrate fingerprint"]
  s2["Sprint 59.3: SplitMix seed derivation, worker-count-independent"]
  s3["Sprint 59.4: The live same-substrate reproducibility gate"]
  s4["Sprint 59.5: The CacheBudget-bounded content-addressed cache…"]
  s5["Sprint 59.6: The jit-build resolver — resolve = {download | build} on…"]
  s6["Sprint 59.7: Per-node cache-owner reuse across client pods"]
  s7["Sprint 59.8: The live first-miss / reuse / resource-admission gate…"]
  gate["the phase 59 gate"]
...
```
*Validated orientation. The seams Phase 59 built in order; [Gate integrity](#gate-integrity) owns the apparatus.*
...
backoff, replacement-on-Failed, terminal retention) and is also an exact Phase-48 `WorkflowRuntimeDemand`
...
Phase-48 runtime/mutation unit) must each turn the resource gate red even if the output bytes match. For the
...
- [`content_addressing_doctrine.md §2`](../documents/engineering/content_addressing_doctrine.md#2-the-three-tier-store-blobs--manifests--pointers)
  — *the three-tier store (blobs ← manifests ← pointers)*: the `ContentAddress` typeclass lifts Phase 48's
...
- [`resource_capacity_doctrine.md §3`](../documents/engineering/resource_capacity_doctrine.md#3-the-types-quantity-capacity-demand-budget)
  ...
  ephemeral-storage envelope, and the derived peak bound is the **same** checked capacity fold Phase 14 built and
  Phase 18 invokes at `provision-seal` — "more cached than fits" is rejected by that fold, not discovered as a
  runtime disk-full.
- [`image_build_doctrine.md §7`](../documents/engineering/image_build_doctrine.md#7-what-amoebius-bakes-vs-builds--the-base-container-is-the-supply-chain)
  ...
  identities — the Phase-36 split this phase exercises live for the first time.
...
## Sprint 59.1: `ContentAddress` typeclass kernel primitive ⏸️
...
lift Phase 48's concrete blob/manifest key renderers into a kernel-level `ContentAddress` typeclass so that
"a content-derived name cannot be forged" is a single reusable primitive shared later by the engine cache
(Sprint 59.5) and by both infernix (Phase 60) and jitML (Phase 71), not a per-store copy.
...
- Adapters binding the typeclass to Phase 48's `blobs/<sha256>` and `manifests/<sha256>` writers — the
...
## Sprint 59.2: `experimentHash` identity over the live substrate fingerprint ⏸️
...
consuming the Phase-11 normal form and the Phase-35 full-path substrate probe, per the substrate doctrine's
...
## Sprint 59.3: SplitMix seed derivation, worker-count-independent ⏸️
...
## Sprint 59.4: The live same-substrate reproducibility gate ⏸️
...
without an infernix inference run (Phase 60) or a jit-resolved engine (that is the cache seam below and the
Phase-60 lift), and prove same-substrate reproducibility as one half of the phase gate without overclaiming
...
- The gate `.dhall` (`test/fixture/dhall/phase_49_determinism_repro.dhall`) that spins up the Phase-48 workflow, runs the
...
## Sprint 59.5: The `CacheBudget`-bounded content-addressed cache + peak-occupancy provision fold ⏸️
...
### Deliverables
- `Amoebius.Jit.Cache` — a bounded typed pool keyed by `sha256(resolved-bytes)` (the Sprint-54.1 `ContentAddress`),
...
1. There is no exported path to a cache key from a free string; the only path to a resident entry is content
   ...
   (registered in the Phase-13 negative corpus, authored in this phase's oracle-pinning sprint) failing to
...
4. **Pin-aware pruning is exercised, not declared:** a cache filled to `CacheBudget` with a mix of pinned and
   ...
   ([Gate integrity](#gate-integrity) part (b) mutant (b)) must turn this clause red (the over-budget residency survives). This is the pure-pool property; its live on-disk counterpart is the Sprint 59.8 postflight residency
...
## Sprint 59.6: The jit-build resolver — `resolve = {download | build}` on first miss, no URL arm ⏸️
...
miss into the bounded cache — downloaded prebuilt or built from source with the Phase-36 baked toolchain — with
...
- The build-from-source path invoking the Phase-36 baked toolchain by absolute path (no `PATH`, no env), and the
...
1. The suite drives the resolver against an oracle-pinned backend fixture whose served or compiled bytes
   **sha256-match the `test/oracle/determinism_jitcache/oracle.dhall` pin** — not an arbitrary "fake" blob, so a backend
   ...
   (Phase-13 corpus, independently authored) failing at the constructor locus with "no `Url`/free-string
...
### Remaining Work
Production model inference through the resolved engine is Phase 60, not a hidden acceptance claim here.

## Sprint 59.7: Per-node cache-owner reuse across client pods ⏸️
...
- The pod wiring (rendered by the Phase-37 reconciler) that gives the owner a bounded disk-backed `emptyDir` with
...
## Sprint 59.8: The live first-miss / reuse / resource-admission gate + Register-3 ledger ⏸️
...
**Docs to update**:
`documents/engineering/content_addressing_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-59
...
2. With the cache filled to budget, a postflight on-disk peak/final measurement confirms pin-aware eviction and
   ...
   ephemeral-under-reserved owner each return their **tagged** `Left` at the Phase-14 fold at the Phase-18
...
### Remaining Work
The Tier-2 model and Tier-3 CUDA kernel reuse remain assigned to Phases 60 and 71 respectively.
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Kernel/ContentAddress.hs`,
  ...
  `test/negative/determinism_jitcache/url_arm.hs`, `test/mutant/determinism_jitcache/determinism/`, `test/mutant/determinism_jitcache/cache/`) as Phase-59
  design-first rows.
...
- [phase_14](phase_14_capacity_core_folds.md) — the `fits`/`carve` capacity fold the `CacheBudget` bound reuses
- [phase_18](phase_18_provision_seal.md) — the whole-deployment provision seal where the over-budget cache peak is rejected
- [phase_19](phase_19_inference_accelerator_provision.md) — the `InferenceEngine` binder + closed `EngineRuntime` union the resolver keys on
- [phase_21](phase_21_chain_kernel_boundary.md) — the `chain`/`Step` kernel the kernel primitives plug into
- [phase_35](phase_35_bootstrap_coordinator_kind.md) — the bootstrap coordinator + single-node `kind` cluster + substrate fingerprint this phase runs on
- [phase_36](phase_36_base_image_registry.md) — the base image that bakes the jit-build resolver + toolchain this phase drives live
- [phase_37](phase_37_object_reconciler.md) — the typed SSA object reconciler that renders the cache owner and client pods
- [phase_41](phase_41_platform_backbone.md) — the platform backbone the cache owner/client pods schedule onto
- [phase_48](phase_48_content_store_workflow.md) — the content store + workflow runtime this phase lifts and runs on
- [phase_60](phase_60_infernix_lift.md) — the infernix CPU-inference lift whose reproducibility reuses this kernel and rides this resolver next
- [phase_71](phase_71_jitml_lift_cuda.md) — the jitML/CUDA lift whose kernel tier reuses this resolver and kernel
