# Phase 55: Provider Pulumi deploy-from-inside + enveloped checkpoint
...
- [Sprints](#sprints)
- [Sprint 55.1: Provider-cluster Pulumi deploy from inside a parent ⏸️](#sprint-551-provider-cluster-pulumi-deploy-from-inside-a-parent-)
...
brought up by the [Phase 35](phase_35_bootstrap_coordinator_kind.md) bootstrap coordinator, from inside which the Pulumi engine — under the Deployment-`replicas=1`
...
This is the **first arm** of the provider-managed-cluster split (Phases 55–58): the mechanism by which
...
linux-cpu parent, plus its gate; it is the substrate on which [phase_56_provider_child_bringup.md](phase_56_provider_child_bringup.md)
(the stateless hostless in-cluster control plane), [phase_57_provider_ebs_credential.md](phase_57_provider_ebs_credential.md)
(per-PV EBS + the create-vs-delete credential model), and [phase_58_provider_dynamic_nodes.md](phase_58_provider_dynamic_nodes.md)
...
an already-running amoebius cluster, issued by the Deployment-`replicas=1` control-plane daemon (Phase 44) —
whose single-instance is a k8s/etcd property, never a bespoke amoebius election — generalizing Phase 53's
...
or standard-service convergence for a provider child ([phase_56_provider_child_bringup.md](phase_56_provider_child_bringup.md));
per-PV durable EBS, the `protect`/`Retain` state class, the create-vs-delete credential split, or the static
`ebs.csi.aws.com` CSI path ([phase_57_provider_ebs_credential.md](phase_57_provider_ebs_credential.md)); or
dynamic node provisioning by signal, the second reconcile no-op, and the independent leak-free teardown sweep
([phase_58_provider_dynamic_nodes.md](phase_58_provider_dynamic_nodes.md)). The base node group here is a fixed
size-1 group provisioned and observed by this sub-phase alone; the `Managed Eks` arm carries **no** `LinuxHost`
witness (a foreclosure already unrepresentable in the pre-cluster band — the Dhall dhall-typecheck schema in Phase 11,
the GADT decoder in Phase 12, and the capacity/topology folds in Phase 14).
...
> **Provider corpus split (by design).** Phases 55–58 (the provider split) share one provider **topology > shape**, but each sub-phase commits its own Phase-0 corpus and gates only its own slice: the topology
...
teardown-sweep and dynamic-node slice (`mut-47.2-skip-sweep`, `mut-47.1-ignore-signal`,
`mut-47.1-apply-over-quota`, the expected-sweep oracle, and the two-instance identity map) is
[phase_58_provider_dynamic_nodes.md](phase_58_provider_dynamic_nodes.md)'s; the per-PV durable EBS +
create-vs-delete slice (the `ec2:DeleteVolume` expected-denied-call tag, the `protect`/`Retain` state, the
static-CSI `volumeHandle` bind, `mut-46.1-*`) is [phase_57_provider_ebs_credential.md](phase_57_provider_ebs_credential.md)'s;
and the scheduler-cutover slice (`mut-45.1-public-pull`, the `LinuxHost`-absence foreclosure tag) is
[phase_56_provider_child_bringup.md](phase_56_provider_child_bringup.md)'s. Partitioned to this seam:
...
[phase_58_provider_dynamic_nodes.md](phase_58_provider_dynamic_nodes.md)'s gate, deferred and never depended
...
- `mut-44.1-static-key` — an envelope keyed by a pod-local static key with the seal-status precheck still
...
- `mut-44.1-leak-path` — an engine that exports `PATH` into the child process. It MUST go red on the OS-boundary
  `execve` env assertion.
- `mut-44.1-drop-parallel-executor` — omits one live Job from the executor peak, or admits parallel demand then
...
- [`pulumi_iac_doctrine.md §1`](../documents/engineering/pulumi_iac_doctrine.md#1-pulumi-runs-only-from-inside-an-existing-amoebius-cluster)
  ...
  than a bespoke state machine. (The EBS create-vs-delete credential arm of [§6](../documents/engineering/pulumi_ebs_credential_model.md#6-the-ebs-create-vs-delete-credential-model) is realized in [phase_57_provider_ebs_credential.md](phase_57_provider_ebs_credential.md), not here.)
- [`cluster_lifecycle_doctrine.md §3`](../documents/engineering/cluster_lifecycle_doctrine.md#3-amoebic-spawning--the-recursive-forest)
  — *amoebic spawning — the recursive forest*: provider-cluster spawn is the *cloud-keyed* sibling of Phase 53's
...
## Sprint 55.1: Provider-cluster Pulumi deploy from inside a parent ⏸️
...
**first delivered by Phase 53 and reused here, not rebuilt**; the `observeProviderAccount` boundary and the
...
object set, generalizing Phase 53's SSH-keyed self-managed spawn to a cloud-keyed provider spawn. The `pulumi`
...
- An `Amoebius.Pulumi.Provider.Eks` program that provisions the EKS control plane + a base managed node group
  ...
  the cluster ready for its in-cluster control-plane bootstrap ([phase_56_provider_child_bringup.md](phase_56_provider_child_bringup.md)).
...
- `planInfrastructure` derives the provider-cluster demand from the exact `BoundDeployment` and declared
  ...
  [phase_57_provider_ebs_credential.md](phase_57_provider_ebs_credential.md).) Only the receipt-bound provider
...
> `mut-47.2-skip-sweep`) is [phase_58_provider_dynamic_nodes.md](phase_58_provider_dynamic_nodes.md)'s gate,
...
- `documents/engineering/cluster_lifecycle_doctrine.md` — record that §3 (the cloud-keyed amoebic spawn) gains
  an amoebius EKS reference as the cloud-keyed sibling of the Phase 53 SSH-keyed spawn.
...
- `documents/engineering/testing_doctrine.md` — record the Phase 55 per-run ledger artifact for the
...
- `DEVELOPMENT_PLAN/system_components.md` — map the reused `amoebius-pulumi` Engine and EncryptedMinio backend
  to their first delivery in Phase 53; register `Amoebius.Pulumi.Provider.Eks`, the `observeProviderAccount`
  boundary, and the `PulumiExecutionDemand` / `PulumiCheckpointObjectDemand` types as Phase-55 design-first rows,
  each mapped to its owning doctrine.
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 55 → `linux-cpu` (parent) row with the `provider` (EKS)
  deploy target annotated as a target class, not a fifth hardware substrate.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase 55 row's status once the gate passes; link this document.

## Related Documents

- [README.md](README.md) — the live tracker; the Phase 55 objective, gate, and substrate
...
- [phase_53](phase_53_multicluster_spawn_georepl.md) — the SSH-keyed amoebic spawn, encrypted MinIO backend, and
  per-child envelope this phase generalizes to a cloud-keyed provider spawn
- [phase_56](phase_56_provider_child_bringup.md) — the stateless hostless in-cluster control-plane daemon + capacity-scheduler
  bring-up that layers on this deploy
- [phase_57](phase_57_provider_ebs_credential.md) — the per-PV EBS + create-vs-delete credential model + static
  CSI path that layers on this deploy
- [phase_58](phase_58_provider_dynamic_nodes.md) — dynamic node provisioning by signal and the independent
