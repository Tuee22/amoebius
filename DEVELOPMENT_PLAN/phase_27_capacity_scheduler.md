# Phase 27: amoebius-capacity scheduler + bootstrap cutover

> **Purpose**: Build the same-binary `amoebius-capacity` scheduler as a dedicated role layered on the
> Phase-26 reconciler — the state-indexed reservation ledger, the two-stage bootstrap taint/RBAC cutover
...
statement is design intent, never a tested amoebius result. This phase opens after the **Phase 26 gate** (the
...
live reconciler: Phase 26 converges the object corpus with its Pods scheduled by the default scheduler and
...
The scheduler owns the **state-indexed reservation ledger**. On top of Phase 26's `ObservedLiveResourceSnapshot`
...
those are Phase 26's and are consumed here as the substrate the scheduler runs on. It owns only the scheduling
authority. The scheduler is driven from the **host binary** against the same scratch namespace as Phase 26; the
...
```
*Design intent. Phase 27's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
- [`manifest_generation_doctrine.md §5`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  authenticated deletion, and the rollback/release ledger are Phase 26's or stay deferred.
...
snapshot with the scheduler dimension; target paths, not yet built.
**Blocked by**: Phase 26 gate (the
object reconciler, `ObservedLiveResourceSnapshot`, and `ValidatedLiveTarget` construction).
**Independent Validation**: a fresh snapshot over the pinned corpus normalizes each reservation state exactly once and
produces the committed scheduler-ledger slice of
...
— the reconcile state model, scheduler-ledger slice. Extend Phase 26's observed inventory with the
...
- A scheduler-ledger join over full `Reserved | BindingInFlight | Bound | Terminating | TerminalRetained`
  records plus resourceVersion/CAS version, added to Phase 26's `ObservedLiveResourceSnapshot` (empty before
...
- The host-aware observed identity union consumed from Phase 26 —
...
- State-sensitive Binding recovery at normalization time: an unknown/unbound `BindingInFlight` row remains
  ...
  Pod-UID runtime-storage row (from Phase 26's `RuntimeStorage`) and exact-joins it once with the
  still-`BindingInFlight` reservation — the later repair CAS (Sprint 27.3) changes state, not capacity.
- The scheduler-ledger slice of the `ValidatedLiveTarget`: the ledger CAS version, the normalized
  ...
  handed to Phase 26's target constructor. `PlannedExecutionSlotId` is never equated with a Pod UID; every
...
implementation or a kube-scheduler plugin.
**Blocked by**: Sprint 27.1; Phase 25 gate (the
preloaded/side-loaded base image + in-cluster registry); Phase 26 gate (the reconciler holding the mandatory
reconciler `Lease`).
...
- A read-only scheduler preflight (layered on the Phase-26 reconciler already holding
...
3. `ceil(renewalWindow/retryPeriod) <= maxRenewalsPerWindow` is not re-proven here (the `Lease` is Phase 26's),
   but the scheduler preflight asserts it runs **only** while the Phase-26 reconciler is the exact `Lease`
...
`src/Amoebius/Admission/ExecutionIdentity.hs` — target paths, not yet built.
**Blocked by**: Sprint 27.1,
Sprint 27.2, Phase 7 gate (the `place`/`fits`/`carve` resource-algebra fold the placement invokes).
...
- The reservation transitions are the scheduler's members of Phase 26's `ValidatedExecutionTransitionAction`
  domain; the scheduler never enters Phase 26's generic scoped-SSA path (`fieldManager=amoebius`), and every
...
2. The seeded mutants `bind-before-reservation-CAS`, `numeric-add-instead-of-whole-ledger-refold`,
   ...
   digest or collapses the two witnesses (from Sprint 27.2) also turns the reservation suite red when it lets a
...
4. The scheduler modules import no generic SSA writer; the lint asserts scheduler/ledger/Binding actions never
   enter Phase 26's `Manifest/Apply.hs` scoped-SSA module, and `NoOp` carries no writer capability.
...
- A Register-3 proven/tested/assumed ledger recording the live scheduling authority, marking the
  release-ledger/rollback residue and the in-cluster-singleton ownership (Phase 33) UNVERIFIED (deferred).
...
not yet built. This is the scheduler slice of the deterministic-simulation battery; Phase 26 owns the
object-reconciler slice of the same environment.
**Blocked by**: Sprint 27.4; the Phase-15
deterministic-simulation substrate and the Phase-14 `io-classes` seams / modeled apiserver.
...
**Docs to update**: `documents/engineering/deterministic_simulation_doctrine.md` (Phase-27 status backlink),
...
- `documents/engineering/manifest_generation_doctrine.md` — §5's amoebius scheduler-role CAS/Binding protocol
  ...
  delete/wait engine remains Phase 26's; the rollback/release ledger stays deferred.
...
- `documents/engineering/readiness_ordering_doctrine.md` — the §6 runtime-enactor claim (observe, never sleep)
  ...
  slept).
- `documents/engineering/daemon_topology_doctrine.md` — record that Phase 27 drives the scheduler role from the
...
- `documents/engineering/deterministic_simulation_doctrine.md` — record the Phase-27 scheduler slice of the
  Register-2.5 io-sim battery (Sprint 27.5), with modeled-apiserver fidelity marked assumed.

**Cross-references to add:**
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-27 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 27's gate substrate (linux-cpu) in the per-phase substrate
  map.
...
- [phase_26_object_reconciler.md](phase_26_object_reconciler.md) — the object reconciler (observe → diff → scoped-SSA → staged-enact → delete → wait, and the `ValidatedLiveTarget` + mandatory `Lease`) this phase is
  layered on
...
- [phase_24_midwife_bootstrap_kind.md](phase_24_midwife_bootstrap_kind.md) — the live single-node `kind`
...
- [phase_33_live_dsl_singleton.md](phase_33_live_dsl_singleton.md) — the Deployment-`replicas=1` singleton that
...
- [phase_14_chain_kernel_boundary.md](phase_14_chain_kernel_boundary.md) — the `io-classes` seams / modeled
  apiserver the Register-2.5 scheduler sim (Sprint 27.5) drives the real modules on
...
- [Daemon Topology Doctrine](../documents/engineering/daemon_topology_doctrine.md) — [§3](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-singleton) the Deployment-
  `replicas=1` singleton that will own this scheduling role in Phase 33
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the
  Register-2.5 io-sim environment the scheduler is validated against in Sprint 27.5, before the Register-3 gate
