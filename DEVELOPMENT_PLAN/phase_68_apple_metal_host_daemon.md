# Phase 68: Apple-Metal host compute daemon
...
> sole Phase-42 mutation gateway fronting MinIO, with the native worker built **headless
...
Phase 68 delivers the Apple-Metal host compute daemon; its design is owned by [host_cluster_comms_doctrine.md](../documents/engineering/host_cluster_comms_doctrine.md), [substrate_doctrine.md](../documents/engineering/substrate_doctrine.md), [resource_capacity_types.md](../documents/engineering/resource_capacity_types.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 68.1: Apple substrate management — Lima Linux VM + brew lazy tool-ensure ⏸️](#sprint-681-apple-substrate-management--lima-linux-vm--brew-lazy-tool-ensure-)
- [Sprint 68.2: Host-only loopback NodePort exposure of the content-mutation gateway + Pulsar ⏸️](#sprint-682-host-only-loopback-nodeport-exposure-of-the-content-mutation-gateway--pulsar-)
- [Sprint 68.3: Headless host-native Metal bridge + native worker build (no Tart) ⏸️](#sprint-683-headless-host-native-metal-bridge--native-worker-build-no-tart-)
- [Sprint 68.4: Host compute daemon lifecycle as a managed subprocess ⏸️](#sprint-684-host-compute-daemon-lifecycle-as-a-managed-subprocess-)
- [Sprint 68.5: Channel-2 peer + wild-exposure unrepresentable + the Apple-Metal peer gate ⏸️](#sprint-685-channel-2-peer--wild-exposure-unrepresentable--the-apple-metal-peer-gate-)
...
⏸️ Blocked pending Phase-67 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
binds the in-cluster Phase-42 content-mutation gateway and Pulsar service to **host-only loopback NodePorts** reachable only
...
messages, mutations pass only through the Phase-42 gateway into the content-addressed MinIO store, and there is
...
The scope stops at the host-worker shell and its wire. [Phase 53](phase_53_determinism_jitcache.md) owns resolution
...
them: Phase 29's substrate detection, `pb` bootstrap coordinator handoff, and no-`PATH` tool-ensure kernel; Phase 35's MinIO
and Pulsar standard services; Phase 40's native Pulsar client; Phase 42's content-addressed store and workflow
runtime; Phase 53's determinism kernel; Phase 53's jit-build engine cache; and Phase 34's Vault secrets-by-name.

```mermaid
flowchart LR
%% register: orientation
  apple[Apple Silicon host: detected apple substrate] --> lima[Lima Ubuntu-24.04 VM: single-node cluster]
  lima --> svc[Phase-42 content-mutation gateway fronting MinIO plus Pulsar]
...
- **Representative set (§M.7).** The gate's representative set is exactly: one apple substrate (Apple-Silicon
  ...
  and the four wild-exposure negatives enumerated in Sprint 68.5. No other shape is claimed.
- **VM-disk geometry is materialized and observed (§M.3/§M.4).** Before create, the independent oracle
  ...
  twice.
- **Leak-free, defined ([§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub)).** "Tears down leak-free" at Phase 68 means, asserted by the postflight probe:
  ...
  swept to its pre-run state. Phase 56's general postflight sweep runs earlier in the sequence, but it is written for the cluster-side residue and does not reach a host subprocess's Metal state, so this phase pins its
...
exposes the sole Phase-42 content-mutation gateway and Pulsar (not raw MinIO) on loopback NodePorts, builds
...
The worker's source-equal Phase-40/42 work/result topic and subscription demand also retains finite message/
...
The host content client terminates at the Phase-42 sole content-mutation gateway NodePort; raw MinIO backend
...
This phase applies the exhaustive desired-object and etcd-capacity protocol established in Phase 46 to the
...
- [`host_cluster_comms_doctrine.md §2`](../documents/engineering/host_cluster_comms_doctrine.md#2-the-decision-that-was-open-and-is-now-resolved)
  ...
  (*coordination is Pulsar + MinIO*, with MinIO mutation mediated by the Phase-42 gateway), the
...
## Sprint 68.1: Apple substrate management — Lima Linux VM + brew lazy tool-ensure ⏸️
...
invoked by absolute path through brew — the substrate foundation every later Phase-68 sprint stands on. The
...
## Sprint 68.2: Host-only loopback NodePort exposure of the content-mutation gateway + Pulsar ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `src/Amoebius/HostComms/NodePort.hs`,
`src/Amoebius/HostComms/Loopback.hs`
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: after bring-up, the Phase-42
...
## Sprint 68.3: Headless host-native Metal bridge + native worker build (no Tart) ⏸️
...
- Runtime MSL compilation: the host binary renders Metal Shading Language, writes a content-addressed
  source-metadata cache record into the Phase-53 `CacheBudget`-bounded cache, and dispatches through the
  bridge's `MTLDevice.makeLibrary(source:options:)` with fast-math **off** (the Phase-53 determinism
...
## Sprint 68.4: Host compute daemon lifecycle as a managed subprocess ⏸️
...
## Sprint 68.5: Channel-2 peer + wild-exposure unrepresentable + the Apple-Metal peer gate ⏸️
...
through the Phase-42 mutation gateway on `127.0.0.1:<nodeport>` with no mTLS and no bespoke RPC, its Vault
...
### Remaining Work
Remove the tracked expected outputs and old replay mutant as part of Sprint 68.3. The pure peer/auth and
...
- `documents/engineering/host_cluster_comms_doctrine.md` — its §9 planning-ownership pointer resolves to
  delivered Phase-68 sprints, and the §2/§5/§6 honesty notes flip from "resolved design decision / sibling
...
- `documents/engineering/substrate_doctrine.md` — its §9 planning-ownership pointer resolves to delivered
  Phase-68 sprints; the §4.3 "no macOS build VM" note and the §5 host-worker description gain their first
...
- `documents/engineering/resource_capacity_doctrine.md` — record the Phase-68 live check for private
...
- [README.md](README.md) — flip the Phase-68 row status once the gate passes and link this document.
- [substrates.md](substrates.md) — record Phase 68's gate substrate (apple) in the per-phase substrate map, and
...
- [phase_53](phase_53_determinism_jitcache.md) — the jit-build engine resolver + `CacheBudget` cache the Metal
  kernel is materialized into
- [phase_65](phase_65_jitml_lift_cuda.md) — the CUDA jitML lift; its Windows-CUDA case is the structurally
