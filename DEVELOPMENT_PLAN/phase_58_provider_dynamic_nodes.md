# Phase 58: Dynamic node provisioning by signal + leak-free provider gate
...
AWS audit, and provider leak-freedom remain UNVERIFIED because Phase 55 could not authenticate to AWS.
...
- [Sprints](#sprints)
- [Sprint 58.1: Dynamic node provisioning by signal ⏸️](#sprint-581-dynamic-node-provisioning-by-signal-)
- [Sprint 58.2: Phase gate — spin a provider cluster, provision a node by signal, tear down leak-free ⏸️](#sprint-582-phase-gate--spin-a-provider-cluster-provision-a-node-by-signal-tear-down-leak-free-)
...
⏸️ Blocked pending Phase-57 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
by a tag-only sweep. It cleaned every Phase-58 object. This is a scoped Kubernetes/cloud-metadata analogue,
...
linux-cpu (the [Phase 35](phase_35_bootstrap_coordinator_kind.md) bootstrap coordinator), from inside which the
...
This sub-phase **layers on** its three provider siblings — [Phase 55](phase_55_provider_deploy_checkpoint.md)
(the provider-cluster Pulumi deploy + Vault-Transit-enveloped MinIO checkpoint + `observeProviderAccount`),
[Phase 56](phase_56_provider_child_bringup.md) (the stateless hostless in-cluster control-plane daemon + capacity-scheduler roles + parent→child Lease handoff), and [Phase 57](phase_57_provider_ebs_credential.md) (per-PV EBS decoupled from the node lifecycle + the create-vs-delete credential + static EBS CSI) — and owns dynamic node provisioning
...
set, and drives the live set toward it through the [Phase 37](phase_37_object_reconciler.md)
...
grown bound ([Phase 14](phase_14_capacity_core_folds.md) placement, [Phase 15](phase_15_storage_geometry_folds.md) storage geometry, [Phase 16](phase_16_execution_accelerator_folds.md) execution/accelerator residency) and proves
...
MinIO backend — runs through the [Phase 18](phase_18_provision_seal.md) post-bind observe-then-plan boundary and
seals nothing until receipt-bound provider readback. Join is quarantined behind the `ManagedCapacity` taint,
full supply/layout/device observation, and a [Phase 38](phase_38_capacity_scheduler.md) scheduler-generation CAS;
an `Unreachable` node observation **refuses** rather than stranding an EC2 instance.

Second, the **phase gate** (Sprint 58.2): a single `.dhall` that, from a linux-cpu parent, spins up the EKS
provider cluster ([Phase 55](phase_55_provider_deploy_checkpoint.md)), converges its stateless in-cluster
control plane ([Phase 56](phase_56_provider_child_bringup.md)), dynamically provisions an extra node **by evaluating a declared signal rule** (Sprint 58.1), binds the Pulumi-created durable EBS through the static
`ebs.csi.aws.com` CSI PV and writes a run-unique marker ([Phase 57](phase_57_provider_ebs_credential.md)),
...
is [Phase 62](phase_62_test_topology_dsl.md) work, deferred and never depended on here.
...
flowchart LR
%% register: algebra
  signal["declared ScalingPolicy signal: load / workflow-completion"]:::intent --> fold{{"re-run Phase 14/15/16 fold against grown bound + provider-quota + account residual"}}:::gate
...
reclamation *explicitly deferred to [Phase 62](phase_62_test_topology_dsl.md)*.

**Gate:** `cabal test provider-dynamic-nodes-live` is green: `test/fixture/dhall/phase_48_provider_provision.dhall`
...
> **Provider corpus split (by design).** `test/fixture/dhall/phase_48_provider_provision.dhall` and the
> `mut-37.*` mutant family are this sub-phase's own Phase-0 corpus. The four provider sub-phases
> (Phases 49–52; see [Phase 55](phase_55_provider_deploy_checkpoint.md)) share one provider topology
...
`mut-44.1-drop-parallel-executor` mutants) stays in [Phase 55](phase_55_provider_deploy_checkpoint.md); the bootstrap
scheduler / add-on cutover / Lease-handoff apparatus (the `public-pull` and hostless-`LinuxHost`-foreclosure
tags) stays in [Phase 56](phase_56_provider_child_bringup.md); the per-PV-EBS credential apparatus
...
mutants) stays in [Phase 57](phase_57_provider_ebs_credential.md). This phase inherits only the reactive-node
and teardown slice below; the **composed** gate (Sprint 58.2) additionally re-runs the named sibling mutants and
cites them to their owning sub-phase.

```mermaid
flowchart LR
  %% register: orientation
  s0["Sprint 58.1: Dynamic node provisioning by signal"]
  s1["Sprint 58.2: Phase gate — spin a provider cluster, provision a node by…"]
...
- **Representative set (§M.7)** — `test/fixture/dhall/phase_48_provider_provision.dhall`: one `Managed Eks` control
...
- **Committed one-field negatives (§M.8), each paired with a positive differing only in the foreclosed dimension**: `test/fixture/dhall/phase_48_provider_over_quota.dhall` (elastic maximum count exceeds the pinned provider
  instance/vCPU quota while every node-class field remains valid) and
  `test/fixture/dhall/phase_48_provider_missing_capability.dhall` (the elastic class's explicit `accelerator = None`
...
([Phase 55](phase_55_provider_deploy_checkpoint.md)), and `credit-old-before-observed-delete` / `allow-delete`
([Phase 57](phase_57_provider_ebs_credential.md)) mutants against the full-stack fixture, cited to their owning
...
- [`cluster_lifecycle_doctrine.md §8`](../documents/engineering/cluster_lifecycle_doctrine.md#8-dynamic-node-provisioning)
  ...
  provisioning a node is one more pass of the [Phase 37](phase_37_object_reconciler.md)
...
- [`resource_capacity_doctrine.md §6`](../documents/engineering/resource_capacity_doctrine.md#6-growable--scalingpolicy-the-quota-bounded-dynamic-provisioning-arm)
  ...
  `emptyDir` as a **[Phase 14](phase_14_capacity_core_folds.md) logical local-ephemeral debit** — see below),
...
- [`pulumi_iac_doctrine.md §4`](../documents/engineering/pulumi_iac_doctrine.md#4-what-pulumi-provisions-the-resource-catalog)
  ...
  EBS ([Phase 57](phase_57_provider_ebs_credential.md)) is a distinct retained class the per-run teardown never
  touches.
...
- [`daemon_topology_doctrine.md §3.1`](../documents/engineering/daemon_topology_doctrine.md#31-exactly-one-pod-is-a-k8setcd-property-not-an-amoebius-election)
  ...
  ([Phase 38](phase_38_capacity_scheduler.md)), not control-plane daemon election.
- [`storage_lifecycle_doctrine.md §5.1`](../documents/engineering/storage_lifecycle_doctrine.md#51-storage-is-independent-of-the-node-lifecycle)
  ...
  test-flagged EBS by the elevated harness is [Phase 62](phase_62_test_topology_dsl.md) work, referenced and
  never invoked here.
...
## Sprint 58.1: Dynamic node provisioning by signal ⏸️
...
- An `Amoebius.Cluster.NodeProvisioner` that reads the declared elastic-node rule — a typed `ScalingPolicy`
  ...
  driven by **load** and **workflow completion** (spot-instance-cost-driven scaling is a declared future signal
  class, deferred and not exercised by this phase's gate corpus or seeded mutants) — computes the desired node
  set, and drives the live set toward it through the [Phase 37](phase_37_object_reconciler.md) reconciler; no
  ...
  [Phase 55](phase_55_provider_deploy_checkpoint.md) `observeProviderAccount` boundary re-read). SKU raw
...
- An `Amoebius.Pulumi.NodeGroup` enaction that adds an EC2/managed node (Pulumi, under the control-plane daemon, encrypted
  ...
  node; it is distinct from [Phase 57](phase_57_provider_ebs_credential.md)'s retained durable class. Every
  add/drain runs through the [Phase 18](phase_18_provision_seal.md) observe-then-plan boundary and seals nothing
...
- A staged managed-node join/leave protocol. Provider launch data makes the kubelet register with the exact
  ...
  the active [Phase 38](phase_38_capacity_scheduler.md) scheduler generation. Only an independent readback of
...
4. Run the committed one-field negative `test/fixture/dhall/phase_48_provider_over_quota.dhall`, whose elastic maximum
...
5. Give two child-cluster demands the same `CloudAccountId` and per-cluster quotas that each fit alone but exceed
   ...
   `DescribeInstances`/`DescribeVolumes` usage is one unit short; the [Phase 55](phase_55_provider_deploy_checkpoint.md)
...
6. Fit CPU, memory, storage bytes, and regional EBS count, then make the aggregate simultaneous pod count one
   ...
   MUST go red.
7. Run the committed `test/fixture/dhall/phase_48_provider_missing_capability.dhall`: replace the elastic class's
...
12. Reject before any cloud mutation every worst-case elastic shape the envelope cannot carry, so the refusal
    ...
    cache-owner `emptyDir`'s [Phase 14](phase_14_capacity_core_folds.md) local-ephemeral debit), layout-routed
...
## Sprint 58.2: Phase gate — spin a provider cluster, provision a node by signal, tear down leak-free ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `test/fixture/dhall/phase_48_provider_provision.dhall` (the gate topology),
...
[Phase 62](phase_62_test_topology_dsl.md)), `DEVELOPMENT_PLAN/README.md`.
...
- The gate `test/fixture/dhall/phase_48_provider_provision.dhall`: spin up the EKS provider cluster
  ([Phase 55](phase_55_provider_deploy_checkpoint.md)), first derive the complete app/platform demand and
  provision it against the named base/elastic node classes, the bounded cache demand charged within local
  ephemeral supply (the cache-owner `emptyDir` charged **once** inside the [Phase 14](phase_14_capacity_core_folds.md) logical local-ephemeral fold — **not** a second supply pool and **not** the [Phase 59](phase_59_determinism_jitcache.md) typed `CacheBudget`/jit-build construct, which is out of this phase's scope), parent executor Jobs and
  ...
  in-cluster control plane ([Phase 56](phase_56_provider_child_bringup.md)), provision an extra node by a declared
  rule and observe it join only after node-scoped scheduler authority (Sprint 58.1), bind the Pulumi-created EBS
  through [Phase 57](phase_57_provider_ebs_credential.md)'s static CSI PV and write a run-unique marker through
...
- An `Amoebius.Pulumi.Teardown` step: one `reconcileAbsent` over the **ephemeral** registry subset (VPC, EKS
  ...
  Unreachable → refuse* — leaving the durable EBS class ([Phase 57](phase_57_provider_ebs_credential.md))
...
- A per-run proven/tested/assumed ledger recording: provider bring-up + signal-driven node join as **tested on the EKS provider target from a linux-cpu parent**; per-run teardown leak-freedom **under the broadened run-owned sweep** as **tested**; durable EBS retention as **correct-by-class**; and the elevated-harness
  durable-EBS *reclamation* as **explicitly deferred to [Phase 62](phase_62_test_topology_dsl.md), not asserted here**.

### Validation

1. Run the gate end-to-end over the committed representative set
   `test/fixture/dhall/phase_48_provider_provision.dhall` (named in the Gate above): assert the provider cluster
   ...
   parent bootstrap Lease holder releases before the authenticated child control-plane daemon acquires ([Phase 56](phase_56_provider_child_bringup.md)).
   ...
   - The bounded cache-owner `emptyDir` is charged **once** inside that [Phase 14](phase_14_capacity_core_folds.md) local-ephemeral demand, not as a second supply pool.
...
2. From the provider-stack-absent state left by Validation 1, with the durable EBS retained, run a second full
   spin-up → run → teardown cycle and repeat [Phase 57's retained-volume identity/readback oracle](phase_57_provider_ebs_credential.md)
...
3. Assert the run emits a proven/tested/assumed ledger per
   ...
   elevated-harness durable-EBS reclamation stays explicitly deferred to [Phase 62](phase_62_test_topology_dsl.md)
   and is recorded as deferred, not asserted here.
4. Run the committed over-quota (`test/fixture/dhall/phase_48_provider_over_quota.dhall`) and missing-capability
   (`test/fixture/dhall/phase_48_provider_missing_capability.dhall`) negatives and assert both fail before the first
...
5. Run the one-short executor/plugin/workspace/checkpoint-budget, pod-slot, CSI-attach, root-EBS, durable-EBS, and
   ...
   PVC/PV, and static CSI attachment. The `drop-parallel-executor` ([Phase 55](phase_55_provider_deploy_checkpoint.md)),
   `credit-old-before-observed-delete` ([Phase 57](phase_57_provider_ebs_credential.md)), and this phase's
...
> test-flagged EBS under the elevated credential — is [Phase 62](phase_62_test_topology_dsl.md) and is **not** a
...
- `documents/engineering/resource_capacity_doctrine.md` — record that
  ...
  note that the in-cluster cache-owner `emptyDir` is a Phase-14 logical local-ephemeral debit here, distinct from
  the Phase-59 typed `CacheBudget`.
...
- `documents/engineering/testing_doctrine.md` — record the Phase 58 per-run ledger artifact, the **broadened run-owned OS-boundary leak-free sweep** (run-tag + VPC id + `eks:cluster-name` / `kubernetes.io/cluster/<name>`,
  catching untagged provider-spawned orphans), and the explicit deferral of elevated durable-EBS reclamation to
  [Phase 62](phase_62_test_topology_dsl.md).

**Cross-references to add:**
- `DEVELOPMENT_PLAN/system_components.md` — register `Amoebius.Cluster.NodeProvisioner`,
  `Amoebius.Pulumi.NodeGroup`, and `Amoebius.Pulumi.Teardown` as Phase-58 design-first rows, each mapped to its
  ...
  first delivery in [Phase 55](phase_55_provider_deploy_checkpoint.md) / [Phase 56](phase_56_provider_child_bringup.md)
  / [Phase 57](phase_57_provider_ebs_credential.md).
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 58 → `linux-cpu` (parent) row with the `provider` (EKS)
  deploy target annotated as a target class, not a fifth hardware substrate.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase 58 row's status once the gate passes; link this document.

## Related Documents

- [README.md](README.md) — the live tracker; the Phase 58 one-line gate and status
...
- [phase_55_provider_deploy_checkpoint.md](phase_55_provider_deploy_checkpoint.md) — the provider-cluster Pulumi
  deploy, Vault-Transit-enveloped MinIO checkpoint, and `observeProviderAccount` this phase reuses to add/drain a
  node group
- [phase_56_provider_child_bringup.md](phase_56_provider_child_bringup.md) — the stateless hostless in-cluster
  control-plane daemon + capacity-scheduler roles + parent→child Lease handoff the gate composes before node provisioning
- [phase_44_live_dsl_deploy.md](phase_44_live_dsl_deploy.md) — owns the control-plane daemon runtime role reused by
  the provider child
- [phase_57_provider_ebs_credential.md](phase_57_provider_ebs_credential.md) — per-PV durable EBS decoupled from
  the node lifecycle, the static `ebs.csi.aws.com` CSI PV, and the create-vs-delete credential the gate's marker
  rebind rides on
- [phase_14_capacity_core_folds.md](phase_14_capacity_core_folds.md) — the `fits`/`carve`/`place` fold re-run
  against the grown bound (and the logical local-ephemeral debit the cache-owner `emptyDir` charges)
- [phase_18_provision_seal.md](phase_18_provision_seal.md) — the post-bind observe-then-plan cloud batch and
  opaque `ProvisionedSpec` seal each node-group enaction runs through
- [phase_37_object_reconciler.md](phase_37_object_reconciler.md) — the `discover → diff → enact → re-observe`
  reconciler that makes provisioning a node just another reconcile
- [phase_38_capacity_scheduler.md](phase_38_capacity_scheduler.md) — the `amoebius-capacity`
  scheduler-generation CAS / reservation / exclusive Binding at node join
- [phase_62_test_topology_dsl.md](phase_62_test_topology_dsl.md) — the elevated-harness durable-EBS reclamation
