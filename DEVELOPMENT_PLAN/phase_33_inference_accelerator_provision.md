# Phase 33: InferenceEngine capability + accelerator provision
...
- [Sprints](#sprints)
- [Sprint 33.1: The `InferenceEngine` capability — target-offering-selected runtime + accelerator provision ⏸️](#sprint-331-the-inferenceengine-capability--target-offering-selected-runtime--accelerator-provision-)
- [Sprint 33.2: The accelerator-provision corpus + the Register-1 gate ⏸️](#sprint-332-the-accelerator-provision-corpus--the-register-1-gate-)
...
executable as the strictest instance of the capability→provider→shape binding. Its target **fills the ninth capability arm** whose reserved head [Phase 31](phase_31_capability_bind.md) defines: the `InferenceEngine` capability and
...
handing the owner demands to the [Phase 30](phase_30_execution_accelerator_folds.md) accelerator-residency fold,
...
- the reserved `InferenceEngine` head and the eight-arm closed union around it, the representational `bind`,
  and the object-node-multiset shape oracle ([Phase 31](phase_31_capability_bind.md))
- the `provision` constructor, execution-epoch/runtime-storage/
  ...
  `ProvisionedSpec` seal it plugs into ([Phase 32](phase_32_provision_seal.md))
- the primitive `fits`/`carve`/`place` accelerator-device / net-allocatable-VRAM / identity-complete
  residency-coexistence epoch fold ([Phase 30](phase_30_execution_accelerator_folds.md))
...
- and the **live** jit-build resolve of the named `EngineRuntime` identity into its `CacheBudget`-bounded
  ...
  ([Phase 81](phase_81_determinism_jitcache.md)).
...
**Depends on:** [Phase 32](phase_32_provision_seal.md)
...
| `Command` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: `pb validate phase 33` is future public spelling only. Before current human approval of Phase 51, `pb` is inadmissible validation transport; the candidate must invoke the exact absolute source-bound Haskell executable directly from an authenticated, network-independent toolchain input. The Haskell verdict entry point remains `UNRESOLVED` and blocks validation. |
...
- [`resource_capacity_doctrine.md` §3 — The types: `Quantity`, `Capacity`, `Demand`, `Budget`](../documents/engineering/resource_capacity_doctrine.md#3-the-types-quantity-capacity-demand-budget)
  ...
  matching offering, then run the Phase-30 accelerator-residency/coexistence fold, then hand only the checked
...
## Sprint 33.1: The `InferenceEngine` capability — target-offering-selected runtime + accelerator provision ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: exact authored Haskell implementation paths have not been bound to this sprint.
**Blocked by**: [Phase 32](phase_32_provision_seal.md) human approval
...
- The **partial** family×lane availability relation making a served model whose family is unavailable on the
  ...
  [Phase-30](phase_30_execution_accelerator_folds.md) fold derives all policy-permitted epochs and sums every
...
- The accelerator-provision pairing at the provision seal: after bind expands the `InferenceEngine` provider's
  ...
  `ProvisionedCudaOwnerDemand`/`ProvisionedMetalOwnerDemand` epoch witnesses via the Phase-30 fold, and fits each
...
- An in-file honesty note: this is the representational union + relation and the pure accelerator-provision fold
  ...
  runtime-checked cross-lane weight-load residue, are the live band ([Phase 81](phase_81_determinism_jitcache.md))
...
## Sprint 33.2: The accelerator-provision corpus + the Register-1 gate ⏸️
...
- The concrete corpus named in [Gate integrity](#gate-integrity): the three positive fixtures
  (`legal_inference_{singlenode,distributed}`, `legal_inference_cuda`) with the authored Phase-31 semantic projection,
...
- A Register-1 validation-locus ledger mapping every entry to its catalog id ([§3.25](../documents/illegal_state/illegal_state_ml_asset.md#325-an-ml-asset-named-by-arbitrary-url-or-an-unready--unlanded-model) for the engine-by-URL state)
  ...
  backed by Phase-28-style coverage-assertion machinery (the ledger goes **red** if any corpus entry, negative
...
1. Rejected historical observation: the `capability-spec` Cabal suite was recorded green over the
   ...
   mutants. The validation-locus ledger is present and its coverage-assertion machinery (Phase-28 precedent)
...
- `documents/engineering/resource_capacity_doctrine.md` — record that the accelerator arm of the post-fold
  boundary (§3/§4) is exercised through the `InferenceEngine` binding and the Phase-30 residency/coexistence fold
...
- `DEVELOPMENT_PLAN/system_components.md` — register the Haskell `InferenceEngine` capability projection,
  ...
  Phase-33 design-first rows. The Dhall projection is lazy output beneath `.build/dhall/**`.
...
- [phase_30](phase_30_execution_accelerator_folds.md) — the identity-complete accelerator-device / net-allocatable-VRAM
  / residency-coexistence epoch fold these owner demands feed
- [phase_31](phase_31_capability_bind.md) — the closed nine-arm capability union (whose reserved `InferenceEngine` head this phase fills), the representational `bind`, and the object-node-multiset shape oracle
- [phase_32](phase_32_provision_seal.md) — the whole-deployment provision seal that constructs the accelerator
