# Phase 66: Live DSL deploy via the replicas=1 control-plane daemon
...
- [Sprints](#sprints)
- [Sprint 66.1: The control-plane daemon — a Deployment replicas=1, single-instance from k8s/etcd ⏸️](#sprint-661-the-control-plane-daemon--a-deployment-replicas1-single-instance-from-k8setcd-)
- [Sprint 66.2: Live reconcile of the platform + a trivial app from one Haskell declaration ⏸️](#sprint-662-live-reconcile-of-the-platform--a-trivial-app-from-one-haskell-declaration-)
- [Sprint 66.3: Phase gate harness — live deploy + the pre-cluster negative corpus as a live regression guard ⏸️](#sprint-663-phase-gate-harness--live-deploy--the-pre-cluster-negative-corpus-as-a-live-regression-guard-)
- [Sprint 66.4: The admin REST surface — `vault init/unseal`, `dhall update`, secret KV-CRUD ⏸️](#sprint-664-the-admin-rest-surface--vault-initunseal-dhall-update-secret-kv-crud-)
...
and the Phase-50 hardware-free barrier have external human approval. It may consume the Phase-26/27 typed
projection and decoder, Phase-28 illegal-state corpus, Phase-9 capacity/topology folds, Phase-31/32 binding and
provision seal, Phase-34 `renderAll`, and Phase-35 chain/dry-run semantics only through those exact approvals;
...
linux-cpu cluster toward it, apply the approved standard platform plus a trivial app through the Phase-59
...
re-run the approved Phase-28 Haskell negative corpus through this live deploy path and require every case to
...
in-namespace Sql) is deliberately deferred to Phase 67; the app here is trivial.

The target initial ownership transition is explicit. After future Phase-59/60 approval, the bootstrap-host
...
**Substrate:** linux-cpu — the single-node `kind` cluster and services targeted by Phases 56–65; no apple, linux-cuda, or windows
...
**Depends on:** [Phase 65](phase_65_keycloak_ingress.md)
...
| Key | Contract |
|---|---|
| `Claim` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: one cohesive target claim — *the independently approved pre-cluster language deploys to a real cluster, written by exactly one holder of the Lease*. Single-writer exclusion is delegated, not re-implemented here. Explicit exclusions: every layer named in `Residue` remains UNVERIFIED. |
...
- [`preflight_validation_doctrine.md` §2 — The `Check` algebra](../documents/engineering/preflight_validation_doctrine.md#2-the-check-algebra)
  ...
  by [phase_77](phase_77_provider_deploy_checkpoint.md) / [phase_80](phase_80_provider_dynamic_nodes.md).
- [`daemon_topology_doctrine.md` §3 — The control-plane daemon](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-daemon)
  ...
  bespoke election. This phase also performs the one-way authority handoff from the observed Phase-59
...
## Sprint 66.1: The control-plane daemon — a Deployment replicas=1, single-instance from k8s/etcd ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 65](phase_65_keycloak_ingress.md) human approval
...
- **The collapse of `app/singleton/` into the one executable.** The control-plane daemon branch reached its pod by being
  ...
  Closing this also retires the transitional half of Phase 44's search-path gate check, which can only fail by
  the tree regaining a second executable.
- A control-plane daemon deployed as a **generated typed `Deployment replicas=1`** by the Phase-59
...
- The `discover → diff → enact → re-observe` reconcile loop that decodes the `InForceSpec` in-process
  (Phase-27 decoder), binds capabilities (Phase-31 binder), and applies the resulting manifests through the
  Phase-59 typed reconciler — idempotently, driven only by observed cluster state.
...
- A closed initial `AuthorityHandoff`: while holding the exact Lease the Phase-59 host applies the control-plane daemon
...
- Secret authority fused to the role (operates root Vault as the single in-cluster writer) and the admin-REST
  ...
  four-endpoint surface by [Sprint 66.4](#sprint-664-the-admin-rest-surface--vault-initunseal-dhall-update-secret-kv-crud-).
...
## Sprint 66.2: Live reconcile of the platform + a trivial app from one Haskell declaration ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 66.1
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
- A positive deploy value authored in Haskell and lazily rendered as Dhall beneath `.build/test-corpora/**`,
  composing the standard platform-service stack (Phases 42–43) and a **trivial**
  single-service app — deliberately narrower than the Phase-67 tenancy projection (no per-app namespace,
...
- A leak-free teardown obligation carried by the Haskell deploy fixture; its generated test-topology Dhall
  ...
  reclaimed, while the pre-existing Phase-63/43 service set and Phase-65 edge are restored to Ready rather than
  swept.
...
1. Before the first pass the harness deletes the components named by a separately reviewed Haskell
   ...
   example Prometheus's — so a pre-converged Phase-63/43 stack cannot ride the gate (§M.6). The first pass
   ...
   and equal to the first-pass Haskell expectation. The app is reachable through the Phase-65
...
2. Teardown leaves no leaked resources. The postflight sweep is scoped to this run's provisioned objects,
   ...
   it creates — that label set is authored here, and Phase-49 flag-at-creation machinery is not assumed — and
   the sweep is empty over it. Separately, every platform component the harness perturbed is asserted back at
   Ready so the shared Phase-63/43 stack is left as found, and the apiserver audit log records that **every**
...
## Sprint 66.3: Phase gate harness — live deploy + the pre-cluster negative corpus as a live regression guard ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 66.2
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
pre-cluster (Phase-28) Haskell negative corpus so each lazily generated illegal Dhall projection still fails to type-check or decode
...
- The negative regression guard: the Phase-28 corpus (a bad PVC↔PV pairing, a Keycloak-bypassing open ingress, a
...
- A **Register-3** proven/tested/assumed ledger recording the live-enforcement result (the apiserver admitted
  the rendered manifests) and marking the deferred surfaces — full app tenancy (Phase 67), and the
...
2. "The live deploy path" is pinned to the identical entry point the positive fixture used, foreclosing the
   host-side re-run cheat (§M.3): every Phase-28 negative fixture is submitted through the exact same control-plane daemon
...
## Sprint 66.4: The admin REST surface — `vault init/unseal`, `dhall update`, secret KV-CRUD ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 66.3
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
- **`dhall update`** — deliver a new `InForceSpec` to a running cluster (requires an unsealed Vault and a root
  token). The control-plane daemon decrypts/stores the envelope in-process and reconciles toward it via Sprint 66.2's loop.
...
- A **Register-3** proven/tested/assumed ledger recording the admission gate as **runtime-checked, live**, and
  ...
  ([phase_67](phase_67_app_tenancy.md)) and the parent→child `ParentReachChannel` use of this surface
  ([phase_78](phase_78_provider_child_bringup.md)), neither of which this phase exercises.
...
1. **The post-handoff operator sequence, end to end.** After Sprint 66.1's observed handoff, drive `vault
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-66 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — confirm the Phase-66 linux-cpu gate row (the replicas=1 control-plane daemon, no
  election).
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/ControlPlane/{Daemon,Reconcile,Deploy,AdminApi}.hs`
  as Phase-66 design-first rows, and re-anchor the in-cluster-control-plane row to the current
  `#3-the-control-plane-daemon` (no election).
- `DEVELOPMENT_PLAN/phase_56_bootstrap_coordinator_kind.md` — its exec handoff now names Sprint 66.4 as owner
  of the Haskell admin-REST client.
- `DEVELOPMENT_PLAN/phase_78_provider_child_bringup.md` — its post-handoff child admin-REST bring-up can now
  name Sprint 66.4 in `Blocked by`.
...
- [Bootstrap Sequence Doctrine](../documents/engineering/bootstrap_sequence_doctrine.md) — the [§5](../documents/engineering/bootstrap_sequence_doctrine.md#5-the-admin-control-plane-the-cli--the-control-plane-daemon-rest-api) admin control
  plane (CLI ↔ control-plane daemon REST) delivered by Sprint 66.4, and the [§4](../documents/engineering/bootstrap_sequence_doctrine.md#4-the-host-daemon--control-plane-daemon-handoff) handoff point at which it is exposed
...
- [phase_56](phase_56_bootstrap_coordinator_kind.md) — the bootstrap-to-Haskell exec handoff that precedes
  the command-mode admin client Sprint 66.4 owns
- [phase_62](phase_62_vault_pki.md) — the root Vault, unlock-material envelope, and built-in Vault client the
  admin endpoints call
- [phase_78](phase_78_provider_child_bringup.md) — the parent→child bring-up that drives a child through this
  same surface over its `ParentReachChannel`
- [phase_59](phase_59_object_reconciler.md) — the typed renderer + SSA reconciler that renders and applies
  the control-plane daemon and its manifests
- [phase_63](phase_63_platform_backbone.md) — the standard platform-service stack the live Haskell-declared spec deploys
- [phase_65](phase_65_keycloak_ingress.md) — the Keycloak-owned edge the trivial app routes through
- [phase_67](phase_67_app_tenancy.md) — the app-tenancy projection (namespace + ObjectStore + Sql) deferred
  from this phase
