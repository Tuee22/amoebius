# Phase 11: Dhall dhall-typecheck schema + smart-constructor prelude
...
Phase 11 delivers the Dhall dhall-typecheck schema + smart-constructor prelude; its design is owned by [dsl_doctrine.md](../documents/engineering/dsl_doctrine.md), [resource_capacity_doctrine.md](../documents/engineering/resource_capacity_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 11.1: Dhall prelude + typed surfaces + smart constructors ✅](#sprint-111-dhall-prelude--typed-surfaces--smart-constructors-)
- [Sprint 11.2: dhall-typecheck positive corpus ✅](#sprint-112-dhall-typecheck-positive-corpus-)
- [Sprint 11.3: dhall-typecheck-class negative corpus + partial-foreclosure ledger ✅](#sprint-113-dhall-typecheck-class-negative-corpus--partial-foreclosure-ledger-)
- [Sprint 11.4: The shared `SecretRef` union and the plaintext-secret negative ✅](#sprint-114-the-shared-secretref-union-and-the-plaintext-secret-negative-)
...
✅ Done — resealed 2026-08-17 on the amended contract. `python3 tools/dhall_typecheck_schema_gate.py` passes all
ten sides on substrate `none`, lane `none`, natural `arm64`, untranslated: the whole dhall-typecheck battery is green,
...
`sha256:1cd60cf72d7ad324…`; Phase 11 owns no remaining migration deferral.
...
**Why the type lands here and not in Phase 39.** Phase 40 builds Vault; dhall-typecheck is *this* phase's boundary.
"A production config cannot express a secret value" is a statement about the typechecker, and putting it in
the phase that happens to need it first would leave Phases 11 and 12 claiming a complete dhall-typecheck/gadt-decode
...
**Remaining work.** None. Sprint 11.4 discharged the amendment and the gate is green on all eight sides.
...
and record punning. Separately, the `schema-modules` oracle read 14 while the tree holds 17, because Phase 25
added `SanctionedApi`, Phase 36 added `BakeCatalog`, and Phase 34 added `UiOffline`. A bare count is a weak
...
`ledgers/phase_05_dhall-typecheck.md`; it is migration input, not current evidence.
...
real teeth at the Haskell GADT decoder in Phase 12. This is a **Register 1** (pure/golden, in-process, no
...
**Gate:** `dhall type` over the dhall-typecheck corpus is green — every positive fixture type-checks and every
...
flowchart LR
  ...
  s0["Sprint 11.1: Dhall prelude + typed surfaces + smart constructors"]
  s1["Sprint 11.2: dhall-typecheck positive corpus"]
  s2["Sprint 11.3: dhall-typecheck-class negative corpus + partial-foreclosure ledger"]
...
```
*Orientation. The seams phase 11 builds, in order; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns the apparatus. Not run.*

## Gate integrity

The apparatus phase 11's gate closes over, in the slot
...
[Phase 12](phase_12_gadt_decode_ir.md)'s gadt-decode; its separate produce-side no-constructor subcase is left
to Phase 13's exhaustive registry-driven corpus. Neither [§3.23](../documents/illegal_state/illegal_state_capability_messaging.md#323-a-non-cbor-pulsar-payload) subcase is counted toward this representative
...
[Phase 12](phase_12_gadt_decode_ir.md).
...
belongs to [Phase 12](phase_12_gadt_decode_ir.md); whether the named secret exists is a live question
neither gate can answer, and is owned by
[Phase 40](phase_40_vault_pki.md).
...
- [`dsl_doctrine.md §2 — Dhall carries params, Haskell carries logic`](../documents/engineering/dsl_doctrine.md#2-two-languages-one-system-dhall-carries-params-haskell-carries-logic):
  ...
  chain/Step logic that acts on them (that decode-and-act half is Phase 12 and later). The Dhall never "runs";
  it is authored, type-checked, and (from Phase 12 on) decoded.
- [`dsl_doctrine.md §5 — the illegal-state-unrepresentable contract`](../documents/engineering/dsl_doctrine.md#5-the-illegal-state-unrepresentable-contract):
  ...
  omit it. gadt-decode (the in-process typed decoder) is deferred to [Phase 12](phase_12_gadt_decode_ir.md);
  whole-deployment feasibility and the opaque deployable seal are Phase 18.
- [`illegal_state_catalog.md §1 — Illegal states fail to type-check`](../documents/illegal_state/illegal_state_catalog.md#1-illegal-states-fail-to-type-check),
  ...
  here; decoder-local checked rejections defer to Phase 12, whole-deployment resource/target checks defer to
  Phase 18's `provision-seal`, and runtime-checked entries defer to the live band. The catalog's [§2](../documents/illegal_state/illegal_state_catalog.md#2-the-load-bearing-limit-a-type-check-proves-the-spec-composes-not-that-the-cluster-enforces-it) limit is
...
- [`resource_capacity_doctrine.md §3`](../documents/engineering/resource_capacity_doctrine.md#3-the-types-quantity-capacity-demand-budget):
  ...
  `ResourceEnvelope`/`Capacity` declaration; Phases 12, 14, and 15 own normalization, arithmetic feasibility, and
  ...
  verification layer flips at Phase 14 (capacity arithmetic), not at dhall-typecheck; its absence from the doc-update
...
## Sprint 11.1: Dhall prelude + typed surfaces + smart constructors ✅
...
no Haskell skeleton is needed (that arrives with the gadt-decode decoder in Phase 12).
...
- The last three schema modules close doctrine surfaces that no phase previously owned, so each is delivered
  ...
  - `Consistency.dhall` carries the PACELC surface
    ...
    [Phase 54](phase_54_gateway_migration_drills.md) consumes.
  - `Backup.dhall` carries the closed `BackupPolicy`
    ...
    invariant #23. Phases 0–74 own the *declarable* policy; its live enactment — the put-only credential and
...
- A Dhall prelude and record/union types exposing only *smart constructors* — a vocabulary with no illegal
  ...
  the arm-inventory oracle pins all nine so Phase 17 binds and Phase 19 provisions the same union;
...
- The pure resource declarations of
  ...
  - The `requests + pad ≤ limits` bound and the all-`Zero` pad rejection are cross-field and arithmetic, so
    dhall-typecheck proves only presence and closed shape and [Phase 12](phase_12_gadt_decode_ir.md) refines both;
  ...
  - Phase 12 refines/normalizes the quantities and preserves the identity graph in opaque, unit-tagged
    values;
  - Phase 14 checks global backing/carve uniqueness, exactly-once reference resolution, `requests ≤ limits`,
...
- An in-file **honesty caveat**: because Dhall has no opaque types, binding- and phantom-index foreclosures
  ...
  teeth at the Haskell GADT decoder in [Phase 12](phase_12_gadt_decode_ir.md) (gadt-decode).
...
one-parent ownership, and arithmetic are the Phase-14 fold rather than a dishonest Dhall type claim. The
...
plus allocatable VRAM within raw VRAM. Their value arithmetic is Phase 14, not a Dhall type claim. Every
...
0) pins these required field-name→type bindings; Sprint 11.1 validation compares the shipped record types
...
## Sprint 11.2: dhall-typecheck positive corpus ✅
...
type` against the Sprint-10.1 schema; the harness exit code is a single green/red over the whole positive
set.
...
- Positive fixtures — the explicit representative set `legal_multisubstrate_cluster`, `legal_managed_eks`,
  ...
  Sprint-10.1 smart constructors, and each populating every REQUIRED foreclosing field of its surface record
...
- Each of the eight dhall-typecheck negatives of Sprint 11.3 names one of these positives as its paired sibling (the
...
2. Each positive fixture's surface record instantiates every required foreclosing field named in
   ...
   against the committed oracles), so the positives exercise the Sprint-10.1 foreclosures rather than a toy
...
## Sprint 11.3: dhall-typecheck-class negative corpus + partial-foreclosure ledger ✅
...
`DEVELOPMENT_PLAN/phase_12_gadt_decode_ir.md` (backlink: the decode-foreclosed residue lands there).
...
- The malformed-received-body subcase of the non-CBOR payload entry ([§3.23](../documents/illegal_state/illegal_state_capability_messaging.md#323-a-non-cbor-pulsar-payload)) is explicitly NOT authored as a
  ...
  [Phase 12](phase_12_gadt_decode_ir.md)'s gadt-decode. The separate produce-side no-constructor subcase is
  outside this representative set and lands in Phase 13's exhaustive registry-driven corpus.
...
- The **partial-foreclosure ledger** is the [§K](development_plan_standards.md#k-honesty-proven--tested--assumed) proven/tested/assumed artifact this phase emits under `.build/runs/`,
  ...
  layer-2/3 residue UNVERIFIED, and routes that residue to [Phase 12](phase_12_gadt_decode_ir.md). This
...
4. The run-local partial-foreclosure ledger maps all eight negatives to a catalog entry and foreclosure layer
   — fully no-arm/required-field, versus the residue owned by Phase 12's gadt-decode — records the
...
## Sprint 11.4: The shared `SecretRef` union and the plaintext-secret negative ✅

**Status**: Done — the union is dhall-typecheck surface, its arms are pinned, and the literal is uninhabitable
...
[Phase 12](phase_12_gadt_decode_ir.md)'s and the live presence proof is
[Phase 40](phase_40_vault_pki.md)'s.

## Documentation Requirements

**Engineering docs to update (when the gate runs, flip the honest layer, never before):**
- `documents/engineering/dsl_doctrine.md` — backlink §5's dhall-typecheck to this in-process Phase-11 proof; keep gadt-decode
  (the typed decoder) as the companion boundary owned by Phase 12, and runtime enforcement as the deferred
  live-band residue.
...
- `DEVELOPMENT_PLAN/system_components.md` — register `dhall/amoebius/` and `dhall/examples/` as Phase-11
  design-first rows.
...
- [phase_12](phase_12_gadt_decode_ir.md) — gadt-decode, the GADT-indexed IR and total decoder, the companion boundary
