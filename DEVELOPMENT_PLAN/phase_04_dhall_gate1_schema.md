...
`ledgers/phase_04_gate1.md`; it is migration input, not current evidence.
...
flowchart LR
  ...
  s0["Sprint 4.1: Dhall prelude + typed surfaces + smart constructors"]
  s1["Sprint 4.2: Gate-1 positive corpus"]
  s2["Sprint 4.3: Gate-1-class negative corpus + partial-foreclosure ledger"]
  gate["the phase 4 gate"]
  s0 -->|"produces what the next consumes"| s1
  s1 -->|"produces what the next consumes"| s2
  s2 -->|"the last seam the gate closes over"| gate
```
*Orientation. The seams phase 4 builds, in order; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns the apparatus. Not run.*

## Gate integrity

The apparatus phase 4's gate closes over, in the slot
...
- The pure resource declarations of
  ...
  - The `requests + pad ≤ limits` bound and the all-`Zero` pad rejection are cross-field and arithmetic, so
    Gate 1 proves only presence and closed shape and [Phase 5](phase_05_gadt_decoder_gate2.md) refines both;
  ...
  - Phase 5 refines/normalizes the quantities and preserves the identity graph in opaque, unit-tagged
    values;
  - Phase 7 checks global backing/carve uniqueness, exactly-once reference resolution, `requests ≤ limits`,
...
one-parent ownership, and arithmetic are the Phase-7 fold rather than a dishonest Dhall type claim. The
...
plus allocatable VRAM within raw VRAM. Their value arithmetic is Phase 7, not a Dhall type claim. Every
