# Phase 39: Owner-scoped UI projection runtime
...
- [Sprints](#sprints)
- [Sprint 39.1: Build and independently verify the owner-scoped live projection ⏸️](#sprint-391-build-and-independently-verify-the-owner-scoped-live-projection-)
...
Done (invalidated). Phase 20 owns client/server plan compilation and Phase 23 owns local interpreter composition. This
...
`UiProjectionWorker` consumes canonical CBOR workflow events through the native Phase-36 Pulsar client and
...
`(AppId, TenantId, Owner, CommandId)`, the Phase-38 workflow work-id and `WorkflowHandle`, the normalized input
...
**Dependency:** Phase 22, Phase 37, and Phase 37. Phase 22 supplies sealed server dispatch, Phase 37 supplies
the live trusted request context and scoped authorization result, and Phase 38 supplies the live workflow/event
path. Phase 36 is transitive through Phases 37 and 37.
...
- **Fresh challenges:** after Keycloak, Pulsar, worker, query handler, and observers are ready, the harness
  creates unpredictable per-owner nonces and command ids and emits them through the Phase-38 workflow
...
- A projection cannot be served until caught up to the release-required watermark. Phase 41 pins that
...
## Sprint 39.1: Build and independently verify the owner-scoped live projection ⏸️
...
None for Sprint 39.1. Browser/reconnect, release, ML-lift, HA, and cross-cluster claims remain with their named
later phases.
...
- `DEVELOPMENT_PLAN/phase_41_ui_program_release.md`, `phase_51_infernix_ui_lift.md`, and
  `phase_53_jitml_ui_lift.md` — consume its caught-up watermark/read-model/receipt result without expanding its
  claim.
...
- [Phase 38 — content store and workflow runtime](phase_38_content_store_workflow.md)
