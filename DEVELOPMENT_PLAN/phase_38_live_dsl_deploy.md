# Phase 38: Live DSL deploy via the replicas=1 control-plane daemon
...
- [Sprints](#sprints)
- [Sprint 38.1: The control-plane daemon — a Deployment replicas=1, single-instance from k8s/etcd ⏸️](#sprint-381-the-control-plane-daemon--a-deployment-replicas1-single-instance-from-k8setcd-)
...
flowchart LR
  ...
  s0["Sprint 38.1: The control-plane daemon — a Deployment replicas=1…"]
...
## Sprint 38.1: The control-plane daemon — a Deployment replicas=1, single-instance from k8s/etcd ⏸️
...
- A control-plane daemon deployed as a **generated typed `Deployment replicas=1`** by the Phase-31
...
- A closed initial `AuthorityHandoff`: while holding the exact Lease the Phase-31 host applies the control-plane daemon
...
### Deliverables
- The positive gate: the Sprint-33.2 platform + trivial-app deploy driven to ready by the control-plane daemon and torn
...
- The negative regression guard: the Phase-7 corpus (a bad PVC↔PV pairing, a Keycloak-bypassing open ingress, a
  ...
  Gate 1 or Gate 2 **with its specific foreclosure tag matching the oracle-pinned hand-authored oracle `test/fixture/live_dsl_deploy/negative-expected-tags.tsv`** (each row: fixture → expected `dhall type` error or
...
2. "The live deploy path" is pinned to the identical entry point the positive fixture used, foreclosing the
   host-side re-run cheat (§M.3): every Phase-7 negative fixture is submitted through the exact same control-plane daemon
...
`app/amoebius/Amoebius/Entry/ControlPlane.hs` — delivered. This sprint promotes Sprint 38.1's admin-REST
...
- **`dhall update`** — deliver a new `InForceSpec` to a running cluster (requires an unsealed Vault and a root
  token). The control-plane daemon decrypts/stores the envelope in-process and reconciles toward it via Sprint 38.2's loop.
...
- **Committed seeded mutants (§M.2)**, committed and re-run, each MUST turn Validation red: (i) a
  *dropped-effect* mutant `test/mutant/dhall_gate1_schema/persist-password.patch` that writes the operator password to the container
  filesystem (must fail the §M.5 non-persistence observer); (ii) a *guard-weakening* mutant
  `test/mutant/dhall_gate1_schema/reach-any.patch` that accepts a seal-critical verb over any reach (must fail the reach matrix); and
  (iii) a *guard-weakening* mutant `test/mutant/dhall_gate1_schema/admit-unproven-secret.patch` that admits an upload whose named
...
- `DEVELOPMENT_PLAN/substrates.md` — confirm the Phase-38 linux-cpu gate row (the replicas=1 control-plane daemon, no
  election).
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/ControlPlane/{Daemon,Reconcile,Deploy,AdminApi}.hs`
  as Phase-38 design-first rows, and re-anchor the in-cluster-control-plane row to the current
...
- [Bootstrap Sequence Doctrine](../documents/engineering/bootstrap_sequence_doctrine.md) — the [§5](../documents/engineering/bootstrap_sequence_doctrine.md#5-the-admin-control-plane-the-cli--the-control-plane-daemon-rest-api) admin control
  plane (CLI ↔ control-plane daemon REST) delivered by Sprint 38.4, and the [§4](../documents/engineering/bootstrap_sequence_doctrine.md#4-the-host-daemon--control-plane-daemon-handoff) handoff point at which it is exposed
...
- [phase_29](phase_29_bootstrap_coordinator_kind.md) — the two-mode `pb` CLI whose admin-REST client mode Sprint
  24.3 defers to the control-plane daemon and Sprint 38.4 owns
