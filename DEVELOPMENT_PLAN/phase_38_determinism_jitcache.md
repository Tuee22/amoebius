...
content-addressed cache (bytes sha256-matching the `test/oracle/phase_38_oracle.dhall` pin, the named arm
...
`test/dhall/phase_38_determinism_repro.dhall` (one pinned content-addressed input blob, one pure seeded stage,
...
hand-authored fingerprint schema `test/golden/phase_38_substrate_fingerprint.schema.json` pinning the minimum
witness set (substrate lane `linux-cpu` + GHC / RTS / ISA / libc-ABI witnesses, **each with its absolute probe
path**, §M.3); the hand-computed SplitMix golden `test/golden/phase_38_splitmix_seeds.json` (streams `0`, `1`,
`37` worked by hand under gamma `0x9E3779B97F4A7C15`, never regenerated from `Rng.hs`); the committed compile-fail
fixture `test/compile-fail/phase_38_forge_blobsha.hs` (constructing `BlobSha "deadbeef"` from a `Text` literal,
which MUST fail at the "`BlobSha` constructor not in scope / not exported" locus, §M.8); the committed fake probe
`test/fake/phase_38_fake_ghc` (emits a different version, used by the fingerprint sensitivity check); and the
resource witness `test/golden/phase_38_resource_shape.json`. The committed seeded mutants this part must turn red
...
`test/oracle/phase_38_oracle.dhall` (independent of the SUT, never regenerated from the resolver's output, §M.3),
...
`test/negative/phase_38_cache_over_budget.dhall`, `test/negative/phase_38_cache_digest_size_conflict.dhall`,
`test/negative/phase_38_cache_deletion_credit.dhall`, `test/negative/phase_38_cache_concurrency_overflow.dhall`,
`test/negative/phase_38_ephemeral_under_reserved.dhall`, plus the compile-fail negatives
`test/negative/phase_38_freestring_key.hs` (cache key from a free `String`/`Text`/`Url`) and
`test/negative/phase_38_url_arm.hs` (a free-`Url` resolver arm), each failing *at its constructor locus* with
its named error and paired with a positive that differs only in the foreclosed dimension. The independent
owner+two-client image/resource/slot/transition witness is `test/oracle/phase_38_resource_shape.json`; the pinned
source recipe is committed alongside. The committed seeded mutants under `test/mutants/phase_38_cache/` this part must
...
byte/cache proofs above; the resource-shape witnesses (`test/golden/phase_38_resource_shape.json`,
`test/oracle/phase_38_resource_shape.json`) and the mutants under `test/mutants/phase_38_determinism/` and
`test/mutants/phase_38_cache/` are enumerated in [Resource provision](#resource-provision--the-recompute-runs-the-cache-owner-and-its-clients).
...
`test/mutants/phase_38_determinism/drop_run_resource_envelope.dhall` (omits one run's Pod row),
...
cache workload, `test/mutants/phase_38_cache/drop_client_envelope.dhall`, `drop_owner_envelope.dhall`,
...
compile-fail fixture** (§M.8), not a runtime property: `test/compile-fail/phase_38_forge_blobsha.hs` attempts
...
- The Phase-0-committed compile-fail fixture `test/compile-fail/phase_38_forge_blobsha.hs` (with its expected
...
### Validation
1. Type-level, verified by the committed compile-fail fixture `test/compile-fail/phase_38_forge_blobsha.hs`
...
the **Phase-0-committed schema** `test/golden/phase_38_substrate_fingerprint.schema.json` (§M.3), which pins a
...
**sensitivity check** substitutes one named probe's binary with a committed fake (`test/fake/phase_38_fake_ghc`
...
- The Phase-0-committed fingerprint schema `test/golden/phase_38_substrate_fingerprint.schema.json` (minimum
  witness set + each witness's absolute probe path) and the committed fake probe `test/fake/phase_38_fake_ghc`
...
2. The fingerprint carries every witness required by `test/golden/phase_38_substrate_fingerprint.schema.json`
...
1. A simulated 1-worker vs 100-worker dispatch in arbitrary order seeds stream `37` identically every time. The
   ...
   `test/golden/phase_38_splitmix_seeds.json` (§M.1, SplitMix64 with gamma `0x9E3779B97F4A7C15` worked by hand,
...
**Status**: Planned
**Implementation**: `src/Amoebius/Kernel/Determinism.hs`, `test/dhall/phase_38_determinism_repro.dhall`,
...
- The gate `.dhall` (`test/dhall/phase_38_determinism_repro.dhall`) that spins up the Phase-29 workflow, runs the
...
- The Phase-0-committed representative oracle set (authored before any kernel module exists, §M.1): the positive
  `test/dhall/phase_38_determinism_repro.dhall` and its one-dimension-differing negative siblings
...
3. Changing the resolved `.dhall` (the `..._flipped_metric.dhall` sibling, differing only in metric direction) or
   ...
   leg is exercised by an **in-process substitution using the committed fake probe** (`test/fake/phase_38_fake_ghc`),
...
by a **committed compile-fail negative fixture** `test/negative/phase_38_freestring_key.hs` (registered in the
...
1. There is no exported path to a cache key from a free string; the only path to a resident entry is content
   addressing — asserted by the committed compile-fail negative `test/negative/phase_38_freestring_key.hs`
...
served/compiled bytes **sha256-match the `test/oracle/phase_38_oracle.dhall` pin** — not an arbitrary "fake" blob:
...
by the committed compile-fail negative `test/negative/phase_38_url_arm.hs` (Phase-6 corpus, Phase-0-authored)
...
1. A cold cache triggers exactly one `resolve` and stores the result, **and the stored `ContentAddress` equals
   the `test/oracle/phase_38_oracle.dhall` pin**; a warm cache returns a handle with no resolve, proven by the
   argv-shim/`strace` observer recording zero backend subprocess on the warm path; there is no path that accepts
   a URL or free string, asserted by the committed compile-fail negative `test/negative/phase_38_url_arm.hs`
...
`test/oracle/phase_38_oracle.dhall` pin), the **second** client lookup is a **HIT** that reuses the resident
...
**Status**: Planned
**Implementation**: `test/dhall/phase_38_engine_cache.dhall` (the gate workflow naming a linux-cpu engine
...
`emptyDir`, whose stored bytes **sha256-match the `test/oracle/phase_38_oracle.dhall` pin**, the named arm
...
- The Phase-0-committed oracle `test/oracle/phase_38_oracle.dhall` (expected `ContentAddress`, catalog-owned
  final-resident/temporary byte `Quantity`, `--version`) and the committed seeded mutants under
  `test/mutants/phase_38_cache/` (`resolve _ = <marker>`, `prune = pure ()`, one-byte-short store), authored before
...
1. On the live linux-cpu `kind` cluster, the first client resolves `EngineRuntime.LlamaCppCpu@<pinned-ver>`
   ...
   `test/oracle/phase_38_oracle.dhall` pin, the named arm actually ran (attested by the OS-boundary
...
- `documents/engineering/resource_capacity_doctrine.md` — record that the §3/§4 `Quantity`/`fits` fold is reused
  ...
  witnesses `test/golden/phase_38_resource_shape.json` and `test/oracle/phase_38_resource_shape.json` and the
  resource mutants `test/mutants/phase_38_determinism/*`, `test/mutants/phase_38_cache/*`); record that linux-cpu datapoint here in
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Kernel/ContentAddress.hs`,
  ...
  Phase-0-committed oracle/negative/mutant artifacts (`test/dhall/phase_38_determinism_repro.dhall` and siblings,
  `test/oracle/phase_38_oracle.dhall`, `test/negative/phase_38_freestring_key.hs`,
  `test/negative/phase_38_url_arm.hs`, `test/mutants/phase_38_determinism/`, `test/mutants/phase_38_cache/`) as Phase-38
  design-first rows.
