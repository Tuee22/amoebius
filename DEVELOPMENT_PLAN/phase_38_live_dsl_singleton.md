# Phase 38: Live DSL deploy via the replicas=1 singleton
...
- [Sprints](#sprints)
- [Sprint 38.1: The control-plane singleton — a Deployment replicas=1, single-instance from k8s/etcd ⏸️](#sprint-381-the-control-plane-singleton--a-deployment-replicas1-single-instance-from-k8setcd-)
- [Sprint 38.2: Live reconcile of the platform + a trivial app from one `.dhall` ⏸️](#sprint-382-live-reconcile-of-the-platform--a-trivial-app-from-one-dhall-)
- [Sprint 38.3: Phase gate harness — live deploy + the pre-cluster negative corpus as a live regression guard ⏸️](#sprint-383-phase-gate-harness--live-deploy--the-pre-cluster-negative-corpus-as-a-live-regression-guard-)
- [Sprint 38.4: The admin REST surface — `vault init/unseal`, `dhall update`, secret KV-CRUD ⏸️](#sprint-384-the-admin-rest-surface--vault-initunseal-dhall-update-secret-kv-crud-)
...
⏸️ Blocked pending Phase-37 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
secret is admissible with no Vault interaction at all, which is what leaves Phases 30–33 independent of
Phase 33. The paired positive — the same spec admitted after the Phase-34 prompt CLI writes the secret — is
this claim's other half; neither alone is evidence.

**Invalidated historical record:**

**Done.** All four sprints are implemented and the Phase-38 Register-3 gate is sealed. It ran after the
Phase 37 gate (Keycloak-owned ingress) on the **linux-cpu** substrate: the retained single-node
`kind` cluster after Phases 29–37, with the full standing shape assembled by the registry/base-image work
(Phase 30), Vault/PKI (Phase 34), platform services (Phases 35–36), and Keycloak-owned edge (Phase 37), all
applied through the Phase-31 typed renderer + SSA reconciler onto Phase-33 retained storage. The delivered
stateless Haskell singleton holds the Kubernetes Lease, serves its health and admin endpoints, reconciles the
...
proves the bootstrap-host-holder → observed release/absence → singleton-holder handoff authorizes no
overlapping writers.
...
(Registers 1–2, substrate `none`): the typed spec gates — Gate 1, the Dhall typechecker (Phase 5), and Gate 2,
the in-process `Dhall.inputFile auto` decoder (Phase 6) — the illegal-state corpus and its per-entry
validation-locus ledger (Phase 7), the capacity/topology folds (Phase 8), the capability→provider→shape
binder (Phase 11) and opaque provision seal (Phase 12), the pure `renderAll` goldens (Phase 14), and the
`chain`/`--dry-run` plan (Phase 15) were all
authored and proven **in-process, with no cluster**. Phase 38 adds the runtime residue: the in-cluster
**control-plane singleton** — the `ControlPlaneSingleton` arm of `InClusterRole` ([daemon_topology_doctrine.md §2](../documents/engineering/daemon_topology_doctrine.md#2-context--role-an-orthogonal-grid)), decoded from the pod's frame config — deployed as a Kubernetes **Deployment with `replicas=1`** — exactly one Lease-held
...
it, applying the standard platform stack plus a trivial app through the Phase-31 reconciler to convergence with
...
object) — **never a bespoke amoebius election, no ranked-failover rule, no warm-standby candidate population, no signed-commit-log protocol**. The singleton is **stateless at the pod level** — it holds no PVC; its
durable state is exclusively the Vault-enveloped MinIO bucket — so a lost pod loses nothing. As a regression
belt, the pre-cluster negative corpus of Phase 7 is re-run against this live deploy path and each fixture still
...
in-namespace Sql) is deliberately deferred to Phase 39; the app here is trivial.

The initial ownership transition is explicit. Phase 31 acquired the deployment-global mandatory reconciler
Lease under the bootstrap-host holder before any host-driven apply and kept renewing it through Phases 33–36.
...
**Substrate:** linux-cpu — the single-node `kind` cluster from Phases 29–37; no apple, linux-cuda, or windows
...
flowchart LR
  ...
  s0["Sprint 38.1: The control-plane singleton — a Deployment replicas=1…"]
  s1["Sprint 38.2: Live reconcile of the platform + a trivial app from one .dhall"]
  s2["Sprint 38.3: Phase gate harness — live deploy + the pre-cluster negative…"]
  s3["Sprint 38.4: The admin REST surface — vault init/unseal, dhall update…"]
  gate["the phase 38 gate"]
...
```
*Orientation. The seams Phase 38 delivered in order; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns the sealed apparatus.*
...
- **Attribution via an OS-boundary observer (§M.5, forecloses the decorative-singleton cheat).** The gate
  harness (`test/integration/LiveDslSingletonGate.hs`) runs under a kubeconfig whose RBAC (a committed
  `test/fixtures/phase33/harness-rbac.yaml`) grants it exactly: `create`/`get`/`delete` on the singleton's own
...
- **History capacity is a gate precondition, not assumed retention.** Before the first platform/app mutation,
  the harness reads the Phase-29 `ControlPlaneStorageDemand` enforcement and proves Event/audit retention
...
- **Concrete representative set (§M.7).** The Phase-35/36 service set reconciled by this fixture is exactly:
...
- **oracle-pinned oracle (§M.1).** The positive fixture `dhall/examples/platform_plus_trivial_app.dhall`, the
  ...
  admin-surface oracles of [Sprint 38.4](#sprint-384-the-admin-rest-surface--vault-initunseal-dhall-update-secret-kv-crud-)
...
- **Committed seeded mutant (§M.2).** The gate names **≥1 committed seeded mutant** that MUST turn it red:
  ...
  `test/fixtures/phase33/mutants/enact-noop.patch` and re-run each gate, asserted red because pass-1 restores
  nothing. A second **effect-swap** mutant (the harness principal, not the singleton SA, issues the writes)
  MUST also go red via the attribution clause above. [Sprint 38.4](#sprint-384-the-admin-rest-surface--vault-initunseal-dhall-update-secret-kv-crud-)
...
and no overlapping mutation authority. The pre-cluster (Phase-7) negative corpus is re-run against that same
...
- **The paired positive.** The identical spec, after the Phase-34 prompt CLI writes those secrets, is
...
- [`preflight_validation_doctrine.md`](../documents/engineering/preflight_validation_doctrine.md)
  ...
  by [phase_49](phase_49_provider_deploy_checkpoint.md) / [phase_52](phase_52_provider_dynamic_nodes.md).
- [`daemon_topology_doctrine.md §3`](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-singleton)
  ...
  ("exactly one pod" is a k8s/etcd property, not an amoebius election), the singleton is a **Deployment `replicas=1`**, **stateless** at the pod level (no PVC; durable state exclusively the Vault-enveloped MinIO
  bucket), and single-writer authority is **delegated to k8s/etcd** through the mandatory `Lease`, never a
  bespoke election. This phase also performs the one-way authority handoff from the observed Phase-31
...
- [`dsl_doctrine.md §5`](../documents/engineering/dsl_doctrine.md#5-the-illegal-state-unrepresentable-contract)
  — *the illegal-state-unrepresentable contract*: Gate 1, Gate 2, bind/expand, the Phase-12 provision seal,
  and Phase-14 `renderAll` were discharged in-process in the pre-cluster band. This phase runs the **runtime residue** — the live path must follow decoded IR → bind/expand → `planInfrastructure` → explicit
...
## Sprint 38.1: The control-plane singleton — a Deployment replicas=1, single-instance from k8s/etcd ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
and exact runtime-storage/control-plane-state seals are implemented and live-validated.
**Implementation**: `src/Amoebius/ControlPlane/Singleton.hs` (the in-cluster singleton
role + the shared daemon spine); `src/Amoebius/ControlPlane/Reconcile.hs` (the `discover → diff → enact →
re-observe` loop wrapping the Phase-31 typed reconciler and its observed-Pod/runtime-storage normalization);
`src/Amoebius/ControlPlane/AuthorityHandoff.hs` (bootstrap-holder drain/release/readback and singleton
...
### Deliverables
- **The collapse of `app/singleton/` into the one executable.** The singleton branch reached its pod by being
  a second `executable` stanza — `amoebius.cabal:933`, whose `Main.hs` parses no arguments at all, because the
  file that was executed *was* the role selection. It becomes the `ControlPlaneSingleton` arm of the decoded
  ...
  `ControlPlane/Singleton.hs:138`
  ([legacy_tracking_for_deletion.md](legacy_tracking_for_deletion.md#one-binary-many-roles--2026-08-17)).
  Closing this also retires the transitional half of Phase 26's search-path gate check, which can only fail by
  the tree regaining a second executable.
- A control-plane singleton deployed as a **generated typed `Deployment replicas=1`** by the Phase-31
...
- The `discover → diff → enact → re-observe` reconcile loop that decodes the `InForceSpec` in-process
  (Phase-6 decoder), binds capabilities (Phase-11 binder), and applies the resulting manifests through the
  Phase-31 typed reconciler — idempotently, driven only by observed cluster state.
...
- A closed initial `AuthorityHandoff`: while holding the exact Lease the Phase-31 host applies the singleton
...
- Secret authority fused to the role (operates root Vault as the single in-cluster writer) and the admin-REST
  ...
  four-endpoint surface by [Sprint 38.4](#sprint-384-the-admin-rest-surface--vault-initunseal-dhall-update-secret-kv-crud-).
...
## Sprint 38.2: Live reconcile of the platform + a trivial app from one `.dhall` ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
second-pass audit/enact set after fresh discovery, routes the trivial app through the Phase-37 edge, and
...
down leaving the shared stack as found. Because Phases 29–37 leave that stack pre-converged, the numbered
...
### Deliverables
- A positive deploy `.dhall` composing the standard platform-service stack (Phases 35–36) and a **trivial**
  single-service app — deliberately narrower than the Phase-39 tenancy projection (no per-app namespace,
...
- A leak-free teardown obligation carried by the deploy fixture — a test-topology `.dhall` whose postflight
  ...
  reclaimed, while the pre-existing Phase-35/36 service set and Phase-37 edge are restored to Ready rather than
  swept.
...
1. Before the first pass the harness deletes the components named in
   ...
   example Prometheus's — so a pre-converged Phase-35/36 stack cannot ride the gate (§M.6). The first pass
   ...
   to `expected-enact-pass1.json`; the app is reachable through the Phase-37 Keycloak-owned edge; and a re-run
...
2. Teardown leaves no leaked resources. The postflight sweep is scoped to this run's provisioned objects,
   ...
   it creates — that label set is authored here, and Phase-56 flag-at-creation machinery is not assumed — and
   the sweep is empty over it. Separately, every platform component the harness perturbed is asserted back at
   Ready so the shared Phase-35/36 stack is left as found, and the apiserver audit log records that **every**
...
## Sprint 38.3: Phase gate harness — live deploy + the pre-cluster negative corpus as a live regression guard ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
attribution, durable replacement, leak-free teardown, ledger, and mutation checks are sealed.
**Implementation**: `test/integration/LiveDslSingletonGate.hs` (linux-cpu spin-up / reconcile /
teardown + the negative regression assertions); `test/integration/RuntimeStorage.hs`
(planned-slot→observed-Pod-UID readback, SplitRuntime backing boundaries, node scope/domain/ownership
equality, reservation/observed no-double-debit, and alias controls); the reused Phase-7 negative corpus
under `dhall/examples/illegal_*.dhall` (re-run, not re-authored), `test/live/SingletonLiveSpec.hs`,
...
under the perturbation and attribution regime of Sprint 38.2, tears down leak-free, then re-runs the Phase-7
...
(flip the Phase-38 status when the gate passes).
...
pre-cluster (Phase-7) negative corpus so each deliberately-illegal `.dhall` still fails to type-check or decode
...
### Deliverables
- The positive gate: the Sprint-33.2 platform + trivial-app deploy driven to ready by the singleton and torn
  down leak-free, expressed as a test-topology `.dhall` with a teardown obligation.
- The negative regression guard: the Phase-7 corpus (a bad PVC↔PV pairing, a Keycloak-bypassing open ingress, a
...
- **Committed seeded mutants (§M.2):** at least `test/fixtures/phase33/mutants/enact-noop.patch` (the
...
- A **Register-3** proven/tested/assumed ledger recording the live-enforcement result (the apiserver admitted
  the rendered manifests) and marking the deferred surfaces — full app tenancy (Phase 39), and the
...
2. "The live deploy path" is pinned to the identical entry point the positive fixture used, foreclosing the
   host-side re-run cheat (§M.3): every Phase-7 negative fixture is submitted through the exact same singleton
...
## Sprint 38.4: The admin REST surface — `vault init/unseal`, `dhall update`, secret KV-CRUD ⏸️
...
mode of the two-mode `pb` CLI, deferred to "the singleton" by
[phase_29](phase_29_bootstrap_coordinator_kind.md) Sprint 29.3 and owned here), `pb/pb/cli.py`, and
`app/singleton/Main.hs` — delivered. This sprint promotes Sprint 38.1's admin-REST
**control-surface stub** to the real surface; it does not re-implement the Sprint-29.1 Argon2id→AEAD unlock
envelope, the Phase-34 Vault client, or the Sprint-33.2 reconcile loop — it is the operator-facing channel
into all three.
**Blocked by**: reopened numeric predecessor gates.
...
- **`dhall update`** — deliver a new `InForceSpec` to a running cluster (requires an unsealed Vault and a root
  token). The singleton decrypts/stores the envelope in-process and reconciles toward it via Sprint 38.2's loop.
...
- **The `pb` admin-REST client mode** — the second mode of the two-mode Python CLI, completing the deferral
  Phase 29 Sprint 29.3 records. It is a client of this surface and adds no second control path.
...
- A **Register-3** proven/tested/assumed ledger recording the admission gate as **runtime-checked, live**, and
  ...
  ([phase_39](phase_39_app_tenancy.md)) and the parent→child `ParentReachChannel` use of this surface
  ([phase_50](phase_50_provider_child_bringup.md)), neither of which this phase exercises.

> **HTTP server resolution.** `app/singleton/Main.hs` uses a small static Haskell HTTP/1.1 server over the
...
> singleton lose readiness or act on stale authority.

### Validation
1. **The post-handoff operator sequence, end to end.** After Sprint 38.1's observed handoff, drive `vault
...
- `documents/engineering/substrate_doctrine.md` — the §6 bootstrap coordinator-contract note that `pb`'s second mode is "a
  later phase" resolves to Sprint 38.4.

**Cross-references to add:**
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-38 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — confirm the Phase-38 linux-cpu gate row (the replicas=1 singleton, no
  election).
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/ControlPlane/{Singleton,Reconcile,Deploy,AdminApi}.hs`
  as Phase-38 design-first rows, and re-anchor the in-cluster-singleton row to the current
  `#3-the-control-plane-singleton` (no election).
- `DEVELOPMENT_PLAN/phase_29_bootstrap_coordinator_kind.md` — Sprint 29.3's deferred `pb` admin-REST client mode
  now names Sprint 38.4 as its owner.
- `DEVELOPMENT_PLAN/phase_50_provider_child_bringup.md` — its post-handoff child admin-REST bring-up can now
  name Sprint 38.4 in `Blocked by`.
...
- [Bootstrap Sequence Doctrine](../documents/engineering/bootstrap_sequence_doctrine.md) — the [§5](../documents/engineering/bootstrap_sequence_doctrine.md#5-the-admin-control-plane-the-cli--the-singleton-rest-api) admin control
  plane (CLI ↔ singleton REST) delivered by Sprint 38.4, and the [§4](../documents/engineering/bootstrap_sequence_doctrine.md#4-the-host-daemon--singleton-handoff) handoff point at which it is exposed
...
- [phase_29](phase_29_bootstrap_coordinator_kind.md) — the two-mode `pb` CLI whose admin-REST client mode Sprint
  24.3 defers to the singleton and Sprint 38.4 owns
- [phase_34](phase_34_vault_pki.md) — the root Vault, unlock-material envelope, and built-in Vault client the
  admin endpoints call
- [phase_50](phase_50_provider_child_bringup.md) — the parent→child bring-up that drives a child through this
  same surface over its `ParentReachChannel`
- [phase_31](phase_31_object_reconciler.md) — the typed renderer + SSA reconciler that renders and applies
  the singleton and its manifests
- [phase_35](phase_35_platform_backbone.md) — the standard platform-service stack the live `.dhall` deploys
- [phase_37](phase_37_keycloak_ingress.md) — the Keycloak-owned edge the trivial app routes through
- [phase_39](phase_39_app_tenancy.md) — the app-tenancy projection (namespace + ObjectStore + Sql) deferred
  from this phase
