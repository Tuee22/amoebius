# Phase 39: Tenant/provider provisioning
...
Phase 39 delivers the tenant/provider provisioning; its design is owned by [tenancy_doctrine.md](../documents/engineering/tenancy_doctrine.md), [service_capability_doctrine.md](../documents/engineering/service_capability_doctrine.md), [platform_services_doctrine.md](../documents/engineering/platform_services_doctrine.md), and the plan for reaching it is owned here.
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_38_live_dsl_singleton.md, DEVELOPMENT_PLAN/phase_40_pulsar_client.md, DEVELOPMENT_PLAN/phase_41_user_tenant_isolation_live.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 39.1: Derive, apply, and externally read back tenant provider policy ⏸️](#sprint-391-derive-apply-and-externally-read-back-tenant-provider-policy-)
...
Phase 39 binds a checked `TenantSpec` containing issuer-qualified `SubjectSpec`, `Membership`, `Owner`, and
...
The phase then applies that sealed derivation to the live Phase-38 cluster and reads each provider back through
...
policies: Phase 41 owns the real-credential, own-subject/foreign-subject and own-tenant/foreign-tenant
...
**Depends on:** Phase 21 and Phase 37. Phase 21 supplies the closed authorization graph and derive-not-author
boundary; Phase 38 supplies the live checked-spec singleton. Phase 20 is transitive through Phase 21.

**Substrate:** linux-cpu — the single-node `kind` cluster assembled through Phase 37. No linux-cuda, Apple,
...
[Gate integrity](#gate-integrity) — Register 3 green, the application data path UNVERIFIED until Phase 40.
...
- **Pinned oracle:** `test/fixtures/phase_35/provider_projection_matrix.tsv` is hand-authored before the
...
- The provider index is closed and exhaustive. NetworkPolicy is a Kubernetes-API payload; Postgres roles and
  ...
  objects. Pulsar application produce/consume is owned by Phase 40 and scoped end-to-end enforcement by Phase
  36.
...
- [`tenancy_doctrine.md` §4 — typed tenant and subject shapes](../documents/engineering/tenancy_doctrine.md#4-the-typed-shapes-tenantspec--subjectspec--membership--owner--rolebinding): consume the Phase-20 identity
...
- [`illegal_state_security.md` §3.80](../documents/illegal_state/illegal_state_security.md#380-a-subject-resolving-or-mutating-another-subjects-resource-without-a-grant): Phase 39 provisions the policy
  precondition but leaves its live request-enforcement residue to Phase 40.
...
## Sprint 39.1: Derive, apply, and externally read back tenant provider policy ⏸️
...
None. Phase 41 retains the application request-isolation residue.
...
- `documents/illegal_state/illegal_state_security.md` — attach the killed hand-authored/tenant-collapse evidence
  while retaining Phase 41's live enforcement residue.
...
- `DEVELOPMENT_PLAN/phase_40_pulsar_client.md` and `phase_41_user_tenant_isolation_live.md` — consume provider
  readiness without treating it as user authorization evidence.

## Related Documents

- [Phase 21 — UI authorization kernel](phase_21_ui_authorization_kernel.md)
- [Phase 38 — live DSL singleton](phase_38_live_dsl_singleton.md)
- [Phase 40 — Pulsar client](phase_40_pulsar_client.md)
- [Phase 41 — live subject/tenant isolation](phase_41_user_tenant_isolation_live.md)
