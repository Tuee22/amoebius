# Phase 48: Determinism kernel + jit-build CacheBudget cache
...
brought up by the Phase 24 midwife, whose base image (Phase 25) already bakes the shared **jit-build resolver
...
a request-carried derived seed — deliberately **not** an infernix inference run (that lift is Phase 49) and
**not** a jit-resolved ML engine (this phase resolves the engine, but the infernix inference that rides it is
Phase 49; the CUDA/jitML kernel tier is Phase 51). The `ModelArtifact` staging tier (Tier 2) and the JIT kernel
...
flowchart LR
  ...
  ca[["ContentAddress typeclass"]]:::intent --> store[/"Phase 37 content-addressed store"/]:::effect
  eh --> store
...
hand-authored fingerprint schema `test/golden/phase_48_substrate_fingerprint.schema.json` pinning the minimum
witness set (substrate lane `linux-cpu` + GHC / RTS / ISA / libc-ABI witnesses, **each with its absolute probe
path**, §M.3); the hand-computed SplitMix golden `test/golden/phase_48_splitmix_seeds.json` (streams `0`, `1`,
`37` worked by hand under gamma `0x9E3779B97F4A7C15`, never regenerated from `Rng.hs`); the committed compile-fail
fixture `test/compile-fail/phase_48_forge_blobsha.hs` (constructing `BlobSha "deadbeef"` from a `Text` literal,
which MUST fail at the "`BlobSha` constructor not in scope / not exported" locus, §M.8); the committed fake probe
`test/fake/phase_48_fake_ghc` (emits a different version, used by the fingerprint sensitivity check); and the
resource witness `test/golden/phase_48_resource_shape.json`. The committed seeded mutants this part must turn red
...
`test/oracle/phase_48_oracle.dhall` (independent of the SUT, never regenerated from the resolver's output, §M.3),
...
`test/negative/phase_48_cache_over_budget.dhall`, `test/negative/phase_48_cache_digest_size_conflict.dhall`,
`test/negative/phase_48_cache_deletion_credit.dhall`, `test/negative/phase_48_cache_concurrency_overflow.dhall`,
`test/negative/phase_48_ephemeral_under_reserved.dhall`, plus the compile-fail negatives
`test/negative/phase_48_freestring_key.hs` (cache key from a free `String`/`Text`/`Url`) and
`test/negative/phase_48_url_arm.hs` (a free-`Url` resolver arm), each failing *at its constructor locus* with
its named error and paired with a positive that differs only in the foreclosed dimension. The independent
owner+two-client image/resource/slot/transition witness is `test/oracle/phase_48_resource_shape.json`; the pinned
...
byte/cache proofs above; the resource-shape witnesses (`test/golden/phase_48_resource_shape.json`,
`test/oracle/phase_48_resource_shape.json`) and the mutants under `test/mutants/phase_48_determinism/` and
...
backoff, replacement-on-Failed, terminal retention) and is also an exact Phase-37 `WorkflowRuntimeDemand`
...
Phase-37 runtime/mutation unit) must each turn the resource gate red even if the output bytes match. For the
...
- [`content_addressing_doctrine.md §2`](../documents/engineering/content_addressing_doctrine.md#2-the-three-tier-store-blobs--manifests--pointers)
  — *the three-tier store (blobs ← manifests ← pointers)*: the `ContentAddress` typeclass lifts Phase 37's
...
- [`testing_doctrine.md §2`](../documents/engineering/testing_doctrine.md#2-the-registers-of-amoebius-testing)
  ...
  emits a proven/tested/assumed ledger naming that register, with the model/kernel tiers (Phases 49/51) marked
  deferred.
...
compile-fail fixture** (§M.8), not a runtime property: `test/compile-fail/phase_48_forge_blobsha.hs` attempts
...
lift Phase 37's concrete blob/manifest key renderers into a kernel-level `ContentAddress` typeclass so that
"a content-derived name cannot be forged" is a single reusable primitive shared later by the engine cache
(Sprint 48.5) and by both infernix (Phase 49) and jitML (Phase 51), not a per-store copy.
...
**Blocked by**: Sprint 48.1; Phase 24 gate (substrate detection — the linux-cpu substrate fingerprint gathered
...
the **Phase-0-committed schema** `test/golden/phase_48_substrate_fingerprint.schema.json` (§M.3), which pins a
...
**sensitivity check** substitutes one named probe's binary with a committed fake (`test/fake/phase_48_fake_ghc`
...
1. A simulated 1-worker vs 100-worker dispatch in arbitrary order seeds stream `37` identically every time. The
   ...
   `test/golden/phase_48_splitmix_seeds.json` (§M.1, SplitMix64 with gamma `0x9E3779B97F4A7C15` worked by hand,
...
**Status**: Planned
**Implementation**: `src/Amoebius/Kernel/Determinism.hs`, `test/dhall/phase_48_determinism_repro.dhall`,
...
**Blocked by**: Sprint 48.1; Sprint 48.2; Sprint 48.3; Phase 37 gate (the content store + workflow runtime the
gate workload runs on); Phase 24 gate (the live single-node `kind` cluster and the substrate fingerprint)
...
without an infernix inference run (Phase 49) or a jit-resolved engine (that is the cache seam below and the
Phase-49 lift), and prove same-substrate reproducibility as one half of the phase gate without overclaiming
...
- The gate `.dhall` (`test/dhall/phase_48_determinism_repro.dhall`) that spins up the Phase-37 workflow, runs the
...
3. Changing the resolved `.dhall` (the `..._flipped_metric.dhall` sibling, differing only in metric direction) or
   ...
   leg is exercised by an **in-process substitution using the committed fake probe** (`test/fake/phase_48_fake_ghc`),
...
**Blocked by**: Sprint 48.1 (the `ContentAddress` primitive the cache keys against); Phase 7 gate (the
`fits`/`carve` capacity fold this bound reuses); Phase 37 gate (the content-addressed store shape).
**Independent Validation**: a property + boundary suite shows the cache admits no key from a free string, proven
by a **committed compile-fail negative fixture** `test/negative/phase_48_freestring_key.hs` (registered in the
...
4. **Pin-aware pruning is exercised, not declared:** a cache filled to `CacheBudget` with a mix of pinned and
   ...
   survives). This is the pure-pool property; its live on-disk counterpart is the Sprint 48.8 postflight residency
...
not yet built.
**Blocked by**: Sprint 48.5 (the cache the resolver stores into); Phase 25 gate (the base image baking the
...
served/compiled bytes **sha256-match the `test/oracle/phase_48_oracle.dhall` pin** — not an arbitrary "fake" blob:
...
by the committed compile-fail negative `test/negative/phase_48_url_arm.hs` (Phase-6 corpus, Phase-0-authored)
...
1. A cold cache triggers exactly one `resolve` and stores the result, **and the stored `ContentAddress` equals
   the `test/oracle/phase_48_oracle.dhall` pin**; a warm cache returns a handle with no resolve, proven by the
...
**Blocked by**: Sprint 48.5, Sprint 48.6; Phase 26 gate (the typed SSA object reconciler that renders the cache
owner and client pods); Phase 30 gate (the platform backbone the pods schedule onto).
...
`test/oracle/phase_48_oracle.dhall` pin), the **second** client lookup is a **HIT** that reuses the resident
...
- The pod wiring (rendered by the Phase-26 reconciler) that gives the owner a bounded disk-backed `emptyDir` with
...
**Status**: Planned
**Implementation**: `test/dhall/phase_48_engine_cache.dhall` (the gate workflow naming a linux-cpu engine
identity) and `test/live/EngineCacheGate.hs` (the Register-3 gate harness) — target paths, not yet built.
**Blocked by**: Sprint 48.4 (the determinism half of the phase gate); Sprint 48.5, Sprint 48.6, Sprint 48.7;
Phase 24 gate (the live `kind` cluster + substrate detect); Phase 25 gate (the baked resolver/toolchain and the
...
(flip the Phase-48 status when the gate passes), `DEVELOPMENT_PLAN/substrates.md`.
...
- A Register-3 ledger recording: URL-foreclosure and the required bounded-`CacheBudget` field as
  ...
  **deferred** (Phases 49/51), with cross-node and cross-substrate reuse explicitly not asserted.
...
- `documents/engineering/content_addressing_doctrine.md` — the §6 proven/tested/assumed table gains an
  ...
  Tier-2/Tier-3 realizations remain Phases 49/51 targets, and add the kernel module paths
...
- `documents/engineering/resource_capacity_doctrine.md` — record that the §3/§4 `Quantity`/`fits` fold is reused
  ...
  witnesses `test/golden/phase_48_resource_shape.json` and `test/oracle/phase_48_resource_shape.json` and the
...
- [phase_37](phase_37_content_store_workflow.md) — the content store + workflow runtime this phase lifts and runs on
- [phase_49](phase_49_infernix_lift.md) — the infernix CPU-inference lift whose reproducibility reuses this kernel and rides this resolver next
- [phase_51](phase_51_jitml_lift_cuda.md) — the jitML/CUDA lift whose kernel tier reuses this resolver and kernel
