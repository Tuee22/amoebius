# Phase 12: GADT-indexed IR + total decoder (gadt-decode)
...
- [Sprints](#sprints)
- [Sprint 12.1: The amoebius cabal package + `dsl-spec` test-suite skeleton ✅](#sprint-121-the-amoebius-cabal-package--dsl-spec-test-suite-skeleton-)
- [Sprint 12.2: GADT-indexed IR + smart constructors + phantom tenant refs + ownership indices ✅](#sprint-122-gadt-indexed-ir--smart-constructors--phantom-tenant-refs--ownership-indices-)
- [Sprint 12.3: The fail-closed decoder (`Dhall.inputFile auto` + exception-catch) + structured `DecodeError` ✅](#sprint-123-the-fail-closed-decoder-dhallinputfile-auto--exception-catch--structured-decodeerror-)
- [Sprint 12.4: The gadt-decode decode battery (`dsl-spec`) — the gate ✅](#sprint-124-the-gadt-decode-decode-battery-dsl-spec--the-gate-)
- [Sprint 12.5: Decoding the shared `SecretRef` and rejecting a literal ✅](#sprint-125-decoding-the-shared-secretref-and-rejecting-a-literal-)
...
✅ Done — resealed 2026-08-17 on the amended contract. `python3 tools/gadt_decode_ir_gate.py` passes all
...
deferred; the extension-astcheck locus was pinned to Phase 20 where the registry said 15. Each is now a named constant
...
`sha256:ee3ee22f7aa4b788…`; Phase 12 owns no remaining migration deferral.
...
same scope amendment that reopened Phase 11: gadt-decode must decode the shared `SecretRef` and reject any literal
...
**Remaining work.** None. Sprint 12.5 discharged the amendment and the gate is green on all ten sides.
...
shared decision core depended on the Phase-32 UI-server boundary. While that boundary's ABI is unimplemented
this made the core itself fail to compile and blocked this phase outright. The entry point now belongs to the
`amoebius` executable, which is the component that calls it; the unimplemented seam stays inside Phase 32
...
This phase builds the second of the DSL's typed spec gates. dhall-typecheck (Phase 11) rejects what is not even
...
The Phase-11 prerequisite passed on 2026-08-09 (`python3 tools/dhall_typecheck_schema_gate.py`, ledger
`dynamically-resolved`). gadt-decode passed with five semantic-hash-pinned positives, exactly three tagged refinement negatives, three legal/illegal compile pairs, and 5,527 exact structural rows. Capacity, binding, provisioning, and runtime remain UNVERIFIED.
...
/ reconcile / control-plane daemon runtime (Phase 44), the pure capacity/topology fold implementation and properties
(Phase 14) consumed by the conditional infrastructure-planning/post-materialization provision seal (Phase 18),
the capability→provider binder (Phase 17), and
the exhaustive illegal-state corpus with its per-entry validation-locus
ledger and QuickCheck properties (Phase 13). This phase checks the decoder is non-partial, fail-closed, and structurally
...
Phase-0 documentation lint and the Phase-11 `dhall type` corpus.

**Lane:** none ([§L](development_plan_standards.md#l-one-substrate-discipline))

**Register:** 1 — pure/golden, in-process, no cluster ([§K](development_plan_standards.md#k-honesty-proven--tested--assumed)).

**Gate:** `cabal test dsl-spec` is green over the positive fixtures, tagged gadt-decode negatives, structural
resource oracle, and committed mutants named in [Gate integrity](#gate-integrity). Phase 13 does not open
unless the ledger records Register 1 green and every runtime-enforcement layer UNVERIFIED.

```mermaid
flowchart LR
  %% register: orientation
  s0["Sprint 12.1: The amoebius cabal package + dsl-spec test-suite skeleton"]
  s1["Sprint 12.2: GADT-indexed IR + smart constructors + phantom tenant refs…"]
  s2["Sprint 12.3: The fail-closed decoder (Dhall.inputFile auto…"]
  s3["Sprint 12.4: The gadt-decode decode battery (dsl-spec) — the gate"]
  gate["the phase 12 gate"]
...
```
*Orientation. The seams phase 12 builds, in order; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns the apparatus. Not run.*
...
comprises: (a) the **representative gadt-decode negative set**, defined concretely as **exactly one negative fixture per named `DecodeError` failure class** — `SchemaMismatch`, `OutOfDomainArm`, `UnspellableCombination` — each
...
schema module Phase 11 owns, the expected-tag oracle gains their rows in Phase 0 alongside the rest.
...
- [`dsl_doctrine.md §4 — total composability`](../documents/engineering/dsl_doctrine.md#4-total-composability):
  adopt the **import policy** the doctrine assigns to the Phase-11/12 gate. `env:` and remote (`http(s):`)
  imports are forbidden in any authored or uploaded spec; every spec is resolved-and-frozen to a
  fully-local, `sha256:…`-hashed expression **before** decode. This phase owns the enforcement — Sprint 12.3's
...
- [`resource_capacity_doctrine.md §3/§4`](../documents/engineering/resource_capacity_doctrine.md#3-the-types-quantity-capacity-demand-budget)
  ...
  normalizes the declarations in `ClusterIR`; Phase 14 implements the arithmetic/placement folds; Phase 18 runs
  ...
  the public boundary; a raw decoded or merely bound value can never reach Phase 20's `renderAll`.
...
## Sprint 12.1: The amoebius cabal package + `dsl-spec` test-suite skeleton ✅
...
### Remaining Work
None for Phase 12. Later DSL expansion belongs to the numerically assigned phases.

## Sprint 12.2: GADT-indexed IR + smart constructors + phantom tenant refs + ownership indices ✅
...
type-forecloses), `documents/engineering/dsl_doctrine.md` (Phase-12 status backlink),
...
- The gadt-decode side of the three surfaces [Phase 11](phase_11_dhall_typecheck_schema.md) adds to the schema:
...
- The **role vocabulary itself, as a closed decoded union**, which the relations below already presuppose and
  ...
  decoded total from Phase 11's `dhall/amoebius/Role.dhall`. An arm outside the union is an `OutOfDomainArm`
...
- Normalized resource declarations wired into the real IR, not ornamental side records: every execution-unit
  ...
  - The body also contains a complete structurally compatible Pod/host `ResourceEnvelope` and the identity
    required for Phase 17's exactly-once `BoundExecutionSet` join.
  ...
  - The reserved total the pad contributes to remains underivable here as it is unauthorable in dhall-typecheck:
    Phase 14 folds it and Phase 18's post-bind `provision` boundary alone constructs it.
  ...
  - Phase 18's post-bind `provision` boundary constructs the private rounded `ProvisionedVolumeDemand`
    consumed by render, using the geometry and folds implemented in Phase 14.
- Canonical bounded service-storage inputs remain structural rather than being collapsed to caller-authored
  ...
  backend;
  - Phase 17 alone exact-joins those digests to `RegistryStoredArtifact` metadata and constructs
  ...
  - Only private cache/registry/Vault witnesses constructed at Phase 18's post-bind `provision` boundary,
    using Phase 14's folds, may carry derived peaks.
- Every durable platform declaration retains normalized `BookKeeperGeometry` (ensemble/write/ack quorums,
  ...
  only gateway identity and execution-model selection; Phase 17 merges producer writer policies and constructs
...
- The normalized capacity tree retains the physical identity graph without alias-erasing or synthesizing
  ...
  - Phase 14 owns value-level global uniqueness, exactly-one parent/reference, layout alias/support checks,
...
- A managed provider target preserves the exact normalized `{ account : CloudAccountId, nodeClasses :
  ...
  - Phase 16 derives a private `ProvisionedNodeRootVolumeRequest { volumeType, requiredUsableBytes,
  ...
  - It preserves the fields needed to derive a globally scoped `ProviderInstanceId { account, cluster,
    ...
    template path and the operands for the Phase-14 local uniqueness, reference, layout alias/support,
...
- Every normalized `Observability` deployment binding retains a mandatory finite
  ...
  defaulted field, scalar query-temp, or a descriptor-independent fixed Prometheus provision has no normalized bypass. Phase 17
...
- Every normalized build retains a mandatory `BuildExecutionEnvelope`: a non-empty list of
  ...
  dependency references are closed and the graph acyclic; Phase 14 derives maxima over every legal concurrent
  set. All quantities and references survive in their refined domains; there is no optional, editable-
  aggregate, or descriptor-independent builder-resource bypass. Phase 36 owns snapshot-bound live admission,
  not decoding.
- Every normalized `KindEngineDemand` retains non-empty ordinal-indexed node-container runtime, full
  ...
  Docker/containerd/kind-supervisor reserve. The decoder preserves the nesting; Phase 14 proves in-node reserve
  + allocatable fits the container and then charges that container once plus only the host reserve.
    ...
    - Missing raw host supply, process-template qualification, or SKU identity is a decoder-field-inventory
      failure.
    - Phase 35 owns kind fit/enforcement; live multi-node rke2 admission/enforcement remains an explicitly
...
- An in-file honesty note that binding/capacity/topology totals ([§4.6](../documents/illegal_state/illegal_state_techniques.md#46-capacity-accounting--placement-witness-compute-and-summed-demand-within-capacity-storage-checked)/[§4.7](../documents/illegal_state/illegal_state_techniques.md#47-compatibility--topology-relations-by-construction-over-a-collection)) are *not* foreclosed by these
  types — the decoded declarations are intentionally **unprovisioned**. Phase 14 owns the total feasibility
  folds and Phase 18 invokes them on the fully expanded `BoundDeployment`; only their private constructor
...
1. For each of [§4.2](../documents/illegal_state/illegal_state_techniques.md#42-capability-and-phantom-tenant-tags--cross-tenant-refs-are-uninhabitable) (phantom tenant), [§4.3](../documents/illegal_state/illegal_state_techniques.md#43-gadt-indexed-state-machines--only-legal-transitions-are-typed) (GADT transition index), and [§4.4](../documents/illegal_state/illegal_state_techniques.md#44-ownership-indices--single-owner-ssot-structurally) (ownership index), the phase commits
   ...
   **and** is the constructor a named Phase-11 positive fixture is committed to decode through (its header
   ...
   locus). The compile-time pair is Sprint 12.2's standalone check; the decode-through round-trip to the cited
   positive is confirmed at the Sprint 12.4 gate once `decodeCluster` exists (Sprint 12.3). The check is red if
...
2. The pair cannot be satisfied by a strawman `mkCrossTenantRef` that was simply never defined. Because the
   ...
   the fixtures, and the exhaustive compile-fail corpus is assembled in Phase 13.
...
### Remaining Work
None for Phase 12; exhaustive catalog expansion is Phase 13.

## Sprint 12.3: The fail-closed decoder (`Dhall.inputFile auto` + exception-catch) + structured `DecodeError` ✅
...
## Sprint 12.4: The gadt-decode decode battery (`dsl-spec`) — the gate ✅

**Status**: Done — the capability is re-established by the migrated gate; the sprint's committed-ledger, pinned-toolchain, and repository-resident evidence mechanics are superseded
**Implementation**: `test/spec/dsl/DecodeSpec.hs`; positive fixtures reuse Phase 11's
...
`DEVELOPMENT_PLAN/README.md` (flip the Phase-12 status when the gate passes).
...
with model↔runtime correspondence marked UNVERIFIED (owned by Phase 44). The exhaustive per-catalog-entry
corpus, the QuickCheck closure/round-trip/fold-totality properties, and the per-entry validation-locus ledger
are the front-loaded next phase ([Phase 13](phase_13_illegal_state_corpus.md)); the capacity/topology fold
negatives are [Phase 14](phase_14_capacity_core_folds.md), and provider-expanded/capability feasibility is
checked at [Phase 18](phase_18_provision_seal.md)'s conditional post-bind infrastructure-planning and
...
### Remaining Work
None. The exhaustive per-catalog-entry corpus begins in Phase 13.

## Sprint 12.5: Decoding the shared `SecretRef` and rejecting a literal ✅
...
[Phase 40](phase_40_vault_pki.md), and the prompt write path that turns a `Prompt` reference into a readable
one is Phase 40's too.

## Documentation Requirements

**Engineering docs to update (when the gate runs, flip the honest layer, never before):**
- `documents/engineering/dsl_doctrine.md` — backlink §5's gadt-decode to the in-process Phase-12 decoder; keep the
  runtime-enforcement half as Tier-2 residue owned by Phase 43.
- `documents/illegal_state/illegal_state_catalog.md` — annotate each entry the IR type-/decode-forecloses here
  ...
  deferred, and keep capacity/topology/provider-feasibility entries deferred to the Phase-14 fold and Phase-18
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius` cabal package, `src/Amoebius/Dsl/{Types,
  SmartConstructors,Ref,Decode,Error}.hs`, and the `dsl-spec` test-suite as Phase-12 design-first rows.

## Related Documents
- Phase-12 partial-foreclosure ledger — the tested/proven/unverified split sealed by this gate
...
- [phase_11](phase_11_dhall_typecheck_schema.md) — dhall-typecheck, the Dhall schema this decoder mirrors
- [phase_13](phase_13_illegal_state_corpus.md) — the exhaustive illegal-state corpus, properties, and
  validation-locus ledger built atop this decoder
- [phase_14](phase_14_capacity_core_folds.md) — the pure capacity/topology fold implementation and
  properties deferred from here; Phase 18 invokes them after bind/expansion while deriving the conditional
  infrastructure plan and again at the post-materialization provision seal
- [phase_44](phase_44_live_dsl_deploy.md) — the Tier-2 runtime-enforcement half of the DSL
