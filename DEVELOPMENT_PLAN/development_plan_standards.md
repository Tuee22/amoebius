...
Reviewer inspection owns correspondence between the two surfaces.
...
phase and sprint says `NOT VALIDATED`. An authorized reviewer—the human user or an explicitly delegated
agent—may promote a qualified candidate to ✅ Done. Gates, digests, and generated evidence cannot promote by
themselves.

At this reset, Phase 0 is `🔄 Active — NOT VALIDATED`; every later numbered phase is
`⏸️ Blocked — NOT VALIDATED` pending its immediate predecessor's reviewer approval. No prior completion claim,
...
An authorized promotion is a bound status-only projection, not an unbound edit after approving a different source
...
by the approval. An authorized reviewer applies that projection. A delegated agent may do so after inspecting
the complete qualified candidate, then continue to the next numeric phase. Gate logic may verify the two
digests and allowed-field diff but may not self-apply a promotion. Any other byte change invalidates the
projection and requires a new candidate.
...
public-command target, not an assertion that it exists, runs, or passes. Until Phase 50 has current authorized-reviewer
...
preceding reviewer approval. Phase 49 is the no-hardware end-to-end DSL promotion barrier; no fake-host takeover,
hardware bring-up, container, registry, cluster, GPU, or cloud evidence may open before it is reviewer-approved.
...
**Oracle**: <separate .hs module, independence boundary, and reviewer>
...
parent gate, inspected by an authorized reviewer, and promoted through the bound status projection.

`Blocked by` orders implementation; it is not a request for reviewer approval between sprints. A downstream
...
phase uses only `[Phase N](phase_NN_<slug>.md) reviewer approval` for the immediate predecessor; and every later
...
Authorized review is consolidated at the phase gate. The qualified parent gate must rerun and retain every
sprint seam in one complete candidate run, and an authorized reviewer must inspect that candidate and apply
the status projection before any sprint or phase is Done. Where `Blocked by` names an earlier phase rather
than an earlier sprint, that dependency remains reviewer approval. A delegated agent may promote and continue
across multiple phases in one run, but each phase still receives its own candidate and ordered promotion.
...
the phase's implementation-ready sprint seams without asking for approval of component diagnostics, candidate
fragments, or readiness decisions.
...
finding or verdict. The closed Haskell inventory and total owner/closure bindings govern execution; an
authorized reviewer compares this field and the rest of the sprint prose with those values.
...
in separately reviewed Haskell declarations. Reviewer inspection owns prose correspondence, and a prose decoy must be
...
authority/bypass, freshness, qualification, cleanroom, legacy closure, predecessor, residue, and promotion
authority.
...
two-way complete, stale evidence and self-reporting fail, and cleanup leaves no residue. An authorized reviewer
promotes status; the gate cannot promote itself.
...
1. a gate apparatus showing subject, independent oracle, changed-subject mutant, external observer,
   qualification refusal, candidate evidence, and delegated promotion; and
...
`legacy_tracking_for_deletion.md` file explains that active inventory to readers; reviewer correspondence inspection
