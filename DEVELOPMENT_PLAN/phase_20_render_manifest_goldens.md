# Phase 20: Pure `renderAll` + rendered-output goldens

> **Purpose**: Stand up the pure, total `renderAll :: ProvisionedSpec -> [K8sObject]`, mapping Phase 18's
...
- [Sprints](#sprints)
- [Sprint 20.1: The typed `K8sObject` model + Aeson serialization ✅](#sprint-201-the-typed-k8sobject-model--aeson-serialization-)
- [Sprint 20.2: Pure total `renderAll` + best-practice-by-construction ✅](#sprint-202-pure-total-renderall--best-practice-by-construction-)
- [Sprint 20.3: The rendered-output golden battery (`render-golden`) — the gate ✅](#sprint-203-the-rendered-output-golden-battery-render-golden--the-gate-)
...
`sha256:3a9622cca7322c7e…`; Phase 20 owns no remaining migration deferral.
...
`.build/runs/phase_14/<run-id>/`, and 30 run-time items — eighteen corpus deployments and twelve mutant names —
...
UNVERIFIED. The gap is recorded against Phase 20 in
...
This phase implements only the pure `renderAll` half; Phase 37 still owns live reconciliation and runtime
...
and never hides later-stage objects; Phase 37's typed diff/enactor filters actions by that sealed activation,
so managed-node taint/admission cannot be generically applied during the initial scheduler bootstrap.
The same serializer module is available only inside the amoebius package for Phase 36's
...
convergence — all deferred to [Phase 37](phase_37_object_reconciler.md); and
the `chain`/`[Step]` `--dry-run` plan render, which is [Phase 21](phase_21_chain_kernel_boundary.md). This phase
...
analogous to the Phase-12 decode battery and the Phase-11 `dhall type` corpus.
...
Phase-20 ledger for the exact tested and UNVERIFIED boundary.
...
flowchart LR
  ...
  g{{"the phase 20 gate command"}}:::gate
...
```
*Design intent. Phase 20's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
## Sprint 20.1: The typed `K8sObject` model + Aeson serialization ✅
...
**Docs to update**:
`documents/engineering/manifest_generation_doctrine.md` (Phase-20 backlink for the typed object model),
...
## Sprint 20.2: Pure total `renderAll` + best-practice-by-construction ✅
...
**Docs to update**: `documents/engineering/manifest_generation_doctrine.md` (backlink §3 to the Phase-20
...
- `renderAll :: ProvisionedSpec -> [K8sObject]`, pure and total (no I/O, no apiserver, no partial head),
  producing best-practice-by-construction objects. It maps Phase 18's sealed
...
- A closed `RenderReconcileMode` projection that includes only immutable schema/initial fields for the
  ...
  generic-SSA source path and are owned only by the corresponding typed actions in Phase 37.
...
- An in-file honesty note: this is the render half only — the SSA/ApplySet apply, prune, wait-for-ready, and
  release ledger are the live-band [Phase 37](phase_37_object_reconciler.md) reconciler, run by the
...
### Remaining Work
Done. SSA, ApplySet pruning, readiness, and live convergence remain Phase-37 work.

## Sprint 20.3: The rendered-output golden battery (`render-golden`) — the gate ✅
...
Phase-20 render-golden battery — the rendered-output enactment that gates its foreclosure),
`documents/engineering/generated_artifacts_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-20
...
### Remaining Work
Migrate the gate's generated output to `.build/runs/`, externally attest it, and rerun after Phase 19 closes.
Live enforcement remains UNVERIFIED.

## Documentation Requirements

**Engineering docs to update (when the gate runs, flip the honest layer, never before):**
- `documents/engineering/manifest_generation_doctrine.md` — backlink §2/§3 to the Phase-20 pure renderer and
  rendered-output goldens; keep §5's snapshot-bound typed action reconciler explicitly as the live-band
  [Phase 37](phase_37_object_reconciler.md) residue, run by the Deployment-`replicas=1` control-plane daemon under its
  mandatory Lease.
...
- `DEVELOPMENT_PLAN/substrates.md` — the Phase-20 `none` gate row.
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Manifest/{K8sObject,Types,Render}.hs` and
  the `render-golden` test-suite as Phase-20 design-first rows.
...
- [phase_17](phase_17_capability_bind.md) — the capability→provider→shape binder and provision fold producing
  the opaque whole-deployment `ProvisionedSpec` and its sealed identity-keyed render-source set
- [phase_21](phase_21_chain_kernel_boundary.md) — the `chain`/`[Step]` `--dry-run` plan render deferred from here - [phase_37](phase_37_object_reconciler.md) — the live action-driven reconciler that consumes
