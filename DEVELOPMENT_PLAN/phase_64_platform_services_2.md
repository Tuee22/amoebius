# Phase 64: Platform services-2 (Redis/Sentinel + Percona/Patroni + pgAdmin + observability + readiness-DAG)
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_61_retained_storage.md, DEVELOPMENT_PLAN/phase_63_platform_backbone.md, DEVELOPMENT_PLAN/phase_65_keycloak_ingress.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 64.1: Percona/Patroni Postgres per consumer + pgAdmin + Prometheus/Grafana ⏸️](#sprint-641-perconapatroni-postgres-per-consumer--pgadmin--prometheusgrafana-)
- [Sprint 64.2: Ephemeral Redis/Sentinel realtime coordination ⏸️](#sprint-642-ephemeral-redissentinel-realtime-coordination-)
- [Sprint 64.3: The full derived readiness-DAG bring-up + the standard-stack gate ⏸️](#sprint-643-the-full-derived-readiness-dag-bring-up--the-standard-stack-gate-)
- [Sprint 64.4: Register-2.5 readiness-DAG bring-up under simulated partial failure ⏸️](#sprint-644-register-25-readiness-dag-bring-up-under-simulated-partial-failure-)
...
This phase's target must complete the standard platform-service set on top of the future human-approved
Phase-63 backbone. It must render and reconcile the cluster-wide **Percona operator**, one **Patroni Postgres cluster per consuming capability**
...
`postgres` Pod), each rendered as typed Kubernetes objects by the Phase-59 `renderAll` path (no Helm and no
...
image** with no public-registry pull and each is placed on the Phase-61 `no-provisioner`
...
The phase then assembles the *whole* standard stack — the Phase-63 backbone plus these services — as one
...
order by the Phase-59 reconciler's **event-driven wait-for-ready**: a dependent starts on its dependency's
...
browser surface Grafana and pgAdmin reach a user through — is [Phase 65](phase_65_keycloak_ingress.md); this
...
[Phase 66](phase_66_live_dsl_deploy.md). Until then, a host-side operator drives this phase's fixed service
set.
...
**Depends on:** [Phase 63](phase_63_platform_backbone.md)
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 63; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
- [`ui_realtime_coordination_doctrine.md §5 — Redis is ephemeral platform-internal coordination`](../documents/engineering/ui_realtime_coordination_doctrine.md#5-redis-is-ephemeral-platform-internal-coordination):
  Phase 64's target must stand up the bounded TLS/ACL Redis/Sentinel topology with no persistence or application authority.
...
- [`platform_services_doctrine.md §8 — Postgres, Patroni-via-Percona, one cluster per consumer, with pgAdmin`](../documents/engineering/platform_services_doctrine.md#8-postgres--patroni-via-percona-one-cluster-per-consumer-with-pgadmin)
  ...
  Phase 64's target must stand up the cluster-wide Percona operator reconciling one Patroni Postgres cluster per consuming
...
- [`platform_services_doctrine.md §7 — Prometheus / Grafana, observability is not an add-on`](../documents/engineering/platform_services_doctrine.md#7-prometheus--grafana--observability-is-not-an-add-on)
  ...
  through the generated admission proxy; the browser surfaces gated behind the (Phase 65) Keycloak edge
...
- [`manifest_generation_doctrine.md` §5 — The apply/reconcile engine: snapshot-bound typed actions](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  Phase 64 reuses the Phase-59 pure `renderAll :: ProvisionedSpec -> [K8sObject]` and typed-action reconciler whose **wait-for-ready is observed from the live object, never a `threadDelay`** to apply and sequence the set.
- [`image_build_doctrine.md` §2 — The single distribution rule: bake the binaries, build the amoebius image, pull only in-cluster](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster):
  every service binary (Redis, Percona operator, Patroni, pgAdmin, Prometheus, Grafana) is baked into the Phase-57
...
- [`storage_lifecycle_doctrine.md §2 — one storage class, and it provisions nothing`](../documents/engineering/storage_lifecycle_doctrine.md#2-one-storage-class-and-it-provisions-nothing)
  ...
  each Patroni cluster and Prometheus lands its durable bytes on the Phase-61 `no-provisioner` retained PVs.
...
## Sprint 64.1: Percona/Patroni Postgres per consumer + pgAdmin + Prometheus/Grafana ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 63](phase_63_platform_backbone.md) human approval
...
- The Percona operator rendered as a cluster-wide platform component (from the shared inventory so it installs
  ...
  Distribution `registry:2` takes **no** database ([§3](../documents/engineering/platform_services_doctrine.md#3-the-registry--the-single-image-source)). The consumer set is pinned here (resolving the `platform_services_doctrine.md §8` "Phase 63 delivery detail"); a `PerconaPGCluster` consumed by nothing does
...
- Prometheus + Grafana scraping platform workloads, with the per-workflow recording/alert rules and
  ...
  - The browser surfaces reach a user only through the (Phase 65) Keycloak edge under a mandatory
...
1. Assert the Percona operator is Ready before any `PerconaPGCluster`, then that the named consumer set
   ...
   `{Grafana}` because Keycloak's own store arrives with the [Phase 65](phase_65_keycloak_ingress.md) edge and
...
expectations. The Keycloak browser edge remains Phase 65 and control-plane-owned continuous
reconciliation remains Phase 66, both explicitly UNVERIFIED.

## Sprint 64.2: Ephemeral Redis/Sentinel realtime coordination ⏸️
...
## Sprint 64.3: The full derived readiness-DAG bring-up + the standard-stack gate ⏸️
...
- A `BringUp` assembly that folds the whole standard-service set (Phase-63 backbone + Sprint-58.1 and Sprint-58.2 services)
...
- Live enactment by the Phase-59 reconciler's wait-for-ready — each dependent constructed to start on its
  dependency's observed-ready edge (a `runtime-checked` observation from the live object), never a duration;
  the control-plane daemon that will later own this loop is [Phase 66](phase_66_live_dsl_deploy.md) (Deployment `replicas=1`, no election).
- The phase-gate harness brings the standard stack onto Phase 63's live backbone and observes every required
...
2. Round-trip MinIO put/get and Pulsar produce/consume against the assembled stack; assert Postgres is a
   Patroni cluster, never a bare Pod, carrying the mandated synchronous config (the Sprint 64.1 Haskell expectation).
3. Assert the complete standard stack is ready and preserves Phase 63's already-gated backbone topology and
   provenance. For the Phase-64 additions, a one-versus-many replica render diff may change only count fields;
   ...
   pull log and require every live `imageID` to equal the digest in the verified Phase-57 attestation and the
...
verified Phase-57 identity directly as authenticated run input and rerun the warm reconciliation under
universal artifact hygiene. The deterministic scheduler gate owns the cold/partial-
failure ordering claim.

## Sprint 64.4: Register-2.5 readiness-DAG bring-up under simulated partial failure ⏸️
...
3. Replay a captured seed and assert a bit-identical schedule and outcome; emit the Register-2.5 ledger — substrate `none`, Register 2.5 — recording the honest limit that modeled-substrate fidelity is *assumed* and is discharged only by this phase's Register-3 live gate (Sprint 64.3).
...
- `platform_services_doctrine.md`, `chaos_failover_doctrine.md`, `readiness_ordering_doctrine.md`,
  ...
  `deterministic_simulation_doctrine.md` now record the tested Phase-64 boundary and its honest limits.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the Phase-59 renderer + SSA wait-for-ready that applies and sequences the set
...
- [UI Realtime Coordination](../documents/engineering/ui_realtime_coordination_doctrine.md) — the ephemeral
  Redis/Sentinel topology and failure boundary delivered by Sprint 64.2
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the Register-2.5 `IOSim`/`IOSimPOR` simulation of the real bring-up over the Phase-35.4 modeled substrates
- [phase_62](phase_62_vault_pki.md) — the root Vault/PKI whose unseal edge gates secret-dependent startup here
- [phase_63](phase_63_platform_backbone.md) — the MetalLB/MinIO/Pulsar backbone this phase's services and DAG build on
- [phase_65](phase_65_keycloak_ingress.md) — the Keycloak-owned ingress edge that fronts Grafana and pgAdmin next
