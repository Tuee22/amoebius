# Phase 30: Execution-epoch + scheduler + accelerator + provider-root folds
...
> Phase 9 base capacity fold and the Phase 29 storage geometry into the full-resource-vector `place` witness that
...
- [Sprints](#sprints)
- [Sprint 30.1: Execution-epoch expansion + scheduler-reservation algebra ⏸️](#sprint-301-execution-epoch-expansion--scheduler-reservation-algebra-)
- [Sprint 30.2: kubelet/CRI runtime-metadata + node-local OCI content/snapshot/image + physical-disk parent accounting ⏸️](#sprint-302-kubeletcri-runtime-metadata--node-local-oci-contentsnapshotimage--physical-disk-parent-accounting-)
- [Sprint 30.3: Accelerator residency/net-allocatable-VRAM + provider-root disk template + engine/build/etcd/monitoring compute ⏸️](#sprint-303-accelerator-residencynet-allocatable-vram--provider-root-disk-template--enginebuildetcdmonitoring-compute-)
- [Sprint 30.4: The composed full-resource-vector place-witness — properties + independent validator + per-axis mutants ⏸️](#sprint-304-the-composed-full-resource-vector-place-witness--properties--independent-validator--per-axis-mutants-)
- [Sprint 30.5: The execution/accelerator/provider-root fold-negative corpus + the composed gate ⏸️](#sprint-305-the-executionacceleratorprovider-root-fold-negative-corpus--the-composed-gate-)
...
axes with the Phase-9 base fold and the Phase-29 storage geometry into the single whole-deployment place-witness.
Haskell values own every case and expectation; any serialized case or mutation is generated lazily beneath
`.build/**`.
Phase 32's post-bind provision seal invokes the same folds only after full bind/expansion; Phase 30 does not move
...
durable/cache demand (Phase 29), accelerator devices plus every identity-complete owner residency epoch, and every
...
([phase_29_storage_geometry_folds.md](phase_29_storage_geometry_folds.md)); the capability→provider→shape
binder ([phase_31_capability_bind.md](phase_31_capability_bind.md)) and the whole-deployment provision seal
([phase_32_provision_seal.md](phase_32_provision_seal.md)) that re-exercise these folds post-bind; the
`InferenceEngine` capability + accelerator coexistence provision
([phase_33_inference_accelerator_provision.md](phase_33_inference_accelerator_provision.md)); and the live
residue — the same-binary scheduler role's Reserved→BindingInFlight→Bound around real Kubernetes Binding
([phase_60_capacity_scheduler.md](phase_60_capacity_scheduler.md)), the snapshot-bound live preflight and
action/token/CAS plumbing ([phase_59_object_reconciler.md](phase_59_object_reconciler.md)), retained-carve
allocation ([phase_61_retained_storage.md](phase_61_retained_storage.md)), provider-capacity creation
([phase_77_provider_deploy_checkpoint.md](phase_77_provider_deploy_checkpoint.md), [phase_80_provider_dynamic_nodes.md](phase_80_provider_dynamic_nodes.md)), and live CUDA/Metal enaction
([phase_94_jitml_rederivation.md](phase_94_jitml_rederivation.md), [phase_90_apple_metal_host_daemon.md](phase_90_apple_metal_host_daemon.md)). Phase 30 owns the pure
...
**Depends on:** [Phase 29](phase_29_storage_geometry_folds.md)
...
| Key | Contract |
|---|---|
| `Claim` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Target only — the pure Haskell full-resource-vector fold returns a placement witness or one structured refusing axis. It consumes Haskell values; any serialized case or mutation is generated beneath `.build/**`; and it makes no hardware or live-capacity claim. Explicit exclusions: every layer named in `Residue` remains UNVERIFIED. |
| `Subject` | UNRESOLVED — blocks validation: no production `.hs` module and entry point have been independently established for this reset contract. |
| `Command` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: `pb validate phase 30` is future public spelling only. Before current human approval of Phase 51, `pb` is inadmissible validation transport; the candidate must invoke the exact absolute source-bound Haskell executable directly from an authenticated, network-independent toolchain input. The Haskell verdict entry point remains `UNRESOLVED` and blocks validation. |
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 29; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
## Sprint 30.1: Execution-epoch expansion + scheduler-reservation algebra ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: exact authored Haskell implementation paths have not been bound to this sprint.
**Blocked by**: [Phase 29](phase_29_storage_geometry_folds.md) human approval
...
numbers only — the pure expansion fold Phase 32's `provision` seal later invokes.
...
- The pure scheduler model seals prior+desired, controller-child-indexed candidate templates and one aggregate
  ...
  [phase_60_capacity_scheduler.md](phase_60_capacity_scheduler.md)) performs Reserved→BindingInFlight→Bound
...
## Sprint 30.2: kubelet/CRI runtime-metadata + node-local OCI content/snapshot/image + physical-disk parent accounting ⏸️
...
`PhysicalDiskPartition` parent equations — all as pure, checked `provision-seal` operations invoked by Phase 32's
`provision` seal.
...
## Sprint 30.3: Accelerator residency/net-allocatable-VRAM + provider-root disk template + engine/build/etcd/monitoring compute ⏸️
...
- `MonitoringWorkBudget` with finite workflow/rule/series/sample-rate/interval/CPU/memory/retention, structural
  ...
  concurrent executor/workspace peak as an executor envelope; its checkpoint storage remains the Phase-29
  object-store arm.
...
## Sprint 30.4: The composed full-resource-vector place-witness — properties + independent validator + per-axis mutants ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: exact authored Haskell implementation paths have not been bound to this sprint.
**Blocked by**: Sprint 30.3
**Independent Validation**: UNRESOLVED — blocks validation: independent positive, paired-negative, changed-subject mutant, and residue observations have not been bound to this sprint.
**Oracle**: UNRESOLVED — blocks validation: a separately authored Haskell oracle, its provenance, and its human reviewer have not been bound to this sprint.
**Legacy IDs**: UNRESOLVED — blocks validation: this sprint has not been joined to exact typed Haskell legacy-inventory IDs.
**Docs to update**: UNRESOLVED — blocks validation: the governed documentation owners and exact update set have not been reviewed for this sprint.
...
- The per-axis/per-capability seeded-mutant battery: **one seeded mutant per resource/capability axis** — drop
  ...
  individually required to turn the suite red (§M.2), plus the host-only compute mutants of Sprint 30.3.
...
## Sprint 30.5: The execution/accelerator/provider-root fold-negative corpus + the composed gate ⏸️
...
- The fold-negative fixtures on this seam — `illegal_hard_ceiling_overcommit` ([§3.17](../documents/illegal_state/illegal_state_capacity.md#317-an-over-committed-deploy-or-workload-host--vm--cluster-capacity-exceeded)),
  ...
  - Compile-time-foreclosed neighbours
    ...
    [phase_29_storage_geometry_folds.md](phase_29_storage_geometry_folds.md).
...
- `documents/engineering/daemon_topology_doctrine.md` — reconcile §3's control-plane daemon reservation /
  ...
  [phase_60_capacity_scheduler.md](phase_60_capacity_scheduler.md).
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Capacity/{Scheduler,HostReservation,
  ...
  branches of `Types.hs`/`Fold.hs`, and the composed placement property + gate suites as Phase-30 design-first
  rows.
- `DEVELOPMENT_PLAN/phase_09_resource_index.md` / `DEVELOPMENT_PLAN/phase_29_storage_geometry_folds.md` —
  the base-fold and storage-geometry slices this phase composes into the full-resource-vector witness.
- `DEVELOPMENT_PLAN/phase_32_provision_seal.md` — the post-bind provision seal that re-exercises these folds.
...
- [phase_27](phase_27_gadt_decode_ir.md) — gadt-decode, the IR + decoder these folds decode into
- [phase_28](phase_28_illegal_state_covering.md) — the illegal-state corpus, properties, and validation-locus
...
- [phase_29_storage_geometry_folds.md](phase_29_storage_geometry_folds.md) — the logical→physical storage
  geometry this phase composes into the full-resource vector
- [phase_31_capability_bind.md](phase_31_capability_bind.md) — the capability → provider → shape binder built
  atop these folds
- [phase_32_provision_seal.md](phase_32_provision_seal.md) — the whole-deployment provision seal that re-exercises
  these folds post-bind
- [phase_33_inference_accelerator_provision.md](phase_33_inference_accelerator_provision.md) — the
  `InferenceEngine` capability + accelerator residency/coexistence provision built atop the accelerator fold
- [phase_60_capacity_scheduler.md](phase_60_capacity_scheduler.md) — the live same-binary scheduler role that
  enacts Reserved→BindingInFlight→Bound around Kubernetes Binding
- Phase 30 execution/accelerator ledger — the human-readable
