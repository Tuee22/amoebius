# Phase 81: Determinism kernel + jit-build CacheBudget cache
...
- [Sprints](#sprints)
- [Sprint 81.1: `ContentAddress` typeclass kernel primitive ⏸️](#sprint-811-contentaddress-typeclass-kernel-primitive-)
- [Sprint 81.2: `experimentHash` identity over the live substrate fingerprint ⏸️](#sprint-812-experimenthash-identity-over-the-live-substrate-fingerprint-)
- [Sprint 81.3: SplitMix seed derivation, worker-count-independent ⏸️](#sprint-813-splitmix-seed-derivation-worker-count-independent-)
- [Sprint 81.4: The live same-substrate reproducibility gate ⏸️](#sprint-814-the-live-same-substrate-reproducibility-gate-)
- [Sprint 81.5: The `CacheBudget`-bounded content-addressed cache + peak-occupancy provision fold ⏸️](#sprint-815-the-cachebudget-bounded-content-addressed-cache--peak-occupancy-provision-fold-)
- [Sprint 81.6: The jit-build resolver — `resolve = {download | build}` on first miss, no URL arm ⏸️](#sprint-816-the-jit-build-resolver--resolve--download--build-on-first-miss-no-url-arm-)
- [Sprint 81.7: Per-node cache-owner reuse across client pods ⏸️](#sprint-817-per-node-cache-owner-reuse-across-client-pods-)
- [Sprint 81.8: The live first-miss / reuse / resource-admission gate + Register-3 ledger ⏸️](#sprint-818-the-live-first-miss--reuse--resource-admission-gate--register-3-ledger-)
...
This phase must extend the future human-approved Phase-70 content-addressed store into one cohesive
...
**The determinism kernel.** First, it lifts Phase 70's concrete blob/manifest key renderers into a kernel-level
...
a prebuilt engine or builds it from source (using the Phase-57 baked toolchain) into the cache; there is no arm
...
a request-carried derived seed — deliberately **not** an infernix inference run (that lift is Phase 92) and
**not** a jit-resolved ML engine (this phase resolves the engine, but the infernix inference that rides it is
Phase 92; the CUDA/jitML kernel tier is Phase 94). The `ModelArtifact` staging tier (Tier 2) and the JIT kernel
...
%% register: algebra
  ...
  ca[["ContentAddress typeclass"]]:::intent --> store[/"Phase 70 content-addressed store"/]:::effect
  eh --> store
...
**Depends on:** [Phase 80](phase_80_provider_dynamic_nodes.md)
...
| Key | Contract |
|---|---|
| `Claim` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: the reproducible resolved-engine lifecycle on one linux-cpu cluster, ending in one Register-3 gate; split if work adds another asset tier, substrate, final register, or independently useful live capability. Explicit exclusions: every layer named in `Residue` remains UNVERIFIED. |
| `Subject` | UNRESOLVED — blocks validation: no production `.hs` module and entry point have been independently established for this reset contract. |
| `Command` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: `pb validate phase 81` is the target command only; `pb` may only make the minimal platform distinction, establish the contained toolchain, build the source-bound binary, and exec it with argv unchanged, while the Haskell verdict entry point remains UNRESOLVED and blocks validation. |
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 80; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
backoff, replacement-on-Failed, terminal retention) and is also an exact Phase-70 `WorkflowRuntimeDemand`
...
they omit one run's Pod row, the probe/harness host row, the predecessor-absence wait, or one Phase-70
...
- [`content_addressing_doctrine.md` §2 — The three-tier store: blobs ← manifests ← pointers](../documents/engineering/content_addressing_doctrine.md#2-the-three-tier-store-blobs--manifests--pointers)
  — *the three-tier store (blobs ← manifests ← pointers)*: the `ContentAddress` typeclass lifts Phase 70's
...
- [`resource_capacity_doctrine.md` §3 — The types: `Quantity`, `Capacity`, `Demand`, `Budget`](../documents/engineering/resource_capacity_doctrine.md#3-the-types-quantity-capacity-demand-budget)
  ...
  establish and Phase 32 must invoke at `provision-seal` — "more cached than fits" is rejected by that fold, not discovered as a
  runtime disk-full.
- [`image_build_doctrine.md` §7 — What amoebius bakes vs builds — the base container is the supply chain](../documents/engineering/image_build_doctrine.md#7-what-amoebius-bakes-vs-builds--the-base-container-is-the-supply-chain)
  ...
  identities — the Phase-57 split this phase exercises live for the first time.
...
## Sprint 81.1: `ContentAddress` typeclass kernel primitive ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 80](phase_80_provider_dynamic_nodes.md) human approval
...
lift Phase 70's concrete blob/manifest key renderers into a kernel-level `ContentAddress` typeclass so that
"a content-derived name cannot be forged" is a single reusable primitive shared later by the engine cache
(Sprint 81.5) and by both infernix (Phase 92) and jitML (Phase 94), not a per-store copy.
...
- Adapters binding the typeclass to Phase 70's `blobs/<sha256>` and `manifests/<sha256>` writers — the
  `If-None-Match: *`, `412 = success` protocol stays owned by the store.
- The reviewed Haskell compile-fail source `test/negative/compile_fail/phase_71_forge_blobsha.hs` (with its
...
1. Type-level, verified by the reviewed Haskell compile-fail source `test/negative/compile_fail/phase_71_forge_blobsha.hs`
...
## Sprint 81.2: `experimentHash` identity over the live substrate fingerprint ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 81.1
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
consuming the Phase-26 normal form and the Phase-56 full-path substrate probe, per the substrate doctrine's
...
2. The fingerprint carries every witness required by the independent Haskell fingerprint oracle
   ...
   `.build/test-tools/phase_71_fake_ghc` (which emits a different version),
...
## Sprint 81.3: SplitMix seed derivation, worker-count-independent ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 81.2
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
## Sprint 81.4: The live same-substrate reproducibility gate ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 81.3
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
without an infernix inference run (Phase 92) or a jit-resolved engine (that is the cache seam below and the
Phase-92 lift), and prove same-substrate reproducibility as one half of the phase gate without overclaiming
...
- The reviewed Haskell gate declaration, lazily projected to
  `.build/test-corpora/dhall/phase_71_determinism_repro.dhall`, that spins up the Phase-70 workflow, runs the
...
3. Changing the resolved Haskell DSL value (the flipped-metric sibling, differing only in metric direction) or
   ...
   built lazily at `.build/test-tools/phase_71_fake_ghc`,
...
## Sprint 81.5: The `CacheBudget`-bounded content-addressed cache + peak-occupancy provision fold ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 81.4
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
1. There is no exported path to a cache key from a free string; the only path to a resident entry is content
   ...
   (registered in the Phase-28 negative corpus, authored in this phase's oracle-pinning sprint) failing to
...
4. **Pin-aware pruning is exercised, not declared:** a cache filled to `CacheBudget` with a mix of pinned and
   ...
   ([Gate integrity](#gate-integrity) part (b) mutant (b)) must turn this clause red (the over-budget residency survives). This is the pure-pool property; its live on-disk counterpart is the Sprint 81.8 postflight residency
...
## Sprint 81.6: The jit-build resolver — `resolve = {download | build}` on first miss, no URL arm ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 81.5
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
miss into the bounded cache — downloaded prebuilt or built from source with the Phase-57 baked toolchain — with
...
- The build-from-source path invoking the Phase-57 baked toolchain by absolute path (no `PATH`, no env), and the
...
- An in-file honesty note: URL-foreclosure and identity-from-closed-catalog are **proven-in-types** by the Haskell
  ...
  model (Tier 2) and kernel (Tier 3) tiers reuse this resolver but land in Phases 61/62.
...
1. The suite drives the resolver against a Haskell-declared backend case whose served or compiled bytes
   **sha256-match the independently authored Haskell content pin**. Its reproducible Dhall view is generated
   ...
   (Phase-28 corpus, independently authored) failing at the constructor locus with "no `Url`/free-string
...
Production model inference through the resolved engine is Phase 92, not a hidden acceptance claim here.

## Sprint 81.7: Per-node cache-owner reuse across client pods ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 81.6
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
- The pod wiring (rendered by the Phase-59 reconciler) that gives the owner a bounded disk-backed `emptyDir` with
...
## Sprint 81.8: The live first-miss / reuse / resource-admission gate + Register-3 ledger ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 81.7
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
- A Register-3 ledger recording: URL-foreclosure and the required bounded-`CacheBudget` field as
  ...
  **deferred** (Phases 61/72), with cross-node and cross-substrate reuse explicitly not asserted.
...
2. With the cache filled to budget, a postflight on-disk peak/final measurement confirms pin-aware eviction and
   ...
   ephemeral-under-reserved owner each return their **tagged** `Left` at the Phase-9 fold at the Phase-32
...
3. The Register-3 ledger is emitted and marks first-miss resolution, cross-pod reuse, and pin-aware eviction as
   *tested on linux-cpu*, URL-foreclosure and the required bounded-`CacheBudget` field as *proven-in-types*, the
   numeric cache/ephemeral inequalities as *provision-seal checked*, and the model/kernel tiers (Phases 61/72) and
...
- `documents/engineering/content_addressing_doctrine.md` — the §6 proven/tested/assumed table gains an
  ...
  Tier-2/Tier-3 realizations remain Phases 61/72 targets, and add the kernel module paths
...
- `documents/engineering/resource_capacity_doctrine.md` — record that the §3/§4 `Quantity`/`fits` fold is reused
  ...
  instantiated by the future live recompute runs and the cache owner/clients (the future Phase-81 gate must use
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-81 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 81's gate substrate (linux-cpu) in the per-phase substrate map.
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Kernel/ContentAddress.hs`,
  ...
  `test/negative/determinism_jitcache/url_arm.hs` as Phase-81 design-first rows. Lazily generated serialized
...
- [phase_32](phase_32_provision_seal.md) — the whole-deployment provision seal where the over-budget cache peak is rejected
- [phase_33](phase_33_inference_accelerator_provision.md) — the `InferenceEngine` binder + closed `EngineRuntime` union the resolver keys on
- [phase_35](phase_35_chain_kernel_boundary.md) — the `chain`/`Step` kernel the kernel primitives plug into
- [phase_56](phase_56_bootstrap_coordinator_kind.md) — the bootstrap coordinator + single-node `kind` cluster + substrate fingerprint this phase runs on
- [phase_57](phase_57_base_image_registry.md) — the base image that bakes the jit-build resolver + toolchain this phase drives live
- [phase_59](phase_59_object_reconciler.md) — the typed SSA object reconciler that renders the cache owner and client pods
- [phase_63](phase_63_platform_backbone.md) — the platform backbone the cache owner/client pods schedule onto
- [phase_70](phase_70_content_store_workflow.md) — the content store + workflow runtime this phase lifts and runs on
- [phase_92](phase_92_infernix_rederivation.md) — the infernix CPU-inference lift whose reproducibility reuses this kernel and rides this resolver next
- [phase_94](phase_94_jitml_rederivation.md) — the jitML/CUDA lift whose kernel tier reuses this resolver and kernel
