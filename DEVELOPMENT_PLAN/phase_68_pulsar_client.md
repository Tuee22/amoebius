# Phase 68: Native Pulsar client (CBOR)
...
- [Sprints](#sprints)
- [Sprint 68.1: Fork supernova → amoebius-pulsar native binary protocol ⏸️](#sprint-681-fork-supernova--amoebius-pulsar-native-binary-protocol-)
- [Sprint 68.2: Capability surface + exclusively-CBOR payload codec ⏸️](#sprint-682-capability-surface--exclusively-cbor-payload-codec-)
- [Sprint 68.3: Declarative topology algebra + one-sided-link validation ⏸️](#sprint-683-declarative-topology-algebra--one-sided-link-validation-)
- [Sprint 68.4: At-least-once + broker-side dedup + the command→event round-trip gate ⏸️](#sprint-684-at-least-once--broker-side-dedup--the-commandevent-round-trip-gate-)
- [Sprint 68.5: Register-2.5 exactly-once effect under simulated redelivery ⏸️](#sprint-685-register-25-exactly-once-effect-under-simulated-redelivery-)
...
orchestrator/worker workflow runtime with Pulsar-Failover single-writer takeover (both Phase 70), the topic
...
**Depends on:** [Phase 67](phase_67_app_tenancy.md)
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 67; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
orchestrator/worker. The Phase-68 live gate therefore declares a client-runner Pod with immutable image and OCI
...
bytes, concurrent/rate-window offloads, deletion lag, failed/orphan horizon, mutation admission). The Phase-63
...
## Sprint 68.1: Fork supernova → amoebius-pulsar native binary protocol ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 67](phase_67_app_tenancy.md) human approval
...
- A structural `PulsarClientExecutionDemand` for connection/request/frame/reconnect peaks, bound into the
  complete envelope of the Phase-68 client-runner Pod and, later, each actual consumer Pod; there is no
...
## Sprint 68.2: Capability surface + exclusively-CBOR payload codec ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 68.1
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
seek-based replay — over the persistent session from Sprint 68.1, with **every application payload encoded exclusively as CBOR** through a typed codec.
...
- A typed **CBOR payload codec** (`Amoebius.Pulsar.Cbor`, on `serialise`/`cborg`): `produce`/`consume` accept
  ...
  CBOR (shared with the store's canonical encoder, Phase 70) is used where the payload is content-addressed; a
...
## Sprint 68.3: Declarative topology algebra + one-sided-link validation ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 68.2
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
4. Prove the algebra is on the gate path, not dead code (§M.3): a Haskell-declared gate topology, rendered
   lazily beneath `.build/test-corpora/pulsar-client/**`, carries a `RouteEntry` descriptor. The Sprint 68.4
...
## Sprint 68.4: At-least-once + broker-side dedup + the command→event round-trip gate ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 68.3
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
3. **Idempotency (§M.6) — re-apply, not re-run-from-clean.** Re-apply the topology a second time against a
   **distinct test namespace** (cache-bypass: no reuse of run 1's namespace, cursors, producer name, or dedup
   ...
   tenant contains **zero** topics, subscriptions/cursors, and namespaces, and asserts the standing Phase-63
   ...
   ledger — the deferred content-store, workflow-runtime, and cross-cluster surfaces (Phase 70 and later)
...
## Sprint 68.5: Register-2.5 exactly-once effect under simulated redelivery ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 68.4
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
- A Register-2.5 proven/tested/assumed ledger — the fold upholds exactly-once under the modeled schedules and
  faults; honest limit: modeled-Pulsar fidelity is **assumed**, discharged by the Sprint 68.4 Register-3 live
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-pulsar` package and its target module paths
  ...
  `Topology`, `Dedup`, `Namespace`) as Phase-68 design-first rows against the component inventory.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 68's gate substrate (`linux-cpu`) in the per-phase substrate
  map, and the topology algebra's per-substrate topic lanes.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-68 row's status from this plan once the gate passes; link this
  document.
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the Register-2.5 io-sim environment the dedup fold's exactly-once is validated against in Sprint 68.5
...
- [Daemon Topology Doctrine](../documents/engineering/daemon_topology_doctrine.md) — who runs producers/
  consumers and picks subscription roles (Phase 70), delegated here
- [phase_63](phase_63_platform_backbone.md) — the standard-service stack that brings Pulsar up HA
- [phase_67](phase_67_app_tenancy.md) — the app tenancy this phase opens after
- [phase_70](phase_70_content_store_workflow.md) — the content store + workflow runtime that consumes this
  client
