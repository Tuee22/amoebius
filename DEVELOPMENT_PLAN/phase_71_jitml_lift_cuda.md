# Phase 71: Core jitML CUDA artifact lift
...
Phase 71 delivers the core jitML CUDA artifact lift; its design is owned by [lift_and_compose_doctrine.md](../documents/engineering/lift_and_compose_doctrine.md), [capability_extension_doctrine.md](../documents/engineering/capability_extension_doctrine.md), [content_addressing_doctrine.md](../documents/engineering/content_addressing_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 71.1: Produce one committed jitML artifact on CUDA ⏸️](#sprint-711-produce-one-committed-jitml-artifact-on-cuda-)
...
⏸️ Blocked pending Phase-70 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
Phase 48 continues to own Pulsar-Failover takeover and pointer-CAS safety; Phase 59 continues to own the
determinism kernel and bounded first-miss cache. Phase 71 consumes those contracts without adding another
coordinator, election, store, cache, or determinism implementation.

Every scoped training start carries one server-derived `CommandId` unchanged as the Phase-48 workflow
...
revision. Producer resends retain their Phase-46 producer/sequence identity; consumer redelivery and restart
...
The checkpoint adapter uses Phase 48 content digests and canonical component manifests for its pure contract.
...
The output of this phase is the core committed artifact contract. Phase 72 alone owns its downstream
...
live evidence reader, and one aggregate command; no Phase-72 presentation work is included.
...
- [Content Addressing Doctrine §2 — Three-tier store](../documents/engineering/content_addressing_doctrine.md#2-the-three-tier-store-blobs--manifests--pointers)
  and [§4.5 — ML-asset lifecycle](../documents/engineering/content_addressing_determinism.md#45-the-ml-asset-lifecycle-one-bounded-content-addressed-cache-resolved-on-first-miss):
...
## Sprint 71.1: Produce one committed jitML artifact on CUDA ⏸️
...
this phase renders like every other operator install.
...
- The canonical checkpoint adapter and opaque `CommittedJitMLArtifact tenant app`, constructible only from a
  successful Phase-48 pointer-CAS witness.
...
- Record only the tested jitML adapter, CUDA owner binding, and pointer-commit contract in the owning
  lift/capability/content/daemon/capacity doctrines; do not restate inherited Phase-48 or Phase-59 proofs.
- Keep downstream artifact presentation and interaction ownership exclusively in Phase 71.
...
- Keep the Phase-72 dependency on this committed-artifact contract explicit without moving presentation work
  into Phase 61.

## Related Documents

- [Development-plan standards](development_plan_standards.md)
- [Phase 48 — Content store and workflow runtime](phase_48_content_store_workflow.md)
- [Phase 59 — Determinism and jit cache](phase_59_determinism_jitcache.md)
- [Phase 60 — infernix lift](phase_60_infernix_lift.md)
- [Phase 72 — jitML UI lift](phase_72_jitml_ui_lift.md)
