# Phase 53: Determinism kernel + jit-build CacheBudget cache
...
- [Sprints](#sprints)
- [Sprint 53.1: `ContentAddress` typeclass kernel primitive ⏸️](#sprint-531-contentaddress-typeclass-kernel-primitive-)
- [Sprint 53.2: `experimentHash` identity over the live substrate fingerprint ⏸️](#sprint-532-experimenthash-identity-over-the-live-substrate-fingerprint-)
- [Sprint 53.3: SplitMix seed derivation, worker-count-independent ⏸️](#sprint-533-splitmix-seed-derivation-worker-count-independent-)
- [Sprint 53.4: The live same-substrate reproducibility gate ⏸️](#sprint-534-the-live-same-substrate-reproducibility-gate-)
- [Sprint 53.5: The `CacheBudget`-bounded content-addressed cache + peak-occupancy provision fold ⏸️](#sprint-535-the-cachebudget-bounded-content-addressed-cache--peak-occupancy-provision-fold-)
- [Sprint 53.6: The jit-build resolver — `resolve = {download | build}` on first miss, no URL arm ⏸️](#sprint-536-the-jit-build-resolver--resolve--download--build-on-first-miss-no-url-arm-)
- [Sprint 53.7: Per-node cache-owner reuse across client pods ⏸️](#sprint-537-per-node-cache-owner-reuse-across-client-pods-)
- [Sprint 53.8: The live first-miss / reuse / resource-admission gate + Register-3 ledger ⏸️](#sprint-538-the-live-first-miss--reuse--resource-admission-gate--register-3-ledger-)
...
⏸️ Blocked pending Phase-52 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
brought up by the Phase 29 bootstrap coordinator, whose base image (Phase 30) already bakes the shared **jit-build resolver and its build toolchain** but **no** ML engine payload, using the content-addressed store and workflow runtime
landed in Phase 41. The `experimentHash`/SplitMix shapes are already exercised in the sibling `jitML` project
...
This phase turns the content-addressed store delivered in Phase 42 into one cohesive **reproducible resolved- asset lifecycle**. Its ordered sprint seams first establish the shared determinism identity and then use that
...
**The determinism kernel.** First, it lifts Phase 42's concrete blob/manifest key renderers into a kernel-level
...
a prebuilt engine or builds it from source (using the Phase-30 baked toolchain) into the cache; there is no arm
...
"More cached than fits" is rejected at the post-bind `provision-seal` by the Phase-8 capacity fold, not
...
a request-carried derived seed — deliberately **not** an infernix inference run (that lift is Phase 54) and
...
%% register: algebra
  ...
  ca[["ContentAddress typeclass"]]:::intent --> store[/"Phase 42 content-addressed store"/]:::effect
  eh --> store
...
**The two-part acceptance condition.** The run happens on the single-node linux-cpu `kind` cluster — Phase 29
bootstrap coordinator, Phase 30 base image plus baked resolver/toolchain plus in-cluster `distribution` registry,
Phase 42 content store plus workflow runtime. Every *how-it-behaved* claim is read from an OS-boundary observer
...
observation, or bounded-parallel-derived overflow is rejected by the Phase-8 fold at the Phase-12
**`provision-seal`** before any resolve. The run emits a Register-3 proven/tested/assumed ledger.

```mermaid
flowchart LR
  %% register: orientation
  s0["Sprint 53.1: ContentAddress typeclass kernel primitive"]
  s1["Sprint 53.2: experimentHash identity over the live substrate fingerprint"]
  s2["Sprint 53.3: SplitMix seed derivation, worker-count-independent"]
  s3["Sprint 53.4: The live same-substrate reproducibility gate"]
  s4["Sprint 53.5: The CacheBudget-bounded content-addressed cache…"]
  s5["Sprint 53.6: The jit-build resolver — resolve = {download | build} on…"]
  s6["Sprint 53.7: Per-node cache-owner reuse across client pods"]
  s7["Sprint 53.8: The live first-miss / reuse / resource-admission gate…"]
  gate["the phase 53 gate"]
...
backoff, replacement-on-Failed, terminal retention) and is also an exact Phase-42 `WorkflowRuntimeDemand`
...
- [`content_addressing_doctrine.md §2`](../documents/engineering/content_addressing_doctrine.md#2-the-three-tier-store-blobs--manifests--pointers)
  — *the three-tier store (blobs ← manifests ← pointers)*: the `ContentAddress` typeclass lifts Phase 42's
...
- [`resource_capacity_doctrine.md §3`](../documents/engineering/resource_capacity_doctrine.md#3-the-types-quantity-capacity-demand-budget)
  ...
  ephemeral-storage envelope, and the derived peak bound is the **same** checked capacity fold Phase 8 built and
  Phase 12 invokes at `provision-seal` — "more cached than fits" is rejected by that fold, not discovered as a
  runtime disk-full.
- [`image_build_doctrine.md §7`](../documents/engineering/image_build_doctrine.md#7-what-amoebius-bakes-vs-builds--the-base-container-is-the-supply-chain)
  ...
  identities — the Phase-30 split this phase exercises live for the first time.
...
- [`testing_doctrine.md §2`](../documents/engineering/testing_doctrine.md#2-the-registers-of-amoebius-testing)
  ...
  emits a proven/tested/assumed ledger naming that register, with the model/kernel tiers (Phases 54/65) marked
  deferred.
...
## Sprint 53.1: `ContentAddress` typeclass kernel primitive ⏸️
...
lift Phase 42's concrete blob/manifest key renderers into a kernel-level `ContentAddress` typeclass so that
"a content-derived name cannot be forged" is a single reusable primitive shared later by the engine cache
(Sprint 53.5) and by both infernix (Phase 54) and jitML (Phase 65), not a per-store copy.
...
## Sprint 53.2: `experimentHash` identity over the live substrate fingerprint ⏸️
...
consuming the Phase-5 normal form and the Phase-29 full-path substrate probe, per the substrate doctrine's
...
## Sprint 53.3: SplitMix seed derivation, worker-count-independent ⏸️
...
([§4.3](../documents/illegal_state/illegal_state_techniques.md#43-gadt-indexed-state-machines--only-legal-transitions-are-typed)) and its totality argument in [`§4.4`](../documents/engineering/content_addressing_doctrine.md#44-what-the-types-make-these-total-cashes-out-to):
...
## Sprint 53.4: The live same-substrate reproducibility gate ⏸️
...
without an infernix inference run (Phase 54) or a jit-resolved engine (that is the cache seam below and the
Phase-54 lift), and prove same-substrate reproducibility as one half of the phase gate without overclaiming
...
## Sprint 53.5: The `CacheBudget`-bounded content-addressed cache + peak-occupancy provision fold ⏸️
...
### Deliverables
- `Amoebius.Jit.Cache` — a bounded typed pool keyed by `sha256(resolved-bytes)` (the Sprint-48.1 `ContentAddress`),
...
4. **Pin-aware pruning is exercised, not declared:** a cache filled to `CacheBudget` with a mix of pinned and
   ...
   ([Gate integrity](#gate-integrity) part (b) mutant (b)) must turn this clause red (the over-budget residency survives). This is the pure-pool property; its live on-disk counterpart is the Sprint 53.8 postflight residency
...
## Sprint 53.6: The jit-build resolver — `resolve = {download | build}` on first miss, no URL arm ⏸️
...
miss into the bounded cache — downloaded prebuilt or built from source with the Phase-30 baked toolchain — with
...
- The build-from-source path invoking the Phase-30 baked toolchain by absolute path (no `PATH`, no env), and the
...
- An in-file honesty note: URL-foreclosure and identity-from-closed-catalog are **proven-in-types** (Gate 1); the
  ...
  model (Tier 2) and kernel (Tier 3) tiers reuse this resolver but land in Phases 54/55.
...
### Remaining Work
Production model inference through the resolved engine is Phase 54, not a hidden acceptance claim here.

## Sprint 53.7: Per-node cache-owner reuse across client pods ⏸️
...
- The pod wiring (rendered by the Phase-31 reconciler) that gives the owner a bounded disk-backed `emptyDir` with
...
**Docs to update**:
`documents/engineering/content_addressing_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-53
...
- A Register-3 ledger recording: URL-foreclosure and the required bounded-`CacheBudget` field as
  ...
  **deferred** (Phases 54/65), with cross-node and cross-substrate reuse explicitly not asserted.
...
2. With the cache filled to budget, a postflight on-disk peak/final measurement confirms pin-aware eviction and
   ...
   ephemeral-under-reserved owner each return their **tagged** `Left` at the Phase-8 fold at the Phase-12
...
3. The Register-3 ledger is emitted and marks first-miss resolution, cross-pod reuse, and pin-aware eviction as
   *tested on linux-cpu*, URL-foreclosure and the required bounded-`CacheBudget` field as *proven-in-types*, the
   numeric cache/ephemeral inequalities as *provision-seal checked*, and the model/kernel tiers (Phases 54/65) and
   cross-node/cross-substrate reuse as **UNVERIFIED**.

### Remaining Work
The Tier-2 model and Tier-3 CUDA kernel reuse remain assigned to Phases 54 and 65 respectively.
...
- `documents/engineering/content_addressing_doctrine.md` — the §6 proven/tested/assumed table gains an
  ...
  Tier-2/Tier-3 realizations remain Phases 54/65 targets, and add the kernel module paths
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-53 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 53's gate substrate (linux-cpu) in the per-phase substrate map.
...
- [phase_08](phase_08_capacity_core_folds.md) — the `fits`/`carve` capacity fold the `CacheBudget` bound reuses
- [phase_12](phase_12_provision_seal.md) — the whole-deployment provision seal where the over-budget cache peak is rejected
- [phase_13](phase_13_inference_accelerator_provision.md) — the `InferenceEngine` binder + closed `EngineRuntime` union the resolver keys on
- [phase_15](phase_15_chain_kernel_boundary.md) — the `chain`/`Step` kernel the kernel primitives plug into
- [phase_29](phase_29_bootstrap_coordinator_kind.md) — the bootstrap coordinator + single-node `kind` cluster + substrate fingerprint this phase runs on
- [phase_30](phase_30_base_image_registry.md) — the base image that bakes the jit-build resolver + toolchain this phase drives live
- [phase_31](phase_31_object_reconciler.md) — the typed SSA object reconciler that renders the cache owner and client pods
- [phase_35](phase_35_platform_backbone.md) — the platform backbone the cache owner/client pods schedule onto
- [phase_42](phase_42_content_store_workflow.md) — the content store + workflow runtime this phase lifts and runs on
- [phase_54](phase_54_infernix_lift.md) — the infernix CPU-inference lift whose reproducibility reuses this kernel and rides this resolver next
- [phase_65](phase_65_jitml_lift_cuda.md) — the jitML/CUDA lift whose kernel tier reuses this resolver and kernel
