# Phase 48: Dynamic node provisioning by signal + leak-free provider gate
...
AWS audit, and provider leak-freedom remain UNVERIFIED because Phase 45 could not authenticate to AWS.
...
- [Sprints](#sprints)
- [Sprint 48.1: Dynamic node provisioning by signal ⏸️](#sprint-481-dynamic-node-provisioning-by-signal-)
- [Sprint 48.2: Phase gate — spin a provider cluster, provision a node by signal, tear down leak-free ⏸️](#sprint-482-phase-gate--spin-a-provider-cluster-provision-a-node-by-signal-tear-down-leak-free-)
...
by a tag-only sweep. It cleaned every Phase-48 object. This is a scoped Kubernetes/cloud-metadata analogue,
...
This sub-phase **layers on** its three provider siblings — [Phase 45](phase_45_provider_deploy_checkpoint.md)
(the provider-cluster Pulumi deploy + Vault-Transit-enveloped MinIO checkpoint + `observeProviderAccount`),
[Phase 46](phase_46_provider_child_bringup.md) (the stateless hostless in-cluster singleton + capacity-scheduler roles + parent→child Lease handoff), and [Phase 47](phase_47_provider_ebs_credential.md) (per-PV EBS decoupled from the node lifecycle + the create-vs-delete credential + static EBS CSI) — and owns dynamic node provisioning
...
set, and drives the live set toward it through the [Phase 27](phase_27_object_reconciler.md)
...
grown bound ([Phase 7](phase_07_capacity_core_folds.md) placement, [Phase 8](phase_08_storage_geometry_folds.md) storage geometry, [Phase 9](phase_09_execution_accelerator_folds.md) execution/accelerator residency) and proves
...
full supply/layout/device observation, and a [Phase 28](phase_28_capacity_scheduler.md) scheduler-generation CAS;
an `Unreachable` node observation **refuses** rather than stranding an EC2 instance.

Second, the **phase gate** (Sprint 48.2): a single `.dhall` that, from a linux-cpu parent, spins up the EKS
provider cluster ([Phase 45](phase_45_provider_deploy_checkpoint.md)), converges its stateless in-cluster
control plane ([Phase 46](phase_46_provider_child_bringup.md)), dynamically provisions an extra node **by evaluating a declared signal rule** (Sprint 48.1), binds the Pulumi-created durable EBS through the static
`ebs.csi.aws.com` CSI PV and writes a run-unique marker ([Phase 47](phase_47_provider_ebs_credential.md)),
...
is [Phase 55](phase_55_test_topology_dsl.md) work, deferred and never depended on here.
...
reclamation *explicitly deferred to [Phase 55](phase_55_test_topology_dsl.md)*.
...
> (Phases 45–48; see [Phase 45](phase_45_provider_deploy_checkpoint.md)) share one provider topology
...
`mut-44.1-drop-parallel-executor` mutants) stays in [Phase 45](phase_45_provider_deploy_checkpoint.md); the bootstrap
scheduler / add-on cutover / Lease-handoff apparatus (the `public-pull` and hostless-`LinuxHost`-foreclosure
tags) stays in [Phase 46](phase_46_provider_child_bringup.md); the per-PV-EBS credential apparatus
...
mutants) stays in [Phase 47](phase_47_provider_ebs_credential.md). This phase inherits only the reactive-node
and teardown slice below; the **composed** gate (Sprint 48.2) additionally re-runs the named sibling mutants and
cites them to their owning sub-phase.

```mermaid
flowchart LR
  %% register: orientation
  s0["Sprint 48.1: Dynamic node provisioning by signal"]
  s1["Sprint 48.2: Phase gate — spin a provider cluster, provision a node by…"]
...
([Phase 45](phase_45_provider_deploy_checkpoint.md)), and `credit-old-before-observed-delete` / `allow-delete`
([Phase 47](phase_47_provider_ebs_credential.md)) mutants against the full-stack fixture, cited to their owning
...
- [`cluster_lifecycle_doctrine.md §8`](../documents/engineering/cluster_lifecycle_doctrine.md#8-dynamic-node-provisioning)
  ...
  provisioning a node is one more pass of the [Phase 27](phase_27_object_reconciler.md)
...
- [`pulumi_iac_doctrine.md §4`](../documents/engineering/pulumi_iac_doctrine.md#4-what-pulumi-provisions-the-resource-catalog)
  ...
  EBS ([Phase 47](phase_47_provider_ebs_credential.md)) is a distinct retained class the per-run teardown never
  touches.
...
- [`daemon_topology_doctrine.md §3.1`](../documents/engineering/daemon_topology_doctrine.md#31-exactly-one-pod-is-a-k8setcd-property-not-an-amoebius-election)
  ...
  ([Phase 28](phase_28_capacity_scheduler.md)), not singleton election.
- [`storage_lifecycle_doctrine.md §5.1`](../documents/engineering/storage_lifecycle_doctrine.md#51-storage-is-independent-of-the-node-lifecycle)
  ...
  test-flagged EBS by the elevated harness is [Phase 55](phase_55_test_topology_dsl.md) work, referenced and
  never invoked here.
...
## Sprint 48.1: Dynamic node provisioning by signal ⏸️
...
- An `Amoebius.Cluster.NodeProvisioner` that reads the declared elastic-node rule — a typed `ScalingPolicy`
  ...
  driven by **load** and **workflow completion** (spot-instance-cost-driven scaling is a declared future signal
  class, deferred and not exercised by this phase's gate corpus or seeded mutants) — computes the desired node
  set, and drives the live set toward it through the [Phase 27](phase_27_object_reconciler.md) reconciler; no
  ...
  [Phase 45](phase_45_provider_deploy_checkpoint.md) `observeProviderAccount` boundary re-read). SKU raw
...
- An `Amoebius.Pulumi.NodeGroup` enaction that adds an EC2/managed node (Pulumi, under the singleton, encrypted
  ...
  node; it is distinct from [Phase 47](phase_47_provider_ebs_credential.md)'s retained durable class. Every
...
- A staged managed-node join/leave protocol. Provider launch data makes the kubelet register with the exact
  ...
  the active [Phase 28](phase_28_capacity_scheduler.md) scheduler generation. Only an independent readback of
...
5. Give two child-cluster demands the same `CloudAccountId` and per-cluster quotas that each fit alone but exceed
   ...
   `DescribeInstances`/`DescribeVolumes` usage is one unit short; the [Phase 45](phase_45_provider_deploy_checkpoint.md)
...
## Sprint 48.2: Phase gate — spin a provider cluster, provision a node by signal, tear down leak-free ⏸️
...
only through the observe-then-plan chain, composes the Phase 45–47 siblings, provisions an extra node by
...
[Phase 55](phase_55_test_topology_dsl.md)), `DEVELOPMENT_PLAN/README.md`.
...
- The gate `test/dhall/phase_48_provider_provision.dhall`: spin up the EKS provider cluster
  ([Phase 45](phase_45_provider_deploy_checkpoint.md)), first derive the complete app/platform demand and
  provision it against the named base/elastic node classes, the bounded cache demand charged within local
  ephemeral supply (the cache-owner `emptyDir` charged **once** inside the [Phase 7](phase_07_capacity_core_folds.md) logical local-ephemeral fold — **not** a second supply pool and **not** the [Phase 49](phase_49_determinism_jitcache.md) typed `CacheBudget`/jit-build construct, which is out of this phase's scope), parent executor Jobs and
  ...
  in-cluster control plane ([Phase 46](phase_46_provider_child_bringup.md)), provision an extra node by a declared
  rule and observe it join only after node-scoped scheduler authority (Sprint 48.1), bind the Pulumi-created EBS
  through [Phase 47](phase_47_provider_ebs_credential.md)'s static CSI PV and write a run-unique marker through
...
- An `Amoebius.Pulumi.Teardown` step: one `reconcileAbsent` over the **ephemeral** registry subset (VPC, EKS
  ...
  Unreachable → refuse* — leaving the durable EBS class ([Phase 47](phase_47_provider_ebs_credential.md))
...
- A per-run proven/tested/assumed ledger recording: provider bring-up + signal-driven node join as **tested on the EKS provider target from a linux-cpu parent**; per-run teardown leak-freedom **under the broadened run-owned sweep** as **tested**; durable EBS retention as **correct-by-class**; and the elevated-harness
  durable-EBS *reclamation* as **explicitly deferred to [Phase 55](phase_55_test_topology_dsl.md), not asserted here**.
...
1. Run the gate end-to-end over the committed representative set
   ...
   parent bootstrap Lease holder releases before the authenticated child singleton acquires ([Phase 46](phase_46_provider_child_bringup.md)).
   ...
   - The bounded cache-owner `emptyDir` is charged **once** inside that [Phase 7](phase_07_capacity_core_folds.md) local-ephemeral demand, not as a second supply pool.
...
2. From the provider-stack-absent state left by Validation 1, with the durable EBS retained, run a second full
   spin-up → run → teardown cycle and repeat [Phase 47's retained-volume identity/readback oracle](phase_47_provider_ebs_credential.md)
...
3. Assert the run emits a proven/tested/assumed ledger per
   ...
   elevated-harness durable-EBS reclamation stays explicitly deferred to [Phase 55](phase_55_test_topology_dsl.md)
...
5. Run the one-short executor/plugin/workspace/checkpoint-budget, pod-slot, CSI-attach, root-EBS, durable-EBS, and
   ...
   PVC/PV, and static CSI attachment. The `drop-parallel-executor` ([Phase 45](phase_45_provider_deploy_checkpoint.md)),
   `credit-old-before-observed-delete` ([Phase 47](phase_47_provider_ebs_credential.md)), and this phase's
...
> test-flagged EBS under the elevated credential — is [Phase 55](phase_55_test_topology_dsl.md) and is **not** a
...
- `documents/engineering/resource_capacity_doctrine.md` — record that
  ...
  the Phase-49 typed `CacheBudget`.
...
- `documents/engineering/testing_doctrine.md` — record the Phase 48 per-run ledger artifact, the **broadened run-owned OS-boundary leak-free sweep** (run-tag + VPC id + `eks:cluster-name` / `kubernetes.io/cluster/<name>`,
  catching untagged provider-spawned orphans), and the explicit deferral of elevated durable-EBS reclamation to
  [Phase 55](phase_55_test_topology_dsl.md).

**Cross-references to add:**
- `DEVELOPMENT_PLAN/system_components.md` — register `Amoebius.Cluster.NodeProvisioner`,
  `Amoebius.Pulumi.NodeGroup`, and `Amoebius.Pulumi.Teardown` as Phase-48 design-first rows, each mapped to its
  ...
  first delivery in [Phase 45](phase_45_provider_deploy_checkpoint.md) / [Phase 46](phase_46_provider_child_bringup.md)
  / [Phase 47](phase_47_provider_ebs_credential.md).
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 48 → `linux-cpu` (parent) row with the `provider` (EKS)
  deploy target annotated as a target class, not a fifth hardware substrate.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase 48 row's status once the gate passes; link this document.

## Related Documents

- [README.md](README.md) — the live tracker; the Phase 48 one-line gate and status
...
- [phase_45_provider_deploy_checkpoint.md](phase_45_provider_deploy_checkpoint.md) — the provider-cluster Pulumi
  deploy, Vault-Transit-enveloped MinIO checkpoint, and `observeProviderAccount` this phase reuses to add/drain a
  node group
- [phase_46_provider_child_bringup.md](phase_46_provider_child_bringup.md) — the stateless hostless in-cluster
  singleton + capacity-scheduler roles + parent→child Lease handoff the gate composes before node provisioning
- [phase_34_live_dsl_singleton.md](phase_34_live_dsl_singleton.md) — owns the singleton runtime role reused by
  the provider child
- [phase_47_provider_ebs_credential.md](phase_47_provider_ebs_credential.md) — per-PV durable EBS decoupled from
...
- [phase_27_object_reconciler.md](phase_27_object_reconciler.md) — the `discover → diff → enact → re-observe`
  reconciler that makes provisioning a node just another reconcile
- [phase_28_capacity_scheduler.md](phase_28_capacity_scheduler.md) — the `amoebius-capacity`
  scheduler-generation CAS / reservation / exclusive Binding at node join
- [phase_55_test_topology_dsl.md](phase_55_test_topology_dsl.md) — the elevated-harness durable-EBS reclamation
