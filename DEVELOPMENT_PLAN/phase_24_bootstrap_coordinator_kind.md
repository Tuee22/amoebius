# Phase 24: Python bootstrap coordinator + substrate detect + single kind cluster
...
and Windows uses WSL2 for the same clean boundary. The complete Phase-24 gate passed on the Incus route;
...
opens only after the pre-cluster band (Phases 1–23) is green, and it composes four things: the Python `pb`
...
control-plane singleton — those land in Phase 25 onward. Before `kind create`, bootstrap observes physical-host
...
flowchart LR
  ...
  s2["Sprint 24.3: The Python pb bootstrap coordinator (package-manager root → toolchain →…"]
...
not a sixth Phase-24 `HostTool`: it establishes the machine boundary from which the five-member bootstrap coordinator gate
...
### Deliverables
- A closed `HostTool` enum naming exactly the Phase-24 tool set (`ghcup`, `cabal`, `kubectl`, `kind`, and the
...
- A **CI structural check that no module outside `src/Amoebius/Host/Ensure.hs` imports any process-spawn API**
  ...
  level).
- A `HostConfig` carrying the detected substrate (Sprint 24.1) plus a `Map HostTool AbsExe`, and a probe-first,
...
### Deliverables
- A `bootstrap` command chain that detects the substrate (Sprint 24.1), assembles a `BinaryContext`
  (parameters + context + file/socket witnesses), ensures `kind`/`kubectl` by absolute path (Sprint 24.2), and
...
- A typed `ObservedInventory` assembled from an independent apiserver/OS-boundary read: net node allocatable
  ...
  in Phase 53.) The bootstrap handoff exists only after `declaredCapacity <= observedCapacity` on every
...
- [README.md](README.md) — flip the Phase 24 tracker-row status once work begins and keep its link current.
- [substrates.md](substrates.md) — record `linux-cpu` as the Phase 24 gate substrate (the first Register-3 row).
