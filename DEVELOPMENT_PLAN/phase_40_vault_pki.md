# Phase 40: Root Vault + PKI + built-in Haskell Vault client
...
Phase 40 delivers the root Vault + PKI + built-in Haskell Vault client; its design is owned by [vault_pki_doctrine.md](../documents/engineering/vault_pki_doctrine.md), [platform_services_doctrine.md](../documents/engineering/platform_services_doctrine.md), [resource_capacity_doctrine.md](../documents/engineering/resource_capacity_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 40.1: Root single-node password-encrypted Vault — init-once / unseal-on-rebuild ✅](#sprint-401-root-single-node-password-encrypted-vault--init-once--unseal-on-rebuild-)
- [Sprint 40.2: The self-signed PKI trust anchor issues ✅](#sprint-402-the-self-signed-pki-trust-anchor-issues-)
- [Sprint 40.3: Built-in Haskell Vault client (no agent sidecar) reads a `SecretRef` by name — the gate ✅](#sprint-403-built-in-haskell-vault-client-no-agent-sidecar-reads-a-secretref-by-name--the-gate-)
- [Sprint 40.4: Register-2.5 fail-closed Vault unseal under simulated faults ✅](#sprint-404-register-25-fail-closed-vault-unseal-under-simulated-faults-)
...
Blocked (superseded) — reopened 2026-08-16 behind the amended Phase-36 handoff and Phase-39 revalidation. Its prior capability record remains historical until the exact predecessor chain is resealed.
...
`python3 tools/vault_pki_gate.py --execute` verified the exact Phase-39 predecessor and Phase-36 image
...
The `SecretRef` *type* is not here: it is dhall-typecheck/gadt-decode surface and belongs to Phases 11 and 12, which are
...
a single-node `kind` cluster on the **linux-cpu** lane, exact Phase-36 OCI archive, and Phase-39 retained ext4
...
(Phases 41–42), the Keycloak-owned edge (Phase 43), and the parent/child unseal modes, parent secret injection, and the
...
```
*Design intent. Phase 40's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
## Sprint 40.1: Root single-node password-encrypted Vault — init-once / unseal-on-rebuild ✅
...
**Blocked by**: [Phase 39](phase_39_retained_storage.md) gate.
...
### Deliverables
- Root Vault in **Shamir seal mode**, rendered and reconciled onto the Phase-39 retained PV; first-ever `vault
...
- A **pluggable unlock-material backend** behind one interface — the load-bearing property is only that the
  material is password-AEAD-sealed and never plaintext at rest. **At the root Phase-40 bring-up the backend is the host-side `.age` file**: MinIO does not exist until Phase 41, so a MinIO-sealed object (and equally a cloud
...
## Sprint 40.2: The self-signed PKI trust anchor issues ✅
...
- The **three-planes distinction** recorded and enforced: internal PKI (this phase) is not public-edge TLS
  (ZeroSSL/route53, Phase 43) and is not the distro's own self-signed cluster CA (the chicken-and-egg floor,
...
## Sprint 40.3: Built-in Haskell Vault client (no agent sidecar) reads a `SecretRef` by name — the gate ✅
...
**Docs to update**: `documents/engineering/vault_pki_doctrine.md`,
`documents/engineering/testing_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-40 status when the
...
## Sprint 40.4: Register-2.5 fail-closed Vault unseal under simulated faults ✅
...
**Blocked by**: Sprint 40.3.
**Independent Validation**: the real `io-classes` init/unseal client, unchanged from the live path, holds the
fail-closed invariant under every adversarial `IOSim`/`IOSimPOR` schedule against the Phase-22 modeled Vault,
...
unproven. This is a **Register-2.5** deterministic-simulation check, run in-process **before** the Sprint-35.3
...
- An `IOSim`/`IOSimPOR` harness running the **real** `src/Amoebius/Vault/{Init,Unseal,Seal,Client}.hs` code
  (`io-classes`-written, byte-for-byte the live path — no simulation-only fork) against the **Phase 22 Sprint 22.2 modeled Vault** (`src/Amoebius/Sim/Fakes/*`) with its fault knobs — **sealed**, **unreachable**, **lease-expiry**, and
...
- A **Register-2.5** proven/tested/assumed ledger (substrate `none`), stating the **honest limit** — the harness
  ...
  assumption is discharged only by this phase's **Sprint-35.3 Register-3 live gate**, never by simulation.
...
3. Emit the Register-2.5 ledger (substrate `none`); assert it records modeled-Vault fidelity as **assumed** and
   names the Sprint-35.3 Register-3 live gate as the discharge, never marking the live invariant green from
   simulation.
...
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 40's gate substrate (linux-cpu) in the per-phase substrate map.
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Vault/{Init,Unseal,Seal,Pki,Client,SecretRef,Error}.hs`
  as Phase-40 design-first rows against the component inventory.
...
- [phase_37](phase_37_object_reconciler.md) — the typed renderer + SSA reconciler that renders and applies Vault
- [phase_39](phase_39_retained_storage.md) — the no-provisioner retained PV Vault's durable KV lives on
- [phase_41](phase_41_platform_backbone.md) — the standard-service stack that consumes these Vault secrets
