# Phase 61: Root Vault + PKI + built-in Haskell Vault client
...
Blocked by redesigned Phase 60, its independent validation, and gate pass; every earlier
...
| Key | Contract |
|---|---|
| `Claim` | UNRESOLVED — blocks validation: typed semantic payload and gate evidence missing; prior prose: one cohesive claim — *secrets have a fail-closed root, and the client that reads them is compiled in rather than run beside*. No agent sidecar is a security claim, not a packaging preference. Explicit exclusions: every layer named in `Residue` remains UNVERIFIED. |
| `Subject` | UNRESOLVED — blocks validation: no production `.hs` module and entry point have been independently established for this reset contract. |
| `Command` | UNRESOLVED — blocks validation: typed semantic payload and gate evidence missing; prior prose: `pb validate phase 61` is the target command only; `pb` may only make the minimal platform distinction, establish the contained toolchain, build the source-bound binary, and exec it with argv unchanged, while the Haskell verdict entry point remains UNRESOLVED and blocks validation. |
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and complete gate execution missing; prior prose: Exact `ImmediatePredecessorPass` for Phase 60; candidate execution refuses an absent, stale, replayed, or different-source result. |
...
**Blocked by**: [Phase 60](phase_60_retained_storage.md) gate pass
...
- [phase_58](phase_58_object_reconciler.md) — the typed renderer + SSA reconciler that renders and applies Vault
- [phase_60](phase_60_retained_storage.md) — the no-provisioner retained PV Vault's durable KV lives on
- [phase_62](phase_62_platform_backbone.md) — the standard-service stack that consumes these Vault secrets
