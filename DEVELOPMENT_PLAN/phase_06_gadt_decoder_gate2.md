# Phase 6: GADT-indexed IR + total decoder (Gate 2)
...
> **Read this if**: phase 6 is next in the queue, or a later phase depends on what its gate establishes.

Phase 6 delivers the GADT-indexed IR + total decoder (Gate 2); its design is owned by [dsl_doctrine.md](../documents/engineering/dsl_doctrine.md), [resource_capacity_doctrine.md](../documents/engineering/resource_capacity_doctrine.md), [testing_doctrine.md](../documents/engineering/testing_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 6.1: The amoebius cabal package + `dsl-spec` test-suite skeleton ✅](#sprint-61-the-amoebius-cabal-package--dsl-spec-test-suite-skeleton-)
- [Sprint 6.2: GADT-indexed IR + smart constructors + phantom tenant refs + ownership indices ✅](#sprint-62-gadt-indexed-ir--smart-constructors--phantom-tenant-refs--ownership-indices-)
- [Sprint 6.3: The fail-closed decoder (`Dhall.inputFile auto` + exception-catch) + structured `DecodeError` ✅](#sprint-63-the-fail-closed-decoder-dhallinputfile-auto--exception-catch--structured-decodeerror-)
- [Sprint 6.4: The Gate-2 decode battery (`dsl-spec`) — the gate ✅](#sprint-64-the-gate-2-decode-battery-dsl-spec--the-gate-)
- [Sprint 6.5: Decoding the shared `SecretRef` and rejecting a literal ✅](#sprint-65-decoding-the-shared-secretref-and-rejecting-a-literal-)
...
`sha256:ee3ee22f7aa4b788…`; Phase 6 owns no remaining migration deferral.
...
same scope amendment that reopened Phase 5: Gate 2 must decode the shared `SecretRef` and reject any literal
...
**Remaining work.** None. Sprint 6.5 discharged the amendment and the gate is green on all ten sides.
...
`amoebius` executable, which is the component that calls it; the unimplemented seam stays inside Phase 26
...
**This gate cannot run today.** `tools/gadt_decoder_gate2_gate.py:55-57` resolve their oracles to
`tests/oracle/gate2/{positive_trees,compile_pairs,decode_cases}.tsv`; the tree has
`test/oracle/gadt_decoder_gate2/`. Both halves of each path are wrong and all three are read at run time, so
...
This phase builds the second of the DSL's typed spec gates. Gate 1 (Phase 5) rejects what is not even
...
The Phase-5 prerequisite passed on 2026-08-09 (`python3 tools/dhall_gate1_schema_gate.py`, ledger
...
ledger and QuickCheck properties (Phase 7). This phase checks the decoder is non-partial, fail-closed, and structurally
...
Phase-0 documentation lint and the Phase-5 `dhall type` corpus.
...
resource oracle, and committed mutants named in [Gate integrity](#gate-integrity). Phase 7 does not open
unless the ledger records Register 1 green and every runtime-enforcement layer UNVERIFIED.

```mermaid
flowchart LR
  %% register: orientation
  s0["Sprint 6.1: The amoebius cabal package + dsl-spec test-suite skeleton"]
  s1["Sprint 6.2: GADT-indexed IR + smart constructors + phantom tenant refs…"]
  s2["Sprint 6.3: The fail-closed decoder (Dhall.inputFile auto…"]
  s3["Sprint 6.4: The Gate-2 decode battery (dsl-spec) — the gate"]
  gate["the phase 6 gate"]
...
```
*Orientation. The seams phase 6 builds, in order; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns the apparatus. Not run.*
...
comprises: (a) the **representative Gate-2 negative set**, defined concretely as **exactly one negative fixture per named `DecodeError` failure class** — `SchemaMismatch`, `OutOfDomainArm`, `UnspellableCombination` — each
...
schema module Phase 5 owns, the expected-tag oracle gains their rows in Phase 0 alongside the rest.
...
- [`dsl_doctrine.md §4 — total composability`](../documents/engineering/dsl_doctrine.md#4-total-composability):
  adopt the **import policy** the doctrine assigns to the Phase-5/6 gate. `env:` and remote (`http(s):`)
  imports are forbidden in any authored or uploaded spec; every spec is resolved-and-frozen to a
  fully-local, `sha256:…`-hashed expression **before** decode. This phase owns the enforcement — Sprint 6.3's
...
- [`resource_capacity_doctrine.md §3/§4`](../documents/engineering/resource_capacity_doctrine.md#3-the-types-quantity-capacity-demand-budget)
  ...
  normalizes the declarations in `ClusterIR`; Phase 8 implements the arithmetic/placement folds; Phase 12 runs
  ...
  the public boundary; a raw decoded or merely bound value can never reach Phase 14's `renderAll`.
...
## Sprint 6.1: The amoebius cabal package + `dsl-spec` test-suite skeleton ✅
...
### Remaining Work
None for Phase 6. Later DSL expansion belongs to the numerically assigned phases.

## Sprint 6.2: GADT-indexed IR + smart constructors + phantom tenant refs + ownership indices ✅
...
type-forecloses), `documents/engineering/dsl_doctrine.md` (Phase-6 status backlink),
...
- The Gate-2 side of the three surfaces [Phase 5](phase_05_dhall_gate1_schema.md) adds to the schema:
...
- Normalized resource declarations wired into the real IR, not ornamental side records: every execution-unit
  ...
  - The body also contains a complete structurally compatible Pod/host `ResourceEnvelope` and the identity
    required for Phase 11's exactly-once `BoundExecutionSet` join.
  - The Deployment rollout constructor is private and returns `Left (UnspellableCombination
    ...
    requirement.
  - Gate 2 rejects Deployment↔Host, HostProcess↔Pod, kind-policy, kind-cardinality, CUDA-with-rolling,
  ...
  - An optional `ComputeHeadroomDemand`/`HostComputeHeadroomDemand` normalizes its pad on the same axes into
    the zero-capable `Residual` representation, and its checked construction proves two things Gate 1 cannot
  ...
  - The reserved total the pad contributes to remains underivable here as it is unauthorable in Gate 1:
    Phase 8 folds it and Phase 12's post-bind `provision` boundary alone constructs it.
  ...
  - Phase 12's post-bind `provision` boundary constructs the private rounded `ProvisionedVolumeDemand`
    consumed by render, using the geometry and folds implemented in Phase 8.
- Canonical bounded service-storage inputs remain structural rather than being collapsed to caller-authored
  ...
  backend;
  - Phase 11 alone exact-joins those digests to `RegistryStoredArtifact` metadata and constructs
    `RegistryStorageDemand`.
  - Every deployment preserves exactly one raw `FirstDeployment | UpdateFrom PriorProvisionRefSource`;
  - Gate 2 requires the update's `Execution` resource arm and brands it as `PriorExecutionProvisionRef`.
  ...
  - Only private cache/registry/Vault witnesses constructed at Phase 12's post-bind `provision` boundary,
    using Phase 8's folds, may carry derived peaks.
- Every durable platform declaration retains normalized `BookKeeperGeometry` (ensemble/write/ack quorums,
  ...
  only gateway identity and execution-model selection; Phase 11 merges producer writer policies and constructs
...
- The normalized capacity tree retains the physical identity graph without alias-erasing or synthesizing
  ...
  - Gate 2 therefore cannot erase whether a debit belongs to the physical-raw parent or the nested VM-usable
  ...
  - Gate 2 preserves this graph as opaque branded ids;
  - Phase 8 owns value-level global uniqueness, exactly-one parent/reference, layout alias/support checks,
...
- A managed provider target preserves the exact normalized `{ account : CloudAccountId, nodeClasses :
  ...
  - Gate 2 preserves `InstanceStore.provisionedRawBytes` as fixed SKU raw supply and every carve's
  ...
  - Phase 10 derives a private `ProvisionedNodeRootVolumeRequest { volumeType, requiredUsableBytes,
  ...
  - Gate 2 never invents or reuses a future node's physical identity.
  - It preserves the fields needed to derive a globally scoped `ProviderInstanceId { account, cluster,
    ...
    template path and the operands for the Phase-8 local uniqueness, reference, layout alias/support,
...
- Every normalized `Observability` deployment binding retains a mandatory finite
  ...
  defaulted field, scalar query-temp, or a descriptor-independent fixed Prometheus provision has no normalized bypass. Phase 11
...
- Every normalized build retains a mandatory `BuildExecutionEnvelope`: a non-empty list of
  ...
  dependency references are closed and the graph acyclic; Phase 8 derives maxima over every legal concurrent
  set. All quantities and references survive in their refined domains; there is no optional, editable-
  aggregate, or descriptor-independent builder-resource bypass. Phase 30 owns snapshot-bound live admission,
  not decoding.
- Every normalized `KindEngineDemand` retains non-empty ordinal-indexed node-container runtime, full
  ...
  Docker/containerd/kind-supervisor reserve. The decoder preserves the nesting; Phase 8 proves in-node reserve
  + allocatable fits the container and then charges that container once plus only the host reserve.
    ...
    - Missing raw host supply, process-template qualification, or SKU identity is a decoder-field-inventory
      failure.
    - Phase 29 owns kind fit/enforcement; live multi-node rke2 admission/enforcement remains an explicitly
...
- An in-file honesty note that binding/capacity/topology totals ([§4.6](../documents/illegal_state/illegal_state_techniques.md#46-capacity-accounting--placement-witness-compute-and-summed-demand-within-capacity-storage-checked)/[§4.7](../documents/illegal_state/illegal_state_techniques.md#47-compatibility--topology-relations-by-construction-over-a-collection)) are *not* foreclosed by these
  types — the decoded declarations are intentionally **unprovisioned**. Phase 8 owns the total feasibility
  folds and Phase 12 invokes them on the fully expanded `BoundDeployment`; only their private constructor
...
1. For each of [§4.2](../documents/illegal_state/illegal_state_techniques.md#42-capability-and-phantom-tenant-tags--cross-tenant-refs-are-uninhabitable) (phantom tenant), [§4.3](../documents/illegal_state/illegal_state_techniques.md#43-gadt-indexed-state-machines--only-legal-transitions-are-typed) (GADT transition index), and [§4.4](../documents/illegal_state/illegal_state_techniques.md#44-ownership-indices--single-owner-ssot-structurally) (ownership index), the phase commits
   ...
   **and** is the constructor a named Phase-5 positive fixture is committed to decode through (its header
   ...
   locus). The compile-time pair is Sprint 6.2's standalone check; the decode-through round-trip to the cited
   positive is confirmed at the Sprint 6.4 gate once `decodeCluster` exists (Sprint 6.3). The check is red if
...
2. The pair cannot be satisfied by a strawman `mkCrossTenantRef` that was simply never defined. Because the
   ...
   the fixtures, and the exhaustive compile-fail corpus is assembled in Phase 7.
...
### Remaining Work
None for Phase 6; exhaustive catalog expansion is Phase 7.

## Sprint 6.3: The fail-closed decoder (`Dhall.inputFile auto` + exception-catch) + structured `DecodeError` ✅
...
## Sprint 6.4: The Gate-2 decode battery (`dsl-spec`) — the gate ✅

**Status**: Done — the capability is re-established by the migrated gate; the sprint's committed-ledger, pinned-toolchain, and repository-resident evidence mechanics are superseded
**Implementation**: `test/spec/dsl/DecodeSpec.hs`; positive fixtures reuse Phase 5's
...
with model↔runtime correspondence marked UNVERIFIED (owned by Phase 38). The exhaustive per-catalog-entry
corpus, the QuickCheck closure/round-trip/fold-totality properties, and the per-entry validation-locus ledger
are the front-loaded next phase ([Phase 7](phase_07_illegal_state_corpus.md)); the capacity/topology fold
negatives are [Phase 8](phase_08_capacity_core_folds.md), and provider-expanded/capability feasibility is
checked at [Phase 12](phase_12_provision_seal.md)'s conditional post-bind infrastructure-planning and
...
### Remaining Work
None. The exhaustive per-catalog-entry corpus begins in Phase 7.

## Sprint 6.5: Decoding the shared `SecretRef` and rejecting a literal ✅
...
[Phase 34](phase_34_vault_pki.md), and the prompt write path that turns a `Prompt` reference into a readable
one is Phase 34's too.

## Documentation Requirements

**Engineering docs to update (when the gate runs, flip the honest layer, never before):**
- `documents/engineering/dsl_doctrine.md` — backlink §5's Gate 2 to the in-process Phase-6 decoder; keep the
  runtime-enforcement half as Tier-2 residue owned by Phase 37.
- `documents/illegal_state/illegal_state_catalog.md` — annotate each entry the IR type-/decode-forecloses here
  ...
  deferred, and keep capacity/topology/provider-feasibility entries deferred to the Phase-8 fold and Phase-12
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius` cabal package, `src/Amoebius/Dsl/{Types,
  SmartConstructors,Ref,Decode,Error}.hs`, and the `dsl-spec` test-suite as Phase-6 design-first rows.

## Related Documents
- Phase-6 partial-foreclosure ledger — the tested/proven/unverified split sealed by this gate
...
- [phase_05](phase_05_dhall_gate1_schema.md) — Gate 1, the Dhall schema this decoder mirrors
- [phase_07](phase_07_illegal_state_corpus.md) — the exhaustive illegal-state corpus, properties, and
