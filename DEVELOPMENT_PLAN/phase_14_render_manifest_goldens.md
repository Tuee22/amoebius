# Phase 14: Pure `renderAll` + rendered-output goldens

> **Purpose**: Stand up the pure, total `renderAll :: ProvisionedSpec -> [K8sObject]`, mapping Phase 12's
...
- [Sprints](#sprints)
- [Sprint 14.1: The typed `K8sObject` model + Aeson serialization ✅](#sprint-141-the-typed-k8sobject-model--aeson-serialization-)
- [Sprint 14.2: Pure total `renderAll` + best-practice-by-construction ✅](#sprint-142-pure-total-renderall--best-practice-by-construction-)
- [Sprint 14.3: The rendered-output golden battery (`render-golden`) — the gate ✅](#sprint-143-the-rendered-output-golden-battery-render-golden--the-gate-)
...
`sha256:3a9622cca7322c7e…`; Phase 14 owns no remaining migration deferral.
...
UNVERIFIED. The gap is recorded against Phase 14 in
...
This phase implements only the pure `renderAll` half; Phase 31 still owns live reconciliation and runtime
...
and never hides later-stage objects; Phase 31's typed diff/enactor filters actions by that sealed activation,
so managed-node taint/admission cannot be generically applied during the initial scheduler bootstrap.
The same serializer module is available only inside the amoebius package for Phase 30's
...
convergence — all deferred to [Phase 31](phase_31_object_reconciler.md); and
the `chain`/`[Step]` `--dry-run` plan render, which is [Phase 15](phase_15_chain_kernel_boundary.md). This phase
...
analogous to the Phase-6 decode battery and the Phase-5 `dhall type` corpus.
...
Phase-14 ledger for the exact tested and UNVERIFIED boundary.
...
```
*Design intent. Phase 14's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
## Sprint 14.1: The typed `K8sObject` model + Aeson serialization ✅
...
**Docs to update**:
`documents/engineering/manifest_generation_doctrine.md` (Phase-14 backlink for the typed object model),
...
## Sprint 14.2: Pure total `renderAll` + best-practice-by-construction ✅
...
**Docs to update**: `documents/engineering/manifest_generation_doctrine.md` (backlink §3 to the Phase-14
...
- `renderAll :: ProvisionedSpec -> [K8sObject]`, pure and total (no I/O, no apiserver, no partial head),
  producing best-practice-by-construction objects. It maps Phase 12's sealed
...
- A closed `RenderReconcileMode` projection that includes only immutable schema/initial fields for the
  ...
  generic-SSA source path and are owned only by the corresponding typed actions in Phase 31.
...
### Remaining Work
Done. SSA, ApplySet pruning, readiness, and live convergence remain Phase-31 work.

## Sprint 14.3: The rendered-output golden battery (`render-golden`) — the gate ✅
...
Phase-14 render-golden battery — the rendered-output enactment that gates its foreclosure),
`documents/engineering/generated_artifacts_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-14
...
### Remaining Work
Migrate the gate's generated output to `.build/runs/`, externally attest it, and rerun after Phase 13 closes.
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Manifest/{K8sObject,Types,Render}.hs` and
  the `render-golden` test-suite as Phase-14 design-first rows.
...
- [phase_11](phase_11_capability_bind.md) — the capability→provider→shape binder and provision fold producing
  the opaque whole-deployment `ProvisionedSpec` and its sealed identity-keyed render-source set
- [phase_15](phase_15_chain_kernel_boundary.md) — the `chain`/`[Step]` `--dry-run` plan render deferred from here - [phase_31](phase_31_object_reconciler.md) — the live action-driven reconciler that consumes
