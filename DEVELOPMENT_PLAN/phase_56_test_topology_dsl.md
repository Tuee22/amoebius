# Phase 56: Test-topology DSL + suggest-test + elevated harness
...
- [Sprints](#sprints)
- [Sprint 56.1: The test-topology type — a deployment-rules layer that always tears down ⏸️](#sprint-561-the-test-topology-type--a-deployment-rules-layer-that-always-tears-down-)
- [Sprint 56.2: `suggest-test` — detect substrate + credential authority, emit a representative test `.dhall` ⏸️](#sprint-562-suggest-test--detect-substrate--credential-authority-emit-a-representative-test-dhall-)
- [Sprint 56.3: Flagged test credentials + test-owned resource tagging ⏸️](#sprint-563-flagged-test-credentials--test-owned-resource-tagging-)
- [Sprint 56.4: The elevated harness as sole automated deleter of test-owned storage + leak-free sweep ⏸️](#sprint-564-the-elevated-harness-as-sole-automated-deleter-of-test-owned-storage--leak-free-sweep-)
- [Sprint 56.5: The per-run ledger artifact + the delegated-failover gate topology ⏸️](#sprint-565-the-per-run-ledger-artifact--the-delegated-failover-gate-topology-)
...
⏸️ Blocked pending Phase-55 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
single-node `kind` cluster brought up by the Phase 29 bootstrap coordinator with Pulsar and MinIO already standing HA
(Phase 35) on retained storage (Phase 33). The mechanisms generalize patterns *proven in the sibling prodbox
...
comes from a real `amoebius suggest-test` execution on the gate host: a real Phase-29 host classification,
...
the `linux-cpu` single-node `kind` cluster, at minimum: the pre-standing Phase-35 HA Pulsar and MinIO on
retained Phase-33 storage; **at least one newly-allocated retained `no-provisioner` PV** (so the sole-deleter
rule of Sprint 56.4 is exercised against a resource this phase created); **one control-plane workload as a Deployment `replicas=1`**; and **at least two Pulsar `Failover`-subscription consumer pods** (an active worker
...
flowchart LR
  ...
  s0["Sprint 56.1: The test-topology type — a deployment-rules layer that…"]
  s1["Sprint 56.2: suggest-test — detect substrate + credential authority…"]
  s2["Sprint 56.3: Flagged test credentials + test-owned resource tagging"]
  s3["Sprint 56.4: The elevated harness as sole automated deleter of…"]
  s4["Sprint 56.5: The per-run ledger artifact + the delegated-failover gate…"]
...
```
*Orientation. The seams phase 56 builds, in order; [Gate integrity](#gate-integrity) owns the apparatus. Not run.*
...
below is discharged in the register named on the criterion. Where a criterion names Register 2 (Phase-15 fake
tools / fixed fake inputs), it is a type-level or pure-function check and its live layer is recorded UNVERIFIED
in the ledger; the leak-free teardown, the live identity-boundary denial (Sprint 56.4), and the failover
...
The topology inherits and source-checks Phase-42's exact command/event topic, Failover subscription,
...
- [`testing_doctrine.md §3`](../documents/engineering/testing_doctrine.md#3-the-test-topology-contract-spin-up--run--always-tear-down)
  — *the test-topology contract: spin up → run → always tear down*: Sprint 56.1 realizes the four-clause
...
- [`testing_doctrine.md §5`](../documents/engineering/testing_doctrine.md#5-suggest-test-detect-the-world-emit-a-representative-test-dhall)
  — *`suggest-test`: detect the world, emit a representative test `.dhall`*: Sprint 56.2 builds the generator
...
- [`testing_doctrine.md §6`](../documents/engineering/testing_doctrine.md#6-flagged-test-credentials)
  — *flagged test credentials*: Sprint 56.3 establishes the distinct flagged test-simulation identity (the
...
- [`testing_doctrine.md §7`](../documents/engineering/testing_doctrine.md#7-the-elevated-harness-is-the-sole-automated-deleter-of-test-owned-durable-storage-leak-free-cycles)
  ...
  Sprint 56.4 implements one-deleter/one-credential, flag-then-sweep, and the independent postflight-inventory
...
## Sprint 56.1: The test-topology type — a deployment-rules layer that always tears down ⏸️
...
**Register 3** on the `linux-cpu` `kind` cluster (a teardown against Phase-15 fakes does not prove real
...
## Sprint 56.2: `suggest-test` — detect substrate + credential authority, emit a representative test `.dhall` ⏸️
...
**Independent Validation**: Register 2 — pure, over Phase-15 fake tools, so the live probe layer is recorded
...
## Sprint 56.3: Flagged test credentials + test-owned resource tagging ⏸️
...
paths; not yet built)
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: Register 2 — type-level, over Phase-15 fake tools. This sprint pins the
*representability* boundary only; the *live* identity-boundary denial is Sprint 56.4's Register-3 obligation.
...
- The flagged credential resolved by name only through Vault (Phase 34) — flagging changes *which* credential
...
## Sprint 56.4: The elevated harness as sole automated deleter of test-owned storage + leak-free sweep ⏸️
...
### Deliverables
- A delete path reachable only by the elevated harness under the flagged credential of Sprint 56.3, scoped to
...
- An explicit scope boundary: this harness reclaims **test-owned** backing only. Production
  ...
  Phase 56 neither holds nor tests authority over production backing.
...
## Sprint 56.5: The per-run ledger artifact + the delegated-failover gate topology ⏸️
...
injects is delegated to Pulsar (Phase 42), never a bespoke amoebius election.
...
> and delivered by Phase 48's gateway-migration drills, not here. The delegated-failover shape is proven in the sibling `infernix`
...
- `documents/engineering/app_vs_deployment_doctrine.md` — §3's deployment-rules surface gains the realized
  test-topology layer (Sprint 56.1): the `TestTopology` is an ordinary deployment-rules layer over a production
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-56 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/system_components.md` — add the `Amoebius/Test/*` modules (`Topology`, `Runner`,
  `SuggestTest`, `Credentials`, `Harness`, `Sweep`, `Ledger`) to the component inventory as Phase-56
  design-first rows, mapped to the owning testing and storage-lifecycle doctrines.
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 56 "per generated test" substrate posture (each generated
  test substrate-locked; the canonical gate run on `linux-cpu`) in the per-phase substrate map.

## Related Documents
- [README.md](README.md) — the live tracker; Phase 56 objective, gate, and substrate
...
- [phase_42](phase_42_content_store_workflow.md) — the Pulsar-`Failover` worker takeover this phase's gate injects
- [phase_48](phase_48_gateway_migration_drills.md) — the cross-cluster gateway-migration obligation, distinct
  from this phase's intra-cluster failover
- [phase_52](phase_52_provider_dynamic_nodes.md) — the leak-free provider teardown this harness extends to test cycles
