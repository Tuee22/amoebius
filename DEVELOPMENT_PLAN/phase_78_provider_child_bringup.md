# Phase 78: Hostless provider child + convergence + Lease handoff

> **Purpose**: Bring a provider-managed EKS child — deployed and checkpoint-observed by
> [Phase 77](phase_77_provider_deploy_checkpoint.md) — to the same fungible shape as any self-managed amoebius
...
- [Sprints](#sprints)
- [Sprint 78.1: Two-stage capacity bootstrap for a hostless provider child ⏸️](#sprint-781-two-stage-capacity-bootstrap-for-a-hostless-provider-child-)
- [Sprint 78.2: Parent→child Lease handoff + hostless daemon topology ⏸️](#sprint-782-parentchild-lease-handoff--hostless-daemon-topology-)
- [Sprint 78.3: Standard-HA platform-service convergence from typed manifests ⏸️](#sprint-783-standard-ha-platform-service-convergence-from-typed-manifests-)
- [Sprint 78.4: The provider-child bring-up gate ⏸️](#sprint-784-the-provider-child-bring-up-gate-)
...
things, all driven from the single linux-cpu parent over the cloud/K8s API against a child that Phase 77 must
first deploy and a human must approve.

First, **the two-stage capacity bootstrap for a hostless child**. Once the EKS API and Phase-77 base node are
...
and capacity-debited into the base node's CRI store by Phase 77, never the not-yet-ready child registry or a
...
Human-approved Phase 26 schema generation and Phase 27 GADT decoding must make that state unrepresentable before
...
Envoy/Gateway API, Keycloak, cloud LoadBalancer — through the Phase-59 reconciler, **not** a thinner or different
...
flowchart LR
%% register: algebra
  p34["Phase 77: ready Managed Eks control plane + base node, scheduler OCI preloaded"]:::intent --> boot[/"Parent bootstrap Lease holder: create amoebius-capacity-scheduler pods=1"/]:::effect
...
`kind` cluster from inside which the control-plane daemon drove the Phase-77 deploy and now drives this
...
**Depends on:** [Phase 77](phase_77_provider_deploy_checkpoint.md)
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 77; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
- [`cluster_lifecycle_doctrine.md` §1 — Two cluster kinds, one lifecycle shape](../documents/engineering/cluster_lifecycle_doctrine.md#1-two-cluster-kinds-one-lifecycle-shape)
  ...
  mandatory capacity-scheduler role — as the child-side of Phase 77's cloud-keyed amoebic spawn, converging the
...
- [`image_build_doctrine.md` §2 — The single distribution rule: bake the binaries, build the amoebius image, pull only in-cluster](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster)
  ...
  reference the exact OCI digest that the future human-approved Phase 77 gate preloads into the base node's CRI
...
## Sprint 78.1: Two-stage capacity bootstrap for a hostless provider child ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 77](phase_77_provider_deploy_checkpoint.md) human approval
...
- Creation of `amoebius-capacity-scheduler` with exact `pods=1`, whose Deployment references the **exact OCI digest preloaded and capacity-debited into the base node's CRI store by Phase 77** — never the not-yet-ready child
...
- The managed-authority mint: install the managed-node taint, execution-identity admission, and full exclusive
  ...
  the cutover.
- An in-file honesty note: this stages a *provider child* through the same two-stage cutover Phase 60 proved on a
  self-managed cluster; the provider realization is validated here for the first time.

### Validation

1. Against a Phase-77-deployed child, the parent bootstrap holder creates `amoebius-capacity-scheduler`
...
Run the built protocol against a Phase-77-created EKS control plane and managed node, then read back the real
provider add-on and CRI-preload boundaries. The local and retained-Kubernetes protocol seams are complete.

## Sprint 78.2: Parent→child Lease handoff + hostless daemon topology ⏸️
...
## Sprint 78.3: Standard-HA platform-service convergence from typed manifests ⏸️
...
typed manifests via the Phase-59 reconciler, with no Helm and no public-registry pull, so substrate equivalence
...
- Convergence of the **complete** standard HA platform-service stack — registry (Distribution `registry:2`), MinIO, Vault,
  ...
  LoadBalancer — through the Phase-59 reconciler from typed manifests, using the same HA-capable topology at
...
- An in-file honesty note: the standard set and its HA-capable shape are planned for self-managed clusters in
  Phases 37/40–43; convergence of that exact set on a hostless provider child is tested here for the first time.
...
## Sprint 78.4: The provider-child bring-up gate ⏸️
...
- A per-run proven/tested/assumed ledger recording: provider-child bootstrap + handoff + standard-HA convergence
  ...
  OS-boundary audit; and the elevated-harness leak-free durable-resource *sweep* as **explicitly deferred to Phase 80, not asserted here** — skipping an applicable observation move marks that layer UNVERIFIED, never
  green.
...
4. Assert the run emits a proven/tested/assumed ledger recording the independent Lease-handoff sequence, the
   ...
   Phase 77's deploy lifecycle and the leak-free tag-sweep is recorded **deferred to Phase 80**, never asserted
   green here.

### Remaining Work

Re-run without `PARTIAL_EXTERNAL_AUTHORITY` once valid AWS authority permits Phase 77 to materialize the EKS
child. Eleven enumerated provider/cloud surfaces remain explicitly UNVERIFIED; Phase 80 still owns the final
...
- `documents/engineering/testing_doctrine.md` — record the Phase 78 per-run ledger artifact and the explicit
  deferral of the elevated leak-free durable-resource sweep to Phase 79.

**Cross-references to add:**

- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-runtime` provider-child control-plane daemon wiring
  (`Amoebius.Daemon.InClusterControlPlane`) and `Amoebius.Cluster.ProviderBringUp` as Phase-78 design-first rows,
  each mapped to its owning doctrine; map the reused `Amoebius.Scheduler.*` role to its Phase 60 delivery and the
  reconciler to Phase 59.
...
- [phase_77](phase_77_provider_deploy_checkpoint.md) — the provider-cluster Pulumi deploy-from-inside +
...
- [phase_66](phase_66_live_dsl_deploy.md) — supplies the control-plane daemon role and Lease authority protocol used by
  the provider child
- [phase_79](phase_79_provider_ebs_credential.md) — the per-PV durable EBS + create-vs-delete credential + static
  EBS CSI arm, layered on Phase 77, not exercised here
- [phase_80](phase_80_provider_dynamic_nodes.md) — dynamic node provisioning by signal and the leak-free provider
  gate (the independent teardown tag-sweep), layered on Phases 56/57/57
