...
- [Sprints](#sprints)
- [Sprint 49.1: Provider-cluster Pulumi deploy from inside a parent ⏸️](#sprint-491-provider-cluster-pulumi-deploy-from-inside-a-parent-)
...
(per-PV EBS + the create-vs-delete credential model), and [phase_52_provider_dynamic_nodes.md](phase_52_provider_dynamic_nodes.md)
...
([phase_52_provider_dynamic_nodes.md](phase_52_provider_dynamic_nodes.md)). The base node group here is a fixed
size-1 group provisioned and observed by this sub-phase alone; the `Managed Eks` arm carries **no** `LinuxHost`
witness (a foreclosure already unrepresentable in the pre-cluster band — the Dhall Gate-1 schema in Phase 5,
the GADT decoder in Phase 6, and the capacity/topology folds in Phase 8).
...
[phase_52_provider_dynamic_nodes.md](phase_52_provider_dynamic_nodes.md)'s; the per-PV durable EBS +
create-vs-delete slice (the `ec2:DeleteVolume` expected-denied-call tag, the `protect`/`Retain` state, the
static-CSI `volumeHandle` bind, `mut-46.1-*`) is [phase_51_provider_ebs_credential.md](phase_51_provider_ebs_credential.md)'s;
and the scheduler-cutover slice (`mut-45.1-public-pull`, the `LinuxHost`-absence foreclosure tag) is
[phase_50_provider_child_bringup.md](phase_50_provider_child_bringup.md)'s. Partitioned to this seam:
...
[phase_52_provider_dynamic_nodes.md](phase_52_provider_dynamic_nodes.md)'s gate, deferred and never depended
...
- [`cluster_lifecycle_doctrine.md §3`](../documents/engineering/cluster_lifecycle_doctrine.md#3-amoebic-spawning--the-recursive-forest)
  — *amoebic spawning — the recursive forest*: provider-cluster spawn is the *cloud-keyed* sibling of Phase 47's
...
- An `Amoebius.Pulumi.Provider.Eks` program that provisions the EKS control plane + a base managed node group
  ...
  the cluster ready for its in-cluster control-plane bootstrap ([phase_50_provider_child_bringup.md](phase_50_provider_child_bringup.md)).
...
- `planInfrastructure` derives the provider-cluster demand from the exact `BoundDeployment` and declared
  ...
  [phase_51_provider_ebs_credential.md](phase_51_provider_ebs_credential.md).) Only the receipt-bound provider
...
- `documents/engineering/testing_doctrine.md` — record the Phase 49 per-run ledger artifact for the
...
- `DEVELOPMENT_PLAN/system_components.md` — map the reused `amoebius-pulumi` Engine and EncryptedMinio backend
  to their first delivery in Phase 47; register `Amoebius.Pulumi.Provider.Eks`, the `observeProviderAccount`
  boundary, and the `PulumiExecutionDemand` / `PulumiCheckpointObjectDemand` types as Phase-49 design-first rows,
  each mapped to its owning doctrine.
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 49 → `linux-cpu` (parent) row with the `provider` (EKS)
  deploy target annotated as a target class, not a fifth hardware substrate.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase 49 row's status once the gate passes; link this document.

## Related Documents

- [README.md](README.md) — the live tracker; the Phase 49 objective, gate, and substrate
...
- [phase_52](phase_52_provider_dynamic_nodes.md) — dynamic node provisioning by signal and the independent
