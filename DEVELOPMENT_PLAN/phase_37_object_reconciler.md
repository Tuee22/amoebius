# Phase 37: Typed renderer + object reconciler

> **Purpose**: Take an opaque whole-deployment `ProvisionedSpec`, re-observe and cross-check the target's
> complete resource/capability inventory before mutation, construct the Phase-20 deployment-global
...
- [Sprints](#sprints)
- [Sprint 37.1: Deployment-global desired state + authenticated live inventory + typed action plan ✅](#sprint-371-deployment-global-desired-state--authenticated-live-inventory--typed-action-plan-)
- [Sprint 37.2: Bootstrap Lease authority + generic typed-action dispatcher + scoped SSA + storage-scaling dispatch ✅](#sprint-372-bootstrap-lease-authority--generic-typed-action-dispatcher--scoped-ssa--storage-scaling-dispatch-)
- [Sprint 37.3: Staged execution transitions, Job terminal protocol, and authenticated deletion ✅](#sprint-373-staged-execution-transitions-job-terminal-protocol-and-authenticated-deletion-)
- [Sprint 37.4: Wait-for-ready + the idempotent-convergence gate (re-run no-op) ✅](#sprint-374-wait-for-ready--the-idempotent-convergence-gate-re-run-no-op-)
- [Sprint 37.5: Register-2.5 reconciler + staged-execution convergence under simulated faults ✅](#sprint-375-register-25-reconciler--staged-execution-convergence-under-simulated-faults-)
...
⏸️ Blocked pending Phase-36 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
Blocked (superseded) — reopened 2026-08-16 because Phase 36's successor OCI handoff is being amended to include Pulsar's S3 offloader bundle. Revalidate this phase against the amended exact handoff before Phase 38 proceeds.
...
The run verified Phase 36 attestation `sha256:e6ff9163d299b66b8ce77c216d5114e13af6fe4beb7e299834ee390e9963816a`
...
`kind` cluster from Phase 35, pulling its corpus Deployment from the `distribution` registry Phase 36 published
...
batteries, and two live observations out of `DEVELOPMENT_PLAN/evidence/phase_27` — a plan-tree directory that
...
`tools/object_reconciler_execution_live.py` each pinned the Phase-36 base image by a literal digest from a build that is
gone, and the one assertion tying the running pod to Phase 36's published artifact compared against that same
...
Complete. Sprints 27.1–27.5 and the complete Register-3/2.5 phase gate passed. This phase opened after the **Phase 36 gate** (the
native-architecture base image + jit-build resolver + in-cluster `distribution` registry) and the **Phase 35 gate**
...
them to convergence. It builds the **object-reconciler half** of the manifest doctrine: Phase 20 already stood
...
cluster from Phase 35. The reconciler is exercised here **from the host binary against a scratch namespace**,
against the pinned Phase-20 corpus — the in-cluster **control-plane daemon** that eventually owns it
(a Deployment `replicas=1` whose single-instance guarantee is delegated to k8s/etcd with **no bespoke election**) arrives in Phase 43. The `amoebius-capacity` scheduler — its CAS `Reserved`→`BindingInFlight`→
`Bound` reservation ledger, two-stage bootstrap cutover, and execution-identity admission — is **Phase 38, layered on this reconciler**; in this phase the corpus Pods are bound by the **default Kubernetes scheduler**,
...
- **2026-08-09 — Sprint 37.1 complete.** The separately validated desired index, authenticated observed
  ...
  unchanged Phase-20 renderer gate. An import lint found no writer boundary in the read-only preflight modules.
  Receipt: `sprint-26.1-receipt.json`, fingerprint
  `dynamically-resolved`.
- **2026-08-09 — Sprint 37.2 complete.** The typed Lease token/renewal rounding, effect dispatcher, and scoped
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 37.3 complete.** The pure serial, ordinary/CUDA/Metal release, Job terminal, and
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 37.4 complete.** A clean Register-3 run converged 19 externally observed objects,
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 37.5 complete.** The real Lease-token, scoped-SSA, serial, host/device-release, Job,
  authenticated-delete, and readiness modules ran through the Phase-22 `IOSim` environment: eight fault
  ...
  independently bounded by Sprint 37.4's live evidence. Receipt:
  `sprint-26.5-receipt.json`, fingerprint
  `dynamically-resolved`.

## Phase Summary

This phase delivers amoebius's live typed-action **object reconciler**. Pure Phase 20 supplies
...
Phase 37. This phase owns the index-preservation and the fail-closed refusal to grant generic apply authority
over a Pod/controller or mutable ledger/Lease field; Phase 38 owns the witnesses that unlock the guarded
stages.
...
TerminalRetained` records are a distinct structure added in Phase 37.) The observed set validates the
...
the exact map of `ValidatedStorageScalingAction`s derived from Phase-15 policy-only envelopes and fresh storage
snapshots. (The scheduler-ledger CAS-version arm of `ValidatedLiveTarget` is added in Phase 37.) A final
...
completion variants, digest/outcome equality, cleanup deadlines, and `CompletedJobNoOp`; the live Phase-37
cluster has neither MinIO nor the Phase-48 sole content-mutation gateway, so it does **not** claim a durable
...
deadline/release → terminal cleanup gate is Phase 47. The Job controller still uses `restartPolicy=Never`,
...
migration/executor is `UnknownCommitment` and fails closed. Phase-35 engine/static components enter through
...
Phase 35; no apple, linux-cuda, or windows substrate is touched (the CUDA/Metal transition arms are exercised
against deterministic observed-device models here, and their live substrate proof is owned by Phase 71 and
Phase 74 respectively).
...
`bind-before-reservation-CAS` mutant are partitioned to Phase 38 and are **not** duplicated here. The
`amoebius-capacity` scheduler bootstrap, its CAS reservation/Binding ledger, and execution-identity admission are
likewise **not** part of this gate; they are Phase 38's gate, layered on this reconciler.

**What the acceptance run must do.** The Phase-20 pure deployment-global `renderAll` list for the pinned
...
Phase-36 registry/proxy objects are authenticated read-only as the one declared bootstrap predecessor and cannot
...
flowchart LR
  ...
  s0["Sprint 37.1: Deployment-global desired state + authenticated live…"]
  s1["Sprint 37.2: Bootstrap Lease authority + generic typed-action dispatcher…"]
  s2["Sprint 37.3: Staged execution transitions, Job terminal protocol, and…"]
  s3["Sprint 37.4: Wait-for-ready + the idempotent-convergence gate (re-run no-op)"]
  s4["Sprint 37.5: Register-2.5 reconciler + staged-execution convergence…"]
  gate["the phase 37 gate"]
...
```
*Orientation. The seams phase 37 builds, in order; [Gate integrity](#gate-integrity) owns the apparatus. Not run.*

**The representative reconcile corpus (oracle-pinned, §M.7 concrete corpus).** The gate's applied service set
is the committed fixture `test/fixture/live/reconcile-corpus/` — a subset of the Phase-20 byte-for-byte
...
instantaneous*:
- (i) at least one Deployment whose container image is pulled from the Phase-36 in-cluster `distribution`
...
**The fix that pins item (v) to amoebius's own CRD rather than an external workload operator (e.g. the Percona operator built in Phase 42) is deliberate:** it keeps the corpus and its over-bound-child mutant
self-contained within this phase and earlier phases, removing a latent forward dependency on Phase 41. The
...
Pod without typed authorization — must fail Sprint 37.3; and (e) a **healthy-CR/over-bound-child** mutant whose
amoebius controller reports its **own in-phase capacity/reservation CR** healthy while its actual child exceeds the
provisioned resource/PVC/rollout envelope — must prevent convergence after child enumeration in Sprint 37.4. (The
source's **bind-before-reservation-CAS** mutant (d) audits the scheduler ledger and is partitioned to Phase 37.)
...
- [`manifest_generation_doctrine.md §5`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  named by [§5](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions) is Phase 38; durable Job completion/cleanup first runs live in Phase 48; rollback and the release
...
- [`manifest_generation_doctrine.md §2`](../documents/engineering/manifest_generation_doctrine.md#2-the-typed-manifest-model-renderall-is-the-sole-public-pure-function-to-objects)
  — **the typed manifest model** (the pure renderer half): this phase *consumes* the Phase-20 pure, total private
...
- [`daemon_topology_doctrine.md §3`](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-daemon)
  ...
  it up *inside* the control-plane daemon is Phase 43.
- [`conformance_harness_doctrine.md §3`](../documents/engineering/conformance_harness_doctrine.md#3-the-load-bearing-invariant-rendering-never-touches-live-infrastructure)
  ...
  `renderAll`/plan/`--dry-run` path stayed cluster-free through Phase 21, and **apply is the first live step** —
...
- [`deterministic_simulation_doctrine.md §4`](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  — **Register 2.5, where deterministic simulation sits.** Sprint 37.5 validates the *built* reconciler and staged
...
> **Current seal rule.** Sprints 27.1–27.5 were resealed in order by the 2026-08-16 repository-contained run.
...
## Sprint 37.1: Deployment-global desired state + authenticated live inventory + typed action plan ✅

**Status**: Done — resealed 2026-08-16 inside `python3 tools/object_reconciler_gate.py --execute`; the corpus, its
independently authored action table, and the read-only preflight boundary all held, and
...
`src/Amoebius/Scheduler/Ledger.hs` reservation-ledger normalization is partitioned to Phase 37.)
**Blocked by**: the [Phase-36](phase_36_base_image_registry.md) gate.
...
- `renderAll → validateAndIndexRenderedObjects → observeInventoryAndObjects → authenticateExecutions →
  ...
  grants generic SSA over the full list; the scheduler-cutover stage-unlock witnesses are Phase 38). The observed
...
- A host-aware observed identity union: `KubernetesPod PodUid | HostProcess HostProcessInstanceId |
  ...
  reservation ledger, distinct from the Phase-38 k8s scheduler reservation ledger.
...
- A complete `ValidatedLiveTarget` constructed from one `ObservedLiveResourceSnapshot`: mandatory whole
  ...
  and exact action-domain witness. Its storage-scaling map exact-joins every Phase-15
  ...
  `ValidatedLiveTarget` is added in Phase 37.)
- Execution actions cover `NoOp`, kind-indexed Pod-controller apply, the three serial OnDelete stages, host
  ...
  gateway capability, which is absent from the Phase-37 live environment. Every mutating constructor carries only
...
- Preserve the operator child-admission and migration envelopes: admission/quota policy must be Ready before a CR
  ...
  retain old+new+workspace/temp/WAL until verified cutover. Phase 37 owns only the generic storage-scaling
  observation/validation/token boundary: Phase 39 supplies the retained-carve and verified-migration enactors, and
  Phase 55 supplies the cloud provider-capacity enactor. Durable backing is never generic prune.
...
into the plan tree. Sprint 37.2 consumes the typed action/authority boundary.

## Sprint 37.2: Bootstrap Lease authority + generic typed-action dispatcher + scoped SSA + storage-scaling dispatch ✅

**Status**: Done — resealed 2026-08-16 inside `python3 tools/object_reconciler_gate.py --execute`; the Lease CAS, scoped
managed fields, stable no-op, and clean postflight were observed live, and `sprint-26.2-receipt.json` in
that run's bundle records fingerprint
...
(`src/Amoebius/Admission/ExecutionIdentity.hs`) are layered on this dispatcher in Phase 37.
**Blocked by**: none; Sprint 37.1 is sealed.
...
precursor must hold the same provisioned mandatory `Lease` that the Phase-44 control-plane daemon later receives by an
observed handoff. The scheduler's CAS reservation/Binding path and its two-stage bootstrap are Phase 37.
...
- A dispatcher for every non-scheduler `ValidatedExecutionTransitionAction`. `ApplyDesiredPodController` preserves
  ...
  and prior-controller delete are sent to their own enactors (built in Sprint 37.3); Job completion/cleanup dispatch
  additionally requires the gateway capability and therefore cannot run in the live Phase-37 gate. `NoOp` and
...
- A storage-scaling dispatcher that accepts only a `ValidatedStorageScalingAction`, immediately rechecks its exact
  ...
  migration are abstract effect arms supplied in Phase 39, while provider-capacity creation also consumes the
  transition's exact validated cloud-action batch supplied in Phase 55. Every attempted effect requires post-attempt
...
- Scoped SSA under `fieldManager=amoebius` only for an ordinary desired-object action. It declares the exact owned
  ...
  `ManagedCapacityReady` is added in Phase 37.
...
3. Storage-scaling action tests run at the pure/fake boundary in this phase: stale allocation, backing,
   ...
   a lost response forces a fresh observation. Live retained and cloud mutations remain the Phase-39 and Phase-55
   gates respectively.
...
## Sprint 37.3: Staged execution transitions, Job terminal protocol, and authenticated deletion ✅

**Status**: Done — resealed 2026-08-16 inside `python3 tools/object_reconciler_gate.py --execute`; both ordered
replacements, the retained terminal Pod, and the authenticated delete were observed live, and
...
**Blocked by**: none; Sprint 37.2 is sealed.
...
validated in pure, fake, and IOSim executions here; the live Phase-37 environment intentionally supplies no such
capability.
...
- Job terminal protocol for both `Succeeded` and `FailedBackoffExhausted`: the closed model writes the exact
  ...
  `CompletedJobNoOp`; a failed write retains the terminal Pod/ledger axes and retries safely. In the Phase-37 live
  cluster the gateway capability is absent, so the only inhabitant is `RetainTerminalAwaitingCompletionGateway`; the
  Pod, API bytes, logs/metadata, and retained partition remain charged. Phase 48 performs the first live
...
2. Register-2/2.5 transition tests cover ordinary host exit, CUDA device holds/free VRAM, and Metal drain/allocation/
   ...
   substrate proof remains owned by Phase 71 and Phase 72.
3. Register-1/2 Job tests, available at this sprint's completion, cover all-success and backoff-exhausted failure
   ...
   these same variants are additionally exercised in Sprint 37.5. In the live Phase-37 run, an independent apiserver
   observer proves the terminal UID remains, no Job-completion object is written, and no delete occurs. The first live
   object-store/apiserver persist-before-delete proof is the Phase-48 gate.
...
None. Sprint 37.4 composes these actions into the full convergence/no-op corpus.

## Sprint 37.4: Wait-for-ready + the idempotent-convergence gate (re-run no-op) ✅

**Status**: Done — resealed 2026-08-16 inside `python3 tools/object_reconciler_gate.py --execute`; the corpus converged
...
`test/spec/live/SchedulerReservationSpec.hs` live scheduler suite is Phase 37.)
**Blocked by**: none; Sprint 37.3 is sealed.
...
amoebius validation), `DEVELOPMENT_PLAN/README.md` (flip the Phase-37 status when the gate passes).
...
- The convergence battery over the pinned corpus: enact the exact `renderAll`/action plan → observe workload
  ...
  bind-before-CAS mutants are Phase 38's gate.)
...
## Sprint 37.5: Register-2.5 reconciler + staged-execution convergence under simulated faults ✅

**Status**: Done — resealed 2026-08-16 inside `python3 tools/object_reconciler_gate.py --execute`; 2,048 deterministic
...
real Manifest/Execution modules above on the Phase-21 `io-classes` `Env` — built and validated. (The
`test/spec/sim/SchedulerSim.hs` schedule battery is Phase 37.)
**Blocked by**: none; Sprint 37.4 is sealed.
...
replay by seed.
**Docs to update**: `documents/engineering/deterministic_simulation_doctrine.md` (Phase-37
...
open. The scheduler's CAS-race schedules are Phase 38's `SchedulerSim`.
...
- Committed mutants for lost `Lease`/resourceVersion retry, mutation without holder, sleep-gated readiness, serial
  ...
  collapsed scheduler readiness stages, premature managed taint/full RBAC — are Phase 38's `SchedulerSim`.)
- A Register-2.5 proven/tested/assumed ledger — the reconciler upholds convergence + fail-closed under the modeled
  schedules and faults; honest limit: modeled-apiserver fidelity is **assumed**, discharged by the Sprint-32.4
...
- `documents/engineering/manifest_generation_doctrine.md` — §5's typed SSA/staged-action/delete/wait engine flips
  ...
  design intent until Phase 38); §6's planned-vs-observed and `renderAll` model gains its first validation. Keep
...
- `documents/engineering/daemon_topology_doctrine.md` — record that Phase 37 drives the reconciler from the host
  binary; the §3 control-plane daemon that *owns* it (Deployment `replicas=1`, delegated single-instance, no election) is stood
  up in Phase 44.
- `documents/engineering/generated_artifacts_doctrine.md` — note that the applied `[K8sObject]` set is generated at apply time and never committed. - `documents/engineering/resource_capacity_doctrine.md` — record the read-only pre-mutation live inventory cross-check and its zero-write failure path. - `documents/engineering/deterministic_simulation_doctrine.md` — add the Phase-37 Register-2.5 status backlink for the Sprint-32.5 reconciler/execution fault battery.
...
- `DEVELOPMENT_PLAN/system_components.md` — register the Phase-37
  ...
  `Admission/ExecutionIdentity` modules are registered by Phase 37.)
...
- [Daemon Topology Doctrine](../documents/engineering/daemon_topology_doctrine.md) — [§3](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-daemon) the Deployment-`replicas=1`
  control-plane daemon (delegated single-instance, no election) that will own this reconciler in Phase 44
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — [§4](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits) the
  Register-2.5 io-sim environment the reconciler is validated against in Sprint 37.5, before the Register-3 live gate
...
- [phase_20_render_manifest_goldens.md](phase_20_render_manifest_goldens.md) — the pure per-projection renderers and
  deployment-global `renderAll` owner-union goldens this phase enacts
- [phase_21_chain_kernel_boundary.md](phase_21_chain_kernel_boundary.md) — the Register-2 fake-apply this phase
  replaces with real tools, and the `io-classes` seams Sprint 37.5 drives
- [phase_22_deterministic_sim_substrate.md](phase_22_deterministic_sim_substrate.md) — the `IOSimPOR`
  deterministic-simulation substrate Sprint 37.5 runs on
- [phase_35_bootstrap_coordinator_kind.md](phase_35_bootstrap_coordinator_kind.md) — the live single-node `kind` cluster this
  phase applies to
- [phase_36_base_image_registry.md](phase_36_base_image_registry.md) — the in-cluster registry the applied workloads
  resolve images against
- [phase_38_capacity_scheduler.md](phase_38_capacity_scheduler.md) — the `amoebius-capacity` scheduler, two-stage
  bootstrap cutover, and execution-identity admission layered on this reconciler
- [phase_39_retained_storage.md](phase_39_retained_storage.md) — the retained-carve and verified-migration
  storage-scaling enactors this phase's dispatch boundary calls into
- [phase_44_live_dsl_deploy.md](phase_44_live_dsl_deploy.md) — the Deployment-`replicas=1` control-plane daemon that stands
  the reconciler up in-cluster and receives the `Lease` by observed handoff
- [phase_48_content_store_workflow.md](phase_48_content_store_workflow.md) — the first live gateway write → readback →
