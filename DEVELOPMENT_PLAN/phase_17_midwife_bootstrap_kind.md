# Phase 17: Python midwife + substrate detect + single kind cluster
...
opens only after the pre-cluster band (Phases 1–16) is green, and it composes four things: the Python `pb`
...
control-plane singleton — those land in Phase 18 onward. Before `kind create`, bootstrap observes physical-host
...
state the identical run **converges without recreating the cluster** (divergence-repair, Sprint 17.4); **every
...
input; the enum's closedness (no sixth member) is asserted structurally in Sprint 17.2.
...
implementation exists: the `classify` decision table (Sprint 17.1), the per-substrate `[InstallStep]` golden
plans and the `mkAbsExe`-reject expected-error set (Sprint 17.2), and the named divergent-start fixtures with
their expected converged observation (Sprint 17.4). A golden regenerated from the implementation is not
admitted.
...
## Sprint 17.1: Live substrate detection 📋

**Status**: Planned
**Implementation**: `src/Amoebius/Host/Substrate.hs` (target: the total `classify` plus the three-read `detect`)
**Blocked by**: Phase 16 gate (the pre-cluster band closes; this is the first live phase).
...
## Sprint 17.2: No-`PATH` lazy tool-ensure — closed enum, `AbsExe`, install-and-verify 📋
...
### Deliverables
- A closed `HostTool` enum naming exactly the Phase-17 tool set (`ghcup`, `cabal`, `kubectl`, `kind`, and the
...
- A **CI structural check that no module outside `src/Amoebius/Host/Ensure.hs` imports any process-spawn API**
  ...
  level).
- A `HostConfig` carrying the detected substrate (Sprint 17.1) plus a `Map HostTool AbsExe`, and a probe-first,
...
## Sprint 17.3: The Python `pb` midwife (package-manager root → toolchain → build → `exec`) 📋
...
## Sprint 17.4: The in-binary `bootstrap` command — idempotent single-node kind bring-up 📋
...
**Blocked by**: Sprint 17.2, Sprint 17.3.
...
### Deliverables
- A `bootstrap` command chain that detects the substrate (Sprint 17.1), assembles a `BinaryContext`
  (parameters + context + file/socket witnesses), ensures `kind`/`kubectl` by absolute path (Sprint 17.2), and
...
- An empty cluster as the end state: one node, `Ready`, with **no** platform services (those are Phase 18+).
- A typed `ObservedInventory` assembled from an independent apiserver/OS-boundary read: net node allocatable
  ...
  in Phase 41.) The bootstrap handoff exists only after `declaredCapacity <= observedCapacity` on every
...
- An engine-system accounting rule: every kind node has an enforced role-indexed `EngineSystemReserve`.
  ...
  These bootstrap add-ons may still use `default-scheduler` in this Phase-17 empty-cluster state; the later
  ...
  leaving the scheduler bootstrap Pod as the sole `default-scheduler` exception — is Phase 20's scope and is
  specified there.
...
- [README.md](README.md) — flip the Phase 17 tracker-row status once work begins and keep its link current.
- [substrates.md](substrates.md) — record `linux-cpu` as the Phase 17 gate substrate (the first Register-3 row).
...
- [README.md](README.md) — the live tracker; its Phase 17 row is the authoritative one-line gate and status.
