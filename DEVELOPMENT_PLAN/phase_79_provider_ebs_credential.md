# Phase 79: Per-PV EBS decoupling + create-vs-delete credential
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_77_provider_deploy_checkpoint.md, DEVELOPMENT_PLAN/phase_78_provider_child_bringup.md, DEVELOPMENT_PLAN/phase_80_provider_dynamic_nodes.md
...
- [Sprints](#sprints)
- [Sprint 79.1: Per-PV durable EBS in its own state + node-vs-storage decoupling ⏸️](#sprint-791-per-pv-durable-ebs-in-its-own-state--node-vs-storage-decoupling-)
- [Sprint 79.2: The create-vs-delete credential model (cloud-API delete-deny) ⏸️](#sprint-792-the-create-vs-delete-credential-model-cloud-api-delete-deny-)
- [Sprint 79.3: Static-only baked AWS EBS CSI + static PV over volumeHandle ⏸️](#sprint-793-static-only-baked-aws-ebs-csi--static-pv-over-volumehandle-)
- [Sprint 79.4: Provider-volume migration + the `CreateProviderCapacity` storage-scaling arm ⏸️](#sprint-794-provider-volume-migration--the-createprovidercapacity-storage-scaling-arm-)
- [Sprint 79.5: Phase gate — durable EBS retained across teardown + cloud-API delete-deny + static-CSI reattach ⏸️](#sprint-795-phase-gate--durable-ebs-retained-across-teardown--cloud-api-delete-deny--static-csi-reattach-)
...
and exercised in [Phase 91](phase_91_test_topology_live.md) — referenced, never invoked here. Production reclaim
...
[Phase 29](phase_29_storage_geometry_folds.md) owns the policy-only envelope and the observe-then-plan
`Growable`/scaling fold; [Phase 59](phase_59_object_reconciler.md) owns the generic fresh-snapshot validation and
...
EBS/checkpoint readback. Retained-carve allocation and host migration remain [Phase 61](phase_61_retained_storage.md)
arms.
...
([Phase 77](phase_77_provider_deploy_checkpoint.md)); the hostless in-cluster control-plane daemon, the capacity-scheduler
roles, full platform-service convergence, and the parent→child Lease handoff
([Phase 78](phase_78_provider_child_bringup.md)); dynamic node provisioning by signal, the ephemeral
node-root EBS class, and the leak-free tag-sweep teardown gate ([Phase 80](phase_80_provider_dynamic_nodes.md));
and the elevated-harness reclamation of durable test-flagged EBS that makes a *full* leak-free test *cycle*
possible ([Phase 91](phase_91_test_topology_live.md)).
...
lifetime, and leave elevated-harness reclamation to [Phase 91](phase_91_test_topology_live.md). The ledger
cannot promote the phase.

**Depends on:** [Phase 78](phase_78_provider_child_bringup.md)
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 78; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
[Phase 77](phase_77_provider_deploy_checkpoint.md)):
...
- [`storage_lifecycle_doctrine.md` §5.1 — Storage is independent of the node lifecycle](../documents/engineering/storage_lifecycle_doctrine.md#51-storage-is-independent-of-the-node-lifecycle)
  ...
  the [Phase 91](phase_91_test_topology_live.md) elevated harness may destroy a test-owned volume, and production
...
- [`chaos_failover_doctrine.md` §12 — The moral core — proven, tested, assumed](../documents/engineering/chaos_failover_doctrine.md#12-the-moral-core--proven-tested-assumed)
  ...
  deferred to [Phase 91](phase_91_test_topology_live.md)*; skipping an applicable teardown-observation move marks
...
## Sprint 79.1: Per-PV durable EBS in its own state + node-vs-storage decoupling ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 78](phase_78_provider_child_bringup.md) human approval
...
- An in-file honesty note: `Amoebius.Pulumi.Ebs` produces the durable-class state and the `Promised`/`Materialized`
  ...
  is Sprint 79.2, and the static attach/mount path is Sprint 79.3.
...
## Sprint 79.2: The create-vs-delete credential model (cloud-API delete-deny) ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 79.1
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
delete authority is the elevated test credential ([Phase 91](phase_91_test_topology_live.md), referenced not invoked here).
...
- The only automated delete authority is the elevated test credential, limited to test-owned volumes and
  exercised in [Phase 91](phase_91_test_topology_live.md) — referenced, not invoked here. Production reclaim uses a
...
## Sprint 79.3: Static-only baked AWS EBS CSI + static PV over volumeHandle ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 79.2
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
1. Create a per-PV EBS (Sprint 79.1), render a static PV whose CSI `volumeHandle` is that exact EBS ID and whose
...
## Sprint 79.4: Provider-volume migration + the `CreateProviderCapacity` storage-scaling arm ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 79.3
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
- An in-file honesty note: this sprint builds the create-only guard's scaling/migration arms;
  ...
  refines are owned by [Phase 29](phase_29_storage_geometry_folds.md) and
  [Phase 59](phase_59_object_reconciler.md).
...
3. Drive a `Growable` durable-EBS budget across its threshold without editing its desired demand. The fresh
   observation and [Phase 29](phase_29_storage_geometry_folds.md) planner must select `CreateProviderCapacity`;
...
## Sprint 79.5: Phase gate — durable EBS retained across teardown + cloud-API delete-deny + static-CSI reattach ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 79.4
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
- The gate over a reviewed Haskell durable-EBS declaration, lazily projected to
  ...
  cluster ([Phase 77](phase_77_provider_deploy_checkpoint.md)); create the per-PV durable EBS (Sprint 79.1);
  render and observe Ready the baked static-only CSI (Sprint 79.3); bind the Pulumi-created EBS through the static
  ...
  reattachment.
- The credential proof (Sprint 79.2): a real `ec2:DeleteVolume` under the operational credential is denied at the
...
- A per-run proven/tested/assumed ledger recording: durable-EBS creation, static-CSI bind, and marker read as
  ...
  *reclamation* as **explicitly deferred to [Phase 91](phase_91_test_topology_live.md), not asserted here**.
...
5. Assert the run emits a proven/tested/assumed ledger per
   ...
   deferred to [Phase 91](phase_91_test_topology_live.md); skipping an applicable teardown-observation move
...
Re-run the enumerated provider surfaces after Phase 77 can create the EKS child. Fifteen AWS/EBS/CSI/runtime
surfaces remain UNVERIFIED, including Phase-91 elevated durable reclamation.
...
> [Phase 80](phase_80_provider_dynamic_nodes.md), and the leak-free durable-EBS *reclamation* under the elevated
> credential — the move that makes a *full* leak-free test *cycle* possible — is
> [Phase 91](phase_91_test_topology_live.md) and is **not** a dependency of this phase. Live AWS spend (EKS,
...
- `documents/engineering/storage_lifecycle_doctrine.md` — record the required-usable versus allocation-rounded
  ...
  reclamation deferred to [Phase 91](phase_91_test_topology_live.md) (§7.1, the single elevated-harness exception).
...
- `documents/engineering/testing_doctrine.md` — record the Phase 79 per-run ledger artifact and the explicit
  deferral of elevated durable-EBS reclamation to [Phase 91](phase_91_test_topology_live.md).

**Cross-references to add:**

- `DEVELOPMENT_PLAN/system_components.md` — register `Amoebius.Pulumi.Ebs`, `Amoebius.Pulumi.Credential`,
  `Amoebius.Storage.EbsCsi`, and `Amoebius.Storage.ProviderScaling` as Phase-79 design-first rows, each mapped to
  its owning doctrine; note the reused `amoebius-pulumi` Engine + EncryptedMinio backend map to their first
  delivery in [Phase 77](phase_77_provider_deploy_checkpoint.md).
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 79 → `linux-cpu` (parent) row with the `provider` (EKS)
  deploy target annotated as a target class, not a fifth hardware substrate.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase 79 row's status once the gate passes; link this document.

## Related Documents

- [README.md](README.md) — the live tracker; Phase 79 objective, gate, and substrate
...
- [phase_77](phase_77_provider_deploy_checkpoint.md) — the provider-cluster Pulumi deploy + Vault-Transit MinIO
  checkpoint + `observeProviderAccount` this phase spins a cluster with and reuses for the durable-class state
- [phase_61](phase_61_retained_storage.md) — the `no-provisioner` retained PVs + lossless rebind the EBS backs
- [phase_57](phase_57_base_image_registry.md) — the multi-arch baked-binary supply chain this phase extends with
  provider-only CSI binaries
- [phase_29](phase_29_storage_geometry_folds.md) — the storage-scaling policy-only envelope and observe-then-plan
  `Growable`/scaling fold the `CreateProviderCapacity` enactor refines
- [phase_59](phase_59_object_reconciler.md) — the typed SSA reconciler that installs the static CSI and the
  generic fresh-snapshot validation + single-use dispatcher this scaling arm invokes
- [phase_80](phase_80_provider_dynamic_nodes.md) — the dynamic node provisioning + leak-free ephemeral teardown
  gate that layers on this phase (and Phases 76, 77)
- [phase_91](phase_91_test_topology_live.md) — the elevated-harness durable-EBS reclamation that completes the [§7](../documents/engineering/testing_doctrine.md#7-the-elevated-harness-is-the-sole-automated-deleter-of-test-owned-durable-storage-leak-free-cycles)
