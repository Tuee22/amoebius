# Phase 33: Gateway-migration drills + model-correspondence

**Status**: Authoritative source
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_31_network_fabric_wireguard.md, DEVELOPMENT_PLAN/phase_32_multicluster_spawn_georepl.md, DEVELOPMENT_PLAN/phase_42_test_topology_dsl.md, DEVELOPMENT_PLAN/system_components.md
...
prescriptive statement is a target shape, not a tested amoebius result. This phase opens after the Phase 32 gate
...
Because [Phase 32](phase_32_multicluster_spawn_georepl.md) turned the single cluster into a geo-replicated
...
**Substrate:** linux-cpu — the gate drives the migration over the parent and both child clusters that Phase 32
spins up as `kind` clusters on a single linux-cpu host; no accelerator and no provider cluster is in
scope (provider-managed clusters are [Phase 34](phase_34_provider_deploy_checkpoint.md)). Partition tolerance is
...
forest) is authored and committed in `test/dhall/phase_33_gateway_migration.dhall` **in Phase 0, before any
...
- **Committed numeric budget fixture.** `test/dhall/phase_33_gateway_migration.dhall` fixes `lagBound = 5 s`,
...
The runtime executes inside the Phase-26 singleton, whose complete pod envelope is expanded for migration-plan
...
clusters' Phase-25 edge controllers, operator-derived data-plane children, admission webhooks/gateways, and
rollout operands; the source transparent proxy and target serving path coexist through the full DNS drain
window. The Phase-31 `NetworkFabricSystemDemand` is derived for the source, overlap, and target peer/hub
graphs, including kernel/listener CPU and memory, queues, and rotated nodefs logs. The route53 repoint retains
the Phase-32 in-cluster `PulumiExecutionDemand`, exact executor-Job envelopes, plugin/workspace volumes,
...
The non-idle drill workload is provisioned too. Its source-equal Phase-28/29 client/workflow projection carries
...
## Sprint 33.1: The gateway-migration runtime — both branches over the Phase-3 `interpret` core 📋
...
built.
**Blocked by**: Phase 32 (the geo-replicated forest + invariant-confluence classifier, Sprint 32.2); Phase 3
(the `GatewayMigration` `Model` + `interpret` + structural-fit fold); Phase 25 (Keycloak-owned wild ingress via
the LB + Gateway API); Phase 31 (the WireGuard fabric — the `Planned` handover repoints the hub role).
...
1. A `Planned` handover under a **non-idle** workflow (the drill client journals every source-acked write ID to
   ...
   **measured loss = 0 proven by set-equality of journaled-vs-present IDs on the new owner** (not by a
   ...
   those budgets are the Phase-0-committed numeric values in `test/dhall/phase_33_gateway_migration.dhall`
...
## Sprint 33.2: Teardown-with-cleanup vs chaos-failover + unsatisfiable-`.dhall` push-back 📋

**Status**: Planned
**Implementation**: `src/Amoebius/Multicluster/Teardown.hs`, `src/Amoebius/Multicluster/Pushback.hs` — target
paths, not yet built.
**Blocked by**: Phase 32 (the spawn managed-resource registry, Sprint 32.1); Sprint 33.1 (a clean gateway
...
## Sprint 33.3: Register-2.5 gateway-migration runtime fidelity — simulation + trace validation 📋
...
**Docs to update**: `documents/engineering/deterministic_simulation_doctrine.md` (Phase-33 status backlink),
...
simulation against the modeled world here (Register 2.5), then as live Inject drills (Sprint 33.4, Register 3)
...
- A Register-2.5 proven/tested/assumed ledger — the built forest upholds the safety invariants and refines the
  ...
  replication-lag / clock-skew physics remain the Register-3 residue (Sprint 33.4).
...
## Sprint 33.4: Register-3 correspondence — Inject drills against the running forest + live gate `.dhall` + ledger 📋

**Status**: Planned
**Implementation**: `test/dhall/phase_33_gateway_migration.dhall` (the live gate topology) and
...
fold); Phase 32 (the geo-replicated forest); Sprint 33.1; Sprint 33.2; Sprint 33.3 (the Register-2.5 simulation
...
`PlannedIsLossless`, `NoWriteAfterStaleFailover`); and `phase_33_gateway_migration.dhall` spins the forest up,
...
- `test/dhall/phase_33_gateway_migration.dhall`: spin two children up, geo-replicate, run a `Planned` handover
...
1. The built decision core resolves to the Phase-3 `interpret` — the correspondence-check mechanic is fixed as:
   ...
   retained `no-provisioner` PVs of Sprint 33.2 explicitly exempt. The committed seeded mutants ([Gate integrity](#gate-integrity):
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `src/Amoebius/Multicluster/GatewayMigration.hs`,
  ...
  modules and the Register-2.5 simulation + Register-3 Inject + gate suites as Phase-33 rows.
- `DEVELOPMENT_PLAN/substrates.md` — the Phase-33 → linux-cpu row in the per-phase substrate map.
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the Register-2.5 io-sim + trace-validation (Sprint 33.3) that pulls the runtime-fidelity obligation forward from Register-3-only chaos
...
- [phase_32](phase_32_multicluster_spawn_georepl.md) — the prior phase; the geo-replicated forest and confluence classifier this phase runs over
- [phase_34](phase_34_provider_deploy_checkpoint.md) — the next phase; the forest extended to provider-managed clusters
