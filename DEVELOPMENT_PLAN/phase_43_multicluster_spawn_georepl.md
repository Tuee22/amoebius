# Phase 43: Multi-cluster spawn + geo-replication
...
> over which [Phase 44](phase_44_gateway_migration_drills.md) drives the gateway-migration runtime.
> **Read this if**: phase 43 is next in the queue, or a later phase depends on what its gate establishes.

Phase 43 delivers the multi-cluster spawn + geo-replication; its design is owned by [cluster_lifecycle_doctrine.md](../documents/engineering/cluster_lifecycle_doctrine.md), [pulumi_iac_doctrine.md](../documents/engineering/pulumi_iac_doctrine.md), [vault_pki_doctrine.md](../documents/engineering/vault_pki_doctrine.md), and the plan for reaching it is owned here.
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_42_network_fabric_wireguard.md, DEVELOPMENT_PLAN/phase_44_gateway_migration_drills.md, DEVELOPMENT_PLAN/phase_45_provider_deploy_checkpoint.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 43.1: Amoebic spawn — `project(subtree)` handoff + per-child unseal / Transit key / secret injection ⏸️](#sprint-431-amoebic-spawn--projectsubtree-handoff--per-child-unseal--transit-key--secret-injection-)
- [Sprint 43.2: Geo-replication of two siblings + invariant-confluence classification ⏸️](#sprint-432-geo-replication-of-two-siblings--invariant-confluence-classification-)
...
in [Phase 44](phase_44_gateway_migration_drills.md); this phase stands up the geo-replicated forest that
...
Phase 44 still owns correspondence to the live forest and remains UNVERIFIED until its Register-3 gate.
...
non-confluent bucket — is precisely the boundary hand-off the [Phase 44](phase_44_gateway_migration_drills.md)
...
This phase consumes earlier phases and does not re-implement them: Phase 24's `pb` bootstrap of a `kind`
cluster, Phase 30's root Vault/PKI trust anchor, Phases 31–32's platform services (MinIO, Pulsar, Patroni Postgres),
Phase 34's live DSL deploy via the `replicas=1` singleton, Phase 36's native Pulsar client, and Phase 38's
...
The pure `Rke2ServerNode`/`Rke2AgentNode` and reserve/template folds exist in Phases 4–9, but no live
...
[Phase 45](phase_45_provider_deploy_checkpoint.md)). Before either child is created, the parent-owned
...
[Phase 44](phase_44_gateway_migration_drills.md) drills; here the boundary is established and classified.
...
- **External-observer teardown check.** "Tears down leak-free" is scoped for Phase 43 (the flagged-credential +
  postflight tag-sweep machinery of testing_doctrine [§6](../documents/engineering/testing_doctrine.md#6-flagged-test-credentials)–[§7](../documents/engineering/testing_doctrine.md#7-the-elevated-harness-is-the-sole-automated-deleter-of-test-owned-durable-storage-leak-free-cycles) is Phase 55) to: after teardown, an **OS-boundary observer** — `pulumi stack ls` and kubeconfig-context enumeration, read outside the forest — reports zero
...
- [`cluster_lifecycle_doctrine.md §3`](../documents/engineering/cluster_lifecycle_doctrine.md#3-amoebic-spawning--the-recursive-forest)
  ...
  gateway-migration drills of [Phase 44](phase_44_gateway_migration_drills.md).
...
- [`content_addressing_doctrine.md §5`](../documents/engineering/content_addressing_doctrine.md#5-confluence-content-addressed-data-crosses-cluster-boundaries-safely)
  ...
  gateway authority and any CAS "latest" pointer in bucket (ii) for the [Phase 34](phase_44_gateway_migration_drills.md) migration runtime.
- [`chaos_failover_second_axis.md §16`](../documents/engineering/chaos_failover_second_axis.md#16-the-second-axis--when-one-cluster-becomes-a-forest)
  ...
  honest. The R7/R8/R9 boundary rules and the [§19](../documents/engineering/chaos_failover_second_axis.md#19-the-cross-boundary-ledger-and-conformance-rows) cross-boundary ledger are consumed by [Phase 34](phase_44_gateway_migration_drills.md).
...
## Sprint 43.1: Amoebic spawn — `project(subtree)` handoff + per-child unseal / Transit key / secret injection ⏸️
...
## Sprint 43.2: Geo-replication of two siblings + invariant-confluence classification ⏸️
...
(ii) for the [Phase 44](phase_44_gateway_migration_drills.md) migration runtime.
...
- `documents/engineering/cluster_lifecycle_doctrine.md` — §3/§9 gain the realized module paths for the spawn,
  ...
  land with [Phase 44](phase_44_gateway_migration_drills.md)).
...
- [phase_42](phase_42_network_fabric_wireguard.md) — the prior phase (the WireGuard fabric); its gate opens this one
- [phase_44](phase_44_gateway_migration_drills.md) — the next phase; the gateway-migration runtime and correspondence over this forest
- [phase_45](phase_45_provider_deploy_checkpoint.md) — the forest extended to provider-managed clusters
