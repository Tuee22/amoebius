# Phase 46: Hostless provider child + convergence + Lease handoff

> **Purpose**: Bring a provider-managed EKS child — deployed and checkpoint-observed by
> [Phase 45](phase_45_provider_deploy_checkpoint.md) — to the same fungible shape as any self-managed amoebius
...
Phase 46 delivers the hostless provider child + convergence + Lease handoff; its design is owned by [cluster_lifecycle_doctrine.md](../documents/engineering/cluster_lifecycle_doctrine.md), [daemon_topology_doctrine.md](../documents/engineering/daemon_topology_doctrine.md), [image_build_doctrine.md](../documents/engineering/image_build_doctrine.md), and the plan for reaching it is owned here.
...
UNVERIFIED because Phase 45 could not authenticate to AWS.
...
- [Sprints](#sprints)
- [Sprint 46.1: Two-stage capacity bootstrap for a hostless provider child ⏸️](#sprint-461-two-stage-capacity-bootstrap-for-a-hostless-provider-child-)
- [Sprint 46.2: Parent→child Lease handoff + hostless daemon topology ⏸️](#sprint-462-parentchild-lease-handoff--hostless-daemon-topology-)
- [Sprint 46.3: Standard-HA platform-service convergence from typed manifests ⏸️](#sprint-463-standard-ha-platform-service-convergence-from-typed-manifests-)
- [Sprint 46.4: The provider-child bring-up gate ⏸️](#sprint-464-the-provider-child-bring-up-gate-)
...
[Phase 45](phase_45_provider_deploy_checkpoint.md) gate (the provider-cluster Pulumi deploy-from-inside + Vault-Transit-enveloped MinIO checkpoint + `observeProviderAccount`, which lands a ready `Managed Eks` control plane and its base managed node group with the pinned amoebius base/scheduler OCI content already imported and capacity-debited into the first node's CRI store). It runs on the **linux-cpu → provider** substrate in
**Register 3** (live infrastructure): the parent amoebius cluster is a single-node `kind` cluster on linux-cpu
(the Phase 24 bootstrap coordinator), from inside which the Deployment-`replicas=1` singleton (Phase 34) drove the Phase-45
...
convergence** — never the Pulumi deploy/checkpoint that produced its input ([Phase 45](phase_45_provider_deploy_checkpoint.md)),
never the per-PV durable EBS + create-vs-delete credential model ([Phase 47](phase_47_provider_ebs_credential.md)),
and never dynamic node provisioning or the leak-free teardown sweep ([Phase 48](phase_48_provider_dynamic_nodes.md)).
...
First, **the two-stage capacity bootstrap for a hostless child**. Once the EKS API and Phase-45 base node are
...
and capacity-debited into the base node's CRI store by Phase 45, never the not-yet-ready child registry or a
...
substrate the stateless singleton needs), then applies the child singleton **while the parent still holds the Lease** — the child Pod stays non-Serving and cannot mutate. The parent then drains/releases the bootstrap
...
Envoy/Gateway API, Keycloak, cloud LoadBalancer — through the Phase-27 reconciler, **not** a thinner or different
...
flowchart LR
%% register: algebra
  p34["Phase 45: ready Managed Eks control plane + base node, scheduler OCI preloaded"]:::intent --> boot[/"Parent bootstrap Lease holder: create amoebius-capacity-scheduler pods=1"/]:::effect
...
Phase-45 deploy and now drives this convergence; `→ provider` (EKS) is the deploy target class, not a hardware
...
Phase-45-deployed `Managed Eks` child. Exit 0 and a self-reported empty diff cannot satisfy it.
...
`test/negatives/host_shell_pulumi_up.sh` no-in-cluster-singleton negative, and the `mut-44.1-static-key` /
`mut-44.1-leak-path` / `mut-44.1-drop-parallel-executor` mutants) stays in
[Phase 45](phase_45_provider_deploy_checkpoint.md); the per-PV EBS / credential apparatus
...
`mut-46.1-drop-copy-executor` mutants) stays in [Phase 47](phase_47_provider_ebs_credential.md); the dynamic-node
/ quota / teardown-sweep apparatus (`test/dhall/phase_48_provider_over_quota.dhall`, the expected two-instance
identity map and tag-sweep oracle, and the `mut-47.1-*` / `mut-47.2-skip-sweep` mutants) stays in
[Phase 48](phase_48_provider_dynamic_nodes.md). This phase inherits only the convergence/handoff slice below.
...
- **Representative set (§M.7)** — the committed topology `test/dhall/phase_46_provider_provision.dhall`, restricted
  to its **provider-child bring-up + standard-service-convergence slice**: one `Managed Eks` control plane, one
  base managed node group (size 1, deployed by Phase 45), the child `amoebius-capacity-scheduler` (`pods=1`), the
...
2. the **no-Helm / no-public-pull** predicate reads an **OS-boundary observer** (an argv-recording shim on the
   ...
   `test/goldens/convergence_argv.txt`, and is the predicate `mut-45.1-public-pull` turns red;
...
`test/dhall/phase_46_provider_provision.dhall` plus the explicit committed service set). The bootstrap-ordering
...
handoff, and the no-op re-run against a Phase-45-deployed base node group; it does **not** provision a dynamic
extra node ([Phase 48](phase_48_provider_dynamic_nodes.md)), bind a per-PV durable EBS
([Phase 47](phase_47_provider_ebs_credential.md)), or assert the independent leak-free tag-sweep at teardown
([Phase 48](phase_48_provider_dynamic_nodes.md)) — the run tears the base provider stack down through Phase 45's
own deploy lifecycle, and the leak-free sweep witness is recorded **deferred to Phase 48**, never as proven here.
...
- [`cluster_lifecycle_doctrine.md §1`](../documents/engineering/cluster_lifecycle_doctrine.md#1-two-cluster-kinds-one-lifecycle-shape)
  ...
  mandatory capacity-scheduler role — as the child-side of Phase 45's cloud-keyed amoebic spawn, converging the
...
- [`image_build_doctrine.md §2`](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster)
  ...
  the exact OCI digest Phase 45 preloaded into the base node's CRI store, and every standard platform service is
...
- [`illegal_state_catalog.md §3`](../documents/illegal_state/illegal_state_catalog.md#3-the-catalog--states-a-valid-spec-cannot-represent)
  ...
  the pre-cluster band (the Dhall Gate-1 schema and the GADT decoder); this phase **observes that foreclosure at runtime** via the substrate-shape assertion and the committed foreclosure tag.
...
## Sprint 46.1: Two-stage capacity bootstrap for a hostless provider child ⏸️
...
Phase-28 `Amoebius.Scheduler.*` role and its two-stage bootstrap-cutover / execution-identity-admission
...
- Creation of `amoebius-capacity-scheduler` with exact `pods=1`, whose Deployment references the **exact OCI digest preloaded and capacity-debited into the base node's CRI store by Phase 45** — never the not-yet-ready child
...
- The managed-authority mint: install the managed-node taint, execution-identity admission, and full exclusive
  ...
  the cutover.
- An in-file honesty note: this stages a *provider child* through the same two-stage cutover Phase 28 proved on a
  self-managed cluster; the provider realization is validated here for the first time.

### Validation
1. Against a Phase-45-deployed child, the parent bootstrap holder creates `amoebius-capacity-scheduler`
...
### Remaining Work
Run the built protocol against a Phase-45-created EKS control plane and managed node, then read back the real
provider add-on and CRI-preload boundaries. The local and retained-Kubernetes protocol seams are complete.

## Sprint 46.2: Parent→child Lease handoff + hostless daemon topology ⏸️
...
- The handoff: drain/release the parent bootstrap holder, **freshly observe holder absence on that same still-present Lease object**, then admit **only** the authenticated singleton Pod UID to acquire the same Lease
...
## Sprint 46.3: Standard-HA platform-service convergence from typed manifests ⏸️
...
standard-service convergence), converging through the Phase-27 reconciler and consuming the Phase 25/29–33
...
Phase-27 reconciler — reachable, HA, wild ingress only via Keycloak — **not** a thinner or different service
set. The converged live inventory exact-matches the committed `test/goldens/standard_service_set.txt`; an
...
typed manifests via the Phase-27 reconciler, with no Helm and no public-registry pull, so substrate equivalence
...
- Convergence of the **complete** standard HA platform-service stack — registry (`distribution`), MinIO, Vault,
  ...
  LoadBalancer — through the Phase-27 reconciler from typed manifests, using the same HA-capable topology at
...
- An in-file honesty note: the standard set and its HA-capable shape are planned for self-managed clusters in
  Phases 25/29–33; convergence of that exact set on a hostless provider child is tested here for the first time.
...
1. The child reaches the standard-service fungible shape — the **explicit** committed service set (registry,
   ...
   `test/goldens/standard_service_set.txt`. "No Helm, no public-registry pulls" is read from an **OS-boundary observer** (an argv-recording shim on the convergence path plus a CNI/containerd image-pull log or an egress
   ...
   `test/goldens/convergence_argv.txt`. The committed mutant `mut-45.1-public-pull` (a manifest pinned to a
...
## Sprint 46.4: The provider-child bring-up gate ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `test/dhall/phase_46_provider_provision.dhall` (the committed gate
...
`tools/phase45_{provider_child_live,gate}.py`, with its generated ledger under `.build/runs/phase_46/`
(observed footprint; provider OS/cloud observers UNVERIFIED)
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: one end-to-end run takes a Phase-45-deployed `Managed Eks` child from bootstrap
...
per-run ledger; the leak-free sweep deferred to Phase 48), `DEVELOPMENT_PLAN/README.md`.
...
- The gate over the committed representative set (§M.7): the provider-child bring-up + standard-service-convergence
  slice of `test/dhall/phase_46_provider_provision.dhall`, driven end-to-end — bootstrap scheduler readiness,
...
- A per-run proven/tested/assumed ledger recording: provider-child bootstrap + handoff + standard-HA convergence
  ...
  OS-boundary audit; and the elevated-harness leak-free durable-resource *sweep* as **explicitly deferred to Phase 48, not asserted here** — skipping an applicable observation move marks that layer UNVERIFIED, never
  green.

### Validation
1. Run the gate `InForceSpec` end-to-end over `test/dhall/phase_46_provider_provision.dhall`
...
4. Assert the run emits a proven/tested/assumed ledger recording the independent Lease-handoff sequence, the
   ...
   Phase 45's deploy lifecycle and the leak-free tag-sweep is recorded **deferred to Phase 48**, never asserted
   green here.

### Remaining Work
Re-run without `PARTIAL_EXTERNAL_AUTHORITY` once valid AWS authority permits Phase 45 to materialize the EKS
child. Eleven enumerated provider/cloud surfaces remain explicitly UNVERIFIED; Phase 48 still owns the final
...
- `documents/engineering/testing_doctrine.md` — record the Phase 46 per-run ledger artifact and the explicit
  deferral of the elevated leak-free durable-resource sweep to Phase 47.

**Cross-references to add:**
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-runtime` provider-child singleton wiring
  (`Amoebius.Daemon.InClusterSingleton`) and `Amoebius.Cluster.ProviderBringUp` as Phase-46 design-first rows,
  each mapped to its owning doctrine; map the reused `Amoebius.Scheduler.*` role to its Phase 28 delivery and the
  reconciler to Phase 26.
...
- [phase_45](phase_45_provider_deploy_checkpoint.md) — the provider-cluster Pulumi deploy-from-inside +
...
- [phase_34](phase_34_live_dsl_singleton.md) — supplies the singleton role and Lease authority protocol used by
  the provider child
- [phase_47](phase_47_provider_ebs_credential.md) — the per-PV durable EBS + create-vs-delete credential + static
  EBS CSI arm, layered on Phase 45, not exercised here
- [phase_48](phase_48_provider_dynamic_nodes.md) — dynamic node provisioning by signal and the leak-free provider
  gate (the independent teardown tag-sweep), layered on Phases 45/46/47
