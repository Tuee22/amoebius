# Phase 65: Core jitML CUDA artifact lift
...
Phase 65 delivers the core jitML CUDA artifact lift; its design is owned by [lift_and_compose_doctrine.md](../documents/engineering/lift_and_compose_doctrine.md), [capability_extension_doctrine.md](../documents/engineering/capability_extension_doctrine.md), [content_addressing_doctrine.md](../documents/engineering/content_addressing_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 65.1: Produce one committed jitML artifact on CUDA ⏸️](#sprint-651-produce-one-committed-jitml-artifact-on-cuda-)
...
⏸️ Blocked pending Phase-64 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
Phase 42 continues to own Pulsar-Failover takeover and pointer-CAS safety; Phase 53 continues to own the
determinism kernel and bounded first-miss cache. Phase 65 consumes those contracts without adding another
coordinator, election, store, cache, or determinism implementation.

Every scoped training start carries one server-derived `CommandId` unchanged as the Phase-42 workflow
...
revision. Producer resends retain their Phase-40 producer/sequence identity; consumer redelivery and restart
...
The checkpoint adapter uses Phase 42 content digests and canonical component manifests for its pure contract.
...
The output of this phase is the core committed artifact contract. Phase 66 alone owns its downstream
...
live evidence reader, and one aggregate command; no Phase-66 presentation work is included.
...
## Sprint 65.1: Produce one committed jitML artifact on CUDA ⏸️
...
- The canonical checkpoint adapter and opaque `CommittedJitMLArtifact tenant app`, constructible only from a
  successful Phase-42 pointer-CAS witness.
...
- Record only the tested jitML adapter, CUDA owner binding, and pointer-commit contract in the owning
  lift/capability/content/daemon/capacity doctrines; do not restate inherited Phase-42 or Phase-53 proofs.
- Keep downstream artifact presentation and interaction ownership exclusively in Phase 65.
...
- Keep the Phase-66 dependency on this committed-artifact contract explicit without moving presentation work
  into Phase 55.
...
- [Phase 66 — jitML UI lift](phase_66_jitml_ui_lift.md)
