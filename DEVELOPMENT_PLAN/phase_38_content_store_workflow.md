# Phase 38: Content store + workflow runtime (Pulsar-Failover single-writer)

> **Purpose**: Stand up amoebius's durable-artifact substrate — the three-tier content-addressed MinIO store —
> and the orchestrator/worker workflow runtime on top of the Phase-36 native Pulsar client, gated live on
...
- [Sprints](#sprints)
- [Sprint 38.1: Three-tier content-addressed MinIO store ⏸️](#sprint-381-three-tier-content-addressed-minio-store-)
- [Sprint 38.2: Orchestrator/worker workflow runtime + store/fetch by manifest SHA ⏸️](#sprint-382-orchestratorworker-workflow-runtime--storefetch-by-manifest-sha-)
- [Sprint 38.3: Pulsar Failover standby takeover + leak-free teardown (gate) ⏸️](#sprint-383-pulsar-failover-standby-takeover--leak-free-teardown-gate-)
- [Sprint 38.4: Register-2.5 workflow failover takeover under simulated fault ⏸️](#sprint-384-register-25-workflow-failover-takeover-under-simulated-fault-)
...
Pulsar and MinIO already standing as HA platform services (Phase 31) on retained storage (Phase 29), and it
opens only after the Phase 36 gate (the native-protocol Pulsar client, CBOR codec, four subscription types,
...
ML-workflow runtime; those remain sibling context, while the Phase-38 evidence is an amoebius result. The
gate sealed the canonical store, terminal-Job protocol, two native Failover cycles, full three-class cleanup,
256 deterministic schedules, and all ten red mutants. Cross-cluster replication, the Phase-49 experiment-hash
...
`experiment-hash` namespace within an app's Phase-35 ObjectStore bucket. Every namespace also binds a finite
...
charged until an observed GC deletion; the logical peak consumes Phase 31's MinIO erasure/healing and uniform
claim-plan witness rather than assuming logical bytes equal physical disk. Second, an **orchestrator/worker workflow runtime** on top of the Phase-36 client: an orchestrator worker produces a workflow `command` on a
...
This phase also closes the Job-terminal live-proof boundary deliberately left open in Phase 26. Phase 27
...
name-ordered standby, with the Phase-36 at-least-once contract redelivering the un-acked command; the store's
...
typeclass, and SplitMix seed derivation are the Phase 49 determinism kernel, not this phase.
...
- **The critical-window kill:** the active worker is killed **after its store write and before its `event`
  ack** — the same window Sprint 38.4 injects in simulation — and that placement (store-write-done,
...
- **The failed-commit capacity drill:** a separate drill uploads the maximum blob/manifest write set and
  ...
  the Phase-31 per-drive erasure/healing and uniform-claim witness.
- **The first live Phase-27 terminal protocol:** the collector/verification Job's exact completion variant is
...
- **Leak-free teardown and an independent re-run:** the whole topology spins up, runs, and tears down
  leak-free — the postflight sweep inventories every resource class enumerated in Sprint 38.3 and fails hard
...
- [`daemon_topology_doctrine.md §3.1`](../documents/engineering/daemon_topology_doctrine.md#31-exactly-one-pod-is-a-k8setcd-property-not-an-amoebius-election)
  ...
  control-plane singleton (Phase 34), whose single-instance is delegated to k8s/etcd, so nothing in this phase
...
- [`pulsar_client_doctrine.md §5`](../documents/engineering/pulsar_client_doctrine.md#5-the-capability-surface-lookup--produce--consume--subscribe--seek)
  ...
  — *the capability surface (the Failover subscription)* / *at-least-once with broker-side dedup*: the Phase-36
...
- [`deterministic_simulation_doctrine.md §4`](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  — *Register 2.5 — where deterministic simulation sits*: Sprint 38.4 runs the real Sprint-37.2/32.3 workflow
...
- [`chaos_failover_doctrine.md §12`](../documents/engineering/chaos_failover_doctrine.md#12-the-moral-core--proven-tested-assumed)
  ...
  **cross-cluster** failover boundary and its formal model are owned by
  ...
  and realized by Phase 43's geo-replication plus Phase 44's gateway-migration drills, not here — this phase
...
## Sprint 38.1: Three-tier content-addressed MinIO store ⏸️
...
- A mandatory `ObjectStoreDemand` per namespace: exact physical-id-keyed committed residents, structural
  ...
  future/transient extent peak, merges it with all Phase-31 producer arms, and feeds that structure—not a byte
...
- The collector/verification Job consumes the Phase-27 terminal state machine live for the first time. Its
...
- Store keys taken under a caller-supplied `experiment-hash` namespace string within the app's ObjectStore
  ...
  seed derivation (Phase 49 kernel work).
...
1. Run this suite at **Register 3** against the **single-node kind cluster's live MinIO** — the standing
   Phase-31 HA service on the Phase-29 retained PV, never an in-process or local S3 fake, so the evidential
...
2. Encode the same logical manifest from **two writers that each first construct it with a distinct component
   ...
   independent canonicalizer under `.build/runs/phase_38/`. Assert the generated noncanonical case fails
...
the run, and rerun the Phase-38 gate.

## Sprint 38.2: Orchestrator/worker workflow runtime + store/fetch by manifest SHA ⏸️
...
through the Phase-36 dedup. The numbered `### Validation` list below adds the no-election audit and the
...
wire the Phase-36 client, its topology algebra, and the Sprint-37.1 store into an orchestrator/worker runtime
...
### Deliverables
- An orchestrator daemon that, using the Phase-36 topology algebra, produces a workflow `command` on the
  derived topic and consumes the corresponding `event`; it is an unelected worker daemon, not a leader.
- Worker daemons that consume the command, write a content-addressed artifact to the store (Sprint 38.1), and
  produce an `event` carrying the manifest SHA — CBOR payloads throughout (Phase 36), a large artifact carried
...
- The runtime is scheduled under the Deployment-`replicas=1` singleton (Phase 34); no orchestrator/worker role
...
2. Assert a retried produce and a redelivered consume are collapsed by the Phase-36 broker-side dedup so
...
### Remaining Work
None. Delivered and validated by the Phase-38 gate.

## Sprint 38.3: Pulsar Failover standby takeover + leak-free teardown (gate) ⏸️
...
### Deliverables
- Worker daemons attached over a Pulsar **Failover** subscription (Phase 36): one active, the rest
  name-ordered hot standbys; single-writer liveness is the subscription, safety is the store's ETag-CAS commit
  point plus the typed `AdvancePredicate` (Sprint 38.1), so even a bounded failover overlap cannot regress
  HEAD.
- The **critical-window kill-injection path**: the kill lands **after the active worker has completed its store write and before it acks the `event`** (the same window Sprint 38.4 injects in simulation, restated for the
  live gate), so the load-bearing standby re-fetch + bounded failover overlap are actually exercised — not a kill
  against an idle, already-drained worker. Pulsar promotes the name-ordered standby, the Phase-36 at-least-once
...
- The **postflight sweep's explicit inventory contract**: the sweep MUST inventory, and the ledger MUST record,
  ...
  (the durable test-flagged bytes reclaimed by Phase 55). The sweep emits its **full inventory list and the named retained set** into the per-run ledger; **any non-empty remainder outside the retained set is a hard gate failure**. (Durable-byte reclaim staying with Phase 55 is the *only* exemption, and only for the
...
- **Reference and mutant apparatus:** execute the independent no-fault path during the run and retain its
  `pointers/latest` HEAD only beneath `.build/runs/phase_38/`; remove
...
2. **Idempotency and leak-free teardown, disambiguated.** "**Idempotent setup**" means a *second `apply` of the
   topology against the still-standing topology is a byte-stable no-op* (the Phase-27 sense — zero fields
...
> and realized by Phase 43's geo-replication plus Phase 44's gateway-migration drills, not here. Pulsar's own
...
> reclaim of test-flagged durable bytes is the elevated harness's prerogative (Phase 55), kept out of the
...
## Sprint 38.4: Register-2.5 workflow failover takeover under simulated fault ⏸️
...
Phase 15 Sprint 15.2 modeled fault-injectable environment, and assert the same load-bearing properties the Sprint 38.3
...
> only by this phase's **Register-3 live gate** (Sprint 38.3) on the linux-cpu kind cluster — the deterministic
...
- `documents/engineering/content_addressing_doctrine.md` — record that §2 (the three-tier store + the two write
  ...
  `experimentHash` derivation and seed kernel explicitly deferred to Phase 49; note §5 confluence is consumed
...
- `documents/engineering/resource_capacity_doctrine.md` — record the content-store logical peak boundary
  corpus and its consumption of Phase 31's MinIO physical/uniform-claim witness.
- `documents/engineering/storage_lifecycle_doctrine.md` — record that the store's blob/manifest/pointer bytes
  land on the Phase-29 retained PV under the standing Phase-31 MinIO service, and that CAS-loser orphans stay
...
- `documents/engineering/chaos_failover_doctrine.md` — record the §12 per-run proven/tested/assumed ledger for
  the intra-cluster failover injection, and that the §16 cross-cluster boundary stays deferred to Phase 43
  geo-replication plus Phase 44 gateway-migration drills.
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-store` and `amoebius-runtime` packages and
  ...
  Phase-38 design-first rows.

## Related Documents
- [README.md](README.md) — the live tracker; Phase 38 objective, gate, and substrate
...
- [Native Pulsar Client Doctrine](../documents/engineering/pulsar_client_doctrine.md) — the Phase-36 capability
...
- [phase_36](phase_36_pulsar_client.md) — the native Pulsar client this workflow runtime is built on
- [phase_49](phase_49_determinism_jitcache.md) — the `experimentHash` derivation + SplitMix seed kernel deferred
