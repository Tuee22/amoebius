# Phase 46: Native Pulsar client (CBOR)
...
Phase 46 delivers the native Pulsar client (CBOR); its design is owned by [pulsar_client_doctrine.md](../documents/engineering/pulsar_client_doctrine.md), [substrate_doctrine.md](../documents/engineering/substrate_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 46.1: Fork supernova → amoebius-pulsar native binary protocol ⏸️](#sprint-461-fork-supernova--amoebius-pulsar-native-binary-protocol-)
- [Sprint 46.2: Capability surface + exclusively-CBOR payload codec ⏸️](#sprint-462-capability-surface--exclusively-cbor-payload-codec-)
- [Sprint 46.3: Declarative topology algebra + one-sided-link validation ⏸️](#sprint-463-declarative-topology-algebra--one-sided-link-validation-)
- [Sprint 46.4: At-least-once + broker-side dedup + the command→event round-trip gate ⏸️](#sprint-464-at-least-once--broker-side-dedup--the-commandevent-round-trip-gate-)
- [Sprint 46.5: Register-2.5 exactly-once effect under simulated redelivery ⏸️](#sprint-465-register-25-exactly-once-effect-under-simulated-redelivery-)
...
orchestrator/worker workflow runtime with Pulsar-Failover single-writer takeover (both Phase 48), the topic
...
```
*Validated Phase-46 gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*

## Gate integrity

The run happens on a `linux-cpu` kind cluster with Pulsar up as a standard HA service (Phase 41). There the
...
concurrency/rate window, deletion lag, failure/orphan horizon, and admission model); Phase 41 owns the live
...
- **Committed oracle corpus (representative set, §M.7, §M.1).** The gate's representative set is named
  ...
  hand-computed canonical-CBOR byte strings; (c) the non-CBOR negative fixtures of Sprint 46.2 V4 with their
  ...
  strings), authored independently of `topicFor` (§M.3); (e) a pre-run snapshot of the standing Phase-41
...
- **Leak-free is an external enumerate-and-compare sweep (§M.5).** "Leak-free" is defined as: after teardown,
  ...
  bookkeeping) and asserts the set is empty, and asserts the standing Phase-41 namespace's policy set
...
orchestrator/worker. The Phase-46 live gate therefore declares a client-runner Pod with immutable image and OCI
...
bytes, concurrent/rate-window offloads, deletion lag, failed/orphan horizon, mutation admission). The Phase-41
...
## Sprint 46.1: Fork supernova → amoebius-pulsar native binary protocol ⏸️
...
- A structural `PulsarClientExecutionDemand` for connection/request/frame/reconnect peaks, bound into the
  complete envelope of the Phase-46 client-runner Pod and, later, each actual consumer Pod; there is no
...
## Sprint 46.2: Capability surface + exclusively-CBOR payload codec ⏸️
...
seek-based replay — over the persistent session from Sprint 46.1, with **every application payload encoded exclusively as CBOR** through a typed codec.
...
- A typed **CBOR payload codec** (`Amoebius.Pulsar.Cbor`, on `serialise`/`cborg`): `produce`/`consume` accept
  ...
  CBOR (shared with the store's canonical encoder, Phase 48) is used where the payload is content-addressed; a
...
## Sprint 46.3: Declarative topology algebra + one-sided-link validation ⏸️
...
4. Prove the algebra is on the gate path, not dead code (§M.3): the gate topology `round_trip_dedup.dhall`
   carries a committed `RouteEntry` descriptor, and the Sprint 46.4 gate run asserts the actually-produced and
...
## Sprint 46.4: At-least-once + broker-side dedup + the command→event round-trip gate ⏸️
...
Phase-46 status when the gate passes).
...
3. **Idempotency (§M.6) — re-apply, not re-run-from-clean.** Re-apply the topology a second time against a
   **distinct test namespace** (cache-bypass: no reuse of run 1's namespace, cursors, producer name, or dedup
   ...
   tenant contains **zero** topics, subscriptions/cursors, and namespaces, and asserts the standing Phase-41
   ...
   ledger — the deferred content-store, workflow-runtime, and cross-cluster surfaces (Phase 48 and later)
...
## Sprint 46.5: Register-2.5 exactly-once effect under simulated redelivery ⏸️
...
**Docs to update**: `documents/engineering/deterministic_simulation_doctrine.md` (Phase-46 status backlink),
...
- A Register-2.5 proven/tested/assumed ledger — the fold upholds exactly-once under the modeled schedules and
  faults; honest limit: modeled-Pulsar fidelity is **assumed**, discharged by the Sprint 46.4 Register-3 live
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-pulsar` package and its target module paths
  ...
  `Topology`, `Dedup`, `Namespace`) as Phase-46 design-first rows against the component inventory.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 46's gate substrate (`linux-cpu`) in the per-phase substrate
  map, and the topology algebra's per-substrate topic lanes.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-46 row's status from this plan once the gate passes; link this
  document.
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the Register-2.5 io-sim environment the dedup fold's exactly-once is validated against in Sprint 46.5
...
- [Daemon Topology Doctrine](../documents/engineering/daemon_topology_doctrine.md) — who runs producers/
  consumers and picks subscription roles (Phase 48), delegated here
- [phase_41](phase_41_platform_backbone.md) — the standard-service stack that brings Pulsar up HA
- [phase_45](phase_45_app_tenancy.md) — the app tenancy this phase opens after
- [phase_48](phase_48_content_store_workflow.md) — the content store + workflow runtime that consumes this
  client
