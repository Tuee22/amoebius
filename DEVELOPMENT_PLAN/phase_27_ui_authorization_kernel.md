# Phase 27: UI authorization kernel
...
Phase 27 delivers the UI authorization kernel; its design is owned by [low_code_ui_runtime_doctrine.md](../documents/engineering/low_code_ui_runtime_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 27.1: Sealed action registry and authorized-action transition ✅](#sprint-271-sealed-action-registry-and-authorized-action-transition-)
...
`sha256:ff29cbf28bdb6886…`; Phase 27 owns no remaining migration deferral.
...
Evidence and the ledger move into `.build/runs/phase_21/<run-id>/`, and 40 surfaces join two-way to 57 run-time
...
the relation. See the Phase-27 ledger.
...
**Depends on:** [Phase 26](phase_26_scoped_identity_kernel.md) — scope-indexed request contexts, handles, audiences, and flow witnesses.
...
- **Effect discipline:** the pure reference interpreter records an effect only after `AuthorizedAction` exists;
  ...
  applicable in Register 1 and are deliberately deferred to Phases 47 and 63.
...
## Sprint 27.1: Sealed action registry and authorized-action transition ✅
...
None in Phase 27. Live identity, UI-server, policy-provider, and tenant-isolation enforcement remains owned by
...
- [Phase 26](phase_26_scoped_identity_kernel.md) — the required scoped identity and flow kernel.
