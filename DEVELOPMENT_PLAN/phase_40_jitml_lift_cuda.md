...
the Phase 39 gate (the infernix CPU-inference lift and its application-logic-only demo web app). It consumes
earlier phases rather than re-implementing them: Phase 29's three-tier content-addressed store and the
Pulsar-Failover single-writer workflow runtime, Phase 38's determinism kernel (`deriveExperimentHash` +
SplitMix seed derivation), Phase 38's jit-build engine resolver and `CacheBudget`-bounded content-addressed
cache, and Phase 28's native Pulsar CBOR client. The mechanisms it lifts exist only as **sibling evidence, not
...
checkpoint blob/manifest/pointer format becomes entries in the Phase-29 three-tier content-addressed store; the
`jitML` `.dhall` **nests inside** the `InForceSpec` as a shared library whose surface carries no replica count,
region, substrate selector, or failover field; and the SplitMix determinism kernel is consumed from Phase 38
...
and **jit-resolved** — a named catalog identity the Phase-38 resolver materializes on first miss into the
...
the Phase-40 fixture), derived as the selected node offering's wholesale device count, and gives its pod the
...
[Phase 32](phase_32_multicluster_spawn_georepl.md); a Continuous run here is single-cluster, and other
...
flowchart LR
  ...
  up --> engine[CUDA engine jit-resolved into the Phase-38 CacheBudget cache: not baked, no URL]
  ...
  train --> ckpt[Checkpoints written to the Phase-29 three-tier content store]
...
- **Representative set (§M.7).** The determinism gate runs a **named, committed workload corpus** —
  ...
  (off-policy RL) — each pinned in `test/dhall/phase_40_det_workloads.dhall` (committed Phase 0) with a floor of
...
CUDA node, (2) the Phase-38 cache owner surviving on that node, (3) the sole accelerator-owner Pod and its
...
declared CNI/IP mode. This is either an exact Phase-17 linux-cuda survivor witness or a prerequisite provision;
...
container that performs the call; no client Pods are invented. The source-equal Phase-28/29 projection retains
...
- [`content_addressing_doctrine.md` §4.5](../documents/engineering/content_addressing_doctrine.md#45-the-ml-asset-lifecycle-one-bounded-content-addressed-cache-resolved-on-first-miss)
  ...
  topology*: the CUDA engine is a named catalog identity the Phase-38 resolver materializes on first miss into
...
- [`chaos_failover_doctrine.md` §12](../documents/engineering/chaos_failover_doctrine.md#12-the-moral-core--proven-tested-assumed)
  ...
  and scheduled for Phase 32, not here.
...
## Sprint 40.1: jitML lifted as a shared library — checkpoints on the Phase-29 content store 📋
...
manifest encoder mapped onto the Phase-29 store.
**Blocked by**: Phase 29 gate (external prereq — the three-tier content-addressed MinIO store and its
blob/manifest/pointer write protocols); Phase 28 gate (external prereq — the native Pulsar CBOR client the
...
re-home the sibling jitML checkpoint format onto the Phase-29 store and expose the training/JIT call graph as a
...
- `blobKey`/`manifestKey`/`latestPointerKey`/`bestPointerKey`/`trialPointerKey` renderers producing the fixed
  prefix schema under the app's Phase-27 ObjectStore bucket (`jitml-checkpoints/<experiment-hash>/…`), and a
  **canonical** `encodeManifestCbor` (tensors sorted by name, optimizer blobs by kind, RNG blobs by stream id,
  ...
  Phase-29 blob/manifest PUT + pointer CAS protocols.
...
## Sprint 40.2: CUDA engine — substrate-selected, jit-resolved, run under the accelerator-owner worker 📋
...
**Blocked by**: Phase 38 gate (external prereq — the jit-build engine resolver + `CacheBudget`-bounded
content-addressed cache the CUDA engine identity resolves into); Phase 38 gate (external prereq — the
determinism kernel the engine dispatch respects); Sprint 40.1 (the jitML library the engine backs).
...
- The CUDA engine payload delivered as a **named catalog identity** the Phase-38 resolver materializes on first
...
1. `cabal build` on a host with no CUDA toolchain compiles and links the CPU path; on the linux-cuda host the
   CUDA engine resolves its named identity into the Phase-38 cache on first miss and a second pod reuses it —
...
## Sprint 40.3: jitML training bit-determinism on linux-cuda 📋

**Status**: Planned
**Implementation**: `src/Amoebius/JitML/Determinism.hs` (target path; not yet built) — the jitML binding of the
Phase-38 `deriveExperimentHash` and SplitMix seed streams to the linux-cuda substrate fingerprint and the
training stream set.
**Blocked by**: Sprint 40.1 (checkpoints on the store), Sprint 40.2 (the CUDA engine the run dispatches on);
Phase 38 gate (external prereq — the determinism kernel: `experimentHash` + SplitMix seed derivation).
...
- Per-stream SplitMix seeds for the full jitML training stream set (per-experiment, per-game RL self-play,
  per-HPO-trial, MCTS root noise) via the Phase-38 `deriveSplitMixSeed`, with the in-types property that a
...
- The honest ceiling encoded as the determinism *contract* this run is checked against: same-substrate
  ...
  **Off-policy RL is a mandatory gate component, not conditional**: the committed `det-offpolicy` workload
  ...
  resulting `N` are fixed in `test/dhall/phase_40_det_workloads.dhall` (committed Phase 0).
...
## Sprint 40.4: Feed-sourced single-writer trainer — delegated failover, never elected 📋
...
**Blocked by**: Sprint 40.1 (the checkpoint store and its `latest` pointer CAS); Phase 29 gate (external prereq
— the orchestrator/worker workflow runtime and the Failover standby-takeover path); Phase 28 gate
...
- An explicit non-goal note in the module and docs: this is single-cluster and **not** an election, **not** the
  ...
  Phase 32); candidates share no in-memory state and coordinate only through Pulsar + the content store.
...
## Sprint 40.5: Demo web app application-logic-only + the linux-cuda gate 📋
...
never-committed `test/dhall/phase_40_jitml_cuda.dhall` and drives it)
(target paths; not yet built) — the demo app deployment and the end-to-end gate topology.
**Blocked by**: Sprint 40.3 (bit-determinism), Sprint 40.4 (the delegated Feed trainer failover); Phase 39 gate
(external prereq — the infernix demo web app pattern this reuses); Phase 16 (the representational SPA-composition
fixtures).
...
- The gate `.dhall` (`test/dhall/phase_40_jitml_cuda.dhall`, emitted from Haskell, never committed): bring up
...
- A proven/tested/assumed ledger artifact per [`chaos_failover_doctrine.md` §12](../documents/engineering/chaos_failover_doctrine.md#12-the-moral-core--proven-tested-assumed):
  ...
  the cross-cluster boundary *not asserted* (deferred to Phase 32); GPU-float physics *assumed* (sibling
...
- `documents/engineering/lift_and_compose_doctrine.md` — record that the jitML numerical/checkpoint core and the
  SplitMix kernel are lifted onto the amoebius seams (Phase-29 store, Phase-38 kernel, native CBOR client), the
...
- `documents/engineering/content_addressing_doctrine.md` — confirm §2/§3/§4 and the §6 ceiling are exercised on
  a GPU substrate, and that the §4.5 CUDA engine stayed jit-resolved into the Phase-38 cache (never baked, never
...
- `documents/engineering/chaos_failover_doctrine.md` — record the §12 per-run ledger for the intra-cluster
  trainer-kill injection, and that the §16 cross-cluster boundary stays deferred to Phase 32.

**Cross-references to add:**
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-40 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 40's gate substrate (linux-cuda, the first GPU substrate) in
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `Amoebius.JitML.*`, `Amoebius.Accelerator.Owner`, and
  the CUDA engine + Feed-trainer + gate modules as Phase-40 design-first rows.

## Related Documents
- [README.md](README.md) — the live tracker; the Phase 40 row is the authoritative one-line gate and status
...
- [phase_29](phase_29_content_store_workflow.md) — the content store + Pulsar-Failover workflow runtime this
  phase's checkpoints and Feed trainer build on
- [phase_38](phase_38_determinism_jitcache.md) — the `experimentHash` + SplitMix determinism kernel this phase
  binds to the linux-cuda fingerprint
- [phase_38](phase_38_determinism_jitcache.md) — the jit-build engine resolver + `CacheBudget` cache the CUDA
  engine is materialized into
- [phase_39](phase_39_infernix_lift.md) — the adjacent infernix CPU-inference lift and its application-logic-only
  demo web app
- [phase_41](phase_41_apple_metal_host_daemon.md) — the Apple-Metal host worker; the windows-CUDA host worker
  named here is its structurally identical case on a different substrate
- [phase_32](phase_32_multicluster_spawn_georepl.md) — the cross-cluster gateway-migration boundary and its
