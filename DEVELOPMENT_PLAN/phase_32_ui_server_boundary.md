...
- [Sprints](#sprints)
- [Sprint 32.1: Authenticated scoped UI-server dispatch ✅](#sprint-321-authenticated-scoped-ui-server-dispatch-)
...
remain beneath `.build/**`; the outside-host inventory is unchanged; Phase 32 owns no migration deferral. The
...
by the tree regaining a second executable — which is Phase 44's closure, not a property this gate can
...
Phase 32 in the legacy register.
...
Phase-31 fixture. The private `UiServerPlan`, dispatch table, policy graph, and server codecs have no asset
route.
...
stood up in Phase 41. Redis identifiers never enter handler input or establish authorization.
...
**Depends on:** [Phase 29](phase_29_ui_plan_compiler.md) — canonical `UiServerPlan`, public contracts, route dispatch, and authority digests.
...
HA layers UNVERIFIED, and Phase 33's server branch waits on it. The apparatus is
[Gate integrity](#gate-integrity).
...
- **Pinned oracles:** `test/fixture/ui_server/requests.tsv`, `access_matrix.tsv`, `expected_http.tsv`,
  ...
  state, origin, or authority epoch. Asset/plan responses must exactly match the Phase-31 browser-enforced
  header policy.
...
## Sprint 32.1: Authenticated scoped UI-server dispatch ✅
...
- Phase 33 — compose this boundary with the generic browser without replacing either oracle.

## Related Documents

- [Phase 29](phase_29_ui_plan_compiler.md) — the required immutable `UiServerPlan` and dispatch contracts.
