# Phase 42: Content store + workflow runtime (Pulsar-Failover single-writer)

> **Purpose**: Stand up amoebius's durable-artifact substrate — the three-tier content-addressed MinIO store —
> and the orchestrator/worker workflow runtime on top of the Phase-40 native Pulsar client, gated live on
...
- [Sprints](#sprints)
- [Sprint 42.1: Three-tier content-addressed MinIO store ⏸️](#sprint-421-three-tier-content-addressed-minio-store-)
- [Sprint 42.2: Orchestrator/worker workflow runtime + store/fetch by manifest SHA ⏸️](#sprint-422-orchestratorworker-workflow-runtime--storefetch-by-manifest-sha-)
- [Sprint 42.3: Pulsar Failover standby takeover + leak-free teardown (gate) ⏸️](#sprint-423-pulsar-failover-standby-takeover--leak-free-teardown-gate-)
- [Sprint 42.4: Register-2.5 workflow failover takeover under simulated fault ⏸️](#sprint-424-register-25-workflow-failover-takeover-under-simulated-fault-)
...
`kind` cluster brought up by the Phase 29 bootstrap coordinator with
Pulsar and MinIO already standing as HA platform services (Phase 35) on retained storage (Phase 33), and it
opens only after the Phase 40 gate (the native-protocol Pulsar client, CBOR codec, four subscription types,
...
ML-workflow runtime; those remain sibling context, while the Phase-42 evidence is an amoebius result. The
gate sealed the canonical store, terminal-Job protocol, two native Failover cycles, full three-class cleanup,
256 deterministic schedules, and all ten red mutants. Cross-cluster replication, the Phase-53 experiment-hash
...
`experiment-hash` namespace within an app's Phase-39 ObjectStore bucket. Every namespace also binds a finite
...
charged until an observed GC deletion; the logical peak consumes Phase 35's MinIO erasure/healing and uniform
claim-plan witness rather than assuming logical bytes equal physical disk. Second, an **orchestrator/worker workflow runtime** on top of the Phase-40 client: an orchestrator worker produces a workflow `command` on a
...
This phase also closes the Job-terminal live-proof boundary deliberately left open in Phase 31. Phase 31
...
name-ordered standby, with the Phase-40 at-least-once contract redelivering the un-acked command; the store's
...
- **The critical-window kill:** the active worker is killed **after its store write and before its `event`
  ack** — the same window Sprint 42.4 injects in simulation — and that placement (store-write-done,
...
- **The failed-commit capacity drill:** a separate drill uploads the maximum blob/manifest write set and
  ...
  the Phase-35 per-drive erasure/healing and uniform-claim witness.
- **The first live Phase-31 terminal protocol:** the collector/verification Job's exact completion variant is
...
- **Leak-free teardown and an independent re-run:** the whole topology spins up, runs, and tears down
  leak-free — the postflight sweep inventories every resource class enumerated in Sprint 42.3 and fails hard
...
- [`pulsar_client_doctrine.md §5`](../documents/engineering/pulsar_client_doctrine.md#5-the-capability-surface-lookup--produce--consume--subscribe--seek)
  ...
  — *the capability surface (the Failover subscription)* / *at-least-once with broker-side dedup*: the Phase-40
...
- [`deterministic_simulation_doctrine.md §4`](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  — *Register 2.5 — where deterministic simulation sits*: Sprint 42.4 runs the real Sprint-37.2/32.3 workflow
...
- [`chaos_failover_doctrine.md §12`](../documents/engineering/chaos_failover_doctrine.md#12-the-moral-core--proven-tested-assumed)
  ...
  **cross-cluster** failover boundary and its formal model are owned by
  ...
  and realized by Phase 47's geo-replication plus Phase 48's gateway-migration drills, not here — this phase
...
- A mandatory `ObjectStoreDemand` per namespace: exact physical-id-keyed committed residents, structural
  ...
  future/transient extent peak, merges it with all Phase-35 producer arms, and feeds that structure—not a byte
...
- The collector/verification Job consumes the Phase-31 terminal state machine live for the first time. Its
...
- Store keys taken under a caller-supplied `experiment-hash` namespace string within the app's ObjectStore
  ...
  seed derivation (Phase 53 kernel work).
...
1. Run this suite at **Register 3** against the **single-node kind cluster's live MinIO** — the standing
   Phase-35 HA service on the Phase-33 retained PV, never an in-process or local S3 fake, so the evidential
...
the run, and rerun the Phase-42 gate.
...
through the Phase-40 dedup. The numbered `### Validation` list below adds the no-election audit and the
...
wire the Phase-40 client, its topology algebra, and the Sprint-37.1 store into an orchestrator/worker runtime
...
### Deliverables
- An orchestrator daemon that, using the Phase-40 topology algebra, produces a workflow `command` on the
  derived topic and consumes the corresponding `event`; it is an unelected worker daemon, not a leader.
- Worker daemons that consume the command, write a content-addressed artifact to the store (Sprint 42.1), and
  produce an `event` carrying the manifest SHA — CBOR payloads throughout (Phase 40), a large artifact carried
...
2. Assert a retried produce and a redelivered consume are collapsed by the Phase-40 broker-side dedup so
...
### Remaining Work
None. Delivered and validated by the Phase-42 gate.
...
### Deliverables
- Worker daemons attached over a Pulsar **Failover** subscription (Phase 40): one active, the rest
  name-ordered hot standbys; single-writer liveness is the subscription, safety is the store's ETag-CAS commit
  point plus the typed `AdvancePredicate` (Sprint 42.1), so even a bounded failover overlap cannot regress
  HEAD.
- The **critical-window kill-injection path**: the kill lands **after the active worker has completed its store write and before it acks the `event`** (the same window Sprint 42.4 injects in simulation, restated for the
  live gate), so the load-bearing standby re-fetch + bounded failover overlap are actually exercised — not a kill
  against an idle, already-drained worker. Pulsar promotes the name-ordered standby, the Phase-40 at-least-once
...
- The **postflight sweep's explicit inventory contract**: the sweep MUST inventory, and the ledger MUST record,
  ...
  (the durable test-flagged bytes reclaimed by Phase 56). The sweep emits its **full inventory list and the named retained set** into the per-run ledger; **any non-empty remainder outside the retained set is a hard gate failure**. (Durable-byte reclaim staying with Phase 56 is the *only* exemption, and only for the
...
2. **Idempotency and leak-free teardown, disambiguated.** "**Idempotent setup**" means a *second `apply` of the
   topology against the still-standing topology is a byte-stable no-op* (the Phase-31 sense — zero fields
...
> and realized by Phase 47's geo-replication plus Phase 48's gateway-migration drills, not here. Pulsar's own
...
> reclaim of test-flagged durable bytes is the elevated harness's prerogative (Phase 56), kept out of the
...
Phase 16 Sprint 16.2 modeled fault-injectable environment, and assert the same load-bearing properties the Sprint 42.3
...
> only by this phase's **Register-3 live gate** (Sprint 42.3) on the linux-cpu kind cluster — the deterministic
...
- `documents/engineering/content_addressing_doctrine.md` — record that §2 (the three-tier store + the two write
  ...
  `experimentHash` derivation and seed kernel explicitly deferred to Phase 53; note §5 confluence is consumed
...
- `documents/engineering/resource_capacity_doctrine.md` — record the content-store logical peak boundary
  corpus and its consumption of Phase 35's MinIO physical/uniform-claim witness.
- `documents/engineering/storage_lifecycle_doctrine.md` — record that the store's blob/manifest/pointer bytes
  land on the Phase-33 retained PV under the standing Phase-35 MinIO service, and that CAS-loser orphans stay
...
- `documents/engineering/chaos_failover_doctrine.md` — record the §12 per-run proven/tested/assumed ledger for
  the intra-cluster failover injection, and that the §16 cross-cluster boundary stays deferred to Phase 47
  geo-replication plus Phase 48 gateway-migration drills.
...
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 42's gate substrate (linux-cpu) in the per-phase substrate map.
...
- [Native Pulsar Client Doctrine](../documents/engineering/pulsar_client_doctrine.md) — the Phase-40 capability
...
- [phase_40](phase_40_pulsar_client.md) — the native Pulsar client this workflow runtime is built on
- [phase_53](phase_53_determinism_jitcache.md) — the `experimentHash` derivation + SplitMix seed kernel deferred
