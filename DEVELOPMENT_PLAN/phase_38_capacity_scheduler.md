# Phase 38: amoebius-capacity scheduler + bootstrap cutover

> **Purpose**: Build the same-binary `amoebius-capacity` scheduler as a dedicated role layered on the
> Phase-37 reconciler — the state-indexed reservation ledger, the two-stage bootstrap taint/RBAC cutover
...
- [Sprints](#sprints)
- [Sprint 38.1: State-indexed reservation ledger + normalization + absent-Pod recovery arms ✅](#sprint-381-state-indexed-reservation-ledger--normalization--absent-pod-recovery-arms-)
- [Sprint 38.2: Scheduler bootstrap authority + two-stage taint/RBAC cutover + readiness witnesses ✅](#sprint-382-scheduler-bootstrap-authority--two-stage-taintrbac-cutover--readiness-witnesses-)
- [Sprint 38.3: Scheduler loop + `Reserved`→`BindingInFlight`→Binding→`Bound` CAS + placement + recovery ✅](#sprint-383-scheduler-loop--reservedbindinginflightbindingbound-cas--placement--recovery-)
- [Sprint 38.4: Live scheduler binding + bootstrap→steady cutover gate ✅](#sprint-384-live-scheduler-binding--bootstrapsteady-cutover-gate-)
- [Sprint 38.5: Register-2.5 scheduler convergence under simulated faults ✅](#sprint-385-register-25-scheduler-convergence-under-simulated-faults-)
...
Blocked (superseded) — reopened 2026-08-16 behind the amended Phase-36 handoff and Phase-37 revalidation. Its prior capability record remains historical until the exact predecessor chain is resealed.
...
The run verified Phase 37 attestation `sha256:85cf27abdea88522d4b4e5d3941d22d11fe68c6ed53fd6ec43bab1305643f9f1`,
Phase 36 attestation `sha256:e6ff9163d299b66b8ce77c216d5114e13af6fe4beb7e299834ee390e9963816a`,
...
content-store phase, and the control-plane daemon that will own the scheduler is [Phase 44](phase_44_live_dsl_deploy.md)'s.

*The gate was unrunnable in the same four ways Phase 37's was.* It read five receipts, four mutant batteries,
...
and cabal and the compiler resolve per run. `tools/capacity_scheduler_live.py` pinned the Phase-36 base image
...
live reconciler: Phase 37 converges the object corpus with its Pods scheduled by the default scheduler and
...
- **2026-08-09 — Sprint 38.1 complete.** The pinned nine-action scheduler oracle and the read-only
...
- **2026-08-09 — Sprint 38.2 complete.** Exact reconciler-Lease-holder admission minted a single-use
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 38.3 complete.** The scheduler loop authenticated the protected Pod identity,
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 38.4 complete.** A live Kubernetes admission guard rejected premature and
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 38.5 complete.** Seven scheduler fault classes each replayed 256 byte-identical
  ...
  and modeled-apiserver fidelity remains explicitly assumed, bounded by Sprint 38.4's separate live run.
  Receipt: `sprint-27.5-receipt.json`, fingerprint
  `dynamically-resolved`.
- **2026-08-09 — Phase 38 gate complete.** The phase command repeated the live/source-mutation and
...
The scheduler owns the **state-indexed reservation ledger**. On top of Phase 37's `ObservedLiveResourceSnapshot`
...
unique-node affinity, the exact `amoebius-capacity-scheduler` namespace `ResourceQuota pods=1`, the Phase-36
...
capability can only patch the finite observed distro/Phase-36 bootstrap-controller set to
...
resident/whole-root/candidate resource algebra (over Phase 14's `place` fold) under one aggregate CAS;
...
those are Phase 37's and are consumed here as the substrate the scheduler runs on. It owns only the scheduling
authority. The scheduler is driven from the **host binary** against the same scratch namespace as Phase 37; the
...
and WSL2 on Windows. This execution uses Phase 35's single-node `kind` cluster and the Phase-37 reconciler.
...
```
*Orientation for the ordered scheduler delivery owned by the [Phase 38 plan](phase_38_capacity_scheduler.md).*
...
this seam; Phase 37 owns the object/convergence slice of the same corpus, and the two do not duplicate each
other. All identifiers are oracle-pinned before `Scheduler/Ledger.hs` exists (§M.1 oracle-pinning), except the
scheduler-role fixtures that depend on the Phase-36 registry/preloaded image and the Phase-35 live cluster,
...
flowchart LR
  ...
  g{{"the phase 38 gate command"}}:::gate
...
```
*Design intent. Phase 38's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*

**Inherited committed fixtures (§M.1/§M.7 concrete corpus).** This phase reuses
[Phase 37's concrete reconcile corpus](phase_37_object_reconciler.md) rather than selecting new service specs. It
...
- the **finite observed distro/Phase-36 bootstrap-controller set** that is patched from the default scheduler
  to `schedulerName=amoebius-capacity` during the cutover;
- corpus item **(i)** — the guarded Deployment whose container image is pulled from the Phase-36 in-cluster
...
- a committed **premature-guarded-workload negative** (§M.8): a Pod naming `amoebius-capacity` / tolerating the
  ...
  OnDelete, (iii) the Job, (iv) the Ready/Available object, and (v) the CustomResource are Phase 37's slice and
...
code under test: **no `Binding` request precedes a successful `Reserved` CAS**; **every guarded Pod UID has exactly one reservation debit** (no Pod+ledger double debit); at every audit resourceVersion **at most one holder** and no non-authority write without the exact mandatory-`Lease` holder (held by the Phase-37
...
`CapacitySchedulerSystemDemand` is the scheduler's own explicit, pure-first provision, fitted by the Phase-14
`place` fold before any effect, so the scheduler is never silently free overhead:
- **Image** — the Phase-36 side-loaded/preloaded native-architecture amoebius base image (never a public-registry pull),
...
- [`manifest_generation_doctrine.md §5`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  authenticated deletion, and the rollback/release ledger are Phase 37's or stay deferred.
...
- [`resource_capacity_doctrine.md §8`](../documents/engineering/resource_capacity_doctrine.md#8-where-the-numbers-come-from-declared-in-pure-input-provisioned-before-render-cross-checked-at-runtime)
  ...
  static/foreign/resident/whole-ledger/candidate resource algebra over the Phase-14 `place` fold and re-observes
...
- [`daemon_topology_doctrine.md §3`](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-daemon)
  ...
  `Lease`, no bespoke election) is stood up in Phase 43.
...
## Sprint 38.1: State-indexed reservation ledger + normalization + absent-Pod recovery arms ✅
...
ledger join and normalization that extends Phase 37's `src/Amoebius/Execution/Normalize.hs` observed
...
— the reconcile state model, scheduler-ledger slice. Extend Phase 37's observed inventory with the
...
- A scheduler-ledger join over full `Reserved | BindingInFlight | Bound | Terminating | TerminalRetained`
  records plus resourceVersion/CAS version, added to Phase 37's `ObservedLiveResourceSnapshot` (empty before
...
- The host-aware observed identity union consumed from Phase 37 —
...
- State-sensitive Binding recovery at normalization time: an unknown/unbound `BindingInFlight` row remains
  ...
  Pod-UID runtime-storage row (from Phase 37's `RuntimeStorage`) and exact-joins it once with the
  still-`BindingInFlight` reservation — the later repair CAS (Sprint 38.3) changes state, not capacity.
- The scheduler-ledger slice of the `ValidatedLiveTarget`: the ledger CAS version, the normalized
  ...
  handed to Phase 37's target constructor. `PlannedExecutionSlotId` is never equated with a Pod UID; every
...
2. Positive recovery fixtures cover absent-Pod rows in **every** closed ledger state and prove each remains
   charged until its state-specific CAS. The host-ledger controls reuse the Phase-37 failure classes: missing
...
## Sprint 38.2: Scheduler bootstrap authority + two-stage taint/RBAC cutover + readiness witnesses ✅
...
scheduler-system-only token; the cold-start `Lease` authority itself is Phase 37), and
...
- A read-only scheduler preflight (layered on the Phase-37 reconciler already holding
  ...
  the Phase-36 side-loaded/preloaded amoebius image, has unique-node affinity, and merges its static owner row
...
- `BootstrapCapacitySchedulerReady`: exact active generation/config/root readback mints it **while the managed taint, general identity admission, and full Binding authority remain absent**. Its restricted capability can
  only patch the exact observed distro/Phase-36 bootstrap-controller set to `schedulerName=amoebius-capacity`.
...
3. `ceil(renewalWindow/retryPeriod) <= maxRenewalsPerWindow` is not re-proven here (the `Lease` is Phase 37's),
   but the scheduler preflight asserts it runs **only** while the Phase-37 reconciler is the exact `Lease`
...
## Sprint 38.3: Scheduler loop + `Reserved`→`BindingInFlight`→Binding→`Bound` CAS + placement + recovery ✅
...
- The scheduler loop for `schedulerName=amoebius-capacity` (`Scheduler/Loop.hs`): authenticate Pod UID,
  ...
  static/foreign/resident + whole-root + candidate resource algebra (`Scheduler/Placement.hs` over the Phase-14
...
- The reservation transitions are the scheduler's members of Phase 37's `ValidatedExecutionTransitionAction`
  domain; the scheduler never enters Phase 37's generic scoped-SSA path (`fieldManager=amoebius`), and every
...
2. The seeded mutants `bind-before-reservation-CAS`, `numeric-add-instead-of-whole-ledger-refold`,
   ...
   digest or collapses the two witnesses (from Sprint 38.2) also turns the reservation suite red when it lets a
...
4. The scheduler modules import no generic SSA writer; the lint asserts scheduler/ledger/Binding actions never
   enter Phase 37's `Manifest/Apply.hs` scoped-SSA module, and `NoOp` carries no writer capability.

### Remaining Work

None. The receipt and mutation results are written into this run's bundle under `.build/runs/`.

## Sprint 38.4: Live scheduler binding + bootstrap→steady cutover gate ✅
...
`test/spec/live/SchedulerBootstrapCutoverSpec.hs`, driving the Sprint 38.1–28.3 `Scheduler/*` and
`Admission/ExecutionIdentity` modules against the live Phase-35 `kind` cluster and the converged Phase-37
...
`DEVELOPMENT_PLAN/README.md` (flip the Phase-38 status when the gate passes).
...
- A Register-3 proven/tested/assumed ledger recording the live scheduling authority, marking the
  release-ledger/rollback residue and the in-cluster-control-plane ownership (Phase 44) UNVERIFIED (deferred).
...
## Sprint 38.5: Register-2.5 scheduler convergence under simulated faults ✅
...
`Admission/ExecutionIdentity` modules through `test/spec/sim/CapacitySchedulerSimCommon.hs` on the Phase-21
`io-classes` `Env` / modeled apiserver. This is the scheduler slice of the deterministic-simulation battery; Phase 37 owns the
...
**Docs to update**: `documents/engineering/deterministic_simulation_doctrine.md` (Phase-38 status backlink),
...
below, rather than earlier in time than, Sprint 38.4's live Register-3 gate and covers concurrency behavior that
...
- A Register-2.5 ledger records convergence and fail-closed outcomes only for the explored scheduler traces.
  Fidelity of the simulated apiserver remains **assumed** and is bounded by Sprint 38.4's separate live
...
- `documents/engineering/manifest_generation_doctrine.md` — §5's amoebius scheduler-role CAS/Binding protocol
  ...
  delete/wait engine remains Phase 37's; the rollback/release ledger stays deferred.
...
- `documents/engineering/readiness_ordering_doctrine.md` — the §6 runtime-enactor claim (observe, never sleep)
  ...
  slept).
- `documents/engineering/daemon_topology_doctrine.md` — record that Phase 38 drives the scheduler role from the
  host binary; the §3 control-plane daemon that *owns* both the reconciler and its scheduling role (Deployment
  `replicas=1`, delegated single-instance, no election) is stood up in Phase 43.
- `documents/engineering/deterministic_simulation_doctrine.md` — record the Phase-38 scheduler slice of the
  Register-2.5 io-sim battery (Sprint 38.5), with modeled-apiserver fidelity marked assumed.

**Completed cross-references:**
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-38 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 38's gate substrate (linux-cpu) in the per-phase substrate
  map.
- `DEVELOPMENT_PLAN/system_components.md` — register the Phase-38
...
- [phase_37_object_reconciler.md](phase_37_object_reconciler.md) — the object reconciler (observe → diff → scoped-SSA → staged-enact → delete → wait, and the `ValidatedLiveTarget` + mandatory `Lease`) this phase is
  layered on
- [phase_14_capacity_core_folds.md](phase_14_capacity_core_folds.md) — the `place`/`fits`/`carve` resource
  algebra the scheduler placement re-folds under aggregate CAS
- [phase_20_render_manifest_goldens.md](phase_20_render_manifest_goldens.md) — the `renderAll` golden corpus
  the pinned reconcile corpus is a subset of
- [phase_35_bootstrap_coordinator_kind.md](phase_35_bootstrap_coordinator_kind.md) — the live single-node `kind`
  cluster this phase's scheduler binds on
- [phase_36_base_image_registry.md](phase_36_base_image_registry.md) — the in-cluster registry and the
  preloaded/side-loaded amoebius image the bootstrap scheduler Pod uses
- [phase_44_live_dsl_deploy.md](phase_44_live_dsl_deploy.md) — the Deployment-`replicas=1` control-plane daemon that
  stands the reconciler and its scheduling role up in-cluster
- [phase_21_chain_kernel_boundary.md](phase_21_chain_kernel_boundary.md) — the `io-classes` seams / modeled
  apiserver the Register-2.5 scheduler sim (Sprint 38.5) drives the real modules on
- [phase_22_deterministic_sim_substrate.md](phase_22_deterministic_sim_substrate.md) — the deterministic-
...
- [Daemon Topology Doctrine](../documents/engineering/daemon_topology_doctrine.md) — [§3](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-daemon) the Deployment-
  `replicas=1` control-plane daemon that will own this scheduling role in Phase 44
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the
  Register-2.5 io-sim environment the scheduler is validated against in Sprint 38.5, before the Register-3 gate
