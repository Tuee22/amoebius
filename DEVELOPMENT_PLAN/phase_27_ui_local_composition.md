...
- [Sprints](#sprints)
- [Sprint 27.1: Single-/multi-tenant workflow-to-artifact composition gate ✅](#sprint-271-single-multi-tenant-workflow-to-artifact-composition-gate-)
...
remain beneath `.build/**`; the outside-host inventory is unchanged; Phase 27 owns no migration deferral. The
...
**The composition only became runnable when Phase 26's ABI landed.** `use-artifact` had no row in the closed
action table, so the third step of the workflow returned the non-enumerating refusal and the artifact result
never appeared. The table now carries one row per Phase-22 port effect.
...
**Dependencies:** Phase 25 — generic browser interpreter; Phase 26 — authenticated scoped UI-server boundary.
...
of [Gate integrity](#gate-integrity). Phase 29 does not open unless it emits a green Register-2 ledger with all
...
- [Phase 25](phase_25_ui_browser_interpreter.md) — the required generic browser runtime.
- [Phase 26](phase_26_ui_server_boundary.md) — the required authenticated scoped server boundary.
