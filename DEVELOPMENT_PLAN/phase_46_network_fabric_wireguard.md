# Phase 46: WireGuard network fabric
...
Phase 46 delivers the WireGuard network fabric; its design is owned by [network_fabric_doctrine.md](../documents/engineering/network_fabric_doctrine.md), [vault_pki_doctrine.md](../documents/engineering/vault_pki_doctrine.md), [manifest_generation_doctrine.md](../documents/engineering/manifest_generation_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 46.1: Vault-KV Curve25519 peer keys — secrets by name, minted and custodied in Vault ⏸️](#sprint-461-vault-kv-curve25519-peer-keys--secrets-by-name-minted-and-custodied-in-vault-)
- [Sprint 46.2: Rendered peer config + the wg reconcile — render → wg show/diff/wg set ⏸️](#sprint-462-rendered-peer-config--the-wg-reconcile--render--wg-showdiffwg-set-)
- [Sprint 46.3: Phase gate harness — live fabric + external-observer reachability over the VPN IP ⏸️](#sprint-463-phase-gate-harness--live-fabric--external-observer-reachability-over-the-vpn-ip-)
...
⏸️ Blocked pending Phase-45 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
`none`): the typed decoder (Phase 6), the illegal-state corpus (Phase 7), and the pure `renderAll` manifest goldens
(Phase 14) already establish, in-process, that a keyless peer will not construct and an overlapping-IP peer set
will not decode. Phase 46 adds the **runtime residue**: a real `wg0` interface on each node, brought up by the
...
geo-replication wire is *not* delivered here — its per-peer `render()` obligation is deferred to Phase 47; the
two spans this phase renders are the remote-worker↔home attach carrier and the fabric-bound listener boundary.
The gateway-migration hub *repoint* is Phase 48; this phase establishes the static fabric it will
later move.

**Substrate:** linux-cpu — the single-node `kind` host of Phases 29–38, whose kernel provides the WireGuard
...
flowchart LR
  ...
  g{{"the phase 46 gate command"}}:::gate
...
```
*Design intent. Phase 46's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
fabric-render/diff work performed by the already-existing Phase-38 control-plane daemon is added to that control-plane daemon's
...
- [`network_fabric_doctrine.md §4` — Topology: the hub is the gateway role](../documents/engineering/network_fabric_doctrine.md#4-topology-the-hub-is-the-gateway-role-and-the-fabric-moves-with-it):
  ...
  phase stands up the static hub; Phase 48 later repoints it on migration.
...
## Sprint 46.1: Vault-KV Curve25519 peer keys — secrets by name, minted and custodied in Vault ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `src/Amoebius/Fabric/Keys.hs` (the Curve25519 peer-key KV secret
class: validate and resolve a peer's key by `SecretRef` name through the Phase-34 Vault client); the Dhall
...
rendered config (Sprint 46.2) stands on.
...
- The Curve25519 peer-key KV secret class: mint a keypair into Vault-KV, and resolve a peer's private/public key
  by `SecretRef` name at render time through the Phase-34 built-in Vault client (no agent sidecar).
...
### Validation
1. A minted peer keypair is readable from Vault-KV by `SecretRef` name through the Phase-34 client, and no key
   bytes appear in any committed `.dhall` — the schema field is a `SecretRef` name. The inline-key-literal
   negative fixture fails at Gate 1/Gate 2 with the exact tag committed in
...
2. A `SecretRef` naming an absent/rotated KV entry yields a specific-reason resolution error (never an empty or
   default key), so the missing-key mutant of the gate (Sprint 46.3) has a defined failure to trip on.

### Remaining Work
None for this sprint.

## Sprint 46.2: Rendered peer config + the wg reconcile — render → wg show/diff/wg set ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `src/Amoebius/Fabric/WgRender.hs` (`render(nodeInventory) ->
[WireGuardPeerConfig]`, the pure total render lifted from the Phase-14 goldens);
...
- The pure total `render(nodeInventory) -> [WireGuardPeerConfig]`: keyless peer type-foreclosed; overlapping VPN
  ...
  `SecretRef` name resolved from Vault-KV (Sprint 46.1). The gateway-role hub renders a stable hub VPN-IP +
...
## Sprint 46.3: Phase gate harness — live fabric + external-observer reachability over the VPN IP ⏸️
...
`documents/engineering/testing_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-46 status when the
gate passes).
...
- A **Register-3** proven/tested/assumed ledger recording the live-reachability result (the spoke reached the
  ...
  **broker↔broker** geo-replication render obligation (Phase 47), the **gateway-migration hub repoint**
  (Phase 48), and the stretched kubelet↔apiserver `ControlPlanePeer` span (owned by cluster_topology). The
...
### Remaining Work
None for this sprint. The Phase-47/48 and stretched-control-plane surfaces named above remain UNVERIFIED.
...
- `documents/engineering/network_fabric_doctrine.md` — the §2–§5 "Phase 0 design intent" honesty note flips from
  ...
  gateway-migration hub repoint remain deferred (Phase 48).
...
- `DEVELOPMENT_PLAN/README.md` — add the Phase-46 row and flip its status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — add the Phase-46 linux-cpu gate row (the raw-kernel WireGuard fabric).
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Fabric/{Keys,WgRender,WgReconcile}.hs` as
  Phase-46 design-first rows against `network_fabric_doctrine.md`.
...
- [phase_34](phase_34_vault_pki.md) — the root Vault + `SecretRef`-by-name client custodying the peer keys
- [phase_38](phase_38_live_dsl_deploy.md) — the control-plane daemon whose reconcile enacts the fabric
- [phase_47](phase_47_multicluster_spawn_georepl.md) — multi-cluster spawn + geo-replication (the deferred broker↔broker per-peer render obligation)
- [phase_48](phase_48_gateway_migration_drills.md) — the gateway migration that later repoints the WireGuard hub
