# Phase 27: Typed renderer + object reconciler
...
- [Sprints](#sprints)
- [Sprint 27.1: Deployment-global desired state + authenticated live inventory + typed action plan ✅](#sprint-271-deployment-global-desired-state--authenticated-live-inventory--typed-action-plan-)
- [Sprint 27.2: Bootstrap Lease authority + generic typed-action dispatcher + scoped SSA + storage-scaling dispatch ✅](#sprint-272-bootstrap-lease-authority--generic-typed-action-dispatcher--scoped-ssa--storage-scaling-dispatch-)
- [Sprint 27.3: Staged execution transitions, Job terminal protocol, and authenticated deletion ✅](#sprint-273-staged-execution-transitions-job-terminal-protocol-and-authenticated-deletion-)
- [Sprint 27.4: Wait-for-ready + the idempotent-convergence gate (re-run no-op) ✅](#sprint-274-wait-for-ready--the-idempotent-convergence-gate-re-run-no-op-)
- [Sprint 27.5: Register-2.5 reconciler + staged-execution convergence under simulated faults ✅](#sprint-275-register-25-reconciler--staged-execution-convergence-under-simulated-faults-)
...
⏸️ Blocked pending Phase-26 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
Blocked (superseded) — reopened 2026-08-16 because Phase 25's successor OCI handoff is being amended to include Pulsar's S3 offloader bundle. Revalidate this phase against the amended exact handoff before Phase 28 proceeds.
...
The run verified Phase 25 attestation `sha256:e6ff9163d299b66b8ce77c216d5114e13af6fe4beb7e299834ee390e9963816a`
...
`kind` cluster from Phase 24, pulling its corpus Deployment from the `distribution` registry Phase 25 published
...
`tools/object_reconciler_execution_live.py` each pinned the Phase-25 base image by a literal digest from a build that is
gone, and the one assertion tying the running pod to Phase 25's published artifact compared against that same
...
Complete. Sprints 27.1–27.5 and the complete Register-3/2.5 phase gate passed. This phase opened after the **Phase 25 gate** (the
...
`Bound` reservation ledger, two-stage bootstrap cutover, and execution-identity admission — is **Phase 28, layered on this reconciler**; in this phase the corpus Pods are bound by the **default Kubernetes scheduler**,
...
- **2026-08-09 — Sprint 27.1 complete.** The separately validated desired index, authenticated observed
...
- **2026-08-09 — Sprint 27.2 complete.** The typed Lease token/renewal rounding, effect dispatcher, and scoped
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 27.3 complete.** The pure serial, ordinary/CUDA/Metal release, Job terminal, and
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 27.4 complete.** A clean Register-3 run converged 19 externally observed objects,
  ...
  `dynamically-resolved`.
- **2026-08-09 — Sprint 27.5 complete.** The real Lease-token, scoped-SSA, serial, host/device-release, Job,
  ...
  independently bounded by Sprint 27.4's live evidence. Receipt:
...
Phase 27. This phase owns the index-preservation and the fail-closed refusal to grant generic apply authority
over a Pod/controller or mutable ledger/Lease field; Phase 28 owns the witnesses that unlock the guarded
stages.
...
TerminalRetained` records are a distinct structure added in Phase 27.) The observed set validates the
...
snapshots. (The scheduler-ledger CAS-version arm of `ValidatedLiveTarget` is added in Phase 27.) A final
...
completion variants, digest/outcome equality, cleanup deadlines, and `CompletedJobNoOp`; the live Phase-27
cluster has neither MinIO nor the Phase-38 sole content-mutation gateway, so it does **not** claim a durable
...
deadline/release → terminal cleanup gate is Phase 37. The Job controller still uses `restartPolicy=Never`,
...
migration/executor is `UnknownCommitment` and fails closed. Phase-24 engine/static components enter through
...
Phase 24; no apple, linux-cuda, or windows substrate is touched (the CUDA/Metal transition arms are exercised
against deterministic observed-device models here, and their live substrate proof is owned by Phase 52 and
Phase 54 respectively).
...
`bind-before-reservation-CAS` mutant are partitioned to Phase 28 and are **not** duplicated here. The
`amoebius-capacity` scheduler bootstrap, its CAS reservation/Binding ledger, and execution-identity admission are
likewise **not** part of this gate; they are Phase 28's gate, layered on this reconciler.
...
flowchart LR
  ...
  s0["Sprint 27.1: Deployment-global desired state + authenticated live…"]
  s1["Sprint 27.2: Bootstrap Lease authority + generic typed-action dispatcher…"]
  s2["Sprint 27.3: Staged execution transitions, Job terminal protocol, and…"]
  s3["Sprint 27.4: Wait-for-ready + the idempotent-convergence gate (re-run no-op)"]
  s4["Sprint 27.5: Register-2.5 reconciler + staged-execution convergence…"]
  gate["the phase 27 gate"]
...
```
*Orientation. The seams phase 27 builds, in order; [Gate integrity](#gate-integrity) owns the apparatus. Not run.*
...
**The fix that pins item (v) to amoebius's own CRD rather than an external workload operator (e.g. the Percona operator built in Phase 32) is deliberate:** it keeps the corpus and its over-bound-child mutant
...
Pod without typed authorization — must fail Sprint 27.3; and (e) a **healthy-CR/over-bound-child** mutant whose
amoebius controller reports its **own in-phase capacity/reservation CR** healthy while its actual child exceeds the
provisioned resource/PVC/rollout envelope — must prevent convergence after child enumeration in Sprint 27.4. (The
source's **bind-before-reservation-CAS** mutant (d) audits the scheduler ledger and is partitioned to Phase 27.)
...
- [`manifest_generation_doctrine.md §5`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  named by [§5](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions) is Phase 28; durable Job completion/cleanup first runs live in Phase 38; rollback and the release
...
- [`daemon_topology_doctrine.md §3`](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-singleton)
  ...
  it up *inside* the singleton is Phase 33.
...
- [`deterministic_simulation_doctrine.md §4`](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  — **Register 2.5, where deterministic simulation sits.** Sprint 27.5 validates the *built* reconciler and staged
...
## Sprint 27.1: Deployment-global desired state + authenticated live inventory + typed action plan ✅
...
- `renderAll → validateAndIndexRenderedObjects → observeInventoryAndObjects → authenticateExecutions →
  ...
  grants generic SSA over the full list; the scheduler-cutover stage-unlock witnesses are Phase 28). The observed
...
- A host-aware observed identity union: `KubernetesPod PodUid | HostProcess HostProcessInstanceId |
  ...
  reservation ledger, distinct from the Phase-28 k8s scheduler reservation ledger.
...
- A complete `ValidatedLiveTarget` constructed from one `ObservedLiveResourceSnapshot`: mandatory whole
  ...
  `ValidatedLiveTarget` is added in Phase 27.)
- Execution actions cover `NoOp`, kind-indexed Pod-controller apply, the three serial OnDelete stages, host
  ...
  gateway capability, which is absent from the Phase-27 live environment. Every mutating constructor carries only
...
- Preserve the operator child-admission and migration envelopes: admission/quota policy must be Ready before a CR
  ...
  retain old+new+workspace/temp/WAL until verified cutover. Phase 27 owns only the generic storage-scaling
  observation/validation/token boundary: Phase 29 supplies the retained-carve and verified-migration enactors, and
  Phase 45 supplies the cloud provider-capacity enactor. Durable backing is never generic prune.
- A mechanical no-release-store/no-write preflight boundary: AST/import lint over the modules above, plus a runtime
  ...
  Phase 38 supplies the live gateway.
...
into the plan tree. Sprint 27.2 consumes the typed action/authority boundary.

## Sprint 27.2: Bootstrap Lease authority + generic typed-action dispatcher + scoped SSA + storage-scaling dispatch ✅
...
**Blocked by**: none; Sprint 27.1 is sealed.
...
precursor must hold the same provisioned mandatory `Lease` that the Phase-34 singleton later receives by an
observed handoff. The scheduler's CAS reservation/Binding path and its two-stage bootstrap are Phase 27.
...
- A dispatcher for every non-scheduler `ValidatedExecutionTransitionAction`. `ApplyDesiredPodController` preserves
  ...
  and prior-controller delete are sent to their own enactors (built in Sprint 27.3); Job completion/cleanup dispatch
  additionally requires the gateway capability and therefore cannot run in the live Phase-27 gate. `NoOp` and
...
- A storage-scaling dispatcher that accepts only a `ValidatedStorageScalingAction`, immediately rechecks its exact
  ...
  migration are abstract effect arms supplied in Phase 29, while provider-capacity creation also consumes the
  transition's exact validated cloud-action batch supplied in Phase 44. Every attempted effect requires post-attempt
...
- Scoped SSA under `fieldManager=amoebius` only for an ordinary desired-object action. It declares the exact owned
  ...
  `ManagedCapacityReady` is added in Phase 27.
...
3. Storage-scaling action tests run at the pure/fake boundary in this phase: stale allocation, backing,
   ...
   a lost response forces a fresh observation. Live retained and cloud mutations remain the Phase-29 and Phase-45
   gates respectively.
...
## Sprint 27.3: Staged execution transitions, Job terminal protocol, and authenticated deletion ✅
...
**Blocked by**: none; Sprint 27.2 is sealed.
...
validated in pure, fake, and IOSim executions here; the live Phase-27 environment intentionally supplies no such
capability.
...
- Job terminal protocol for both `Succeeded` and `FailedBackoffExhausted`: the closed model writes the exact
  ...
  `CompletedJobNoOp`; a failed write retains the terminal Pod/ledger axes and retries safely. In the Phase-27 live
  cluster the gateway capability is absent, so the only inhabitant is `RetainTerminalAwaitingCompletionGateway`; the
  Pod, API bytes, logs/metadata, and retained partition remain charged. Phase 38 performs the first live
...
2. Register-2/2.5 transition tests cover ordinary host exit, CUDA device holds/free VRAM, and Metal drain/allocation/
   ...
   substrate proof remains owned by Phase 52 and Phase 53.
3. Register-1/2 Job tests, available at this sprint's completion, cover all-success and backoff-exhausted failure
   ...
   these same variants are additionally exercised in Sprint 27.5. In the live Phase-27 run, an independent apiserver
   observer proves the terminal UID remains, no Job-completion object is written, and no delete occurs. The first live
   object-store/apiserver persist-before-delete proof is the Phase-38 gate.
...
None. Sprint 27.4 composes these actions into the full convergence/no-op corpus.

## Sprint 27.4: Wait-for-ready + the idempotent-convergence gate (re-run no-op) ✅
...
**Blocked by**: none; Sprint 27.3 is sealed.
...
amoebius validation), `DEVELOPMENT_PLAN/README.md` (flip the Phase-27 status when the gate passes).
...
- The convergence battery over the pinned corpus: enact the exact `renderAll`/action plan → observe workload
  ...
  bind-before-CAS mutants are Phase 28's gate.)
...
## Sprint 27.5: Register-2.5 reconciler + staged-execution convergence under simulated faults ✅
...
`test/spec/sim/SchedulerSim.hs` schedule battery is Phase 27.)
**Blocked by**: none; Sprint 27.4 is sealed.
...
replay by seed.
**Docs to update**: `documents/engineering/deterministic_simulation_doctrine.md` (Phase-27
...
open. The scheduler's CAS-race schedules are Phase 28's `SchedulerSim`.
...
- Committed mutants for lost `Lease`/resourceVersion retry, mutation without holder, sleep-gated readiness, serial
  ...
  collapsed scheduler readiness stages, premature managed taint/full RBAC — are Phase 28's `SchedulerSim`.)
...
- `documents/engineering/manifest_generation_doctrine.md` — §5's typed SSA/staged-action/delete/wait engine flips
  ...
  design intent until Phase 28); §6's planned-vs-observed and `renderAll` model gains its first validation. Keep
...
- `documents/engineering/daemon_topology_doctrine.md` — record that Phase 27 drives the reconciler from the host
  binary; the §3 singleton that *owns* it (Deployment `replicas=1`, delegated single-instance, no election) is stood
  up in Phase 33.
- `documents/engineering/generated_artifacts_doctrine.md` — note that the applied `[K8sObject]` set is generated at apply time and never committed. - `documents/engineering/resource_capacity_doctrine.md` — record the read-only pre-mutation live inventory cross-check and its zero-write failure path. - `documents/engineering/deterministic_simulation_doctrine.md` — add the Phase-27 Register-2.5 status backlink for the Sprint-26.5 reconciler/execution fault battery.
...
- `DEVELOPMENT_PLAN/system_components.md` — register the Phase-27
  ...
  `Admission/ExecutionIdentity` modules are registered by Phase 27.)
...
- [Daemon Topology Doctrine](../documents/engineering/daemon_topology_doctrine.md) — [§3](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-singleton) the Deployment-`replicas=1`
  singleton (delegated single-instance, no election) that will own this reconciler in Phase 34
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — [§4](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits) the
  Register-2.5 io-sim environment the reconciler is validated against in Sprint 27.5, before the Register-3 live gate
...
- [phase_14_chain_kernel_boundary.md](phase_14_chain_kernel_boundary.md) — the Register-2 fake-apply this phase
  replaces with real tools, and the `io-classes` seams Sprint 27.5 drives
- [phase_15_deterministic_sim_substrate.md](phase_15_deterministic_sim_substrate.md) — the `IOSimPOR`
  deterministic-simulation substrate Sprint 27.5 runs on
...
- [phase_28_capacity_scheduler.md](phase_28_capacity_scheduler.md) — the `amoebius-capacity` scheduler, two-stage
  bootstrap cutover, and execution-identity admission layered on this reconciler
- [phase_29_retained_storage.md](phase_29_retained_storage.md) — the retained-carve and verified-migration
  storage-scaling enactors this phase's dispatch boundary calls into
- [phase_34_live_dsl_singleton.md](phase_34_live_dsl_singleton.md) — the Deployment-`replicas=1` singleton that stands
  the reconciler up in-cluster and receives the `Lease` by observed handoff
- [phase_38_content_store_workflow.md](phase_38_content_store_workflow.md) — the first live gateway write → readback →
