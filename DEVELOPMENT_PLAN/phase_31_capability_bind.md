# Phase 31: Capability union + representational bind
...
- [Sprints](#sprints)
- [Sprint 31.1: The closed capability union + the no-product-arm dhall-typecheck foreclosure ⏸️](#sprint-311-the-closed-capability-union--the-no-product-arm-dhall-typecheck-foreclosure-)
- [Sprint 31.2: The `CapabilityBinding` + total representational `bind` ⏸️](#sprint-312-the-capabilitybinding--total-representational-bind-)
- [Sprint 31.3: The bind property/corpus + the Register-1 gate ⏸️](#sprint-313-the-bind-propertycorpus--the-register-1-gate-)
...
([Phase 32](phase_32_provision_seal.md)); the `InferenceEngine` family×lane availability relation, the
target-offering→lane quotient, and the accelerator residency/coexistence provision
([Phase 33](phase_33_inference_accelerator_provision.md)); the pure
`renderAll :: ProvisionedSpec -> [K8sObject]` ([Phase 34](phase_34_render_manifest_oracles.md)); and the live
jit-resolve of an engine into its `CacheBudget`-bounded cache ([Phase 81](phase_81_determinism_jitcache.md)).
...
**Depends on:** [Phase 30](phase_30_execution_accelerator_folds.md)
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 30; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
— is deliberately **not** adopted here; it is owned by [Phase 32](phase_32_provision_seal.md). This phase stops
...
## Sprint 31.1: The closed capability union + the no-product-arm dhall-typecheck foreclosure ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: exact authored Haskell implementation paths have not been bound to this sprint.
**Blocked by**: [Phase 30](phase_30_execution_accelerator_folds.md) human approval
...
- The `InferenceEngine` head's closed `EngineRuntime` lane union (`AppleMetal` · `Cuda` · `LinuxCpu`) with **no arbitrary-`Url`/`Download` arm** — an ML engine is a **named catalog identity**, so "name the engine by URL"
  ...
  owner demands are [Phase 33](phase_33_inference_accelerator_provision.md).
...
- An in-file honesty note that this union is the app-facing *what*; the provider/shape *how* is Sprint 31.2, the
  provision seal is [Phase 32](phase_32_provision_seal.md), and the capability set is invariant across every
...
## Sprint 31.2: The `CapabilityBinding` + total representational `bind` ⏸️
...
- `bind :: CapabilityNeed -> CapabilityBinding -> BoundServiceSpec`, pure and total, selecting the provider's
  **manifest graph for the chosen shape** — a structurally different object graph, not a scalar `replicas` edit.
  ...
  [Phase 32](phase_32_provision_seal.md), never an authored or bound peak here.
- Controller-child lowering: for every operator/CR arm a version-pinned expander joins the descriptor's exact
  ...
  [Phase 32](phase_32_provision_seal.md)'s, not a second unit here). A caller cannot author a scalar child peak,
...
- `BoundDeployment` retains **only** these unprovisioned units, the opaque source ref, and controller
  ...
  `ProvisionContext` is [Phase 32](phase_32_provision_seal.md)'s work, not `bind`'s. `bind` carries the typed
...
- An in-file honesty note: a single-node shape is the canonical provider deployed honestly at small scale (a
  ...
  not a provisioned deployment — the provision seal is [Phase 32](phase_32_provision_seal.md) and the live
  realization is the live band. The source inventory this sprint authors imports none of that: not the
  [Phase 32](phase_32_provision_seal.md) `provision`/`ProvisionedSpec` machinery, not the
  [Phase 30](phase_30_execution_accelerator_folds.md) execution-epoch fold, and not the
  [Phase 34](phase_34_render_manifest_oracles.md) `K8sObject`/Aeson renderer.
...
1. The same `CapabilityNeed`, bound under two shapes, produces two `BoundServiceSpec`s that are **structurally different by the object-node-multiset oracle** (deep structural diff per [§5](../documents/engineering/service_capability_doctrine.md#5-per-cluster-structural-shapes--beyond-values), red on a scalar-only or copied-shape-tag difference; each equal to its oracle-pinned semantic projection), while the **app-surface bytes**
   ...
   subject beneath `.build/mutants/**` (Sprint 31.3) — which makes
...
## Sprint 31.3: The bind property/corpus + the Register-1 gate ⏸️
...
- The **concrete Haskell positive corpus** — **one Haskell-declared case per each of the nine capability arms** (`ObjectStore`,
  ...
  (Sprint 31.1), independent of `bind`'s own case analysis.
...
- The negative corpus — `illegal_product_in_app` ([§3.12](../documents/illegal_state/illegal_state_capability_messaging.md#312-an-app-that-names-a-product-instead-of-a-capability), dhall-typecheck), `illegal_engine_by_url` ([§3.25](../documents/illegal_state/illegal_state_ml_asset.md#325-an-ml-asset-named-by-arbitrary-url-or-an-unready--unlanded-model), dhall-typecheck),
  ...
  [Phase 32](phase_32_provision_seal.md) and [Phase 33](phase_33_inference_accelerator_provision.md).
...
- A Register-1 validation-locus ledger mapping every entry to its catalog id and layer, backed by
  **Phase-28-style coverage-assertion machinery** (the ledger is not a static hand-written file: the suite goes
...
1. Rejected historical observation: the `capability-bind-spec` Cabal suite was recorded green — each of the
   **nine per-arm** positives binds byte-invariant
   ...
   present and its coverage-assertion machinery (Phase-28 precedent) turns the suite **red** if any named fixture,
...
- `documents/engineering/content_addressing_doctrine.md` — reconcile §4.5's Tier-1 engine as the
  ...
  cache as the live-band residue ([Phase 81](phase_81_determinism_jitcache.md)).
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Capability/{Types,Binding}.hs`, the Haskell
  capability-projection declaration, and the Haskell bind property/oracle suites as Phase-31 design-first
...
- [phase_27](phase_27_gadt_decode_ir.md) — gadt-decode, the IR + decoder the bound specs project from
- [phase_28](phase_28_illegal_state_covering.md) — the illegal-state corpus + validation-locus ledger machinery
  this gate reuses
- [phase_30](phase_30_execution_accelerator_folds.md) — the execution-epoch / runtime-storage / accelerator
  folds whose kind-indexed vocabulary `bind`'s `BoundExecutionUnit`s are fold-compatible with
- [phase_32](phase_32_provision_seal.md) — the whole-deployment provision seal that layers `planInfrastructure`/
...
- [phase_33](phase_33_inference_accelerator_provision.md) — the `InferenceEngine` family×lane availability
...
- [phase_34](phase_34_render_manifest_oracles.md) — the pure deployment-global
  `renderAll :: ProvisionedSpec -> [K8sObject]` downstream of the provision seal - [phase_81](phase_81_determinism_jitcache.md) — the live jit-build engine resolver + `CacheBudget` cache that
