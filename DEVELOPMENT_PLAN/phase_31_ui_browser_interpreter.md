# Phase 31: Generic browser interpreter
...
- [Sprints](#sprints)
- [Sprint 31.1: Generic `ClientPlan` interpreter and browser boundary gate ✅](#sprint-311-generic-clientplan-interpreter-and-browser-boundary-gate-)
...
`sha256:a70b1cd50e69733b…`; Phase 31 owns no remaining migration deferral.
...
`.build/runs/phase_25/<run-id>/`, and 66 surfaces join two-way to 84 run-time enumerated items.
...
server only as a boundary peer; server authorization semantics belong to Phase 32.
...
**Depends on:** [Phase 29](phase_29_ui_plan_compiler.md) — canonical immutable `ClientPlan` encoding and public contracts.
...
in [Gate integrity](#gate-integrity), emitting a Register-2 ledger whose live layers stay UNVERIFIED. Phase 33
...
## Sprint 31.1: Generic `ClientPlan` interpreter and browser boundary gate ✅
...
- Phase 33 — consume this interpreter unchanged in the local composed application.

## Related Documents

- [Phase 29](phase_29_ui_plan_compiler.md) — the required immutable `ClientPlan` and public contracts.
