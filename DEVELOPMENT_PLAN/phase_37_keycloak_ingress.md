# Phase 37: Keycloak-owned ingress
...
- [Sprints](#sprints)
- [Sprint 37.1: The Keycloak-owned edge — LoadBalancer → Envoy/Gateway API → Keycloak ⏸️](#sprint-371-the-keycloak-owned-edge--loadbalancer--envoygateway-api--keycloak-)
- [Sprint 37.2: No self-published wild ingress + public-edge TLS ⏸️](#sprint-372-no-self-published-wild-ingress--public-edge-tls-)
- [Sprint 37.3: East-west NetworkPolicy posture — derived default-deny ⏸️](#sprint-373-east-west-networkpolicy-posture--derived-default-deny-)
- [Sprint 37.4: The single-door + storage-rebind regression gate ⏸️](#sprint-374-the-single-door--storage-rebind-regression-gate-)
...
**Done.** All four sprints are implemented and the Phase-37 gate is sealed. The retained linux-cpu stack
...
real Phase-36 operator; as in Phase 36, the exact Patroni child is honestly recorded as an amoebius-owned
...
objects owned by field manager `amoebius`, all using the private Phase-30 digest. Six committed mutants turn
...
scratch cluster is removed. This isolated projection reuses the Phase-33 persistence harness and deliberately
does not destroy the retained Phase 29–37 platform cluster.
...
for every wild request. It composes the LoadBalancer address published by Phase 35 (MetalLB on `linux-cpu`)
with an **Envoy + Gateway API L7 data plane** and **Keycloak OIDC/JWT enforcement**, rendered as typed
manifests by the Phase-31 reconciler and applied to the live cluster, so that WAN, LAN, and even a
...
is applied and exercised live. Finally the phase re-runs the Phase-33 lossless-rebind proof behind the new
...
replayed-nonce, wrong-subprotocol, and direct-Service attempts produce no backend frame. And the Phase-33
...
- **Image provenance (§M.5):** every live `imageID` is observed at the CRI boundary and must equal both the
  verified Phase-30 image identity supplied to this run and the current in-cluster registry catalog. A public
...
flowchart LR
  ...
  g{{"the phase 37 gate command"}}:::gate
...
```
*Validated Phase-37 gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
- [`storage_lifecycle_doctrine.md` §6](../documents/engineering/storage_lifecycle_doctrine.md#6-the-lossless-teardown-guarantee-deterministic-rebind)
  — **the lossless-teardown guarantee: deterministic rebind**: the gate re-runs the Phase-33 marker-bytes
...
## Sprint 37.1: The Keycloak-owned edge — LoadBalancer → Envoy/Gateway API → Keycloak ⏸️
...
- Envoy + Gateway API rendered as the L7 edge (a `Gateway` listener plus `HTTPRoute`s as typed `K8sObject`s),
  terminating TLS and routing, applied by the Phase-31 reconciler.
- Keycloak deployed against its Phase-36 Patroni DB, owning OIDC/JWT enforcement in front of every platform
...
## Sprint 37.2: No self-published wild ingress + public-edge TLS ⏸️
...
- A live audit proving there is no non-Keycloak wild path: no chart opens a backdoor NodePort to the wild, and
  ...
  edge, per the render invariant golden-locked in Phase 14.
...
## Sprint 37.3: East-west NetworkPolicy posture — derived default-deny ⏸️
...
- A default-deny east-west baseline plus allow-edges derived from the declared dependency graph, rendered by
  the Phase-31 reconciler and applied to the live cluster — no hand-authored policy.
...
3. Prove "derived" by **graph variation**: deploy a scratch consumer, add a declared edge to a provider,
   ...
   applied set must therefore be a total function of the declared graph, not of the fixed Phase-35/36 service
...
## Sprint 37.4: The single-door + storage-rebind regression gate ⏸️
...
- The storage-rebind regression: write the committed marker row (`marker-row.sql`) into the Keycloak Patroni DB
  ...
  same bytes back — the Phase-33 guarantee re-run behind the new edge. Plus committed mutant (c), a
...
5. Compare CRI-observed image identities with the verified Phase-30 run input and current registry catalog;
   a seeded gate variant that reads an expected-digest file and a side-loaded public image each fail.

### Remaining Work
Remove the committed expected-base-digest file and gate dependency, then rerun with Phase-30 identity supplied
as run-local verified input.

## Documentation Requirements

**Engineering docs updated with the tested Phase-37 result:**
...
- [README.md](README.md) — flip the Phase 37 row status once work begins, and link this document from the
  Phase 37 paragraph.
- [substrates.md](substrates.md) — record `linux-cpu` as the Phase 37 gate substrate in the per-phase map.
...
- [README.md](README.md) — the live tracker; its Phase 37 row is the authoritative one-line gate and status.
...
- [Manifest Generation Doctrine](../documents/engineering/manifest_generation_doctrine.md) — the typed renderer
  + server-side-apply reconciler that enacts the edge (delivered in Phase 31).
