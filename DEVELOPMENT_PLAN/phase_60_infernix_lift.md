# Phase 60: infernix core artifact lift
...
- [Sprints](#sprints)
- [Sprint 60.1: Lift infernix through one scoped artifact adapter ⏸️](#sprint-601-lift-infernix-through-one-scoped-artifact-adapter-)
...
⏸️ Blocked pending Phase-59 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
sibling source boundary is reviewed and resolved dynamically. Store operations use the Phase-48 three-tier content-addressed store;
commands and events use typed native CBOR over Pulsar; credentials remain Vault `SecretRef` names; named
engines resolve through the Phase-59 jit-build cache; CPU decode uses a deterministic pinned micro-model.
...
preserves it unchanged as the Phase-48 work-id in the canonical command payload and in every progress or
terminal event; it never derives a replacement from a pod, retry, run id, or artifact digest. Producer resends
retain the producer identity/sequence required by Phase 46, while consumer redelivery folds on that stable
...
raw provider handle. Phase 61 is the sole infernix-to-UI boundary; this phase owns no frontend, browser probe,
...
and buffer bounds, and complete CPU/memory/ephemeral demand to the inherited Phase-48 workflow envelope. Its
...
accept it with the Phase-60 aggregate gate. Split if the work changes sibling inference algorithms, adds a
...
- [Content Addressing Doctrine §2 — Three-tier store](../documents/engineering/content_addressing_doctrine.md#2-the-three-tier-store-blobs--manifests--pointers),
  ...
  and [§4 — Determinism by construction](../documents/engineering/content_addressing_determinism.md#4-determinism-by-construction-pinned-inputs--pure-stages--derived-seed):
  bind verified artifacts, substrate identity, pure decode, and derived seed.
- [Content-addressing determinism §4.5 — Bounded ML-asset cache](../documents/engineering/content_addressing_determinism.md#45-the-ml-asset-lifecycle-one-bounded-content-addressed-cache-resolved-on-first-miss)
...
## Sprint 60.1: Lift infernix through one scoped artifact adapter ⏸️
...
- `DEVELOPMENT_PLAN/phase_61_infernix_ui_lift.md` — remain the sole owner of infernix UI projection and
  interaction.
...
- [Phase 61 — infernix UI lift](phase_61_infernix_ui_lift.md)
- [Phase 71 — jitML lift](phase_71_jitml_lift_cuda.md)
