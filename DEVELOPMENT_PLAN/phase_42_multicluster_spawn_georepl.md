...
non-confluent bucket — is precisely the boundary hand-off the [Phase 43](phase_43_gateway_migration_drills.md)
...
cluster, Phase 29's root Vault/PKI trust anchor, Phases 30–31's platform services (MinIO, Pulsar, Patroni Postgres),
Phase 33's live DSL deploy via the `replicas=1` singleton, Phase 35's native Pulsar client, and Phase 37's
...
[Phase 44](phase_44_provider_deploy_checkpoint.md)). Before either child is created, the parent-owned
...
- [`cluster_lifecycle_doctrine.md §3`](../documents/engineering/cluster_lifecycle_doctrine.md#3-amoebic-spawning--the-recursive-forest)
  ...
  gateway-migration drills of [Phase 43](phase_43_gateway_migration_drills.md).
...
(ii) for the [Phase 43](phase_43_gateway_migration_drills.md) migration runtime.
...
- `documents/engineering/cluster_lifecycle_doctrine.md` — §3/§9 gain the realized module paths for the spawn,
  ...
  land with [Phase 43](phase_43_gateway_migration_drills.md)).
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `src/Amoebius/Multicluster/Spawn.hs`,
  ...
  gate suites as Phase-42 rows.
- `DEVELOPMENT_PLAN/substrates.md` — the Phase-42 → linux-cpu row in the per-phase substrate map.
...
- [phase_41](phase_41_network_fabric_wireguard.md) — the prior phase (the WireGuard fabric); its gate opens this one
- [phase_43](phase_43_gateway_migration_drills.md) — the next phase; the gateway-migration runtime and correspondence over this forest
- [phase_44](phase_44_provider_deploy_checkpoint.md) — the forest extended to provider-managed clusters
