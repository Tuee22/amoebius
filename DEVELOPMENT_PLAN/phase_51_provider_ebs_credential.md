# Phase 51: Per-PV EBS decoupling + create-vs-delete credential
...
Phase 51 delivers the Per-PV EBS decoupling + create-vs-delete credential; its design is owned by [pulumi_iac_doctrine.md](../documents/engineering/pulumi_iac_doctrine.md), [storage_lifecycle_doctrine.md](../documents/engineering/storage_lifecycle_doctrine.md), [image_build_doctrine.md](../documents/engineering/image_build_doctrine.md), and the plan for reaching it is owned here.
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_49_provider_deploy_checkpoint.md, DEVELOPMENT_PLAN/phase_50_provider_child_bringup.md, DEVELOPMENT_PLAN/phase_52_provider_dynamic_nodes.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 51.1: Per-PV durable EBS in its own state + node-vs-storage decoupling ⏸️](#sprint-511-per-pv-durable-ebs-in-its-own-state--node-vs-storage-decoupling-)
- [Sprint 51.2: The create-vs-delete credential model (cloud-API delete-deny) ⏸️](#sprint-512-the-create-vs-delete-credential-model-cloud-api-delete-deny-)
- [Sprint 51.3: Static-only baked AWS EBS CSI + static PV over volumeHandle ⏸️](#sprint-513-static-only-baked-aws-ebs-csi--static-pv-over-volumehandle-)
- [Sprint 51.4: Provider-volume migration + the `CreateProviderCapacity` storage-scaling arm ⏸️](#sprint-514-provider-volume-migration--the-createprovidercapacity-storage-scaling-arm-)
- [Sprint 51.5: Phase gate — durable EBS retained across teardown + cloud-API delete-deny + static-CSI reattach ⏸️](#sprint-515-phase-gate--durable-ebs-retained-across-teardown--cloud-api-delete-deny--static-csi-reattach-)
...
⏸️ Blocked pending Phase-50 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
Vault Transit and MinIO. It cleaned every Phase-51 object. This is a scoped storage/checkpoint analogue,
...
[Phase 49](phase_49_provider_deploy_checkpoint.md) gate (the provider-cluster Pulumi deploy-from-inside + Vault-Transit-enveloped MinIO checkpoint + `observeProviderAccount` — the deploy this phase spins a cluster with and whose durable-class checkpoint/state machinery it reuses), the
[Phase 33](phase_33_retained_storage.md) gate (`no-provisioner` retained PVs + lossless rebind — the storage substrate the EBS backs), and the [Phase 30](phase_30_base_image_registry.md) gate (the multi-arch baked-binary supply chain this phase extends with the provider-only CSI binaries). It runs on the **linux-cpu → provider**
...
deploy itself (that is the [Phase 49](phase_49_provider_deploy_checkpoint.md) gate), never the hostless
in-cluster control plane and Lease handoff ([Phase 50](phase_50_provider_child_bringup.md)), never dynamic node
provisioning or the leak-free ephemeral teardown gate ([Phase 52](phase_52_provider_dynamic_nodes.md)), and
never the elevated-harness *reclamation* of durable test-flagged EBS ([Phase 56](phase_56_test_topology_dsl.md), which completes the [§7](../documents/engineering/testing_doctrine.md#7-the-elevated-harness-is-the-sole-automated-deleter-of-test-owned-durable-storage-leak-free-cycles) model's full leak-free test *cycle*). Status transitions are recorded
...
and exercised in [Phase 56](phase_56_test_topology_dsl.md) — referenced, never invoked here. Production reclaim
...
[Phase 9](phase_09_storage_geometry_folds.md) owns the policy-only envelope and the observe-then-plan
`Growable`/scaling fold; [Phase 31](phase_31_object_reconciler.md) owns the generic fresh-snapshot validation and
...
EBS/checkpoint readback. Retained-carve allocation and host migration remain [Phase 33](phase_33_retained_storage.md)
arms.
...
possible ([Phase 56](phase_56_test_topology_dsl.md)).
...
recorded *correct-by-class* and the elevated-harness durable-EBS *reclamation* marked **deferred to [Phase 56](phase_56_test_topology_dsl.md), not asserted here**.
...
[Phase 49](phase_49_provider_deploy_checkpoint.md)):
...
- [`storage_lifecycle_doctrine.md §5.1`](../documents/engineering/storage_lifecycle_doctrine.md#51-storage-is-independent-of-the-node-lifecycle)
  ...
  the [Phase 56](phase_56_test_topology_dsl.md) elevated harness may destroy a test-owned volume, and production
...
- [`chaos_failover_doctrine.md §12`](../documents/engineering/chaos_failover_doctrine.md#12-the-moral-core--proven-tested-assumed)
  ...
  deferred to [Phase 56](phase_56_test_topology_dsl.md)*; skipping an applicable teardown-observation move marks
...
- An in-file honesty note: `Amoebius.Pulumi.Ebs` produces the durable-class state and the `Promised`/`Materialized`
  ...
  is Sprint 51.2, and the static attach/mount path is Sprint 51.3.
...
delete authority is the elevated test credential ([Phase 56](phase_56_test_topology_dsl.md), referenced not invoked here).
...
- The only automated delete authority is the elevated test credential, limited to test-owned volumes and
  exercised in [Phase 56](phase_56_test_topology_dsl.md) — referenced, not invoked here. Production reclaim uses a
...
1. Create a per-PV EBS (Sprint 51.1), render a static PV whose CSI `volumeHandle` is that exact EBS ID and whose
...
- An in-file honesty note: this sprint builds the create-only guard's scaling/migration arms;
  ...
  refines are owned by [Phase 9](phase_09_storage_geometry_folds.md) and
  [Phase 31](phase_31_object_reconciler.md).
...
3. Drive a `Growable` durable-EBS budget across its threshold without editing its desired demand. The fresh
   observation and [Phase 9](phase_09_storage_geometry_folds.md) planner must select `CreateProviderCapacity`;
...
separation realized live; durable-EBS reclamation deferred to [Phase 56](phase_56_test_topology_dsl.md)),
...
- The gate over the durable-EBS slice of `test/dhall/phase_47_provider_provision.dhall`: spin up the provider
  ...
  reattachment.
- The credential proof (Sprint 51.2): a real `ec2:DeleteVolume` under the operational credential is denied at the
...
- A per-run proven/tested/assumed ledger recording: durable-EBS creation, static-CSI bind, and marker read as
  ...
  *reclamation* as **explicitly deferred to [Phase 56](phase_56_test_topology_dsl.md), not asserted here**.
...
5. Assert the run emits a proven/tested/assumed ledger per
   ...
   deferred to [Phase 56](phase_56_test_topology_dsl.md); skipping an applicable teardown-observation move
...
Re-run the enumerated provider surfaces after Phase 49 can create the EKS child. Fifteen AWS/EBS/CSI/runtime
surfaces remain UNVERIFIED, including Phase-56 elevated durable reclamation.
...
> [Phase 52](phase_52_provider_dynamic_nodes.md), and the leak-free durable-EBS *reclamation* under the elevated
> credential — the move that makes a *full* leak-free test *cycle* possible — is
> [Phase 56](phase_56_test_topology_dsl.md) and is **not** a dependency of this phase. Live AWS spend (EKS, EC2, > EBS) is the *expected* outcome of asking the harness to provision a provider cluster, exactly as in the prodbox
...
- `documents/engineering/storage_lifecycle_doctrine.md` — record the required-usable versus allocation-rounded
  ...
  reclamation deferred to [Phase 56](phase_56_test_topology_dsl.md) (§7.1, the single elevated-harness exception).
...
- `documents/engineering/testing_doctrine.md` — record the Phase 51 per-run ledger artifact and the explicit
  deferral of elevated durable-EBS reclamation to [Phase 56](phase_56_test_topology_dsl.md).

**Cross-references to add:**
- `DEVELOPMENT_PLAN/system_components.md` — register `Amoebius.Pulumi.Ebs`, `Amoebius.Pulumi.Credential`,
  `Amoebius.Storage.EbsCsi`, and `Amoebius.Storage.ProviderScaling` as Phase-51 design-first rows, each mapped to
  its owning doctrine; note the reused `amoebius-pulumi` Engine + EncryptedMinio backend map to their first
  delivery in [Phase 49](phase_49_provider_deploy_checkpoint.md).
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 51 → `linux-cpu` (parent) row with the `provider` (EKS)
  deploy target annotated as a target class, not a fifth hardware substrate.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase 51 row's status once the gate passes; link this document.

## Related Documents
- [README.md](README.md) — the live tracker; Phase 51 objective, gate, and substrate
...
- [phase_49](phase_49_provider_deploy_checkpoint.md) — the provider-cluster Pulumi deploy + Vault-Transit MinIO
  checkpoint + `observeProviderAccount` this phase spins a cluster with and reuses for the durable-class state
- [phase_33](phase_33_retained_storage.md) — the `no-provisioner` retained PVs + lossless rebind the EBS backs
- [phase_30](phase_30_base_image_registry.md) — the multi-arch baked-binary supply chain this phase extends with
  provider-only CSI binaries
- [phase_09](phase_09_storage_geometry_folds.md) — the storage-scaling policy-only envelope and observe-then-plan
  `Growable`/scaling fold the `CreateProviderCapacity` enactor refines
- [phase_31](phase_31_object_reconciler.md) — the typed SSA reconciler that installs the static CSI and the
  generic fresh-snapshot validation + single-use dispatcher this scaling arm invokes
- [phase_52](phase_52_provider_dynamic_nodes.md) — the dynamic node provisioning + leak-free ephemeral teardown
  gate that layers on this phase (and Phases 49, 50)
- [phase_56](phase_56_test_topology_dsl.md) — the elevated-harness durable-EBS reclamation that completes the [§7](../documents/engineering/testing_doctrine.md#7-the-elevated-harness-is-the-sole-automated-deleter-of-test-owned-durable-storage-leak-free-cycles)
