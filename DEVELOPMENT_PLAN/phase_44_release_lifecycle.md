...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_45_ui_program_release.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 44.1: The immutable `Release` ledger (`releaseHash`) ⏸️](#sprint-441-the-immutable-release-ledger-releasehash-)
- [Sprint 44.2: The `Environment` ETag-CAS promotion pointer ⏸️](#sprint-442-the-environment-etag-cas-promotion-pointer-)
- [Sprint 44.3: The `PromotionGate` — promote-unverified→prod type-foreclosed ⏸️](#sprint-443-the-promotiongate--promote-unverifiedprod-type-foreclosed-)
- [Sprint 44.4: `RolloutPlan`/`RolloutPhase` readiness-gated apply + DB schema-migration phase (gate) ⏸️](#sprint-444-rolloutplanrolloutphase-readiness-gated-apply--db-schema-migration-phase-gate-)
...
`releaseHash = sha256(resolved-deployment-dhall ‖ image-digests ‖ substrate-fp)`, written into the Phase-42
...
**`RolloutPlan`/`RolloutPhase`** apply on the Phase-38 in-cluster SSA reconciler: an ordered plan whose each
...
given: the `releaseHash` formula and the hash/pointer master registry are the Phase-42/Phase-53 store's
(consumed here as an opaque content-address protocol); the proven/tested/assumed evidence ledger the gate
reads is the contract owned by `testing_doctrine` (consumed here as an opaque witness; Phase 56 later automates
test-topology generation but is not a prerequisite); the Gateway-API canary weight-shift remains later-phase
work, and the cross-cluster/geo promotion boundary is exercised in Phase 47 — neither is part of this gate.
...
- **A live `Release`-ledger write produces a content-addressed `releaseHash`.** The key is recomputed by an
  ...
  to the oracle-pinned golden key, with the ledger entry immutable in the Phase-42 store.
...
- **A readiness-gated `RolloutPlan` applies its phases in order.** The phase-apply order is read from an
  ...
  **includes a DB schema-migration `RolloutPhase`** obeying `create-new → verified-migrate → retire-old`
  against the standing Phase-39 Postgres, whose retire phase never denotes byte destruction.
...
- [`inforcespec_migration_doctrine.md §3`](../documents/engineering/inforcespec_migration_doctrine.md#3-the-dsl-exposes-no-destructive-verb--the-closed-storagemutation-union)
  ...
  denotes durable-byte destruction (Sprint 44.4).
- [`release_lifecycle_doctrine.md §1`](../documents/engineering/release_lifecycle_doctrine.md#1-no-external-cicd-control-plane--delivery-is-typed-composition-on-primitives-amoebius-owns)
  ...
  plane; the whole lifecycle is a handful of typed values composed over the Phase-38 reconciler and the
  Phase-42 store, with desired state recomputed from the immutable release's authenticated Dhall reference,
...
- [`app_vs_deployment_doctrine.md §3`](../documents/engineering/app_vs_deployment_doctrine.md#3-the-deployment-rules-surface--how-the-same-app-runs)
  ...
  `if prod then …` in an app spec and no rebuild between environments (realized by Sprint 44.2's app-bytes
  invariance).
...
- [`release_lifecycle_doctrine.md §5`](../documents/engineering/release_lifecycle_doctrine.md#5-rolloutplan--rolloutphase-the-readiness-gated-apply)
  — *`RolloutPlan`/`RolloutPhase`: the readiness-gated apply*: an ordered plan enacted by the Phase-38 SSA
  reconciler
...
- A `Release` value — `{ releaseHash, deploymentDhallRef, imageDigests, substrateFp }` — written as an
  append-only entry into the Phase-42 store (pointers → manifests → blobs), keyed by
...
### Validation
1. Execute at **Register 3** against the Phase-42 single-node kind cluster's live MinIO store, never an
...
> validates the *live wiring* of that foreclosure through the Phase-42 store and the consumed per-run evidence
...
enact a satisfied promotion as an ordered, readiness-gated `RolloutPlan` on the Phase-38 SSA reconciler — with
...
### Deliverables
- `RolloutPlan = [RolloutPhase]` where each `RolloutPhase` carries `{ phaseObjects, phaseGate }`: the desired slice this phase applies and the readiness condition observed from live state — rollout complete, `Ready`, or CR `status` healthy, never a `threadDelay` — that gates the next phase. Enacted by the Phase-38 SSA/ApplySet engine ([`manifest_generation_doctrine.md §5`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions));
  it introduces **no new reconciler**.
- A **DB schema-migration `RolloutPhase`** against the standing Phase-39 Postgres obeying
...
- **Pre-runtime evidence pinned in this phase's oracle-pinning sprint:** the committed
  ...
  **red** — `mutant/gate-admits-unverified` (Sprint 44.3; the phase-level mandated mutant — the promotion that
...
1. Run the gate topology end-to-end on the linux-cpu kind cluster and assert, **live**: (a) the `Release`
   ledger write emits a `releaseHash` byte-equal to the committed golden (Sprint 44.1, independently
   recomputed); (b) `release_unverified → Prod` is **refused with `PromotionRefused:RuntimeEvidenceMissing`**
   and the `Prod` pointer HEAD is unchanged in the store (Sprint 44.3); (c) `release_verified → Prod`
   **advances the ETag-CAS pointer** (Sprint 44.2), observed from the store's pointer history; (d) the
...
> their proof rides a later phase and Phase 47, respectively. Pulumi cloud-IaC (tier a) and the host spot-fleet
> reconciler
...
- `documents/engineering/release_lifecycle_doctrine.md` — record that §2 (the `releaseHash` release ledger),
  ...
  promotion boundary explicitly still deferred (later phase / Phase 47); flip the Phase-0 reference-only honesty
...
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 44's gate substrate (linux-cpu) in the per-phase substrate
  map.
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-release` package and its target module paths
  ...
  `SchemaMigration`), mapped to the owning release-lifecycle doctrine, as Phase-44 design-first rows.
