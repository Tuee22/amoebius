# Phase 52: The host-ensure kernel
...
- [Sprints](#sprints)
- [Sprint 52.1: The closed substrate algebra ⏸️](#sprint-521-the-closed-substrate-algebra-)
- [Sprint 52.2: Install steps as typed data ⏸️](#sprint-522-install-steps-as-typed-data-)
- [Sprint 52.3: The reconciler table ⏸️](#sprint-523-the-reconciler-table-)
- [Sprint 52.4: The probe-first ensure driver ⏸️](#sprint-524-the-probe-first-ensure-driver-)
- [Sprint 52.5: The lift fold to argv ⏸️](#sprint-525-the-lift-fold-to-argv-)
- [Sprint 52.6: Host claim participation against a real root ⏸️](#sprint-526-host-claim-participation-against-a-real-root-)
...
The binary takes over every assertion the moment [Phase 51](phase_51_host_assert_cli.md) execs it, and the
...
**Depends on:** [Phase 51](phase_51_host_assert_cli.md)
...
## Sprint 52.2: Install steps as typed data ⏸️
...
## Sprint 52.3: The reconciler table ⏸️
...
## Sprint 52.4: The probe-first ensure driver ⏸️
...
## Sprint 52.5: The lift fold to argv ⏸️
...
## Sprint 52.6: Host claim participation against a real root ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 52.5
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
Exchange real records with a real ledger root, without observing any hardware.

### Deliverables

- Resolution of the one fixed per-user root, with no environment selection, search, or repository-relative fallback.
- The admission section held across a try-lock, a scan, and one flushed write, and never across a workload.
- A `.live` lock for a `Transient` claim, and release of the participant's own stale `Transient` claims at the start of every admission.
- A refusal to participate, stated rather than silently assumed, when a guest cannot mount the host root.

### Validation

Charges come only from a supplied observed host and an operator-authored budget; a candidate that derives either from a probed device fails, because that observation belongs to a later hardware-bearing phase.

### Remaining Work

Every `UNRESOLVED` contract row, predecessor approval, owned legacy closure, and phase-specific obligation in the gate. Contention with a second independently built participant is not established here.
