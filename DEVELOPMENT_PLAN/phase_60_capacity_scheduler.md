# Phase 60: amoebius-capacity scheduler + bootstrap cutover

> **Purpose**: Build the same-binary `amoebius-capacity` scheduler as a dedicated role layered on the
> Phase-59 reconciler — the state-indexed reservation ledger, the two-stage bootstrap taint/RBAC cutover
...
- [Sprints](#sprints)
- [Sprint 60.1: State-indexed reservation ledger + normalization + absent-Pod recovery arms ⏸️](#sprint-601-state-indexed-reservation-ledger--normalization--absent-pod-recovery-arms-)
- [Sprint 60.2: Scheduler bootstrap authority + two-stage taint/RBAC cutover + readiness witnesses ⏸️](#sprint-602-scheduler-bootstrap-authority--two-stage-taintrbac-cutover--readiness-witnesses-)
- [Sprint 60.3: Scheduler loop + `Reserved`→`BindingInFlight`→Binding→`Bound` CAS + placement + recovery ⏸️](#sprint-603-scheduler-loop--reservedbindinginflightbindingbound-cas--placement--recovery-)
- [Sprint 60.4: Live scheduler binding + bootstrap→steady cutover gate ⏸️](#sprint-604-live-scheduler-binding--bootstrapsteady-cutover-gate-)
- [Sprint 60.5: Register-2.5 scheduler convergence under simulated faults ⏸️](#sprint-605-register-25-scheduler-convergence-under-simulated-faults-)
...
Blocked by redesigned Phase 59, its independent validation, and human promotion; every earlier
...
The scheduler owns the **state-indexed reservation ledger**. It is an inner ledger and is subordinate: the
capacity it may schedule is bounded by the host claim amoebius already holds under
[hostclaim_spec.md](../documents/engineering/hostclaim_spec.md), and it never authorises a host launch on
its own. Two roots that both authorise one would recreate double-spending inside a single program, so this
ledger references the outer claim rather than competing with it. On top of Phase 59's `ObservedLiveResourceSnapshot`
...
unique-node affinity, the exact `amoebius-capacity-scheduler` namespace `ResourceQuota pods=1`, the Phase-57
...
capability can only patch the finite observed distro/Phase-57 bootstrap-controller set to
...
those are Phase 59's and are consumed here as the substrate the scheduler runs on. It owns only the scheduling
authority. The scheduler is driven from the **host binary** against the same scratch namespace as Phase 59; the
...
and WSL2 on Windows. This execution uses Phase 56's single-node `kind` cluster and the Phase-59 reconciler.

**Lane:** linux-cpu/amd64 ([§L](development_plan_standards.md#l-one-substrate-discipline))

**Register:** 3 — live infrastructure ([§K](development_plan_standards.md#k-honesty-proven--tested--assumed)).

**Depends on:** [Phase 59](phase_59_object_reconciler.md)
...
| Key | Contract |
|---|---|
| `Claim` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: one cohesive claim — *a reservation is a state-indexed ledger entry, and the cutover to it is two-staged so the cluster is never unscheduled*. Execution identity is admitted, never assumed. Explicit exclusions: every layer named in `Residue` remains UNVERIFIED. |
...
- **Image** — the Phase-57 side-loaded/preloaded native-architecture amoebius base image (never a public-registry pull),
...
- [`manifest_generation_doctrine.md` §5 — The apply/reconcile engine: snapshot-bound typed actions](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  authenticated deletion, and the rollback/release ledger are Phase 59's or stay deferred.
...
- [`daemon_topology_doctrine.md` §3 — The control-plane daemon](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-daemon)
  ...
  `Lease`, no bespoke election) is a Phase-66 target. Phase 60 cannot consume that later daemon.
...
## Sprint 60.1: State-indexed reservation ledger + normalization + absent-Pod recovery arms ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 59](phase_59_object_reconciler.md) human approval
...
— the reconcile state model, scheduler-ledger slice. Extend Phase 59's observed inventory with the
...
- A scheduler-ledger join over full `Reserved | BindingInFlight | Bound | Terminating | TerminalRetained`
  records plus resourceVersion/CAS version, added to Phase 59's `ObservedLiveResourceSnapshot` (empty before
...
- The host-aware observed identity union consumed from Phase 59 —
...
- State-sensitive Binding recovery at normalization time: an unknown/unbound `BindingInFlight` row remains
  ...
  Pod-UID runtime-storage row (from Phase 59's `RuntimeStorage`) and exact-joins it once with the
  still-`BindingInFlight` reservation — the later repair CAS (Sprint 60.3) changes state, not capacity.
- The scheduler-ledger slice of the `ValidatedLiveTarget`: the ledger CAS version, the normalized
  ...
  handed to Phase 59's target constructor. `PlannedExecutionSlotId` is never equated with a Pod UID; every
...
2. Positive Haskell recovery cases cover absent-Pod rows in **every** closed ledger state and prove each remains
   charged until its state-specific CAS. The host-ledger controls reuse the Phase-59 failure classes: missing
...
## Sprint 60.2: Scheduler bootstrap authority + two-stage taint/RBAC cutover + readiness witnesses ⏸️
...
- A read-only scheduler preflight (layered on the Phase-59 reconciler already holding
  ...
  the Phase-57 side-loaded/preloaded amoebius image, has unique-node affinity, and merges its static owner row
...
- `BootstrapCapacitySchedulerReady`: exact active generation/config/root readback mints it **while the managed taint, general identity admission, and full Binding authority remain absent**. Its restricted capability can
  only patch the exact observed distro/Phase-57 bootstrap-controller set to `schedulerName=amoebius-capacity`.
...
3. `ceil(renewalWindow/retryPeriod) <= maxRenewalsPerWindow` is not re-proven here (the `Lease` is Phase 59's),
   but the scheduler preflight asserts it runs **only** while the Phase-59 reconciler is the exact `Lease`
...
## Sprint 60.3: Scheduler loop + `Reserved`→`BindingInFlight`→Binding→`Bound` CAS + placement + recovery ⏸️
...
- The reservation transitions are the scheduler's members of Phase 59's `ValidatedExecutionTransitionAction`
  domain; the scheduler never enters Phase 59's generic scoped-SSA path (`fieldManager=amoebius`), and every
...
2. The seeded mutants `bind-before-reservation-CAS`, `numeric-add-instead-of-whole-ledger-refold`,
   ...
   digest or collapses the two witnesses (from Sprint 60.2) also turns the reservation suite red when it lets a
...
4. The scheduler modules import no generic SSA writer; the lint asserts scheduler/ledger/Binding actions never
   enter Phase 59's `Manifest/Apply.hs` scoped-SSA module, and `NoOp` carries no writer capability.

### Remaining Work

The pre-reset record said `None`; that statement is permanently invalid for promotion. Current remaining work includes every `UNRESOLVED`/`MISSING` contract row, predecessor approval, owned legacy closure, and phase-specific obligation in the redesigned gate. The receipt and mutation results are written into this run's bundle under `.build/runs/`.

## Sprint 60.4: Live scheduler binding + bootstrap→steady cutover gate ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 60.3
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
- A Register-3 proven/tested/assumed ledger recording the live scheduling authority, marking the
  release-ledger/rollback residue and the in-cluster-control-plane ownership (Phase 66) UNVERIFIED (deferred).
...
## Sprint 60.5: Register-2.5 scheduler convergence under simulated faults ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 60.4
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
below, rather than earlier in time than, Sprint 60.4's live Register-3 gate and covers concurrency behavior that
...
- A Register-2.5 ledger records convergence and fail-closed outcomes only for the explored scheduler traces.
  Fidelity of the simulated apiserver remains **assumed** and is bounded by Sprint 60.4's separate live
...
- `documents/engineering/manifest_generation_doctrine.md` — §5's amoebius scheduler-role CAS/Binding protocol
  ...
  delete/wait engine remains Phase 59's; the rollback/release ledger stays deferred.
...
- `documents/engineering/readiness_ordering_doctrine.md` — the §6 runtime-enactor claim (observe, never sleep)
  ...
  slept).
- `documents/engineering/daemon_topology_doctrine.md` — record that Phase 60 drives the scheduler role from the
  host binary; the §3 control-plane daemon that *owns* both the reconciler and its scheduling role (Deployment
  `replicas=1`, delegated single-instance, no election) must be stood up by human-approved Phase 66.
- `documents/engineering/deterministic_simulation_doctrine.md` — record the Phase-60 scheduler slice of the
  Register-2.5 io-sim battery (Sprint 60.5), with modeled-apiserver fidelity marked assumed.

**Cross-references to add:**

- `DEVELOPMENT_PLAN/README.md` — flip the Phase-60 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 60's gate substrate (linux-cpu) in the per-phase substrate
  map.
- `DEVELOPMENT_PLAN/system_components.md` — register the Phase-60
...
- [phase_59_object_reconciler.md](phase_59_object_reconciler.md) — the object reconciler (observe → diff → scoped-SSA → staged-enact → delete → wait, and the `ValidatedLiveTarget` + mandatory `Lease`) this phase is
  layered on
...
- [phase_34_render_manifest_oracles.md](phase_34_render_manifest_oracles.md) — the Haskell `renderAll` expectation corpus
  the pinned reconcile corpus is a subset of
- [phase_56_bootstrap_coordinator_kind.md](phase_56_bootstrap_coordinator_kind.md) — the live single-node `kind`
  cluster this phase's scheduler binds on
- [phase_57_base_image_registry.md](phase_57_base_image_registry.md) — the in-cluster registry and the
  preloaded/side-loaded amoebius image the bootstrap scheduler Pod uses
- [phase_66_live_dsl_deploy.md](phase_66_live_dsl_deploy.md) — the Deployment-`replicas=1` control-plane daemon that
  stands the reconciler and its scheduling role up in-cluster
- [phase_35_chain_kernel_boundary.md](phase_35_chain_kernel_boundary.md) — the `io-classes` seams / modeled
  apiserver the Register-2.5 scheduler sim (Sprint 60.5) drives the real modules on
- [phase_17_deterministic_sim_substrate.md](phase_17_deterministic_sim_substrate.md) — the deterministic-
...
- [Daemon Topology Doctrine](../documents/engineering/daemon_topology_doctrine.md) — [§3](../documents/engineering/daemon_topology_doctrine.md#3-the-control-plane-daemon) the Deployment-
  `replicas=1` control-plane daemon that will own this scheduling role in Phase 66
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the
  Register-2.5 io-sim environment the scheduler is validated against in Sprint 60.5, before the Register-3 gate
