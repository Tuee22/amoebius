# Phase 65: Keycloak-owned ingress
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_63_platform_backbone.md, DEVELOPMENT_PLAN/phase_64_platform_services_2.md, DEVELOPMENT_PLAN/phase_66_live_dsl_deploy.md, DEVELOPMENT_PLAN/system_components.md, documents/engineering/vault_pki_doctrine.md
...
- [Sprints](#sprints)
- [Sprint 65.1: The Keycloak-owned edge — LoadBalancer → Envoy/Gateway API → Keycloak ⏸️](#sprint-651-the-keycloak-owned-edge--loadbalancer--envoygateway-api--keycloak-)
- [Sprint 65.2: No self-published wild ingress + public-edge TLS ⏸️](#sprint-652-no-self-published-wild-ingress--public-edge-tls-)
- [Sprint 65.3: East-west NetworkPolicy posture — derived default-deny ⏸️](#sprint-653-east-west-networkpolicy-posture--derived-default-deny-)
- [Sprint 65.4: The single-door + storage-rebind regression gate ⏸️](#sprint-654-the-single-door--storage-rebind-regression-gate-)
...
door** for every wild request. It may compose only future human-approved results: the Phase-63 LoadBalancer
address, the **Envoy + Gateway API L7 data plane**, **Keycloak OIDC/JWT enforcement**, and typed manifests
applied by the Phase-59 reconciler. The target is that WAN, LAN, and even a
...
must be applied and exercised live. Finally, the phase must re-run the Phase-61 lossless-rebind target behind
...
app tenancy, and the Pulsar/workflow runtime are Phase 66+ concerns; this phase exercises the edge from the
host binary against the fixed standard service set targeted by Phases 63–64. The one genuinely new-vs-prodbox
...
impossibility of a self-published ingress must first receive Phase-34 and Phase-50 approval).

**Depends on:** [Phase 64](phase_64_platform_services_2.md)
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 64; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
- [`storage_lifecycle_doctrine.md` §6 — The lossless-teardown guarantee: deterministic rebind](../documents/engineering/storage_lifecycle_doctrine.md#6-the-lossless-teardown-guarantee-deterministic-rebind)
  — **the lossless-teardown guarantee: deterministic rebind**: the gate re-runs the Phase-61 marker-bytes
...
## Sprint 65.1: The Keycloak-owned edge — LoadBalancer → Envoy/Gateway API → Keycloak ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 64](phase_64_platform_services_2.md) human approval
...
- Envoy + Gateway API rendered as the L7 edge (a `Gateway` listener plus `HTTPRoute`s as typed `K8sObject`s),
  terminating TLS and routing, applied by the Phase-59 reconciler.
- Keycloak deployed against its Phase-64 Patroni DB, owning OIDC/JWT enforcement in front of every platform
...
## Sprint 65.2: No self-published wild ingress + public-edge TLS ⏸️
...
- A live audit proving there is no non-Keycloak wild path: no chart opens a backdoor NodePort to the wild, and
  ...
  edge, per the Haskell render invariant owned by Phase 34.
...
## Sprint 65.3: East-west NetworkPolicy posture — derived default-deny ⏸️
...
- A default-deny east-west baseline plus allow-edges derived from the declared dependency graph, rendered by
  the Phase-59 reconciler and applied to the live cluster — no hand-authored policy.
...
3. Prove "derived" by **graph variation**: deploy a scratch consumer, add a declared edge to a provider,
   ...
   applied set must therefore be a total function of the declared graph, not of the fixed Phase-63/43 service
...
## Sprint 65.4: The single-door + storage-rebind regression gate ⏸️
...
- Run-local image provenance that consumes the verified Phase-57 identity and current registry catalog; a
...
- The storage-rebind regression: derive run-unique marker bytes in Haskell, write them as a row into the
  ...
  run-local observer. Perform the Phase-61 intermediate
  ...
  same bytes back — the Phase-61 guarantee re-run behind the new edge. Plus applied Haskell mutant (c), a
...
5. Compare CRI-observed image identities with the verified Phase-57 run input and current registry catalog;
...
candidate must receive the Phase-57 identity directly as authenticated run input.
...
- [README.md](README.md) — flip the Phase 65 row status once work begins, and link this document from the
  Phase 65 paragraph.
- [substrates.md](substrates.md) — record `linux-cpu` as the Phase 65 gate substrate in the per-phase map.
...
- [README.md](README.md) — the live tracker; its Phase 65 row is the authoritative one-line gate and status.
...
- [Manifest Generation Doctrine](../documents/engineering/manifest_generation_doctrine.md) — the typed renderer
  + server-side-apply reconciler that enacts the edge (delivered in Phase 59).
