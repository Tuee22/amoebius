...
for every wild request. It composes the LoadBalancer address published by Phase 30 (MetalLB on `linux-cpu`)
with an **Envoy + Gateway API L7 data plane** and **Keycloak OIDC/JWT enforcement**, rendered as typed
manifests by the Phase-26 reconciler and applied to the live cluster, so that WAN, LAN, and even a
...
host binary against the fixed standard service set that Phases 30–31 stood up. The one genuinely new-vs-prodbox
...
- [`storage_lifecycle_doctrine.md` §6](../documents/engineering/storage_lifecycle_doctrine.md#6-the-lossless-teardown-guarantee-deterministic-rebind)
  — **the lossless-teardown guarantee: deterministic rebind**: the gate re-runs the Phase-28 marker-bytes
...
- A default-deny east-west baseline plus allow-edges derived from the declared dependency graph, rendered by
  the Phase-26 reconciler and applied to the live cluster — no hand-authored policy.
...
- [README.md](README.md) — flip the Phase 32 row status once work begins, and link this document from the
  Phase 32 paragraph.
- [substrates.md](substrates.md) — record `linux-cpu` as the Phase 32 gate substrate in the per-phase map.
...
- [Manifest Generation Doctrine](../documents/engineering/manifest_generation_doctrine.md) — the typed renderer
  + server-side-apply reconciler that enacts the edge (delivered in Phase 26).
