# Phase 3: Formal-model EDSL (`Model`/`interpret`/`emitTLA`)
...
Phase 3 delivers the formal-model EDSL (`Model`/`interpret`/`emitTLA`); its design is owned by [formal_model_doctrine.md](../documents/engineering/formal_model_doctrine.md), [generated_artifacts_doctrine.md](../documents/engineering/generated_artifacts_doctrine.md), [conformance_harness_doctrine.md](../documents/engineering/conformance_harness_doctrine.md), and the plan for reaching it is owned here.
Register 1: an in-process battery, no cluster.
The gate passed on 2026-08-09; Phase-4 code correspondence and runtime fidelity remain UNVERIFIED.
...
- [Sprints](#sprints)
- [Sprint 3.1: The `Model` fragment EDSL (the reifiable value) ✅](#sprint-31-the-model-fragment-edsl-the-reifiable-value-)
- [Sprint 3.2: `interpret` + the in-process reachability explorer ✅](#sprint-32-interpret--the-in-process-reachability-explorer-)
- [Sprint 3.3: `emitTLA` renderer + never-committed emission ✅](#sprint-33-emittla-renderer--never-committed-emission-)
- [Sprint 3.4: Round-trip + single-source correspondence on `ToyModel` ✅](#sprint-34-round-trip--single-source-correspondence-on-toymodel-)
...
`sha256:a0b29d7c344b8990…`; Phase 3 owns no remaining migration deferral.
...
proven-for-the-model/tested kernel result on substrate `none`, not correspondence to Phase-4 code and not
...
**not** authored here; that is [Phase 4](phase_04_gateway_migration_model.md). This phase proves the *kernel*
...
The apparatus the phase-3 gate closes over, in the slot
...
mutual-exclusion model of Sprint 3.1 — plus the QuickCheck fragment generator below. No other model is in
scope for Phase 3.
...
golden itself, **not** by any in-process liveness checker: the Sprint 3.1 structural assertion forces
...
fixed in Sprint 3.3**, the **expected `INVARIANT`/`PROPERTY` name set** the emitted `.cfg` must equal exactly
...
exist** (§M.1) — the convention-independent oracles in Phase 0, and the byte-for-byte golden in Sprint 3.3
...
per-constructor coverage percentages; and model-correspondence-to-Phase-4-code and runtime fidelity marked
...
## Sprint 3.1: The `Model` fragment EDSL (the reifiable value) ✅
...
transition relation.
**Docs to update**: `documents/engineering/formal_model_doctrine.md` (Phase-3 status
...
2. `ToyModel` is not a boolean-only strawman: it **exercises the harder fragment constructors** the round-trip
   ...
   and at least one each of an `Always`, an `Eventually`, and a `LeadsTo` temporal property — so the Sprint 3.4
   round-trip and mutation checks cannot pass while quantifier/function/fairness translation stays stubbed, and
   so the Sprint 3.3 byte golden pins the rendered bytes of **all five** liveness/fairness constructors
...
## Sprint 3.2: `interpret` + the in-process reachability explorer ✅
...
2. The `(event, state) → state` transition table and the `ToyModel` reachable-**distinct**-state count and
   ...
   independent re-derivation of the same count in Sprint 3.4 corroborates but does not replace this Phase-0
...
## Sprint 3.3: `emitTLA` renderer + never-committed emission ✅
...
`documents/engineering/generated_artifacts_doctrine.md` (the `emitTLA` row → built by Phase 3),
...
- **The canonical TLA+ rendering convention**, pinned in Phase 0 before the golden is authored — the analogue
  of the canonical Aeson encoding [`phase_14`](phase_14_render_manifest_goldens.md) pins for `renderAll`, and
...
- Because the Sprint 3.1 structural assertion forces `ToyModel` to carry **all five** liveness/fairness
  constructors, this same byte golden is the **only** oracle that pins liveness/fairness *rendering*
  faithfulness (the Sprint 3.4 differential test is legitimately safety-scoped and cannot see a
...
2. The byte golden pins the rendered bytes of **all five** liveness/fairness constructors carried by `ToyModel`
   ...
   **red** — so a renderer that mistranslates a fairness or temporal constructor cannot pass, even though the
   Sprint 3.4 differential oracle stays legitimately safety-scoped.

### Remaining Work
None. The emitter and CLI are byte-golden locked; generated `.tla`/`.cfg` files remain ignored and untracked.

## Sprint 3.4: Round-trip + single-source correspondence on `ToyModel` ✅
...
+ TLC pairing as a no-live-infrastructure instance). The Phase-3 status flip in `DEVELOPMENT_PLAN/README.md`
...
- **The `tla2tools` pin.** [`README.md`](README.md) and [`phase_01`](phase_01_toolchain_spike.md) both delegate
  this acquisition path to Phase 3, so this sprint records the **exact `tla2tools.jar` release** the harness
...
- The **generated, schema-checked Register-1 ledger** under `.build/runs/` ([§K](development_plan_standards.md#k-honesty-proven--tested--assumed); its schema and external retention are owned by `testing_doctrine.md` and Phase 0), whose Phase-3 rows are:
  ...
  differential-test case count and per-constructor coverage percentages; (d) model-correspondence-to-Phase-4-code
...
machine-derived ledger all pass. Phase-4 code correspondence and runtime fidelity remain UNVERIFIED by design.

## Documentation Requirements

**Engineering docs to update (when the gate runs, flip the honest layer, never before):**
- `documents/engineering/formal_model_doctrine.md` — backlink §2/§3/§4/§5/§6 to the Phase-3 kernel that
...
- `documents/engineering/generated_artifacts_doctrine.md` — mark the `emitTLA` row's renderer as built by
  Phase 3 and note the golden-locked, never-committed emission.
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Formal/{Model,Interpret,Explore,EmitTLA}.hs`
  and `src/Amoebius/Cli/Formal.hs` as Phase-3 design-first rows.
- `DEVELOPMENT_PLAN/phase_04_gateway_migration_model.md` — backlink: the gateway-migration `Model` is authored
...
- [Gateway Migration Model Doctrine](../documents/engineering/gateway_migration_model_doctrine.md) — the one
  concrete `Model` that rides this kernel, authored in Phase 4
...
- [phase_04](phase_04_gateway_migration_model.md) — the gateway-migration model built on this kernel
