# Phase 54: Gateway-migration drills + model-correspondence
...
- [Sprints](#sprints)
- [Sprint 54.1: The gateway-migration runtime — both branches over the Phase-10 `interpret` core ⏸️](#sprint-541-the-gateway-migration-runtime--both-branches-over-the-phase-10-interpret-core-)
- [Sprint 54.2: Teardown-with-cleanup vs chaos-failover + unsatisfiable-`.dhall` push-back ⏸️](#sprint-542-teardown-with-cleanup-vs-chaos-failover--unsatisfiable-dhall-push-back-)
- [Sprint 54.3: Register-2.5 gateway-migration runtime fidelity — simulation + trace validation ⏸️](#sprint-543-register-25-gateway-migration-runtime-fidelity--simulation--trace-validation-)
- [Sprint 54.4: Register-3 correspondence — Inject drills against the running forest + live gate `.dhall` + ledger ⏸️](#sprint-544-register-3-correspondence--inject-drills-against-the-running-forest--live-gate-dhall--ledger-)
...
Done (invalidated). The runtime delegates every modeled edge to the Phase-10 `interpret` core, validates both pinned traces
...
Because [Phase 53](phase_53_multicluster_spawn_georepl.md) turned the single cluster into a geo-replicated
...
modeled route53/Pulsar and its observed transition log is validated step-by-step against the Phase-10 emitted
spec's `Next` relation, pulling the runtime-fidelity obligation forward from Register-3-only chaos into
deterministic, replayable simulation. Fourth, **the correspondence** — because Phase 10 rendered one reifiable
...
This phase consumes earlier phases and does not re-implement them: Phase 53's geo-replicated forest and
invariant-confluence classifier, Phase 10's `GatewayMigration` `Model` + `interpret` + decode-time
structural-fit fold, Phase 43's Keycloak-owned wild ingress, Phase 52's WireGuard fabric (whose hub role the
`Planned` handover repoints), and Phase 22's (Sprints 15.1/15.2) `io-classes` seams and modeled route53/Pulsar. A
...
**Substrate:** linux-cpu — the gate drives the migration over the parent and both child clusters that Phase 53
spins up as `kind` clusters on a single linux-cpu host; no accelerator and no provider cluster is in
scope (provider-managed clusters are [Phase 55](phase_55_provider_deploy_checkpoint.md)). Partition tolerance is
...
- **The five named invariants, and correspondence.** The Register-3 Inject drills assert all five Phase-10 safety
  ...
  `src/Amoebius/Multicluster/*` runtime is the Phase-10 `GatewayMigration` model's `interpret` is discharged as
  the step-by-step trace-validation of Sprint 54.3 (Register 2.5); Register 3 adds only the
...
- **External-observer teardown check.** Phase 54 claims only its pre-Phase-62 teardown scope; the later
  ...
  `no-provisioner` PVs that Sprint 54.2 deliberately preserves are explicitly exempt (named in the fixture as the
  retained set).
- **Machine-derived ledger + validator.** The ledger is generated from the run record (measured RPO/RTO,
  ...
  *proven-for-the-model at scope 2* — a Phase-10 design result, never a runtime guarantee. Layers outside
...
flowchart LR
  ...
  g{{"the phase 54 gate command"}}:::gate
...
```
*Design intent. Phase 54's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
The runtime executes inside the Phase-44 control-plane daemon, whose complete pod envelope is expanded for migration-plan
...
clusters' Phase-43 edge controllers, operator-derived data-plane children, admission webhooks/gateways, and
rollout operands; the source transparent proxy and target serving path coexist through the full DNS drain
window. The Phase-52 `NetworkFabricSystemDemand` is derived for the source, overlap, and target peer/hub
graphs, including kernel/listener CPU and memory, queues, and rotated nodefs logs. The route53 repoint retains
the Phase-53 in-cluster `PulumiExecutionDemand`, exact executor-Job envelopes, plugin/workspace volumes,
...
The non-idle drill workload is provisioned too. Its source-equal Phase-46/48 client/workflow projection carries
...
- [`gateway_migration_doctrine.md §2`](../documents/engineering/gateway_migration_doctrine.md#2-the-planned-branch--a-coordinated-strong-consistency-handover)
  ...
  built as the effectful shell around the Phase-10 decision core, honest per
...
- [`deterministic_simulation_doctrine.md §4`](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  ...
  route53/Pulsar, trace-validated against the Phase-10 emitted spec's `Next` relation before the Register-3 live
...
## Sprint 54.1: The gateway-migration runtime — both branches over the Phase-10 `interpret` core ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `src/Amoebius/Multicluster/GatewayMigration.hs` (the effectful
orchestrator whose per-edge decision delegates to the Phase-10 `src/Amoebius/Formal/GatewayMigration.hs`
...
[`chaos_failover_second_axis.md §18`](../documents/engineering/chaos_failover_second_axis.md#18-the-rules-scale-to-the-boundary):
build the effectful gateway-migration shell for **both** branches whose every branch decision is the Phase-10
...
## Sprint 54.2: Teardown-with-cleanup vs chaos-failover + unsatisfiable-`.dhall` push-back ⏸️
...
## Sprint 54.3: Register-2.5 gateway-migration runtime fidelity — simulation + trace validation ⏸️
...
forest code lifted onto the Phase-22 `io-classes` `Env` interface — delivered as the deterministic pure
...
Phase-10 emitted spec, pulling the runtime-fidelity (Tier-2) obligation **forward** from Register-3-only chaos
into deterministic, replayable simulation. Substrate `none`, Register 2.5.
**Docs to update**:
`documents/engineering/deterministic_simulation_doctrine.md` (Phase-54 status backlink),
...
simulation against the modeled world here (Register 2.5), then as live Inject drills (Sprint 54.4, Register 3)
...
- A Register-2.5 proven/tested/assumed ledger — the built forest upholds the safety invariants and refines the
  ...
  replication-lag / clock-skew physics remain the Register-3 residue (Sprint 54.4).
...
## Sprint 54.4: Register-3 correspondence — Inject drills against the running forest + live gate `.dhall` + ledger ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `test/fixture/dhall/phase_44_gateway_migration.dhall` (the live gate
topology) and `test/spec/inject/GatewayMigrationForest.hs` (the Register-3 Inject harness re-using the Phase-10
named invariants against the built runtime), plus `tools/gateway_migration_drills_live.py` — delivered. The Phase-10 `GatewayMigration`
`Model`, its `emitTLA` proof, and its io-sim agreement were authored and discharged in Phase 10; this sprint
consumes them, it does not re-author them.
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: the built `src/Amoebius/Multicluster/*` decision core is shown to be the Phase-10
...
- The Register-3 Inject drill set run in the inter-cluster dimension against the running forest — cut
  ...
  past the bound, and fail back with late plus duplicate arrivals — each asserting the Phase-10 named invariants.
...
- `test/fixture/dhall/phase_44_gateway_migration.dhall`: spin two children up, geo-replicate, run a `Planned` handover
  ...
  and `promote-before-fence` mutants that must go red) mutate the Sprint-49.1 implementation, so they are
  **committed at the start of Phase 54, before that implementation is trusted** (the §M.1
  start-of-owning-phase form for oracles that depend on later code).
- A machine-derived per-run proven/tested/assumed ledger that marks `Planned` RPO=0 **proven-for-the-model (Phase 10, scope 2) + drilled (tested)**, the `Failover` recovery time + reconciliation **tested (drilled)**, the data-loss /
  replication-lag bound **assumed (monitored, never proven)**, and the modeled safety/liveness
  **proven-for-the-model at scope 2** (Phase-10, a design-layer result) — and never reports an
  assumed-and-monitored result as proven.

### Validation
1. The built decision core resolves to the Phase-10 `interpret` — the correspondence-check mechanic is fixed as:
   the step-by-step trace-validation of Sprint 54.3 (Register 2.5) **plus** a Register-3 **modeled-action-coverage assertion** that every modeled action fired ≥ 1 time across the drill set (no orphaned modeled action); step-by-step
   trace-validation is the Sprint-49.3 obligation and is not re-run in Register 3. The Inject drills run against
...
- `documents/engineering/gateway_migration_doctrine.md` — backlink §2/§3/§4/§5 to the built
  `src/Amoebius/Multicluster/*` runtime; confirm `Planned` RPO=0 stayed proven-for-the-model (Phase 10, scope 2; now drilled live) and
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `src/Amoebius/Multicluster/GatewayMigration.hs`,
  ...
  modules and the Register-2.5 simulation + Register-3 Inject + gate suites as Phase-54 rows.
- `DEVELOPMENT_PLAN/substrates.md` — the Phase-54 → linux-cpu row in the per-phase substrate map.
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the Register-2.5 io-sim + trace-validation (Sprint 54.3) that pulls the runtime-fidelity obligation forward from Register-3-only chaos
...
- [phase_10](phase_10_gateway_migration_model.md) — the `GatewayMigration` design-model whose Register-3 correspondence against the built forest is discharged here
- [phase_53](phase_53_multicluster_spawn_georepl.md) — the prior phase; the geo-replicated forest and confluence classifier this phase runs over
- [phase_55](phase_55_provider_deploy_checkpoint.md) — the next phase; the forest extended to provider-managed clusters
