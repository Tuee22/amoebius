# Phase 50: infernix core artifact lift
...
- [Sprints](#sprints)
- [Sprint 50.1: Lift infernix through one scoped artifact adapter ⏸️](#sprint-501-lift-infernix-through-one-scoped-artifact-adapter-)
...
sibling source boundary is reviewed and resolved dynamically. Store operations use the Phase-38 three-tier content-addressed store;
commands and events use typed native CBOR over Pulsar; credentials remain Vault `SecretRef` names; named
engines resolve through the Phase-49 jit-build cache; CPU decode uses a deterministic pinned micro-model.
...
preserves it unchanged as the Phase-38 work-id in the canonical command payload and in every progress or
terminal event; it never derives a replacement from a pod, retry, run id, or artifact digest. Producer resends
retain the producer identity/sequence required by Phase 36, while consumer redelivery folds on that stable
...
raw provider handle. Phase 51 is the sole infernix-to-UI boundary; this phase owns no frontend, browser probe,
...
and buffer bounds, and complete CPU/memory/ephemeral demand to the inherited Phase-38 workflow envelope. Its
...
accept it with the Phase-50 aggregate gate. Split if the work changes sibling inference algorithms, adds a
...
- **Independent oracle and reversibility.** The sibling expected behavior is recomputed by the independently
  built sibling reference during the run; its output stays under `.build/runs/phase_50/`. The reviewed
...
## Sprint 50.1: Lift infernix through one scoped artifact adapter ⏸️
...
- `DEVELOPMENT_PLAN/phase_51_infernix_ui_lift.md` — remain the sole owner of infernix UI projection and
  interaction.
...
- [Phase 51 — infernix UI lift](phase_51_infernix_ui_lift.md)
- [Phase 52 — jitML lift](phase_52_jitml_lift_cuda.md)
