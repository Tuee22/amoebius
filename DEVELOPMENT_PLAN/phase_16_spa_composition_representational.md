# Phase 16: SPA composition (representational) + demo-SPA local
...
is design intent, never a tested amoebius result. This phase opens after its numeric predecessor Phase 15 is
validated (the README phase-order rule), substantively building on the Phase 14 boundary fake-tool harness it
...
stays [Phase 43](phase_43_spa_live_deploy.md).
...
[Phase 43](phase_43_spa_live_deploy.md). This phase front-loads exactly the parts of SPA composition that a
...
([Phase 43](phase_43_spa_live_deploy.md)) UNVERIFIED. The gate is red unless the three committed seeded mutants
...
## Sprint 16.1: The SPA app-spec type — a multi-service surface of capability needs 📋
...
Phase 10 (the capability → provider → shape binder the SPA's needs resolve against)
...
## Sprint 16.2: Compose an ML-workflow demo fragment as shared-library use — `prop_spaCompositionDecodes` 📋
...
built)
**Blocked by**: Sprint 16.1; Phase 5 (the total Gate-2 `Dhall.inputFile auto` decoder the composed value
...
2. `prop_spaCompositionDecodes` is green at the [Gate integrity](#gate-integrity) `cover` thresholds (a run below them fails), and both committed
   ...
   live deploy left to [Phase 43](phase_43_spa_live_deploy.md).

### Remaining Work
The whole sprint (📋 Planned).

## Sprint 16.3: `purescript-bridge` contract generation from the composed ADTs (never committed) 📋
...
**Blocked by**: Sprint 16.2 (the composed Haskell app/workflow ADTs the contract reflects)
...
## Sprint 16.4: The demo-SPA-local gate — Playwright against a faked backend + the ledger 📋
...
**Blocked by**: Sprint 16.2; Sprint 16.3; Phase 14 (the boundary fake-tool / faked-backend harness this reuses
...
deploy ([Phase 43](phase_43_spa_live_deploy.md)) as UNVERIFIED.
...
- A `test/spa/DemoSpaLocalSpec.hs` that serves **both** demo SPAs locally, stands up a faked backend that
  records its calls at the OS boundary (argv/HTTP-access log, reusing the Phase-14 fake-tool seam) **and
...
- A composite gate ledger recording: the SPA composition + `prop_spaCompositionDecodes` as **proven for the
  ...
  [Phase 43](phase_43_spa_live_deploy.md)).
