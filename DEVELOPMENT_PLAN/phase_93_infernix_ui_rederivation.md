# Phase 93: The infernix workflow and artifact contracts, re-derived
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_82_ui_single_tenant_live.md, DEVELOPMENT_PLAN/phase_92_infernix_rederivation.md, DEVELOPMENT_PLAN/phase_94_jitml_rederivation.md
...
- [Sprints](#sprints)
- [Sprint 93.1: Bind the infernix workflow-to-interaction UI adapter ⏸️](#sprint-931-bind-the-infernix-workflow-to-interaction-ui-adapter-)
...
Blocked by redesigned Phase 92, its independent validation, and human promotion; every earlier
...
This phase owns one adapter seam: the scoped Phase-92 `ReadyArtifactHandle` and Phase-71 durable-receipt fold
...
app/tenant/owner/port context, and makes that command identity the work identity. Its Phase-71 fold produces a
...
does not establish correspondence with the complete Phase-92 inference chain, production TinyLlama, or the
...
command; it excludes a new generic UI constructor, Phase-92 compute change, or second client runtime.
...
**Depends on:** [Phase 92](phase_92_infernix_rederivation.md)
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 92; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
## Sprint 93.1: Bind the infernix workflow-to-interaction UI adapter ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 92](phase_92_infernix_rederivation.md) human approval
...
- One Haskell-declared UI module using only trusted catalog components and the Phase-73 release path; any Dhall
...
1. The pre-reset Python command is rejected and must not run. The future Haskell Phase-93 supporting suite must run on `linux-cpu`.
...
replicas, Phase-93 native CBOR, full Phase-92 inference-output correspondence, production TinyLlama,
...
- `documents/engineering/ui_realtime_coordination_doctrine.md` — record only the tested infernix
  terminal-event-to-receipt correlation; cross-pod Redis/socket fault behavior remains Phase 85's claim.
...
- `DEVELOPMENT_PLAN/system_components.md` — register the Haskell infernix UI adapter and Haskell projection
  emitter under Phase 93; its generated Dhall output remains untracked and is not a component-inventory row.
...
- [Phase 92 — infernix lift](phase_92_infernix_rederivation.md)
