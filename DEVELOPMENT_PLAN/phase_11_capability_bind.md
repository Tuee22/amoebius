# Phase 11: Capability union + representational bind
...
Phase-11 capability-bind ledger.
...
- [Sprints](#sprints)
- [Sprint 11.1: The closed capability union + the no-product-arm Gate-1 foreclosure ✅](#sprint-111-the-closed-capability-union--the-no-product-arm-gate-1-foreclosure-)
- [Sprint 11.2: The `CapabilityBinding` + total representational `bind` ✅](#sprint-112-the-capabilitybinding--total-representational-bind-)
- [Sprint 11.3: The bind property/corpus + the Register-1 gate ✅](#sprint-113-the-bind-propertycorpus--the-register-1-gate-)
...
bound to source snapshot `sha256:f11ac11d30e778b9…`; Phase 11 owns no remaining migration deferral.
...
per run, failing closed when unset. And, as in Phases 10 and 11's siblings, the join found five contract
...
did not produce, and the gap is recorded against Phase 11 in
...
([Phase 12](phase_12_provision_seal.md)); the `InferenceEngine` family×lane availability relation, the
target-offering→lane quotient, and the accelerator residency/coexistence provision
([Phase 13](phase_13_inference_accelerator_provision.md)); the pure
`renderAll :: ProvisionedSpec -> [K8sObject]` ([Phase 14](phase_14_render_manifest_goldens.md)); and the live
jit-resolve of an engine into its `CacheBudget`-bounded cache ([Phase 53](phase_53_determinism_jitcache.md)).

**Substrate:** none — no host, no cluster, no provider; the gate is an in-process `cabal test` bind + property +
corpus battery, analogous to the Phase-6 decode battery.
...
availability stay UNVERIFIED here; they are Phases 12 and 13.

## Gate integrity

This section is the [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) apparatus for the Phase-11 gate — the **bind-core slice** of the source corpus, partitioned
along this seam. It does not duplicate the provision-seal or accelerator corpora, which are owned by
[Phase 12](phase_12_provision_seal.md) and [Phase 13](phase_13_inference_accelerator_provision.md) respectively.
...
flowchart LR
  ...
  g{{"the phase 11 gate command"}}:::gate
...
```
*Design intent. Phase 11's gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
- The **expected error/locus tags** for every negative in `dhall/examples/`: the asserted `dhall type` error
  locus (Gate 1) or the specific `DecodeError` tag (Gate 2) for each of `illegal_product_in_app`,
...
id and honest foreclosure layer, backed by **Phase-7-style coverage-assertion machinery**: the suite goes **red**
...
- [`service_capability_doctrine.md §4.1`](../documents/engineering/service_capability_doctrine.md#41-the-inferenceengine-capability--the-engine-is-target-offering-selected-and-jit-resolved-never-authored)
  ...
  **This phase adopts only the union's representational shape** — the closed `EngineRuntime` lane union with **no arbitrary-`Url`/`Download` arm**, so "name the engine by URL" has no syntax and fails Gate 1. The family×lane
  ...
  jit-resolve are **not** here (Phases 13 / 39).
...
— is deliberately **not** adopted here; it is owned by [Phase 12](phase_12_provision_seal.md). This phase stops
...
## Sprint 11.1: The closed capability union + the no-product-arm Gate-1 foreclosure ✅
...
**Docs to update**:
`documents/engineering/service_capability_doctrine.md` (Phase-11 status backlink for §1/§2 and the §4.1
...
- The `InferenceEngine` head's closed `EngineRuntime` lane union (`AppleMetal` · `Cuda` · `LinuxCpu`) with **no arbitrary-`Url`/`Download` arm** — an ML engine is a **named catalog identity**, so "name the engine by URL"
  ...
  owner demands are [Phase 13](phase_13_inference_accelerator_provision.md).
...
- An in-file honesty note that this union is the app-facing *what*; the provider/shape *how* is Sprint 11.2, the
  provision seal is [Phase 12](phase_12_provision_seal.md), and the capability set is invariant across every
...
2. Each Gate-1 negative fails at a named locus rather than merely somewhere. `illegal_product_in_app`, which
...
### Remaining Work
None. The closed union, need records, and Gate-1 corpus are sealed by the Phase-11 gate.

## Sprint 11.2: The `CapabilityBinding` + total representational `bind` ✅
...
- `bind :: CapabilityNeed -> CapabilityBinding -> BoundServiceSpec`, pure and total, selecting the provider's
  **manifest graph for the chosen shape** — a structurally different object graph, not a scalar `replicas` edit.
  ...
  [Phase 12](phase_12_provision_seal.md), never an authored or bound peak here.
- Controller-child lowering: for every operator/CR arm a version-pinned expander joins the descriptor's exact
  ...
  [Phase 12](phase_12_provision_seal.md)'s, not a second unit here). A caller cannot author a scalar child peak,
...
- `BoundDeployment` retains **only** these unprovisioned units, the opaque source ref, and controller
  ...
  `ProvisionContext` is [Phase 12](phase_12_provision_seal.md)'s work, not `bind`'s. `bind` carries the typed
...
- An in-file honesty note: a single-node shape is the canonical provider deployed honestly at small scale (a
  ...
  not a provisioned deployment — the provision seal is [Phase 12](phase_12_provision_seal.md) and the live
  realization is the live band. The source inventory this sprint authors imports none of that: not the
  [Phase 12](phase_12_provision_seal.md) `provision`/`ProvisionedSpec` machinery, not the
  [Phase 10](phase_10_execution_accelerator_folds.md) execution-epoch fold, and not the
  [Phase 14](phase_14_render_manifest_goldens.md) `K8sObject`/Aeson renderer.
...
1. The same `CapabilityNeed`, bound under two shapes, produces two `BoundServiceSpec`s that are **structurally different by the object-node-multiset oracle** (deep structural diff per [§5](../documents/engineering/service_capability_doctrine.md#5-per-cluster-structural-shapes--beyond-values), red on a scalar-only or copied-shape-tag difference; each equal to its oracle-pinned golden), while the **app-surface bytes**
   ...
   validation must go **red** on the committed `mutant_copy_shape_tag` seeded mutant (Sprint 11.3) — which makes
...
## Sprint 11.3: The bind property/corpus + the Register-1 gate ✅
...
- The **concrete positive corpus** — **one fixture per each of the nine capability arms** (`ObjectStore`,
  ...
  (Sprint 11.1), independent of `bind`'s own case analysis.
...
- The negative corpus — `illegal_product_in_app` ([§3.12](../documents/illegal_state/illegal_state_capability_messaging.md#312-an-app-that-names-a-product-instead-of-a-capability), Gate 1), `illegal_engine_by_url` ([§3.25](../documents/illegal_state/illegal_state_ml_asset.md#325-an-ml-asset-named-by-arbitrary-url-or-an-unready--unlanded-model), Gate 1),
  ...
  [Phase 12](phase_12_provision_seal.md) and [Phase 13](phase_13_inference_accelerator_provision.md).
...
- A Register-1 validation-locus ledger mapping every entry to its catalog id and layer, backed by
  **Phase-7-style coverage-assertion machinery** (the ledger is not a static hand-written file: the suite goes
...
1. `cabal test capability-bind-spec` is green — each of the **nine per-arm** positives binds byte-invariant
   ...
   present and its coverage-assertion machinery (Phase-7 precedent) turns the suite **red** if any named fixture,
   ...
   out of scope here and validated by Phases 12/13.
...
- `documents/engineering/content_addressing_doctrine.md` — reconcile §4.5's Tier-1 engine as the
  ...
  cache as the live-band residue ([Phase 53](phase_53_determinism_jitcache.md)).
...
- `DEVELOPMENT_PLAN/system_components.md` — register `dhall/amoebius/Capability.dhall`,
  `src/Amoebius/Capability/{Types,Binding}.hs`, and the capability bind property + gate suites as Phase-11
  design-first rows.
...
- [phase_06](phase_06_gadt_decoder_gate2.md) — Gate 2, the IR + decoder the bound specs project from
- [phase_07](phase_07_illegal_state_corpus.md) — the illegal-state corpus + validation-locus ledger machinery
  this gate reuses
- [phase_10](phase_10_execution_accelerator_folds.md) — the execution-epoch / runtime-storage / accelerator
  folds whose kind-indexed vocabulary `bind`'s `BoundExecutionUnit`s are fold-compatible with
- [phase_12](phase_12_provision_seal.md) — the whole-deployment provision seal that layers `planInfrastructure`/
...
- [phase_13](phase_13_inference_accelerator_provision.md) — the `InferenceEngine` family×lane availability
...
- [phase_14](phase_14_render_manifest_goldens.md) — the pure deployment-global
  `renderAll :: ProvisionedSpec -> [K8sObject]` downstream of the provision seal - [phase_53](phase_53_determinism_jitcache.md) — the live jit-build engine resolver + `CacheBudget` cache that
