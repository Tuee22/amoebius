...
- [Sprints](#sprints)
- [Sprint 17.1: Author the `GatewayMigration` `Model` — both branches 📋](#sprint-171-author-the-gatewaymigration-model--both-branches-)
- [Sprint 17.2: `emitTLA` render + TLC exhaustive proof (both branches) 📋](#sprint-172-emittla-render--tlc-exhaustive-proof-both-branches-)
- [Sprint 17.3: io-sim agreement + seeded-mutation catch 📋](#sprint-173-io-sim-agreement--seeded-mutation-catch-)
- [Sprint 17.4: Scope-2 pairwise cutoff + decode-time structural-fit fold 📋](#sprint-174-scope-2-pairwise-cutoff--decode-time-structural-fit-fold-)
...
⏸️ Blocked pending Phase-16 revalidation. Reopened 2026-08-19 by the generative re-baseline: the artifact, budget, lift, workflow and evidence calculi change what this phase's gate must cover, so any earlier seal is history and no longer presents completion evidence.
...
naming Phase 17 in its own run bundle rather than Phase 11.

**Opened 2026-08-17** when Phase 11 resealed; **reopened 2026-08-16 by the natural-architecture amendment.**
...
`sha256:ab0f84be5f1d3562…`; Phase 17 owns no remaining migration deferral.
...
`.build/runs/phase_51/<run-id>/`, and 15 surfaces join to 17 run-time enumerated items.
...
five named invariants — from which the Phase-11 `interpret` (the runtime decision core) and `emitTLA` (the
...
**Phase scope:** one cohesive claim — *amoebius has exactly one formal proof obligation, and both its branches are discharged from one model*. The structural-fit fold is what reduces that model to every `InForceSpec`.

**Substrate:** `none` — no host, no cluster. The gate is an in-process check battery (TLC + io-sim +
explorer), analogous to the Phase-0 documentation lint and the Phase-11 kernel round-trip.

**Lane:** none ([§L](development_plan_standards.md#l-one-substrate-discipline))

**Register:** 1 — pure/golden, in-process, no cluster ([§K](development_plan_standards.md#k-honesty-proven--tested--assumed)).

**Depends on:** [Phase 16](phase_16_deterministic_sim_substrate.md) — deterministic-simulation substrate, which this phase consumes rather than rebuilds.

**Gate:** `python3 tools/gateway_migration_model_gate.py` is green: over the one reifiable `GatewayMigration`
...
The suite model-checks both migration branches; the gate additionally compares the emitted module against the hand-authored transition table and drives the fairness-drop mutants.


This section pins the apparatus the Phase-17 gate closes over, in the slot
...
authored in Sprint 17.1, is enabled on some reachable state. An invariant whose antecedent is unreachable, or
...
[Phase 11](phase_11_formal_model_kernel.md#phase-summary)** is caught on this concrete model. A **committed
...
[`phase_11`](phase_11_formal_model_kernel.md#phase-summary) §M.1 pins the `ToyModel`
...
## Sprint 17.1: Author the `GatewayMigration` `Model` — both branches 📋
**Status**: Planned
**Implementation**: `src/Amoebius/Formal/GatewayMigration.hs` (the concrete `Model`
value + its five named invariants), atop the Phase-11
...
**Independent Validation**: the value typechecks against the Phase-11 `Model` EDSL, and the reachability
...
(Phase-17 status backlink), `documents/engineering/backup_recovery_doctrine.md` (§8 — the `FreshnessWitness`
...
### Deliverables
- The `GatewayMigration` `Model` value in the Phase-11 first-order fragment, both branches expressed as guarded
...
### Validation
1. The value typechecks against the Phase-11 `Model` EDSL; the explorer visits both branches and enumerates a
...
## Sprint 17.2: `emitTLA` render + TLC exhaustive proof (both branches) 📋
**Status**: Planned
...
## Sprint 17.3: io-sim agreement + seeded-mutation catch 📋
**Status**: Planned
...
## Sprint 17.4: Scope-2 pairwise cutoff + decode-time structural-fit fold 📋
**Status**: Planned
...
- A QuickCheck property over random migration graphs asserting **accepts ⟺ pairwise ∧ independent ∧ acyclic ∧ in-parameter-envelope** (equivalence) — the *in-parameter-envelope* conjunct co-equal with the
  ...
  `CONSTANTS` — the four regime bounds being the Sprint-11.1 model `CONSTANTS`, oracle-pinned in the §M.1
...
- `DEVELOPMENT_PLAN/phase_74_multicluster_spawn_georepl.md` — backlink: this design-model is the artifact
