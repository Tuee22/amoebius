# Phase 90: Apple-Metal host compute daemon
...
> sole Phase-70 mutation gateway fronting MinIO, with the native worker built **headless
...
> **Read this if**: phase 90 is next in the queue, or a later phase depends on what its gate establishes.
...
- [Sprints](#sprints)
- [Sprint 90.1: Apple substrate management — Lima Linux VM + brew lazy tool-ensure ⏸️](#sprint-901-apple-substrate-management--lima-linux-vm--brew-lazy-tool-ensure-)
- [Sprint 90.2: Host-only loopback NodePort exposure of the content-mutation gateway + Pulsar ⏸️](#sprint-902-host-only-loopback-nodeport-exposure-of-the-content-mutation-gateway--pulsar-)
- [Sprint 90.3: Headless host-native Metal bridge + native worker build (no Tart) ⏸️](#sprint-903-headless-host-native-metal-bridge--native-worker-build-no-tart-)
- [Sprint 90.4: Host compute daemon lifecycle as a managed subprocess ⏸️](#sprint-904-host-compute-daemon-lifecycle-as-a-managed-subprocess-)
- [Sprint 90.5: Channel-2 peer + wild-exposure unrepresentable + the Apple-Metal peer gate ⏸️](#sprint-905-channel-2-peer--wild-exposure-unrepresentable--the-apple-metal-peer-gate-)
...
binds the in-cluster Phase-70 content-mutation gateway and Pulsar service to **host-only loopback NodePorts** reachable only
...
messages, mutations pass only through the Phase-70 gateway into the content-addressed MinIO store, and there is
...
The scope stops at the host-worker shell and its wire. [Phase 81](phase_81_determinism_jitcache.md) owns resolution
...
predecessor capabilities rather than re-implement them: Phase 56's substrate detection, `pb` bootstrap
coordinator handoff, and no-`PATH` tool-ensure kernel; Phase 63's MinIO
and Pulsar standard services; Phase 68's native Pulsar client; Phase 70's content-addressed store and workflow
runtime; Phase 81's determinism kernel; Phase 81's jit-build engine cache; and Phase 62's Vault secrets-by-name.

```mermaid
flowchart LR
%% register: orientation
  apple[Apple Silicon host: detected apple substrate] --> lima[Lima Ubuntu-24.04 VM: single-node cluster]
  lima --> svc[Phase-70 content-mutation gateway fronting MinIO plus Pulsar]
...
**Depends on:** [Phase 89](phase_89_offline_multizone_continuity.md)
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 89; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
The worker's source-equal Phase-68/49 work/result topic and subscription demand also retains finite message/
...
The host content client terminates at the Phase-70 sole content-mutation gateway NodePort; raw MinIO backend
...
The content gateway/collector, MinIO backend, Pulsar, Vault and Phase-66 control-plane daemon instances are inherited
...
`HostProcessReplacementPolicy.MetalDrainThenReplaceAfterObservedExit`. The host resident ledger this supervisor
maintains is an inner ledger with its own compare-and-swap root, and it is subordinate to the outer host
claim under [hostclaim_spec.md](../documents/engineering/hostclaim_spec.md): its reservations are bounded by
the claim amoebius already holds on the machine, and its CAS never substitutes for admission there. Its supervisor first CAS-reserves the
...
This phase must apply the exhaustive desired-object and etcd-capacity protocol that human-approved Phase 74 must supply to the
...
- [`host_cluster_comms_doctrine.md` §2 — The decision that was open, and is now resolved](../documents/engineering/host_cluster_comms_doctrine.md#2-the-decision-that-was-open-and-is-now-resolved)
  ...
  (*coordination is Pulsar + MinIO*, with MinIO mutation mediated by the Phase-70 gateway), the
...
## Sprint 90.1: Apple substrate management — Lima Linux VM + brew lazy tool-ensure ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 89](phase_89_offline_multizone_continuity.md) human approval
...
invoked by absolute path through brew — the substrate foundation every later Phase-90 sprint stands on. The
...
## Sprint 90.2: Host-only loopback NodePort exposure of the content-mutation gateway + Pulsar ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 90.1
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
## Sprint 90.3: Headless host-native Metal bridge + native worker build (no Tart) ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 90.2
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
- Runtime MSL compilation: the host binary renders Metal Shading Language, writes a content-addressed
  source-metadata cache record into the Phase-81 `CacheBudget`-bounded cache, and dispatches through the
  bridge's `MTLDevice.makeLibrary(source:options:)` with fast-math **off** (the Phase-81 determinism
...
## Sprint 90.4: Host compute daemon lifecycle as a managed subprocess ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 90.3
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
## Sprint 90.5: Channel-2 peer + wild-exposure unrepresentable + the Apple-Metal peer gate ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 90.4
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
- The reviewed Haskell gate declaration lazily emits its untracked Dhall transport at
  `.build/test-corpora/dhall/phase_76_apple_metal_peer.dhall` at gate-run time; byte authority remains in the Haskell emitter in
...
1. Each of the four independently authored Haskell wild-exposure negatives, with serialized forms generated
   lazily beneath `.build/test-corpora/dhall/phase_76_illegal/` (NodePort as
...
Sprint 90.3 instead retains Haskell reference logic and lazy `.build/**` outputs. The pure peer/auth and
...
- `documents/engineering/host_cluster_comms_doctrine.md` — its §9 planning-ownership pointer resolves to
  delivered Phase-90 sprints, and the §2/§5/§6 honesty notes flip from "resolved design decision / sibling
...
- `documents/engineering/substrate_doctrine.md` — its §9 planning-ownership pointer resolves to delivered
  Phase-90 sprints; the §4.3 "no macOS build VM" note and the §5 host-worker description gain their first
...
- `documents/engineering/resource_capacity_doctrine.md` — record the Phase-90 live check for private
...
- [README.md](README.md) — flip the Phase-90 row status once the gate passes and link this document.
- [substrates.md](substrates.md) — record Phase 90's gate substrate (apple) in the per-phase substrate map, and
...
- [system_components.md](system_components.md) — register the host-worker and host-comms modules
  ...
  as Phase-90 design-first rows.

## Related Documents

- [README.md](README.md) — the live tracker; the Phase 90 row is the authoritative one-line gate and status
...
- [Substrate Doctrine](../documents/engineering/substrate_doctrine.md) — the apple host worker, the Lima
  ...
  not a Phase-90 implementation surface
...
- [phase_81](phase_81_determinism_jitcache.md) — the jit-build engine resolver + `CacheBudget` cache the Metal
  kernel is materialized into
- [phase_94](phase_94_jitml_rederivation.md) — the CUDA jitML lift; its Windows-CUDA case is the structurally
