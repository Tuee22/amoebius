# Phase 52: Dynamic node provisioning by signal + leak-free provider gate
...
AWS audit, and provider leak-freedom remain UNVERIFIED because Phase 49 could not authenticate to AWS.
...
- [Sprints](#sprints)
- [Sprint 52.1: Dynamic node provisioning by signal ⏸️](#sprint-521-dynamic-node-provisioning-by-signal-)
- [Sprint 52.2: Phase gate — spin a provider cluster, provision a node by signal, tear down leak-free ⏸️](#sprint-522-phase-gate--spin-a-provider-cluster-provision-a-node-by-signal-tear-down-leak-free-)
...
⏸️ Blocked pending Phase-51 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
by a tag-only sweep. It cleaned every Phase-52 object. This is a scoped Kubernetes/cloud-metadata analogue,
...
linux-cpu (the [Phase 29](phase_29_bootstrap_coordinator_kind.md) bootstrap coordinator), from inside which the
...
This sub-phase **layers on** its three provider siblings — [Phase 49](phase_49_provider_deploy_checkpoint.md)
(the provider-cluster Pulumi deploy + Vault-Transit-enveloped MinIO checkpoint + `observeProviderAccount`),
[Phase 50](phase_50_provider_child_bringup.md) (the stateless hostless in-cluster singleton + capacity-scheduler roles + parent→child Lease handoff), and [Phase 51](phase_51_provider_ebs_credential.md) (per-PV EBS decoupled from the node lifecycle + the create-vs-delete credential + static EBS CSI) — and owns dynamic node provisioning
...
set, and drives the live set toward it through the [Phase 31](phase_31_object_reconciler.md)
...
grown bound ([Phase 8](phase_08_capacity_core_folds.md) placement, [Phase 9](phase_09_storage_geometry_folds.md) storage geometry, [Phase 10](phase_10_execution_accelerator_folds.md) execution/accelerator residency) and proves
...
MinIO backend — runs through the [Phase 12](phase_12_provision_seal.md) post-bind observe-then-plan boundary and
seals nothing until receipt-bound provider readback. Join is quarantined behind the `ManagedCapacity` taint,
full supply/layout/device observation, and a [Phase 32](phase_32_capacity_scheduler.md) scheduler-generation CAS;
an `Unreachable` node observation **refuses** rather than stranding an EC2 instance.

Second, the **phase gate** (Sprint 52.2): a single `.dhall` that, from a linux-cpu parent, spins up the EKS
provider cluster ([Phase 49](phase_49_provider_deploy_checkpoint.md)), converges its stateless in-cluster
control plane ([Phase 50](phase_50_provider_child_bringup.md)), dynamically provisions an extra node **by evaluating a declared signal rule** (Sprint 52.1), binds the Pulumi-created durable EBS through the static
`ebs.csi.aws.com` CSI PV and writes a run-unique marker ([Phase 51](phase_51_provider_ebs_credential.md)),
...
is [Phase 56](phase_56_test_topology_dsl.md) work, deferred and never depended on here.
...
flowchart LR
%% register: algebra
  signal["declared ScalingPolicy signal: load / workflow-completion"]:::intent --> fold{{"re-run Phase 8/9/10 fold against grown bound + provider-quota + account residual"}}:::gate
...
reclamation *explicitly deferred to [Phase 56](phase_56_test_topology_dsl.md)*.

**Gate:** `cabal test provider-dynamic-nodes-live` is green: `test/dhall/phase_48_provider_provision.dhall`
...
> **Provider corpus split (by design).** `test/dhall/phase_48_provider_provision.dhall` and the
...
`test/negatives/host_shell_pulumi_up.sh`, and the `mut-44.1-static-key` / `mut-44.1-leak-path` /
`mut-44.1-drop-parallel-executor` mutants) stays in [Phase 49](phase_49_provider_deploy_checkpoint.md); the bootstrap
scheduler / add-on cutover / Lease-handoff apparatus (the `public-pull` and hostless-`LinuxHost`-foreclosure
tags) stays in [Phase 50](phase_50_provider_child_bringup.md); the per-PV-EBS credential apparatus
(`test/goldens/ebs_credential_matrix.txt`, `test/fixtures/phase46/ebs_csi_bake_expected.dhall`, and the
`allow-delete` / `enable-dynamic-provisioner` / `credit-old-before-observed-delete` / `drop-copy-executor`
mutants) stays in [Phase 51](phase_51_provider_ebs_credential.md). This phase inherits only the reactive-node
and teardown slice below; the **composed** gate (Sprint 52.2) additionally re-runs the named sibling mutants and
cites them to their owning sub-phase.

```mermaid
flowchart LR
  %% register: orientation
  s0["Sprint 52.1: Dynamic node provisioning by signal"]
  s1["Sprint 52.2: Phase gate — spin a provider cluster, provision a node by…"]
  gate["the phase 52 gate"]
  s0 -->|"produces what the next consumes"| s1
  s1 -->|"the last seam the gate closes over"| gate
```
*Orientation. The seams phase 52 builds, in order; [Gate integrity](#gate-integrity) owns the apparatus. Not run.*
...
- **Representative set (§M.7)** — `test/dhall/phase_48_provider_provision.dhall`: one `Managed Eks` control
...
- **Committed one-field negatives (§M.8), each paired with a positive differing only in the foreclosed dimension**: `test/dhall/phase_48_provider_over_quota.dhall` (elastic maximum count exceeds the pinned provider
  instance/vCPU quota while every node-class field remains valid) and
  `test/dhall/phase_48_provider_missing_capability.dhall` (the elastic class's explicit `accelerator = None`
...
([Phase 49](phase_49_provider_deploy_checkpoint.md)), and `credit-old-before-observed-delete` / `allow-delete`
([Phase 51](phase_51_provider_ebs_credential.md)) mutants against the full-stack fixture, cited to their owning
...
- [`cluster_lifecycle_doctrine.md §8`](../documents/engineering/cluster_lifecycle_doctrine.md#8-dynamic-node-provisioning)
  ...
  provisioning a node is one more pass of the [Phase 31](phase_31_object_reconciler.md)
...
- [`resource_capacity_doctrine.md §6`](../documents/engineering/resource_capacity_doctrine.md#6-growable--scalingpolicy-the-quota-bounded-dynamic-provisioning-arm)
  ...
  `emptyDir` as a **[Phase 8](phase_08_capacity_core_folds.md) logical local-ephemeral debit** — see below),
...
- [`pulumi_iac_doctrine.md §4`](../documents/engineering/pulumi_iac_doctrine.md#4-what-pulumi-provisions-the-resource-catalog)
  ...
  EBS ([Phase 51](phase_51_provider_ebs_credential.md)) is a distinct retained class the per-run teardown never
  touches.
...
- [`daemon_topology_doctrine.md §3.1`](../documents/engineering/daemon_topology_doctrine.md#31-exactly-one-pod-is-a-k8setcd-property-not-an-amoebius-election)
  ...
  ([Phase 32](phase_32_capacity_scheduler.md)), not singleton election.
- [`storage_lifecycle_doctrine.md §5.1`](../documents/engineering/storage_lifecycle_doctrine.md#51-storage-is-independent-of-the-node-lifecycle)
  ...
  test-flagged EBS by the elevated harness is [Phase 56](phase_56_test_topology_dsl.md) work, referenced and
  never invoked here.
...
## Sprint 52.1: Dynamic node provisioning by signal ⏸️
...
EC2/managed nodes), and `amoebius-runtime/test/provider/ContractSpec.hs`
...
- An `Amoebius.Cluster.NodeProvisioner` that reads the declared elastic-node rule — a typed `ScalingPolicy`
  ...
  driven by **load** and **workflow completion** (spot-instance-cost-driven scaling is a declared future signal
  class, deferred and not exercised by this phase's gate corpus or seeded mutants) — computes the desired node
  set, and drives the live set toward it through the [Phase 31](phase_31_object_reconciler.md) reconciler; no
  ...
  [Phase 49](phase_49_provider_deploy_checkpoint.md) `observeProviderAccount` boundary re-read). SKU raw
...
- An `Amoebius.Pulumi.NodeGroup` enaction that adds an EC2/managed node (Pulumi, under the singleton, encrypted
  ...
  node; it is distinct from [Phase 51](phase_51_provider_ebs_credential.md)'s retained durable class. Every
  add/drain runs through the [Phase 12](phase_12_provision_seal.md) observe-then-plan boundary and seals nothing
...
- A staged managed-node join/leave protocol. Provider launch data makes the kubelet register with the exact
  ...
  the active [Phase 32](phase_32_capacity_scheduler.md) scheduler generation. Only an independent readback of
...
4. Run the committed one-field negative `test/dhall/phase_48_provider_over_quota.dhall`, whose elastic maximum
...
5. Give two child-cluster demands the same `CloudAccountId` and per-cluster quotas that each fit alone but exceed
   ...
   `DescribeInstances`/`DescribeVolumes` usage is one unit short; the [Phase 49](phase_49_provider_deploy_checkpoint.md)
...
6. Fit CPU, memory, storage bytes, and regional EBS count, then make the aggregate simultaneous pod count one
   ...
   MUST go red.
7. Run the committed `test/dhall/phase_48_provider_missing_capability.dhall`: replace the elastic class's
...
12. Reject before any cloud mutation every worst-case elastic shape the envelope cannot carry, so the refusal
    ...
    cache-owner `emptyDir`'s [Phase 8](phase_08_capacity_core_folds.md) local-ephemeral debit), layout-routed
...
## Sprint 52.2: Phase gate — spin a provider cluster, provision a node by signal, tear down leak-free ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `test/dhall/phase_48_provider_provision.dhall` (the gate topology),
...
only through the observe-then-plan chain, composes the Phase 49–51 siblings, provisions an extra node by
...
[Phase 56](phase_56_test_topology_dsl.md)), `DEVELOPMENT_PLAN/README.md`.
...
- The gate `test/dhall/phase_48_provider_provision.dhall`: spin up the EKS provider cluster
  ([Phase 49](phase_49_provider_deploy_checkpoint.md)), first derive the complete app/platform demand and
  provision it against the named base/elastic node classes, the bounded cache demand charged within local
  ephemeral supply (the cache-owner `emptyDir` charged **once** inside the [Phase 8](phase_08_capacity_core_folds.md) logical local-ephemeral fold — **not** a second supply pool and **not** the [Phase 53](phase_53_determinism_jitcache.md) typed `CacheBudget`/jit-build construct, which is out of this phase's scope), parent executor Jobs and
  ...
  in-cluster control plane ([Phase 50](phase_50_provider_child_bringup.md)), provision an extra node by a declared
  rule and observe it join only after node-scoped scheduler authority (Sprint 52.1), bind the Pulumi-created EBS
  through [Phase 51](phase_51_provider_ebs_credential.md)'s static CSI PV and write a run-unique marker through
...
- An `Amoebius.Pulumi.Teardown` step: one `reconcileAbsent` over the **ephemeral** registry subset (VPC, EKS
  ...
  Unreachable → refuse* — leaving the durable EBS class ([Phase 51](phase_51_provider_ebs_credential.md))
...
- The **broadened run-owned OS-boundary sweep** harness (the P30 confirmed C3 fix): a distinct read-only audit
  credential issuing direct AWS `Describe*` queries that enumerate ephemeral-class resources by **resource class and cluster/VPC ownership** — keyed on the run tag `amoebius:test-run=<run-id>` **and** the run's VPC id **and**
...
- A per-run proven/tested/assumed ledger recording: provider bring-up + signal-driven node join as **tested on the EKS provider target from a linux-cpu parent**; per-run teardown leak-freedom **under the broadened run-owned sweep** as **tested**; durable EBS retention as **correct-by-class**; and the elevated-harness
  durable-EBS *reclamation* as **explicitly deferred to [Phase 56](phase_56_test_topology_dsl.md), not asserted here**.

### Validation

1. Run the gate end-to-end over the committed representative set
   `test/dhall/phase_48_provider_provision.dhall` (named in the Gate above): assert the provider cluster
   ...
   parent bootstrap Lease holder releases before the authenticated child singleton acquires ([Phase 50](phase_50_provider_child_bringup.md)).
   - Only then does the in-cluster control plane converge.
   - The extra node is provisioned **by signal**, registers already `ManagedCapacity`-tainted, passes full
     supply/layout/device observation and scheduler-generation CAS/readback, and only then joins the
     placement domain.
   - Cross-check each joined node's observed allocatable CPU, memory, logical pod ephemeral capacity,
     kubelet/CNI pod slots, driver-indexed `CSINode` attach slots, nodefs/imagefs/containerfs identities and
     capacities, every resident OCI content object and committed/active snapshot, enforced pull policy and
     ...
     `test/goldens/provider_two_instance_identity_map.txt`).
   - The bounded cache-owner `emptyDir` is charged **once** inside that [Phase 8](phase_08_capacity_core_folds.md) local-ephemeral demand, not as a second supply pool.
   - Write a fresh run-unique marker through `<ns>/sts0/pv_0` after asserting its static PV uses `driver:
     ...
     the OS-boundary cloud-API audit trail records **zero mutating** create/modify/delete calls (§M.5/§M.6),
   ...
   - The sweep MUST return zero ephemeral-class resources against
     `test/goldens/provider_ephemeral_sweep_expected.txt`; retained durable EBS is the sole permitted
   ...
   - The committed seeded mutant `mut-47.2-skip-sweep` (teardown that leaves one **tagged** provider-spawned
     ...
     checkpoint alone, is thereby foreclosed (§M.2).
2. From the provider-stack-absent state left by Validation 1, with the durable EBS retained, run a second full
   spin-up → run → teardown cycle and repeat [Phase 51's retained-volume identity/readback oracle](phase_51_provider_ebs_credential.md)
...
3. Assert the run emits a proven/tested/assumed ledger per
   ...
   elevated-harness durable-EBS reclamation stays explicitly deferred to [Phase 56](phase_56_test_topology_dsl.md)
   and is recorded as deferred, not asserted here.
4. Run the committed over-quota (`test/dhall/phase_48_provider_over_quota.dhall`) and missing-capability
   (`test/dhall/phase_48_provider_missing_capability.dhall`) negatives and assert both fail before the first
...
5. Run the one-short executor/plugin/workspace/checkpoint-budget, pod-slot, CSI-attach, root-EBS, durable-EBS, and
   migration-overlap fixtures from Sprints 48.1 and its provider siblings and assert the exact private provisioned
   witness is the sole input to Job manifests, checkpoint gateway writes, launch templates, `CreateVolume`,
   PVC/PV, and static CSI attachment. The `drop-parallel-executor` ([Phase 49](phase_49_provider_deploy_checkpoint.md)),
   `credit-old-before-observed-delete` ([Phase 51](phase_51_provider_ebs_credential.md)), and this phase's
...
> test-flagged EBS under the elevated credential — is [Phase 56](phase_56_test_topology_dsl.md) and is **not** a
...
- `documents/engineering/resource_capacity_doctrine.md` — record that
  ...
  note that the in-cluster cache-owner `emptyDir` is a Phase-8 logical local-ephemeral debit here, distinct from
  the Phase-53 typed `CacheBudget`.
...
- `documents/engineering/testing_doctrine.md` — record the Phase 52 per-run ledger artifact, the **broadened run-owned OS-boundary leak-free sweep** (run-tag + VPC id + `eks:cluster-name` / `kubernetes.io/cluster/<name>`,
  catching untagged provider-spawned orphans), and the explicit deferral of elevated durable-EBS reclamation to
  [Phase 56](phase_56_test_topology_dsl.md).

**Cross-references to add:**
- `DEVELOPMENT_PLAN/system_components.md` — register `Amoebius.Cluster.NodeProvisioner`,
  `Amoebius.Pulumi.NodeGroup`, and `Amoebius.Pulumi.Teardown` as Phase-52 design-first rows, each mapped to its
  ...
  first delivery in [Phase 49](phase_49_provider_deploy_checkpoint.md) / [Phase 50](phase_50_provider_child_bringup.md)
  / [Phase 51](phase_51_provider_ebs_credential.md).
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 52 → `linux-cpu` (parent) row with the `provider` (EKS)
  deploy target annotated as a target class, not a fifth hardware substrate.
- `DEVELOPMENT_PLAN/README.md` — flip the Phase 52 row's status once the gate passes; link this document.

## Related Documents

- [README.md](README.md) — the live tracker; the Phase 52 one-line gate and status
...
- [overview.md](overview.md) — the target architecture and cross-cutting invariants (no bespoke election; single-instance delegated to k8s/etcd; ephemeral infrastructure, durable backing retained independently; no unbounded storage behind a quota-capped `ScalingPolicy`)
- [substrates.md](substrates.md) — the substrate registry and per-phase map (`linux-cpu` parent → `provider` target)
- [phase_49_provider_deploy_checkpoint.md](phase_49_provider_deploy_checkpoint.md) — the provider-cluster Pulumi
  deploy, Vault-Transit-enveloped MinIO checkpoint, and `observeProviderAccount` this phase reuses to add/drain a
  node group
- [phase_50_provider_child_bringup.md](phase_50_provider_child_bringup.md) — the stateless hostless in-cluster
  singleton + capacity-scheduler roles + parent→child Lease handoff the gate composes before node provisioning
- [phase_38_live_dsl_singleton.md](phase_38_live_dsl_singleton.md) — owns the singleton runtime role reused by
  the provider child
- [phase_51_provider_ebs_credential.md](phase_51_provider_ebs_credential.md) — per-PV durable EBS decoupled from
  the node lifecycle, the static `ebs.csi.aws.com` CSI PV, and the create-vs-delete credential the gate's marker
  rebind rides on
- [phase_08_capacity_core_folds.md](phase_08_capacity_core_folds.md) — the `fits`/`carve`/`place` fold re-run
  against the grown bound (and the logical local-ephemeral debit the cache-owner `emptyDir` charges)
- [phase_12_provision_seal.md](phase_12_provision_seal.md) — the post-bind observe-then-plan cloud batch and
  opaque `ProvisionedSpec` seal each node-group enaction runs through
- [phase_31_object_reconciler.md](phase_31_object_reconciler.md) — the `discover → diff → enact → re-observe`
  reconciler that makes provisioning a node just another reconcile
- [phase_32_capacity_scheduler.md](phase_32_capacity_scheduler.md) — the `amoebius-capacity`
  scheduler-generation CAS / reservation / exclusive Binding at node join
- [phase_56_test_topology_dsl.md](phase_56_test_topology_dsl.md) — the elevated-harness durable-EBS reclamation
