# Phase 52: Core jitML CUDA artifact lift
...
Phase 52 delivers the core jitML CUDA artifact lift; its design is owned by [lift_and_compose_doctrine.md](../documents/engineering/lift_and_compose_doctrine.md), [capability_extension_doctrine.md](../documents/engineering/capability_extension_doctrine.md), [content_addressing_doctrine.md](../documents/engineering/content_addressing_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 52.1: Produce one committed jitML artifact on CUDA ⏸️](#sprint-521-produce-one-committed-jitml-artifact-on-cuda-)
...
Phase 38 continues to own Pulsar-Failover takeover and pointer-CAS safety; Phase 49 continues to own the
determinism kernel and bounded first-miss cache. Phase 52 consumes those contracts without adding another
coordinator, election, store, cache, or determinism implementation.

Every scoped training start carries one server-derived `CommandId` unchanged as the Phase-38 workflow
...
revision. Producer resends retain their Phase-36 producer/sequence identity; consumer redelivery and restart
...
The checkpoint adapter uses Phase 38 content digests and canonical component manifests for its pure contract.
...
The output of this phase is the core committed artifact contract. Phase 53 alone owns its downstream
...
live evidence reader, and one aggregate command; no Phase-53 presentation work is included.
...
## Sprint 52.1: Produce one committed jitML artifact on CUDA ⏸️
...
- The canonical checkpoint adapter and opaque `CommittedJitMLArtifact tenant app`, constructible only from a
  successful Phase-38 pointer-CAS witness.
...
- Record only the tested jitML adapter, CUDA owner binding, and pointer-commit contract in the owning
  lift/capability/content/daemon/capacity doctrines; do not restate inherited Phase-38 or Phase-49 proofs.
...
- Keep the Phase-53 dependency on this committed-artifact contract explicit without moving presentation work
  into Phase 51.
...
- [Phase 49 — Determinism and jit cache](phase_49_determinism_jitcache.md)
- [Phase 50 — infernix lift](phase_50_infernix_lift.md)
- [Phase 53 — jitML UI lift](phase_53_jitml_ui_lift.md)
