# Phase 15: Logical→physical storage geometry folds
...
Phase 15 delivers the logical→physical storage geometry folds; its design is owned by [resource_capacity_doctrine.md](../documents/engineering/resource_capacity_doctrine.md), [testing_doctrine.md](../documents/engineering/testing_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 15.1: The `StorageBudget`/`Growable` arithmetic + logical→physical geometry fold ✅](#sprint-151-the-storagebudgetgrowable-arithmetic--logicalphysical-geometry-fold-)
- [Sprint 15.2: The policy-only storage-scaling fold — `ProvisionedStorageScalingEnvelope` / `planStorageScaling` ✅](#sprint-152-the-policy-only-storage-scaling-fold--provisionedstoragescalingenvelope--planstoragescaling-)
- [Sprint 15.3: QuickCheck properties — storage `accepts ⟺ in-envelope`, Pulsar two-ceiling, uniform-claim ✅](#sprint-153-quickcheck-properties--storage-accepts--in-envelope-pulsar-two-ceiling-uniform-claim-)
- [Sprint 15.4: The storage-geometry fold-negative corpus + the gate ✅](#sprint-154-the-storage-geometry-fold-negative-corpus--the-gate-)
...
bound to source snapshot `sha256:6eb21174546f6510…`; Phase 15 owns no remaining migration deferral.
...
`test/oracle/storage_geometry_surfaces.tsv` and recorded against Phase 15 in
...
[Phase 14 gate](phase_14_capacity_core_folds.md). Live backing observation/mutation, execution/runtime storage,
...
- The **closed `StorageBudget` / `Growable` unions' arithmetic**: the single-owner ceiling-per-arm
  ...
  bare-unbounded arm). The union *shapes* are type-foreclosed upstream (Phase 11/13); the *arithmetic* over them
...
compatibility relation, and `mkRke2` distinctness ([Phase 14](phase_14_capacity_core_folds.md)); the
...
integrates these storage folds ([Phase 16](phase_16_execution_accelerator_folds.md)); the logical etcd
API-object/churn diff whose derived MVCC total this phase's control-plane physical formula *consumes* as a
declared input (owned by [Phase 16](phase_16_execution_accelerator_folds.md)); the capability → provider → shape
binder and the whole-deployment provision seal that re-exercises these folds
([Phase 17](phase_17_capability_bind.md)/[Phase 18](phase_18_provision_seal.md)); and any live snapshot
validation or mutation of a scaling transition — [Phase 37](phase_37_object_reconciler.md) owns the generic
snapshot-bound action/token/CAS plumbing, [Phase 39](phase_39_retained_storage.md) enacts the retained-carve
arms, and [Phase 58](phase_58_provider_dynamic_nodes.md) enacts the `CreateProviderCapacity` arm.

**Substrate:** none — no host, no cluster, no backing; the gate is an in-process `cabal test` storage-geometry
fold + QuickCheck battery, analogous to the Phase 12 decode battery and the Phase 13 property suite.
...
clauses require for Phase 15; it strengthens, never weakens, the Gate and sprint Validations above. It carries
**this seam's slice** of the storage-geometry corpus committed for the old capacity/topology phase; the base
capacity/topology negatives are gated by [Phase 14](phase_14_capacity_core_folds.md) and the execution/
accelerator/VM-partition negatives by [Phase 16](phase_16_execution_accelerator_folds.md). This phase does **not**
...
no `bind` or `provision` call enters this gate, because [Phase 18](phase_18_provision_seal.md) re-exercises
...
defined in Sprint 15.3) accepts a returned physical demand *iff* it is in-envelope.

```mermaid
flowchart LR
  %% register: orientation
  s0["Sprint 15.1: The StorageBudget/Growable arithmetic + logical→physical…"]
  s1["Sprint 15.2: The policy-only storage-scaling fold —…"]
  s2["Sprint 15.3: QuickCheck properties — storage accepts ⟺ in-envelope…"]
  s3["Sprint 15.4: The storage-geometry fold-negative corpus + the gate"]
  gate["the phase 15 gate"]
...
```
*Implemented Phase 15 seams; [Gate integrity](#gate-integrity) owns the apparatus.*
...
[Phase 14](phase_14_capacity_core_folds.md) and the execution/accelerator/VM-partition and
...
[Phase 16](phase_16_execution_accelerator_folds.md).
...
the emitted `.build/dsl/phase8/validation-locus-ledger.tsv` supplies evidence for all **5** Phase-15-owned
...
- **Provider-root storage** — under-size a fixed `InstanceStore` root; omit the root policy/presentation/
  ...
  owned by [Phase 16](phase_16_execution_accelerator_folds.md); this mutant guards the shared geometry fold, not
  a Phase-15 gate fixture.)
- **Control-plane etcd physical transition** — omit WAL preallocation/overshoot, the snapshot-save temporary,
  ...
  [Phase 16](phase_16_execution_accelerator_folds.md); this mutant guards the shared geometry fold, not a
  Phase-15 gate fixture.)
...
Defined in Sprint 15.3 Deliverables; it never calls `bookKeeperPhysicalDemand`, `minioPhysicalDemand`,
...
- [`resource_capacity_doctrine.md §2`](../documents/engineering/resource_capacity_doctrine.md#2-the-load-bearing-honesty-limit-a-capacity-sum-is-a-decode-foreclosed-check-never-type-foreclosed)
  ...
  union *shapes* are type-foreclosed (Phase 11/13); the *arithmetic* over them is the pure fold this phase adds.
...
- [`testing_doctrine.md`](../documents/engineering/testing_doctrine.md#2-the-registers-of-amoebius-testing) [§2](../documents/engineering/testing_doctrine.md#2-the-registers-of-amoebius-testing) (**Register 1** — pure/golden, in-process, no cluster) and [§4](../documents/engineering/testing_doctrine.md#4-no-skips-fail-fast-and-the-per-run-ledger-artifact) (the per-run proven/tested/assumed ledger): the register this gate reaches and
  ...
  live band — [Phase 37](phase_37_object_reconciler.md)/[Phase 39](phase_39_retained_storage.md)/
  [Phase 41](phase_41_platform_backbone.md)/[Phase 58](phase_58_provider_dynamic_nodes.md)).
...
## Sprint 15.1: The `StorageBudget`/`Growable` arithmetic + logical→physical geometry fold ✅
...
**Blocked by**: [Phase 14 gate](phase_14_capacity_core_folds.md) (the base capacity types and
subtraction folds); Phase 12 gate (the GADT-indexed IR + total decoder).
...
check.
**Docs to update**: `documents/engineering/resource_capacity_doctrine.md` (Phase-15 status
...
- The closed `StorageBudget`/`Growable` unions (no unbounded / no bare-unbounded arm) and the aggregate backing
  ...
  in-file honesty note records that the union *shapes* are type-foreclosed (Phase 11/13) while every capacity/
...
- The cache storage geometry: `provisionCacheDemand` distinguishes an in-cluster cache-owner (the
  ...
  logical pod ephemeral (owned by [Phase 16](phase_16_execution_accelerator_folds.md)) and is not added again;
...
- The etcd/control-plane physical storage-transition peak: `provisionControlPlaneStorage` consumes the
  ...
  [Phase 16](phase_16_execution_accelerator_folds.md)) and derives backend + WAL segment/overshoot/
...
Shared declarations live beside their owning folds rather than enlarging the Phase-14 base `Types.hs` module.
[Phase 14](phase_14_capacity_core_folds.md)'s base subset defers every storage member of `Types.hs` to this
...
## Sprint 15.2: The policy-only storage-scaling fold — `ProvisionedStorageScalingEnvelope` / `planStorageScaling` ✅
...
**Prerequisite satisfied**: Sprint 15.1 (the `StorageBudget` arithmetic + backing/quota types the envelope
...
- An in-file honesty note: Phase 15 has no mutation capability; the generic live action is validated by
  [Phase 37 (the object reconciler)](phase_37_object_reconciler.md), the retained
  `AllocateWithinRetainedCarve`/`ShrinkByVerifiedMigration` arms are enacted by
  [Phase 39 (retained storage)](phase_39_retained_storage.md), and `CreateProviderCapacity` by
  [Phase 58 (provider dynamic nodes)](phase_58_provider_dynamic_nodes.md).
...
## Sprint 15.3: QuickCheck properties — storage `accepts ⟺ in-envelope`, Pulsar two-ceiling, uniform-claim ✅
...
and the storage `Σ ≤ backing` equivalence), reusing the Phase-13 property harness; the independently selected
mutants live in `test/spec/dsl/StorageGeometryMutants.hs` and `test/mutant/storage_geometry/mutants.tsv`.
**Blocked by**: Sprint 15.1, Sprint 15.2.
...
## Sprint 15.4: The storage-geometry fold-negative corpus + the gate ✅
...
**Blocked by**: Sprint 15.1, Sprint 15.2, Sprint 15.3; [Phase 11](phase_11_dhall_typecheck_schema.md) gate.
...
`documents/engineering/testing_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-15 status when the
gate passes), `DEVELOPMENT_PLAN/substrates.md` (the Phase-15 `none` gate row).
...
- The fold-negative fixtures — `illegal_store_over_backing` ([§3.19](../documents/illegal_state/illegal_state_storage.md#319-an-application-consuming-more-storage-than-its-backing-minio-and-pulsar)),
  ...
  - `illegal_incluster_cache_bound_mismatch` ([§3.11](../documents/illegal_state/illegal_state_security.md#311-an-unsafe-workload-no-resource-limits-no-hardened-securitycontext)/[§3.17](../documents/illegal_state/illegal_state_capacity.md#317-an-over-committed-deploy-or-workload-host--vm--cluster-capacity-exceeded)),
    ...
    with the type-foreclosed neighbours noted as already foreclosed upstream (Phase 11/13) and the base
    ...
    [Phase 14](phase_14_capacity_core_folds.md)/[Phase 16](phase_16_execution_accelerator_folds.md).
  - This fold additionally *implements* the provider-root and control-plane-storage byte-fit — the
    ...
    owned by [Phase 16](phase_16_execution_accelerator_folds.md) per the §M.7 partition — fold mechanics
    here, gate oracle in Phase 16; the cache negatives `illegal_cache_over_local_pool` /
...
2. The gate applies the Phase-15 storage-geometry folds directly to each hand-authored logical-demand/backing
   fixture, testing the lower fold boundary on purpose: binding and whole-deployment provisioning belong to
   [Phase 17](phase_17_capability_bind.md) and [Phase 18](phase_18_provision_seal.md), so each positive row
...
- `DEVELOPMENT_PLAN/system_components.md` — register
  ...
  storage-geometry property + gate suites as Phase-15 design-first rows.
- `DEVELOPMENT_PLAN/phase_14_capacity_core_folds.md` and `DEVELOPMENT_PLAN/phase_16_execution_accelerator_folds.md`
...
- [phase_14](phase_14_capacity_core_folds.md) — the base `fits`/`carve`/`place` fold and the shared capacity
  types this geometry fold layers on
- [phase_16](phase_16_execution_accelerator_folds.md) — the execution-epoch/accelerator folds and the composed
  full-resource-vector place-witness that integrates this storage geometry
- [phase_18](phase_18_provision_seal.md) — the whole-deployment provision seal that re-exercises these storage
  folds post-bind
- Phase 15 storage-geometry ledger — the human-readable Register-1 proof/test/assumption boundary
