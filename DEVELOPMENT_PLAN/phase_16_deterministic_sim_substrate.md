...
- [Sprints](#sprints)
- [Sprint 16.1: The `io-classes` `Env` effect interface + its two interpreters + `sim-spec` skeleton 📋](#sprint-161-the-io-classes-env-effect-interface--its-two-interpreters--sim-spec-skeleton-)
- [Sprint 16.2: The modeled fault-injectable substrates + the per-fake fault-contract corpus 📋](#sprint-162-the-modeled-fault-injectable-substrates--the-per-fake-fault-contract-corpus-)
- [Sprint 16.3: The deterministic-replay battery — same-seed determinism + schedule-sensitivity + fault-mutant — the gate 📋](#sprint-163-the-deterministic-replay-battery--same-seed-determinism--schedule-sensitivity--fault-mutant--the-gate-)
...
⏸️ Blocked pending Phase-15 revalidation. Reopened 2026-08-19 by the generative re-baseline: the artifact, budget, lift, workflow and evidence calculi change what this phase's gate must cover, so any earlier seal is history and no longer presents completion evidence.
...
bound to source snapshot `sha256:4a7809fb4d96d811…`; Phase 16 owns no remaining migration deferral.
...
reddens at `NoActOnStaleRead`. Evidence and the ledger move into `.build/runs/phase_29/<run-id>/`.
...
run. What is *not* here: the boundary fake-tool harness (Phase 34, which this phase's Sprint 16.1 generalizes),
...
**Phase scope:** one cohesive claim — *the same seed yields the same trace, so a fault is reproducible rather than anecdotal*. The real reconciler runs unmodified; only its environment is simulated.
...
**Depends on:** [Phase 15](phase_15_compile_fail_harness.md) — the compile-fail fixture harness, which this phase consumes rather than rebuilds.
...
Phase-16 ledger records the exact tested/model-proven boundary
...
- **Independent oracle.** A hand-authored expected-outcome table, one row per committed schedule fixture giving the invariant verdict the reconciler must produce under that schedule, authored without reference to the `Env m` reconciler and sharing none of its code.
- **Committed mutants.** A seeded fault-mutant must flip a row of that table from `upheld` to `violated`, which is what shows the table discriminates rather than merely agrees.
- **Specific-reason negatives.** Each schedule fixture asserts the invariant it violates, paired with a schedule differing only in the interleaving that makes the violation reachable.
- **Fresh challenge.** Not applicable — the substrate is simulated, so the separately authored verdict table stands in for a live challenge.
...
## Sprint 16.1: The `io-classes` `Env` effect interface + its two interpreters + `sim-spec` skeleton 📋
**Status**: Planned
...
**Docs to update**: `documents/engineering/deterministic_simulation_doctrine.md` (Phase-16 status
...
test (`m = IOSim s`) from one source, generalizing the Phase-34 single IO seam.
...
- `src/Amoebius/Sim/Env.hs`: the typed `Env m` effect interface (publish/consume, put/get-blob, apply-object,
  ...
  clock and the seed seams the determinism kernel ([phase_80](phase_80_determinism_jitcache.md)) also uses — one
...
## Sprint 16.2: The modeled fault-injectable substrates + the per-fake fault-contract corpus 📋
**Status**: Planned
**Implementation**:
...
## Sprint 16.3: The deterministic-replay battery — same-seed determinism + schedule-sensitivity + fault-mutant — the gate 📋
**Status**: Planned
...
**Docs to update**: `DEVELOPMENT_PLAN/README.md` (flip the Phase-16 status
...
- `test/spec/sim/SimSpec.hs` asserting: same-seed → byte-identical trace; a distinct-seed / distinct-schedule run
  yields a **different** trace ([§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub)-6); the named per-fake fault contracts fire ([§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub)-7, from Sprint 16.2); and the
...
- `documents/engineering/deterministic_simulation_doctrine.md` — backlink §2/§3/§4 to the in-process Phase-16
...
- `documents/engineering/conformance_harness_doctrine.md` — backlink §2's Register-2.5 deterministic-simulation
  entry to the in-process Phase-16 substrate that serves it.
...
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Sim/Env.hs`,
  `src/Amoebius/Sim/Interp/{Real,Sim}.hs`, `src/Amoebius/Sim/Fakes/`, and the `sim-spec` test-suite as Phase-16
  design-first rows.
...
- [phase_34](phase_34_chain_kernel_boundary.md) — the `chain`/`Step` kernel + `--dry-run` plan the toy reconcile
  loop consumes
- [phase_34](phase_34_chain_kernel_boundary.md) — the boundary fake-tool harness whose single IO seam Sprint
  12.1 generalizes into the typed `Env m` effect interface
- [phase_80](phase_80_determinism_jitcache.md) — the determinism kernel that shares the seed / `MonadTime` seams
