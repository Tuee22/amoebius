...
run. What is *not* here: the boundary fake-tool harness (Phase 15, which this phase's Sprint 16.1 generalizes),
...
test (`m = IOSim s`) from one source, generalizing the Phase-15 single IO seam.
...
- `src/Amoebius/Sim/Env.hs`: the typed `Env m` effect interface (publish/consume, put/get-blob, apply-object,
  ...
  clock and the seed seams the determinism kernel ([phase_53](phase_53_determinism_jitcache.md)) also uses — one
...
- [phase_15](phase_15_chain_kernel_boundary.md) — the `chain`/`Step` kernel + `--dry-run` plan the toy reconcile
  loop consumes
- [phase_15](phase_15_chain_kernel_boundary.md) — the boundary fake-tool harness whose single IO seam Sprint
  12.1 generalizes into the typed `Env m` effect interface
- [phase_53](phase_53_determinism_jitcache.md) — the determinism kernel that shares the seed / `MonadTime` seams
