# Phase 47: Live subject/tenant isolation
...
Phase 47 delivers the live subject/tenant isolation; its design is owned by [tenancy_doctrine.md](../documents/engineering/tenancy_doctrine.md), [low_code_ui_runtime_doctrine.md](../documents/engineering/low_code_ui_runtime_doctrine.md), [testing_doctrine.md](../documents/engineering/testing_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 47.1: Live scoped request and provider-isolation gate ⏸️](#sprint-471-live-scoped-request-and-provider-isolation-gate-)
...
provider observers, cleanup equality, and two red source mutants. Browser tenant switching remains Phase 64;
...
coordinates or credentials to the caller. The phase tests enforcement, not a full browser UX; Phase 64 owns
...
**Depends on:** Phases 43, 45, and 46 — respectively Keycloak-owned ingress, delivered and independently read
six-provider administrative policy, and the native Pulsar client. Phase 45 supplies readiness only; this phase
...
`src/Amoebius/Test/{Harness,Sweep}.hs` that [Phase 62](phase_62_test_topology_dsl.md) later consolidates these
...
- One minimal same-binary server worker behind the Phase-43 Keycloak/Envoy edge, with a sealed action registry
...
## Sprint 47.1: Live scoped request and provider-isolation gate ⏸️
...
Connect authenticated identity to the Phase-26/27 scoped authorization values and test that provider effects
...
None in this phase. Browser tenant switching is deliberately deferred to Phase 63. Cross-cluster isolation and
...
- `DEVELOPMENT_PLAN/phase_64_ui_multi_tenant_live.md` — consume this provider-isolation result unchanged.

## Related Documents

- [Phase 45](phase_45_app_tenancy.md) — tenant policy derivation and provider materialization.
- [Phase 46](phase_46_pulsar_client.md) — native authenticated message path used by this gate.
