...
- [Sprints](#sprints)
- [Sprint 26.1: Authenticated scoped UI-server dispatch ✅](#sprint-261-authenticated-scoped-ui-server-dispatch-)
...
⏸️ Blocked pending Phase-25 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
remain beneath `.build/**`; the outside-host inventory is unchanged; Phase 26 owns no migration deferral. The
...
`.build/runs/phase_26/<run-id>/`, and 77 surfaces join two-way to 94 run-time enumerated items.
...
by the tree regaining a second executable — which is Phase 38's closure, not a property this gate can
...
Phase 26 in the legacy register.
...
Phase-25 fixture. The private `UiServerPlan`, dispatch table, policy graph, and server codecs have no asset
route.
...
stood up in Phase 35. Redis identifiers never enter handler input or establish authorization.
...
**Depends on:** Phase 23 — canonical `UiServerPlan`, public contracts, route dispatch, and authority digests.
...
HA layers UNVERIFIED, and Phase 27's server branch waits on it. The apparatus is
[Gate integrity](#gate-integrity).
...
- **Pinned oracles:** `test/fixture/ui_server/requests.tsv`, `access_matrix.tsv`, `expected_http.tsv`,
  ...
  state, origin, or authority epoch. Asset/plan responses must exactly match the Phase-25 browser-enforced
  header policy.
...
## Sprint 26.1: Authenticated scoped UI-server dispatch ✅
...
- Phase 27 — compose this boundary with the generic browser without replacing either oracle.

## Related Documents

- [Phase 23](phase_23_ui_plan_compiler.md) — the required immutable `UiServerPlan` and dispatch contracts.
