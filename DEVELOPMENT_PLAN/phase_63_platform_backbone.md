# Phase 63: Platform backbone (MetalLB + MinIO + Pulsar HA)
...
- [Sprints](#sprints)
- [Sprint 63.1: MetalLB LoadBalancer + MinIO object substrate + registry S3-driver rehoming ⏸️](#sprint-631-metallb-loadbalancer--minio-object-substrate--registry-s3-driver-rehoming-)
- [Sprint 63.2: Pulsar native-protocol backbone + size-triggered S3 offload drill ⏸️](#sprint-632-pulsar-native-protocol-backbone--size-triggered-s3-offload-drill-)
- [Sprint 63.3: The backbone HA bring-up gate ⏸️](#sprint-633-the-backbone-ha-bring-up-gate-)
...
Blocked by redesigned Phase 62, its independent validation, and human promotion; every earlier
...
cluster of Phase 62 into an amoebius cluster carrying the
...
Phase-59 `renderAll` path (no Helm and no third-party charts); emitted manifests are generated lazily from
...
public-registry pull, and placed on the Phase-61 `no-provisioner` retained PVs where it holds durable state.

Two deferred gaps from earlier phases must close here. First, the **registry→MinIO S3-driver rehoming**: the
future Phase-57 Distribution `registry:2` state uses **interim node-local (filesystem-driver) blob storage**
with the MinIO S3 driver named as this phase's target; Phase 63 must move the registry's blob store onto MinIO
...
replace the registry's capacity obligation. Phase 63 must preserve the artifact/upload
operands from Phase 57's canonical `RegistryStorageDemand` — every digest-keyed compressed
...
erasure/healing geometry. Every pre-existing Phase-57 artifact must be copied and independently digest-verified
...
clusters, pgAdmin, Prometheus/Grafana, and the **full derived readiness-DAG bring-up of the whole standard stack** are [Phase 64](phase_64_platform_services_2.md). The **Keycloak-owned ingress edge** — Envoy/Gateway
API terminating TLS and Keycloak owning all wild ingress so no workload publishes its own path — is
[Phase 65](phase_65_keycloak_ingress.md); this phase must bring the backbone up behind no public edge. The
Deployment-`replicas=1` control-plane daemon that will eventually *own* this reconcile loop is
[Phase 66](phase_66_live_dsl_deploy.md); the future gate must drive the reconciler from the operator/host path
...
**Depends on:** [Phase 62](phase_62_vault_pki.md)
...
- [`platform_services_doctrine.md §1 — the invariant: every cluster is the same cluster`](../documents/engineering/platform_services_doctrine.md#1-the-invariant-every-cluster-is-the-same-cluster)
  ...
  Phase 63 materializes the backbone slice of the fixed standard service set on linux-cpu, each service the
...
- [`platform_services_doctrine.md §4 — MinIO, the object substrate`](../documents/engineering/platform_services_doctrine.md#4-minio--the-object-substrate),
  ...
  (the MetalLB LoadBalancer half — the Keycloak edge is Phase 65): the concrete providers Phase 63 stands up
...
- [`image_build_doctrine.md §2 — the single distribution rule: bake the binaries, build the amoebius image, pull only in-cluster`](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster)
  ...
  every backbone binary is baked into the Phase-57 native-architecture base image and resolved only in-cluster; the
  registry stores its blobs in MinIO via the S3 driver, so MinIO must be serving before the registry — the
  thin ordering edge [`image_build_doctrine.md` §9 — Bring-up ordering — the registry chicken-and-egg dissolves](../documents/engineering/image_build_doctrine.md#9-bring-up-ordering--the-registry-chicken-and-egg-dissolves) names, and the rehoming Phase 63 delivers.
- [`manifest_generation_doctrine.md §5 — the apply/reconcile engine: snapshot-bound typed actions`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  Phase 63 reuses the Phase-59 pure `renderAll :: ProvisionedSpec -> [K8sObject]` and typed-action reconciler
...
- [`platform_services_doctrine.md §10 — every execution unit declares its complete resource envelope`](../documents/engineering/platform_services_doctrine.md#10-every-execution-unit-declares-its-complete-resource-envelope)
  ...
  witness are unchanged from Phase 57 — including compressed objects,
...
- [`storage_lifecycle_doctrine.md §2 — one storage class, and it provisions nothing`](../documents/engineering/storage_lifecycle_doctrine.md#2-one-storage-class-and-it-provisions-nothing)
  ...
  each stateful backbone service (MinIO, Pulsar's ZooKeeper members and bookies) lands its durable bytes on the Phase-61
...
## Sprint 63.1: MetalLB LoadBalancer + MinIO object substrate + registry S3-driver rehoming ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 62](phase_62_vault_pki.md) human approval
...
rehome the Phase-57 Distribution `registry:2` blob store off the interim node-local filesystem driver onto the
MinIO S3 driver — closing the Phase-57 deferred gap.

### Deliverables

- MetalLB rendered as a standard service that publishes a LoadBalancer address on the kind node, available
  before the (Phase 65) Envoy/Gateway edge needs one — the backbone's LoadBalancer root.
- MinIO (HA / distributed) as the S3 object substrate on Phase-61 retained PVs, holding the content store,
...
- The Distribution `registry:2` blob store rehomed onto the MinIO S3 driver — the registry holds no PV of its
  ...
  witness exactly match Phase 57's interim provision and whose
  ...
  observer sees it.
- A `RegistryBackendMigrationDemand` from the still-live Phase-57 private provision to the MinIO replacement.
...
1. Apply through the Phase-59 reconciler; assert MetalLB advertises an address and MinIO reaches its Ready
...
3. Assert the registry is serving from the MinIO S3 driver: its storage config is structurally equal to the
   ...
   a pre-existing Phase-57 artifact was copied/verified and that artifact still pulls by the same digest, and a
   ...
   Compare the Phase-57 and Phase-63 private registry witnesses: `objectSet`, structured `objectStorePeak`,
...
5. Assert every app/init/sidecar container and volume in the `renderAll`-emitted MetalLB/MinIO/registry objects
   ...
   logging) records no public-registry pull; every service image except Registry resolves to the Phase-57
...
## Sprint 63.2: Pulsar native-protocol backbone + size-triggered S3 offload drill ⏸️
...
2. Produce then consume a message; assert an at-least-once round-trip. Dedup is **exercised**: inject a
   ...
   byte-for-byte (full native-client proof deferred to Phase 68, marked UNVERIFIED here).
...
## Sprint 63.3: The backbone HA bring-up gate ⏸️
...
and [`image_build_doctrine.md §9 — bring-up ordering`](../documents/engineering/image_build_doctrine.md#9-bring-up-ordering--the-registry-chicken-and-egg-dissolves):
assemble the Sprint 63.1–27.2 backbone services, bring them up event-driven behind the reconciler's
...
- A backbone bring-up that applies MetalLB, MinIO, the rehomed Distribution `registry:2` service, and Pulsar through the
  Phase-59 reconciler, each dependent starting on its dependency's observed-ready edge (MinIO serving before
...
- The phase-gate harness: bring the whole backbone up on a fresh single-node linux-cpu `kind` cluster and
  ...
  Postgres/observability (Phase 64), and control-plane-owned reconcile layers UNVERIFIED; the independent
  ...
  and the `M-dag-drop-edge` Haskell mutant, is [Phase 64](phase_64_platform_services_2.md)'s.
- The separately authored Haskell gate expectations reused here: the registry storage stanza, the drill-topic
  ...
  recovery/healing, orphan horizon, and uniform-ordinal rounding. The gate also reconstructs the Phase-26
...
3. Recompute the Haskell storage-geometry boundary table with the independent Haskell checker and compare it with the opaque
   ...
   replacement `objectSet`, `derivedPeak`, and upload/orphan witness exactly match Phase 57 while its backend
...
4. Assert the whole backbone is up and HA-shaped from manifests generated lazily beneath `.build/**` plus
   ...
   **and** every running container's `imageID` digest (`kubectl get pods -A -o jsonpath={..imageID}`) MUST
   equal the digest in the verified Phase-57 attestation and current in-cluster `registry:2` catalog — any
   ...
   layer *tested* not *proven*, with the Keycloak edge, Phase-64 Postgres/observability, and control-plane-owned
...
verified Phase-57 digest directly as authenticated run input and rerun only after the gate contract and
...
- `documents/engineering/image_build_doctrine.md` — the §9 "registry stores its blobs in MinIO via the S3
  driver" edge is delivered; the Phase-57 interim filesystem-driver residue is discharged, recorded as the
  Phase-63 rehoming.
- `documents/engineering/pulsar_client_doctrine.md` — the §6.1 size-triggered offload bound gains its first
  live drill (the platform-side bring-up); the native-client CBOR round-trip proof stays owned by Phase 68.
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-63 status when the gate passes and link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 63's gate substrate (linux-cpu) in the per-phase substrate map.
- `DEVELOPMENT_PLAN/system_components.md` — reconcile the Phase-63 `Amoebius.Platform` Haskell module names
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the Phase-59 renderer + SSA wait-for-ready that applies and sequences the backbone
...
- [phase_57](phase_57_base_image_registry.md) — the base image + Distribution `registry:2` service whose interim filesystem-driver blob store this phase rehomes onto MinIO
- [phase_62](phase_62_vault_pki.md) — the root Vault/PKI whose unseal edge gates Pulsar's secret-dependent startup here
- [phase_64](phase_64_platform_services_2.md) — the Percona/Patroni + pgAdmin + observability services and the full derived readiness-DAG gate that build on this backbone
- [phase_65](phase_65_keycloak_ingress.md) — the Keycloak-owned ingress edge that fronts this backbone next
- [phase_68](phase_68_pulsar_client.md) — the native `amoebius-pulsar` client whose full CBOR round-trip proof this phase defers
