# Phase 29: Python bootstrap coordinator + substrate detect + single kind cluster
...
and Windows uses WSL2 for the same clean boundary. The complete Phase-29 gate passed on the Incus route;
...
- [Sprints](#sprints)
- [Sprint 29.1: Live substrate detection ✅](#sprint-291-live-substrate-detection-)
- [Sprint 29.2: No-`PATH` lazy tool-ensure — closed enum, `AbsExe`, install-and-verify ✅](#sprint-292-no-path-lazy-tool-ensure--closed-enum-absexe-install-and-verify-)
- [Sprint 29.3: The Python `pb` bootstrap coordinator (package-manager root → toolchain → build → `exec`) ✅](#sprint-293-the-python-pb-bootstrap-coordinator-package-manager-root--toolchain--build--exec-)
- [Sprint 29.4: The in-binary `bootstrap` command — idempotent single-node kind bring-up ✅](#sprint-294-the-in-binary-bootstrap-command--idempotent-single-node-kind-bring-up-)
...
opens only after the pre-cluster band (Phases 1–27) is green, and it composes four things: the Python `pb`
...
flowchart LR
  ...
  s0["Sprint 29.1: Live substrate detection"]
  s1["Sprint 29.2: No-PATH lazy tool-ensure — closed enum, AbsExe…"]
  s2["Sprint 29.3: The Python pb bootstrap coordinator (package-manager root → toolchain →…"]
  s3["Sprint 29.4: The in-binary bootstrap command — idempotent single-node…"]
...
```
*Orientation. The seams Phase 29 built in order; [Gate integrity](#gate-integrity) owns the now-passed apparatus.*
...
start state the identical run **converges without recreating the cluster** (divergence-repair, Sprint 29.4).
...
not a sixth Phase-29 `HostTool`: it establishes the machine boundary from which the five-member bootstrap coordinator gate
...
implementation exists: the `classify` decision table (Sprint 29.1), the per-substrate `[InstallStep]` golden
plans and the `mkAbsExe`-reject expected-error set (Sprint 29.2), and the named divergent-start fixtures with
their expected converged observation (Sprint 29.4). A golden regenerated from the implementation is not
admitted.
...
## Sprint 29.1: Live substrate detection ✅
...
## Sprint 29.2: No-`PATH` lazy tool-ensure — closed enum, `AbsExe`, install-and-verify ✅
...
- A **CI structural check that no module outside `src/Amoebius/Host/Ensure.hs` imports any process-spawn API**
  ...
  level).
- A `HostConfig` carrying the detected substrate (Sprint 29.1) plus a `Map HostTool AbsExe`, and a probe-first,
...
## Sprint 29.3: The Python `pb` bootstrap coordinator (package-manager root → toolchain → build → `exec`) ✅
...
Linux frameworks on Apple and Windows and their fresh-guest command plans are unit-tested; Phase 29 exercises
...
## Sprint 29.4: The in-binary `bootstrap` command — idempotent single-node kind bring-up ✅
...
### Deliverables
- A `bootstrap` command chain that detects the substrate (Sprint 29.1), assembles the **`FrameConfig`** —
  ...
  ensures `kind`/`kubectl` by absolute path (Sprint 29.2), and
...
- An empty cluster as the end state: one node, `Ready`, with **no** platform services (those are Phase 30+).
- A typed `ObservedInventory` assembled from an independent apiserver/OS-boundary read: net node allocatable
  ...
  in Phase 68.) The bootstrap handoff exists only after `declaredCapacity <= observedCapacity` on every
...
- An engine-system accounting rule: every kind node has an enforced role-indexed `EngineSystemReserve`.
  ...
  These bootstrap add-ons may still use `default-scheduler` in this Phase-29 empty-cluster state; the later
  ...
  leaving the scheduler bootstrap Pod as the sole `default-scheduler` exception — is Phase 32's scope and is
  specified there.
...
- `documents/engineering/resource_capacity_doctrine.md` — record Phase 29 as the first live producer of the
...
- [README.md](README.md) — flip the Phase 29 tracker-row status once work begins and keep its link current.
- [substrates.md](substrates.md) — record `linux-cpu` as the Phase 29 gate substrate (the first Register-3 row).
...
- [README.md](README.md) — the live tracker; its Phase 29 row is the authoritative one-line gate and status.
