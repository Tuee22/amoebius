# Phase 47: Dynamic node provisioning by signal + leak-free provider gate
...
This sub-phase **layers on** its three provider siblings — [Phase 44](phase_44_provider_deploy_checkpoint.md)
(the provider-cluster Pulumi deploy + Vault-Transit-enveloped MinIO checkpoint + `observeProviderAccount`),
[Phase 45](phase_45_provider_child_bringup.md) (the stateless hostless in-cluster singleton + capacity-scheduler roles + parent→child Lease handoff), and [Phase 46](phase_46_provider_ebs_credential.md) (per-PV EBS decoupled from the node lifecycle + the create-vs-delete credential + static EBS CSI) — and owns dynamic node provisioning
...
set, and drives the live set toward it through the [Phase 26](phase_26_object_reconciler.md)
...
full supply/layout/device observation, and a [Phase 27](phase_27_capacity_scheduler.md) scheduler-generation CAS;
an `Unreachable` node observation **refuses** rather than stranding an EC2 instance.

Second, the **phase gate** (Sprint 47.2): a single `.dhall` that, from a linux-cpu parent, spins up the EKS
provider cluster ([Phase 44](phase_44_provider_deploy_checkpoint.md)), converges its stateless in-cluster
control plane ([Phase 45](phase_45_provider_child_bringup.md)), dynamically provisions an extra node **by evaluating a declared signal rule** (Sprint 47.1), binds the Pulumi-created durable EBS through the static
`ebs.csi.aws.com` CSI PV and writes a run-unique marker ([Phase 46](phase_46_provider_ebs_credential.md)),
...
is [Phase 54](phase_54_test_topology_dsl.md) work, deferred and never depended on here.
...
`mut-44.1-drop-parallel-executor` mutants) stays in [Phase 44](phase_44_provider_deploy_checkpoint.md); the bootstrap
scheduler / add-on cutover / Lease-handoff apparatus (the `public-pull` and hostless-`LinuxHost`-foreclosure
tags) stays in [Phase 45](phase_45_provider_child_bringup.md); the per-PV-EBS credential apparatus
...
mutants) stays in [Phase 46](phase_46_provider_ebs_credential.md). This phase inherits only the reactive-node
and teardown slice below; the **composed** gate (Sprint 47.2) additionally re-runs the named sibling mutants and
cites them to their owning sub-phase.

```mermaid
flowchart LR
  %% register: orientation
  s0["Sprint 47.1: Dynamic node provisioning by signal"]
  s1["Sprint 47.2: Phase gate — spin a provider cluster, provision a node by…"]
  gate["the phase 47 gate"]
...
- **Representative set (§M.7)** — `test/dhall/phase_47_provider_provision.dhall`: one `Managed Eks` control
...
- **Committed one-field negatives (§M.8), each paired with a positive differing only in the foreclosed dimension**: `test/dhall/phase_47_provider_over_quota.dhall` (elastic maximum count exceeds the pinned provider
  instance/vCPU quota while every node-class field remains valid) and
  `test/dhall/phase_47_provider_missing_capability.dhall` (the elastic class's explicit `accelerator = None`
...
([Phase 44](phase_44_provider_deploy_checkpoint.md)), and `credit-old-before-observed-delete` / `allow-delete`
...
- [`cluster_lifecycle_doctrine.md §8`](../documents/engineering/cluster_lifecycle_doctrine.md#8-dynamic-node-provisioning)
  ...
  provisioning a node is one more pass of the [Phase 26](phase_26_object_reconciler.md)
...
- [`pulumi_iac_doctrine.md §4`](../documents/engineering/pulumi_iac_doctrine.md#4-what-pulumi-provisions-the-resource-catalog)
  ...
  EBS ([Phase 46](phase_46_provider_ebs_credential.md)) is a distinct retained class the per-run teardown never
  touches.
...
- [`daemon_topology_doctrine.md §3.1`](../documents/engineering/daemon_topology_doctrine.md#31-exactly-one-pod-is-a-k8setcd-property-not-an-amoebius-election)
  ...
  ([Phase 27](phase_27_capacity_scheduler.md)), not singleton election.
- [`storage_lifecycle_doctrine.md §5.1`](../documents/engineering/storage_lifecycle_doctrine.md#51-storage-is-independent-of-the-node-lifecycle)
  ...
  test-flagged EBS by the elevated harness is [Phase 54](phase_54_test_topology_dsl.md) work, referenced and
  never invoked here.
...
- An `Amoebius.Cluster.NodeProvisioner` that reads the declared elastic-node rule — a typed `ScalingPolicy`
  ...
  driven by **load** and **workflow completion** (spot-instance-cost-driven scaling is a declared future signal
  class, deferred and not exercised by this phase's gate corpus or seeded mutants) — computes the desired node
  set, and drives the live set toward it through the [Phase 26](phase_26_object_reconciler.md) reconciler; no
  ...
  [Phase 44](phase_44_provider_deploy_checkpoint.md) `observeProviderAccount` boundary re-read). SKU raw
...
- An `Amoebius.Pulumi.NodeGroup` enaction that adds an EC2/managed node (Pulumi, under the singleton, encrypted
  ...
  node; it is distinct from [Phase 46](phase_46_provider_ebs_credential.md)'s retained durable class. Every
...
- A staged managed-node join/leave protocol. Provider launch data makes the kubelet register with the exact
  ...
  the active [Phase 27](phase_27_capacity_scheduler.md) scheduler generation. Only an independent readback of
...
4. Run the committed one-field negative `test/dhall/phase_47_provider_over_quota.dhall`, whose elastic maximum
...
5. Give two child-cluster demands the same `CloudAccountId` and per-cluster quotas that each fit alone but exceed
   ...
   `DescribeInstances`/`DescribeVolumes` usage is one unit short; the [Phase 44](phase_44_provider_deploy_checkpoint.md)
...
6. Fit CPU, memory, storage bytes, and regional EBS count, then make the aggregate simultaneous pod count one
   ...
   MUST go red.
7. Run the committed `test/dhall/phase_47_provider_missing_capability.dhall`: replace the elastic class's
...
[Phase 54](phase_54_test_topology_dsl.md)), `DEVELOPMENT_PLAN/README.md`.
...
- The gate `test/dhall/phase_47_provider_provision.dhall`: spin up the EKS provider cluster
  ([Phase 44](phase_44_provider_deploy_checkpoint.md)), first derive the complete app/platform demand and
  provision it against the named base/elastic node classes, the bounded cache demand charged within local
  ephemeral supply (the cache-owner `emptyDir` charged **once** inside the [Phase 7](phase_07_capacity_core_folds.md) logical local-ephemeral fold — **not** a second supply pool and **not** the [Phase 48](phase_48_determinism_jitcache.md) typed `CacheBudget`/jit-build construct, which is out of this phase's scope), parent executor Jobs and
  ...
  in-cluster control plane ([Phase 45](phase_45_provider_child_bringup.md)), provision an extra node by a declared
  rule and observe it join only after node-scoped scheduler authority (Sprint 47.1), bind the Pulumi-created EBS
  through [Phase 46](phase_46_provider_ebs_credential.md)'s static CSI PV and write a run-unique marker through
...
- An `Amoebius.Pulumi.Teardown` step: one `reconcileAbsent` over the **ephemeral** registry subset (VPC, EKS
  ...
  Unreachable → refuse* — leaving the durable EBS class ([Phase 46](phase_46_provider_ebs_credential.md))
...
- A per-run proven/tested/assumed ledger recording: provider bring-up + signal-driven node join as **tested on the EKS provider target from a linux-cpu parent**; per-run teardown leak-freedom **under the broadened run-owned sweep** as **tested**; durable EBS retention as **correct-by-class**; and the elevated-harness
  durable-EBS *reclamation* as **explicitly deferred to [Phase 54](phase_54_test_topology_dsl.md), not asserted here**.

### Validation

1. Run the gate end-to-end over the committed representative set
   `test/dhall/phase_47_provider_provision.dhall` (named in the Gate above): assert the provider cluster
   ...
   parent bootstrap Lease holder releases before the authenticated child singleton acquires ([Phase 45](phase_45_provider_child_bringup.md)).
...
3. Assert the run emits a proven/tested/assumed ledger per
   ...
   elevated-harness durable-EBS reclamation stays explicitly deferred to [Phase 54](phase_54_test_topology_dsl.md)
   and is recorded as deferred, not asserted here.
4. Run the committed over-quota (`test/dhall/phase_47_provider_over_quota.dhall`) and missing-capability
   (`test/dhall/phase_47_provider_missing_capability.dhall`) negatives and assert both fail before the first
...
5. Run the one-short executor/plugin/workspace/checkpoint-budget, pod-slot, CSI-attach, root-EBS, durable-EBS, and
   migration-overlap fixtures from Sprints 47.1 and its provider siblings and assert the exact private provisioned
   witness is the sole input to Job manifests, checkpoint gateway writes, launch templates, `CreateVolume`,
   PVC/PV, and static CSI attachment. The `drop-parallel-executor` ([Phase 44](phase_44_provider_deploy_checkpoint.md)),
   `credit-old-before-observed-delete` ([Phase 46](phase_46_provider_ebs_credential.md)), and this phase's
...
- `documents/engineering/resource_capacity_doctrine.md` — record that
  ...
  the Phase-48 typed `CacheBudget`.
...
- `documents/engineering/testing_doctrine.md` — record the Phase 47 per-run ledger artifact, the **broadened run-owned OS-boundary leak-free sweep** (run-tag + VPC id + `eks:cluster-name` / `kubernetes.io/cluster/<name>`,
  catching untagged provider-spawned orphans), and the explicit deferral of elevated durable-EBS reclamation to
  [Phase 54](phase_54_test_topology_dsl.md).

**Cross-references to add:**
- `DEVELOPMENT_PLAN/system_components.md` — register `Amoebius.Cluster.NodeProvisioner`,
  `Amoebius.Pulumi.NodeGroup`, and `Amoebius.Pulumi.Teardown` as Phase-47 design-first rows, each mapped to its
  ...
  first delivery in [Phase 44](phase_44_provider_deploy_checkpoint.md) / [Phase 45](phase_45_provider_child_bringup.md)
  / [Phase 46](phase_46_provider_ebs_credential.md).
...
- [phase_44_provider_deploy_checkpoint.md](phase_44_provider_deploy_checkpoint.md) — the provider-cluster Pulumi
...
- [phase_26_object_reconciler.md](phase_26_object_reconciler.md) — the `discover → diff → enact → re-observe`
  reconciler that makes provisioning a node just another reconcile
- [phase_27_capacity_scheduler.md](phase_27_capacity_scheduler.md) — the `amoebius-capacity`
  scheduler-generation CAS / reservation / exclusive Binding at node join
- [phase_54_test_topology_dsl.md](phase_54_test_topology_dsl.md) — the elevated-harness durable-EBS reclamation
