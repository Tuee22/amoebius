# Phase 39: No-provisioner retained storage + lossless rebind
...
- [Sprints](#sprints)
- [Sprint 39.1: The one inert `no-provisioner` StorageClass ✅](#sprint-391-the-one-inert-no-provisioner-storageclass-)
- [Sprint 39.2: Deterministic retained-PV generation + the explicit bind ✅](#sprint-392-deterministic-retained-pv-generation--the-explicit-bind-)
- [Sprint 39.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip ✅](#sprint-393-the-lossless-rebind-gate--postgres-row--minio-marker-round-trip-)
...
⏸️ Blocked pending Phase-38 revalidation — **reopened 2026-08-16 by the natural-architecture amendment.**
...
Blocked (superseded) — reopened 2026-08-16 behind the amended Phase-36 handoff and Phase-38 revalidation. Its prior capability record remains historical until the exact predecessor chain is resealed.
...
The gate verified the exact Phase-38 seal and Phase-36 OCI handoff, sealed all three sprints, matched all
...
**Register 3** (live infrastructure) — a single-node `kind` cluster brought up by the Phase 35 bootstrap coordinator, and
it opens only after the Phase 37 gate (the typed renderer + live SSA reconciler) closes, because the
...
8 supplies the policy-only `ProvisionedStorageScalingEnvelope` and pure observe-then-plan fold, and Phase 37
...
resize. `CreateProviderCapacity` has no retained-host mutation capability and remains owned by Phase 54.
...
Phase 41 and HA Patroni-via-Percona Postgres in Phase 42, the Vault-enveloping of secrets is Phase 40, and the
Keycloak-owned edge is Phase 43 — none of which this phase requires. The control plane
...
The live gate uses the Phase-35 single-node `kind` cluster; pure StorageClass/PV rendering remains Register 1–2.
...
- **Real teardown, not soft delete.** After `cluster delete`, an OS-boundary observer on the host (not the
  ...
  fresh Phase-35 `pb bootstrap` producing a **new** apiserver/etcd (verified by a changed apiserver
...
- **Bytes survive on the host, not in a surviving apiserver.** "Bytes intact between delete and recreate" is
  ...
  destroyed. The PV `Released` *status* is observed only in Sprint 39.2's live PVC-delete step against the
...
- **Honest ledger.** The gate emits its proven/tested/assumed ledger; the aggregate durable-backing fold and
  image-backed host hard cap are live-tested here. The Phase-44 control-plane daemon's no-PVC property
  (which has no realized subject at Phase 39) stays marked **UNVERIFIED**, not asserted as passing.
...
flowchart LR
  ...
  s0["Sprint 39.1: The one inert no-provisioner StorageClass"]
  s1["Sprint 39.2: Deterministic retained-PV generation + the explicit bind"]
  s2["Sprint 39.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip"]
  gate["the phase 39 gate"]
  s0 -->|"produces what the next consumes"| s1
  s1 -->|"produces what the next consumes"| s2
  s2 -->|"the last seam the gate closes over"| gate
```
*Orientation. The seams Phase 39 built and validated in order; [Gate integrity](#gate-integrity) owns the apparatus.*
...
witness executes the exact Phase-36 baked binary from the run-audited Phase-36 OCI archive, restored
locally into each fresh node under the Phase-36 digest with `imagePullPolicy: Never`; an OS-boundary observer
confirms zero public-registry pull during the cycle. The harness reads the PostgreSQL executable and major
version from Phase 36's independently authored bake-inventory expectation, then uses that exact baked
...
- `test/oracle/retained_storage/storage_class.yaml` — the exact single-StorageClass golden (Sprint 39.1),
...
- `test/oracle/retained_storage/claimref_table.csv` — the independent reference table mapping
  ...
  authored by hand, never by the renderer's naming or sizing helper (Sprints 29.2, 29.3).
...
- `tools/no_retained_delete_check.sh` — the committed static check that no non-harness `src/` module issues a
  ...
  explicitly outside this check because the backing lives outside the cluster (Sprint 39.3 Validation 2a).
...
- **M-soft-delete** (dropped-effect operator) — a `cluster delete` that deletes only the witness
  ...
  absent" OS-boundary assertion (Gate; 39.3 V1).
...
- **M-reclaim-delete** (guard weakening) — a PV rendered with `reclaimPolicy: Delete` instead of `Retain`. Must
  go red on the `Released`/rebind assertion (39.2 V3/V4).
...
- **M-raw-host-directory** (mechanism substitution) — backs a PV with an ordinary retained-root directory
  ...
  `ENOSPC`, changes shared-parent occupancy, or lacks the raw-size/fs-type/usable witness (39.2 V2).
- **M-skip-durable-aggregate** (dropped validation) — allocates/applies an aggregate retained set larger than
  `DurableBacking`. Must go red on the over-backing negative and zero-write assertion (39.2 V1).
- **M-sum-unequal-ordinals** (wrong aggregation) — debits the unequal per-ordinal provisioned map instead of
  ...
  `uniform_claim_skew_over_backing` and the accepted group's byte-identical PVC-size assertion (39.2 V1/V2).
- **M-uniform-before-allocation** (stage-reordering operator) — groups authorable logical/usable demand before
  ...
  fixtures and the per-slot-witness-before-uniformity assertion (39.2 V1/V2).
- **M-collapse-uniform-backing-debits** (wrong aggregation) — collapses the per-backing debit map into a single
  ...
  member backing is one byte short (39.2 V1).
- **M-cutover-before-verify** (guard-weakening / stage-reordering operator) —
  ...
  `migration_verify_mismatch`, where a mismatched copy is cut over anyway (39.2 V1).
- **M-credit-before-cleanup** (dropped-validation operator) — `src/Amoebius/Storage/RetainedScaling.hs`:
  ...
  observed deletion" step (39.2 V1).
- **M-fake-verify** (mechanism-substitution operator) — `src/Amoebius/Storage/RetainedScaling.hs`: the
  ...
  byte-for-byte nonce readback (39.2 V1).
...
- [`cluster_lifecycle_doctrine.md §7`](../documents/engineering/cluster_lifecycle_doctrine.md#7-ephemeral-spin-updown-with-deterministic-rebind)
  ...
  (delivered in Phase 37) — the SSA reconciler that renders and applies the StorageClass and PV objects.
...
## Sprint 39.1: The one inert `no-provisioner` StorageClass ✅
...
- A single rendered StorageClass — `provisioner: kubernetes.io/no-provisioner`, `reclaimPolicy: Retain`,
  `volumeBindingMode: WaitForFirstConsumer` — applied through the Phase-37 reconciler under the `amoebius`
  field manager.
...
### Remaining Work
Generate the sprint receipt under `.build/runs/phase_33/` and retain it only through repository-local attestation.

## Sprint 39.2: Deterministic retained-PV generation + the explicit bind ✅
...
`.build/runs/phase_33/` for repository-local attestation, never an authored root.

## Sprint 39.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip ✅
...
### Deliverables
- Minimal single-ordinal Postgres and MinIO witness StatefulSets on retained PVs (from Sprint 39.2), served
  from baked-binary images; these are rebind witnesses, not distributed MinIO (Phase 41) or HA
  Patroni-via-Percona (Phase 42), and carry no Vault-enveloping (Phase 40).
...
1. Run the cycle on the concrete representative set of [Gate integrity](#gate-integrity) (exactly two witnesses): generate a per-run nonce, assert its absence, write it as the Postgres row and the MinIO object,
   ...
   `${RETAINED_ROOT}` still holds the bytes, `cluster recreate` as a fresh Phase-35 bootstrap (new apiserver
...
2. Assert the full deletion reclaimed no backing volume (fresh PV objects re-appear post-recreate and
   ...
   host bytes are present. The control-plane daemon is a Phase-44 subject with **no realized instance at Phase 39**, so its "mounts
...
`.build/runs/phase_33/`; externally attest the bundle and rerun after Phase 38 closes.
...
- `documents/engineering/storage_lifecycle_doctrine.md` — the §6 lossless-rebind guarantee gains its first
  ...
  planning-ownership pointer resolves to delivered Phase-39 sprints.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the
  Phase-37 SSA reconciler that applies the StorageClass and retained-PV objects
...
- [phase_37](phase_37_object_reconciler.md) — the typed renderer + live SSA reconciler this phase builds on
- [phase_40](phase_40_vault_pki.md) — the root Vault whose durable KV rebinds on the retained storage proven here
- [phase_41](phase_41_platform_backbone.md) / [phase_42](phase_42_platform_services_2.md) — the HA
