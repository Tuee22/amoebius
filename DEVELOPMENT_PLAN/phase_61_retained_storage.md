# Phase 61: No-provisioner retained storage + lossless rebind
...
- [Sprints](#sprints)
- [Sprint 61.1: The one inert `no-provisioner` StorageClass ⏸️](#sprint-611-the-one-inert-no-provisioner-storageclass-)
- [Sprint 61.2: Deterministic retained-PV generation + the explicit bind ⏸️](#sprint-612-deterministic-retained-pv-generation--the-explicit-bind-)
- [Sprint 61.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip ⏸️](#sprint-613-the-lossless-rebind-gate--postgres-row--minio-marker-round-trip-)
...
8 supplies the policy-only `ProvisionedStorageScalingEnvelope` and pure observe-then-plan fold, and Phase 59
...
resize. `CreateProviderCapacity` has no retained-host mutation capability and remains owned by Phase 76.
...
Phase 63 and HA Patroni-via-Percona Postgres in Phase 64, the Vault-enveloping of secrets is Phase 62, and the
Keycloak-owned edge is Phase 65 — none of which this phase requires. The control plane
...
The live gate uses the Phase-56 single-node `kind` cluster; pure StorageClass/PV rendering remains Register 1–2.

**Lane:** linux-cpu/amd64 ([§L](development_plan_standards.md#l-one-substrate-discipline))

**Register:** 3 — live infrastructure ([§K](development_plan_standards.md#k-honesty-proven--tested--assumed)).

**Depends on:** [Phase 60](phase_60_capacity_scheduler.md)
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 60; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
- [`cluster_lifecycle_doctrine.md` §7 — Ephemeral spin-up/down with deterministic rebind](../documents/engineering/cluster_lifecycle_doctrine.md#7-ephemeral-spin-updown-with-deterministic-rebind)
  ...
  (targeted by Phase 59) — the SSA reconciler that must render and apply the StorageClass and PV objects; its
  future predecessor approval, not this sentence, is the only admissible dependency evidence.
...
## Sprint 61.1: The one inert `no-provisioner` StorageClass ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 60](phase_60_capacity_scheduler.md) human approval
...
- A single rendered StorageClass — `provisioner: kubernetes.io/no-provisioner`, `reclaimPolicy: Retain`,
  `volumeBindingMode: WaitForFirstConsumer` — applied through the Phase-59 reconciler under the `amoebius`
  field manager.
...
## Sprint 61.2: Deterministic retained-PV generation + the explicit bind ⏸️
...
## Sprint 61.3: The lossless-rebind gate — Postgres row + MinIO marker round-trip ⏸️
...
- Minimal single-ordinal Postgres and MinIO witness StatefulSets on retained PVs (from Sprint 61.2), served
  from baked-binary images; these are rebind witnesses, not distributed MinIO (Phase 63) or HA
  Patroni-via-Percona (Phase 64), and carry no Vault-enveloping (Phase 62).
...
1. Run the cycle on the concrete representative set of [Gate integrity](#gate-integrity) (exactly two witnesses): generate a per-run nonce, assert its absence, write it as the Postgres row and the MinIO object,
   ...
   `${RETAINED_ROOT}` still holds the bytes, `cluster recreate` as a fresh Phase-56 bootstrap (new apiserver
...
2. Assert the full deletion reclaimed no backing volume (fresh PV objects re-appear post-recreate and
   ...
   host bytes are present. The control-plane daemon is a Phase-66 subject with **no realized instance at Phase 61**, so its "mounts
...
`.build/runs/phase_60/`; externally attest that untracked run-local bundle and rerun after Phase 60 closes.
...
- `documents/engineering/storage_lifecycle_doctrine.md` — the §6 lossless-rebind guarantee gains its first
  ...
  planning-ownership pointer resolves to delivered Phase-61 sprints.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the
  Phase-59 SSA reconciler that applies the StorageClass and retained-PV objects
...
- [phase_59](phase_59_object_reconciler.md) — the typed renderer + live SSA reconciler this phase builds on
- [phase_62](phase_62_vault_pki.md) — the root Vault whose durable KV rebinds on the retained storage proven here
- [phase_63](phase_63_platform_backbone.md) / [phase_64](phase_64_platform_services_2.md) — the HA
