# Phase 43: Owner-scoped UI projection runtime
...
- [Sprints](#sprints)
- [Sprint 43.1: Build and independently verify the owner-scoped live projection ⏸️](#sprint-431-build-and-independently-verify-the-owner-scoped-live-projection-)
...
Done (invalidated). Phase 23 owns client/server plan compilation and Phase 27 owns local interpreter composition. This
...
`(AppId, TenantId, Owner, CommandId)`, the Phase-42 workflow work-id and `WorkflowHandle`, the normalized input
...
**Depends on:** Phase 26, Phase 41, and Phase 42. Phase 26 supplies sealed server dispatch, Phase 41 supplies
the live trusted request context and scoped authorization result, and Phase 42 supplies the live workflow/event
path. Phase 40 is transitive through Phases 41 and 42.
...
- **Fresh challenges:** after Keycloak, Pulsar, worker, query handler, and observers are ready, the harness
  creates unpredictable per-owner nonces and command ids and emits them through the Phase-42 workflow
...
- A projection cannot be served until caught up to the release-required watermark. Phase 45 pins that
...
## Sprint 43.1: Build and independently verify the owner-scoped live projection ⏸️
...
None for Sprint 43.1. Browser/reconnect, release, ML-lift, HA, and cross-cluster claims remain with their named
later phases.
...
- `DEVELOPMENT_PLAN/phase_45_ui_program_release.md`, `phase_55_infernix_ui_lift.md`, and
  `phase_66_jitml_ui_lift.md` — consume its caught-up watermark/read-model/receipt result without expanding its
  claim.

## Related Documents

- [Phase 26 — UI server boundary](phase_26_ui_server_boundary.md)
