...
Phase 28 delivers the UI effect binding; its design is owned by [low_code_ui_runtime_doctrine.md](../documents/engineering/low_code_ui_runtime_doctrine.md), [service_capability_doctrine.md](../documents/engineering/service_capability_doctrine.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 28.1: Seal the effect-handler-capability relation ✅](#sprint-281-seal-the-effect-handler-capability-relation-)
...
bound to source snapshot `sha256:27a64549485f9ff0…`; Phase 28 owns no remaining migration deferral.
...
move into `.build/runs/phase_22/<run-id>/`, and 55 surfaces join two-way to 85 run-time enumerated items.
...
This phase implements one seam: the total binder from the Phase-27 checked authorization program and trusted
...
current-authority kernel from Phase 27; it does not reproduce policy evaluation, compile plans, or run effects.
...
**Depends on:** [Phase 27](phase_27_ui_authorization_kernel.md) — the sealed action registry, scoped authority transition, and stale-authority refusal.
...
## Sprint 28.1: Seal the effect-handler-capability relation ✅
...
None in Phase 28. Browser, handler, provider-state, authorization-runtime, and live tenant-isolation truth stay
...
- Phase 29 — consume only the sealed `BoundUiProgram`.

## Related Documents

- [Phase 27](phase_27_ui_authorization_kernel.md) — the required action registry and authority transition.
