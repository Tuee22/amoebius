# Phase 45: Tenant/provider provisioning
...
Phase 45 delivers the tenant/provider provisioning; its design is owned by [tenancy_doctrine.md](../documents/engineering/tenancy_doctrine.md), [service_capability_doctrine.md](../documents/engineering/service_capability_doctrine.md), [platform_services_doctrine.md](../documents/engineering/platform_services_doctrine.md), and the plan for reaching it is owned here.
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_44_live_dsl_deploy.md, DEVELOPMENT_PLAN/phase_46_pulsar_client.md, DEVELOPMENT_PLAN/phase_47_user_tenant_isolation_live.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 45.1: Derive, apply, and externally read back tenant provider policy ⏸️](#sprint-451-derive-apply-and-externally-read-back-tenant-provider-policy-)
...
Phase 45 binds a checked `TenantSpec` containing issuer-qualified `SubjectSpec`, `Membership`, `Owner`, and
...
The phase then applies that sealed derivation to the live Phase-44 cluster and reads each provider back through
...
policies: Phase 47 owns the real-credential, own-subject/foreign-subject and own-tenant/foreign-tenant
...
**Depends on:** [Phase 27](phase_27_ui_authorization_kernel.md) and [Phase 43](phase_43_keycloak_ingress.md). Phase 27 supplies the closed authorization graph and derive-not-author
boundary; Phase 44 supplies the live checked-spec control-plane daemon. Phase 26 is transitive through Phase 27.

**Substrate:** linux-cpu — the single-node `kind` cluster assembled through Phase 43. No linux-cuda, Apple,
...
[Gate integrity](#gate-integrity) — Register 3 green, the application data path UNVERIFIED until Phase 46.
...
- The provider index is closed and exhaustive. NetworkPolicy is a Kubernetes-API payload; Postgres roles and
  ...
  objects. Pulsar application produce/consume is owned by Phase 46 and scoped end-to-end enforcement by Phase
  36.
...
- [`tenancy_doctrine.md` §4 — typed tenant and subject shapes](../documents/engineering/tenancy_doctrine.md#4-the-typed-shapes-tenantspec--subjectspec--membership--owner--rolebinding): consume the Phase-26 identity
...
- [`illegal_state_security.md` §3.80](../documents/illegal_state/illegal_state_security.md#380-a-subject-resolving-or-mutating-another-subjects-resource-without-a-grant): Phase 45 provisions the policy
  precondition but leaves its live request-enforcement residue to Phase 46.
...
## Sprint 45.1: Derive, apply, and externally read back tenant provider policy ⏸️
...
4. Tear down and require authenticated provider inventories to equal preflight; persist only hashed evidence
   and explicitly mark application data-path isolation `UNVERIFIED (Phase 41)`.

### Remaining Work

None. Phase 47 retains the application request-isolation residue.
...
- `documents/illegal_state/illegal_state_security.md` — attach the killed hand-authored/tenant-collapse evidence
  while retaining Phase 47's live enforcement residue.
...
- `DEVELOPMENT_PLAN/phase_46_pulsar_client.md` and `phase_47_user_tenant_isolation_live.md` — consume provider
  readiness without treating it as user authorization evidence.

## Related Documents

- [Phase 27 — UI authorization kernel](phase_27_ui_authorization_kernel.md)
- [Phase 44 — live DSL control-plane daemon](phase_44_live_dsl_deploy.md)
- [Phase 46 — Pulsar client](phase_46_pulsar_client.md)
- [Phase 47 — live subject/tenant isolation](phase_47_user_tenant_isolation_live.md)
