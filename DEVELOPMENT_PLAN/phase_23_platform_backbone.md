# Phase 23: Platform backbone (MetalLB + MinIO + Pulsar HA)
...
statement is design intent, never a tested amoebius result. This phase opens after the Phase 22 root-Vault/PKI
gate passes and runs on the **linux-cpu** substrate across **Register 3** (live infrastructure) — a
single-node `kind` cluster on a linux-cpu host, on top of the Phase-18 registry + baked base image, the
Phase-19 typed renderer + SSA reconciler, the Phase-21 no-provisioner retained storage, and the Phase-22
...
This phase turns the storage-and-secrets-provisioned cluster of Phase 22 into an amoebius cluster carrying the
...
Phase-19 `renderAll` path (no Helm, no third-party charts, and the emitted manifests are generated from Haskell
and never committed), each served from binaries **baked into the multi-arch base image** with no
public-registry pull, and each on the Phase-21 `no-provisioner` retained PVs where it holds durable state.

Two deferred gaps from earlier phases close here. First, the **registry→MinIO S3-driver rehoming**: the
Phase-18 `distribution` registry ran against **interim node-local (filesystem-driver) blob storage** with the
MinIO S3 driver named as this phase's target; Phase 23 moves the registry's blob store onto MinIO via the S3
...
registry's capacity proof. Phase 23 preserves the artifact/upload
operands from Phase 18's canonical `RegistryStorageDemand` — every digest-keyed compressed
...
erasure/healing geometry. Every pre-existing Phase-18 artifact is copied and independently digest-verified
...
stack** are [Phase 24](phase_24_platform_services_2.md). The **Keycloak-owned ingress edge** — Envoy/Gateway
API terminating TLS and Keycloak owning all wild ingress so no workload publishes its own path — is
[Phase 25](phase_25_keycloak_ingress.md); this phase brings the backbone up behind no public edge. The
Deployment-`replicas=1` control-plane singleton that will eventually *own* this reconcile loop is
[Phase 26](phase_26_live_dsl_singleton.md); here the reconciler is driven from the operator/host path against a
...
apple, linux-cuda, or windows substrate is touched in Phase 23.
...
(typed objects rendered by the Phase-19 `renderAll`, images resolved only in-cluster with **no public-registry
...
off the interim node-local filesystem driver onto the MinIO S3 driver, closing the Phase-18 deferred gap and
verified by an external observer that the registry's pushed blobs materialize as objects in the MinIO bucket
(not on the node filesystem). The MinIO-bound private `ProvisionedRegistryStorageDemand` preserves Phase 18's
...
gate oracles in the Sprint 23.1–19.3 Deliverables:
...
- **Image-identity provenance (§M.5 OS-boundary observer).** "No public-registry pull" is not a self-emitted
  ...
  amoebius-rendered namespaces) MUST equal the Phase-18 baked base-image digest resolved from the in-cluster
  ...
  fixture-oracle `test/fixtures/phase23/expected-base-digest.txt` pins the accepted digest.
- **Render byte-identity (§M.3 independent provenance).** At gate time the pure `renderAll` is re-run in-process
  ...
  filtered) MUST be byte-identical to that fresh `renderAll` output — pinning applied manifests to the Phase-19
...
- **Registry rehoming (§M.3 independent oracle, §M.5 external observer, §M.2 committed mutant).** The
  ...
  `test/fixtures/phase23/registry-storage-driver.golden` (the expected `distribution` storage stanza — S3
  driver, MinIO endpoint, bucket — authored by hand, not read from the running config), and the rehoming is
  observed externally: every digest in a Phase-18 preexisting artifact is copied to its exact target object,
  ...
  **`mutant/registry-fs-driver`** — the `distribution` config left on the interim node-local filesystem driver
  ...
  upload/orphan witness to match Phase 18. It also independently derives the private
...
- **Pulsar offload bound (§M.5 external observer, §M.2 committed mutant, §M.7 concrete drill).** The
  ...
  cap committed in `test/fixtures/phase23/hot-tier-cap.golden`; sustained ingest past that cap MUST cause
...
Phase 18) is present as a rehoming consumer of MinIO, not re-delivered here.
...
- [`platform_services_doctrine.md §1 — the invariant: every cluster is the same cluster`](../documents/engineering/platform_services_doctrine.md#1-the-invariant-every-cluster-is-the-same-cluster)
  ...
  Phase 23 materializes the backbone slice of the fixed standard service set on linux-cpu, each service the
...
- [`platform_services_doctrine.md §4 — MinIO, the object substrate`](../documents/engineering/platform_services_doctrine.md#4-minio--the-object-substrate),
  ...
  (the MetalLB LoadBalancer half — the Keycloak edge is Phase 25): the concrete providers Phase 23 stands up
...
- [`image_build_doctrine.md §2 — the single distribution rule: bake the binaries, build the amoebius image, pull only in-cluster`](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster)
  ...
  every backbone binary is baked into the Phase-18 multi-arch base image and resolved only in-cluster; the
  registry stores its blobs in MinIO via the S3 driver, so MinIO must be serving before the registry — the
  thin ordering edge [§9](../documents/engineering/image_build_doctrine.md#9-bring-up-ordering--the-registry-chicken-and-egg-dissolves) names, and the rehoming Phase 23 delivers.
- [`manifest_generation_doctrine.md §5 — the apply/reconcile engine: snapshot-bound typed actions`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  Phase 23 reuses the Phase-19 pure `renderAll :: ProvisionedSpec -> [K8sObject]` and typed-action reconciler whose
...
- [`platform_services_doctrine.md §10 — every execution unit declares its complete resource envelope`](../documents/engineering/platform_services_doctrine.md#10-every-execution-unit-declares-its-complete-resource-envelope)
  ...
  witness are unchanged from Phase 18 — including compressed objects,
...
- [`storage_lifecycle_doctrine.md §2 — one storage class, and it provisions nothing`](../documents/engineering/storage_lifecycle_doctrine.md#2-one-storage-class-and-it-provisions-nothing)
  ...
  each stateful backbone service (MinIO, Pulsar's ZooKeeper members and bookies) lands its durable bytes on the Phase-21
...
## Sprint 23.1: MetalLB LoadBalancer + MinIO object substrate + registry S3-driver rehoming 📋
...
rehome the Phase-18 `distribution` registry's blob store off the interim node-local filesystem driver onto the
MinIO S3 driver — closing the Phase-18 deferred gap.

### Deliverables
- MetalLB rendered as a standard service that publishes a LoadBalancer address on the kind node, available
  before the (Phase 25) Envoy/Gateway edge needs one — the backbone's LoadBalancer root.
- MinIO (HA / distributed) as the S3 object substrate on Phase-21 retained PVs, holding the content store,
...
- The `distribution` registry's blob store rehomed onto the MinIO S3 driver — the registry holds no PV of its
  own, its bytes live in MinIO — asserted against the committed `test/fixtures/phase23/registry-storage-driver.golden`
  ...
  witness exactly match Phase 18's interim provision and whose
  ...
  observer sees it.
- A `RegistryBackendMigrationDemand` from the still-live Phase-18 private provision to the MinIO replacement.
...
### Validation
1. Apply through the Phase-19 reconciler; assert MetalLB advertises an address and MinIO reaches its Ready
...
3. Assert the registry is serving from the MinIO S3 driver: its storage config is byte-equal to the committed
   ...
   a pre-existing Phase-18 artifact was copied/verified and that artifact still pulls by the same digest, and a
   ...
   Compare the Phase-18 and Phase-23 private registry witnesses: `objectSet`, structured `objectStorePeak`,
...
- Bounded per-topic retention with a **size-triggered MinIO offload** (no unbounded storage, no time-only
  trigger), wired to the Sprint-23.1 MinIO substrate; the drill topic's hot-tier size cap committed in
  `test/fixtures/phase23/hot-tier-cap.golden`, with the committed `mutant/offload-time-only` seeded mutant
...
2. Produce then consume a message; assert an at-least-once round-trip. Dedup is **exercised**: inject a
   ...
   byte-for-byte (full native-client proof deferred to Phase 28, marked UNVERIFIED here).
...
and [`image_build_doctrine.md §9 — bring-up ordering`](../documents/engineering/image_build_doctrine.md#9-bring-up-ordering--the-registry-chicken-and-egg-dissolves):
assemble the Sprint 23.1–19.2 backbone services, bring them up event-driven behind the reconciler's
...
- A backbone bring-up that applies MetalLB, MinIO, the rehomed `distribution` registry, and Pulsar through the
  Phase-19 reconciler, each dependent starting on its dependency's observed-ready edge (MinIO serving before
...
- The phase-gate harness: bring the whole backbone up on a fresh single-node linux-cpu `kind` cluster and
  ...
  Postgres/observability (Phase 24), and singleton-owned reconcile layers UNVERIFIED; the independent
...
- The gate oracles reused here, **authored and committed in Phase 0 before any implementation** (§M.1): the
  baked-base-image digest oracle `test/fixtures/phase23/expected-base-digest.txt`, the registry storage-stanza
  oracle `test/fixtures/phase23/registry-storage-driver.golden`, and the drill-topic hot-tier cap
  `test/fixtures/phase23/hot-tier-cap.golden`, plus the independently computed
  `test/fixtures/phase23/storage-geometry-boundaries.csv` covering BookKeeper/MinIO exact-fit, one-byte-over,
  recovery/healing, orphan horizon, and uniform-ordinal rounding. The gate also reuses
  `test/fixtures/phase18/registry_storage_demand.dhall` unchanged and pins the expected mapping from its private
...
3. Recompute `storage-geometry-boundaries.csv` with the independent checker and compare it with the opaque
   ...
   replacement `objectSet`, `derivedPeak`, and upload/orphan witness exactly match Phase 18 while its backend
...
4. Assert the whole backbone is up and HA-shaped from generated manifests + baked-binary images. "HA-shaped"
   ...
   **and** every running container's `imageID` digest (`kubectl get pods -A -o jsonpath={..imageID}`) MUST
   equal the Phase-18 baked base digest committed in `test/fixtures/phase23/expected-base-digest.txt` and
   ...
   layer *tested* not *proven*, with the Keycloak edge, Phase-24 Postgres/observability, and singleton-owned
...
- `documents/engineering/image_build_doctrine.md` — the §9 "registry stores its blobs in MinIO via the S3
  driver" edge is delivered; the Phase-18 interim filesystem-driver residue is discharged, recorded as the
  Phase-23 rehoming.
- `documents/engineering/pulsar_client_doctrine.md` — the §6.1 size-triggered offload bound gains its first
  live drill (the platform-side bring-up); the native-client CBOR round-trip proof stays owned by Phase 28.
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-23 status when the gate passes and link this document.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the Phase-19 renderer + SSA wait-for-ready that applies and sequences the backbone
...
- [phase_18](phase_18_base_image_registry.md) — the base image + `distribution` registry whose interim filesystem-driver blob store this phase rehomes onto MinIO
- [phase_22](phase_22_vault_pki.md) — the root Vault/PKI whose unseal edge gates Pulsar's secret-dependent startup here
- [phase_24](phase_24_platform_services_2.md) — the Percona/Patroni + pgAdmin + observability services and the full derived readiness-DAG gate that build on this backbone
- [phase_25](phase_25_keycloak_ingress.md) — the Keycloak-owned ingress edge that fronts this backbone next
- [phase_28](phase_28_pulsar_client.md) — the native `amoebius-pulsar` client whose full CBOR round-trip proof this phase defers
