# Phase 54: Apple-Metal host compute daemon
...
> sole Phase-38 mutation gateway fronting MinIO, with the native worker built **headless
...
Phase 54 delivers the Apple-Metal host compute daemon; its design is owned by [host_cluster_comms_doctrine.md](../documents/engineering/host_cluster_comms_doctrine.md), [substrate_doctrine.md](../documents/engineering/substrate_doctrine.md), [resource_capacity_types.md](../documents/engineering/resource_capacity_types.md), and the plan for reaching it is owned here.
...
- [Sprints](#sprints)
- [Sprint 54.1: Apple substrate management — Lima Linux VM + brew lazy tool-ensure ⏸️](#sprint-541-apple-substrate-management--lima-linux-vm--brew-lazy-tool-ensure-)
- [Sprint 54.2: Host-only loopback NodePort exposure of the content-mutation gateway + Pulsar ⏸️](#sprint-542-host-only-loopback-nodeport-exposure-of-the-content-mutation-gateway--pulsar-)
- [Sprint 54.3: Headless host-native Metal bridge + native worker build (no Tart) ⏸️](#sprint-543-headless-host-native-metal-bridge--native-worker-build-no-tart-)
- [Sprint 54.4: Host compute daemon lifecycle as a managed subprocess ⏸️](#sprint-544-host-compute-daemon-lifecycle-as-a-managed-subprocess-)
- [Sprint 54.5: Channel-2 peer + wild-exposure unrepresentable + the Apple-Metal peer gate ⏸️](#sprint-545-channel-2-peer--wild-exposure-unrepresentable--the-apple-metal-peer-gate-)
...
binds the in-cluster Phase-38 content-mutation gateway and Pulsar service to **host-only loopback NodePorts** reachable only
...
messages, mutations pass only through the Phase-38 gateway into the content-addressed MinIO store, and there is
...
The scope stops at the host-worker shell and its wire. [Phase 49](phase_49_determinism_jitcache.md) owns resolution
...
them: Phase 24's substrate detection, `pb` bootstrap coordinator handoff, and no-`PATH` tool-ensure kernel; Phase 31's MinIO
and Pulsar standard services; Phase 36's native Pulsar client; Phase 38's content-addressed store and workflow
runtime; Phase 49's determinism kernel; Phase 49's jit-build engine cache; and Phase 30's Vault secrets-by-name.

```mermaid
flowchart LR
%% register: orientation
  apple[Apple Silicon host: detected apple substrate] --> lima[Lima Ubuntu-24.04 VM: single-node cluster]
  lima --> svc[Phase-38 content-mutation gateway fronting MinIO plus Pulsar]
...
- **Input-dependent output oracle (§M.1/§M.3).** The artifact retrieved from MinIO must equal a fresh output
  ...
  `.build/runs/phase_54/`. A nonce-derived challenge input C is also generated per run. A, B, and C must produce
...
- **Representative set (§M.7).** The gate's representative set is exactly: one apple substrate (Apple-Silicon
  ...
  and the four wild-exposure negatives enumerated in Sprint 54.5. No other shape is claimed.
- **VM-disk geometry is materialized and observed (§M.3/§M.4).** Before create, the independent oracle
  ...
  twice.
- **Leak-free, defined ([§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub)).** "Tears down leak-free" at Phase 54 means, asserted by the postflight probe:
  ...
  swept to its pre-run state. Phase 55's general postflight sweep is not yet available, so this phase pins its
...
exposes the sole Phase-38 content-mutation gateway and Pulsar (not raw MinIO) on loopback NodePorts, builds
...
- `test/golden/phase_54/metal_job_ref.py` — the independently reviewed CPU/NumPy reference for the kernel.
  It generates A/B/C expected bytes and identities only at gate-run time beneath `.build/runs/phase_54/`.
...
- `test/dhall/phase_54_illegal/` — the four wild-exposure negatives (see Sprint 54.5), each a one-field
...
The worker's source-equal Phase-36/38 work/result topic and subscription demand also retains finite message/
...
The host content client terminates at the Phase-38 sole content-mutation gateway NodePort; raw MinIO backend
...
The content gateway/collector, MinIO backend, Pulsar, Vault and Phase-34 singleton instances are inherited
...
This phase applies the exhaustive desired-object and etcd-capacity protocol established in Phase 42 to the
...
- [`host_cluster_comms_doctrine.md §2`](../documents/engineering/host_cluster_comms_doctrine.md#2-the-decision-that-was-open-and-is-now-resolved)
  ...
  (*coordination is Pulsar + MinIO*, with MinIO mutation mediated by the Phase-38 gateway), the
...
## Sprint 54.1: Apple substrate management — Lima Linux VM + brew lazy tool-ensure ⏸️
...
invoked by absolute path through brew — the substrate foundation every later Phase-54 sprint stands on. The
...
## Sprint 54.2: Host-only loopback NodePort exposure of the content-mutation gateway + Pulsar ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `src/Amoebius/HostComms/NodePort.hs`,
`src/Amoebius/HostComms/Loopback.hs`
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: after bring-up, the Phase-38
...
## Sprint 54.3: Headless host-native Metal bridge + native worker build (no Tart) ⏸️
...
- Runtime MSL compilation: the host binary renders Metal Shading Language, writes a content-addressed
  source-metadata cache record into the Phase-49 `CacheBudget`-bounded cache, and dispatches through the
  bridge's `MTLDevice.makeLibrary(source:options:)` with fast-math **off** (the Phase-49 determinism
...
2. Compile generated MSL at runtime and dispatch A, B, and nonce-derived C. Generate all three expected outputs
   with `test/golden/phase_54/metal_job_ref.py` under `.build/runs/phase_54/`; require exact equality, distinct
...
## Sprint 54.4: Host compute daemon lifecycle as a managed subprocess ⏸️
...
## Sprint 54.5: Channel-2 peer + wild-exposure unrepresentable + the Apple-Metal peer gate ⏸️
...
through the Phase-38 mutation gateway on `127.0.0.1:<nodeport>` with no mTLS and no bespoke RPC, its Vault
...
### Remaining Work
Remove the tracked expected outputs and old replay mutant as part of Sprint 54.3. The pure peer/auth and
...
- `documents/engineering/host_cluster_comms_doctrine.md` — its §9 planning-ownership pointer resolves to
  delivered Phase-54 sprints, and the §2/§5/§6 honesty notes flip from "resolved design decision / sibling
...
- `documents/engineering/substrate_doctrine.md` — its §9 planning-ownership pointer resolves to delivered
  Phase-54 sprints; the §4.3 "no macOS build VM" note and the §5 host-worker description gain their first
...
- `documents/engineering/resource_capacity_doctrine.md` — record the Phase-54 live check for private
...
- [README.md](README.md) — flip the Phase-54 row status once the gate passes and link this document.
- [substrates.md](substrates.md) — record Phase 54's gate substrate (apple) in the per-phase substrate map, and
...
- [system_components.md](system_components.md) — register the host-worker and host-comms modules
  ...
  as Phase-54 design-first rows.

## Related Documents
- [README.md](README.md) — the live tracker; the Phase 54 row is the authoritative one-line gate and status
...
- [phase_49](phase_49_determinism_jitcache.md) — the jit-build engine resolver + `CacheBudget` cache the Metal
  kernel is materialized into
- [phase_52](phase_52_jitml_lift_cuda.md) — the CUDA jitML lift; its Windows-CUDA case is the structurally
