# Phase 14: Capacity core fold + topology relation
...
- [Sprints](#sprints)
- [Sprint 14.1: The `Topology` relation — `ComputeEngine` / `LinuxHost` witness / elementwise compatibility fold ✅](#sprint-141-the-topology-relation--computeengine--linuxhost-witness--elementwise-compatibility-fold-)
- [Sprint 14.2: The base capacity fold — `fits` / `podFits` / `carve` / `place` ✅](#sprint-142-the-base-capacity-fold--fits--podfits--carve--place-)
- [Sprint 14.3: QuickCheck properties — soundness, totality, elementwise compatibility + the independent witness validator ✅](#sprint-143-quickcheck-properties--soundness-totality-elementwise-compatibility--the-independent-witness-validator-)
- [Sprint 14.4: The base capacity/topology fold-negative corpus + the gate ✅](#sprint-144-the-base-capacitytopology-fold-negative-corpus--the-gate-)
...
bound to source snapshot `sha256:6eecf9051956a260…`; Phase 14 owns no remaining migration deferral.
...
resolves it per run, failing closed when unset — a PATH fallback would have defeated the Phase-12 absolute-argv
...
[phase_15_storage_geometry_folds.md](phase_15_storage_geometry_folds.md) owns the logical→physical storage
...
[phase_16_execution_accelerator_folds.md](phase_16_execution_accelerator_folds.md) owns the execution-epoch
...
representation, and the two-ceiling Pulsar fold ([phase_15_storage_geometry_folds.md](phase_15_storage_geometry_folds.md));
...
full-resource-vector place-witness gate ([phase_16_execution_accelerator_folds.md](phase_16_execution_accelerator_folds.md)).
...
([phase_17_capability_bind.md](phase_17_capability_bind.md)); and the whole-deployment provision seal
([phase_18_provision_seal.md](phase_18_provision_seal.md)) that re-exercises these same base folds after full
bind/expansion. Phase 14 does not move the folds into `Dhall.inputFile`.

**Substrate:** none — no host, no cluster; the gate is an in-process `cabal test` fold + QuickCheck battery,
analogous to the Phase 12 decode battery and the Phase 13 property suite.
...
clauses require for Phase 14; it strengthens, never weakens, the Gate and sprint Validations above. The source
...
storage-geometry fixtures are exercised by [phase_15_storage_geometry_folds.md](phase_15_storage_geometry_folds.md)
and the execution/accelerator/provider-root fixtures by
[phase_16_execution_accelerator_folds.md](phase_16_execution_accelerator_folds.md); the full corpus is not
re-checked here.
...
[phase_18_provision_seal.md](phase_18_provision_seal.md) re-exercises these same folds through its post-bind
...
flowchart LR
  ...
  g{{"the phase 14 gate command"}}:::gate
...
```
*Implemented Phase 14 gate apparatus; [§M](development_plan_standards.md#m-gate-integrity-a-gate-cannot-be-passed-by-a-stub) owns its clauses.*
...
variant is owned by [phase_16_execution_accelerator_folds.md](phase_16_execution_accelerator_folds.md). The
...
`legal_tmpfs_two_concurrent_writers_single_debit` are routed to phases 15 and 16 and are **not** in this gate's
...
declared `owner_phase` is `Phase-14`; every row has evidence at its registered dhall-typecheck, GADT-index, or
...
undercounted). The per-axis and per-eligibility validator mutants of Sprint 14.3 (drop CPU, memory, ephemeral
...
provider-root mutants of the source battery are the property of phases 15 and 16 and are not named here.
...
Defined in Sprint 14.3 Deliverables; it never calls `podFits` or
...
- [`resource_capacity_doctrine.md §4`](../documents/engineering/resource_capacity_doctrine.md#4-the-total-fold-fits-carve-place-and-the-nesting)
  ...
  adopted by [phase_15_storage_geometry_folds.md](phase_15_storage_geometry_folds.md); the accelerator and
  provider-root arithmetic by [phase_16_execution_accelerator_folds.md](phase_16_execution_accelerator_folds.md).
...
flowchart LR
  ...
  s1["Sprint 14.1: the Topology relation and its witnesses"]
  s2["Sprint 14.2: the base capacity fold"]
  s3["Sprint 14.3: QuickCheck properties and the independent validator"]
  s4["Sprint 14.4: the negative corpus and the gate"]
  g{{"the phase 14 gate"}}
...
## Sprint 14.1: The `Topology` relation — `ComputeEngine` / `LinuxHost` witness / elementwise compatibility fold ✅
...
**Blocked by**: [Phase 12](phase_12_gadt_decode_ir.md) gate (the GADT-indexed IR + total decoder); [Phase 13](phase_13_illegal_state_corpus.md) gate
...
each check.
**Docs to update**: `documents/engineering/cluster_topology_doctrine.md` (Phase-14 status backlink),
...
- The derived closed `NodeSupply = Fixed (NonEmpty Node) | Elastic { floor : [Node],
  ...
  **templates**, the fresh instance-scoped symbolic-id qualification, and the root-EBS/instance-store arithmetic
  are the deliverable of [phase_16_execution_accelerator_folds.md](phase_16_execution_accelerator_folds.md);
  Phase 14 consumes only the CPU/memory/slot capacity of each class.)
...
- The `mkRke2` distinctness fold over `servers ∪ agentFloor` rejecting a duplicate `HostId` — the checked
  ...
  "more nodes than hosts" are already type-foreclosed upstream (Phase 11/12).
...
2. The `bareAppleHost` / `bareWindowsHost` / even-server-quorum constructors have no inhabitant, proven the
   Phase-13 way: seven committed `ghc -fno-code` expect-fail compile goldens ([Gate integrity](#gate-integrity),
...
## Sprint 14.2: The base capacity fold — `fits` / `podFits` / `carve` / `place` ✅
...
`Amoebius/Capacity/*.hs` modules, are added by phases 15 and 16.
**Blocked by**: Sprint 14.1 (the `Topology` `place` folds over); [Phase 12](phase_12_gadt_decode_ir.md) gate (the IR + decoder).
...
**Docs to update**:
`documents/engineering/resource_capacity_doctrine.md` (Phase-14 status backlink for §3/§4),
...
- `Quantity` (refined non-zero, unit-tagged) for declarations plus `Residual = Zero | Remaining Quantity` /
  ...
  that [Phase 12](phase_12_gadt_decode_ir.md) refines. The pad enters the effective demand once, after the
  ...
  by phases 15 and 16; the base `PodLocalStorageDemand` is the disk-backed `emptyDir` + writable-rootfs/log
...
- An in-file honesty note: every capacity **sum** here is a total checked provision-seal operation,
  ...
  `Growable` growth re-run ([phase_15_storage_geometry_folds.md](phase_15_storage_geometry_folds.md)) never
...
## Sprint 14.3: QuickCheck properties — soundness, totality, elementwise compatibility + the independent witness validator ✅
...
the Phase-13 property harness. (The
runtime-metadata and provider-root property modules are the deliverable of phases 15 and 16.)
...
## Sprint 14.4: The base capacity/topology fold-negative corpus + the gate ✅
...
**Blocked by**: Sprint 14.1, Sprint 14.2, Sprint 14.3; [Phase 11](phase_11_dhall_typecheck_schema.md) gate (the positive dhall-typecheck corpus).
...
`documents/engineering/testing_doctrine.md`, `DEVELOPMENT_PLAN/README.md` (flip the Phase-14 status when the
gate passes), `DEVELOPMENT_PLAN/substrates.md` (the Phase-14 `none` gate row).
...
- The base fold-negative fixtures: `illegal_engine_substrate_mismatch` ([§3.13](../documents/illegal_state/illegal_state_topology.md#313-a-compute-engine-incompatible-with-its-substrates-managed-providers-first-class)), `illegal_rke2_reused_host`
  ([§3.16](../documents/illegal_state/illegal_state_topology.md#316-a-multi-node-rke2-cluster-with-fewer-linux-hosts-than-nodes-or-a-host-reused) distinctness), `illegal_overcommit_{host,vm,cluster}` ([§3.17](../documents/illegal_state/illegal_state_capacity.md#317-an-over-committed-deploy-or-workload-host--vm--cluster-capacity-exceeded), on the base CPU/memory/pod-slot/ CSI-attach/logical-ephemeral axis — the `PhysicalDiskPartition` disk-parent variant of `illegal_overcommit_host` is owned by [phase_16_execution_accelerator_folds.md](phase_16_execution_accelerator_folds.md)),
...
- The positive fixtures `legal_multisubstrate_cluster` (the [§3.13](../documents/illegal_state/illegal_state_topology.md#313-a-compute-engine-incompatible-with-its-substrates-managed-providers-first-class) heterogeneous carve-out, exercising the elementwise compatibility fold and the fixed-topology first-fit-decreasing witness) and `legal_managed_eks`
  ...
  concern of phases 15 and 16; this gate exercises only their compatibility/base-capacity aspects.
  `legal_tmpfs_two_concurrent_writers_single_debit` is routed to phases 15 and 16.)
- The committed corpus behind those fixtures: `test/spec/dsl/CapacityTopologyFixtures.hs` holds fifteen direct,
  ...
  are pinned by `test/oracle/capacity_topology/compile_fail.tsv`; and the actual Phase-12 `legal_multisubstrate_cluster`
...
2. The gate applies `fits`/`podFits`/`carve`/`place` and the topology relation **directly to each
   ...
   [phase_17_capability_bind.md](phase_17_capability_bind.md) and
   [phase_18_provision_seal.md](phase_18_provision_seal.md), whose gate re-exercises these same folds through
...
- `documents/engineering/resource_capacity_doctrine.md` — backlink §3's types and §4/§4.1's base fold to the
  ...
  storage arithmetic backlink is owned by [phase_15_storage_geometry_folds.md](phase_15_storage_geometry_folds.md).)
...
- `documents/illegal_state/illegal_state_catalog.md` — annotate the applicable §3.13–§3.17 parts with their
  ...
  runtime-checked entries (layer 3) deferred, and leave §3.19–§3.22/§3.27–§3.30 to phases 15 and 16.
...
- [phase_13](phase_13_illegal_state_corpus.md) — the illegal-state corpus, properties, and validation-locus ledger this phase extends
- [phase_15_storage_geometry_folds.md](phase_15_storage_geometry_folds.md) — the logical→physical storage geometry, `StorageBudget`/`Growable`, and two-ceiling Pulsar fold built atop this base fold
- [phase_16_execution_accelerator_folds.md](phase_16_execution_accelerator_folds.md) — the execution-epoch expansion, scheduler-reservation algebra, accelerator/VRAM, and provider-root arithmetic that compose the full-resource-vector place witness on this base fold
- [phase_17_capability_bind.md](phase_17_capability_bind.md) — the capability → provider → shape binder built atop these folds
- [phase_18_provision_seal.md](phase_18_provision_seal.md) — the whole-deployment provision seal that re-exercises these base folds after bind
- Phase 14 capacity/topology ledger — the human-readable Register-1 proof/test/assumption boundary
