...
the standard HA platform stack (Phase 18), Keycloak-owned ingress (Phase 19), the live DSL deploy via the
Deployment-`replicas=1` singleton (Phase 20), app tenancy (Phase 21), the content store + workflow runtime
(Phase 23), the jit-build engine cache (Phase 25), and the infernix CPU-inference lift (Phase 26) all already
...
in [Phase 12](phase_12_spa_composition_representational.md)** — that is the representational result this phase
...
gateway migration is claimed here (those are Phase 29). It proves that the established live surfaces compose
...
- [`daemon_topology_doctrine.md §3.1`](../documents/engineering/daemon_topology_doctrine.md#31-exactly-one-pod-is-a-k8setcd-property-not-an-amoebius-election)
  ...
  Deployment-`replicas=1` control-plane singleton (Phase 20), whose single-instance is delegated to k8s/etcd, so
...
**Blocked by**: Phase 12 (the decode-proven SPA spec `Amoebius.Spa.Spec` + `dhall/amoebius/Spa.dhall` +
`prop_spaCompositionDecodes` this deploys); Phase 8 (the capability → provider → shape binder the SPA's needs
resolve against); Phase 15 (the typed SSA reconciler); Phase 18 (the standard HA platform stack); Phase 20 (the
live DSL deploy via the Deployment-`replicas=1` singleton); Phase 21 (the app-tenancy namespace + `<app>/<bucket>`
...
1. The deployment-rules `.dhall` composes with the SPA spec and renders to the standard HA stack at the chosen
   replica count in the tenant namespace; a re-run of the reconciler is a no-op (convergent, ApplySet-pruned).
2. Changing the replica count or the inference-substrate binding changes no SPA `.dhall` or `.hs` source — the
   SPA app-spec hash is unchanged across the variations.
3. No orchestration path runs an election; the SPA is scheduled by the `replicas=1` singleton whose
   single-instance is delegated to k8s/etcd.
...
**Blocked by**: Sprint 32.1; Phase 19 (Keycloak owns all wild ingress via Envoy/Gateway API — the wild-ingress
...
UI; a variant SPA that attempts to publish an unauthenticated/open edge route fails Gate 1 (there is no syntax
for a backdoor); each capability the SPA consumes appears in the derived east-west graph and a surface consuming
an undeclared capability has no derived connectivity to it.
...
1. The chatbot SPA's published surface renders a live `HTTPRoute` gated by Keycloak; a request without a valid
   session is refused at the edge and an authenticated session reaches the UI.
2. A variant attempting an unauthenticated/open edge route fails Gate 1 (no backdoor syntax), and a surface
...
**Blocked by**: Sprint 32.1; Phase 23 (the content store + orchestrator/worker workflow runtime the inference
request round-trips over); Phase 26 (the infernix CPU-inference lift + its demo web app); Phase 25 (the
jit-build engine resolver + CacheBudget content-addressed cache the engine materializes into); Phase 27 (the
...
infernix chatbot workflow on the Phase-23 runtime — the response returns to the caller — with the inference
engine **jit-resolved on first miss** into the Phase-25 CacheBudget-bounded content-addressed cache and reused by
a second request without re-resolution; the jitML RL-gaming SPA composes and type-checks against the same SPA
spec, while a grep of neither SPA surface names a substrate; running the jitML workflow on a CUDA/Apple-Metal
substrate is explicitly out of contract for this single-substrate gate.
**Docs to update**: `documents/engineering/app_vs_deployment_doctrine.md`,
...
### Validation
1. An inference request round-trips through the composed infernix workflow and the response returns to the
   caller; a second request reuses the cached engine identity with no re-resolution and no URL fetch.
...
**Status**: Planned
**Implementation**: `amoebius-spa/dhall/test/phase_32_spa_live.dhall` (the gate topology),
...
inference round-trip the gate exercises); Phase 13 / Phase 16 (the cluster-lifecycle + retained-storage teardown
the InForceSpec drives).
**Independent Validation**: a gate `InForceSpec` composes the multi-service SPA + the infernix demo app's ML
workflow with the `linux-cpu` deployment-rules layer, deploys via the typed reconciler under the `replicas=1`
singleton, reaches the SPA UI through the Keycloak/Envoy edge, round-trips an inference request through the
composed infernix workflow, tears the deployment down **leak-free** (the postflight sweep observes no orphaned
resource), re-runs idempotently, and emits a per-run proven/tested/assumed ledger that marks no GPU/Apple-Metal
ML-workflow claim and no geo-replication claim green.
**Docs to update**: `documents/engineering/app_vs_deployment_doctrine.md`,
...
### Deliverables
- A gate `phase_32_spa_live.dhall` `InForceSpec` and its `SpaGateSpec` that spins up the SPA composing the
...
### Validation
1. The SPA deploys on the standard HA stack and its UI is reachable only through the Identity-owned edge; an
   inference request is served by the composed infernix workflow and returns to the caller.
2. The RL-gaming SPA composes and type-checks; the topology tears down leak-free (postflight sweep empty) and
...
> [Phase 12](phase_12_spa_composition_representational.md); the representational battery and the local
...
> no geo-replication is claimed here; those are Phase 29.
...
## Related Documents
- [README.md](README.md) — the live tracker; Phase 32 objective, gate, and substrate
...
- [phase_12](phase_12_spa_composition_representational.md) — the representational SPA composition proven in-process
