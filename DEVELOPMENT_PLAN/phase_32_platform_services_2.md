# Phase 32: Platform services-2 (Redis/Sentinel + Percona/Patroni + pgAdmin + observability + readiness-DAG)
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_29_retained_storage.md, DEVELOPMENT_PLAN/phase_31_platform_backbone.md, DEVELOPMENT_PLAN/system_components.md
...
- [Sprints](#sprints)
- [Sprint 32.1: Percona/Patroni Postgres per consumer + pgAdmin + Prometheus/Grafana ⏸️](#sprint-321-perconapatroni-postgres-per-consumer--pgadmin--prometheusgrafana-)
- [Sprint 32.2: Ephemeral Redis/Sentinel realtime coordination ⏸️](#sprint-322-ephemeral-redissentinel-realtime-coordination-)
- [Sprint 32.3: The full derived readiness-DAG bring-up + the standard-stack gate ⏸️](#sprint-323-the-full-derived-readiness-dag-bring-up--the-standard-stack-gate-)
- [Sprint 32.4: Register-2.5 readiness-DAG bring-up under simulated partial failure ⏸️](#sprint-324-register-25-readiness-dag-bring-up-under-simulated-partial-failure-)
...
`postgres-share-package.sha256` is a package/archive integrity observation. Both are generated. Phase 32 must
consume Phase 25's verified image identity and Phase 1's dynamic package resolution without committed checksum
files.

**Invalidated historical record:**

**Done.** All four sprints are implemented and the Phase-32 gate is sealed. The gate combines the pure
service model, 256 deterministic `IOSim` fault schedules plus `IOSimPOR`, eight specifically red mutants,
an independent live-receipt reader, and a warm reconciliation of the real Phase-31 cluster. It runs on the
...
Phase-27 typed renderer + SSA reconciler, the Phase-29 no-provisioner retained storage, the Phase-30 unsealed
root Vault, and the Phase-31 MetalLB/MinIO/Pulsar backbone. Redis/Sentinel, Patroni Postgres, pgAdmin,
...
This phase completes the standard platform-service set on top of the Phase-31 backbone. It renders and
...
`postgres` Pod), each rendered as typed Kubernetes objects by the Phase-27 `renderAll` path (no Helm, no
third-party charts, manifests generated from Haskell and never committed), each served from binaries **baked into the multi-arch base image** with no public-registry pull, and each on the Phase-29 `no-provisioner`
...
The phase then assembles the *whole* standard stack — the Phase-31 backbone plus these services — as one
...
order by the Phase-27 reconciler's **event-driven wait-for-ready**: a dependent starts on its dependency's
...
browser surface Grafana and pgAdmin reach a user through — is [Phase 33](phase_33_keycloak_ingress.md); this
...
[Phase 34](phase_34_live_dsl_singleton.md). Until then, a host-side operator drives this phase's fixed service
set.
...
**Gate:** `python3 tools/phase31_gate.py` brings the whole standard stack up on the Phase-31 backbone cluster
...
The apparatus phase 32's gate closes over, in the slot
...
- **Derived-DAG order (§M.2 committed mutant, §M.3 independent oracle, §M.4 coverage floor, §M.5 external-observer trace).** The
  ...
  (Sprint 32.3), checked against a committed hand-authored edge→order reference table
...
- **Applied-manifest oracle (§M.3 independent provenance).** Recompute `renderAll` during the gate and compare its
  ...
  third-party YAML therefore diverges from the Phase-27 renderer and fails.
- **Phase-32 resource witness readback (§M.3 independent projection).** Normalize every live resource-bearing
...
operator, the per-consumer Patroni Postgres clusters with pgAdmin for the named consumer set of Sprint 32.1,
Prometheus + Grafana, and Redis primary/replicas/Sentinel — no more, no fewer. The full derived DAG spans these plus the Phase-31 backbone
(MetalLB, MinIO, Pulsar) and the Phase-30 Vault.
...
- [`ui_realtime_coordination_doctrine.md §5 — Redis is ephemeral platform-internal coordination`](../documents/engineering/ui_realtime_coordination_doctrine.md#5-redis-is-ephemeral-platform-internal-coordination):
  Phase 32 stands up the bounded TLS/ACL Redis/Sentinel topology with no persistence or application authority.
...
- [`platform_services_doctrine.md §8 — Postgres, Patroni-via-Percona, one cluster per consumer, with pgAdmin`](../documents/engineering/platform_services_doctrine.md#8-postgres--patroni-via-percona-one-cluster-per-consumer-with-pgadmin)
  ...
  Phase 32 stands up the cluster-wide Percona operator reconciling one Patroni Postgres cluster per consuming
...
- [`platform_services_doctrine.md §7 — Prometheus / Grafana, observability is not an add-on`](../documents/engineering/platform_services_doctrine.md#7-prometheus--grafana--observability-is-not-an-add-on)
  ...
  through the generated admission proxy; the browser surfaces gated behind the (Phase 33) Keycloak edge
...
- [`manifest_generation_doctrine.md §5 — the apply/reconcile engine: server-side apply, owned field manager, prune, wait`](../documents/engineering/manifest_generation_doctrine.md#5-the-applyreconcile-engine-snapshot-bound-typed-actions)
  ...
  Phase 32 reuses the Phase-27 pure `renderAll :: ProvisionedSpec -> [K8sObject]` and typed-action reconciler whose **wait-for-ready is observed from the live object, never a `threadDelay`** to apply and sequence the set. - [`image_build_doctrine.md §2 — the single distribution rule`](../documents/engineering/image_build_doctrine.md#2-the-single-distribution-rule-bake-the-binaries-build-the-amoebius-image-pull-only-in-cluster):
...
- [`storage_lifecycle_doctrine.md §2 — one storage class, and it provisions nothing`](../documents/engineering/storage_lifecycle_doctrine.md#2-one-storage-class-and-it-provisions-nothing)
  ...
  each Patroni cluster and Prometheus lands its durable bytes on the Phase-29 `no-provisioner` retained PVs.
...
## Sprint 32.1: Percona/Patroni Postgres per consumer + pgAdmin + Prometheus/Grafana ⏸️
...
- The Percona operator rendered as a cluster-wide platform component (from the shared inventory so it installs
  ...
  the `distribution` registry takes **no** database ([§3](../documents/engineering/platform_services_doctrine.md#3-the-registry--the-single-image-source)). The consumer set is pinned here (resolving the `platform_services_doctrine.md §8` "Phase 31 delivery detail"); a `PerconaPGCluster` consumed by nothing does
...
- Prometheus + Grafana scraping platform workloads, with the per-workflow recording/alert rules and
  ...
  - The browser surfaces reach a user only through the (Phase 33) Keycloak edge under a mandatory
...
1. Assert the Percona operator is Ready before any `PerconaPGCluster`, then that the named consumer set
   ...
   `{Grafana}` because Keycloak's own store arrives with the [Phase 33](phase_33_keycloak_ingress.md) edge and
...
and monitoring expectations. The Keycloak browser edge remains Phase 33 and singleton-owned continuous
reconciliation remains Phase 34, both explicitly UNVERIFIED.

## Sprint 32.2: Ephemeral Redis/Sentinel realtime coordination ⏸️
...
## Sprint 32.3: The full derived readiness-DAG bring-up + the standard-stack gate ⏸️

**Status**: Blocked by the reopened numeric sequence; prior capability footprint retained for migration
**Implementation**: `src/Amoebius/Platform/Services.hs`,
`src/Amoebius/Platform/BringUp.hs`, `tools/phase31_gate.py`
**Blocked by**: reopened numeric predecessor gates.
**Independent Validation**: the full standard stack (Phase-31 backbone + the
...
### Deliverables
- A `BringUp` assembly that folds the whole standard-service set (Phase-31 backbone + Sprint-31.1 and Sprint-31.2 services)
...
- Live enactment by the Phase-27 reconciler's wait-for-ready — each dependent constructed to start on its
  dependency's observed-ready edge (a `runtime-checked` observation from the live object), never a duration;
  the singleton that will later own this loop is [Phase 34](phase_34_live_dsl_singleton.md) (Deployment `replicas=1`, no election).
- The phase-gate harness brings the standard stack onto Phase 31's live backbone and observes every required
...
2. Round-trip MinIO put/get and Pulsar produce/consume against the assembled stack; assert Postgres is a
   Patroni cluster, never a bare Pod, carrying the mandated synchronous config (the Sprint 32.1 oracle).
3. Assert the complete standard stack is ready and preserves Phase 31's already-gated backbone topology and
   provenance. For the Phase-32 additions, a one-versus-many replica render diff may change only count fields;
...
## Sprint 32.4: Register-2.5 readiness-DAG bring-up under simulated partial failure ⏸️
...
3. Replay a captured seed and assert a bit-identical schedule and outcome; emit the Register-2.5 ledger — substrate `none`, Register 2.5 — recording the honest limit that modeled-substrate fidelity is *assumed* and is discharged only by this phase's Register-3 live gate (Sprint 32.3).
...
- `platform_services_doctrine.md`, `chaos_failover_doctrine.md`, `readiness_ordering_doctrine.md`,
  ...
  `deterministic_simulation_doctrine.md` now record the tested Phase-32 boundary and its honest limits.
...
- [Manifest Generation & the Typed Reconciler](../documents/engineering/manifest_generation_doctrine.md) — the Phase-27 renderer + SSA wait-for-ready that applies and sequences the set
...
- [UI Realtime Coordination](../documents/engineering/ui_realtime_coordination_doctrine.md) — the ephemeral
  Redis/Sentinel topology and failure boundary delivered by Sprint 32.2
...
- [phase_30](phase_30_vault_pki.md) — the root Vault/PKI whose unseal edge gates secret-dependent startup here
- [phase_31](phase_31_platform_backbone.md) — the MetalLB/MinIO/Pulsar backbone this phase's services and DAG build on
- [phase_33](phase_33_keycloak_ingress.md) — the Keycloak-owned ingress edge that fronts Grafana and pgAdmin next
