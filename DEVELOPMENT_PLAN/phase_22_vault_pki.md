# Phase 22: Root Vault + PKI + built-in Haskell Vault client
...
design intent, never a tested amoebius result. This phase opens after the Phase 21 gate (no-provisioner retained
storage + lossless rebind) and runs on the **linux-cpu** substrate in **Register 3** — live infrastructure: a
single-node `kind` cluster on a linux-cpu host, its Vault rendered and applied by the Phase-19 reconciler onto
the Phase-21 retained PV. The whole model is **proven in the sibling prodbox project** — its `vault_doctrine.md`,
...
(Phases 23–24), the Keycloak-owned edge (Phase 25), and the parent/child unseal modes, parent secret injection, and the
...
## Sprint 22.1: Root single-node password-encrypted Vault — init-once / unseal-on-rebuild 📋
...
**Blocked by**: Phase 19 gate (the typed renderer + SSA reconciler — Vault is rendered and applied through it);
Phase 21 gate (no-provisioner retained storage — Vault's durable KV lives on a retained PV, so a rebuild
*unseals* rather than re-initializes); Phase 18 (the baked Vault binary in the in-cluster `distribution`
registry).
...
### Deliverables
- Root Vault in **Shamir seal mode**, rendered and reconciled onto the Phase-21 retained PV; first-ever `vault
...
- A **pluggable unlock-material backend** behind one interface — the load-bearing property is only that the
  material is password-AEAD-sealed and never plaintext at rest. **At the root Phase-22 bring-up the backend is
  the host-side `.age` file**: MinIO does not exist until Phase 23, so a MinIO-sealed object (and equally a cloud
...
## Sprint 22.2: The self-signed PKI trust anchor issues 📋

**Status**: Planned
**Implementation**: `src/Amoebius/Vault/Pki.hs` (the `pki/` root-CA mount + internal leaf issuance) — target path,
not yet built.
**Blocked by**: Sprint 22.1 (a live, unsealed Vault is a precondition for enabling `pki/` and issuing).
...
- The **three-planes distinction** recorded and enforced: internal PKI (this phase) is not public-edge TLS
  (ZeroSSL/route53, Phase 25) and is not the distro's own self-signed cluster CA (the chicken-and-egg floor,
...
## Sprint 22.3: Built-in Haskell Vault client (no agent sidecar) reads a `SecretRef` by name — the gate 📋
...
**Blocked by**: Sprint 22.1 (a live, unsealed Vault to read from); Sprint 22.2 (a workload verifying a peer cert
...
`DEVELOPMENT_PLAN/README.md` (flip the Phase-22 status when the gate passes), `DEVELOPMENT_PLAN/system_components.md`.
...
## Sprint 22.4: Register-2.5 fail-closed Vault unseal under simulated faults 📋
...
**Blocked by**: Sprint 22.1 (the real init/unseal/seal client under test), Sprint 22.3 (the real `SecretRef`
resolver + typed error under test); Phase 15 Sprint 15.2 (`src/Amoebius/Sim/Env.hs` + `src/Amoebius/Sim/Fakes/*` —
...
from the live path — runs under `IOSim`/`IOSimPOR` against the Sprint 15.2 modeled Vault with injected **sealed /
...
- An `IOSim`/`IOSimPOR` harness running the **real** `src/Amoebius/Vault/{Init,Unseal,Seal,Client}.hs` code
  (`io-classes`-written, byte-for-byte the live path — no simulation-only fork) against the **Sprint 15.2 modeled
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-22 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 22's gate substrate (linux-cpu) in the per-phase substrate map.
- `DEVELOPMENT_PLAN/system_components.md` — register `src/Amoebius/Vault/{Init,Unseal,Seal,Pki,Client,SecretRef,Error}.hs`
  as Phase-22 design-first rows against the component inventory.
...
- [phase_19](phase_19_object_reconciler.md) — the typed renderer + SSA reconciler that renders and applies Vault
- [phase_21](phase_21_retained_storage.md) — the no-provisioner retained PV Vault's durable KV lives on
- [phase_23](phase_23_platform_backbone.md) — the standard-service stack that consumes these Vault secrets
