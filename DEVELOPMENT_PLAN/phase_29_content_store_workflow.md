# Phase 29: Content store + workflow runtime (Pulsar-Failover single-writer)
...
> and the orchestrator/worker workflow runtime on top of the Phase-28 native Pulsar client, gated live on
...
Pulsar and MinIO already standing as HA platform services (Phase 23) on retained storage (Phase 21), and it
opens only after the Phase 28 gate (the native-protocol Pulsar client, CBOR codec, four subscription types,
...
`experiment-hash` namespace within an app's Phase-27 ObjectStore bucket. Every namespace also binds a finite
...
charged until an observed GC deletion; the logical peak consumes Phase 23's MinIO erasure/healing and uniform
claim-plan witness rather than assuming logical bytes equal physical disk. Second, an **orchestrator/worker
workflow runtime** on top of the Phase-28 client: an orchestrator worker produces a workflow `command` on a
...
This phase also closes the Job-terminal live-proof boundary deliberately left open in Phase 19. Phase 19
...
name-ordered standby, with the Phase-28 at-least-once contract redelivering the un-acked command; the store's
...
typeclass, and SplitMix seed derivation are the Phase 38 determinism kernel, not this phase.
...
window Sprint 29.4 injects in simulation) and **observes the lexically next-in-name-order standby take over the
...
checker projects committed + concurrent + orphan logical extents through the Phase-23 per-drive
erasure/healing and uniform-claim witness. The collector/verification Job also exercises the first live
Phase-19 terminal protocol: its exact completion variant is gateway-written and independently read back before
...
enumerated in Sprint 29.3 and fails hard on any non-empty remainder outside its explicitly named
...
- [`daemon_topology_doctrine.md §3.1`](../documents/engineering/daemon_topology_doctrine.md#31-exactly-one-pod-is-a-k8setcd-property-not-an-amoebius-election)
  ...
  control-plane singleton (Phase 26), whose single-instance is delegated to k8s/etcd, so nothing in this phase
...
- [`pulsar_client_doctrine.md §5`](../documents/engineering/pulsar_client_doctrine.md#5-the-capability-surface-lookup--produce--consume--subscribe--seek)
  ...
  — *the capability surface (the Failover subscription)* / *at-least-once with broker-side dedup*: the Phase-28
...
- [`chaos_failover_doctrine.md §12`](../documents/engineering/chaos_failover_doctrine.md#12-the-moral-core--proven-tested-assumed)
  ...
  **cross-cluster** failover boundary and its formal model are owned by
  ...
  and realized by Phase 32's geo-replication plus Phase 33's gateway-migration drills, not here — this phase
  exercises the intra-cluster subscription only.

## Sprints

## Sprint 29.1: Three-tier content-addressed MinIO store 📋
...
**Blocked by**: Phase 19 gate (the modeled typed Job-terminal protocol), Phase 23 gate (MinIO reachable as a standard HA platform service) and Phase 21 gate (the
`no-provisioner` retained PV the MinIO bytes land on); Phase 27 gate (the `<app>/<bucket>` ObjectStore the
...
(the standing Phase-23 HA service on the Phase-21 retained PV), never an in-process or local S3 fake — the
...
- A mandatory `ObjectStoreDemand` per namespace: exact physical-id-keyed committed residents, structural
  ...
  future/transient extent peak, merges it with all Phase-23 producer arms, and feeds that structure—not a byte
...
- The collector/verification Job consumes the Phase-19 terminal state machine live for the first time. Its
...
- Store keys taken under a caller-supplied `experiment-hash` namespace string within the app's ObjectStore
  ...
  seed derivation (Phase 38 kernel work).
...
## Sprint 29.2: Orchestrator/worker workflow runtime + store/fetch by manifest SHA 📋
...
**Blocked by**: Sprint 29.1 (the content store the worker writes and the orchestrator fetches); Phase 28 gate
(the native Pulsar client — the capability surface, CBOR codec, and broker-side dedup); Phase 26 gate (the live
...
Phase-28 topology algebra and consumes the corresponding `event`; the active worker writes a content-addressed
...
### Deliverables
- An orchestrator daemon that, using the Phase-28 topology algebra, produces a workflow `command` on the
...
2. Assert a retried produce and a redelivered consume are collapsed by the Phase-28 broker-side dedup so
...
## Sprint 29.3: Pulsar Failover standby takeover + leak-free teardown (gate) 📋
...
**Blocked by**: Sprint 29.2 (the orchestrator/worker round-trip the injection acts on); Phase 28 gate (the
`Exclusive`/`Failover` subscription type and the at-least-once redelivery); Phase 21 / Phase 17 gates (the
...
### Deliverables
- Worker daemons attached over a Pulsar **Failover** subscription (Phase 28): one active, the rest
  name-ordered hot standbys; single-writer liveness is the subscription, safety is the store's ETag-CAS commit
  point plus the typed `AdvancePredicate` (Sprint 29.1), so even a bounded failover overlap cannot regress
  HEAD.
...
- The **postflight sweep's explicit inventory contract**: the sweep MUST inventory, and the ledger MUST record,
  ...
  (the durable test-flagged bytes reclaimed by Phase 42). The sweep emits its **full inventory list and the
  named retained set** into the per-run ledger; **any non-empty remainder outside the retained set is a hard
  gate failure**. (Durable-byte reclaim staying with Phase 42 is the *only* exemption, and only for the
...
2. **Idempotency and leak-free teardown, disambiguated.** "**Idempotent setup**" means a *second `apply` of the
   topology against the still-standing topology is a byte-stable no-op* (the Phase-19 sense — zero fields
...
> and realized by Phase 32's geo-replication plus Phase 33's gateway-migration drills, not here. Pulsar's own
...
> reclaim of test-flagged durable bytes is the elevated harness's prerogative (Phase 42), kept out of the
> normal teardown path.

### Remaining Work
The whole sprint (📋 Planned).

## Sprint 29.4: Register-2.5 workflow failover takeover under simulated fault 📋
...
Failover-takeover + leak-free-teardown behaviour whose properties are replayed here); Phase 15 Sprint 15.2 (the
...
> only by this phase's **Register-3 live gate** (Sprint 29.3) on the linux-cpu kind cluster — the deterministic
...
- `documents/engineering/content_addressing_doctrine.md` — record that §2 (the three-tier store + the two write
  ...
  `experimentHash` derivation and seed kernel explicitly deferred to Phase 38; note §5 confluence is consumed
...
- `documents/engineering/resource_capacity_doctrine.md` — record the content-store logical peak boundary
  corpus and its consumption of Phase 23's MinIO physical/uniform-claim witness.
- `documents/engineering/storage_lifecycle_doctrine.md` — record that the store's blob/manifest/pointer bytes
  land on the Phase-21 retained PV under the standing Phase-23 MinIO service, and that CAS-loser orphans stay
...
- `documents/engineering/chaos_failover_doctrine.md` — record the §12 per-run proven/tested/assumed ledger for
  the intra-cluster failover injection, and that the §16 cross-cluster boundary stays deferred to Phase 32
  geo-replication plus Phase 33 gateway-migration drills.
...
- [Native Pulsar Client Doctrine](../documents/engineering/pulsar_client_doctrine.md) — the Phase-28 capability
...
- [phase_28](phase_28_pulsar_client.md) — the native Pulsar client this workflow runtime is built on
- [phase_38](phase_38_determinism_jitcache.md) — the `experimentHash` derivation + SplitMix seed kernel deferred
