# Phase 45: Atomic immutable UI-program release
...
- [Sprints](#sprints)
- [Sprint 45.1: Release immutable UI plans without rebuilding the runtime ⏸️](#sprint-451-release-immutable-ui-plans-without-rebuilding-the-runtime-)
...
Done (invalidated). Two fresh live releases over the unchanged Phase-30 private runtime image published atomic
...
Phase-44 release ledger. The server-plan object contains dispatch/policy/handler identities and codecs, not
...
- **Observer outside the SUT.** An elevated observer provisioned in this phase — not the shared
  `src/Amoebius/Test/{Harness,Sweep}.hs` that [Phase 56](phase_56_test_topology_dsl.md) later consolidates —
  reads Envoy access records, the Phase-44 release-ledger
...
- **Single generic image.** The independent containerd/registry observer must see one unchanged **amoebius
  ...
  [Phase 30](phase_30_base_image_registry.md) bakes and publishes, carrying the compiled PureScript client
...
## Sprint 45.1: Release immutable UI plans without rebuilding the runtime ⏸️
...
- [Phase 44 — release lifecycle](phase_44_release_lifecycle.md)
