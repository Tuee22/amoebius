# Phase 76: Gateway-migration drills + model-correspondence
...
> emergency takeover (bounded rebind) against the future [Phase 75](phase_75_multicluster_spawn_georepl.md)
> forest, then compare its trace with the Phase-18 model. NOT VALIDATED.
> **Read this if**: phase 76 is next in the queue, or a later phase depends on its future gate.
...
- [Sprints](#sprints)
- [Sprint 76.1: The gateway-migration runtime — both branches over the Phase-18 `interpret` core ⏸️](#sprint-761-the-gateway-migration-runtime--both-branches-over-the-phase-18-interpret-core-)
- [Sprint 76.2: Teardown-with-cleanup vs chaos-failover + unsatisfiable-spec push-back ⏸️](#sprint-762-teardown-with-cleanup-vs-chaos-failover--unsatisfiable-spec-push-back-)
- [Sprint 76.3: Register-2.5 gateway-migration runtime fidelity — simulation + trace validation ⏸️](#sprint-763-register-25-gateway-migration-runtime-fidelity--simulation--trace-validation-)
- [Sprint 76.4: Register-3 correspondence — Inject drills against the running forest + lazy live-gate projection + ledger ⏸️](#sprint-764-register-3-correspondence--inject-drills-against-the-running-forest--lazy-live-gate-projection--ledger-)
...
**Target capability — NOT VALIDATED.** After Phase 75's future approval, this phase must exercise the authority
...
against modeled boundaries and its transition log must satisfy the independently reviewed Phase-18 `Model`
...
This phase consumes earlier phases and does not re-implement them: Phase 75's geo-replicated forest and
invariant-confluence classifier, Phase 18's `GatewayMigration` `Model` + `interpret` + decode-time
structural-fit fold, Phase 65's Keycloak-owned wild ingress, Phase 74's WireGuard fabric (whose hub role the
`Planned` handover repoints), and Phase 17's (Sprints 28.1/15.2) `io-classes` seams and modeled route53/Pulsar. A
...
**Substrate:** linux-cpu — the future gate drives the migration over the parent and both child clusters that Phase 75
must spin up as `kind` clusters on a single linux-cpu host; no accelerator and no provider cluster is in
scope (provider-managed clusters are [Phase 77](phase_77_provider_deploy_checkpoint.md)). Partition tolerance is
...
**Depends on:** [Phase 75](phase_75_multicluster_spawn_georepl.md)
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 75; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
The runtime executes inside the Phase-66 control-plane daemon, whose complete pod envelope is expanded for migration-plan
...
clusters' Phase-65 edge controllers, operator-derived data-plane children, admission webhooks/gateways, and
rollout operands; the source transparent proxy and target serving path coexist through the full DNS drain
window. The Phase-74 `NetworkFabricSystemDemand` is derived for the source, overlap, and target peer/hub
graphs, including kernel/listener CPU and memory, queues, and rotated nodefs logs. The route53 repoint retains
the Phase-75 in-cluster `PulumiExecutionDemand`, exact executor-Job envelopes, plugin/workspace volumes,
...
The non-idle drill workload is provisioned too. Its source-equal Phase-68/49 client/workflow projection carries
...
- [`gateway_migration_doctrine.md` §2 — The `Planned` branch — a coordinated strong-consistency handover](../documents/engineering/gateway_migration_doctrine.md#2-the-planned-branch--a-coordinated-strong-consistency-handover)
  ...
  to be built as the effectful shell around the Phase-18 decision core, honest per
...
- [`deterministic_simulation_doctrine.md` §4 — Register 2.5 — where deterministic simulation sits](../documents/engineering/deterministic_simulation_doctrine.md#4-register-25--where-deterministic-simulation-sits)
  ...
  route53/Pulsar, trace-validated against the Phase-18 emitted spec's `Next` relation before the Register-3 live
...
## Sprint 76.1: The gateway-migration runtime — both branches over the Phase-18 `interpret` core ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 75](phase_75_multicluster_spawn_georepl.md) human approval
...
build the effectful gateway-migration shell for **both** branches whose every branch decision is the Phase-18
...
1. A `Planned` handover under a **non-idle** workflow (the drill client journals every source-acked write ID to
   ...
   **measured loss = 0 proven by set-equality of journaled-vs-present IDs on the new owner** (not by a
   ...
   `.build/test-corpora/dhall/phase_66_gateway_migration.dhall`
...
## Sprint 76.2: Teardown-with-cleanup vs chaos-failover + unsatisfiable-spec push-back ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 76.1
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
## Sprint 76.3: Register-2.5 gateway-migration runtime fidelity — simulation + trace validation ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 76.2
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
simulation against the modeled world here (Register 2.5), then as live Inject drills (Sprint 76.4, Register 3)
...
- A Register-2.5 proven/tested/assumed ledger — the built forest upholds the safety invariants and refines the
  ...
  replication-lag / clock-skew physics remain the Register-3 residue (Sprint 76.4).
...
## Sprint 76.4: Register-3 correspondence — Inject drills against the running forest + lazy live-gate projection + ledger ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 76.3
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
- The Register-3 Inject drill set run in the inter-cluster dimension against the running forest — cut
  ...
  past the bound, and fail back with late plus duplicate arrivals — each asserting the Phase-18 named invariants.
...
- A reviewed Haskell gate-topology declaration that lazily renders
  `.build/test-corpora/dhall/phase_66_gateway_migration.dhall`: spin two children up, geo-replicate, run a `Planned` handover
  ...
  so they are **reviewed at the start of Phase 76, before that implementation is trusted** (the §M.1
  start-of-owning-phase form for oracles that depend on later code).
- A machine-derived per-run proven/tested/assumed ledger that marks `Planned` RPO=0 **proven-for-the-model (Phase 18, scope 2) + drilled (tested)**, the `Failover` recovery time + reconciliation **tested (drilled)**, the data-loss /
  replication-lag bound **assumed (monitored, never proven)**, and the modeled safety/liveness
  **proven-for-the-model at scope 2** (Phase-18, a design-layer result) — and never reports an
  assumed-and-monitored result as proven.

### Validation

1. The built decision core resolves to the Phase-18 `interpret` — the correspondence-check mechanic is fixed as:
   the step-by-step trace-validation of Sprint 76.3 (Register 2.5) **plus** a Register-3 **modeled-action-coverage assertion** that every modeled action fired ≥ 1 time across the drill set (no orphaned modeled action); step-by-step
   ...
   lazily projected to `.build/test-corpora/dhall/phase_66_gateway_migration.dhall`,
...
- `documents/engineering/gateway_migration_doctrine.md` — backlink §2/§3/§4/§5 to the built
  `src/Amoebius/Multicluster/*` runtime; confirm `Planned` RPO=0 stayed proven-for-the-model (Phase 18, scope 2; now drilled live) and
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-76 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/system_components.md` — register the `src/Amoebius/Multicluster/GatewayMigration.hs`,
  ...
  modules and the Register-2.5 simulation + Register-3 Inject + gate suites as Phase-76 rows.
- `DEVELOPMENT_PLAN/substrates.md` — the Phase-76 → linux-cpu row in the per-phase substrate map.

## Related Documents

- [README.md](README.md) — the live tracker; Phase 76 objective, gate, and substrate
...
- [Deterministic Simulation Doctrine](../documents/engineering/deterministic_simulation_doctrine.md) — the Register-2.5 io-sim + trace-validation (Sprint 76.3) that pulls the runtime-fidelity obligation forward from Register-3-only chaos
...
- [phase_18](phase_18_gateway_migration_model.md) — the `GatewayMigration` design-model whose Register-3 correspondence against the built forest is discharged here
- [phase_75](phase_75_multicluster_spawn_georepl.md) — the prior phase; the geo-replicated forest and confluence classifier this phase runs over
- [phase_77](phase_77_provider_deploy_checkpoint.md) — the next phase; the forest extended to provider-managed clusters
