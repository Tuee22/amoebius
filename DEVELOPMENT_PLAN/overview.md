...
| **Resource demand never exceeds capacity** — a workload / VM / compute-engine over-committing its host or cluster is decode-rejected (a total fold over declared per-host `Capacity`; honestly grade-2). | [`resource_capacity_doctrine.md`](../documents/engineering/resource_capacity_doctrine.md); catalog [`§3.17`](../documents/engineering/illegal_state_catalog.md#3-the-catalog--states-a-valid-spec-cannot-represent) |
