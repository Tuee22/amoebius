# Phase 55: Test-topology DSL + suggest-test + elevated harness
...
- [Sprints](#sprints)
- [Sprint 55.1: The test-topology type — a deployment-rules layer that always tears down ⏸️](#sprint-551-the-test-topology-type--a-deployment-rules-layer-that-always-tears-down-)
- [Sprint 55.2: `suggest-test` — detect substrate + credential authority, emit a representative test `.dhall` ⏸️](#sprint-552-suggest-test--detect-substrate--credential-authority-emit-a-representative-test-dhall-)
- [Sprint 55.3: Flagged test credentials + test-owned resource tagging ⏸️](#sprint-553-flagged-test-credentials--test-owned-resource-tagging-)
- [Sprint 55.4: The elevated harness as sole automated deleter of test-owned storage + leak-free sweep ⏸️](#sprint-554-the-elevated-harness-as-sole-automated-deleter-of-test-owned-storage--leak-free-sweep-)
- [Sprint 55.5: The per-run ledger artifact + the delegated-failover gate topology ⏸️](#sprint-555-the-per-run-ledger-artifact--the-delegated-failover-gate-topology-)
...
(Phase 31) on retained storage (Phase 29). The mechanisms generalize patterns *proven in the sibling prodbox
...
Pulsar `Exclusive`/`Failover` subscription of Phase 38, while the control-plane singleton it deploys under is
...
the `replicas=1` singleton (Phase 34), the native Pulsar client and Pulsar-`Failover` worker takeover
(Phases 36–38), the retained `no-provisioner` PV model (Phase 29), substrate detection (Phase 24),
Vault secret-by-name injection (Phase 30), and the leak-free provider teardown (Phase 48).
...
1. **Leak-freedom by implementation-independent inventory diff, not tag query.** Leak-freedom is asserted by a
   ...
   parent-drives-provider form over Phase 48's leak-free provider teardown, still one substrate per generated
...
7. **Failover takeover pinned to an external broker-stats observer, not the `ExpectationWitness`.** The
   ...
   statistics (`pulsar-admin topics stats`/`topics partitioned-stats` on the inherited Phase-38 command topic)
...
the `linux-cpu` single-node `kind` cluster, at minimum: the pre-standing Phase-31 HA Pulsar and MinIO on
retained Phase-29 storage; **at least one newly-allocated retained `no-provisioner` PV** (so the sole-deleter
rule of Sprint 55.4 is exercised against a resource this phase created); **one control-plane workload as a Deployment `replicas=1`**; and **at least two Pulsar `Failover`-subscription consumer pods** (an active worker
...
flowchart LR
  ...
  s0["Sprint 55.1: The test-topology type — a deployment-rules layer that…"]
  s1["Sprint 55.2: suggest-test — detect substrate + credential authority…"]
  s2["Sprint 55.3: Flagged test credentials + test-owned resource tagging"]
  s3["Sprint 55.4: The elevated harness as sole automated deleter of…"]
  s4["Sprint 55.5: The per-run ledger artifact + the delegated-failover gate…"]
...
```
*Orientation. The seams phase 55 builds, in order; [Gate integrity](#gate-integrity) owns the apparatus. Not run.*
...
**Register binding for all sprint validations.** Every Sprint 55.1–47.5 Independent Validation and Validation
...
in the ledger; the leak-free teardown, the live identity-boundary denial (Sprint 55.4), and the failover
...
The topology inherits and source-checks Phase-38's exact command/event topic, Failover subscription,
...
- [`testing_doctrine.md §3`](../documents/engineering/testing_doctrine.md#3-the-test-topology-contract-spin-up--run--always-tear-down)
  — *the test-topology contract: spin up → run → always tear down*: Sprint 55.1 realizes the four-clause
...
- [`testing_doctrine.md §5`](../documents/engineering/testing_doctrine.md#5-suggest-test-detect-the-world-emit-a-representative-test-dhall)
  — *`suggest-test`: detect the world, emit a representative test `.dhall`*: Sprint 55.2 builds the generator
...
- [`testing_doctrine.md §6`](../documents/engineering/testing_doctrine.md#6-flagged-test-credentials)
  — *flagged test credentials*: Sprint 55.3 establishes the distinct flagged test-simulation identity (the
...
- [`testing_doctrine.md §7`](../documents/engineering/testing_doctrine.md#7-the-elevated-harness-is-the-sole-automated-deleter-of-test-owned-durable-storage-leak-free-cycles)
  ...
  Sprint 55.4 implements one-deleter/one-credential, flag-then-sweep, and the independent postflight-inventory
...
- [`testing_doctrine.md §4`](../documents/engineering/testing_doctrine.md#4-no-skips-fail-fast-and-the-per-run-ledger-artifact)
  — *no skips, fail fast, and the per-run ledger artifact*: Sprint 55.5 emits the per-run
...
## Sprint 55.1: The test-topology type — a deployment-rules layer that always tears down ⏸️
...
## Sprint 55.2: `suggest-test` — detect substrate + credential authority, emit a representative test `.dhall` ⏸️
...
2. Property tests perturb each supply axis independently.
   ...
   - Reject-branch coverage — that the structured "no representative topology fits" result actually fires —
     ...
     mutants (the pinned `phase_55_resource_overcommit_*`/`phase_55_missing_capability` and `drop_*`
...
3. Exercise the closed optional-branch fixture matrix over its four fixed-input shapes — all-three-negative,
   ...
   `phase_55_optional_resource_shapes.json`. The committed
   `drop_registry_publication_envelope.dhall`, `drop_pulumi_executor_envelope.dhall`,
   `drop_pulumi_checkpoint_demand.dhall`, `drop_migration_copy_envelope.dhall`, and
...
## Sprint 55.3: Flagged test credentials + test-owned resource tagging ⏸️
...
*representability* boundary only; the *live* identity-boundary denial is Sprint 55.4's Register-3 obligation.
...
- The flagged credential resolved by name only through Vault (Phase 30) — flagging changes *which* credential
...
## Sprint 55.4: The elevated harness as sole automated deleter of test-owned storage + leak-free sweep ⏸️
...
### Deliverables
- A delete path reachable only by the elevated harness under the flagged credential of Sprint 55.3, scoped to
...
- An explicit scope boundary: this harness reclaims **test-owned** backing only. Production
  ...
  Phase 55 neither holds nor tests authority over production backing.
...
3. Leak detection uses the Gate-criterion-1 inventory diff, not a test-owned tag query: the committed seeded
   ...
   (Phase 45), keeping this single-substrate `linux-cpu` run's own scope intact ([§L](development_plan_standards.md#l-one-substrate-discipline)).
...
## Sprint 55.5: The per-run ledger artifact + the delegated-failover gate topology ⏸️
...
injects is delegated to Pulsar (Phase 38), never a bespoke amoebius election.
...
5. Independently make the generator/harness, controller, active, standby, replacement, selected images,
   ...
   structural-takeover-defeat `phase_55_failover_standby_wrong_subscription.dhall` (caught by the external
...
> and delivered by Phase 44's gateway-migration drills, not here. The delegated-failover shape is proven in the sibling `infernix`
...
- `documents/engineering/app_vs_deployment_doctrine.md` — §3's deployment-rules surface gains the realized
  test-topology layer (Sprint 55.1): the `TestTopology` is an ordinary deployment-rules layer over a production
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-55 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/system_components.md` — add the `Amoebius/Test/*` modules (`Topology`, `Runner`,
  `SuggestTest`, `Credentials`, `Harness`, `Sweep`, `Ledger`) to the component inventory as Phase-55
  design-first rows, mapped to the owning testing and storage-lifecycle doctrines.
- `DEVELOPMENT_PLAN/substrates.md` — record the Phase 55 "per generated test" substrate posture (each generated
  test substrate-locked; the canonical gate run on `linux-cpu`) in the per-phase substrate map.

## Related Documents
- [README.md](README.md) — the live tracker; Phase 55 objective, gate, and substrate
...
- [phase_38](phase_38_content_store_workflow.md) — the Pulsar-`Failover` worker takeover this phase's gate injects
- [phase_44](phase_44_gateway_migration_drills.md) — the cross-cluster gateway-migration obligation, distinct
  from this phase's intra-cluster failover
- [phase_48](phase_48_provider_dynamic_nodes.md) — the leak-free provider teardown this harness extends to test cycles
