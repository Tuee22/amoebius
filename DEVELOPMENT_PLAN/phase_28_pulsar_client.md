# Phase 28: Native Pulsar client (CBOR)

**Status**: Authoritative source
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/legacy_tracking_for_deletion.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_23_platform_backbone.md, DEVELOPMENT_PLAN/phase_27_app_tenancy.md, DEVELOPMENT_PLAN/phase_29_content_store_workflow.md, DEVELOPMENT_PLAN/system_components.md
...
statement is design intent, never a tested amoebius result. This phase opens after the Phase 27 gate (app
tenancy + `TenantSpec`) and runs on the **linux-cpu** substrate in **Register 3** — live infrastructure: a
single-node `kind` cluster with Pulsar up as a standard HA service (Phase 23). The mechanisms are generalized
...
orchestrator/worker workflow runtime with Pulsar-Failover single-writer takeover (both Phase 29), the topic
...
**Gate:** on a `linux-cpu` kind cluster with Pulsar up as a standard HA service (Phase 23), an `InForceSpec`
...
failure/orphan horizon, and admission model); Phase 23 owns the live MinIO geometry/drill.
...
- **Committed oracle corpus (representative set, §M.7, §M.1).** The gate's representative set is named
  ...
  hand-computed canonical-CBOR byte strings; (c) the non-CBOR negative fixtures of Sprint 28.2 V4 with their
  ...
  strings), authored independently of `topicFor` (§M.3); (e) a pre-run snapshot of the standing Phase-23
...
- **Leak-free is an external enumerate-and-compare sweep (§M.5).** "Leak-free" is defined as: after teardown,
  ...
  bookkeeping) and asserts the set is empty, and asserts the standing Phase-23 namespace's policy set
...
orchestrator/worker. The Phase-28 live gate therefore declares a client-runner Pod with immutable image and OCI
...
bytes, concurrent/rate-window offloads, deletion lag, failed/orphan horizon, mutation admission). The Phase-23
...
## Sprint 28.1: Fork supernova → amoebius-pulsar native binary protocol 📋
...
**Blocked by**: Phase 23 — Pulsar reachable as a standard HA service (external earlier-phase prerequisite);
Phase 18 — the baked Pulsar/service binaries in the in-cluster `distribution` registry.
...
- A structural `PulsarClientExecutionDemand` for connection/request/frame/reconnect peaks, bound into the
  complete envelope of the Phase-28 client-runner Pod and, later, each actual consumer Pod; there is no
...
## Sprint 28.2: Capability surface + exclusively-CBOR payload codec 📋
...
seek-based replay — over the persistent session from Sprint 28.1, with **every application payload encoded
...
- A typed **CBOR payload codec** (`Amoebius.Pulsar.Cbor`, on `serialise`/`cborg`): `produce`/`consume` accept
  ...
  CBOR (shared with the store's canonical encoder, Phase 29) is used where the payload is content-addressed; a
...
## Sprint 28.3: Declarative topology algebra + one-sided-link validation 📋
...
4. Prove the algebra is on the gate path, not dead code (§M.3): the gate topology `round_trip_dedup.dhall`
   carries a committed `RouteEntry` descriptor, and the Sprint 28.4 gate run asserts the actually-produced and
...
## Sprint 28.4: At-least-once + broker-side dedup + the command→event round-trip gate 📋
...
`test/integration/Phase24RuntimeStorage.hs` (runner planned-slot→observed-Pod-UID join, component-role/layout
backings, node scope/domain/ownership equality, reservation/observed no-double-debit, alias controls) — target paths, not yet built.
**Blocked by**: Sprint 28.2, Sprint 28.3; Phase 26 — the live DSL deploy via the `replicas=1` singleton (the
...
3. **Idempotency (§M.6) — re-apply, not re-run-from-clean.** Re-apply the topology a second time against a
   **distinct test namespace** (cache-bypass: no reuse of run 1's namespace, cursors, producer name, or dedup
   ...
   tenant contains **zero** topics, subscriptions/cursors, and namespaces, and asserts the standing Phase-23
   ...
   ledger — the deferred content-store, workflow-runtime, and cross-cluster surfaces (Phase 29 and later)
...
## Sprint 28.5: Register-2.5 exactly-once effect under simulated redelivery 📋

**Status**: Planned
**Implementation**: `test/sim/PulsarDedupSimSpec.hs` (the `IOSimPOR` battery over the modeled Pulsar), driving
the real `amoebius-pulsar/src/Amoebius/Pulsar/Dedup.hs` consumer-side fold lifted onto the Phase-15
`io-classes` `Env` interface — target paths, not yet built.
**Blocked by**: Sprint 28.4 (the built at-least-once + dedup client); the Phase 15 gate (the `io-classes`
...
- A Register-2.5 proven/tested/assumed ledger — the fold upholds exactly-once under the modeled schedules and
  faults; honest limit: modeled-Pulsar fidelity is **assumed**, discharged by the Sprint 28.4 Register-3 live
  gate.
...
- `DEVELOPMENT_PLAN/system_components.md` — register the `amoebius-pulsar` package and its target module paths
  ...
  `Topology`, `Dedup`, `Namespace`) as Phase-28 design-first rows against the component inventory.
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-28 row's status from this plan once the gate passes; link this
  document.
...
- [Daemon Topology Doctrine](../documents/engineering/daemon_topology_doctrine.md) — who runs producers/
  consumers and picks subscription roles (Phase 29), delegated here
- [phase_23](phase_23_platform_backbone.md) — the standard-service stack that brings Pulsar up HA
- [phase_27](phase_27_app_tenancy.md) — the app tenancy this phase opens after
- [phase_29](phase_29_content_store_workflow.md) — the content store + workflow runtime that consumes this
  client
