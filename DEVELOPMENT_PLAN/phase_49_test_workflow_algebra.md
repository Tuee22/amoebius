# Phase 49: The test-workflow algebra
...
**Supersedes**: N/A
**Referenced by**: DEVELOPMENT_PLAN/README.md, DEVELOPMENT_PLAN/overview.md, DEVELOPMENT_PLAN/phase_50_self_referential_gates.md, DEVELOPMENT_PLAN/phase_80_provider_dynamic_nodes.md
...
- [Sprints](#sprints)
- [Sprint 49.1: The test-topology type — a deployment-rules layer that always tears down ⏸️](#sprint-491-the-test-topology-type--a-deployment-rules-layer-that-always-tears-down-)
- [Sprint 49.2: Pure `suggestTest` over supplied models and lazy proposal projection ⏸️](#sprint-492-pure-suggesttest-over-supplied-models-and-lazy-proposal-projection-)
- [Sprint 49.3: Flagged-authority and test-owned tagging vocabulary ⏸️](#sprint-493-flagged-authority-and-test-owned-tagging-vocabulary-)
- [Sprint 49.4: Phase-91 transfer for destructive cleanup and leak observation ⏸️](#sprint-494-phase-91-transfer-for-destructive-cleanup-and-leak-observation-)
- [Sprint 49.5: Pure evidence algebra and Phase-91 failover transfer ⏸️](#sprint-495-pure-evidence-algebra-and-phase-91-failover-transfer-)
...
Blocked by redesigned Phase 48, its independent validation, and human promotion; every earlier
...
**Target capability — NOT VALIDATED.** Phase 49 owns only a pure Haskell `TestTopology` algebra. Its
...
Phase 49 performs no substrate detection, credential probe, resource creation, fault injection, teardown,
inventory readback, browser action, hardware observation, or cluster validation. Those effectful and
destructive responsibilities belong to Phase 91 after the Phase-50 hardware-free DSL promotion barrier and
...
**Lane:** `none` — live per-test lanes belong to Phase 91.

**Register:** 1 target only; live execution, cleanup, and evidence remain UNVERIFIED.
**Depends on:** [Phase 48](phase_48_tool_and_mutant_generation.md)
...
| Key | Contract |
|---|---|
| `Claim` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Target only — a pure Haskell test-topology value carries a typed teardown continuation, and pure Haskell `suggestTest` maps supplied model values to a proposal or structured refusal. Generated external forms remain beneath `.build/**`; live execution, deletion, and observation belong to Phase 91. Explicit exclusions: every layer named in `Residue` remains UNVERIFIED. |
| `Subject` | UNRESOLVED — blocks validation: no production `.hs` module and entry point have been independently established for this reset contract. |
| `Command` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: `pb validate phase 49` is future public spelling only. Before current human approval of Phase 51, `pb` is inadmissible validation transport; the candidate must invoke the exact absolute source-bound Haskell executable directly from an authenticated, network-independent toolchain input. The Haskell verdict entry point remains `UNRESOLVED` and blocks validation. |
...
| `Predecessor` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: Exact external `ImmediatePredecessorApproval` for Phase 48; candidate execution separately refuses an absent, stale, replayed, or locally shaped receipt. |
...
- [`testing_doctrine.md` §1 — A test is an amoebius spec](../documents/engineering/testing_doctrine.md#1-a-test-is-an-amoebius-spec) and
  ...
  — Phase 49 owns the Haskell algebra and typed teardown obligation only; Phase 91 owns spin-up, run, teardown,
...
- [`testing_doctrine.md` §5 — `suggest-test`: detect the world, emit a representative test `.dhall`](../documents/engineering/testing_doctrine.md#5-suggest-test-detect-the-world-emit-a-representative-test-dhall)
  ...
  Phase-91 work; any Dhall proposal is generated under `.build/test-corpora/**` for human review.
...
- [`resource_capacity_doctrine.md` §4 — The total fold: `fits`, `carve`, `place`, and the nesting](../documents/engineering/resource_capacity_doctrine.md#4-the-total-fold-fits-carve-place-and-the-nesting)
  — Phase 49 may compose pure capacity terms over supplied model values; it cannot establish live fit.
- [`testing_doctrine.md` §8 — One substrate per validation](../documents/engineering/testing_doctrine.md#8-one-substrate-per-validation)
  — Phase 49 has no substrate. Phase 91 must name and observe exactly one live substrate when it eventually
  seeks promotion.

## Sprints

Every sprint below is hardware-free. Its subject, cases, expectations, and changed-production-subject mutants
are Haskell; any serialized projection is generated lazily beneath `.build/test-corpora/**`. Phase 49 has no
effect interpreter and cannot establish that any modeled host, authority, inventory, teardown, or failover
event occurred. Phase 91 owns those live obligations after all numerical predecessors are human-approved.

## Sprint 49.1: The test-topology type — a deployment-rules layer that always tears down ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: exact authored Haskell implementation paths have not been bound to this sprint.
**Blocked by**: [Phase 48](phase_48_tool_and_mutant_generation.md) human approval
...
teardown outcome has been supplied to the pure transition fold. Phase 49 does not spin up, interrupt, or tear
down anything.
...
This sprint remains blocked and NOT VALIDATED. Phase 91 owns workflow execution, interruption handling,
resource reclamation, and external confirmation that teardown occurred.

## Sprint 49.2: Pure `suggestTest` over supplied models and lazy proposal projection ⏸️
...
define `suggestTest` as a total, pure proposal function over explicitly supplied Haskell model values. Phase 49
...
- A symbolic delegated-failover `FaultIntent` and named authority references only. Phase 49 neither resolves
...
This sprint remains blocked and NOT VALIDATED. Phase 91 owns substrate and hardware discovery, inventory and
quota readback, credential probing, provider access, allocation, and comparison of a proposal with the live
world.

## Sprint 49.3: Flagged-authority and test-owned tagging vocabulary ⏸️
...
- Symbolic authority references by name only, plus an explicit `AuthorityUnverified` state that Phase 49 cannot
  promote. Secret resolution, permission checks, and application of ownership metadata belong to Phase 91.
...
4. A production mutant that adds a delete operation to the Phase-49 topology language turns the gate red. This
   is a language-boundary check only; Phase 49 performs no deletion or permission probe.

### Remaining Work

This sprint remains blocked and NOT VALIDATED. Phase 91 owns acquiring and resolving credentials, verifying
their external permissions, applying test-owned metadata during allocation, and observing that metadata at the
live boundary.

## Sprint 49.4: Phase-91 transfer for destructive cleanup and leak observation ⏸️
...
keep destructive cleanup and leak observation outside Phase 49 while defining the pure teardown-result and
modeled-inventory-difference vocabulary that Phase 91 must interpret. The Phase-49 DSL exposes no deletion
...
- No delete path, resource selector, credential operation, API command, provider operation, host path, or live
  inventory collector. Phase 91 owns their implementation and independent observation.
...
This sprint remains blocked and NOT VALIDATED. Phase 91 owns allocation tracking, credentials, destructive
...
## Sprint 49.5: Pure evidence algebra and Phase-91 failover transfer ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: exact authored Haskell implementation paths have not been bound to this sprint.
**Blocked by**: Sprint 49.4
**Independent Validation**: UNRESOLVED — blocks validation: independent positive, paired-negative, changed-subject mutant, and residue observations have not been bound to this sprint.
**Oracle**: UNRESOLVED — blocks validation: a separately authored Haskell oracle, its provenance, and its human reviewer have not been bound to this sprint.
**Legacy IDs**: UNRESOLVED — blocks validation: this sprint has not been joined to exact typed Haskell legacy-inventory IDs.
**Docs to update**: UNRESOLVED — blocks validation: the governed documentation owners and exact update set have not been reviewed for this sprint.
...
Phase 49 can validate only derivation over supplied Haskell declarations. It cannot award Runtime strength,
...
- Separate evidence constructors for pure proof/model results and future authenticated Runtime observations.
  The Phase-49 evaluator cannot construct or promote the latter; applicable runtime moves remain UNVERIFIED.
...
- An explicit Phase-91 handoff contract for live preflight, allocation, credential and tool checks, topology
...
2. Paired pure cases differing only by one declared fault intent produce the exact additional applicable move;
   a declared-but-unperformed Runtime move remains UNVERIFIED. No Phase-49 case can classify it as tested.
...
4. A Haskell case with a missing modeled prerequisite returns the exact refusal rather than a skip. This does
   not validate any live prerequisite check; Phase 91 must probe and observe the real boundary.
...
> **Honesty.** Phase 49 validates no intra-cluster or cross-cluster failover. It represents delegated-failover
> intent and evidence strengths as pure Haskell values only. Phase 91 owns live test-topology execution and
...
This sprint remains blocked and NOT VALIDATED. Phase 91 owns authenticated Runtime evidence, live topology and
...
- `documents/engineering/testing_doctrine.md` — record the pure teardown, suggestion, credential, and evidence
  module and leave generated/live topology, harness, sweep, and per-run evidence explicitly to Phase 91.
- `documents/engineering/app_vs_deployment_doctrine.md` — retain the deployment-rules target while recording
  that Phase 49 validates only its pure workflow value.
- `documents/engineering/resource_capacity_doctrine.md` — retain the full live provision target while
  distinguishing Phase 49's nine-axis pure suggestion projection from Phase 91's allocation/readback proof.
- `documents/engineering/storage_lifecycle_doctrine.md` — leave §7.1's automated test-reclaim owner UNVERIFIED
  until Phase 91; Phase 49 owns no delete authority.
- `documents/engineering/pulumi_iac_doctrine.md` — record only Phase 49's pure flagged-authority and ownership
  vocabulary. Leave credential resolution, permission checks, allocation metadata, and destroy authority
  UNVERIFIED for Phase 91.
- `documents/engineering/chaos_failover_doctrine.md` — record Phase 49's pure ledger grammar and explicit
  inability to award Runtime strength. Leave live fault injection and failover observation to Phase 91; the
  cross-cluster gateway-migration obligation stays in Phase 76.

**Cross-references to add:**

- `DEVELOPMENT_PLAN/README.md` — flip the Phase-49 status when the gate passes; link this document.
- `DEVELOPMENT_PLAN/system_components.md` — record `lib:test-workflow-algebra-core` as Phase 49's Decision-layer
  component and keep live `Amoebius.Test` module-namespace ownership at Phase 91.
- `DEVELOPMENT_PLAN/substrates.md` — retain Phase 49 at `none`/`none`; Phase 91 owns the generated test's live substrate.

## Related Documents

- [README.md](README.md) — the live tracker; Phase 49 objective, gate, and substrate
...
- [Testing Doctrine](../documents/engineering/testing_doctrine.md) — the test-as-a-topology contract,
  whose pure topology, suggestion, authority-intent, teardown-result, and ledger vocabulary Phase 49 defines;
  Phase 91 owns the live harness
- [Storage Lifecycle Doctrine](../documents/engineering/storage_lifecycle_doctrine.md) — the retained PV model,
  the no-normal-operation-deletion rule, and the elevated-harness exception that remains deferred to Phase 91
- [Chaos / Failover Doctrine](../documents/engineering/chaos_failover_doctrine.md) — the proven/tested/assumed
  ledger grammar Phase 49 models without performing either fault injection or failover observation
...
- [Application Logic vs Deployment Rules](../documents/engineering/app_vs_deployment_doctrine.md) — the
  deployment-rules surface on which Phase 49 represents fault intent as data for a later interpreter
- [Vault / PKI Doctrine](../documents/engineering/vault_pki_doctrine.md) — the `SecretRef`-by-name contract the
  pure authority reference obeys; Phase 49 does not resolve it
- [Pulumi EBS Credential Model](../documents/engineering/pulumi_ebs_credential_model.md) — the authority and
  ownership vocabulary Phase 49 can model without resolving credentials, provisioning storage, or observing
  provider state
...
- [phase_70](phase_70_content_store_workflow.md) — the delegated Pulsar capability that must exist before a
  later live test can exercise the modeled failover intent
- [phase_76](phase_76_gateway_migration_drills.md) — the distinct cross-cluster gateway-migration obligation;
  Phase 49 executes neither axis
- [phase_80](phase_80_provider_dynamic_nodes.md) — the provider behavior that remains input to Phase 91's live
  test cycle, not Phase-49 evidence
