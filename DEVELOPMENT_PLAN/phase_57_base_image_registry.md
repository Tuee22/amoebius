# Phase 57: The base image, the jit-build resolver, and the in-cluster registry
...
- [Sprints](#sprints)
- [Sprint 57.1: Native-architecture base image bake — services + jit-build resolver/toolchain, not engine payloads ⏸️](#sprint-571-native-architecture-base-image-bake--services--jit-build-resolvertoolchain-not-engine-payloads-)
- [Sprint 57.2: Node side-load + Distribution `registry:2` standup ⏸️](#sprint-572-node-side-load--distribution-registry2-standup-)
- [Sprint 57.3: Atomic publication + immutable digest-pinned refs ⏸️](#sprint-573-atomic-publication--immutable-digest-pinned-refs-)
- [Sprint 57.4: The no-public-registry-pull gate ⏸️](#sprint-574-the-no-public-registry-pull-gate-)
...
This phase must take the empty `kind` cluster targeted by Phase 56 and make it pull only from itself. It must
...
[Phase 94](phase_94_jitml_rederivation.md) under [§L](development_plan_standards.md#l-one-substrate-discipline);
and **not** the Apple-Metal bridge,
which is a headless on-host Mach-O dylib produced only on the **apple** substrate at
[Phase 90](phase_90_apple_metal_host_daemon.md), never a linux ELF here). The ML **engine payloads**
...
architecture's independently tagged image is [Phase 58](phase_58_complementary_arch_child.md)'s; no index or
manifest list may join the two, because no host may attest the half it cannot execute
...
SSA reconciler that will eventually own the registry is a Phase 59 concern; `no-provisioner` retained storage
is Phase 61 and MinIO is Phase 63. Phase 57 is an explicit bootstrap cycle-break, not a resource or render
exception. `provisionBootstrapRegistry` binds the complete registry/proxy execution, storage, and node-image
import demand against the Phase-56 topology and returns an opaque `ProvisionedBootstrapRegistry`. A fresh
...
same package-private Phase-34 source serializer, but neither constructs a minimal `ProvisionedServiceSpec` nor
...
`BootstrapRegistryWholeDeploymentHandoffIdentityDigest`. When Phase 59 first constructs the complete
...
(Phase 62), so host-only **read** access is credential-free on the node. The target must make mutation
...
at `amd64`. An accelerator-bearing host does not expose its accelerator to this gate, so Phase 57 exercises
...
**Lane:** linux-cpu/amd64 — the architecture assigned to the post-Phase-56 live chain and later provider-node
pulls. Its complementary architecture is owned by [Phase 58](phase_58_complementary_arch_child.md).

**Register:** 3 — live infrastructure ([§K](development_plan_standards.md#k-honesty-proven--tested--assumed)).

**Depends on:** [Phase 56](phase_56_bootstrap_coordinator_kind.md)
...
| Key | Contract |
|---|---|
| `Claim` | UNRESOLVED — blocks validation: typed semantic payload and reviewer custody missing; prior prose: one cohesive claim — *the cluster pulls only from itself*. One architecture, one atomically published image, and no ML engine payload inside it. Explicit exclusions: every layer named in `Residue` remains UNVERIFIED. |
...
- [`image_build_doctrine.md` §7 — what amoebius bakes vs builds: the base container is the supply chain](../documents/engineering/image_build_doctrine.md#7-what-amoebius-bakes-vs-builds--the-base-container-is-the-supply-chain):
  ...
  image is rebuilt and republished at [Phase 92](phase_92_infernix_rederivation.md) /
  [Phase 94](phase_94_jitml_rederivation.md), never here), so Phase 57 carries no forward dependency on the
  extension lifts. The amoebius binary's own UI-server surface travels with it: the generic client source and
  bundle generated lazily from Haskell by [Phase 47](phase_47_ui_contract_generation.md) are a **baked asset of this
  image**, not a second image — the UI server is a worker responsibility of the same executable, and a UI
  release is release *data* ([Phase 73](phase_73_ui_program_release.md)), never an image build. That is
...
- [`image_build_doctrine.md` §3 — one image per architecture: the tag carries the architecture, not an index](../documents/engineering/image_build_doctrine.md#3-multi-architecture-images--one-natively-built-child-per-architecture):
  ...
  [Phase 58](phase_58_complementary_arch_child.md) independently builds and publishes the complementary tag.
  No index joins them.
...
flowchart LR
  ...
  s0["Sprint 57.1: Multi-arch base image bake — services + jit-build resolver…"]
  s1["Sprint 57.2: Node side-load + Distribution registry:2 standup"]
  s2["Sprint 57.3: Atomic publication + immutable digest-pinned refs"]
  s3["Sprint 57.4: The no-public-registry-pull gate"]
  gate["the phase 57 gate"]
...
```
*Orientation only. These are the seams Phase 57 would build in order; [Gate integrity](#gate-integrity) owns the apparatus. Every sprint is blocked and NOT VALIDATED, and no earlier seal or pause state is operative.*

## Sprint 57.1: Native-architecture base image bake — services + jit-build resolver/toolchain, not engine payloads ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: [Phase 56](phase_56_bootstrap_coordinator_kind.md) human approval
...
- A base `Dockerfile` baking this platform's binaries by the supply-chain preference order, plus the
  ...
  [Phase 92](phase_92_infernix_rederivation.md) / [Phase 94](phase_94_jitml_rederivation.md), never here, so Phase 57
...
## Sprint 57.2: Node side-load + Distribution `registry:2` standup ⏸️
...
reconciler (Phase 59), retained storage (Phase 61), and MinIO (Phase 63) do not yet exist, the registry comes up
...
registry/proxy object domain through Phase 34's package-private serializer; it is not a minimal
...
- A registry-specific pure/live boundary:
  ...
  bootstrap authority only; it cannot require the Phase-59 scheduler-ready/full managed inventory. The action
...
- `ProvisionedBootstrapRegistryExecution` is a finite cycle-break, not ordinary
  ...
  mandatory members of Phase 60's default→`amoebius-capacity` cutover. They cannot remain as a second
...
- A `ProvisionedBootstrapRegistry` whose `BootstrapRegistryAction` side-loads the selected image and initializes
  ...
  Phase 34. There is no public `render :: ProvisionedServiceSpec -> …` or bootstrap `ProvisionedSpec`; public
  ...
  initialized-field digest. Phase 59 may adopt those exact objects into a later whole `ProvisionedSpec` only
  after equal live readback and a one-time typed ownership handoff; mismatch rejects without a second writer,
  apply, delete, or recreate. The backing is replaced by the MinIO S3 driver in Phase 63.
...
7. A fake later whole-deployment adoption with the equal handoff digest succeeds once without object
   ...
   object writes. Phase 63 preserves this private demand's `objectSet`, `derivedPeak`, and upload/orphan
...
9. Assert no public-registry pull occurred during registry standup **from the OS-boundary observer** (§M.5 —
   ...
   registries), recorded for the Sprint 57.4 gate — never from a self-emitted compliance trace.

### Remaining Work

Repeat the side-load and registry standup in the uninterrupted phase run, then pass the Sprint 57.2 gate and
retain its receipt. Atomic publication remains exclusively Sprint 57.3 work. The MinIO-backed backend is
Phase 63's, and Phase 59 owns adoption of these objects through the one-time typed handoff.

## Sprint 57.3: Atomic publication + immutable digest-pinned refs ⏸️
...
against an ephemeral config directory. Vault does not yet exist (Phase 62), so host-only reads are
...
- The no-env build mechanics: an ephemeral `docker --config <dir>` created per build and scrubbed afterward, the
  ...
  Vault `SecretRef` identity is flagged as the Phase-62+ transport-hardening target.
...
Revalidate atomic publication of the amended handoff. Sprint 57.4 consumes this run's immutable digest reference and the standup/publication OS-boundary
captures, and installs the enforcing deny boundary the paired canaries prove.

## Sprint 57.4: The no-public-registry-pull gate ⏸️

**Status**: Blocked — NOT VALIDATED
**Implementation**: UNRESOLVED — blocks validation: the authored Haskell implementation path has not been established.
**Blocked by**: Sprint 57.3
**Independent Validation**: UNRESOLVED — blocks validation: no falsifiable positive control, paired specific-reason negative, changed-subject mutant, and residue seam has been established.
**Oracle**: UNRESOLVED — blocks validation: no separate Haskell oracle, independence boundary, and human reviewer have been established.
**Legacy IDs**: UNRESOLVED — blocks validation: typed Haskell legacy bindings have not been reconciled for this sprint.
**Docs to update**: UNRESOLVED — blocks validation: governed doctrine owners have not been established for this sprint.
...
later reconciler-owned rendering (Phase 59) and the MinIO-backed blob store (Phase 63) marked UNVERIFIED here.
...
externally attested against the run's source snapshot, and the gate ran after Phase 56 closed. The later
...
- `documents/engineering/content_addressing_doctrine.md` — annotate §4.5 that the base image contributes the
  ...
  resolved on first miss (the resolver's own live proof lands in Phase 81).
...
- `documents/engineering/platform_services_doctrine.md` — the §3 registry-as-single-image-source note flips from
  design intent to a delivered Distribution `registry:2` standup, with the MinIO-backed S3 driver still a Phase-63 target.
...
- `DEVELOPMENT_PLAN/README.md` — flip the Phase-57 row from Planned to its delivered status and link this document.
- `DEVELOPMENT_PLAN/substrates.md` — record Phase 57's gate substrate (linux-cpu) in the per-phase substrate map.
- `DEVELOPMENT_PLAN/system_components.md` — register the Haskell `BakeCatalog`/renderer components and the
  Distribution `registry:2` standup as Phase-57 design-first rows, reconciled against the component inventory;
...
- [phase_56](phase_56_bootstrap_coordinator_kind.md) — the Haskell cluster bootstrap coordinator + empty `kind` cluster this phase publishes into; Python `pb` is only the earlier pre-binary handoff
- [phase_59](phase_59_object_reconciler.md) — the typed reconciler that adopts the equal-digest bootstrap
  objects into the first whole-deployment `ProvisionedSpec` through the one-time ownership handoff
- [phase_63](phase_63_platform_backbone.md) — the standard service stack whose MinIO backs the registry's S3-driver blob store
- [phase_81](phase_81_determinism_jitcache.md) — the live jit-build engine resolver + `CacheBudget` cache the toolchain here feeds
