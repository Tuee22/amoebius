...
helperPath :: FilePath
helperPath = "/phase33-artifacts/phase33_runtime_helper.py"

uploadPath :: FilePath
uploadPath = "/phase33-dhall/examples/phase33-upload.dhall"
...
main = withSocketsDo $ do
  ...
  unless (not (Text.null podUid)) (die "singleton-pod-uid-empty")
...
handleRequest podUid ready adminLock authorityLock request = case (requestMethod request, requestPath request) of
  ...
  ("GET", "/metrics") -> pure (response 200 "text/plain" "amoebius_singleton_ready 1\namoebius_singleton_replicas 1\n")
...
withFreshAuthority podUid ready authorityLock continuation = do
  ...
  if held
    then continuation
    else pure (jsonResponse 503 (object ["tag" .= ("singleton-lease-not-held" :: Text)]))
