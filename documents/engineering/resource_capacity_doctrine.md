...
1. The `Capacity` / `Demand` / `Budget` records and the refined non-zero `Quantity` they are built from (§3).
2. The total fold — `fits` / `carve` / `place` — and its nesting (host → cluster/VM → workload) (§4).
...
4. The `Growable` / `ScalingPolicy` escape valve: dynamic provisioning owned by amoebius, the only path by
   which a bounded budget grows (§6).
...
[illegal_state_catalog.md §3.17-§3.21 / §4.6](./illegal_state_catalog.md); this doc is the normative home of
...
having **no unbounded arm** (§5) and the `Growable` union having **no bare-unbounded arm** (§6) are grade-1
...
[chaos_failover_doctrine.md](./chaos_failover_doctrine.md) and the testing doctrine. §8 states the one
...
- **`Capacity`** — what a *provider* offers: a record of `Quantity` per axis. A physical host advertises one
  ...
  a `StorageBacking` advertises a storage `Capacity` (§5).
...
- **`Budget`** — a `Capacity` an owner is *allowed to consume against*, which may be a fixed cap or a
  quota-capped growable (§5, §6). `Budget` is where capacity meets the escape valve; `Capacity` is the raw
...
The fold is three total functions (grade-2 checked rejections, §2):
...
- **`place :: Topology -> [Workload] -> Either Overcommit Placement`** — the cluster-level fold: the summed
  ...
  schedulability existence check ([illegal_state_catalog.md §3.5](./illegal_state_catalog.md)).

The nesting is where the illegal states §3.17 (I5/I6/I7) live:
...
The fold is **total and re-runnable**: after any `Growable` policy grows a capacity (§6) the fold re-runs
...
- **No unbounded constructor** — the union shape is grade-1: a value cannot denote unbounded storage. This is
  ...
  [storage_lifecycle_doctrine.md §5.2](./storage_lifecycle_doctrine.md), and this doc owns the *aggregate
  arithmetic* over it.
...
- **Both MinIO and Pulsar fold against a `StorageBudget`.** An app's object usage (`<app>/<bucket>` MinIO
  buckets) and a topic's retained bytes (§7) each contribute a storage `Demand`; the sum against the backing
  is the same §4 fold. "An app that can consume more storage than is available" (I10) is therefore
...
- **Hot-tier fit (availability-critical).** The offload trigger is a **size high-water mark** on the primary
  ...
  uninhabitable, [illegal_state_catalog.md §3.20](./illegal_state_catalog.md)). The per-topic hot cap **plus
  ...
  decode rejection.
- **Durable-total fit.** The total retained bytes fold against the selected offload target's ceiling (§5) —
...
- **Declared (grade-2).** Each host/node advertises a `Capacity` in the substrate node inventory
  ...
  ([pulumi_iac_doctrine.md](./pulumi_iac_doctrine.md)); each `StorageBacking` declares its size. The §4 fold
...
> ([cluster_lifecycle_doctrine.md §6](./cluster_lifecycle_doctrine.md#6-push-back-when-teardown-would-break-the-global-dhall)),
> that is sibling evidence, not amoebius proof ([documentation_standards.md §6](../documentation_standards.md)).
...
| Per-container cpu/ram declaration | [platform_services_doctrine.md §10](./platform_services_doctrine.md) |
| Cloud quota provisioning; dynamic node provisioning enaction; per-PV EBS | [pulumi_iac_doctrine.md](./pulumi_iac_doctrine.md) |
| The Pulsar topic-lifecycle policy (retention, size-triggered offload, backlog quota) | [pulsar_client_doctrine.md §6](./pulsar_client_doctrine.md) |
| The content-addressed MinIO store as a storage backing | [content_addressing_doctrine.md](./content_addressing_doctrine.md) |
| Which capacity states are illegal and the §4.6 technique that forecloses them | [illegal_state_catalog.md](./illegal_state_catalog.md) |
...
links back for status, per [documentation_standards.md §6](../documentation_standards.md).
