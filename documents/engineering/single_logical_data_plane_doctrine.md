...
- **Many data planes, reached by gateway migration.** The remote compute *is* a second cluster, with its
  ...
  [chaos_failover_doctrine.md](./chaos_failover_doctrine.md) and Phases 42–43.
...
| Owned by | **This doc** + [network_fabric_doctrine.md](./network_fabric_doctrine.md) | [chaos_failover_doctrine.md](./chaos_failover_doctrine.md), [gateway_migration_doctrine.md](./gateway_migration_doctrine.md), Phases 42–43 |
...
orientation only: the attach topology depends on the native Pulsar/MinIO client (Phase 35), the
host-compute-daemon peer model it generalizes (Phase 53), and cloud spot provisioning + price-shopping
(Phase 44), and rides the WireGuard fabric promoted from the former later-phase candidate into Phase 41 — but is
*independent of Phase 42's geo-replication*, precisely because an attach pool is not a second cluster.
...
> unbuilt Phase-53 design, and its loopback-NodePort shape has only a prodbox precedent (evidence, not amoebius
