...
- **Many data planes, reached by gateway migration.** The remote compute *is* a second cluster, with its
  ...
  [chaos_failover_doctrine.md](./chaos_failover_doctrine.md) and Phases 32–33.
...
| Owned by | **This doc** + [network_fabric_doctrine.md](./network_fabric_doctrine.md) | [chaos_failover_doctrine.md](./chaos_failover_doctrine.md), [gateway_migration_doctrine.md](./gateway_migration_doctrine.md), Phases 32–33 |
...
orientation only: the attach topology depends on the native Pulsar/MinIO client (Phase 28), the
host-compute-daemon peer model it generalizes (Phase 41), and cloud spot provisioning + price-shopping
(Phase 34), and rides the WireGuard fabric promoted from the former later-phase candidate into Phase 31 — but is
*independent of Phase 32's geo-replication*, precisely because an attach pool is not a second cluster.
...
> unbuilt Phase-41 design, and its loopback-NodePort shape has only a prodbox precedent (evidence, not amoebius
