...
- **Many data planes, reached by gateway migration.** The remote compute *is* a second cluster, with its
  ...
  [chaos_failover_doctrine.md](./chaos_failover_doctrine.md) and Phases 43–43.
...
| Owned by | **This doc** + [network_fabric_doctrine.md](./network_fabric_doctrine.md) | [chaos_failover_doctrine.md](./chaos_failover_doctrine.md), [gateway_migration_doctrine.md](./gateway_migration_doctrine.md), Phases 43–44 |
...
orientation only: the attach topology depends on the native Pulsar/MinIO client (Phase 36), the
host-compute-daemon peer model it generalizes (Phase 54), and cloud spot provisioning + price-shopping
(Phase 45), and rides the WireGuard fabric promoted from the former later-phase candidate into Phase 42 — but is
*independent of Phase 43's geo-replication*, precisely because an attach pool is not a second cluster.
...
> unbuilt Phase-54 design, and its loopback-NodePort shape has only a prodbox precedent (evidence, not amoebius
