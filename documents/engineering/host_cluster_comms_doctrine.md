...
> **Honesty.** Phase 54 now tests the exact two-service `NodePort`/loopback type, all four wild-exposure
...
- **This generalizes a pattern proven in the sibling prodbox project**, where in-cluster Harbor is reached
  ...
  a sibling precedent. Phase 54's scoped Linux-host probe tests the same bind restriction locally, but
...
compute-daemon contracts land in **Phase 54**; its scoped gate does not claim the physical Apple peer. The
