...
> **Purpose**: Define the topology by which amoebius and its seed projects coordinate finite resources on one
> host without a shared scheduler or a present-day seed-to-amoebius dependency.
> **Read this if**: a host workload, resource-family extension, mechanism provider, project adapter, or
> substrate interpreter has to participate in shared-host admission.

This reference-only cross-project proposal records the proposed host-coordination topology and rationale. It does not
establish implementation status, conformance, validation, or phase promotion; the
[development plan](../../DEVELOPMENT_PLAN/README.md) owns amoebius sequencing and status. It assumes
familiarity with the project-local resource and lifecycle doctrine routed through the
...
- [Substrate Doctrine](./substrate_doctrine.md) — kernel and hardware substrate ownership
- [Extension Conformance Doctrine](./extension_conformance_doctrine.md) — finite extension laws and evidence
