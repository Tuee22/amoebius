...
Phase 28's live gate observed this exact order on Kubernetes: the bootstrap witness preceded the finite
...
supplies the *observation*, and the honesty is in keeping those two claims apart. Phase 27 first validates
...
scan over the enactor all passed without a sleep-gated continuation. Phase 28 extended that evidence to both
scheduler witnesses and the guarded-Pod admission edge; every live wait used an observed Kubernetes state.
Phase 40 extended it to an ordered `RolloutPlan`: external API observations and increasing resource versions
...
### Phase-32 derived service-DAG validation

Phase 32 implemented the 14-service declared dependency graph and checked it byte-for-byte against an
...
**Phases 24, 27, and 30**; the **typed `Readiness` gate** and the [§3.41](../illegal_state/illegal_state_lifecycle.md#341-a-duration-gated--hand-ordered-bring-up-sequence-a-readiness-race)
catalog foreclosure land in **Phase 34** with the orchestration DSL and the control-plane singleton. This doc
states the target shape and links back for status.

> **Honesty.** Phase 27, sealed 2026-08-14, provides tested amoebius evidence for the reconciler's
...
> and execution modules. Phase 28 adds the scheduler-readiness proof: `BootstrapCapacitySchedulerReady` and
> `ManagedCapacityReady` are observed witnesses in a recorded event order, never sleeps. The standard-service
> DAG (Phase 32) is **UNVERIFIED** pending its reopened gate. The broader daemon spine,
