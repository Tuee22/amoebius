...
by `provision`, has no dhall-typecheck or gadt-decode surface, and is carried on the reservation ledger beside the request
...
Every `ContainerEnvelope` names a content-digested OCI image manifest whose
...
| Host image build | expanded acyclic per-architecture `BuildExecutionEnvelope`: per-stage CPU/memory reservation+ceiling, intermediate bytes, and cache-write delta; build-cache budget; finite architecture + stage concurrency | physical host/engine-VM residual plus named `BuildScratch`/cache carves | enumerate every dependency-valid concurrent stage set and derive runtime/scratch/cache-write peaks; fit observed cache residents + write peak against budget/backing until observed GC; snapshot mismatch/unknown commitment rejects before `buildx` | enforced BuildKit/engine-VM CPU/memory ceiling and named scratch/cache locations; resulting `ImageArtifact` is accounted separately |
| Pre-binary bootstrap/toolchain | `BootstrapExecutionEnvelope`: installer CPU/memory, per-tool installed + peak download/unpack bytes, and the cabal build's complete stage envelope | physical host residual plus named `ToolInstall`, `BuildScratch`, and cache carves | Python performs no capacity validation and cannot mint or recheck a provision token. The Phase-51 Haskell boundary harness supplies and independently observes a fake adapter/envelope; a real first establishment has no pre-Haskell capacity guarantee and remains explicit `UNVERIFIED` residue. After exec, Haskell re-observes the run and may refuse later work, but cannot retroactively turn bootstrap success into evidence. | installer/build process containment and fixed backing/cache locations only; the built binary is output, not a capacity verdict or free overhead |
