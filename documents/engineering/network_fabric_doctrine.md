...
full-node kubelet↔apiserver wire (K2, [§3](#3-keys-config-and-distribution--wireguard-as-just-another-reconcile)/[§4](#4-topology-the-hub-is-the-gateway-role-and-the-fabric-moves-with-it)) — and **defers the cross-cluster broker↔broker geo-replication wire to Phase 47**: it is design-intent, its `render()` obligation is not yet
...
[Phase 46](../../DEVELOPMENT_PLAN/phase_46_network_fabric_wireguard.md) validates this v1 seam on the
...
> named in-scope ([§1](#1-why-this-doctrine-exists-the-inter-cluster-wire-is-an-open-gap)) but is Phase-47 design intent — the disjoint-per-cluster addressing exists, the
...
orientation only: the fabric was promoted from the former later-phase candidate into Phase 46; the old
...
> **Honesty.** Phase 46 supplies Register-3 tested evidence for the static two-peer raw-kernel fabric,
