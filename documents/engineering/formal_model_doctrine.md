...
**Phase ownership is explicit.** The Phase-11 kernel gate checks the in-process explorer's safety semantics and
the emitter's independently authored structural meaning without acquiring or trusting an external checker.
Executable TLA+ checking is a consumer claim owned by the later concrete-model/checker phases; it cannot be a
prerequisite for sealing the kernel that supplies their input. The in-process explorer
...
check `modelProperties`. A later TLC-backed liveness verdict must bind its checker identity, emitted input,
fairness assumptions, and concrete model; Phase 11 claims only that those temporal and fairness constructors
are preserved structurally. This asymmetry is carried into the honesty ledger
([§6](#6-what-a-green-model-check-proves-and-what-it-does-not)).
...
Correspondence is made *testable* at the kernel seam by a second in-process reading of the same value: a bounded reachability
explorer over `interpret` (breadth-first over reachable states, pruned by the TLC state constraint, checking every
invariant on every reachable state — the same shape an external checker later applies to the emitted spec).
The kernel is validated by:

- the in-process Haskell explorer suite over exact hand-derived and generated expectations, and
- an independently authored Haskell semantic-fact oracle over the emitted `.tla`/`.cfg` structure.

The unchanged semantic oracle must reject applied production mutations to guard interpretation, frame-condition
emission, and invariant meaning. That establishes the kernel's source correspondence without presenting a
generated byte snapshot or an unavailable external executable as independent authority. A later concrete-model
gate may add an explorer↔TLC differential, but that evidence proves its checker/model pairing and is not
retroactively a Phase-11 prerequisite.
...
Phase 17 applies the same rule to the concrete gateway protocol without turning generated bytes into an
oracle. `GatewayMigrationOracle.hs` fixes twelve renderer facts and the model/cutoff corpus as Haskell values;
the explorer and a freshly generated TLC run must agree on all 53 canonical state fingerprints. The emitted
TLA/CFG/DOT/log files remain run-scoped products, and compiled production mutations of the ownership fence,
structural budget, and fairness set must each be rejected independently. This establishes the bounded formal
claim only; runtime fidelity and the decomposition lemma remain explicitly outside that gate.

Phase 18 binds the DSL instance to `Amoebius.Formal.Dsl.Models` and a separately authored
`DslFormalModelOracle.hs`. Its finite claim is six model contracts and 18 explorer states, with independent
TLC fingerprint comparison for the five transition-bearing models, eight exact safety mutations, and four
fairness deletions. Actual-code correspondence is deliberately narrower: all 6,561 vectors in the declared
`0..2` four-axis capacity differential, the five-calculus projection, and the token, reservation, and
unreachable-reconcile decisions. Decoder and provision/render/chain correspondence stay with their later
artifact-owning phases; effectful and live fidelity remain UNVERIFIED.

[Phase 19](../../DEVELOPMENT_PLAN/phase_19_reconcile_core_simulation.md) consumes four of those names only as correspondence labels: `NoTokenReuse`,
`OneDebitPerReservation`, `RefuseOnUnreachable`, and `ConvergedIsStable`. Its independent Haskell oracle checks
the labels against the actual Phase-18 model inventory while its production code is exercised under modeled
schedules. This is tested-code-to-name correspondence, not a model-to-code refinement proof; modeled-store
fidelity is ASSUMED and effectful runtime fidelity remains UNVERIFIED.
...
The implemented boundary is `Amoebius.Checker.ExplicitState`: a positive `SearchBound`, deterministic BFS,
model-bound verdict digest, and independently replayable counterexample steps. Its Phase-12 oracle is authored
in Haskell, while result tables, object files, and binaries are generated only beneath the fresh gate run root.
This establishes finite-state semantics only; symbolic reach, refinement, simulation, runtime fidelity, and
hardware observations remain separate later layers.

**The symbolic layer has a separate reading.** amoebius owns a total classifier, QF
linear-integer/boolean SMT-LIB translation, and full-conjunction base/step induction schema over `Model`; an
injected absolute solver path supplies only formula decisions. Phase qualification builds an independent
Haskell decision process beneath its fresh run root, avoiding ambient solver installation and proving the
translation against authored semantics. Compatibility with a dynamically resolved production Z3 executable
remains a later runtime/tool-integration observation rather than a premise smuggled into this pure gate.
Unsupported syntax and `unknown` stay explicit. A reachable-safe but non-inductive Haskell model must
demonstrate conservative incompleteness.
...
queries. Phase 14 compiles the source fixtures and the Phase-11 model projection serially, emits named
invariant expressions beneath its fresh `.build/runs/phase-14/**` root, and compares them with independent
Haskell semantic expectations. Its fake SMT process supplies raw decisions only; no Python or serialized
behavioral oracle participates in the verdict.
