...
**Node-local read-side status.** The [Phase 10 gate](../../DEVELOPMENT_PLAN/phase_10_execution_accelerator_folds.md)
...
not proof in amoebius. The Phase-33 amoebius retained-host slice is now independently live-tested: the exact
scope and ledger are owned by
[Phase 33](../../DEVELOPMENT_PLAN/phase_33_retained_storage.md). Phase 35 additionally live-tested the
...
*Normative model. Phase 33 live-tested the host-retained arm: PV name and claimRef were pure identity functions, and independently observed fixed-size host images supplied the backing; provider-backed EBS remains a later-phase runtime proof obligation.*
...
- **Hard allocation, not advisory accounting.** The provisioned raw capacity is the real ceiling; a workload
  ...
  a raw shared-filesystem subdirectory is not an admissible `StorageBacking`. Phase 33 live-tested this
...
**Pure read-side status.** The [Phase 9 gate](../../DEVELOPMENT_PLAN/phase_09_storage_geometry_folds.md)
...
*Normative model with delivered host-retained instances. Phase 33 observed the real cluster/node/API absent while host images retained run-unique bytes, recreated fresh cluster/PV/claim identities, and read the same Postgres row and MinIO object without a post-recreate write. Phase 35 then bound distributed MinIO/ZooKeeper/BookKeeper StatefulSet claims to identity-named fixed ext4 backings; provider instances remain later-phase proof obligations.*
...
Phase 44 is the validated database-schema instance of this discipline: two live PostgreSQL rounds performed
...
intent; the Phase-44 retire step neither represented nor performed durable-byte deletion.
...
Phase 34 exercised the Phase-33 retained-storage contract with the real root Vault: a 128 MiB fixed ext4 Raft
...
Phase 42 consumes the retained Phase-33/35 MinIO backing without changing its lifetime rules. Its live gate
...
Phase 48 exercises the same lifetime distinction during gateway movement. Graceful teardown is admitted only
after synchronization and keeps the retained Phase-29 platform; chaos failover is labeled budgeted instead of
lossless. The survivor-capacity check refuses durable, cache, ephemeral, compute, device, and unreachable
shortfalls before resource release, and the live postflight removes only Phase-48 objects and temporary roots.

Phase 51 adds the provider-storage model without claiming a provider effect. The pure contract keeps required
...
and Phase-56 reclamation remain UNVERIFIED.
...
status ledger; it states the target shape and links back for status. Phase 33 delivered and live-tested the
...
Phase 35 further exercised the no-provisioner distributed-service face: four MinIO drive claims, three
...
This is a regression projection of the Phase-33 storage guarantee, not a claim that the retained production-
