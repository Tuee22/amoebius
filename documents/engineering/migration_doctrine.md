...
| DSL schema evolution | widening (optional field / uninhabited arm) is transparent; narrowing is a dhall-typecheck rejection the operator resolves by editing the authored value; reinterpretation is inadmissible | dhall-typecheck typecheck over the committed fixture corpus | no automatic rewrite of an authored `.dhall`; the prior schema is the prior `Release` | `type-foreclosed` (dhall-typecheck) + review residue on reinterpretation | [generated_artifacts §5.1](./generated_artifacts_doctrine.md#51-when-the-authored-schema-changes-under-an-authored-dhall) |
