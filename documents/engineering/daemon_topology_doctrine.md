...
- The steady state is **one pod**, kept so by the Deployment controller (etcd-backed).
- Where *strict* at-most-one-writer during a rolling update or a node partition is required, the mechanism is a
  **Kubernetes `Lease`** — the standard etcd-backed leader-election object (the client-go leader-election
  pattern) — **not** an amoebius protocol. This uses etcd's consensus through the k8s API; it does not
  duplicate it.
