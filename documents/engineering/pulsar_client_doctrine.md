...
> native protocol" remains a design rationale, not a benchmark result. Phase 40 did validate the native wire,
...
The native client is server-side only. A browser never receives a Pulsar credential, topic, subscription, or
WebSocket/native-protocol bridge. Low-code UI actions terminate at the same-origin UI server; UI projection
workers consume native CBOR and expose only authorized owner-scoped projections
...
(the client lands in **Phase 40**); this doc states the target shape and links back, never a status ledger.
...
*Validated Phase-40 chain: the CBOR encode/frame path and total decoder are tested in process; the broker seam and running consumer are Register-3 runtime-checked, not proven.*

> **Honesty.** Phase 40 validates the typed producer/consumer API, a hand-authored API-surface golden, a raw
...
> **Honesty.** Supernova remains provenance, not evidence. Phase 40 independently validates amoebius's
...
- **Exposed (derived): topic compaction + TableView.** *Compaction* is a namespace/topic policy the
  ...
  A low-code `UiProjectionWorker` is a third application: its key is derived from
  `(AppId, TenantId, Owner, ProjectionId)`, its value is a bounded public contract, and its consumer authority
  is bound by the server plan. Omitting tenant or owner from the key/subscription is an illegal state, not a
  filter convention
  ...
  [Phase 43](../../DEVELOPMENT_PLAN/phase_43_ui_projection_runtime.md) validates this application live: native
...
**Storage-fold status.** The [Phase 9 gate](../../DEVELOPMENT_PLAN/phase_09_storage_geometry_folds.md)
...
> **Honesty.** The training-feed view is Phase-38-and-later design intent layered on the client, not a built
...
> **Honesty.** Phase 40 now validates the reusable Haskell client at Register 3: two distinct namespaces use
...
Phase 42 consumes this surface through native ranked `Failover` subscriptions. The protocol subset now carries
...
Phase 54 adds a scoped infernix adapter and a separate native driver using typed CBOR command/event payloads,
...
Phase 59's scoped UI rollout result tests cursor keys containing tenant, owner, and stream and preserves the
...
`amoebius-pulsar` instead — one subsystem at a time, per the Phase 54 migration in
...
the topology algebra, and the round-trip gate landed in **Phase 40**, and the infernix/jitML migration onto it
is **Phases 54 (infernix) and 52 (jitML)**. This doc never maintains a competing status ledger.
