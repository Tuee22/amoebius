...
([storage_lifecycle_doctrine.md [§5](#5-dns-route53-and-tls-zerossl-the-provider-integrations-this-doctrine-owns), [§7](#7-applicative-parallelism-for-independent-deploys)](./storage_lifecycle_doctrine.md#5-sizes-are-explicit-hard-capped-and-one-volume-per-claim)) and the one the original vision
...
1. **EBS volumes are in Pulumi, but in their own durable class — never in the ephemeral cluster stack.**
   ...
   ([storage_lifecycle_doctrine.md [§5.1](#51-dns--route53), [§6](#6-the-ebs-create-vs-delete-credential-model)](./storage_lifecycle_doctrine.md#51-storage-is-independent-of-the-node-lifecycle)).
...
only (the plan is authoritative): root Vault/PKI lands in **Phase 22**; public-edge ZeroSSL/route53 integration
lands in **Phase 25**; gateway-migration route53 repointing lands in **Phase 33**; amoebic spawning via SSH-key
Pulumi with the MinIO backend + Vault-envelope encryption lands in **Phase 32**; provider-managed clusters
(EKS) and dynamic node
provisioning land in **Phase 34**; the elevated-harness storage-deletion safety that makes the [§6](#6-the-ebs-create-vs-delete-credential-model)
create-vs-delete model leak-free lands in **Phase 42**. Per
