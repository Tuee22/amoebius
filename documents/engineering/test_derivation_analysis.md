...
wording cannot drift back. `DEVELOPMENT_PLAN/phase_40_release_lifecycle.md` propagated the same claim in
four places; a first pass corrected only one (`:194`), and an audit found the other three surviving in the
Sprint 40.3 validation, deliverable, and the committed `evidence_strength.txt` oracle it produces. All four
