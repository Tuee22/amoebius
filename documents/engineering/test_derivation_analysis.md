...
- a separately authored Haskell module states the expected relation over those surfaces.
...
authorship has no independent provenance until the qualified gate establishes the oracle boundary.
...
DSL gate barrier. Generated PureScript, JavaScript, browser scripts, and JSON observations are never
...
Importing an operator value into a gate does not permit committing it. The gate stages it as external input
...
until the hardware-free DSL gate and every predecessor gate pass. A cluster or image
...
active inventory to readers; editing or removing its rows cannot alter a verdict. The documentation gate owns prose
...
- a same-change oracle without an oracle-independence check;
...
- a separately authored expectation before it is treated as independent.
...
- [Development Plan](../../DEVELOPMENT_PLAN/README.md) — phase contracts and gate-controlled status
