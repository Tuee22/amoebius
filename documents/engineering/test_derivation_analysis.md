...
`locus_registry.tsv` that `phase_27_illegal_state_covering.md` Sprint 27.1 already schedules, and lands there.
...
wording cannot drift back. `DEVELOPMENT_PLAN/phase_71_release_lifecycle.md` propagated the same claim in
four places; a first pass corrected only one (`:194`), and an audit found the other three surviving in the
Sprint 71.3 validation, deliverable, and the committed `evidence_strength.txt` oracle it produces. All four
...
`DEVELOPMENT_PLAN/phase_17_gateway_migration_model.md` defined `independent` as graph-independence **and**
resource-independence, had the fold reject cluster-reuse across records, and claimed the two readings were
thereby reconciled. They were not: the phase-17 fold would reject the doctrine's own stress case.
...
| [§4.2](#42-an-illegal-state-catalog--fixture-coverage-check) the fixture-coverage join | Phase 27, as a consumer of `locus_registry.tsv` |
