...
not left to Patroni's default. Phase 42 supplied the first amoebius live evidence for that premise: the
committed oracle matched `synchronous_mode: on`, `synchronous_mode_strict: on`, and
`maximum_lag_on_failover: 1048576` on a Ready three-member Patroni cluster, while the async-default mutant
failed for its specific reason. This is evidence that the delegation precondition was rendered and operated,
not a proof of Patroni consensus or a multi-zone failover theorem. The gate ran on the universal `linux-cpu`
baseline; when pristine Linux isolation is required the route is Incus on Linux/Linux-CUDA, Lima on Apple,
or WSL2 on Windows. amoebius **delegates** the
...
**design-model and invariant catalog** are authored and TLC-checked design-first in **Phase 10** (Tier 1 —
...
obligation confirmed by **Register-3 chaos injection** in **Phase 54**. The sibling prodbox spec
...
under `IOSimPOR`, exercising the schedule the pure decision leaves open — **is adopted early, in Phase 10**, as
...
what DST newly earns is that the built daemon's real schedule, and its behaviour under injected
partition/reorder/redelivery/crash, are **validated in-process and deterministically replayable before any live deployment**. The mechanics, the register's place in the spine, and the honest tradeoff it buys — replacing a
...
The first concrete rung is built in
[Phase 22](../../DEVELOPMENT_PLAN/phase_22_deterministic_sim_substrate.md): `Env m`, injected real clients, an
`IOSim` interpreter, six modeled substrates, and deterministic/POR replay of a reference reconciler. The phase
keeps real-substrate fidelity assumed and live behavior unverified.
...
**Applied to amoebius today, the ledger is layered — and that is by design.** Phases 9 and 10 are built:
the formal kernel and bounded `GatewayMigration` protocol are proven-for-the-model/tested at Register 1.
...
when the multi-cluster runtime is built and its live model↔code correspondence is closed (Phase 54, deferred
...
### Phase-48 layered result

Phase 48 records a Register-2.5 cross-check (256 deterministic schedules plus bounded `IOSimPOR`) and a
Register-3 live injection on `linux-cpu`. The live fault kills `worker-a` after immutable store commit and
before acknowledgement; broker-ranked `worker-b` takes over, the broker output counter proves redispatch, the
external subscription observes one command, and Kubernetes/MinIO/Pulsar remainders are empty. Double-apply and
orphan-consumer mutants fail in simulation, while all ten phase mutants fail in the assembled gate. These are
tested intra-cluster results. Cross-cluster failover and Pulsar consensus internals remain UNVERIFIED. Every
hardware substrate can run the CPU lane; pristine Linux uses Incus on Linux/Linux-CUDA, Lima on Apple, or WSL2
on Windows.
...
Under amoebius's two-tier schedule this dependency runs *ahead of the built code*: the Phase-10 Model is
...
lands, not a gap in the Phase-10 design-model.
...
Phase 46; Pulsar's own consensus is delegated, not modelled); the **impossibility-bounded (R7)** and
...
**Phase-53 tested boundary.** `Amoebius.Multicluster.ConfluenceClass` classifies the pinned crossing set and
defaults every unknown invariant to non-confluent; `Amoebius.Multicluster.GeoReplication` supplies the
duplicate/reorder-stable work fold. The Register-3 gate used two real projected child clusters and external
Vault, MinIO, native Pulsar, and Patroni observers. Three classifier/projection/resource-accounting mutants
turn red. This establishes the classifier and the child boundary as tested, not the Phase-54 planned/failover
migration correspondence and not physically independent brokers per child. The lane this ran on, and the guest that
supplies it on each substrate, are owned by
[substrate_doctrine.md §1.1](./substrate_doctrine.md#11-the-natural-architecture-rule) rather than restated here.

**Phase-54 migration datapoint.** `Amoebius.Multicluster.GatewayMigration` delegates its decisions to the
Phase-10 `interpret` model. Planned and Failover traces cover all sixteen migration actions; an outside-forest
journal observes eight unreplicated acknowledgements at each cut, zero Planned loss, fenced promotion, and
post-heal convergence. Recovery time is tested, while the data-loss bound remains assumed-and-monitored.
Authoritative local DNS and a raw-kernel hub move are tested; Route53 and real WAN behavior are UNVERIFIED.
...
## Related Documents
- [Development Plan](../../DEVELOPMENT_PLAN/README.md) — phase order, adoption ownership, and validation closure (Phase 54 carries the cross-cluster failover proof). This doctrine maintains no competing status ledger.
...
- [Gateway Migration Model Doctrine](./gateway_migration_model_doctrine.md) — the concrete formal spec and
  invariant catalog, authored design-first in Phase 10 and covering both branches. Correspondence between
  model and code is differentially checked; runtime fidelity is the deferred Tier-2 obligation (Phase 54, via Register-3 chaos) this doctrine's Model move requires.
