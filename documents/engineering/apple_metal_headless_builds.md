...
> **Honesty.** This shape is **evidenced by the `jitML` seed**, whose implemented headless Apple path closed
> on Apple Silicon with no Tart, SwiftPM, offline-`metal`, Xcode or keychain step on the core path. That is
...
> Phase 90 owns implementation and validation of amoebius's fixed-MSL numerical contract, bridge/build plan,
...
|--------------|--------------|-----------------|
| `apple.command-line-tools` | **Core** execution, via the bridge build | `/usr/bin/clang` and the `Foundation`/`Metal` framework headers it links come from the Xcode Command Line Tools, which a clean macOS install does not carry. Verify `xcode-select -p`. This is a floor item the operator supplies ([`substrate_doctrine.md` §3.1](./substrate_doctrine.md#31-the-per-substrate-floor-what-only-the-operator-can-supply)), and it is the small thing the no-full-Xcode commitment of [§1](#1-the-commitment-headless-on-host-no-vm) still needs. |
...
> amoebius measurement. amoebius re-derives the same conclusion independently; the sibling `infernix` library likewise
...
headless fixed-Metal-bridge build + the native Apple-Metal host worker are assigned to **Phase 90** (`apple`), whose
target gate ([phase_90_apple_metal_host_daemon.md](../../DEVELOPMENT_PLAN/phase_90_apple_metal_host_daemon.md)) must bring up
...
- [Phase 90 — Apple-Metal host daemon](../../DEVELOPMENT_PLAN/phase_90_apple_metal_host_daemon.md)
