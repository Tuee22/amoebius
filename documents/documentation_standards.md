...
The **pure-workflow diagram vocabulary** — the canonical shape (functional-programming role), colour
(`classDef` honesty / validation-locus band), and edge conventions, with the legend and authoring rules — is
owned by [`engineering/diagram_conventions.md`](./engineering/diagram_conventions.md); this section owns only
the general flat/solid/no-subgraph/no-dotted constraint it builds on. A document that draws a pure-workflow
diagram adds one back-link to that file near its first diagram and does not restate the palette.
