...
- [`preview`, `accept`, and `replay`](./engineering/gate_runner_doctrine.md#6-commands-generations-and-receipts) — the gate run that mints nothing, the run that records one phase with a reproducible receipt, and the re-derivation that voids a receipt which no longer reproduces.
...
- [validation frame](./engineering/validation_frame_doctrine.md#1-native-haskell-is-the-validation-environment) — native, hardware-free Haskell validation first; effectful supervisors use fixed-count challenges and explicit resource envelopes, while a later `amoebius-base` replay checks portability only after that barrier is accepted. Unrelated to *frame config* below: two senses of "frame", one about where replay executes and one about what a running binary reads.
...
- [the handoff](./engineering/bootstrap_sequence_doctrine.md#4-the-host-daemon--control-plane-daemon-handoff) — the instant the host daemon stops being the authority and the control-plane daemon starts.
