...
`data` type on the Phase 3 surface, and bring-up order is *derived*, never Dhall-authored — so no `dhall
...
only Prod advance. Even successful live wiring would be tested, never proven; Phase 7 owns only the pure
