...
async def handle_vault_initialization(secrets_file_path: str) -> None:
    ...
    if local_vault_hosts is None:
        print(f"Error: Output '{output_name}' not found in Terraform state.")
        sys.exit(1)
    if not isinstance(local_vault_hosts, list):
        print(f"Error: Output '{output_name}' is not a list.")
        sys.exit(1)
    # Now MyPy knows local_vault_hosts is List[str]
    ...
    try:
        ...
        if not isinstance(unseal_keys, list):
            print("Error: 'unseal_keys_b64' is not a list.")
            sys.exit(1)
        # Now unseal_keys is List[str]
...
async def handle_vault_unsealing(secrets_file_path: str) -> None:
    ...
    if secrets_dict is None:
        ...
        sys.exit(1)
    # Now we can proceed knowing secrets_dict is not None
    ...
    if not isinstance(unseal_keys, list):
        print("Error: 'unseal_keys_b64' is not a list.")
        sys.exit(1)
    if not isinstance(secret_threshold, int):
        print("Error: 'unseal_threshold' is not an integer.")
        sys.exit(1)
    # Now MyPy knows unseal_keys is List[str] and secret_threshold is int
