...
fixtureContract :: IO ()
fixtureContract = do
  requestBytes <- ByteString.readFile "../test/fixtures/phase_49/request.cbor"
  goldenBytes <- ByteString.readFile "../test/fixtures/phase_49/sibling_golden.cbor"
  ...
  assert (requestText == "phase49-request-v1|tenant=tenant-a|command=phase0-command-49|seed=1|input=content-addressing\n") "phase0-request-cbor"
 ...
 where
  request ready command run input =
    WorkflowRequest
      ...
      , workflowInput = input
      , workflowSeed = 1
      , workflowExperimentHash = "sha256:phase49-linux-cpu-experiment"
