...
runGreen = do
  ...
  expectedBytes <- ByteString.readFile "test/kernel/fixtures/plan/expected_steps.json"
...
planPath kernelCase = "test/kernel/fixtures/plan/" <> Text.unpack (kernelCaseId kernelCase) <> ".plan.golden"

descentPath :: KernelCase -> FilePath
descentPath kernelCase = "test/kernel/fixtures/descent/" <> Text.unpack (kernelCaseId kernelCase) <> ".descent.golden"
