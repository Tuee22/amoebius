...
sourceClosureSelectorIntentsDocumentationCatalog =
  [ ("VALIDATION_SOURCE_CLOSURE_DOCUMENTATION_SUFFIX_REMOVAL_MUTANT", "governed documentation path catalog")
  , ("VALIDATION_SOURCE_CLOSURE_DOCUMENTATION_ROOT_FILE_REMOVAL_MUTANT", "governed documentation path catalog")
  , ("VALIDATION_SOURCE_CLOSURE_DOCUMENTATION_DOCUMENTS_ROOT_REMOVAL_MUTANT", "governed documentation path catalog")
  , ("VALIDATION_SOURCE_CLOSURE_DOCUMENTATION_PLAN_ROOT_REMOVAL_MUTANT", "governed documentation path catalog")
  ]
...
literalIntegrityProblems =
  ...
  | ByteString.length canonicalPbBytes /= 4770
    ...
    -- of this file, while reddening Phase 0 for every selector a later phase
    -- legitimately adds or retires. The structural properties below — no
    -- duplicate selector, no duplicate exact-case label, and the two-way
    -- intent/impact join — are what hold the corpus together.
...
expectedPrimaryClass path
  ...
  | expectedDocumentationPath path = ExpectedDocumentation
  | path `elem` expectedProjectPaths = ExpectedProject
  | otherwise = ExpectedUnregistered

-- | The governed-documentation rule, authored here from the requirement.
--
-- This restated production's 196-path inventory verbatim, including a
-- @CRASH_SUMMARY.md@ that no longer exists, so it could only ever detect that
-- two copies of the same wrong list had diverged. The requirement it stands for
-- is that governed prose is Markdown at the repository root or beneath either
-- governed directory.
expectedDocumentationPath :: FilePath -> Bool
expectedDocumentationPath path =
  ".md" `isSuffixOf` path
    && ( '/' `notElem` path
           || "documents/" `isPrefixOf` path
           || "DEVELOPMENT_PLAN/" `isPrefixOf` path
       )

-- | Representative governed paths for fixtures that need concrete entries: one
-- per admitted class.
