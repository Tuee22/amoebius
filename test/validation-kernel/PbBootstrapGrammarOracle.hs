...
selectorIndependentControlProblems =
  concat
    [ expectEqual "selector control keeps oracle-local fixture size" 4915 (ByteString.length canonicalBytes)
      ...
      expectExact
        ...
        (hardExpected [(canonicalPath, canonicalMode, shortBytes)]
          [preflightFinding "PB-GRAMMAR-BYTE-COUNT-EXACT"
            "expected exactly 4915 bytes; observed 4914"])
  ...
  | target == overByteIdentityCaseLabel =
      expectExact
        ...
        (hardExpected [(canonicalPath, canonicalMode, overByteBytes)]
          [ preflightFinding "PB-GRAMMAR-RESOURCE-LIMIT"
              "source-bytes exceeds the 4915 bound; observed 4916"
          , preflightFinding "PB-GRAMMAR-BYTE-COUNT-EXACT"
              "expected exactly 4915 bytes; observed 4916"
          ])
...
fixtureIntegrityProblems =
  concat
    [ expectEqual "local fixture byte count" 4915 (ByteString.length canonicalBytes)
...
grammarCases =
  ...
  , grammarWithAdditional "indexed monkeypatch targets retain their scope and subject"
      (replaceLine "        environment[\"TMP\"] = str(temporary)" "        environment[\"ZZZ\"] = str(temporary)" canonicalBytes)
...
canonicalProofObservations =
  ...
  , Observation "proof.subject.bytes" "4915"
  ...
  , Observation "proof.expected.sha256" expectedSha256
    ...
    <> [ Observation
       ...
       , Observation "proof.resolved-call-count" "57"
...
expectedControlFlow =
  Text.intercalate
    ...
    , "BootstrapAdapter.environment|18|17|terminal|0"
...
expectedStaticClaims =
  Text.intercalate
    ...
    , "environment|true|CABAL_DIR,GHCUP_INSTALL_BASE_PREFIX,GHCUP_SKIP_UPDATE_CHECK,HOME,PATH,XDG_CACHE_HOME,TMPDIR,TEMP,TMP|PATH,CABAL_DIR,GHCUP_INSTALL_BASE_PREFIX,HOME,XDG_CACHE_HOME,TMPDIR,TEMP,TMP|true"
    ...
    <> case (boundedLength 2 inventory, take 1 inventory) of
      (1, [(path, mode, bytes)]) ->
        ...
        , Observation "input.bytes" (decimal (ByteString.length bytes))
          ...
          <> if ByteString.length bytes <= 4915
            then
...
canonicalMetrics = Metrics 4915 92 403 867 4 74 25 28 0
...
metricObservations metrics =
  ...
  , Observation "limit.source-bytes" "4915"
...
resourceBoundaryFixtures =
  [ ResourceBoundary
      "physical lines"
      astBoundaryBytes
      (Metrics 4915 128 128 128 0 0 0 0 0)
      astOverBytes
      (Metrics 4915 129 129 129 0 0 0 0 0)
      [resourceFinding "physical-lines" 128 129]
  , ResourceBoundary
      "AST nodes"
      astNodeBoundaryBytes
      (Metrics 4915 1 512 512 0 0 0 0 0)
      astNodeOverBytes
      (Metrics 4915 1 513 513 0 0 0 0 0)
      [resourceFinding "ast-nodes" 512 513]
  , ResourceBoundary
      "lexical tokens"
      tokenBoundaryBytes
      (Metrics 4915 1 1 1024 0 0 0 0 0)
      tokenOverBytes
      (Metrics 4915 1 1 1025 0 0 0 0 0)
      [resourceFinding "lexical-tokens" 1024 1025]
  , ResourceBoundary
      "syntax depth"
      depthBoundaryBytes
      (Metrics 4915 1 17 33 16 16 0 0 0)
      depthOverBytes
      (Metrics 4915 1 18 35 17 17 0 0 0)
      [resourceFinding "syntax-depth" 16 17]
  , ResourceBoundary
      "indentation depth"
      indentationBoundaryBytes
      (Metrics 4915 1 1 1 16 0 0 0 0)
      indentationOverBytes
      (Metrics 4915 1 1 1 17 0 0 0 0)
      [resourceFinding "syntax-depth" 16 17]
  , ResourceBoundary
      "call markers"
      callBoundaryBytes
      (Metrics 4915 1 257 513 2 128 0 0 0)
      callOverBytes
      (Metrics 4915 1 259 517 2 129 0 0 0)
      [resourceFinding "resolved-call-markers" 128 129]
  , ResourceBoundary
      "effect markers"
      effectBoundaryBytes
      (Metrics 4915 64 128 192 1 64 64 0 0)
      effectOverBytes
      (Metrics 4915 65 130 195 1 65 65 0 0)
      [resourceFinding "potential-effect-markers" 64 65]
  , ResourceBoundary
      "control-flow markers"
      controlBoundaryBytes
      (Metrics 4915 33 49 113 1 16 0 32 0)
      controlOverBytes
      (Metrics 4915 34 51 118 1 17 0 33 0)
      [resourceFinding "control-flow-markers" 32 33]
  , ResourceBoundary
      "problem markers"
      problemMarkerBoundaryBytes
      (Metrics 4915 1 1 65 0 0 0 0 64)
      problemMarkerOverBytes
      (Metrics 4915 1 1 66 0 0 0 0 65)
...
exactSize bytes
  | ByteString.length bytes > 4915 = ByteString.take 4915 bytes
  | otherwise = case ByteString.unsnoc bytes of
      Nothing -> ByteString8.replicate 4914 ' ' <> "\n"
      Just (prefix, finalByte)
        | finalByte == 10 ->
            prefix
              <> ByteString8.replicate (4915 - ByteString.length bytes) ' '
              <> "\n"
        | otherwise ->
            bytes
              <> ByteString8.replicate (4914 - ByteString.length bytes) ' '
              <> "\n"
...
shortBytes = ByteString.take 4914 canonicalBytes
