...
legacySelectorIntents =
  ...
  , ("VALIDATION_LEGACY_RAW_REVIEWER_INSPECTION_DROP_MUTANT", "reviewer-inspection residue", "canonical legacy wire")
  ...
  , ("VALIDATION_LEGACY_MANDATORY_REVIEWER_INSPECTION_CODE_MUTANT", "Legacy Mandatory reviewer inspection code locus", "canonical legacy wire")
  , ("VALIDATION_LEGACY_MANDATORY_REVIEWER_INSPECTION_DETAIL_MUTANT", "Legacy Mandatory reviewer inspection detail locus", "canonical legacy wire")
  , ("VALIDATION_LEGACY_MANDATORY_REVIEWER_INSPECTION_SUBJECT_MUTANT", "Legacy Mandatory reviewer inspection subject locus", "canonical legacy wire")
...
aggregateFailureCase label bindings joins =
  resourceCase label "00" bindings joins "00" "25" "9"
    ("aggregate-byte-limit:2728:" <> Text.pack (show actual))
    (literalResource "LEGACY-AGGREGATE-BYTE-LIMIT" "<legacy-input>" 2728 actual)
 where
    ...
    , checkFindings =
        mandatoryFindings commitmentDetail
          ...
          <> [Finding "LEGACY-PHASE-BLOCKED" ("phase-" <> Text.unpack safePhase)
                ("every later phase requires its predecessor's external reviewer approval" <> commitmentDetail)
             | phaseBlocked]
    }
 where
...
mandatoryFindings detail =
  ...
  , Finding "LEGACY-REVIEWER-INSPECTION-UNAVAILABLE" "DEVELOPMENT_PLAN/legacy_tracking_for_deletion.md"
      ("reader-facing prose correspondence has not received independent reviewer inspection" <> detail)
  ]
...
canonicalBindings =
  ...
  , ("LTD-VAL-004", "Active", "00", "delegated-promotion", "promotion-authorization", "delegated-promotion", ["reject-unqualified-promotion"])
