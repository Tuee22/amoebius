  ...
  case arguments of
    [argument]
      | Just mutant <- Text.stripPrefix "--mutant=" (Text.pack argument) -> do
          ...
          if caught
            then do
              putStrLn ("phase10-mutant: RED " <> Text.unpack mutant)
              fail ("Phase-10 mutant rejected: " <> Text.unpack mutant)
            else putStrLn ("phase10-mutant: SURVIVED " <> Text.unpack mutant)
    _ -> runBindGate
