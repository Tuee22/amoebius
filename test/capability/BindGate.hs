...
checkGate1 :: IO ()
checkGate1 = do
  rows <- rowsOf "tests/oracle/phase10/gate1_cases.tsv"
...
checkGate2 = do
  ...
  oracle <- rowsOf "tests/oracle/phase10/gate2_cases.tsv"
...
checkLedgerCoverage = do
  rows <- rowsOf "tests/oracle/phase10/validation_locus.tsv"
...
loadArmOracle = do
  rows <- rowsOf "tests/oracle/phase10/arm_cases.tsv"
  traverse parse rows
 where
