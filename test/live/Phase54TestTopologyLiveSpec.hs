...
main = do
  ...
  validate value
  putStrLn "phase54-test-topology-live-gate: PASS-SCOPED (host inventory/forced-failure/SIGINT/idempotent teardown/process takeover; Kubernetes/Pulsar/retained-PV/Vault/AWS UNVERIFIED)"
...
projectRoot = getCurrentDirectory >>= ascend
 where
  ascend path = do
    ...
    if found then pure path else let parent = takeDirectory path in if parent == path then die "phase54-project-root-absent" else ascend parent
