...
main = do
  ...
  verifyEdges
  putStrLn "phase55-ui-single-tenant-live: PASS-SCOPED (pinned access/CSRF/edge oracles; fresh challenge; two-replica routing model; durable receipt authority; provider/edge live infrastructure UNVERIFIED)"
...
verifyFreshEffect coordinated = do
  let nonce = "phase55-fresh-challenge-7f31"
...
projectRoot = getCurrentDirectory >>= ascend
 where
  ascend path = do
    ...
    if found then pure path else let parent = takeDirectory path in if parent == path then die "phase55-project-root-absent" else ascend parent
