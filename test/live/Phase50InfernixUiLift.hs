...
verify (Evidence register substrate result authority browser workflow providers cleanup universal honesty prerequisite) = do
  ...
  case prerequisite of
    Prerequisite fingerprint phase49 -> assert (digestShape fingerprint && phase49 == "PASS-SCOPED") "phase49-prerequisite"
...
projectRoot = getCurrentDirectory >>= ascend
 where
  ascend path = do
    ...
    if found then pure path else let parent = takeDirectory path in if parent == path then die "phase50-project-root" else ascend parent
