...
main :: IO ()
main = do
  legal <- decodeFixture "test/fixtures/phase_34/legal-tenant-graph.json"
  ...
  reuse <- lookupEnv "PHASE34_REUSE_FRESH_LIVE"
  callProcess "python3" (["tools/phase34_tenant_provider_live.py"] <> ["--reuse-fresh-live" | reuse == Just "1"])
...
checkIllegalFixtures = do
  authored <- decodeFixture "test/fixtures/phase_34/illegal-hand-authored-grant.json"
  mismatch <- decodeFixture "test/fixtures/phase_34/illegal-tenant-mismatch.json"
...
checkMatrix actions = do
  source <- TextIO.readFile "test/fixtures/phase_34/provider_projection_matrix.tsv"
