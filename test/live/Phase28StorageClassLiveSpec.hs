...
instance FromJSON Evidence where
  parseJSON = withObject "Phase28StorageClassEvidence" $ \value ->
...
main :: IO ()
main = do
  decoded <- eitherDecodeFileStrict' "DEVELOPMENT_PLAN/evidence/phase_28/sprint-28.1-live.json"
  case decoded of
    Left problem -> die problem
    Right evidence -> verify evidence
  putStrLn "phase28-storageclass-live: PASS (live inert class, wait reason, explicit bind, negative, cleanup)"
