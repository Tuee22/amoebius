...
main = do
  ...
  expectedDigest <- Text.strip <$> TextIO.readFile "test/fixtures/phase32/expected-base-digest.txt"
  either die pure (verify expectedDigest evidence)
  putStrLn "phase32-keycloak-ingress-live-spec: PASS (sole OIDC edge, WebSocket guard, derived policy, exact rebind markers)"
