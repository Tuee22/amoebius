...
main :: IO ()
main = do
  decoded <- eitherDecodeFileStrict' "DEVELOPMENT_PLAN/evidence/phase_30/backbone-live.json"
  evidence <- either die pure decoded
  expectedDigest <- Text.strip <$> TextIO.readFile "test/fixtures/phase30/expected-base-digest.txt"
  either die pure (verify expectedDigest evidence)
  putStrLn "phase30-backbone-live-spec: PASS (external VIP, object migration, native dedup/offload, runtime provenance)"
...
verify expectedDigest evidence = do
  ...
  contains ["pulsar", "drill", "dedupProbeOutput"] "phase30-dedup-probe: PASS"
  ...
  equal ["deferred", "singletonOwnedReconcile"] ("UNVERIFIED" :: Text)
