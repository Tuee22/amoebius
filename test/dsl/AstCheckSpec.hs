...
loadExpectedNegatives = do
  contents <- Text.readFile "test/fixtures/phase14/astcheck/astcheck_negatives.expected"
...
checkSanctionedApi = do
  oracle <- Text.readFile "test/fixtures/phase14/sanctioned_api_expected.dhall"
...
rejectMutant name caught =
  if caught
    then putStrLn ("phase14-ast-mutant: RED " <> name) >> fail ("Phase-14 AST mutant rejected: " <> name)
    else putStrLn ("phase14-ast-mutant: SURVIVED " <> name)

fixturePath :: FilePath -> FilePath
fixturePath name = "test/fixtures/phase14/astcheck/" <> name
