...
runMutantMode root = do
  ...
  case outcome of
    Upheld -> putStrLn "phase15-sim-mutant: SURVIVED dropped-partition-handling"
    Violated invariant ->
      putStrLn ("phase15-sim-mutant: RED dropped-partition-handling " <> Text.unpack invariant)
  exitFailure

loadSchedules :: FilePath -> IO [FaultSchedule]
loadSchedules root = do
  let directory = root </> "test/sim/schedules"
...
loadExpected root = do
  source <- readFile (root </> "test/sim/schedules/expected_outcomes.tsv")
