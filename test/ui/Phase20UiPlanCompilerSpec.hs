...
mutants :: [Mutant]
mutants =
  [ Mutant "M-drop-server-action" "workflow.start" "phase_20_M-drop-server-action.mutant" "dropped-effect"
  , Mutant "M-swap-action-targets" "handler-projection" "phase_20_M-swap-action-targets.mutant" "effect-swap"
  , Mutant "M-emit-private-field" "public-allowlist" "phase_20_M-emit-private-field.mutant" "projection-guard-deletion"
  , Mutant "M-client-only-authority-digest" "authority-digest" "phase_20_M-client-only-authority-digest.mutant" "invariant-clause-delete"
  , Mutant "M-link-navigation-as-fetch" "docs.link" "phase_20_M-link-navigation-as-fetch.mutant" "effect-swap"
  , Mutant "M-preserve-map-insertion-order" "fresh-process-bytes" "phase_20_M-preserve-map-insertion-order.mutant" "determinism-violation"
  ]
...
buildFixture root reverseOrder = do
  rows <- loadTable (root </> "test/fixtures/ui_plan_compiler/projection_rows.tsv")
  authRows <- loadTable (root </> "test/fixtures/ui_authorization/action_registry.tsv")
...
runMutant root fixture name = case firstMatching ((== name) . mutantName) mutants of
  ...
  Just mutant -> do
    ...
    checkMutantControls root fixture compiled
    putStrLn ("phase20-plan-mutant: RED " <> mutantName mutant <> " " <> mutantLocus mutant)
    exitFailure
