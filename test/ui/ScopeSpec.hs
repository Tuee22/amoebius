...
checkOwnerOracle root fixture = do
  rows <- loadTable (root </> "test/fixtures/ui_scope/owner_join_table.tsv")
...
checkFlowOracle root fixture = do
  rows <- loadTable (root </> "test/fixtures/ui_scope/flow_matrix.tsv")
...
checkDecodeOracle root = do
  rows <- loadTable (root </> "test/fixtures/ui_scope/decode_errors.tsv")
  assertEqual "decode error rows"
    ...
    rows
  let compileRoot = root </> "test/fixtures/ui_scope/compile_fail"
