...
mutants :: [Mutant]
mutants =
  [ Mutant "M-first-handler-wins" "duplicate-handler" "phase_19_M-first-handler-wins.mutant" "quantifier-weakening"
  , Mutant "M-drop-capability" "missing-capability" "phase_19_M-drop-capability.mutant" "guard-deletion"
  , Mutant "M-erase-handler-scope" "scope-mismatch" "phase_19_M-erase-handler-scope.mutant" "scope-guard-deletion"
  , Mutant "M-swap-response-codec" "codec-mismatch" "phase_19_M-swap-response-codec.mutant" "effect-type-swap"
  , Mutant "M-retry-without-idempotency" "unsafe-retry" "phase_19_M-retry-without-idempotency.mutant" "invariant-clause-delete"
  , Mutant "export_raw_topic" "raw-topic" "phase_19_export_raw_topic.mutant" "provider-coordinate-escape"
  , Mutant "M-link-id-as-url" "link-as-url" "phase_19_M-link-id-as-url.mutant" "escape-arm-addition"
  ]
...
buildFixture root = do
  portRows <- loadTable (root </> "test/fixtures/ui_effect_binding/ports.tsv")
  handlerRows <- loadTable (root </> "test/fixtures/ui_effect_binding/handlers.tsv")
  capabilityRows <- loadTable (root </> "test/fixtures/ui_effect_binding/capabilities.tsv")
  linkRows <- loadTable (root </> "test/fixtures/ui_effect_binding/external_link_catalog.tsv")
...
buildAuthorizationRegistry root = do
  decoded <- decodeUiSource (root </> "test/fixtures/ui_program_schema/minimal_single_tenant.dhall")
  ...
  actionRows <- loadTable (root </> "test/fixtures/ui_authorization/action_registry.tsv")
...
checkBindingOracle root fixture = do
  ...
  handlerRows <- loadTable (root </> "test/fixtures/ui_effect_binding/handlers.tsv")
  capabilityRows <- loadTable (root </> "test/fixtures/ui_effect_binding/capabilities.tsv")
  expected <- loadTable (root </> "test/fixtures/ui_effect_binding/expected_bindings.tsv")
...
checkExternalLinkOracle root fixture = do
  expected <- loadTable (root </> "test/fixtures/ui_effect_binding/expected_external_links.tsv")
  catalog <- loadTable (root </> "test/fixtures/ui_effect_binding/external_link_catalog.tsv")
...
checkPinnedErrors root fixture = do
  rows <- loadTable (root </> "test/fixtures/ui_effect_binding/bind_errors.tsv")
...
runMutant root fixture name = case firstMatching ((== name) . mutantName) mutants of
  ...
  Just mutant -> do
    checkMutantControls root fixture
    putStrLn ("phase19-bind-mutant: RED " <> mutantName mutant <> " " <> mutantCase mutant)
    exitFailure
