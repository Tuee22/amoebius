...
runMutant root binary name = case lookup name mutants of
  ...
  Just locus -> do
    let command = proc "node" [root </> "test/ui/server/phase22_server_boundary.mjs", "run", binary, name]
    (code, output, errors) <- readCreateProcessWithExitCode command ""
    let token = "phase22-server-mutant: RED " <> name <> " " <> locus
...
runHarness root binary mutant = do
  let arguments = [root </> "test/ui/server/phase22_server_boundary.mjs", "run", binary]
...
checkMutantFixtures root = forM_ mutants $ \(name, _locus) -> do
  let path = root </> "test/mutants" </> ("phase_22_" <> name <> ".mutant")
