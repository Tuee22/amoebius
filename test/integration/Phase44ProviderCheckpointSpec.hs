...
data Singleton = Singleton Int Int Int Bool
...
instance FromJSON Singleton where
  parseJSON = withObject "Singleton" $ \value ->
    Singleton <$> value .: "replicas" <*> value .: "readyReplicas" <*> value .: "availableReplicas" <*> value .: "bespokeElection"
...
verify evidence = do
  ...
  case evidenceSingleton evidence of
    Singleton replicas ready available election ->
      assert (replicas == 1 && ready == 1 && available == 1 && not election) "singleton"
