...
main = do
  ...
  case arguments of
    ...
    _ -> die "usage: phase31-services-spec [--render-plan]"
...
verifyAll postgres observability redis rendered = do
  patroniOracle <- TextIO.readFile "test/fixtures/phase31/patroni-sync-config.golden"
  ...
  dagOracle <- TextIO.readFile "test/fixtures/phase31/dag-edges.golden"
  ...
  putStrLn "phase31-services-spec: PASS (Patroni strict sync, derived monitoring/Redis bounds, independent readiness DAG, 11 Haskell projections)"
