...
main = do
  ...
  case arguments of
    ["--render-plan"] -> Lazy.putStr (encode (renderBackbone loadBalancer minio pulsar <> renderRegistryRehome privateImage registryResources registry))
    [] -> do
      verifyMinio minio
      verifyRegistry registry
      verifyPulsar pulsar
      verifyBackbone minio pulsar
      putStrLn "phase30-backbone-spec: PASS (HA shape, physical storage, verified registry rehome, size offload, readiness DAG)"
    _ -> die "usage: phase30-backbone-spec [--render-plan]"
...
verifyRegistry provision = do
  expected <- TextIO.readFile "test/fixtures/phase30/registry-storage-driver.golden"
