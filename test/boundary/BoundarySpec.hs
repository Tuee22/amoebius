...
main = do
  arguments <- getArgs
  withSystemTempDirectory "amoebius-phase14-boundary" $ \runDirectory -> do
...
runBoundary runDirectory = do
  ...
  fakeDirectory <- makeAbsolute "test/boundary/fakes"
  ...
  let hostileEnvironment =
      ...
      manifest = root </> "test/manifest/golden/objectstore_singlenode.json.golden"
      command =
...
expectedTranscripts =
  [ ("kubectl.1.argv", "test/boundary/golden/argv/kubectl.1.argv.golden")
  , ("docker.1.argv", "test/boundary/golden/argv/docker.1.argv.golden")
  , ("docker.2.argv", "test/boundary/golden/argv/docker.2.argv.golden")
  , ("pulumi.1.argv", "test/boundary/golden/argv/pulumi.1.argv.golden")
  ]
...
reject name caught =
  if caught
    then putStrLn ("phase14-boundary-mutant: RED " <> name) >> fail ("Phase-14 boundary mutant rejected: " <> name)
    else putStrLn ("phase14-boundary-mutant: SURVIVED " <> name)
