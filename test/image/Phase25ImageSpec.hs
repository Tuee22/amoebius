...
main = do
  ...
  putStrLn "phase25-image-spec: PASS (Phase 25 image build, registry/bootstrap boundaries, immutable atomic publication, enforced private pull, and 13 committed mutants)"
...
verifyIndependentInventory catalog = do
  expected <- Dhall.inputFile Dhall.auto "test/fixtures/phase25/bake_inventory_expected.dhall" :: IO [OracleInventory]
  assertEqual "oracle inventory count" 23 (length expected)
  assertEqual "catalog inventory count" 23 (length (Catalog.catalogSteps catalog))
  ...
  forM_ expected $ \row -> do
    ...
    case Map.lookup row.catalogName catalogRows of
      ...
      Just (catalogVersion, image, artifactDigest) -> do
        assertEqual (Text.unpack row.catalogName <> " version") row.version catalogVersion
        assertEqual (Text.unpack row.catalogName <> " source image") row.sourceImage image
        assertEqual (Text.unpack row.catalogName <> " source digest") row.sourceDigest artifactDigest
  ...
  case Map.lookup "redis-cli" catalogRows of
    Just (actualVersion, _, _) -> unless (actualVersion /= "7.4.4") (die "redis-version-skew mutant stayed green")
 ...
 where
  catalogRow step = case step of
    ...
    Catalog.BuildProduct built -> (built.name, (built.expectedVersion, "amoebius-source", "sha256:source-bound-at-build"))

verifyBuildAdmission :: IO ()
verifyBuildAdmission = do
  oracle <- Dhall.inputFile Dhall.auto "test/fixtures/phase25/build_execution_envelope.dhall" :: IO BuildOracle
  ...
  let envelope = oracleEnvelope oracle
      ...
      request =
        Runtime.BuildRequest
          ...
          , Runtime.buildBuilderName = "amoebius-phase25"
          ...
          , Runtime.buildBuildkitContainer = "amoebius-phase25-buildkitd"
          , Runtime.buildStateVolume = "amoebius-phase25-buildkit-state"
          }
  target <- Admission.admitBuildTarget envelope observed >>= either (die . show) pure
    ...
    actualInvocation : _ ->
      assertEqual
        ...
        , [ "--config", "/scratch/docker-config"
          , "buildx", "build", "--builder", "amoebius-phase25"
...
verifyDockerfile catalog = do
  ...
  expected <- Text.readFile "test/fixtures/phase25/Dockerfile.golden"
  ...
  forM_ ["RUN ", "http://", "https://", ":latest"] $ \forbidden ->
...
verifyRegistryStorage = do
  oracle <- Dhall.inputFile Dhall.auto "test/fixtures/phase25/registry_storage_demand.dhall" :: IO RegistryOracle
...
verifyBootstrapRegistry = do
  oracle <- Dhall.inputFile Dhall.auto "test/fixtures/phase25/bootstrap_registry_domain.dhall" :: IO BootstrapOracle
...
verifyRegistryPullGate = do
  endpointText <- Text.readFile "test/fixtures/phase25/public_registry_endpoints.txt"
  expectedFailure <- Text.readFile "test/fixtures/phase25/expected_pull_failure.txt"
...
bootstrapSpec oracle = do
  registryOracle <- Dhall.inputFile Dhall.auto "test/fixtures/phase25/registry_storage_demand.dhall" :: IO RegistryOracle
  let artifact = validArtifact
...
verifyMutantArtifacts = do
  ...
  forM_ names $ \name -> do
    contents <- readFile ("mutants/phase25/" <> name <> ".mutant")
