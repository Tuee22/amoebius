...
verifyDockerfile catalog = do
  ...
  let emitted = Text.lines rendered.dockerfileText
      ...
      authoredUrls =
        Set.fromList
          (concatMap (\artifact -> concatMap (\asset -> [asset.assetUrl, asset.checksumManifest]) artifact.assets) artifacts)
      emittedUrls = Set.fromList (concatMap urlsIn emitted)
  assertEqual "one RUN per apt layer and per official artifact" (aptLayers + length artifacts) (length runLines)
