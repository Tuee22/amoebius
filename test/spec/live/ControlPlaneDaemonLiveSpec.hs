...
main :: IO ()
main = do
  decoded <- eitherDecodeFileStrict' "DEVELOPMENT_PLAN/evidence/phase_33/control-plane-live.json"
  evidence <- either die pure decoded
  expectedValue <- eitherDecodeFileStrict' "test/fixture/live_dsl_deploy/expected-enact-pass1.json"
  ...
  putStrLn "live-dsl-deploy-live-spec: PASS (Lease handoff, exact control-plane daemon effects, admin boundary, durable replacement, teardown)"

verify :: [Text] -> Value -> Either String ()
verify expected evidence = do
  equal ["schema"] ("amoebius.live-dsl-deploy.v1" :: Text)
  ...
  equal ["manifest", "fieldManager"] ("amoebius-live-dsl-deploy" :: Text)
  ...
  true ["handoff", "noControlPlaneDaemonMutationBeforeAcquire"]
  ...
  equal ["edge", "body"] ("live-dsl-deploy-trivial:/probe" :: Text)
  true ["edge", "oidcOwned"]
  ...
  equal ["passwordObserver", "controlPlaneLogs"] ("clear" :: Text)
  ...
  positive ["attribution", "controlPlaneWriteCount"]
  ...
  true ["artifactSource", "haskellControlPlaneDaemon"]
