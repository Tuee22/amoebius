...
main = do
  ...
  verifyCustody
  let content = "offline-blobs-isolation-reference-content"
  ...
  assertLeft "nonowner chunk" (appendChunk nonOwner 0 "offline-blobs-isolation-" started)
  assertLeft "foreign chunk" (appendChunk foreignScope 0 "offline-blobs-isolation-" started)
  first <- either (die . show) pure (appendChunk scope 0 "offline-blobs-isolation-" started)
...
projectRoot = getCurrentDirectory >>= go
  where
    go path = do
      ...
      if found then pure path else let parent = takeDirectory path in if parent == path then die "offline-blobs-isolation-root" else go parent
