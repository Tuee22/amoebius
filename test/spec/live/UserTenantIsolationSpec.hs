...
main = do
  ...
  root <- projectRoot
  rows <- loadMatrix (root </> "test/fixture/live_isolation/user_tenant_access_matrix.tsv")
  checkMatrix rows
  checkProvision
  reuse <- lookupEnv "USER_TENANT_ISOLATION_REUSE_FRESH_LIVE"
  if reuse == Just "1"
...
runLive :: FilePath -> IO ()
runLive root = do
  raw <- readProcess "python3" ["tools/user_tenant_isolation_live.py", "setup"] ""
  setup <- either die pure (eitherDecodeStrict' (Text.encodeUtf8 (Text.strip (Text.pack raw))))
  let resultPath = "/tmp/amoebius-user-tenant-isolation-result-" <> Text.unpack (challenge setup) <> ".json"
      cleanup = callProcess "python3" ["tools/user_tenant_isolation_live.py", "cleanup", "--state", stateFile setup]
  (do
      results <- mapM (runPulsarRound setup) (namespaces setup)
      Lazy.writeFile resultPath (encode results)
      callProcess "python3" ["tools/user_tenant_isolation_live.py", "finish", "--state", stateFile setup, "--result", resultPath]
    ) `finally` cleanup
...
validateEvidence root = do
  ...
  unless present (die "user-tenant-isolation-live-evidence-absent")
  ...
  case value of
    ...
    _ -> die "user-tenant-isolation-live-evidence-not-object"
 where
...
testCredential subject tenantName = do
 ...
 where
  claims = Text.intercalate "|" [subject, tenantName, "write", "active", "36", "user-tenant-isolation-session"]
  signature value = hex (SHA256.hmac (Text.encodeUtf8 signingKeyText) (Text.encodeUtf8 value))
  signingKeyText = "user-tenant-isolation-authority-signing-key-000000000001"
