...
main = do
  ...
  expectedDigest <- Text.strip <$> TextIO.readFile "test/fixture/platform_services_2/expected-base-digest.txt"
  shareFixture <- Text.words <$> TextIO.readFile "test/fixture/platform_services_2/postgres-share-package.sha256"
  ...
  putStrLn "platform-services-2-services-live-spec: PASS (Patroni, pgAdmin, Redis failover, bounded Prometheus, Grafana/Postgres, readiness trace)"
