...
main = do
  ...
  pureOnly <- lookupEnv "UI_PROGRAM_RELEASE_PURE_ONLY"
  reuse <- lookupEnv "UI_PROGRAM_RELEASE_REUSE_FRESH_LIVE"
...
loadFixtures root = do
  plans <- parseTsv (root </> "test/fixture/ui_program_release/plan_pair_matrix.tsv") $ \fields -> case fields of
    [client, server, authority, outcome, count] -> PlanMatrixRow client server authority outcome <$> parseInt count
    _ -> die "ui-program-release-plan-pair-matrix-shape"
  stale <- parseTsv (root </> "test/fixture/ui_program_release/stale_digest_matrix.tsv") $ \fields -> case fields of
    ...
    _ -> die "ui-program-release-stale-matrix-shape"
  assertEqual "ui-program-release-plan-matrix-domain" 6 (length plans)
  assertEqual "ui-program-release-stale-matrix-domain" 5 (length stale)
  custody <- filter (Text.isPrefixOf "40\t") . Text.lines <$> TextIO.readFile (root </> "test/oracle/preimplementation_artifacts.tsv")
  assertEqual "ui-program-release-phase0-custody" 8 (length custody)
  pure (plans, stale)
...
parseInt value = case reads (Text.unpack value) of
  [(number, "")] -> pure number
  _ -> die ("ui-program-release-invalid-integer:" <> Text.unpack value)
...
validateProjection root runtime releaseA releaseB = do
  ...
  expectedKeys <- filter (not . Text.null) . Text.lines <$> TextIO.readFile (root </> "test/fixture/ui_program_release/source_key_set.txt")
  assertEqual "ui-program-release-source-key-set" expectedKeys actualKeys
  assertWith "ui-program-release-rebuild-runtime-per-program:" $
    manifestRuntimeImage manifestA == runtime && manifestRuntimeImage manifestB == runtime
  assertWith "ui-program-release-program-change-content-identity" $
  ...
  assertWith "ui-program-release-publish-mixed-plan-pair:" $ case publishPlanPair (Just (pairClient pairA)) (Just (pairServer pairB)) of
    Left (MixedProgramRevision RevisionA RevisionB) -> True
    _ -> False
  assertEqual "ui-program-release-client-half-required" (Left ClientPlanMissing) (publishPlanPair Nothing (Just (pairServer pairA)))
  assertEqual "ui-program-release-server-half-required" (Left ServerPlanMissing) (publishPlanPair (Just (pairClient pairA)) Nothing)
  dhall <- TextIO.readFile (root </> "test/fixture/dhall/ui_program_release/ui_program_release.dhall")
  forM_ ["label = \"A\"", "label = \"B\"", "policyEpoch = 1", "policyEpoch = 2", "runtimeAbi = \"ui-runtime-v1\""] $ \needle ->
    assertWith "ui-program-release-dhall-representative-set" (needle `Text.isInfixOf` dhall)
  golden <- TextIO.readFile (root </> "test/fixture/ui_program_release/release_content_manifest.golden")
  assertEqual "ui-program-release-release-manifest-oracle" golden normalizedManifest
  forM_ [manifestA, manifestB] $ \manifest -> do
    assertWith "ui-program-release-websocket-identity" (manifestWebSocketSubprotocol manifest == "amoebius.ui.v1")
    assertWith "ui-program-release-routing-identity" (manifestRoutingEnvelopeSchema manifest == "amoebius.routing.v1")
    assertWith "ui-program-release-cursor-identity" (manifestCursorCodec manifest == "amoebius.cursor.v1")
...
validateMatrices (plans, staleRows) releaseA releaseB = do
  planCases <- forM plans $ \(PlanMatrixRow client server authority expected count) -> do
    ...
    assertEqual "ui-program-release-plan-matrix-outcome" expected (Text.pack (show result))
    assertEqual "ui-program-release-plan-matrix-effect" count effectCount
  ...
  staleCases <- forM staleRows $ \(StaleMatrixRow name client server clientAuthority serverAuthority expected count) -> do
    let current = releaseFor serverAuthority releaseA releaseB
        ...
        effectCount = if result == Accepted then 1 else 0
    assertWith (if name == "stale-authority" then "ui-program-release-accept-stale-authority-digest:" else "ui-program-release-stale-admission:") $
      Text.pack (show result) == expected
    assertEqual "ui-program-release-stale-effect" count effectCount
    pure CaseResult
  ...
  assertEqual "ui-program-release-authorized-effect-count" 2 (sum (map caseEffectCount planCases))
  assertEqual "ui-program-release-stale-current-effect-count" 1 (sum (map caseEffectCount staleCases))
...
runLive root releases cases = do
  let helper = root </> "tools/ui_program_release_live.py"
  (preflightPath, preflightHandle) <- openTempFile "/tmp" "amoebius-ui-program-release-preflight.json"
  ...
  let resultPath = "/tmp/amoebius-ui-program-release-result-" <> Text.unpack (challenge setup) <> ".json"
...
decodeSetup raw = case lines raw of
  [] -> Left "ui-program-release-live-setup-empty"
...
validateEvidence root = do
  ...
  unless exists (die "ui-program-release-live-evidence-absent")
  raw <- TextIO.readFile path
  forM_ ["\"sealed\": true", "\"register\": 3", "\"substrate\": \"linux-cpu\"", "\"allHardwareSubstrates\": true", "\"Incus\"", "\"Lima\"", "\"WSL2\""] $ \needle ->
    assertWith "ui-program-release-live-evidence-field" (needle `Text.isInfixOf` raw)
 ...
 where
  search directory = do
    ...
    if found then pure directory else do
      ...
      if parent == directory then die "ui-program-release-project-root-not-found" else search parent
