...
main = do
  ...
  pureOnly <- lookupEnv "UI_PROJECTION_RUNTIME_PURE_ONLY"
  reuse <- lookupEnv "UI_PROJECTION_RUNTIME_REUSE_FRESH_LIVE"
...
loadFixtures root = do
  let fixture name = root </> "test/fixture/phase_38" </> name
...
parseInt value = case reads (Text.unpack value) of
  [(number, "")] -> pure number
  _ -> die ("ui-projection-runtime-invalid-integer:" <> Text.unpack value)
...
runFixtureOracle (matrix, latest, receipts, watermarks) suffix = do
  ...
  assertEqual "ui-projection-runtime-conflict-cardinality" 1 (length (filter isConflict outcomes))
  forM_ latest $ \(LatestRow tenantName owner entity expected) -> do
    ...
    unless (actual == Just expectedValue) $
      die ("ui-projection-runtime-drop-owner-key:expected=" <> Text.unpack expectedValue <> ":actual=" <> show actual)
  forM_ receipts $ \(ReceiptRow tenantName owner command status effectCount) ->
    if "/changed-input" `Text.isSuffixOf` command
      then assertEqual "ui-projection-runtime-conflicting-input-effect-count" 0 effectCount
      else do
        let receipt = lookupReceipt state (ReceiptKey (coordinate tenantName owner) command)
        assertEqual "ui-projection-runtime-receipt-present" True (maybe False (const True) receipt)
        assertEqual "ui-projection-runtime-receipt-status" status (maybe "missing" (Text.pack . show . durableOutcome) receipt)
        assertEqual "ui-projection-runtime-receipt-original-command" (Just command) (receiptCommandId . durableReceiptKey <$> receipt)
        assertEqual "ui-projection-runtime-receipt-effect-count" 1 effectCount
  forM_ watermarks $ \(WatermarkRow tenantName owner cursor expectedResume) -> do
    let projection = projectionFor tenantName owner
        watermark = lookupWatermark state projection
    assertEqual "ui-projection-runtime-watermark" (Just (StreamCursor cursor)) (watermarkCursor <$> watermark)
    ...
    unless (decision == Just (if cursor == 0 then ResumeCaughtUp else ResumeReplayAfter resumeCursor))
      (die "ui-projection-runtime-resume-decision")
    assertEqual "ui-projection-runtime-resume-value" (Just (expectedResume <> suffix)) (rowValue <$> lookupProjection state projection "entity-1")
  pure state
 where
  applyOne (state, outcomes) (index, event) = case applyProjectionEvent state event of
    Left problem -> die ("ui-projection-runtime-drop-owner-subscription:" <> show problem)
    Right (updated, outcome)
      | isConflictOutcome outcome && index /= 7 -> die "ui-projection-runtime-drop-receipt-command-id:premature-idempotency-conflict"
      | index == 7 && not (isConflictOutcome outcome) -> die "ui-projection-runtime-drop-receipt-command-id:conflicting-input-accepted"
...
checkAuthorityQueries state = do
  seal <- maybe (die "ui-projection-runtime-handle-seal") pure (handleSeal "ui-projection-runtime-private-handle-seal-0000000000001")
  ...
  assertEqual "ui-projection-runtime-own-query" (Right (Just "value-a4")) (fmap (fmap rowValue . fst) own)
  assertEqual "ui-projection-runtime-foreign-owner-public-denial" (Left ResourceUnavailable) foreignOwner
  assertEqual "ui-projection-runtime-foreign-tenant-public-denial" (Left ResourceUnavailable) foreignTenant
  assertEqual "ui-projection-runtime-swapped-handle-public-denial" (Left ResourceUnavailable) swapped
  assertEqual "ui-projection-runtime-stale-epoch-public-denial" (Left ResourceUnavailable) stale
...
credential subject tenantName epoch = do
 ...
 where
  claims = Text.intercalate "|" [subject, tenantName, "read", "active", Text.pack (show epoch), "ui-projection-runtime-session"]
  signature value = hex (SHA256.hmac (Text.encodeUtf8 signingKeyText) (Text.encodeUtf8 value))
  signingKeyText = "ui-projection-runtime-authority-signing-key-000000000001"

runLive :: FilePath -> Fixtures -> IO ()
runLive root fixtures = do
  raw <- readProcess "python3" ["tools/ui_projection_runtime_live.py", "setup"] ""
  setup <- either die pure (eitherDecodeStrict' (Text.encodeUtf8 (Text.strip (Text.pack raw))))
  let resultPath = "/tmp/amoebius-ui-projection-runtime-result-" <> Text.unpack (challenge setup) <> ".json"
      cleanup = callProcess "python3" ["tools/ui_projection_runtime_live.py", "cleanup", "--state", stateFile setup]
  (do
      result <- liveProjectionRound fixtures setup
      Lazy.writeFile resultPath (encode result)
      callProcess "python3" ["tools/ui_projection_runtime_live.py", "finish", "--state", stateFile setup, "--result", resultPath]
    ) `finally` cleanup
...
liveProjectionRound fixtures setup = withBrokerForward (brokerPod setup) $ do
  ...
  (states, inputKeys, projectionKeys, receiptKeys) <- withNativeClients 5 broker $ \clients -> do
    putStrLn "ui-projection-runtime-live-stage:input-consumers"
    consumers <- sequence
      [ Consumer.newConsumer client inputTopic subscription Exclusive
      | (client, subscription) <- zip clients subscriptions
      ]
    inputObserver <- Consumer.newConsumer (clients !! 3) inputTopic "ui-projection-runtime-independent-input-observer" Exclusive
    producer <- Producer.newProducer (clients !! 4) inputTopic "ui-projection-runtime-workflow-event-producer"
    ...
    ownerSlots <- forM (zip consumers owners) $ \(consumer, owner) -> do
      ...
      pure slot
    putStrLn "ui-projection-runtime-live-stage:input-publish"
    forM_ (zip [0 :: Word64 ..] events) $ \(sequenceId, event) -> do
      ...
      pure ()
    putStrLn "ui-projection-runtime-live-stage:owner-consume"
    ownerStates <- mapM (takeMVar >=> fromCaptured) ownerSlots
    putStrLn "ui-projection-runtime-live-stage:input-observer"
    observed <- takeMVar observerSlot >>= fromCaptured
    putStrLn "ui-projection-runtime-live-stage:derived-consumers"
    projectionObserver <- Consumer.newConsumer (clients !! 0) projectionTopic "ui-projection-runtime-independent-projection-observer" Exclusive
    receiptObserver <- Consumer.newConsumer (clients !! 1) receiptTopic "ui-projection-runtime-independent-receipt-observer" Exclusive
    projectionProducer <- Producer.newProducer (clients !! 4) projectionTopic "ui-projection-runtime-projection-worker"
    receiptProducer <- Producer.newProducer (clients !! 4) receiptTopic "ui-projection-runtime-receipt-worker"
    ...
    putStrLn "ui-projection-runtime-live-stage:projection-publish"
  ...
  applyOwned state received = do
    ...
    if projectionOwner (uiEventProjection event) /= owner
      then pure state
        ...
        Left problem -> die ("ui-projection-runtime-live-cursor:" <> show problem)
...
receiveMany consumer count = forM [1 .. count] $ \_ -> do
  ...
  case outcome of
    Nothing -> die "ui-projection-runtime-pulsar-receive-timeout"
    Just (Left problem) -> die ("ui-projection-runtime-pulsar-decode:" <> show problem)
...
receiveManyNamed label consumer count = forM [1 .. count] $ \index -> do
  outcome <- timeout (30 * 1000000) (Consumer.receive consumer)
  case outcome of
    Nothing -> die ("ui-projection-runtime-pulsar-receive-timeout:" <> label <> ":" <> show index)
    Just (Left problem) -> die ("ui-projection-runtime-pulsar-decode:" <> label <> ":" <> show index <> ":" <> show problem)
    Just (Right value) -> do
      putStrLn ("ui-projection-runtime-live-received:" <> label <> ":" <> show index)
...
withBrokerForward pod action = bracket start stop (const action)
 where
  start = do
    ...
    handle <- maybe (die "ui-projection-runtime-port-forward-output-missing") pure outputHandle
  ...
  waitForward process handle = do
    ...
    if "Forwarding from" `Text.isInfixOf` Text.pack line
      then pure ()
        ...
        case status of
          Just code -> die ("ui-projection-runtime-port-forward-exit:" <> show code <> ":" <> line)
...
validateEvidence root = do
  ...
  unless present (die "ui-projection-runtime-live-evidence-absent")
  value <- either die pure =<< eitherDecodeFileStrict' path :: IO Value
  case value of
    Object object -> do
      assertEqual "ui-projection-runtime-evidence-seal" (Just (Bool True)) (object .:? "sealed")
      assertEqual "ui-projection-runtime-evidence-schema" (Just (String "amoebius.phase38.ui-projection-runtime-live.v1")) (object .:? "schemaVersion")
      assertEqual "ui-projection-runtime-evidence-substrate" (Just (String "linux-cpu")) (object .:? "substrate")
    _ -> die "ui-projection-runtime-live-evidence-not-object"
 where
