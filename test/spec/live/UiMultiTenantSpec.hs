...
main = do
  ...
  putStrLn "ui-multi-tenant-live-ui-multi-tenant-live: PASS-SCOPED (opaque choice; current membership; epoch rotation; stale handle refusal; tenant/subject/epoch keys; Keycloak/browser/provider live surfaces UNVERIFIED)"
...
projectRoot = getCurrentDirectory >>= ascend where ascend path = do found <- doesFileExist (path </> "cabal.project"); if found then pure path else let parent=takeDirectory path in if parent==path then die "ui-multi-tenant-live-root" else ascend parent
