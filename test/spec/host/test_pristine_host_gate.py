...
from pristine_host_gate import (  # noqa: E402
    GateConfig,
    GateError,
    Provider,
    command_plan,
...
class Phase24PristineGateSpec(unittest.TestCase):
    ...
    def test_incus_plan_is_a_vm_without_gpu_passthrough(self) -> None:
        ...
        self.assertEqual(plan.destroy[-1][-2:], ("--force", "amoebius-bootstrap-coordinator-pristine"))
    ...
    def test_unified_backing_is_a_finite_ext4_loop_filesystem(self) -> None:
        ...
        self.assertIn("/root/amoebius/.data/bootstrap-coordinator/unified", source)
    ...
    def test_m3_build_uses_the_contained_cabal_environment(self) -> None:
        source = inspect.getsource(observe_one_shot_guard_mutant)
        self.assertIn("export HOME={GUEST_TOOL_HOME}", source)
        self.assertIn("export XDG_CACHE_HOME={GUEST_ROOT}/.build/toolchain/cache/xdg", source)
        self.assertIn("--store-dir={GUEST_ROOT}/.build/cabal-store build", source)
        self.assertIn("--builddir={GUEST_ROOT}/.build/dist-newstyle/bootstrap-coordinator-mutant", source)
        self.assertIn("./amoebius.cabal:exe:amoebius", source)
    ...
    def test_wsl2_plan_requires_and_imports_a_fresh_rootfs(self) -> None:
        ...
        config = GateConfig(
            ...
            wsl_install_dir="C:/vm/amoebius-bootstrap-coordinator-pristine",
        )
