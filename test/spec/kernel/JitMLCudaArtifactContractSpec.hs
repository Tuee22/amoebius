...
main = do
  ...
  let revision = PointerRevision "etag-jitml-lift-cuda-1"
...
baseRequest =
  ScopedTrainingRequest
    ...
    , trainingBatch = ByteString.pack "jitml-lift-cuda-batch|phase0-challenge-51"
...
validatePhase0 root = do
  ...
  dhall <- TextIO.readFile (root </> "test/fixture/dhall/jitml_lift_cuda/jitml_cuda_artifact.dhall")
  ...
  ascend path = do
    ...
    if found then pure path else
      let parent = takeDirectory path
       in if parent == path then die "jitml-lift-cuda-project-root-absent" else ascend parent
