...
import ObjectReconcilerSimCommon
...
classes :: [FaultClass]
classes = [LeaseAcquireFault, LeaseRenewAmbiguity, ScopedSsaConflict, AuthenticatedDeleteConflict, ReadinessInterruption]
...
runGreen = do
  ...
  forM_ [LostLeaseResourceVersionRetry, MutationWithoutHolder, SleepGatedReadiness, LabelOnlyDelete, CachedObservation] checkMutantKilled
  putStrLn "reconcile-sim: PASS (5 fault classes x 256 schedules; IOSimPOR bound 24; 5 mutants red)"
...
checkPor fault = do
  let schedule = Phase26Schedule 73 fault
      ...
      args = stdArgs {maxSuccess = 1, replay = Just (mkQCGen 73, 0), chatty = False}
...
runMutant raw = case parseMutant raw of
  ...
  Just mutant -> case mutantOutcome mutant of
    Upheld -> putStrLn ("phase26-reconcile-mutant: SURVIVED " <> raw)
    Violated invariant -> putStrLn ("phase26-reconcile-mutant: RED " <> raw <> " " <> Text.unpack invariant)
  >> exitFailure
