...
main = do
  ...
  routeOracle <- TextIO.readFile "test/fixture/keycloak_ingress/route-inventory.golden"
  assertEqual "route inventory oracle" routeOracle (renderRoutes (provisionedRoutes edge))
  policyOracle <- TextIO.readFile "test/fixture/keycloak_ingress/netpol-expected.golden"
  ...
  putStrLn "keycloak-ingress-edge-spec: PASS (Keycloak-only OIDC edge, exact routes, derived policies, TLS provenance, recreate witness)"
