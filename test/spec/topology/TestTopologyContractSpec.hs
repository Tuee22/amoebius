...
main = do
  ...
  putStrLn "test-topology-dsl-test-topology-contract: PASS (20 Phase-0 rows; deterministic single-substrate suggestion; nine resource axes; flagged credential; structured teardown; independent leak diff; derived honesty ledger)"
...
verifySuggestAndProvision = do
  ...
  assert ("test-topology-dsl-failover" `Text.isInfixOf` firstRender && "vault/test/phase54" `Text.isInfixOf` firstRender) "suggested topology omissions"
  forM_
  ...
  ascend path = do
    ...
    if found then pure path else
      let parent = takeDirectory path
       in if parent == path then die "test-topology-dsl-project-root-absent" else ascend parent
