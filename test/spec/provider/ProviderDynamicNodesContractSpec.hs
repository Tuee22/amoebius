...
joinContract = do
  ...
  assertEq "managed-instance" "i-provider-dynamic-nodes-1" (managedNodeInstanceId node)
  assertEq "managed-node-uid" "node-uid-provider-dynamic-nodes-1" (managedNodeUid node)
...
receipt = NodeGroupReceipt "snapshot-fp" "cpu-balanced" 1 "i-provider-dynamic-nodes-1"

join :: NodeJoinReadback
join = NodeJoinReadback "i-provider-dynamic-nodes-1" "node-uid-provider-dynamic-nodes-1" True True True True "scheduler-generation-47" True True 0
