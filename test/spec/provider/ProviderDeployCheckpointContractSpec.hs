...
import Amoebius.Pulumi.Backend.CheckpointEnvelope
...
checkpointContract = do
  let demand = PulumiCheckpointObjectDemand
        ...
        , checkpointStorageBudgetId = "provider-deploy-checkpoint-pulumi-checkpoint"
...
engineContract :: IO ()
engineContract = do
  context <- either (die . show) pure (admitControlPlaneDaemonContext "live-dsl-deploy-system/amoebius-control-plane" 1)
  expectLeft NoControlPlaneDaemonContext (admitControlPlaneDaemonContext "" 1) "NoControlPlaneDaemonContext"
