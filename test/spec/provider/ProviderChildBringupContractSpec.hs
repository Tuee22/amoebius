...
import Amoebius.Daemon.InClusterControlPlane
...
validateHandoff managed = do
  let parent = lease "40" (Just "parent-bootstrap")
      ...
      acquired = lease "42" (Just "child-control-plane-pod-uid")
  held <- right (handoffProviderLease managed "parent-bootstrap" "child-control-plane-pod-uid" parent released acquired)
  case handoffProviderLease managed "parent-bootstrap" "child-control-plane-pod-uid" parent (lease "40" Nothing) acquired of
    Left (ProviderLeaseHandoffError (HandoffResourceVersionStale "40")) -> pure ()
    value -> die ("HandoffResourceVersionStale:" <> show value)
  case handoffProviderLease managed "parent-bootstrap" "child-control-plane-pod-uid" parent (released {handoffLeaseUid = "changed"}) acquired of
