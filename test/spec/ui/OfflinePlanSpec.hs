...
readPlanKeyOracle = do
  rows <- drop 1 . lines <$> readFile "test/golden/offline_language_plan/plan_keys.tbl"
...
projectRoot = getCurrentDirectory >>= go
  where
    go path = do
      ...
      if found then pure path else let parent = takeDirectory path in if parent == path then die "offline-language-plan-root" else go parent
