...
runGreen root fixture = do
    ...
    putStrLn "ui-effect-binding-spec: PASS (7 ports, 2 links, 19 exact refusals, 13 coverage classes, 7 production mutants)"
...
buildAuthorizationRegistry _root = do
...
bindCaseResult fixture caseName = case caseName of
    ...
    "duplicate-handler" -> case duplicateHandlerSet fixture of
        Nothing -> die "duplicate-handler fixture is incomplete"
        Just duplicated -> pure (bindFixture fixture (ports fixture) duplicated (capabilities fixture))
    "codec-mismatch" -> do
...
checkAdditionalLinkErrors = do
    ...
    let requirement = [docs]
        ...
        expected =
            ...
            , "InsecureExternalLink"
            , "UserInfoExternalLink"
            , "WildcardExternalLink"
            , "NoncanonicalExternalLink"
            , "CallerTemplatedExternalLink"
            , "UnexpectedExternalLink"
            ]
...
coverageProperty fixture classes selected =
    checkCoverage $
        foldr
            (\coverageClass -> cover 5 (selected == coverageClass) (show coverageClass))
...
exerciseClass fixture coverageClass = case coverageClass of
    ...
    DuplicateHandlerClass -> case duplicateHandlerSet fixture of
        Nothing -> False
        Just duplicated ->
            rejected
                DuplicateHandlerTag
                (bindFixture fixture (ports fixture) duplicated (capabilities fixture))
...
duplicateHandlerSet :: Fixture -> Maybe [HandlerSpec]
duplicateHandlerSet fixture = case handlers fixture of
    [] -> Nothing
    source : _ -> do
        duplicateId <- either (const Nothing) Just (trustedHandlerId "duplicate-data-read")
        pure (handlers fixture <> [source{handlerId = duplicateId}])

parsePort :: [String] -> IO PortRequirement
parsePort row = case row of
    [portName, request, response, scope, effect] ->
        PortRequirement
...
parseHandler row = case row of
    [name, request, response, scope, retry, audit] ->
        HandlerSpec
...
parseAuthorizationAction row = case row of
    [name, effect, permission, visibility, idempotent] ->
        ActionSpec
...
loadTable path = case takeFileName path of
    "ports.tsv" -> pure portsRows
    "handlers.tsv" -> pure handlerRows
    "capabilities.tsv" -> pure capabilityRows
    "expected_bindings.tsv" -> pure expectedBindingRows
    "external_link_catalog.tsv" -> pure externalLinkRows
    "expected_external_links.tsv" -> pure expectedExternalLinkRows
    "bind_errors.tsv" -> pure bindErrorRows
    "calculus_projection.tsv" -> pure calculusRows
    name -> die ("unknown typed table identity: " <> name)
