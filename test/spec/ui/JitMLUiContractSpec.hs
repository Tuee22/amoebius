...
main = do
  ...
  request <- either (die . show) pure (requestId "jitml-ui-lift-request-001")
...
buildCommittedArtifact started input = do
  let request = ScopedTrainingRequest
        ...
        , trainingChallenge = "jitml-ui-lift-contract"
        }
...
validatePhase0 root = do
  ...
  matrix <- TextIO.readFile (root </> "test/fixture/jitml_ui_lift/readiness_owner_scope_matrix.tsv")
  ...
  contract <- TextIO.readFile (root </> "test/fixture/jitml_ui_lift/public_contract.golden")
...
signingSecret :: Text
signingSecret = "jitml-ui-lift-contract-signing-key-000000000000000000000000000000"
  ...
  ascend path = do
    ...
    if found then pure path else
      let parent = takeDirectory path
       in if parent == path then die "jitml-ui-lift-project-root-absent" else ascend parent
