...
main = do
  ...
  request <- either (die . show) pure (requestId "infernix-ui-lift-request-001")
...
validatePhase0 root = do
  ...
  contract <- TextIO.readFile (root </> "test/fixture/infernix_ui_lift/public_contract.golden")
  forM_
  ...
  scope <- TextIO.readFile (root </> "test/fixture/infernix_ui_lift/scope_matrix.tsv")
...
signingSecret :: Text
signingSecret = "infernix-ui-lift-contract-signing-key-000000000000000000000000000000"
...
modelFixture = "infernix-lift-tiny-decoder-v1|vocab=amoebius,deterministic,artifact,ready|weights=3,1,4,1,5,9"
    ...
    if found
      then pure path
        ...
        let parent = takeDirectory path
         in if parent == path then die "infernix-ui-lift-project-root-absent" else ascend parent
