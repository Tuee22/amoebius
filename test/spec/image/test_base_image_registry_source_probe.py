...
SPEC = importlib.util.spec_from_file_location("base_image_registry_source_probe", ROOT / "tools/base_image_registry_source_probe.py")
...
class SourceProbeTest(unittest.TestCase):
    def test_oracle_and_catalog_reconcile(self) -> None:
        ...
        self.assertEqual(1, len(PROBE.published_payload_oracle()))
    ...
    def test_foreign_launcher_uses_explicit_emulator_and_guest_shell(self) -> None:
        completed = subprocess.CompletedProcess([], 0, "launcher 1.2.3\n")
        emulator = Path("/project/.build/qemu-aarch64")
        with mock.patch.object(PROBE, "run", return_value=completed) as invoked:
            PROBE.run_image_command(
                "/usr/bin/docker",
                "registry.local/image@sha256:abc",
                "arm64",
                "/opt/launcher",
                ("version",),
                emulator=emulator,
        ...
        self.assertEqual(
            (
                "/usr/bin/docker", "run", "--rm", "--platform", "linux/arm64",
                "-v", f"{emulator}:/usr/bin/qemu-aarch64:ro",
                "--entrypoint", "/usr/bin/qemu-aarch64",
                "registry.local/image@sha256:abc",
                "/bin/sh", "-c", 'exec "$@"', "amoebius-probe",
                "/opt/launcher", "version",
            ),
            invoked.call_args.args[0],
        )
