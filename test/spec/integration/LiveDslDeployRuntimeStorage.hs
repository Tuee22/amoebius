...
main = do
  ...
  putStrLn "live-dsl-deploy-runtime-storage-spec: PASS (scope domains, role ownership, SplitRuntime exact fit, pinned model)"
...
source = PodRuntimeMetadataSource
  { runtimeContainerIds = Set.singleton "control-plane"
  , runtimeVolumeIds = Set.singleton "projected-token"
  , runtimeMounts = Set.singleton ("control-plane", "projected-token")
  , runtimeNetworkAttachments = Set.singleton "eth0"
  }

plannedDemand :: KubeletRuntimeMetadataDemand
plannedDemand = KubeletRuntimeMetadataDemand (PlannedExecutionSlotId "control-plane-0") "model-v1" source

observedDemand :: KubeletRuntimeMetadataDemand
observedDemand = KubeletRuntimeMetadataDemand (ObservedPodUid "pod-a" "Deployment/live-dsl-deploy-system/amoebius-control-plane") "model-v1" source

plannedScope :: RuntimeAccountingScope
plannedScope = PlannedEpochScope "epoch" (Set.singleton "planned:control-plane-0")
...
imageDemand = ProvisionedNodeImageStorageDemand Map.empty Map.empty 0 [NodeStorageComponent "image:control-plane" ImageStorage 10] 10

overlapImageDemand :: ProvisionedNodeImageStorageDemand
overlapImageDemand = ProvisionedNodeImageStorageDemand Map.empty Map.empty 0 [NodeStorageComponent "planned:control-plane-0:sandbox" ImageStorage 10] 10
