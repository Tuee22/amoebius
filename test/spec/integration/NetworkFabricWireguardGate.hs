...
main = do
  ...
  rendered <- case renderFabric inventory of
    Left GatewayEndpointMissing -> die "network-fabric-wireguard-hub-no-endpoint: gateway-role Endpoint missing from spoke config"
    Left failure -> die ("render:" <> show failure)
    Right value -> pure value
  golden <- TextIO.readFile (root </> "test/fixture/network_fabric_wireguard/expected-peer-config.golden")
  require (renderPeerConfig rendered == golden) "network-fabric-wireguard-hub-no-endpoint: rendered peer config differs from independent golden"
  ...
  expectedDemand <- decodeValue (root </> "test/fixture/network_fabric_wireguard/expected-fabric-demand.json")
  ...
  pureOnly <- (== Just "1") <$> lookupEnv "NETWORK_FABRIC_WIREGUARD_PURE_ONLY"
  unless pureOnly (verifyLiveEvidence root)
  putStrLn "network-fabric-wireguard-wireguard-live-gate: PASS (Vault-by-name keys, exact render/demand, one-shot admission, kernel observers)"
...
verifyCustody root = do
  ...
  forM_
    ...
    , "test/fixture/network_fabric_wireguard/expected-peer-config.golden"
    , "test/fixture/network_fabric_wireguard/expected-fabric-demand.json"
    , "test/fixture/network_fabric_wireguard/reachability-expected.json"
    , "test/fixture/network_fabric_wireguard/negative-expected-tags.tsv"
    , "test/fixture/network_fabric_wireguard/mutants/missing-peer-key.patch"
    , "test/fixture/network_fabric_wireguard/mutants/hub-no-endpoint.patch"
    , "test/fixture/network_fabric_wireguard/mutants/drop-resource-envelope.patch"
    , "test/fixture/network_fabric_wireguard/mutants/early-listener-replacement.patch"
...
verifyNegativeCorpus root inventory = do
  tags <- TextIO.readFile (root </> "test/fixture/network_fabric_wireguard/negative-expected-tags.tsv")
...
verifyKeys = do
  ...
  require (absent == Left VaultSecretMissing) "network-fabric-wireguard-missing-peer-key: absent SecretRef did not fail closed"
...
verifyCapacity provisioned = do
  ...
  require (authorizeEnactment "snapshot-A" Nothing == Left MissingEnactmentToken) "network-fabric-wireguard-drop-resource-envelope: missing token authorized"
...
verifyReconcile rendered _ = do
  ...
  require (replacementActions True == [ObserveOldListenerExit]) "network-fabric-wireguard-early-listener-replacement: replacement overlapped old listener"
