...
main = do
  ...
  verifyCheckpoint
  pureOnly <- (== Just "1") <$> lookupEnv "MULTICLUSTER_SPAWN_GEOREPL_PURE_ONLY"
...
verifyProjection = do
  ...
  require
    ...
    "multicluster-spawn-georepl-project-identity: child projection exposed sibling or ancestor state"
...
verifyConfluence = do
  oracleBytes <- ByteString.pack <$> readProcess "/home/matthewnowak/.local/bin/dhall-to-json"
    ["--file", "test/fixture/inject/confluence/expected_classes.dhall"] ""
  ...
  require
    ...
    "multicluster-spawn-georepl-classifier-default-confluent: unclassified invariant admitted active-active"
...
verifyIdempotence = do
  ...
  oracle <- decodeValue "test/fixture/multicluster_spawn_georepl/idempotent-write.golden.json"
...
verifyForestAdmission = do
  ...
  oracle <- decodeValue "test/fixture/multicluster_spawn_georepl/expected-forest-demand.json"
  require
    (forestDemandJson demand == oracle)
    "multicluster-spawn-georepl-drop-parallel-executor: bounded-parallel demand differs from independent oracle"
