...
main :: IO ()
main = do
  rows <- readRows "test/fixtures/phase24/substrate_decision.tsv"
  ...
  putStrLn "phase24-host-spec: PASS (18 decisions, 4 universal linux-cpu routes, 20 install rows, 9 AbsExe rows, 4 admissions, 4 reconciles, 8 inventory axes, etcd transition geometry, model mutation checks; live mutation rows are sealed by tools/phase24_gate.py)"
...
verifyHostTools = do
  ...
  planOracle <- readFile "test/fixtures/phase24/install_plans.tsv"
  let actualPlans =
  ...
  absOracle <- readFile "test/fixtures/phase24/abs_exe_cases.tsv"
...
verifyEngineAdmission = do
  rows <- lines <$> readFile "test/fixtures/phase24/engine_admission.tsv"
...
verifyReconciler :: IO ()
verifyReconciler = do
  rows <- lines <$> readFile "test/fixtures/phase24/divergent_starts.tsv"
