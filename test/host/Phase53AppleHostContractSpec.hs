...
main = do
  ...
  verifySupervisor
  verifyPeerAndAuth
  putStrLn "phase53-apple-host-contract: PASS (20 custody rows; disk/capacity fold; loopback-only peer; numerical challenge; build/lifecycle/supervisor/auth; universal linux-cpu and Incus/Lima/WSL2 routing)"
...
validateBoundaryOracle = do
  rows <- lines <$> readFile "test/golden/phase_53/vm_disk_boundaries.csv"
...
verifyReferenceAndBridge = do
  inputA <- parseFloats "test/golden/phase_53/job_A.input"
  inputB <- parseFloats "test/golden/phase_53/job_B.input"
  expectedA <- readExpected "test/golden/phase_53/job_A.expected"
  expectedB <- readExpected "test/golden/phase_53/job_B.expected"
  ...
  ascend path = do
    ...
    if present then pure path else
      let parent = takeDirectory path
       in if parent == path then die "phase53-project-root-absent" else ascend parent
