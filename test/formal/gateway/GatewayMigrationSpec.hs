...
main = do
  ...
  let output = root </> "gen/tla/gateway-migration-model-spec"
...
checkContract root = do
  rows <- readTsv (root </> "test/formal/gateway/oracle/model_contract.tsv")
...
checkGolden root = do
  expectedTla <- readFile (root </> "test/formal/gateway/golden/GatewayMigration.tla.golden")
  expectedCfg <- readFile (root </> "test/formal/gateway/golden/GatewayMigration.cfg.golden")
...
checkMutantOracle root mutants = do
  rows <- readTsv (root </> "test/formal/gateway/oracle/invariant_mutants.tsv")
...
readCutoffCases root = do
  rows <- readTsv (root </> "test/formal/gateway/oracle/cutoff_cases.tsv")
...
checkCutoffMutants root cases = do
  rows <- readTsv (root </> "test/formal/gateway/oracle/cutoff_mutants.tsv")
