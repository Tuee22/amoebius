...
WORKDIR /src
COPY . /src
RUN --mount=type=cache,id=amoebius-phase25-arm64-cabal,target=/root/.cache/cabal \
    --mount=type=cache,id=amoebius-phase25-arm64-store,target=/root/.local/state/cabal \
    --mount=type=cache,id=amoebius-phase25-arm64-dist,target=/src/dist-newstyle \
