...
#ifdef PHASE45_PUBLIC_PULL_MUTANT
...
data ChildLeaseHeld = ChildLeaseHeld ManagedProviderChild SingletonLease
  deriving stock (Eq, Show)
...
handoffProviderLease managed parentHolder childPodUid parent released acquired = do
  ...
  singletonLease <- handoff (acquireSingleton childPodUid releasedLease acquired)
  pure (ChildLeaseHeld managed singletonLease)
 where
