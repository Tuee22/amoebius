  ...
  case arguments of
    ["--worker", root, statePath, namespaceName, worker, rawPort, challengeValue] ->
      case readMaybe rawPort of
        Nothing -> fail "phase37-worker-port"
        Just port -> runWorker root statePath (Text.pack namespaceName) (Text.pack worker) port (Text.pack challengeValue)
    [] -> runGate
    _ -> fail ("phase37-arguments:" <> show arguments)

runGate :: IO ()
runGate = do
  root <- projectRoot
  pureChecks root
  pureOnly <- lookupEnv "PHASE37_PURE_ONLY"
  reuse <- lookupEnv "PHASE37_REUSE_FRESH_LIVE"
  if pureOnly == Just "1"
    then pure ()
...
pureChecks root = do
  ...
  expectedSha <- Text.strip <$> TextIO.readFile (root </> "amoebius-store/test/golden/manifest_canonical.sha256")
  assertEqual "canonical-manifest-sha" expectedSha (digestHex (manifestContentDigest first))
  oracle <- helperJson root [root </> "tools/phase37_oracle.py"]
  case oracle of
    Object value -> do
    ...
    _ -> fail "phase37-independent-oracle-not-object"
  noncanonical <- readHexCustody (root </> "amoebius-store/test/golden/manifest_noncanonical.cbor")
  ...
  budgetRows <- loadBudgetRows (root </> "amoebius-store/test/golden/write_budget_boundaries.csv")
  mapM_ checkBudgetRow budgetRows
  pointerChecks first
  terminalChecks
  resourceChecks
  runtimeChecks
  ranks <- filter (not . Text.null) . map Text.strip . Text.lines <$> TextIO.readFile (root </> "amoebius-runtime/test/golden/failover_rank.txt")
  ...
  topology <- TextIO.readFile (root </> "amoebius-runtime/dhall/test/round_trip_failover.dhall")
...
expectedManifest = do
  root <- projectRoot
  readHexCustody (root </> "amoebius-store/test/golden/manifest_canonical.cbor")

pointerChecks :: Manifest -> IO ()
pointerChecks value = do
  let headValue = pointerHead (manifestContentDigest value)
      oldValue = pointerHead (contentDigest "phase37-old")
...
checkBudgetRow row = do
  let demand = ObjectStoreDemand
        (StorageBudgetId "phase37-content")
...
runLive :: FilePath -> IO ()
runLive root = do
  setupValue <- helperJson root [root </> "tools/phase37_workflow_live.py", "setup"]
  setup <- case fromJSON setupValue of
    Error problem -> fail ("phase37-setup-json:" <> problem)
    Success value -> pure value
  let resultPath = "/tmp/amoebius-phase37-result-" <> Text.unpack (challenge setup) <> ".json"
      cleanup = callHelper root [root </> "tools/phase37_workflow_live.py", "cleanup", "--state", stateFile setup]
  (do
      ...
      callHelper root [root </> "tools/phase37_workflow_live.py", "finish", "--state", stateFile setup, "--result", resultPath]
    ) `finally` cleanup

runRound :: FilePath -> LiveSetup -> Text -> Int -> IO RoundResult
runRound root setup namespaceName port = do
  brokerPod <- maybe (fail ("phase37-broker-owner:" <> Text.unpack namespaceName)) pure (Map.lookup namespaceName (brokerPods setup))
...
workerPriority name = error ("phase37-worker-priority-domain:" <> Text.unpack name)
...
storeThroughGateway root statePath namespaceName = do
  ...
  helper <- helperJson root
    [ root </> "tools/phase37_workflow_live.py", "store", "--state", statePath
  ...
  case fromJSON helper of
    Error problem -> fail ("phase37-store-json:" <> problem)
...
observeBroker root setup namespaceName = do
  value <- helperJson root
    [ root </> "tools/phase37_workflow_live.py", "observe", "--state", stateFile setup
    , "--namespace", Text.unpack namespaceName
    ]
  case fromJSON value of
    Error problem -> fail ("phase37-observe-json:" <> problem)
    Success result -> pure result
...
receiveWithin consumer = do
  ...
  case result of
    Nothing -> fail "phase37-pulsar-message-timeout"
    Just (Left problem) -> fail ("phase37-pulsar-CBOR:" <> show problem)
...
receiveWorker consumer = bracket startTicker killThread $ \_ticker -> do
  ...
  case result of
    Left problem -> fail ("phase37-worker-pulsar-CBOR:" <> show problem)
...
keepConsumerAlive consumer = forever $ do
  result <- Consumer.receive consumer
  case result of
    Left problem -> fail ("phase37-worker-keepalive-CBOR:" <> show problem)
...
validateFreshEvidence root = do
  ...
  unless present (fail "phase37-live-evidence-missing")
  decoded <- eitherDecode <$> LazyByteString.readFile path
  case decoded of
    Left problem -> fail ("phase37-evidence-json:" <> problem)
    Right (Object value) -> do
      assertEqual "phase37-evidence-sealed" (Just (Bool True)) (KeyMap.lookup "sealed" value)
      assertEqual "phase37-evidence-register" (Just (Number 3)) (KeyMap.lookup "register" value)
      assertEqual "phase37-evidence-substrate" (Just (String "linux-cpu")) (KeyMap.lookup "substrate" value)
    Right _ -> fail "phase37-evidence-not-object"
...
helperJson _root arguments = do
  ...
  unless (exitCode == ExitSuccess) (fail ("phase37-helper:" <> show arguments <> ":" <> output <> errors))
  case eitherDecodeStrict' (TextEncoding.encodeUtf8 (Text.strip (Text.pack output))) of
    Left problem -> fail ("phase37-helper-json:" <> problem <> ":" <> output)
    Right value -> pure value

callHelper :: FilePath -> [String] -> IO ()
callHelper _ arguments = do
  (exitCode, output, errors) <- readProcessWithExitCode "/usr/bin/python3" arguments ""
  unless (exitCode == ExitSuccess) (fail ("phase37-helper-call:" <> show arguments <> ":" <> output <> errors))
